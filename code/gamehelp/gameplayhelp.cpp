...
struct gameplay_help_buttons
{
  ...
  UI_BUTTON button;             // because we have a class inside this struct, we need the constructor below..
    ...
    gameplay_help_buttons(char *name, int x1, int y1, int h):filename(name),
    x(x1), y(y1), hotspot(h)
  {
  }
};
...
void gameplay_help_blit_control_line_raw(int x, int y, char *control_text,
  ...
  switch (Current_help_page)
    ...
    case GP_HELP_BASIC_KEYS:
      ...
      gameplay_help_blit_control_line_raw(x_offset, y_offset,
                                          XSTR("Shift-Esc", 145),
                                          XSTR
                                          ("quit Wing Commander Saga immediately",
                                           146));
