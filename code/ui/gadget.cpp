...
//
void UI_GADGET::base_create(UI_WINDOW * wnd, int _kind, int _x, int _y,
                            int _w, int _h)
{
  int i;
  ...
  if (!base_dragging)
  ...
  else
    {
      ...
      nprintf(("UI", "UI: X=%d, Y=%d, Delta=(%d,%d)\n", x, y,
               (ui_mouse.x - base_drag_x), (ui_mouse.y - base_drag_y)));
      if (B2_RELEASED)
        {
