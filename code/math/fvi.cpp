...
//
void fvi_two_lines_in_3space(vec3d * p1, vec3d * v1, vec3d * p2, vec3d * v2,
                             float *s, float *t)
{
  vec3d cross, delta;
...
float fvi_ray_plane(vec3d * new_pnt, vec3d * plane_pnt, vec3d * plane_norm,     // Plane description, a point and a normal
...
int fvi_segment_plane(vec3d * new_pnt, vec3d * plane_pnt, vec3d * plane_norm,
                      vec3d * p0, vec3d * p1, float rad)
{
  ...
  t = fvi_ray_plane(new_pnt, plane_pnt, plane_norm,     // Plane description, a point and a normal
...
//else returns 0
int fvi_segment_sphere(vec3d * intp, vec3d * p0, vec3d * p1,
                       vec3d * sphere_pos, float sphere_rad)
{
...
//returns 1 if intersects, and fills in intp
//else returns 0
int fvi_ray_sphere(vec3d * intp, vec3d * p0, vec3d * p1, vec3d * sphere_pos,
                   float sphere_rad)
{
  vec3d d, dn, w, closest_point;
  float mag_d, dist, w_dist, int_dist;
...
int fvi_ray_boundingbox(vec3d * min, vec3d * max, vec3d * p0, vec3d * pdir,
                        vec3d * hitpt)
{
  bool inside = true;
  bool middle[3] = {
...
int fvi_point_face(vec3d * checkp, int nv, vec3d ** verts, vec3d * norm1,
...
int check_sphere_point(vec3d * point, vec3d * sphere_start,
                       vec3d * sphere_vel, float radius, float *collide_time);
...
int fvi_sphere_plane(vec3d * intersect_point, vec3d * sphere_center_start,
                     vec3d * sphere_velocity, float sphere_radius,
                     vec3d * plane_normal, vec3d * plane_point,
                     float *hit_time, float *crossing_time)
{
...
//
int fvi_sphere_perp_edge(vec3d * intersect_point, vec3d * sphere_center_start,
                         vec3d * sphere_velocity, float sphere_radius,
                         vec3d * edge_point1, vec3d * edge_point2,
                         float *collide_time)
{
  ...
  return (check_sphere_point
          (intersect_point, sphere_center_start, sphere_velocity,
           sphere_radius, collide_time));
}
                       ...
                       vec3d * sphere_vel, float radius, float *collide_time)
{
  ...
  float discriminant =
    delta_x_dot_vs * delta_x_dot_vs - vs_sqr * (delta_x_sqr -
                                                radius * radius);
  if (discriminant < 0)
    {
      return 0;
...
int fvi_polyedge_sphereline(vec3d * hit_point, vec3d * xs0, vec3d * vs,
                            float Rs, int nv, vec3d ** verts, float *hit_time)
{
  int i;
  vec3d v0, v1;
  vec3d ve;                     // edge velocity
  ...
  for (i = 0; i < nv; i++)
    {
      ...
      C =
        delta_x_dot_ve * delta_x_dot_ve + Rs * Rs * ve_sqr -
        delta_x_sqr * ve_sqr;
      ...
      C =
        2 * delta_x_dot_ve * delta_x_dot_vs * ve_dot_vs +
      ...
      if ((root1 >= 0) && (root1 <= 1))
        {
          ...
          if (fl_abs(q - Rs * Rs) < 0.2 * Rs)
            {
              // error less than 0.1m absolute (2*delta*Radius)
              goto Hit;
            }
        }
...
//
void fvi_closest_point_on_line_segment(vec3d * closest_point,
                                       vec3d * fixed_point,
                                       vec3d * line_point1,
                                       vec3d * line_point2)
{
  ...
  t =
    -vm_vec_dotprod(&delta_x,
                    &line_velocity) / vm_vec_mag_squared(&line_velocity);
...
//
int fvi_check_sphere_sphere(vec3d * x_p0, vec3d * x_p1, vec3d * x_s0,
                            vec3d * x_s1, float radius_p, float radius_s,
                            float *t1, float *t2)
{
  ...
  discriminant =
    delta_x_dot_delta_v * delta_x_dot_delta_v - delta_v_sqr * (delta_x_sqr -
                                                               separation *
                                                               separation);
...
int fvi_cull_polyface_sphere(vec3d * poly_center, float poly_r,
                             vec3d * sphere_start, vec3d * sphere_end,
                             float sphere_r)
{
...
//
void fvi_closest_line_line(vec3d * x0, vec3d * vx, vec3d * y0, vec3d * vy,
...
int project_point_onto_bbox(vec3d * mins, vec3d * maxs, vec3d * start,
                            vec3d * box_pt)
{
  int inside = TRUE;
