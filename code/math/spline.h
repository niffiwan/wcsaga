...
class herm_spline
{
  public:vec3d pts[MAX_HERM_PTS];       // control points
  vec3d d_pts[MAX_HERM_PTS];    // derivative of control points (think of as velocity)
  int num_pts;
    public:
    // constructor
    herm_spline();
    herm_spline(int _num_pts, vec3d * _pts[MAX_HERM_PTS],
                vec3d * _d_pts[MAX_HERM_PTS]);

  // set the points
  void herm_set_points(int _num_pts, vec3d * _pts[MAX_HERM_PTS],
                       vec3d * _d_pts[MAX_HERM_PTS]);
