...
//dest can equal any source
  vec3d * vm_vec_avg3(vec3d * dest, vec3d * src0, vec3d * src1,
                      vec3d * src2) 
{
  ...
  vec3d * vm_vec_rotate(vec3d * dest, vec3d * src, matrix * m) 
{
    ...
    (src->xyz.x * m->vec.rvec.xyz.x) + (src->xyz.y * m->vec.rvec.xyz.y) +
  ...
  vec3d * vm_vec_unrotate(vec3d * dest, vec3d * src, matrix * m) 
{
    ...
    (src->xyz.x * m->vec.rvec.xyz.x) + (src->xyz.y * m->vec.uvec.xyz.x) +
...
// --------------------------------------------------------------------------------------
 void get_camera_limits(matrix * start_camera, matrix * end_camera,
                                ...
                                int no_overshoot) 
{
  ...
  float delta_bank;
    ...
    
    {
        ...
        //
        vm_vec_crossprod(&rot_axis, &orient->vec.rvec,
                         &goal_orient->vec.rvec);
       t = vm_vec_mag(&rot_axis);
      if (t > 1.0f)
        t = 1.0f;
       r_dotprod =
        vm_vec_dotprod(&orient->vec.rvec, &goal_orient->vec.rvec);
...
void vm_vec_interp_constant(vec3d * out, vec3d * v0, vec3d * v1, float t) 
{
    ...
    vm_rot_point_around_line(out, v0, t * total_ang, &vmd_zero_vector,
                             &cross);
