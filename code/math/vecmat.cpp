...
//
float vm_vec_projection_parallel(vec3d * component, vec3d * src,
                                 vec3d * unit_vec)
{
  float mag;
...
//
void vm_vec_projection_onto_plane(vec3d * projection, vec3d * src,
                                  vec3d * unit_normal)
{
  float mag;
...
//
void vm_project_point_onto_plane(vec3d * new_point, vec3d * point,
                                 vec3d * plane_normal, vec3d * plane_point)
{
...
//averages four vectors. returns ptr to dest
//dest can equal any source
vec3d *vm_vec_avg4(vec3d * dest, vec3d * src0, vec3d * src1, vec3d * src2,
                   vec3d * src3)
{
  dest->xyz.x =
...
float vm_vec_imag(vec3d * v)
{
  ...
  return fl_isqrt((v->xyz.x * v->xyz.x) + (v->xyz.y * v->xyz.y) +
                  (v->xyz.z * v->xyz.z));
}
...
//dest = normalized(end - start).  Returns mag of direction vector
//NOTE: the order of the parameters matches the vector subtraction
float vm_vec_normalized_dir_quick_mag(vec3d * dest, vec3d * end,
                                      vec3d * start)
{
  float t;
  vm_vec_sub(dest, end, start);
...
matrix *sincos_2_matrix(matrix * m, float sinp, float cosp, float sinb,
                        float cosb, float sinh, float cosh)
{
...
matrix *vm_vector_2_matrix(matrix * m, vec3d * fvec, vec3d * uvec,
                           vec3d * rvec)
{
...
matrix *vm_vector_2_matrix_norm(matrix * m, vec3d * fvec, vec3d * uvec,
                                vec3d * rvec)
{
...
vec3d *vm_vec_rotate(vec3d * dest, vec3d * src, matrix * m)
{
  ...
  dest->xyz.y =
    (src->xyz.x * m->vec.uvec.xyz.x) + (src->xyz.y * m->vec.uvec.xyz.y) +
    (src->xyz.z * m->vec.uvec.xyz.z);
  dest->xyz.z =
    (src->xyz.x * m->vec.fvec.xyz.x) + (src->xyz.y * m->vec.fvec.xyz.y) +
...
vec3d *vm_vec_unrotate(vec3d * dest, vec3d * src, matrix * m)
{
  ...
  dest->xyz.y =
    (src->xyz.x * m->vec.rvec.xyz.y) + (src->xyz.y * m->vec.uvec.xyz.y) +
    (src->xyz.z * m->vec.fvec.xyz.y);
  dest->xyz.z =
    (src->xyz.x * m->vec.rvec.xyz.z) + (src->xyz.y * m->vec.uvec.xyz.z) +
    (src->xyz.z * m->vec.fvec.xyz.z);
...
matrix *vm_matrix_x_matrix(matrix * dest, matrix * src0, matrix * src1)
{
  ...
  dest->vec.rvec.xyz.x =
    vm_vec_dot3(src0->vec.rvec.xyz.x, src0->vec.uvec.xyz.x,
  ...
  dest->vec.rvec.xyz.y =
    vm_vec_dot3(src0->vec.rvec.xyz.y, src0->vec.uvec.xyz.y,
  ...
  dest->vec.rvec.xyz.z =
    vm_vec_dot3(src0->vec.rvec.xyz.z, src0->vec.uvec.xyz.z,
...
float find_nearest_point_on_line(vec3d * nearest_point, vec3d * p0,
                                 vec3d * p1, vec3d * int_pnt)
{
...
void compute_point_on_plane(vec3d * q, plane * planep, vec3d * p)
{
  ...
  tv =
    planep->A * q->xyz.x + planep->B * q->xyz.y + planep->C * q->xyz.z +
    planep->D;
}
...
void vm_rot_point_around_line(vec3d * out, vec3d * in, float angle,
                              vec3d * line_point, vec3d * line_dir)
{
  vec3d tmp, tmp1;
  matrix m, r;
  angles ta;
...
void vm_matrix_to_rot_axis_and_angle(matrix * m, float *theta,
                                     vec3d * rot_axis)
{
  ...
  if (cos_theta > 0.999999875f)
    {
      // angle is less than 1 milirad (0.057 degrees)
      *theta = 0.0f;
    ...
    {
      // angle is within limits between 0 and PI
...
float approach(float w_in, float w_max, float theta_goal, float aa,
      ...
      if (delta_theta > theta_goal)
        {
          ...
          delta_theta -=
            away(-w_in, w_max, 0.0f, aa, delta_t - t_goal, w_out,
                 no_overshoot);
          *w_out = -*w_out;
          return delta_theta;
        }
...
void vm_matrix_interpolate(matrix * goal_orient, matrix * curr_orient,
                           vec3d * w_in, float delta_t, matrix * next_orient,
                           vec3d * w_out, vec3d * vel_limit,
                           vec3d * acc_limit, int no_overshoot)
{
  ...
  if (theta_goal.xyz.x > 0)
    {
      if (w_in->xyz.x >= 0)
        {
          delta_theta =
            approach(w_in->xyz.x, vel_limit->xyz.x, theta_goal.xyz.x,
      ...
      else
        {
          // w_in->xyz.x < 0
          delta_theta =
            away(w_in->xyz.x, vel_limit->xyz.x, theta_goal.xyz.x,
                 acc_limit->xyz.x, delta_t, &w_out->xyz.x, no_overshoot);
          theta_end.xyz.x = delta_theta;
        }
    }
  ...
  if (theta_goal.xyz.y > 0)
    {
      if (w_in->xyz.y >= 0)
        {
          delta_theta =
            approach(w_in->xyz.y, vel_limit->xyz.y, theta_goal.xyz.y,
      ...
      else
        {
          // w_in->xyz.y < 0
          delta_theta =
            away(w_in->xyz.y, vel_limit->xyz.y, theta_goal.xyz.y,
                 acc_limit->xyz.y, delta_t, &w_out->xyz.y, no_overshoot);
          theta_end.xyz.y = delta_theta;
        }
    }
  ...
  if (theta_goal.xyz.z > 0)
    {
      if (w_in->xyz.z >= 0)
        {
          delta_theta =
            approach(w_in->xyz.z, vel_limit->xyz.z, theta_goal.xyz.z,
      ...
      else
        {
          // w_in->xyz.z < 0
          delta_theta =
            away(w_in->xyz.z, vel_limit->xyz.z, theta_goal.xyz.z,
                 acc_limit->xyz.z, delta_t, &w_out->xyz.z, no_overshoot);
          theta_end.xyz.z = delta_theta;
        }
    }
...
void get_camera_limits(matrix * start_camera, matrix * end_camera, float time,
...
void vm_fvec_matrix_interpolate(matrix * goal_orient, matrix * orient,
                                vec3d * w_in, float delta_t,
                                matrix * next_orient, vec3d * w_out,
                                vec3d * vel_limit, vec3d * acc_limit,
                                int no_overshoot)
{
  ...
  if (no_bank)
    {
      // no rotation on z, so we're done (no rotation of w)
      ...
      if (t < SMALLER_NUM)
        {
          if (r_dotprod > 0.0f)
            ...
            {
              // the right vector is pointing exactly opposite of goal, so rotate 180 on z
              theta = PI;
      ...
      if (delta_bank > 0)
        {
          if (w_in->xyz.z >= 0)
            {
              delta_theta =
                approach(w_in->xyz.z, vel_limit->xyz.z, delta_bank,
                         acc_limit->xyz.z, delta_t, &w_out->xyz.z,
                         no_overshoot);
              bank = delta_theta;
            }
          else
            {
              // w_in->xyz.z < 0
              delta_theta =
                away(w_in->xyz.z, vel_limit->xyz.z, delta_bank,
                     acc_limit->xyz.z, delta_t, &w_out->xyz.z, no_overshoot);
              bank = delta_theta;
            }
        }
...
//      momentum (velocity)  Attempt to try to move bank by goal delta_bank.  Rotational velocity
//      on x/y is rotated with bank, giving smoother motion.
void vm_forward_interpolate(vec3d * goal_f, matrix * orient, vec3d * w_in,
...
void vm_find_bounding_sphere(vec3d * pnts, int num_pnts, vec3d * center,
                             float *radius)
{
  int i;
...
//
vec3d *vm_rotate_vec_to_body(vec3d * body_vec, vec3d * world_vec,
                             matrix * orient)
{
...
//              orient      =>      orientation matrix
//
vec3d *vm_rotate_vec_to_world(vec3d * world_vec, vec3d * body_vec,
                              matrix * orient)
{
...
//
void vm_estimate_next_orientation(matrix * last_orient,
...
void vm_vec_random_cone(vec3d * out, vec3d * in, float max_angle,
                        matrix * orient)
{
  vec3d t1, t2;
  matrix *rot;
...
void vm_vec_random_cone(vec3d * out, vec3d * in, float min_angle,
                        float max_angle, matrix * orient)
{
  vec3d t1, t2;
  matrix *rot;
  matrix m;
  ...
  vm_rot_point_around_line(&t1, in, fl_radian(temp_ang), &vmd_zero_vector,
                           &rot->vec.fvec);
  ...
  vm_rot_point_around_line(&t2, &t1, fl_radian(temp_ang), &vmd_zero_vector,
                           &rot->vec.rvec);
  ...
  vm_rot_point_around_line(out, &t2, fl_radian(temp_ang), &vmd_zero_vector,
                           &rot->vec.uvec);
}
...
void vm_vec_random_in_circle(vec3d * out, vec3d * in, matrix * orient,
                             float radius, int on_edge)
{
  ...
  vm_rot_point_around_line(out, &temp, fl_radian(frand_range(0.0f, 359.0f)),
                           in, &orient->vec.fvec);
}
...
int vm_vec_dist_to_line(vec3d * p, vec3d * l0, vec3d * l1, vec3d * nearest,
                        float *dist)
{
  vec3d a, b, c;
  float b_mag, comp;
...
void vm_vec_dist_squared_to_line(vec3d * p, vec3d * l0, vec3d * l1,
                                 vec3d * nearest, float *dist_squared)
{
  vec3d a, b, c;
  float b_mag, comp;
