  ...
  for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
       so = GET_NEXT(so))
    {
      ...
      if (weapon_area_calc_damage
          (ship_objp, &asteroid_objp->pos, asip->inner_rad, asip->outer_rad,
           asip->blast, asip->damage, &blast, &damage, asip->outer_rad) == -1)
        continue;
          ...
          if (wip->impact_weapon_expl_index < 0)
            {
              particle_create(hitpos, &vmd_zero_vector, 0.0f,
                              ...
                              PARTICLE_BITMAP, Asteroid_impact_explosion_ani);
            }
        }
    }
                           ...
                           mc_info * mc, bool lazy = false)
{
  ...
  asteroid_ray_dist =
    ...
    ASTEROID_MIN_COLLIDE_TIME;
  distance =
    vm_vec_dist_quick(&asteroid_obj->pos,
                      &ship_obj->pos) - ship_obj->radius -
    asteroid_obj->radius;
  ...
  if (mc.num_hits)
    {
      float total_dist =
        vm_vec_dist(&mc.hit_point_world,
                    &asteroid_obj->pos) - asteroid_obj->radius;
      if (total_dist < 0)
        {
          total_dist = 0.0f;
        }
      ...
      for (i = Species_info.size(); i >= 0; i--)
        {
          ...
          if ((i == 0)
              || stristr(new_asteroid.name, Species_info[i - 1].species_name))
            {
