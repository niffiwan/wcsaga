...
int asteroid_in_inner_bound_with_axes(asteroid_field * asfieldp, vec3d * pos,
                                      float delta) 
{
...
int asteroid_in_inner_bound(asteroid_field * asfieldp, vec3d * pos,
                            float delta) 
{
                           ...
                           int asteroid_subtype) 
{
    ...
    obj_create(OBJ_ASTEROID, -1, n, &orient, &pos, radius,
    ...
    
    {
        ...
        asteroid_cap_speed(asteroid_type,
                           asfieldp->speed * frand_range(0.5f +
                                                         (float)
                                                         Game_skill_level /
                                                         NUM_SKILL_LEVELS,
                                                         2.0f +
    ...
    
    {
        ...
        asteroid_cap_speed(asteroid_type,
                           asfieldp->speed * static_randf_range(rand_base++,
                                                                0.5f +
                                                                (float)
                                                                Game_skill_level
                                                                /
    ...
    asip->initial_asteroid_strength * (0.8f +
                                       (float) Game_skill_level /
...
void asteroid_sub_create(object * parent_objp, int asteroid_type,
                         vec3d * relvec) 
{
    ...
    
    {
      ...
      asteroid_cap_speed(asteroid_type, (frand() + 2.0f) * parent_speed);
  
  else
    speed =
      asteroid_cap_speed(asteroid_type,
                         (static_randf(new_objp->net_signature) +
                          2.0f) * parent_speed);
...
void asteroid_load(int asteroid_info_index, int asteroid_subtype) 
{
    ...
    
    {
        ...
        {
          asip->modelp[asteroid_subtype]->detail_depth[i] =
...
int get_debris_from_same_group(int index) 
{
    ...
    return group_base +
    ((group_offset + rand() % (NUM_DEBRIS_SIZES - 1) + 1) % NUM_DEBRIS_SIZES);
...
int asteroid_should_wrap(object * objp, asteroid_field * asfieldp) 
{
    ...
    
    {
           ...
           && (objp->pos.xyz.x <
               asfieldp->inner_max_bound.xyz.x)  &&(objp->pos.xyz.y >
                                                     asfieldp->
                                                     inner_min_bound.xyz.y)
           && (objp->pos.xyz.y <
               asfieldp->inner_max_bound.xyz.y)  &&(objp->pos.xyz.z >
                                                     asfieldp->
                                                     inner_min_bound.xyz.z)
...
void asteroid_wrap_pos(object * objp, asteroid_field * asfieldp) 
{
    ...
    
    {
        ...
        asfieldp->max_bound.xyz.x + (objp->pos.xyz.x -
                                     asfieldp->min_bound.xyz.x);
    ...
    
    {
        ...
        asfieldp->max_bound.xyz.y + (objp->pos.xyz.y -
                                     asfieldp->min_bound.xyz.y);
    ...
    
    {
        ...
        asfieldp->max_bound.xyz.z + (objp->pos.xyz.z -
                                     asfieldp->min_bound.xyz.z);
    ...
    
    {
        ...
        asfieldp->min_bound.xyz.x + (objp->pos.xyz.x -
                                     asfieldp->max_bound.xyz.x);
    }
   if (objp->pos.xyz.y > asfieldp->max_bound.xyz.y)
    
    {
      objp->pos.xyz.y =
        asfieldp->min_bound.xyz.y + (objp->pos.xyz.y -
                                     asfieldp->max_bound.xyz.y);
    }
   if (objp->pos.xyz.z > asfieldp->max_bound.xyz.z)
    
    {
      objp->pos.xyz.z =
        asfieldp->min_bound.xyz.z + (objp->pos.xyz.z -
                                     asfieldp->max_bound.xyz.z);
...
void asteroid_aim_at_target(object * objp, object * asteroid_objp,
                            float delta_time) 
{
            ...
            
            {
                ...
                
                {
                    ...
                    
                    {
                      ...
                      vm_vec_copy_scale(&objp->phys_info.vel, &old_vel,
                                         -1.0f);
...
}  void asteroid_process_pre(object * objp, float frame_time) 
{
    ...
    
    {
        ...
        lerp(&objp->phys_info.vel.xyz.x, objp->phys_info.vel.xyz.x,
             objp->phys_info.desired_vel.xyz.x, flFrametime);
      lerp(&objp->phys_info.vel.xyz.y, objp->phys_info.vel.xyz.y,
            objp->phys_info.desired_vel.xyz.y, flFrametime);
      lerp(&objp->phys_info.vel.xyz.z, objp->phys_info.vel.xyz.z,
            objp->phys_info.desired_vel.xyz.z, flFrametime);
    }
}

 int asteroid_check_collision(object * pasteroid, object * other_obj,
                                vec3d * hitpos,
                                collision_info_struct * asteroid_hit_info) 
{
  ...
  mc.p1 = &p1;                  // Point 2 of ray to check
    ...
    
    {
      ...
      vm_vec_sub(&asteroid_hit_info->light_rel_vel, &p1, &p0_rotated);
        ...
        
        {
          ...
          vm_vec_sub(&asteroid_hit_info->light_rel_vel,
                      &light->phys_info.vel, &heavy->phys_info.vel);
      ...
      vec3d copy_p0, copy_p1;
        ...
        
        {
            ...
            
            {
                ...
                pm =
                model_get(Ship_info
                          [Ships[heavy_obj->instance].ship_info_index].
                          model_num);
                ...
                
                {
                    ...
                    
                    {
                        ...
                        
                        {
                            ...
                            {
                              model_find_obj_dir(&asteroid_hit_info->
                                                  collision_normal,
                            ...
                            vec3d int_light_pos, diff;
                                            ...
                                            mc.hit_dist);
                          model_find_world_point(&asteroid_hit_info->
            ...
            
            {
                ...
                
                {
                  ...
                  vm_vec_scale_add(&asteroid_hit_info->
                                    light_collision_cm_pos, mc.p0, &diff,
                                    mc.hit_dist);
    ...
    
    {
      ...
      mc.radius =
        model_get_core_radius(Ship_info
                              [Ships[ship_obj->instance].ship_info_index].
                              model_num);
...
float asteroid_create_explosion(object * objp) 
{
    ...
    fireball_create(&objp->pos, FIREBALL_ASTEROID, FIREBALL_LARGE_EXPLOSION,
                    OBJ_INDEX(objp), objp->radius * fireball_scale_multiplier,
...
void asteriod_explode_sound(object * objp, int type, int play_loud) 
{
  ...
   snd_play_3d(&Snds[sound_index], &objp->pos, &Eye_position, objp->radius,
                 NULL, 0, 1.0f, SND_PRIORITY_MUST_PLAY, NULL, range_factor);
...
void asteroid_do_area_effect(object * asteroid_objp) 
{
    ...
    
    {
      ...
       ship_apply_global_damage(ship_objp, asteroid_objp,
                                  &asteroid_objp->pos, damage);
      weapon_area_apply_blast(NULL, ship_objp, &asteroid_objp->pos, blast,
                               0);
...
void asteroid_hit(object * asteroid_obj, object * other_obj, vec3d * hitpos,
                  float damage) 
{
    ...
    
    {
        ...
        
        {
          ...
          asteriod_explode_sound(asteroid_obj, asp->asteroid_type,
                                  play_loud_collision);
...
void asteroid_level_close() 
{
    ...
    
    {
        ...
        
        {
          ...
          Assert(Asteroids[i].objnum >= 0
                  && Asteroids[i].objnum < MAX_OBJECTS);
            ...
            
            {
                ...
                
                {
                    ...
                    
                    {
                      ...
                      asc_get_relvec(&relvec, asteroid_obj,
                                      &asp->death_hit_pos);
                      asteroid_sub_create(asteroid_obj, ASTEROID_TYPE_SMALL,
                                           &relvec);
                       vm_vec_normalized_dir(&vfh, &asteroid_obj->pos,
                                               &asp->death_hit_pos);
                                      ...
                                      &asp->death_hit_pos);
                      asteroid_sub_create(asteroid_obj, ASTEROID_TYPE_MEDIUM,
                                           &relvec);
                        ...
                        
                        {
                          ...
                          asteroid_sub_create(asteroid_obj,
                                               ASTEROID_TYPE_SMALL, &tvec2);
...
 int asteroid_get_random_in_cone(vec3d * pos, vec3d * dir, float ang,
                                   int danger) 
{
...
 void asteroid_test_collide(object * asteroid_obj, object * ship_obj,
                              mc_info * mc, bool lazy = false) 
{
    ...
    asteroid_ray_dist =
    vm_vec_mag_quick(&asteroid_obj->phys_info.desired_vel) *
    ASTEROID_MIN_COLLIDE_TIME;
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                asteroid_will_collide(asteroid_objp, &Objects[escort_objnum]);
...
float asteroid_time_to_impact(object * asteroid_objp) 
{
    ...
    
    {
        ...
        vm_vec_dist(&mc.hit_point_world,
                    &asteroid_objp->pos) - asteroid_objp->radius;
...
void asteroid_parse_section(asteroid_info * asip) 
{
  ...
  asip->num_detail_levels =
    stuff_int_list(asip->detail_distance, MAX_ASTEROID_DETAIL_LEVELS,
                   RAW_INTEGER_TYPE);
...
void asteroid_parse_tbl() 
{
    ...
    
    {
        ...
        
        {
            ...
            if ((i == 0)
                || stristr(new_asteroid.name,
                           Species_info[i - 1].species_name))
  ...
  char *errormsg = new char[75 + (Species_info.size() * (NAME_LENGTH))];
          ...
          "The following species are missing debris types in asteroids.tbl:\n");
                     ...
                     Asteroid_field.max_bound.xyz.x)
                       ...
                       Asteroid_field.max_bound.xyz.y)
                         ...
                         Asteroid_field.max_bound.xyz.z)
                           ...
                           (&Asteroid_field, &ship_objp->pos, radius))
...
void asteroid_show_brackets() 
{
    ...
    
    {
        ...
        
        {
            ...
            {
              hud_show_lead_indicator_quick(&asteroid_objp->pos,
                                             asteroid_objp);
