...
struct brief_common_buttons 
{
 ...
  brief_common_buttons(char *name, int x1, int y1, int xt1, int yt1, int h, int r = 0):filename(name), x(x1), y(y1), xt(xt1), yt(yt1), hotspot(h),
    repeat
    (r)
  {
} };
  ...
  int ship_pool[MAX_SHIP_CLASSES];      // available ships
                    ...
                    int wl_ship_slot);
                      ...
                      NULL, bool resize = true);
                          ...
                          NULL, float closeup_zoom = .65f, float rev_rate =
