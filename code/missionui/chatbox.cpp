...
// chatbox buttons
  ui_button_info Chatbox_buttons[GR_NUM_RESOLUTIONS][CHATBOX_NUM_BUTTONS +
                                                     1] = 
{
...
void chatbox_autosplit_line() 
{
  ...
  else if ((Chat_inputbox.pressed() && (strlen(msg) > 0))
...
int chatbox_create(int mode_flags) 
{
    ...
    Chat_window.create(0, 0, gr_screen.max_w_unscaled,
                       gr_screen.max_h_unscaled, 0);
  Chat_window.set_mask_bmap(Chatbox_mask);
   
    // create the chat text enter input area
    Chat_inputbox.create(&Chat_window, Chatbox_inputbox_x,
                         Chatbox_textenter_y, Chatbox_inputbox_w,
                         CHATBOX_MAX_LEN, "",
    ...
    
    {
        ...
        
        {
            ...
            // create the button
            Chatbox_buttons[gr_screen.res][idx].button.create(&Chat_window,
                                                              "",
                                                              Chatbox_buttons
                                                              [gr_screen.
                                                               res][idx].x,
            ...
            Chatbox_buttons[gr_screen.res][idx].button.
            set_highlight_action(common_play_highlight_sound);
           
            // set the bitmap
            Chatbox_buttons[gr_screen.res][idx].button.
            set_bmaps(Chatbox_buttons[gr_screen.res][idx].filename);
           
            // set the hotspot
            Chatbox_buttons[gr_screen.res][idx].button.
            link_hotspot(Chatbox_buttons[gr_screen.res][idx].hotspot);
        ...
        {
          Chatbox_buttons[gr_screen.res][CHATBOX_TOGGLE_SIZE].button.
            set_bmaps(Chatbox_buttons[gr_screen.res][CHATBOX_TOGGLE_SIZE].
                      filename);
        ...
        
        {
            ...
            set_bmaps(Chatbox_buttons[gr_screen.res][CHATBOX_TOGGLE_SIZE + 1].
                      filename);
...
int chatbox_process(int key_in) 
{
    ...
    
    {
      ...
       if (Chatbox_buttons[gr_screen.res][CHATBOX_SCROLL_DOWN].button.
             pressed())
        
        {
          chatbox_scroll_down();
        }
       if (Chatbox_buttons[gr_screen.res][CHATBOX_TOGGLE_SIZE].button.
             pressed())
...
void chatbox_add_line(char *msg, int pid, int add_id) 
{
  ...
  char msg_extra[CHATBOX_STRING_LEN];
    ...
    // copy in the chars
    strncpy(&Brief_chat_lines[Brief_current_add_line][1], p_str[0],
    ...
    
    {
        ...
        n_lines =
        split_str(msg_extra + n_chars[0], Chatbox_disp_w - CHAT_LINE_INDENT,
                  n_chars, p_str, 3);
        ...
        
        {
            ...
            strncpy(&Brief_chat_lines[Brief_current_add_line][1], p_str[idx],
                    CHATBOX_STRING_LEN - 1);
          if (n_chars[idx] >= CHATBOX_STRING_LEN)
            
            {
              Brief_chat_lines[Brief_current_add_line][CHATBOX_STRING_LEN -
                                                        1] = '\0';
...
 void chatbox_render_chat_lines() 
{
    ...
    
    {
        ...
        gr_string(Chatbox_begin_x + Brief_chat_indents[started_at], ly,
                  &Brief_chat_lines[started_at][1]);
...
void chatbox_force_small() 
{
    ...
    Chatbox_buttons[gr_screen.res][CHATBOX_TOGGLE_SIZE].button.
    set_bmaps(Chatbox_buttons[gr_screen.res][CHATBOX_TOGGLE_SIZE].filename);
   
    // call this to set everything up correctly
    chatbox_set_mode(new_mode_flags);
   
    // change the location of the input box
    Chat_inputbox.update_dimensions(Chatbox_inputbox_x, Chatbox_textenter_y,
                                    Chatbox_inputbox_w, 15);
...
void chatbox_recall_add(char *string) 
{
    ...
    {
      memcpy(&Chatbox_recall_lines[idx], &Chatbox_recall_lines[idx - 1],
              CHATBOX_MAX_LEN + 2);
...
void chatbox_recall_up() 
{
    ...
    
    {
        ...
        
        {
            ...
            set_text(Chatbox_recall_lines[++Chatbox_recall_index]);
