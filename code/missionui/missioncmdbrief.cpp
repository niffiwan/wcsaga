...
 void cmd_brief_init_voice() 
{
    ...
    
    {
        ...
        {
          Cur_cmd_brief->stage[i].wave =
            audiostream_open(Cur_cmd_brief->stage[i].wave_filename,
                             ASF_VOICE);
...
 void cmd_brief_new_stage(int stage) 
{
    ...
    
    {
                     ...
                     Cur_cmd_brief->stage[stage].text);
                         ...
                         Cmd_text_wnd_coords[Uses_scroll_buttons][gr_screen.
                                                                  res]
                         [CMD_W_COORD]);
...
 void cmd_brief_button_pressed(int n) 
{
    ...
    
    {
        ...
        
        {
           ...
           Cmd_brief_text_max_lines[gr_screen.res])
  ...
  if (Cmd_brief_background_bitmap < 0)  // failed to load
    {
      ...
      Cmd_brief_background_bitmap =
        bm_load(Cmd_brief_fname[Uses_scroll_buttons][gr_screen.res]);
    }
   Ui_window.create(0, 0, gr_screen.max_w_unscaled,
                      gr_screen.max_h_unscaled, 0);
  Ui_window.
    set_mask_bmap(Cmd_brief_mask[Uses_scroll_buttons][gr_screen.res]);
    ...
    Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_FIRST_STAGE].button.
    set_hotkey(KEY_SHIFTED | KEY_LEFT);
  Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_LAST_STAGE].button.
    set_hotkey(KEY_SHIFTED | KEY_RIGHT);
  Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_PREV_STAGE].button.
    set_hotkey(KEY_LEFT);
  Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_NEXT_STAGE].button.
    set_hotkey(KEY_RIGHT);
  Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_ACCEPT].button.
    set_hotkey(KEY_CTRLED | KEY_ENTER);
  Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_HELP].button.
    set_hotkey(KEY_F1);
  Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_OPTIONS].button.
    set_hotkey(KEY_F2);
    ...
    {
      Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_SCROLL_UP].button.
        set_hotkey(KEY_UP);
      Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_SCROLL_DOWN].button.
        set_hotkey(KEY_DOWN);
...
 void cmd_brief_do_frame(float frametime) 
{
    ...
    {
      bm_get_info((Cur_Anim.streaming) ? Cur_Anim.bitmap_id : Cur_Anim.
                   first_frame, &x, &y, NULL, NULL, NULL);
      x = Cmd_image_center_coords[gr_screen.res][CMD_X_COORD] - x / 2;
      y = Cmd_image_center_coords[gr_screen.res][CMD_Y_COORD] - y / 2;
      generic_anim_render(&Cur_Anim, (Cmd_brief_paused) ? 0 : frametime, x,
                           y);
    ...
    {
      Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_PAUSE].button.
        draw_forced(2);
  ...
  gr_string(Cmd_text_wnd_coords[Uses_scroll_buttons][gr_screen.res]
             [CMD_X_COORD] +
             Cmd_text_wnd_coords[Uses_scroll_buttons][gr_screen.
                                                      res][CMD_W_COORD] - w,
    ...
    if ((Cmd_brief_text_max_lines[gr_screen.res] + Top_cmd_brief_text_line) <
