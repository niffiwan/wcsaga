...
voice_map
  Debrief_promotion_voice_mapping[NUM_VOLITION_CAMPAIGNS]
  ...
  if (Debrief_cue_voice)
    {
      ...
      if ((Stage_voice < Num_debrief_stages)
        ...
        {
          audiostream_play(Debrief_voices[Stage_voice], Master_voice_volume,
                           0);
...
void debrief_buttons_init()
{
  ...
  Buttons[gr_screen.res][LAST_STAGE].button.
    set_hotkey(KEY_SHIFTED | KEY_RIGHT);
  Buttons[gr_screen.res][FIRST_STAGE].button.
    set_hotkey(KEY_SHIFTED | KEY_LEFT);
...
void debrief_ui_init()
{
  ...
  Debrief_ui_window.create(0, 0, gr_screen.max_w_unscaled,
  ...
  if (Game_mode & GM_MULTIPLAYER)
    {
      ...
      List_region.create(&Debrief_ui_window, "",
  ...
  Award_bg_bitmap =
    bm_load(Debrief_award_filename[gr_screen.res][DB_AWARD_BG]);
...
int debrief_find_persona_index()
{
  ...
  if ((Campaign.current_mission >= 0)
      ...
      && (Campaign.filename))
    {
      ...
      if (Campaign.missions[Campaign.current_mission].debrief_persona_index !=
          0)
...
void debrief_choose_voice(char *voice_dest, char *voice_base,
  ...
  if (Player->stats.m_promotion_earned != -1)
    {
      ...
      debrief_choose_voice(Promotion_stage.voice,
  ...
  if (Player->stats.m_badge_earned != -1)
    {
      ...
      sprintf(buf, NOX("%s%.2d"),
              Debrief_award_filename[gr_screen.res][DB_AWARD_BADGE],
              Medals[i].badge_num + 1);
      ...
      debrief_choose_voice(Badge_stage.voice,
                           Medals[Player->stats.m_badge_earned].voice_base);
  ...
  if (!inited)
    {
      ...
      if (Fred_running)
        {
          stuff_string(stagep->new_recommendation_text, F_MULTITEXT,
                       MAX_RECOMMENDATION_LEN);
        }
      else
        {
          stagep->new_recommendation_text =
...
void debrief_multi_list_scroll_down()
{
  ...
  if ((Multi_list_offset +
       Debrief_multi_list_team_max_display[gr_screen.res]) >= Multi_list_size)
    {
      gamesnd_play_iface(SND_GENERAL_FAIL);
      return;
    }
  ...
  while (y + font_height <= Debrief_list_coords[gr_screen.res][3])
    {
              ...
              if (np->p_info.team == 0)
                {
                  ...
                  if (((np->state == NETPLAYER_STATE_DEBRIEF_ACCEPT)
                      ...
                      && (Multi_common_icons[MICON_TEAM0_SELECT] != -1))
                    {
                      ...
                      gr_bitmap(Debrief_list_coords[gr_screen.res][0],
                                Debrief_list_coords[gr_screen.res][1] + y -
                                2);
                    }
      ...
      if (Net_players[Multi_list[z].net_player_index].
          flags & NETINFO_FLAG_OBSERVER
          && !(Net_players[Multi_list[z].net_player_index].
               flags & NETINFO_FLAG_OBS_PLAYER))
        {
          strcat_s(str, XSTR("(O)", 438));
        }

      // bli
      gr_string(Debrief_list_coords[gr_screen.res][0] +
...
void debrief_accept(int ok_to_post_start_game_event)
{
  ...
  if (( /*Cheats_enabled || */ Turned_traitor || Must_replay_mission)
      && (Game_mode & GM_CAMPAIGN_MODE))
    {
      ...
      z =
        popup(0, 3, XSTR("Return to &Debriefing", 442),
  ...
  for (i = 0; i < MAX_SHIP_CLASSES; i++)
    {
      ...
      std::vector < Kill_pair >::iterator kill_pair =
        std::find_if(kills.begin(), kills.end(), KillClassEqual(ship_class));
...
void debrief_text_stage_init(char *src, int type)
{
  ...
  n_lines =
    split_str(src, Debrief_text_wnd_coords[gr_screen.res][2], n_chars, p_str,
              MAX_DEBRIEF_LINES);
  Assert(n_lines >= 0);
  ...
  for (i = 0; i < Debriefing->num_stages; i++)
    {
      if (Debriefing->stages[i].new_text)
        sexp_replace_variable_names_with_values(Debriefing->stages[i].
        ...
        else
          {
            ...
            if (!Turned_traitor && !Must_replay_mission
                && (Game_mode & GM_CAMPAIGN_MODE))
              {
                ...
                choice =
                  popup(pf_flags, 3, POPUP_CANCEL, XSTR("&Yes", 454),
  ...
  for (i = 0; i < Debrief_award_text_num_lines; i++)
    {
      ...
      x =
        (Medal_bitmap <
         0) ? (Debrief_award_text_coords[gr_screen.res][0] + (field_width -
                                                              sw) /
...
void debrief_add_award_text(char *str)
{
  ...
  if (Debrief_award_text_num_lines < AWARD_TEXT_MAX_LINES)
    {
      line2 =
        split_str_once(Debrief_award_text[Debrief_award_text_num_lines - 1],
                       field_width);
      if (line2 != NULL)
        {
...
void debrief_do_frame(float frametime)
{
  ...
  if (The_mission.flags & MISSION_FLAG_TOGGLE_DEBRIEFING)
    {
      ...
      if (!
          (( /*Cheats_enabled || */ Turned_traitor || Must_replay_mission)
           && (Game_mode & GM_CAMPAIGN_MODE)))
        {
          debrief_accept();
          return;
  ...
  gr_printf(Debrief_title_coords[gr_screen.res][0],
            ...
            NOX("[name: %s, mod: %s]"), Mission_filename,
  ...
  if ((Game_mode & GM_MULTIPLAYER) && Debrief_should_show_popup)
    {
      Assert((Multi_list_select >= 0)
             && (Multi_list_select < Multi_list_size));
      multi_pinfo_popup(&Net_players
                        [Multi_list[Multi_list_select].net_player_index]);
      ...
      if (MULTI_CONNECTED((*np)) && !MULTI_STANDALONE((*np)))
        {
          ...
          gr_force_fit_string(list->callsign, CALLSIGN_LEN - 1,
                              Debrief_list_coords[gr_screen.res][2] -
                              MULTI_LIST_TEAM_OFFSET);
        }
    }                           // end for
