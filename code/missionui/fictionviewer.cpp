...
void fiction_viewer_init() 
{
    ...
    bm_load(Fiction_viewer_screen_filename[gr_screen.res]);
    ...
    Fiction_viewer_text_max_lines =
    Fiction_viewer_text_coordinates[gr_screen.res][3] / gr_get_font_height();
   
    // window
    Fiction_viewer_window.create(0, 0, gr_screen.max_w_unscaled,
                                 gr_screen.max_h_unscaled, 0,
                                 Fiction_viewer_fontnum);
  Fiction_viewer_window.
    set_mask_bmap(Fiction_viewer_screen_mask[gr_screen.res]);
   
    // add the buttons
    for (int i = 0; i < NUM_FVW_BUTTONS; i++)
    
    {
      int repeat = (i == FVW_BUTTON_SCROLL_UP
                     || i == FVW_BUTTON_SCROLL_DOWN);
      ui_button_info * b = &Fiction_viewer_buttons[gr_screen.res][i];
       b->button.create(&Fiction_viewer_window, "", b->x, b->y, b->xt,
                          b->yt, repeat, 1);
    ...
    Fiction_viewer_buttons[gr_screen.res][FVW_BUTTON_ACCEPT].button.
    set_hotkey(KEY_CTRLED | KEY_ENTER);
  Fiction_viewer_buttons[gr_screen.res][FVW_BUTTON_SCROLL_UP].button.
    set_hotkey(KEY_UP);
  Fiction_viewer_buttons[gr_screen.res][FVW_BUTTON_SCROLL_DOWN].button.
    set_hotkey(KEY_DOWN);
   
    // init brief text
    brief_color_text_init(Fiction_viewer_text,
                          Fiction_viewer_text_coordinates[gr_screen.res][2],
                          0, 0);
    ...
    
    {
                                    ...
                                    Fiction_viewer_slider_coordinates
                                    [gr_screen.res][0],
                                    ...
                                    Fiction_viewer_text_max_lines,
                                    Fiction_viewer_slider_filename[gr_screen.
                                                                    res],
...
void fiction_viewer_do_frame(float frametime) 
{
    ...
    brief_render_text(Top_fiction_viewer_text_line,
                      Fiction_viewer_text_coordinates[gr_screen.res][0],
                      Fiction_viewer_text_coordinates[gr_screen.res][1],
                      Fiction_viewer_text_coordinates[gr_screen.res][3],
                      frametime);
    ...
    
    {
      ...
      int more_txt_x =
        Fiction_viewer_text_coordinates[gr_screen.res][0] +
        (Fiction_viewer_text_coordinates[gr_screen.res][2] / 2) - 10;
