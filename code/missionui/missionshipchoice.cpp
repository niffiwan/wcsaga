  ...
  switch (i)
    ...
    case SHIP_SELECT_SHIP_SCROLL_UP:
      ...
      if (common_scroll_down_pressed
          (&SS_active_list_start, SS_active_list_size, MAX_ICONS_ON_SCREEN))
        {
...
void ship_select_blit_ship_info()
{
  ...
  if ((sip->velocity_str != NULL) && strlen(sip->velocity_str))
    {
      gr_string(Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD] + 4,
                y_start, sip->velocity_str);
    }
  else
    {
      sprintf(str, XSTR("%d kps", 743),
              (int) ((sip->max_vel.xyz.z * Hud_unit_multiplier) + 0.5f));
      gr_string(Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD] + 4,
                y_start, str);
    }
  y_start += 10;
          ...
          if (Ss_icons[Carried_ss_icon.ship_class].model_index != -1)
            {
              draw_model_icon(Ss_icons[Carried_ss_icon.ship_class].
                              ...
                              MR_NO_LIGHTING, sip->closeup_zoom / 1.27f, sx,
                              sy, w, h, sip);
            }
      ...
      if ((Selected_ss_class >= 0) && (ShipSelectModelNum >= 0))
        {
          ...
          draw_model_rotating(ShipSelectModelNum,
                              ...
                              &sip->closeup_pos,
                              sip->closeup_zoom * 1.35f, rev_rate);
        }
    }
      ...
      if (ss_icon->model_index != -1)
        {
          draw_model_icon(ss_icon->model_index,
                          MR_LOCK_DETAIL | MR_AUTOCENTER | MR_NO_FOGGING |
                          MR_NO_LIGHTING, sip->closeup_zoom / 1.27f,
      ...
      if (bitmap_to_draw != -1)
      ...
      else if (color_to_draw != NULL)
        {
          ...
          draw_model_icon(icon->model_index,
                          ...
                          Wing_icon_coords[gr_screen.res][slot_index][1], 32,
                          28, sip);
          ...
          if (icon->model_index != -1)
            {
              ...
              draw_model_icon(icon->model_index,
                              MR_LOCK_DETAIL | MR_AUTOCENTER | MR_NO_FOGGING |
                              MR_NO_LIGHTING, sip->closeup_zoom / 1.27f, x, y,
                              32, 28, sip);
