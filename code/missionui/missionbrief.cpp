...
struct brief_buttons 
{
 ...
  brief_buttons(char *name, int x1, int y1, int xt1, int yt1, int h, int r = 0):filename(name), x(x1), y(y1), xt(xt1), yt(yt1), hotspot(h),
    repeat
    (r)
  {
} };
...
void brief_skip_training_pressed() 
{
    ...
    popup(PF_USE_NEGATIVE_ICON | PF_USE_AFFIRMATIVE_ICON, 2, POPUP_NO,
          POPUP_YES,
          XSTR
...
void brief_button_do(int i) 
{
    ...
    
    {
        ...
        {
          Brief_buttons[gr_screen.res][BRIEF_BUTTON_MULTI_LOCK].button.
            disable();
...
 void brief_buttons_init() 
{
    ...
    {
      Brief_buttons[gr_screen.res][BRIEF_BUTTON_SKIP_TRAINING].button.
        enable();
    ...
    Closeup_close_button.create(&Brief_ui_window, "",
                                Closeup_button_coords[gr_screen.
                                                      res][BRIEF_X_COORD],
                                Closeup_button_coords[gr_screen.
                                                      res][BRIEF_Y_COORD], 60,
                                30, 0, 1);
    ...
    Brief_buttons[gr_screen.res][BRIEF_BUTTON_LAST_STAGE].button.
    set_hotkey(KEY_SHIFTED | KEY_RIGHT);
  Brief_buttons[gr_screen.res][BRIEF_BUTTON_NEXT_STAGE].button.
    set_hotkey(KEY_RIGHT);
  Brief_buttons[gr_screen.res][BRIEF_BUTTON_PREV_STAGE].button.
    set_hotkey(KEY_LEFT);
  Brief_buttons[gr_screen.res][BRIEF_BUTTON_FIRST_STAGE].button.
    set_hotkey(KEY_SHIFTED | KEY_LEFT);
  Brief_buttons[gr_screen.res][BRIEF_BUTTON_SCROLL_UP].button.
    set_hotkey(KEY_UP);
  Brief_buttons[gr_screen.res][BRIEF_BUTTON_SCROLL_DOWN].button.
    set_hotkey(KEY_DOWN);
...
void brief_init() 
{
    ...
    
    {
        ...
        sexp_replace_variable_names_with_values(Briefing->stages[i].new_text,
                                                 MAX_BRIEF_LEN);
    ...
    // init common UI
    Brief_ui_window.create(0, 0, gr_screen.max_w_unscaled,
                           gr_screen.max_h_unscaled, 0);
   if (Game_mode & GM_MULTIPLAYER)
    
    {
      Brief_ui_window.
        set_mask_bmap(Brief_multi_mask_filename[gr_screen.res]);
...
void brief_render_closeup(int ship_class, float frametime) 
{
    ...
    {
      model_set_outline_color(HUD_color_red, HUD_color_green,
                               HUD_color_blue);
      model_render_flags =
        MR_NO_LIGHTING | MR_LOCK_DETAIL | MR_AUTOCENTER | MR_NO_POLYS |
        MR_SHOW_OUTLINE;
...
void brief_render(float frametime) 
{
    ...
    
    {
        ...
        brief_render_text(Top_brief_text_line,
        ...
        if ((Brief_text_max_lines[gr_screen.res] + Top_brief_text_line) <
    ...
    
    {
      ...
      gr_string(Title_coords_multi[gr_screen.res][0],
                 Title_coords_multi[gr_screen.res][1], buf);
    ...
    
    {
      ...
      gr_string(Title_coords[gr_screen.res][0] - w,
                 Title_coords[gr_screen.res][1], The_mission.name);
...
void brief_set_closeup_pos(brief_icon * bi) 
{
    ...
    fl2i(320 - Closeup_coords[gr_screen.res][BRIEF_W_COORD] / 2.0f + 0.5f);
...
int brief_setup_closeup(brief_icon * bi) 
{
    ...
    
    {
...
#ifndef FS2_DEMO
        strcpy_s(pof_filename,
                 Asteroid_info[ASTEROID_TYPE_LARGE].pof_files[0]);
      ...
       strcpy_s(Closeup_icon->closeup_label,
        ...
        if (!Cmdline_wcsaga
            && sip->
            flags & (SIF_SMALL_SHIP | SIF_BIG_SHIP | SIF_HUGE_SHIP |
                     SIF_SENTRYGUN))
...
void brief_check_for_anim() 
{
    ...
    {
      Brief_buttons[gr_screen.res][i].button.get_dimensions(&x, &y, &iw,
                                                             &ih);
...
void brief_do_frame(float frametime) 
{
    ...
    snazzy_menu_do(Brief_ui_mask_data, Brief_ui_mask_w, Brief_ui_mask_h,
                   Num_briefing_regions, Briefing_select_region,
                   &snazzy_action, 0);
  ...
  int cam_change = 0;
    ...
    {
      nprintf(("General", "Camera pos: %.2f, %.2f %.2f // ",
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                {
                  if (Briefing->stages[Last_brief_stage].
            ...
            // set the camera target
            brief_set_new_stage(&Briefing->stages[Current_brief_stage].
                                camera_pos,
                                &Briefing->stages[Current_brief_stage].
...
 void brief_maybe_blit_scene_cut(float frametime) 
{
    ...
    
    {
      ...
      int framenum;
        ...
        framenum =
        fl2i((Fade_anim.time_elapsed * Fade_anim.num_frames) /
