...
static Profile *profile_read(char *file)
{
  ...
  snprintf(fullname, MAX_PATH_LEN - 1, "%s%s%s%s%s%s%s", detect_home(),
           DIR_SEPARATOR_STR, Osreg_company_name, DIR_SEPARATOR_STR,
           Osreg_user_dir, DIR_SEPARATOR_STR, file);
...
static Profile *profile_update(Profile * profile, char *section, char *key,
                               char *value)
{
  if (profile == NULL)
    {
