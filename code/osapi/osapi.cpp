...
LRESULT CALLBACK win32_message_handler(HWND hwnd, UINT msg, WPARAM wParam,
                                       LPARAM lParam);
...
void os_check_debugger()
{
  ...
  *p = '\0';
  ...
  EnumWindows((int (__stdcall *) (struct HWND__ *, long)) os_enum_windows,
              (long) &search_string);
}
      ...
      if (fAppActive)
        {
          ...
          if (!Cmdline_window)
            {
              SetWindowPos(hwndApp, HWND_TOPMOST, 0, 0, 0, 0,
      ...
      else
        {
          ...
          if (!Cmdline_window)
            {
              SetWindowPos(hwndApp, HWND_NOTOPMOST, 0, 0, 0, 0,
                           SWP_NOMOVE | SWP_NOSIZE);
            }
        }
    ...
    case WM_ACTIVATE:
      {
        ...
        fAppActive = ((flag == WA_ACTIVE)
                      || (flag == WA_CLICKACTIVE)) ? TRUE : FALSE;
...
void win32_create_window(int width, int height)
{
  ...
  hwndApp = CreateWindowEx((Cmdline_window || Cmdline_fullscreen_window) ? 0 : WS_EX_TOPMOST, szWinClass, szWinTitle, style, start_x,   // x
                           start_y,     // y
                           width + x_add,       // w
                           height + y_add,      // h
  ...
  if (GetKeyboardLayoutName(layout_name))
    {
      ...
      if (layout_string.length() >= german.length())
        {
          if (0 ==
              layout_string.compare(layout_string.length() - german.length(),
                                    german.length(), german))
            {
              layout = LCL_GERMAN;
            }
          else if (0 ==
                   layout_string.compare(layout_string.length() -
                                         french.length(), french.length(),
                                         french))
            {
              layout = LCL_FRENCH;
            }
