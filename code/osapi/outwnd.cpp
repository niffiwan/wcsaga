...
struct outwnd_filter_struct
{
  ...
  bool enabled;
    ...
    outwnd_filter_struct():enabled(false)
  {
    name[0] = NULL;
  }
};
...
int old_marked =
  0, old_marked_top, old_marked_bottom, old_marked_left, old_marked_right;
int marking_started_x, marking_started_y, client_top_yoffset, cur_line_index,
  marking_active = 0;
int scroll_wait = 1;
...
BOOL CALLBACK find_dlg_handler(HWND hwnd, UINT msg, WPARAM wParam,
                               LPARAM lParam);
...
{
asm("mov %0,%%al;" "mov %1,%%dx;" "out %%al,%%dx;": : "g"(value), "g"(port):"al",
      "dx");
}
...
{
  unsigned char value;
asm("mov %1,%%dx;" "in  %%dx,%%al;" "mov %%al,%0": "=g"(value): "g"(port):"al",
      "dx");
  return value;
}
#endif
...
inline void fix_marking_coords(int &x, int &y, LPARAM l_parm)
{
  ...
  y =
    ((signed short) HIWORD(l_parm) - client_top_yoffset) / nTextHeight +
    cur_line_index;
...
void load_filter_info(void)
{
  ...
  snprintf(pathname, MAX_PATH_LEN - 1, "%s%s%s%s%s%s%s%s%s", detect_home(),
           DIR_SEPARATOR_STR, Osreg_company_name, DIR_SEPARATOR_STR,
           Osreg_user_dir, DIR_SEPARATOR_STR, Pathtypes[CF_TYPE_DATA].path,
           DIR_SEPARATOR_STR, NOX("debug_filter.cfg"));
...
void save_filter_info(void)
{
  ...
  memset(pathname, 0, sizeof(pathname));
  snprintf(pathname, MAX_PATH_LEN - 1, "%s%s%s%s%s%s%s", detect_home(),
           DIR_SEPARATOR_STR, Osreg_company_name, DIR_SEPARATOR_STR,
           Osreg_user_dir, DIR_SEPARATOR_STR, Pathtypes[CF_TYPE_DATA].path);
  ...
  if (mono_driver != ((HANDLE) - 1))
    {
      ...
      DeviceIoControl(mono_driver, (DWORD) IOCTL_MONO_PRINT, tmp, strlen(tmp),
                      NULL, 0, &cbReturned, 0);
#ifndef NMONO
    }
  else
...
LRESULT CALLBACK outwnd_handler(HWND hwnd, UINT msg, WPARAM wParam,
                                LPARAM lParam)
{
  switch (msg)
    {
        ...
        for (uint i = 0; i < OutwndFilter.size(); i++)
          {
            ...
            if (OutwndFilter[i].enabled)
              AppendMenu(h_sub_menu, flags | MF_CHECKED, ID_FILTER + i,
                         OutwndFilter[i].name);
            else
              AppendMenu(h_sub_menu, flags, ID_FILTER + i,
                         OutwndFilter[i].name);
          }

        AppendMenu(h_menu, MFT_STRING, ID_COPY, "&Copy\tEnter");
        AppendMenu(h_menu, MFT_STRING, ID_FIND, "&Find Text");
        AppendMenu(h_menu, MF_POPUP, (unsigned int) h_sub_menu,
                   "Filter &Messages");
        pt.x = LOWORD(lParam);
        pt.y = HIWORD(lParam);
              ...
              if (n == marked_top && n == marked_bottom)        // special 1 line case
                {
                  ...
                  if (marked_right < len)
                    TextOut(hdc, x, y, outtext[n] + marked_right,
                            len - marked_right);
              ...
              else if (n == marked_top)
                {
                  ...
                  TextOut(hdc, x, y, outtext[n] + marked_left,
                          len - marked_left);
  ...
  if (marked && (marked_top > marked_bottom || (marked_top == marked_bottom &&
                                                marked_left >= marked_right)))
    {
      ...
      sprintf(msg, "Marking limits invalid!\n"
              "(%d,%d) to (%d,%d)", marked_left, marked_top, marked_right,
              marked_bottom);
...
BOOL outwnd_create(int display_under_freespace_window)
{
      ...
      if (!Is_standalone)
        {
          ...
          rect.bottom =
            GetSystemMetrics(SM_CYSCREEN) - TASKBAR_HEIGHT - rect.top;
  ...
  SetWindowPos(hOutputWnd, HWND_NOTOPMOST, 0, 0, 0, 0,
...
void outwnd_init_debug_window(int display_under_freespace_window)
{
  ...
  hOutputThread = CreateThread(NULL,
                               0, (LPTHREAD_START_ROUTINE) outwnd_thread,
                               (LPVOID) display_under_freespace_window, 0,
                               &OutputThreadID);
  ...
  if ((mono_driver =
       CreateFile("\\\\.\\MONO", GENERIC_READ | GENERIC_WRITE, 0, NULL,
  ...
  if (Log_fp == NULL)
    {
      ...
      FreeSpace_logfilename =
        (Fred_running) ? "fred2_open.log" : "fs2_open.log";

      memset(pathname, 0, sizeof(pathname));
      snprintf(pathname, MAX_PATH_LEN - 1, "%s%s%s%s%s%s%s", detect_home(),
               DIR_SEPARATOR_STR, Osreg_company_name, DIR_SEPARATOR_STR,
               Osreg_user_dir, DIR_SEPARATOR_STR,
               Pathtypes[CF_TYPE_DATA].path);
      cf_create_directory(pathname);

      memset(pathname, 0, sizeof(pathname));
      snprintf(pathname, MAX_PATH_LEN - 1, "%s%s%s%s%s%s%s%s%s",
               detect_home(), DIR_SEPARATOR_STR, Osreg_company_name,
               DIR_SEPARATOR_STR, Osreg_user_dir, DIR_SEPARATOR_STR,
               Pathtypes[CF_TYPE_DATA].path, DIR_SEPARATOR_STR,
               FreeSpace_logfilename);
      ...
      if (Log_fp == NULL)
      ...
      else
        {
          ...
          strftime(datestr, sizeof(datestr) - 1, "%a %b %d %H:%M:%S %Y",
                   localtime(&timedate));
  ...
  while (1)
    {
          ...
          if (pos >= nCharRows - 1)     // outside of window viewport, so scroll
            {
              ...
              SetScrollInfo(hOutputWnd, SB_VERT, &si, 1);
              ScrollWindow(hOutputWnd, 0, (scroll_pos - pos) * nTextHeight,
                           NULL, NULL);
            }
      ...
      if (n == mprintf_last_line)
        {
          MessageBox(hOutputWnd, "Search text not found", "Find Error",
                     MB_OK | MB_ICONERROR);
          find_line = -1;
          return;
        }
