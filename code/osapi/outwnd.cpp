...
void load_filter_info(void) 
{
  ...
  snprintf(pathname, MAX_PATH_LEN, "%s\\%s\\%s", detect_home(),
            Pathtypes[CF_TYPE_DATA].path, NOX("debug_filter.cfg"));
...
 void outwnd_print(char *id, char *tmp) 
{
    ...
    
    {
      ...
       DeviceIoControl(mono_driver, (DWORD) IOCTL_MONO_PRINT, tmp,
                         strlen(tmp), NULL, 0, &cbReturned, 0);
...
 void outwnd_update_marking(LPARAM l_parm, HWND hwnd) 
{
  ...
   if (marked != old_marked || marked_top != old_marked_top
         || marked_bottom != old_marked_bottom
         || marked_left != old_marked_left
         || marked_right != old_marked_right)
...
 BOOL outwnd_create(int display_under_freespace_window) 
{
  ...
  wclass.cbClsExtra = 0;
    ...
    //  Create Game Window
    hOutputWnd =
    CreateWindow(szOutputClass, "Debug Spew", style, rect.left, rect.top,
...
 void outwnd_init_debug_window(int display_under_freespace_window) 
{
    ...
    CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) outwnd_thread,
                 (LPVOID) display_under_freespace_window, 0, &OutputThreadID);
...
 void outwnd_init(int display_under_freespace_window) 
{
    ...
    
    {
      ...
      snprintf(pathname, MAX_PATH_LEN - 1, "%s\\%s\\%s", detect_home(),
                Pathtypes[CF_TYPE_DATA].path, FreeSpace_logfilename);
...
 void find_text_in_outwindow(int n, int p) 
{
    ...
    
    {
        ...
        
        {
            ...
            (max_scroll_pos - GetScrollPos(hOutputWnd, SB_VERT));
