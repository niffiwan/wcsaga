...
 void jumpnode_level_close() 
{
  ...
   for (jnp = Jump_nodes.get_first(); !Jump_nodes.is_end(jnp);
          jnp = next_node)
    ...
    if (Num_jump_nodes != 0)
    Warning(LOCATION, "Num_jump_nodes should be 0, but is actually %d.",
            Num_jump_nodes);
        ...
        
        {
          ...
          float dist;
            ...
            
            {
                ...
                fl2i(HUD_COLOR_ALPHA_USER_MAX - 2 +
                     (dist - 1000) * (HUD_COLOR_ALPHA_USER_MIN -
                                      HUD_COLOR_ALPHA_USER_MAX - 2) / (9000) +
                     0.5f);
...
 void jump_node::set_model(char *model_name, bool show_polys) 
{
  ...
  int new_model = model_load(model_name, 0, NULL, 0);
    ...
    {
      Warning(LOCATION, "Couldn't load model file %s for jump node %s",
               model_name, m_name);
...
 bool jumpnode_check_for_duplicates() 
{
  ...
  for (reference = Jump_nodes.get_first(); !Jump_nodes.is_end(reference);
        reference = reference->get_next())
    
    {
      for (other = reference->get_next(); !Jump_nodes.is_end(other);
