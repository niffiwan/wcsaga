...
void event_music_set_current_pattern(int new_pattern)
{
  ...
  else if (Current_pattern == SONG_NRML_1 || Current_pattern == SONG_NRML_2
...
void event_music_force_switch()
{
  ...
  audiostream_set_sample_cutoff(Patterns[new_pattern].handle,
  ...
  if (Current_pattern != -1)
    {
      ...
      bool locked_in_ambient =
        (Soundtracks[Current_soundtrack_num].flags & EMF_LOCK_IN_NORMAL) != 0;
      ...
      // Third case: switching to a different track by interruption unless we're already doing it with this track
      else if (!locked_in_ambient && (pat->force_pattern == TRUE)
               && (pat->can_force == TRUE)
               && !(pat->forcing_pattern == pat->next_pattern))
        {
          ...
          if (measures_played < pat->num_measures)
            {
              audiostream_set_sample_cutoff(pat->handle,
                                            pat->samples_per_measure *
                                            (measures_played + 1));
              pat->loop_for = 0;
              pat->forcing_pattern = pat->next_pattern;
            }
      ...
      if (timestamp_elapsed(Hostile_presence_timestamp))
        {
          Hostile_presence_timestamp =
            timestamp(HOSTILE_PRESENCE_CHECK_INTERVAL);
      ...
      if (!Event_Music_battle_started)
        {
          ...
          else if (Current_pattern == SONG_BTTL_1
                  ...
                  if (hostile_ships_present() == FALSE)
                    {
                      ...
                      if (Soundtracks[Current_soundtrack_num].
                          flags & EMF_CYCLE_FS1)
                        {
                          Patterns[Current_pattern].next_pattern =
                            SONG_NRML_3;
      ...
      if (timestamp_elapsed(Mission_over_timestamp))
        {
          ...
          if (Current_pattern != SONG_VICT_2 && mission_goals_met()
      ...
      if (Player_obj != NULL && Player_ship != NULL)
        {
          ...
          float integrity =
            Player_obj->hull_strength / Player_ship->ship_max_hull_strength;
          if (integrity < HULL_VALUE_TO_PLAY_INTENSE_BATTLE_MUSIC)
  ...
  if (Current_pattern != -1)
    {
      ...
      // overlay
      if (Soundtracks[Current_soundtrack_num].
          flags & EMF_ALLIED_ARRIVAL_OVERLAY)
        {
          ...
          audiostream_set_sample_cutoff(Patterns[SONG_AARV_1].handle,
                                        fl2i(Patterns[SONG_AARV_1].
                                             num_measures *
                                             Patterns[SONG_AARV_1].
                                             samples_per_measure));
        }
      // don't overlay
      else
...
bool parse_soundtrack_line(int strack_idx, int pattern_idx)
{
  ...
  if (pattern_idx >= MAX_PATTERNS)
    {
      Warning(LOCATION, "Too many $Name: entries for soundtrack %s",
...
void parse_soundtrack()
{
  ...
  if (strack_idx < 0 && (nocreate || Num_soundtracks >= MAX_SOUNDTRACKS))
    {
      if (Num_soundtracks >= MAX_SOUNDTRACKS)
        {
          Warning(LOCATION,
                  "Maximum number of soundtracks reached after '%s'; max is '%d'",
  ...
  if (optional_string("+Allied Arrival Overlay:"))
    {
      stuff_boolean_flag(&Soundtracks[strack_idx].flags,
      ...
      while (required_string_either("#SoundTrack End", "$Name:"))
        {
          ...
          if (i == Num_pattern_types)
            {
              Warning(LOCATION,
                      "Could not find new index for pattern %d of soundtrack '%s'",
...
void parse_menumusic()
{
  ...
  if (cf_exists_full_ext
      (Spooled_music[idx].filename, CF_TYPE_MUSIC, NUM_EXT, exts))
...
void event_music_parse_musictbl(char *filename)
{
  ...
  if ((rval = setjmp(parse_abort)) != 0)
  ...
  else
    {
      ...
      while (skip_to_start_of_string_either
             ("#Soundtrack Start", "#Menu Music Start", NULL))
        {
...
int hostile_ships_present()
{
  ...
  for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
       so = GET_NEXT(so))
    {
      ...
      bool is_ship_disabled = ((shipp->flags & SF_DEPARTING) != 0)
        || (ship_subsys_disrupted(shipp,
                                  SUBSYSTEM_ENGINE));
      bool is_ship_disarmed =
        shipp->subsys_info[SUBSYSTEM_TURRET].current_hits <= 0.0f;

      if (is_ship_dying || is_ship_departing
          || (is_ship_disabled && is_ship_disarmed))
        continue;
