    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
              else if (!strnicmp("#End", token, 4)
                       || !strnicmp("#Beam", token, 5)
                       || !strnicmp("#Primary", token, 8)
                       || !strnicmp("#Secondary", token, 10))
...
}  void print_instructions() 
{
    ...
    ("Encrypt:   scramble [-st | -wt | -fs1 | -8bit] <filename_in> [filename_out] \n");
    ...
    ("   -st		Preprocess ships.tbl (for FS1 Demo version only!!)\n");
  printf
    ("   -wt		Preprocess weapons.tbl (for FS1 Demo version only!!)\n");
  printf
    ("   -fs1		Use 7bit, FreeSpace 1 style encryption (default=no)\n");
  printf
    ("   -8bit	Use 8bit, FreeSpace 1 style encryption (default=no)\n\n");
...
int main(int argc, char *argv[]) 
{
    ...
    
    {
      ...
      if (!stricmp("-u", argv[1]) || !stricmp("-i", argv[1])
           || !stricmp("-st", argv[1]) || !stricmp("-wt", argv[1])
           || !stricmp("-fs1", argv[1]) || !stricmp("-8bit", argv[1]))
