...
int dds_read_header(char *filename, CFILE * img_cfp, int *width, int *height,
                    int *bpp, int *compression_type, int *levels, int *size,
                    ubyte * palette)
{
      ...
      if (dds_header.dwPitchOrLinearSize <= 0)
        {
          ...
          d_size =
            ((dds_header.dwWidth + 3) / 4) * ((dds_header.dwHeight +
                                               3) / 4) *
            ((dds_header.dwDepth +
              3) / 4) * ((dds_header.ddpfPixelFormat.dwFourCC ==
                          FOURCC_DXT1) ? 8 : 16);
        }
      else
        {
      ...
      for (i = 1; i < (int) dds_header.dwMipMapCount; i++)
        {
          ...
          d_size +=
            ((d_width + 3) / 4) * ((d_height +
                                    3) / 4) *
            ((dds_header.ddpfPixelFormat.dwFourCC == FOURCC_DXT1) ? 8 : 16);
        }
  ...
  else if (dds_header.ddpfPixelFormat.
...
void dds_save_image(int width, int height, int bpp, int num_mipmaps,
                    ubyte * data, int cubemap, char *filename)
{
  DDSURFACEDESC2 dds_header;
