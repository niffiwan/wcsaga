...
bool CanAutopilotPos(vec3d targetPos)
{
  ...
  for (ship_obj * so = GET_FIRST(&Ship_obj_list);
       so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so))
    {
      object *other_objp = &Objects[so->objnum];
      // attacks player?
      if (iff_x_attacks_y(obj_team(other_objp), obj_team(Player_obj)) && !(Ship_info[Ships[other_objp->instance].ship_info_index].flags & SIF_CARGO))   // ignore cargo
        {
          // Cannot autopilot if enemy within 5,000 meters
  ...
  for (int n = 0; n < MAX_ASTEROIDS; n++)
    {
      ...
      if (Asteroids[n].flags & AF_USED)
        {
          // Cannot autopilot if asteroid within 1,000 meters
          if (vm_vec_dist_quick(&targetPos, &Objects[Asteroids[n].objnum].pos)
              < 1000)
            return false;
        }
    }
          ...
          for (j = 0; j < MAX_AI_GOALS; j++)
            {
              ...
              if (((aigp->ship_name != NULL)
                   && !stricmp(aigp->ship_name, goal_name))
                  && (aigp->ai_mode == goal))
                {
