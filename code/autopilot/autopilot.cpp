  ...
  bool CanAutopilot(bool send_msg) 
{
    ...
    if (vm_vec_dist_quick(&Player_obj->pos, Navs[CurrentNav].GetPosition()) <
        1000)
    ...
    
    {
        ...
        
        {
            ...
            if (vm_vec_dist_quick
                (&Player_obj->pos, &Objects[Asteroids[n].objnum].pos) < 1000)
  ...
  bool CanAutopilotPos(vec3d targetPos) 
{
    ...
    
    {
        ...
        
        {
            ...
            // Cannot autopilot if asteroid within 1,000 meters
            if (vm_vec_dist_quick
                (&targetPos, &Objects[Asteroids[n].objnum].pos) < 1000)
  ...
  capship_placed[0] = 0;
    ...
    
    {
           ...
           (Ships[i].flags2 & SF2_NAVPOINT_CARRY || 
            (Ships[i].wingnum != -1
             && Wings[Ships[i].wingnum].flags & WF_NAV_CARRY) ) )
    ...
    
    {
        ...
        
        {
            ...
            if (Ship_info[Ships[i].ship_info_index].flags 
                &(SIF_CRUISER | SIF_CAPITAL | SIF_SUPERCAP | SIF_CORVETTE |
                  SIF_AWACS | SIF_GAS_MINER | SIF_FREIGHTER | SIF_TRANSPORT))
            ...
            
            {
                ...
                //vm_vec_sub(&norm1, Navs[CurrentNav].GetPosition(), &Player_obj->pos);
                vm_vector_2_matrix(&Objects[Ships[i].objnum].orient, &norm1,
                                   NULL, NULL);
                ...
                && Player_obj != &Objects[Ships[i].objnum])     //only if not player object
            {
              ...
              int wingnum = aip->wing, wing_index =
                ...
                
                {
                    ...
                    get_absolute_wing_pos_autopilot(&goal_point, leader_objp,
                                                    wing_index,
                                                    aip->
                                                    ai_flags &
                ...
                
                {
                    ...
                    vm_vec_dist_quick(&Player_obj->pos,
            ...
            
            {
                ...
                
                {
                                           ...
                                           AI_GOAL_WAYPOINTS_ONCE, 0,
                                           ((waypoint_list *)
                                            Navs[CurrentNav].target_obj)->
                                           name, &Ai_info[Ships[i].ai_index]);
                ...
                
                {
                                           ...
                                           AI_GOAL_FLY_TO_SHIP, 0,
                                           ((ship *) Navs[CurrentNav].
                                            target_obj)->ship_name,
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
              if (Ship_info[Ships[capIndexes[i]].ship_info_index].
                   flags & (SIF_CAPITAL | SIF_SUPERCAP))
                
                {
                    ...
                    vm_vec_scale(&right,
                                 (1 +
                                  ((float)
                                   floor((float) capship_placed[0] / 3))));
              ...
              else if (Ship_info[Ships[capIndexes[i]].ship_info_index].
                       flags & SIF_CORVETTE)
                
                {
                    ...
                    vm_vec_scale(&right,
                                 (1 +
                                  ((float)
                                   floor((float) capship_placed[1] / 3))));
                ...
                
                {
                    ...
                    vm_vec_scale(&right,
                                 2 * (1 +
                                      ((float)
                                       floor((float) capship_placed[2] /
                                             3))));
                  vm_vec_scale(&front,
                                2 * (1 +
                                     ((float)
                                      floor((float) capship_placed[2] / 3))));
                ...
                vm_vec_add(&Objects[Ships[capIndexes[i]].objnum].pos,
                            &Player_obj->pos, &offset);
...
 void nav_warp(bool prewarp = false) 
{
    ...
    
    {
        ...
        {
          vm_vec_add(&Objects[Ships[i].objnum].pos,
                      &Objects[Ships[i].objnum].pos, &targetPos);
        ...
        
        {
            ...
            
            {
                ...
                
                {
                  ...
                    if (timestamp() >= EndAPCinematic
                           || Autopilot_AutoDiable())
...
void parse_autopilot_table(char *filename) 
{
  ...
  char *msg_tags[] =
    { "$No Nav Selected:", "$Gliding:", "$Too Close:", "$Hostiles:",
"$Linked:", "$Hazard:" };
...
 unsigned int DistanceTo(int nav) 
{
  ...
   return (uint) vm_vec_dist_quick(&Player_obj->pos,
                                     Navs[nav].GetPosition());
