...
anim_instance *anim_play ( anim_play_struct *aps )
{
	...
	if ( aps->start_at > 0 )
	{
		...
		if ( key > instance->frame_num )
		{
			// best key is closer than current position
...
int anim_show_next_frame ( anim_instance *instance, float frametime )
{
	...
	if ( instance->direction == ANIM_DIRECT_FORWARD )
	{
		if ( instance->frame_num >= instance->stop_at )
		{
			...
			else if ( instance->ping_pong )
			{
				// pingponged animations
				instance->frame_num = instance->stop_at;
				// instance->time_elapsed = 0.0f;
			...
			if ( anim_instance_is_streamed ( instance ) )
			{
				if ( instance->xlate_pal )
				{
					temp_file_offset = unpack_frame_from_file ( instance, instance->frame, instance->parent->width *
					                   instance->parent->height, instance->parent->palette_translation, aabitmap, bpp );
				}
				else
				{
					temp_file_offset = unpack_frame_from_file ( instance, instance->frame, instance->parent->width *
					                   instance->parent->height, NULL, aabitmap, bpp );
				}
			}
			else
			{
