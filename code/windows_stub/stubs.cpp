               ...
               ...) 
{
    ...
    mprintf(("ASSERTION: \"%s\" at %s:%d  %s\n", text,
             strrchr(filename, '/') + 1, line, buffer));
...
int _getcwd(char *out_buf, unsigned int len) 
{
    ...
    {
      Error(__FILE__, __LINE__, "buffer overflow in getcwd (buf size = %u)",
             len);
...
int _chdir(const char *path) 
{
  ...
  int m_error = errno;
    ...
    {
      Warning(__FILE__, __LINE__, "Cannot chdir to %s: %s", path,
               strerror(m_error));
    ...
    
    {
        ...
        
        {
          ...
          int m_error = errno;
            ...
            {
              Warning(__FILE__, __LINE__, "Cannot mkdir %s: %s", tmp_path,
                       strerror(m_error));
