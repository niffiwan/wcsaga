              ...
              if (!shipfx_eye_in_shadow(&Eye_position, Viewer_obj, idx))
                {
                  ...
                  if (stars_sun_has_glare(idx))
                    {
                      float dot =
                        vm_vec_dot(&light_dir,
                                   &Eye_matrix.vec.fvec) * 0.5f + 0.5f;
  ...
  if (time > -duration)
    {
      ...
      if (time < TIME_UP)
      ...
      else
        {
          time -= TIME_UP;
          Big_expl_flash.cur_flash_intensity =
            Big_expl_flash.max_flash_intensity * ((float) TIME_DOWN -
                                                  time) / (float) TIME_DOWN;
        }
    }
...
DCF_BOOL(i_framerate, Interface_framerate) DCF(warp, "Tests warpin effect")
{
  if (Dc_command)
    {
      bool warpin = true;
...
DCF(force_fullscreen, "Forces game to startup in fullscreen mode")
{
  ...
  os_config_write_uint(NULL, NOX("ForceFullscreen"),
...
{
#ifdef _WIN32
  const char *launcher_link =
    "explorer.exe \"http://www.randomtiger.pwp.blueyonder.co.uk/freespace/Launcher5.rar\"";
...
void game_init()
{
  ...
  if (SAGA_VERSION_BUILD == 0 && SAGA_VERSION_REVIS == 0)
  ...
  else if (SAGA_VERSION_REVIS == 0)
    {
      mprintf(("FreeSpace version: %i.%i.%i\n", SAGA_VERSION_MAJOR,
               SAGA_VERSION_MINOR, SAGA_VERSION_BUILD));
    }
  else
    {
      mprintf(("FreeSpace version: %i.%i.%i.%i\n", SAGA_VERSION_MAJOR,
               SAGA_VERSION_MINOR, SAGA_VERSION_BUILD, SAGA_VERSION_REVIS));
    }
  ...
  if (!Is_standalone)
    {
      UserSampleRate =
        (ushort) os_config_read_uint(NULL, "SoundSampleRate", 44100);
      UserSampleBits =
        (ushort) os_config_read_uint(NULL, "SoundSampleBits", 16);
      ...
      if (Cmdline_query_speech)
        {
          if (!fsspeech_was_compiled())
            MessageBox((HWND) os_get_window(),
                       ...
                       "FS2_Open Warning", MB_ICONWARNING);
          else
            MessageBox((HWND) os_get_window(),
  ...
  if (gr_init() == false)
    {
      ...
      MessageBox(NULL, "Error intializing graphics!", "Error",
      ...
      os_config_write_string(NULL, NOX("VideocardFs2open"),
                             NOX("OGL -(1024x768)x16 bit"));
  ...
  if (Cmdline_voice_recognition)
    {
      bool voiceRectOn =
        VOICEREC_init((HWND) os_get_window(), WM_RECOEVENT, GRAMMARID1,
                      IDR_CMD_CFG);

      if (voiceRectOn == false)
        {
          MessageBox((HWND) os_get_window(), "Failed to init voice rec",
                     "Error", MB_OK);
        }
    }
...
MONITOR(NumPolys) MONITOR(NumVerts) MONITOR(BmpUsed) MONITOR(BmpNew)
          ...
          if (pss->system_info->type == SUBSYSTEM_TURRET)
            {
              if (pss->turret_enemy_objnum == -1)
                ...
                gr_printf(10, t * 10, "Turret %d: %s", t,
                          Ships[Objects[pss->turret_enemy_objnum].instance].
                          ship_name);
              else
                gr_printf(10, t * 10, "Turret %d: <Object %d>", t,
                          pss->turret_enemy_objnum);
  ...
  if (Cmdline_show_stats && HUD_draw)
    {
      ...
      if (((int) Mem_starttime_phys == -1)
          || ((int) mem_stats.dwAvailPhys == -1))
        sprintf(mem_buffer, "Using Physical: *** (>4G)");
      else
        sprintf(mem_buffer, "Using Physical: %d Meg",
                (Mem_starttime_phys - mem_stats.dwAvailPhys) / 1024 / 1024);

      gr_string(20, 120, mem_buffer);
      sprintf(mem_buffer, "Using Pagefile: %d Meg",
              (Mem_starttime_pagefile -
               mem_stats.dwAvailPageFile) / 1024 / 1024);
      gr_string(20, 130, mem_buffer);
      sprintf(mem_buffer, "Using Virtual:  %d Meg",
              (Mem_starttime_virtual -
               mem_stats.dwAvailVirtual) / 1024 / 1024);
      gr_string(20, 140, mem_buffer);

      if (((int) mem_stats.dwAvailPhys == -1)
          || ((int) mem_stats.dwTotalPhys == -1))
        sprintf(mem_buffer, "Physical Free: *** / *** (>4G)");
      else
        sprintf(mem_buffer, "Physical Free: %d / %d Meg",
                mem_stats.dwAvailPhys / 1024 / 1024,
                mem_stats.dwTotalPhys / 1024 / 1024);

      gr_string(20, 160, mem_buffer);
      sprintf(mem_buffer, "Pagefile Free: %d / %d Meg",
              mem_stats.dwAvailPageFile / 1024 / 1024,
              mem_stats.dwTotalPageFile / 1024 / 1024);
      gr_string(20, 170, mem_buffer);
      sprintf(mem_buffer, "Virtual Free:  %d / %d Meg",
              mem_stats.dwAvailVirtual / 1024 / 1024,
              mem_stats.dwTotalVirtual / 1024 / 1024);
      ...
      if (Timing_total > 0.01f)
        {
          ...
          gr_printf(sx, sy, NOX("GAME: %.0f%%"),
                    (Timing_total -
                     (Timing_render2 + Timing_render3 + Timing_flip +
                      Timing_clear)) * 100.0f / Timing_total);
          sy += dy;
        }
    }
  ...
  if (Show_mem)
    {
      ...
      gr_printf(sx, sy, NOX("%s: %d KB\n"),
                (Cmdline_cache_bitmaps) ? NOX("C-BMP") : NOX("BMP"),
                bm_texture_ram / 1024);
      sy += dy;
  ...
  if (Show_player_pos)
    {
      ...
      sx = 320;
      sy = 100;
      gr_printf(sx, sy, NOX("Player Pos: (%d,%d,%d)"),
...
void game_show_eye_pos(camid cid)
{
  ...
  //Position
  gr_printf(20, 100 - font_height, "X:%f Y:%f Z:%f", cam_pos.xyz.x,
            cam_pos.xyz.y, cam_pos.xyz.z);
  ...
  rot_angles.b *= (180 / PI);
  rot_angles.h *= (180 / PI);
  gr_printf(20, 100 - font_height, "Xr:%f Yr:%f Zr:%f", rot_angles.p,
            rot_angles.b, rot_angles.h);
}
      ...
      if (g3_in_frame() == 0)
        {
          ...
          gr_rect(0, gr_screen.max_h - yborder, gr_screen.max_w, yborder,
                  false);
        }
      ...
      if (View_percent >= 100)
      ...
      else
        {
          ...
          gr_set_clip(xborder, yborder, gr_screen.max_w - xborder * 2,
                      gr_screen.max_h - yborder * 2, false);
        }
    }
}
  ...
  if (tst == 3)
    {
      ...
      // offscreen
      if (!
          ((v.sx >= 0) && (v.sx <= gr_screen.max_w) && (v.sy >= 0)
              ...
              && (Net_players[idx].m_player->objnum < MAX_OBJECTS))
            {
              ...
              if (!NETPLAYER_IS_DEAD(np)
                  && !(Ships[Objects[np->m_player->objnum].instance].
                       flags & (SF_DYING | SF_DEPARTING)))
                {
                  ai_do_repair_frame(&Objects
                                     [Net_players[idx].m_player->objnum],
                                     &Ai_info[Ships
                                              [Objects
                                               [Net_players[idx].m_player->
                                                objnum].instance].ai_index],
                                     frametime);
                }
            }
        }
              ...
              strcpy_s(view_target_name,
                       Weapon_info[Weapons
                                   [Objects[Player_ai->target_objnum].
                                    instance].weapon_info_index].name);
          ...
          if (strlen(view_target_name))
            {
              HUD_fixed_printf(0.0f, XSTR("Viewing %s%s\n", 185),
                               (Viewer_mode & VM_OTHER_SHIP) ? XSTR("from ",
                                                                    186) : "",
                               view_target_name);
  ...
  if (Viewer_obj == Player_obj)
    {
      ...
      if (!timestamp_elapsed(pi->afterburner_decay))
        {
          tangles.p +=
            get_shake(0.07f, timestamp_until(pi->afterburner_decay),
                      ABURN_DECAY_TIME);
          tangles.h +=
            get_shake(0.07f, timestamp_until(pi->afterburner_decay),
                      ABURN_DECAY_TIME);
      ...
      if (Game_shudder_time != -1)
        {
          if (timestamp_elapsed(Game_shudder_time))
            ...
            {
              tangles.p +=
                get_shake(Game_shudder_intensity * 0.005f,
                          timestamp_until(Game_shudder_time),
                          Game_shudder_total);
              tangles.h +=
                get_shake(Game_shudder_intensity * 0.005f,
                          timestamp_until(Game_shudder_time),
...
inline void render_environment(int i, vec3d * eye_pos, matrix * new_orient,
                               float new_zoom)
{
  ...
  gr_set_proj_matrix(PI_2 * new_zoom, 1.0f, Min_draw_distance,
                     Max_draw_distance);
  ...
  if (Game_mode & GM_DEAD)
    {
      ...
      if (Player_ai->target_objnum != -1)
        ...
        {
          dist =
            vm_vec_normalized_dir(&vec_to_deader, &Player_obj->pos,
                                  &Dead_camera_pos);
      ...
      if (!(Game_mode & (GM_DEAD | GM_DEAD_BLEW_UP)))
        {
          ...
          if (Viewer_mode & VM_FREECAMERA)
          ...
          else if (Viewer_mode & VM_EXTERNAL)
            {
              ...
              vm_vector_2_matrix(&eye_orient, &tmp_dir,
                                 &Viewer_obj->orient.vec.uvec, NULL);
              Viewer_obj = NULL;
          ...
          else if (Viewer_mode & VM_CHASE)
            {
              ...
              if (Viewer_obj->phys_info.speed < 62.5f)
                ...
                {
                  move_dir = Viewer_obj->phys_info.vel;
                  vm_vec_scale(&move_dir,
                               (62.5f / Viewer_obj->phys_info.speed));
                }
              ...
              if (Viewer_obj == Player_obj)
                {
                  ...
                  vm_vec_copy_scale(&aim_pt, &Viewer_obj->orient.vec.fvec,
                                    1000.0f);
                  vm_vec_add2(&aim_pt, &Viewer_obj->pos);

                  vm_vec_scale_add(&eye_pos, &Viewer_obj->pos, &move_dir,
                                   -0.02f * Viewer_obj->radius);
                  vm_vec_scale_add2(&eye_pos, &Viewer_obj->orient.vec.fvec,
                                    -2.125f * Viewer_obj->radius -
                                    Viewer_chase_info.distance);
                  vm_vec_scale_add2(&eye_pos, &Viewer_obj->orient.vec.uvec,
              ...
              vm_vec_scale_add2(&tmp_up, &Viewer_obj->orient.vec.rvec,
                                0.00001f);
          ...
          else if (Viewer_mode & VM_WARP_CHASE)
            {
              ...
              vm_vec_normalize(&tmp_dir);
              vm_vector_2_matrix(&eye_orient, &tmp_dir,
                                 &Player_obj->orient.vec.uvec, NULL);
              Viewer_obj = NULL;
            }
        ...
        if (turret->type == SUBSYSTEM_TURRET)
          {
            ...
            model_find_world_dir(&fvec, &turret->turret_matrix.vec.fvec,
                                 turret->model_num, turret->turret_gun_sobj,
                                 &tobj->orient, NULL);
            model_find_world_dir(&uvec, &turret->turret_matrix.vec.uvec,
                                 turret->model_num, turret->turret_gun_sobj,
                                 &tobj->orient, NULL);
...
void game_simulation_frame()
{
  ...
  if ((Game_mode & GM_MULTIPLAYER)
      && !(Net_player->flags & NETINFO_FLAG_AM_MASTER)
      && !multi_endgame_ending() && !(Netgame.flags & NG_FLAG_SERVER_LOST))
    {
...
void game_render_hud_3d(camid cid)
{
  ...
  if ((Game_detail_flags & DETAIL_FLAG_HUD)
      && (supernova_active() <
          3)
      /* && !(Game_mode & GM_MULTIPLAYER) || ( (Game_mode & GM_MULTIPLAYER) && !(Net_player->flags & NETINFO_FLAG_OBSERVER) ) */
      )
    {
...
void game_shade_frame(float frametime)
{
  ...
  gr_flash_alpha(Viewer_shader.r, Viewer_shader.g, Viewer_shader.b,
                 Viewer_shader.c);
}
...
void bars_do_frame(float frametime)
{
  if ((Cutscene_bar_flags & CUB_GRADUAL) && Cutscene_bars_progress < 1.0f)
  ...
  else if (Cutscene_bar_flags & CUB_CUTSCENE)
    {
      ...
      gr_set_clip(0, yborder, gr_screen.max_w,
                  gr_screen.max_h - (yborder * 2), false, false);
...
void game_frame(int paused)
{
  ...
  DEBUG_GET_TIME(total_time1) if (paused)
    {
      ...
      if (!(Game_mode & GM_STANDALONE_SERVER))
        {
          ...
          if (Game_mode & GM_MULTIPLAYER)
            {
              cid.getCamera()->get_info(&Net_player->s_info.eye_pos,
          ...
          if (Script_system.IsOverride(Script_hudhook)
              || Script_system.IsConditionOverride(CHA_HUDDRAW, Viewer_obj))
          ...
          DEBUG_GET_TIME(render3_time2)
            DEBUG_GET_TIME(render2_time1) gr_reset_clip();
          game_get_framerate();
          game_show_framerate();
          ...
          if (!hud_disabled_except_messages()
              && !(Viewer_mode &
                   (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE |
                    VM_PADLOCK_ANY)))
            {
          ...
          if (!popup_running_state())
            {
              DEBUG_GET_TIME(flip_time1) game_flip_page_and_time_it();
...
void set_time_compression(float multiplier, float change_time)
{
  ...
  Time_compression_change_rate =
    fl2f(f2fl(Desired_time_compression - Game_time_compression) /
         change_time);
}
...
void game_enter_state(int old_state, int new_state)
{
  ...
  switch (new_state)
    ...
    case GS_STATE_GAME_PLAY:
      ...
      joy_ff_mission_init(Ship_info[Player_ship->ship_info_index].
                          rotation_time);
      ...
      if (allowed_to_run)
        {
          ...
          int msgbox_rval;
          msgbox_rval =
            MessageBox(NULL, tmp, XSTR("Not Enough RAM", 194), MB_OKCANCEL);
  ...
  if (CopyFile(src_filename, dest_filename, 0) == 0)
    {
      MessageBox(NULL,
                 ...
                 ("Unable to copy freespace.exe from update directory to installed directory.  You should copy freespace.exe from the update directory (located in your FreeSpace install directory) to your install directory",
                  988), NULL, MB_OK | MB_TASKMODAL | MB_SETFOREGROUND);
      return;
    }
...
void game_spew_pof_info_sub(int model_num, polymodel * pm, int sm,
                            CFILE * out, int *out_total,
  ...
  if (ms.dwTotalVirtual < 1024)
    {
      MessageBox(NULL,
                 ...
                 XSTR("No Virtual Memory", 197), MB_OK);
      return 1;
    }
                   ...
                   int nCmdShow)
{
  ...
  snprintf(userdir, MAX_PATH - 1, "%s%s%s%s%s%s", detect_home(),
           DIR_SEPARATOR_STR, Osreg_company_name, DIR_SEPARATOR_STR,
           Osreg_user_dir, DIR_SEPARATOR_STR);
  ...
  __except(RecordExceptionInfo
           (GetExceptionInformation(), "Wing Commander Saga Main Thread"))
  {
...
{
  if (Steering_cursor_bitmap != -1
      && (gameseq_get_state() == GS_STATE_GAME_PLAY)
      && (Use_mouse_to_fly == 1) && !(popup_active()
                                      || popupdead_is_active()))
    {
      ...
      if (am->first_frame != -1)
        {
          ...
          am->current_frame =
            fl2i((am->elapsed_time / am->time) * (am->num_frames - 1));
  ...
  switch (game_state)
    ...
    case GS_STATE_GAME_PLAY:
      if (popup_active() || popupdead_is_active()
          || ((Use_mouse_to_fly == 1) && !(Viewer_mode & VM_NOT_COCKPIT)))
        {
          Mouse_hidden = 0;
        }
      else
...
{
  popup(PF_USE_AFFIRMATIVE_ICON | PF_BODY_BIG, 1, POPUP_OK,
        XSTR("Sorry, this feature is currently not supported.", -1));
}
...
void get_version_string(char *str, int max_size)
{
  ...
  if (SAGA_VERSION_BUILD == 0 && SAGA_VERSION_REVIS == 0)
    {
      sprintf(str, "Wing Commander Saga v%i.%i", SAGA_VERSION_MAJOR,
              SAGA_VERSION_MINOR);
    }
  else if (SAGA_VERSION_REVIS == 0)
    {
      sprintf(str, "Wing Commander Saga v%i.%i.%i", SAGA_VERSION_MAJOR,
              SAGA_VERSION_MINOR, SAGA_VERSION_BUILD);
    }
  else
    {
      sprintf(str, "Wing Commander Saga v%i.%i.%i.%i", SAGA_VERSION_MAJOR,
              SAGA_VERSION_MINOR, SAGA_VERSION_BUILD, SAGA_VERSION_REVIS);
    }
  ...
  if (Oem_upsell_screen_number == (NUM_OEM_UPSELL_SCREENS - 1))
    {
      ...
      Oem_upsell_show_next_bitmap_time =
        timer_get_milliseconds() + OEM_UPSELL_SCREEN_DELAY * 2;
      Mouse_hidden = 0;
    ...
    {
      Oem_upsell_show_next_bitmap_time =
        timer_get_milliseconds() + OEM_UPSELL_SCREEN_DELAY;
    }
}
...
void oem_upsell_load_bitmaps()
{
  ...
  for (i = 0; i < NUM_OEM_UPSELL_SCREENS; i++)
    {
      Oem_upsell_bitmaps[gr_screen.res][i] =
        bm_load(Oem_upsell_bitmap_filenames[gr_screen.res][i]);
    }
}
  ...
  while (!done)
    {
      ...
      if (Oem_upsell_bitmaps[gr_screen.res][Oem_upsell_screen_number] >= 0)
        {
          gr_set_bitmap(Oem_upsell_bitmaps[gr_screen.res]
                        [Oem_upsell_screen_number]);
          gr_bitmap(0, 0);
        }
  ...
  if (Demo_upsell_screen_number == (NUM_DEMO_UPSELL_SCREENS - 1))
    {
      Demo_upsell_show_next_bitmap_time =
        timer_get_milliseconds() + DEMO_UPSELL_SCREEN_DELAY * 4;
    }
  else
    {
      Demo_upsell_show_next_bitmap_time =
        timer_get_milliseconds() + DEMO_UPSELL_SCREEN_DELAY;
    }
...
void demo_upsell_load_bitmaps()
{
  ...
  for (i = 0; i < NUM_DEMO_UPSELL_SCREENS; i++)
    {
      Demo_upsell_bitmaps[gr_screen.res][i] =
        bm_load(Demo_upsell_bitmap_filenames[gr_screen.res][i]);
    }
}
  ...
  while (!done)
    {
      ...
      if (Demo_upsell_bitmaps[gr_screen.res][Demo_upsell_screen_number] >= 0)
        {
          gr_set_bitmap(Demo_upsell_bitmaps[gr_screen.res]
                        [Demo_upsell_screen_number]);
          gr_bitmap(0, 0);
        }
  ...
  if (Subspace_ambient_left_channel < 0)
    {
      Subspace_ambient_left_channel =
        snd_play_looping(&Snds[SND_SUBSPACE_LEFT_CHANNEL], -1.0f);
    }

  if (Subspace_ambient_right_channel < 0)
    {
      Subspace_ambient_right_channel =
        snd_play_looping(&Snds[SND_SUBSPACE_RIGHT_CHANNEL], 1.0f);
    }
}
  ...
  do
    {
      ...
      if ((find.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
          && stricmp(find.cFileName, ".") && stricmp(find.cFileName, ".."))
        {
          // subsearch
      ...
      if (GetDriveType(path) == DRIVE_CDROM)
        {
          ...
          if (GetVolumeInformation
              (path, volume, 256, NULL, NULL, NULL, NULL, 0) == TRUE)
            {
              ...
              if (volume_name)
                {
                  // volume 1
                  if (!stricmp(volume_name, FS_CDROM_VOLUME_1)
                      && volume1_present)
                    {
                      volume_match = 1;
                    }
                  // volume 2
                  if (!stricmp(volume_name, FS_CDROM_VOLUME_2)
                      && volume2_present)
                    {
                      volume_match = 1;
                    }
                  // volume 3
                  if (!stricmp(volume_name, FS_CDROM_VOLUME_3)
                      && volume3_present)
                    {
                      volume_match = 1;
                    }
...
int game_cd_changed()
{
  ...
  found =
    GetVolumeInformation(Game_CDROM_dir, label, 256, NULL, NULL, NULL, NULL,
                         0);
  ...
  while (1)
    {
      ...
      if (Is_standalone)
      ...
      else
        {
          ...
          popup_rval =
            popup(PF_BODY_BIG, 1, POPUP_OK, XSTR("Please insert DVD", 1468));
#else
          popup_rval =
            popup(PF_BODY_BIG, 1, POPUP_OK, XSTR("Please insert CD %d", 1468),
                  cdnum);
#endif
          refresh_files = 1;
          if (popup_rval != 1)
      ...
      if (Game_title_bitmap != -1)
        {
          ...
          gr_bitmap((gr_screen.max_w_unscaled - width) / 2,
                    (gr_screen.max_h_unscaled - height) / 2);
        }
