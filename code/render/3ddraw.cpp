...
int g3_draw_polygon(vec3d * pos, matrix * ori, float width, float height,
                    int tmap_flags, int r, int g, int b, int a)
{
  //idiot-proof
  if (width == 0 || height == 0)
    return 0;
...
int g3_draw_bitmap_3d_v(vertex * pnt, int orient, float rad, uint tmap_flags,
                        float depth, float c)
{
  //return 0;
  //  rad *= 1.41421356f;//1/0.707, becase these are the points of a square or width and hieght rad
...
int g3_draw_bitmap_3d_volume(vertex * pnt, int orient, float rad,
                             uint tmap_flags, float depth, int resolution)
{
  float s = 1.0f;
...
void flash_ball::initialize(int number, float min_ray_width,
                            float max_ray_width, vec3d * dir, vec3d * pcenter,
                            float outer, float inner, ubyte max_r,
                            ubyte max_g, ubyte max_b, ubyte min_r,
                            ubyte min_g, ubyte min_b)
{
  if (number < 1)
    return;
