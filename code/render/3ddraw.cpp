...
 int do_facing_check(vec3d * norm, vertex ** vertlist, vec3d * p) 
{
  ...
  else                          //normal not specified, so must compute
    {
        ...
        vm_vec_perp(&tempv, (vec3d *) & vertlist[0]->x,
                     (vec3d *) & vertlist[1]->x, (vec3d *) & vertlist[2]->x);
...
 int g3_draw_polygon(vec3d * pos, matrix * ori, float width, float height,
                       int tmap_flags) 
{
...
 int g3_draw_polygon(vec3d * pos, vec3d * norm, float width, float height,
                       int tmap_flags) 
{
...
//alternate method
int g3_draw_bitmap_3d(vertex * pnt, int orient, float rad, uint tmap_flags,
                      float depth) 
{
...
 int g3_draw_bitmap_3d_v(vertex * pnt, int orient, float rad,
                           uint tmap_flags, float depth, float c) 
{
...
 int g3_draw_bitmap_3d_volume(vertex * pnt, int orient, float rad,
                                uint tmap_flags, float depth,
                                int resolution) 
{
...
int g3_get_bitmap_dims(int bitmap, vertex * pnt, float rad, int *x, int *y,
...
//alternate method
int g3_draw_rotated_bitmap_3d(vertex * pnt, float angle, float rad,
                              uint tmap_flags, float depth) 
{
  ...
   vm_rot_point_around_line(&uvec, &View_matrix.vec.uvec, angle,
                              &vmd_zero_vector, &fvec);
...
//draws a bitmap with the specified 3d width & height
//returns 1 if off screen, 0 if drew
int g3_draw_rotated_bitmap(vertex * pnt, float angle, float rad,
                           uint tmap_flags, float depth) 
{
...
//Set TMAP_FLAG_TEXTURED in the tmap_flags to texture map it with current texture.
//returns 1 if off screen, 0 if drew
float g3_draw_poly_constant_sw_area(int nv, vertex ** pointlist,
                                    uint tmap_flags, float constant_sw,
                                    float area) 
{
...
void g3_draw_horizon_line() 
{
  ...
  //  int sky_ground_flag;    //0=both, 1=all sky, -1=all gnd
    ...
    Unscaled_matrix.vec.rvec.xyz.y * Matrix_scale.xyz.y * Matrix_scale.xyz.z;
  horizon_vec.xyz.y =
    Unscaled_matrix.vec.uvec.xyz.y * Matrix_scale.xyz.x * Matrix_scale.xyz.z;
  horizon_vec.xyz.z =
    Unscaled_matrix.vec.fvec.xyz.y * Matrix_scale.xyz.x * Matrix_scale.xyz.y;
...
int g3_draw_rod(vec3d * p0, float width1, vec3d * p1, float width2,
                vertex * verts, uint tmap_flags) 
{
...
int g3_draw_perspective_bitmap(angles * a, float scale_x, float scale_y,
                                int div_x, int div_y, uint tmap_flags) 
{
...
 void g3_draw_2d_rect(int x, int y, int w, int h, int r, int g, int b,
                        int a) 
{
  ...
  &v[0], &v[1], &v[2], &v[3]};
    ...
    // draw the polys
    g3_draw_poly_constant_sw(4, verts,
                             TMAP_FLAG_GOURAUD | TMAP_FLAG_RGB |
...
int g3_draw_2d_poly_bitmap(float x, float y, float w, float h,
                           uint additional_tmap_flags) 
{
  ...
  &v[0], &v[1], &v[2], &v[3]};
    ...
    // set debrief
    ret =
    g3_draw_poly_constant_sw(4, vertlist,
                             TMAP_FLAG_TEXTURED | additional_tmap_flags,
                             0.1f);
...
 int g3_draw_2d_poly_bitmap_list(bitmap_2d_list * b_list, int n_bm,
                                   uint additional_tmap_flags) 
{
    ...
    // set debrief
    ret =
    g3_draw_poly_constant_sw(6 * n_bm, bitmap_2d_poly_vertlist,
                             TMAP_FLAG_TEXTURED | TMAP_FLAG_TRILIST |
...
int g3_draw_2d_poly_bitmap_rect_list(bitmap_rect_list * b_list, int n_bm,
                                     uint additional_tmap_flags) 
{
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//
//flash ball stuff
void flash_ball::initialize(int number, float min_ray_width,
                             float max_ray_width, vec3d * dir,
                             vec3d * pcenter, float outer, float inner,
                             ubyte max_r, ubyte max_g, ubyte max_b,
                             ubyte min_r, ubyte min_g, ubyte min_b) 
{
...
  void flash_ball::initialize(ubyte * bsp_data, float min_ray_width,
                                 float max_ray_width, vec3d * dir,
                                 vec3d * pcenter, float outer, float inner,
                                 ubyte max_r, ubyte max_g, ubyte max_b,
                                 ubyte min_r, ubyte min_g, ubyte min_b) 
...
void flash_ball::render(float rad, float intinsity, float life) 
{
    ...
    
    {
      ...
      vm_vec_interp_constant(&end, (vec3d *) & ray[i].start.x,
                              (vec3d *) & ray[i].end.x, life);
      vm_vec_scale(&end, rad);
      vm_vec_add2(&end, &center);
      flash_ball::batcher.draw_beam(&center, &end, ray[i].width * rad,
                                     intinsity);
    } flash_ball::batcher.
    render(TMAP_FLAG_TEXTURED | TMAP_FLAG_XPARENT | TMAP_HTL_3D_UNLIT |
