...
*/
     float g3_draw_laser_htl(vec3d * p0, float width1, vec3d * p1,
...
float g3_draw_laser(vec3d * headp, float head_width, vec3d * tailp,
                    float tail_width, uint tmap_flags, float max_len)
{
  if (!Lasers)
    {
      return 0.0f;
  ...
  if (!Cmdline_nohtl && (tmap_flags & TMAP_HTL_3D_UNLIT))
    {
      return g3_draw_laser_htl(headp, head_width, tailp, tail_width, 255, 255,
                               255, tmap_flags | TMAP_HTL_3D_UNLIT);
    }
  ...
  float len_2d =
    fl_sqrt((tailx - headx) * (tailx - headx) +
  ...
  if ((!Cmdline_nohtl) && tmap_flags & TMAP_HTL_3D_UNLIT)
    {
      ...
      return
        g3_draw_laser_htl(headp, head_width, tailp, tail_width, r, g, b,
                          tmap_flags | TMAP_HTL_3D_UNLIT);
    }
  float headx, heady, headr, tailx, taily, tailr;
  vertex pt1, pt2;
  float depth;
