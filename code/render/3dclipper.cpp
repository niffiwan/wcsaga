...
vertex *clip_edge(int plane_flag, vertex * on_pnt, vertex * off_pnt,
                  uint flags)
{
  float ratio;
  vertex *tmp;
  ...
  if (plane_flag & CC_OFF_USER)
    {
      ...
      vm_vec_sub(&ray_direction, (vec3d *) & off_pnt->x,
                 (vec3d *) & on_pnt->x);
      ...
      if (flags & TMAP_FLAG_RGB)
        {
          ...
          tmp->spec_r =
            ubyte(fl2i(onspec_r + (offspec_r - onspec_r) * ratio));
          tmp->spec_g =
            ubyte(fl2i(onspec_g + (offspec_g - onspec_g) * ratio));
          tmp->spec_b =
            ubyte(fl2i(onspec_b + (offspec_b - onspec_b) * ratio));
        }
    }
  else
    {
...
int clip_plane(int plane_flag, vertex ** src, vertex ** dest, int *nv,
               ccodes * cc, uint flags)
{
  int i;
...
vertex **clip_polygon(vertex ** src, vertex ** dest, int *nv, ccodes * cc,
                      uint flags)
{
  int plane_flag;
  vertex **t;
