...
float glide_ramp(float v_in, float v_goal, float ramp_time_const,
                 float accel_mult, float t);
...
void apply_physics(float damping, float desired_vel, float initial_vel,
                   float t, float *new_vel, float *delta_pos)
{
  if (damping < 0.0001f)
    {
      if (delta_pos)
...
void physics_sim_rot(matrix * orient, physics_info * pi, float sim_time)
{
      ...
      if (timestamp_elapsed(pi->shockwave_decay))
      ...
      else
        {
          shock_fraction_time_left =
            timestamp_until(pi->shockwave_decay) / (float) SW_BLAST_DURATION;
          rotdamp =
            pi->rotdamp + pi->rotdamp * (SW_ROT_FACTOR -
                                         1) * shock_fraction_time_left;
          shock_amplitude =
  ...
  apply_physics(rotdamp, pi->desired_rotvel.xyz.x, pi->rotvel.xyz.x, sim_time,
                &new_vel.xyz.x, NULL);
  apply_physics(rotdamp, pi->desired_rotvel.xyz.y, pi->rotvel.xyz.y, sim_time,
                &new_vel.xyz.y, NULL);
  apply_physics(rotdamp, pi->desired_rotvel.xyz.z, pi->rotvel.xyz.z, sim_time,
                &new_vel.xyz.z, NULL);
  ...
  if (pi->flags & PF_IN_SHOCKWAVE)
    {
      tangles.p +=
        (float) (myrand() - RAND_MAX_2) / RAND_MAX * shock_amplitude;
      tangles.h +=
        (float) (myrand() - RAND_MAX_2) / RAND_MAX * shock_amplitude;
    }
...
void physics_sim_rot_editor(matrix * orient, physics_info * pi,
                            float sim_time)
{
  angles tangles;
  vec3d new_vel;
  ...
  apply_physics(pi->rotdamp, pi->desired_rotvel.xyz.x, pi->rotvel.xyz.x,
                sim_time, &new_vel.xyz.x, NULL);

  apply_physics(pi->rotdamp, pi->desired_rotvel.xyz.y, pi->rotvel.xyz.y,
                sim_time, &new_vel.xyz.y, NULL);

  apply_physics(pi->rotdamp, pi->desired_rotvel.xyz.z, pi->rotvel.xyz.z,
                sim_time, &new_vel.xyz.z, NULL);
...
void physics_sim_vel(vec3d * position, physics_info * pi, float sim_time,
                     matrix * orient)
{
  ...
  if (pi->flags & PF_DEAD_DAMP)
  ...
  else if (pi->flags & PF_REDUCED_DAMP)
    {
      ...
      if (timestamp_elapsed(pi->reduced_damp_decay))
        {
          vm_vec_make(&damp, pi->side_slip_time_const,
                      pi->side_slip_time_const, 0.0f);
        }
      else
        {
          ...
          damp.xyz.y =
            pi->side_slip_time_const * (1 +
                                        ...
                                        reduced_damp_fraction_time_left);
          damp.xyz.z =
            pi->side_slip_time_const * reduced_damp_fraction_time_left *
            REDUCED_DAMP_FACTOR;
        }
    }
  else
    {
      ...
      if (pi->use_newtonian_damp)
        {
          vm_vec_make(&damp, pi->side_slip_time_const,
                      pi->side_slip_time_const, pi->side_slip_time_const);
        }
      else
        {
          vm_vec_make(&damp, pi->side_slip_time_const,
  ...
  if (excess > 5 && (pi->flags & PF_SPECIAL_WARP_IN))
  ...
  else if (pi->flags & PF_SPECIAL_WARP_OUT)
    {
      ...
      local_disp.xyz.z =
        (local_v_in.xyz.z * sim_time) + deficeit * (sim_time -
                                                    (float
                                                     (SPECIAL_WARP_T_CONST) *
                                                     (1.0f - exp_factor)));
    }
  else
    {
...
void physics_sim(vec3d * position, matrix * orient, physics_info * pi,
                 float sim_time)
{
...
void physics_sim_editor(vec3d * position, matrix * orient, physics_info * pi,
                        float sim_time)
{
  physics_sim_vel(position, pi, sim_time, orient);
...
void physics_predict_pos(physics_info * pi, float delta_time,
                         vec3d * predicted_pos)
{
  apply_physics(pi->side_slip_time_const, pi->desired_vel.xyz.x,
                pi->vel.xyz.x, delta_time, NULL, &predicted_pos->xyz.x);

  apply_physics(pi->side_slip_time_const, pi->desired_vel.xyz.y,
                pi->vel.xyz.y, delta_time, NULL, &predicted_pos->xyz.y);

  apply_physics(pi->side_slip_time_const, pi->desired_vel.xyz.z,
                pi->vel.xyz.z, delta_time, NULL, &predicted_pos->xyz.z);
}
                         ...
                         vec3d * predicted_vel)
{
  if (pi->flags & PF_CONST_VEL)
    ...
    {
      apply_physics(pi->side_slip_time_const, pi->desired_vel.xyz.x,
                    pi->vel.xyz.x, delta_time, &predicted_vel->xyz.x, NULL);

      apply_physics(pi->side_slip_time_const, pi->desired_vel.xyz.y,
                    pi->vel.xyz.y, delta_time, &predicted_vel->xyz.y, NULL);

      apply_physics(pi->side_slip_time_const, pi->desired_vel.xyz.z,
                    pi->vel.xyz.z, delta_time, &predicted_vel->xyz.z, NULL);
    }
}

// function to predict position and velocity of an object
void physics_predict_pos_and_vel(physics_info * pi, float delta_time,
                                 vec3d * predicted_vel, vec3d * predicted_pos)
{

  apply_physics(pi->side_slip_time_const, pi->desired_vel.xyz.x,
                pi->vel.xyz.x, delta_time, &predicted_vel->xyz.x,
                &predicted_pos->xyz.x);

  apply_physics(pi->side_slip_time_const, pi->desired_vel.xyz.y,
                pi->vel.xyz.y, delta_time, &predicted_vel->xyz.y,
                &predicted_pos->xyz.y);

  apply_physics(pi->side_slip_time_const, pi->desired_vel.xyz.z,
                pi->vel.xyz.z, delta_time, &predicted_vel->xyz.z,
                &predicted_pos->xyz.z);
}
...
void physics_read_flying_controls(matrix * orient, physics_info * pi,
  ...
  if (pi->flags & PF_ACCELERATES)
    {
      ...
      if (pi->flags & PF_REDUCED_DAMP)
        {
          ...
          reduced_damp_ramp_time_expansion =
            1.0f + (REDUCED_DAMP_FACTOR -
                    1) * reduced_damp_fraction_time_left;
        }
      else
        {
      ...
      if (pi->flags & PF_SLIDE_ENABLED)
        {
          ...
          pi->prev_ramp_vel.xyz.x =
            velocity_ramp(pi->prev_ramp_vel.xyz.x, goal_vel.xyz.x,
          ...
          pi->prev_ramp_vel.xyz.y =
            velocity_ramp(pi->prev_ramp_vel.xyz.y, goal_vel.xyz.y,
                          ramp_time_const, sim_time);
        }
      else
        {
      ...
      pi->prev_ramp_vel.xyz.z =
        velocity_ramp(pi->prev_ramp_vel.xyz.z, goal_vel.xyz.z,
                      ramp_time_const, sim_time);
      ...
      if (pi->flags & PF_GLIDING)
        {
          ...
          if (pi->glide_cap == 0.0f)
            {
              ...
              if (pi->flags & PF_AFTERBURNER_ON)
              ...
              else
                {
                  ...
                  curGlideCap =
                    MAX(MAX(pi->max_vel.xyz.x, pi->max_vel.xyz.y),
                        pi->max_vel.xyz.z);
                }
            }
          else
          ...
          if (curGlideCap >= 0.0f
              ...
              && vm_vec_mag(&goal_vel) > 0.0f && pi->glide_accel_mult != 0.0f)
            {
              if (goal_vel.xyz.x == 0.0f)
                vm_vec_scale_add2(&pi->desired_vel, &orient->vec.rvec,
                                  -cap_decay_amount * local_vel.xyz.x);
              if (goal_vel.xyz.y == 0.0f)
                vm_vec_scale_add2(&pi->desired_vel, &orient->vec.uvec,
                                  -cap_decay_amount * local_vel.xyz.y);
              if (goal_vel.xyz.z == 0.0f)
                vm_vec_scale_add2(&pi->desired_vel, &orient->vec.fvec,
          ...
          float xVal =
            glide_ramp(local_vel.xyz.x, goal_vel.xyz.x,
                       pi->slide_accel_time_const, pi->glide_accel_mult,
                       sim_time);
          float yVal =
            glide_ramp(local_vel.xyz.y, goal_vel.xyz.y,
                       pi->slide_accel_time_const, pi->glide_accel_mult,
                       sim_time);
          float zVal = 0.0;
          if (pi->flags & PF_AFTERBURNER_ON)
            zVal =
              glide_ramp(local_vel.xyz.z, goal_vel.xyz.z,
          ...
          else
            {
              if (goal_vel.xyz.z >= 0.0f)
                zVal =
                  glide_ramp(local_vel.xyz.z, goal_vel.xyz.z,
                             pi->forward_accel_time_const,
                             pi->glide_accel_mult, sim_time);
              else
                zVal =
                  glide_ramp(local_vel.xyz.z, goal_vel.xyz.z,
                             pi->forward_decel_time_const,
                             pi->glide_accel_mult, sim_time);
            }
...
void physics_apply_whack(vec3d * impulse, vec3d * pos, physics_info * pi,
                 ...
                 float accel_mult, float t)
{
  ...
  float decay_factor =
    (ramp_time_const >
     0.0f) ? (1.0f - (float) exp(-t / ramp_time_const)) : 1.0f;
...
void physics_apply_shock(vec3d * direction_vec, float pressure,
                         physics_info * pi, matrix * orient, vec3d * min,
                         vec3d * max, float radius)
{
  ...
  if (normal.xyz.x < 0.0f)      // normal < 0, hits the right face
    vm_vec_copy_scale(&impact_vec, &orient->vec.rvec,
                      max->xyz.x * pressure * area.xyz.x * normal.xyz.x *
                      sin.xyz.x / pi->mass);
  else                          // normal > 0, hits the left face
    vm_vec_copy_scale(&impact_vec, &orient->vec.rvec,
                      min->xyz.x * pressure * area.xyz.x * -normal.xyz.x *
                      sin.xyz.x / pi->mass);
  ...
  if (normal.xyz.y < 0.0f)
    vm_vec_copy_scale(&impact_vec, &orient->vec.uvec,
                      max->xyz.y * pressure * area.xyz.y * normal.xyz.y *
                      sin.xyz.y / pi->mass);
  else
    vm_vec_copy_scale(&impact_vec, &orient->vec.uvec,
                      min->xyz.y * pressure * area.xyz.y * -normal.xyz.y *
                      sin.xyz.y / pi->mass);
  ...
  if (normal.xyz.z < 0.0f)
    vm_vec_copy_scale(&impact_vec, &orient->vec.fvec,
                      max->xyz.z * pressure * area.xyz.z * normal.xyz.z *
                      sin.xyz.z / pi->mass);
  else
    vm_vec_copy_scale(&impact_vec, &orient->vec.fvec,
                      min->xyz.z * pressure * area.xyz.z * -normal.xyz.z *
                      sin.xyz.z / pi->mass);
...
void physics_collide_whack(vec3d * impulse, vec3d * world_delta_rotvel,
  ...
  if (!(pi->flags & PF_DEAD_DAMP))
    {
      ...
      if (fl_abs(pi->rotvel.xyz.x) > pi->max_rotvel.xyz.x)
        {
          pi->rotvel.xyz.x =
            (pi->rotvel.xyz.x / fl_abs(pi->rotvel.xyz.x)) *
      ...
      if (fl_abs(pi->rotvel.xyz.y) > pi->max_rotvel.xyz.y)
        {
          pi->rotvel.xyz.y =
            (pi->rotvel.xyz.y / fl_abs(pi->rotvel.xyz.y)) *
      ...
      if (fl_abs(pi->rotvel.xyz.z) > pi->max_rotvel.xyz.z)
        {
          pi->rotvel.xyz.z =
            (pi->rotvel.xyz.z / fl_abs(pi->rotvel.xyz.z)) *
...
void avd_movement::setAVD(float final_position, float total_movement_time,
                          ...
                          float final_velocity)
{
  ...
  Vm =
    (Pf - Pi - 0.5f * (Vi * Tai) - 0.5f * (Vf * Taf)) / (Tf - 0.5f * Tai -
                                                         0.5f * Taf);
  Ai = (Vm - Vi) / Tai;
  Af = (Vf - Vm) / Taf;
}
