      ...
      if (!(wp->weapon_flags & WF_LOCK_WARNING_PLAYED))
        {
          ...
          if (Weapon_info[wp->weapon_info_index].wi_flags & WIF_HOMING_HEAT ||
            ...
            {
              snd_play(&Snds[SND_HEATLOCK_WARN], 0.0f, 1.0f,
          ...
          else
            {
              ...
              snd_play(&Snds[SND_ASPECTLOCK_WARN], 0.0f, 1.0f,
                       SND_PRIORITY_MUST_PLAY);
            }
        }
    }
          ...
          if (iff_x_attacks_y(wp->team, homing_object_team))
            {
              if (objp->type == OBJ_SHIP)
              ...
              else if (objp->type == OBJ_WEAPON)
                {
                  ...
                  if (wip->
                      wi_flags & (WIF_HUGE | WIF_BOMB | WIF_HOMING_JAVELIN))
                    continue;
...
void weapon_home(object * obj, int num, float frame_time)
{
  ...
  if (wip->wi_flags & WIF_HOMING_JAVELIN &&
      hobjp->type == OBJ_SHIP &&
      wp->target_sig > 0 && wp->homing_subsys == NULL)
    {
      wp->homing_object = &obj_used_list;
      return;
    }
  ...
  if (hobjp != &obj_used_list)
    {
      ...
      if ((dist_to_target > miss_distance) &&
          (old_dot < wip->fov) &&
          (wp->lifeleft > 0.01f) &&
          (wp->homing_object) && (wp->homing_object->type == OBJ_SHIP))
        {
          wp->lifeleft = 0.01f;
        }
  ...
  if (wip->wi_flags2 & WIF2_LOCAL_SSM)
    {
      ...
      if ((wp->lssm_stage == 2)
              ...
              wp->lssm_warp_pct))
        {
          ...
          vm_vec_scale_add(&wp->lssm_target_pos, &Objects[wp->target_num].pos,
      ...
      if ((wp->lssm_stage == 4)
          && (fireball_lifeleft_percent(&Objects[wp->lssm_warp_idx]) <= 0.5f))
        {
          vm_vec_copy_scale(&obj->phys_info.desired_vel,
...
>>>>>>> 7d3993bca3732af9c041d291325bf784ff48f3c7
  ...
  if ((parent_objp != NULL)
      && (The_mission.ai_profile->flags & AIPF_USE_ADDITIVE_WEAPON_VELOCITY))
    {
				          ...
				          wp->homing_subsys->system_info->type != SUBSYSTEM_ENGINE ) )
				{
					...
					else
					{
						...
						wp->homing_subsys = ship_get_closest_subsys_in_sight ( target_ship,
						                    SUBSYSTEM_ENGINE, &Objects[weapon_objnum].pos );
					}
  ...
  if (timestamp_elapsed(Weapon_impact_timer))
    {
      ...
      bool is_weapon_shield_piercing =
        (wip->wi_flags2 & WIF2_PIERCE_SHIELDS) != 0;
  ...
  for (objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list);
       objp = GET_NEXT(objp))
    {
      ...
      if (weapon_area_calc_damage
          (objp, pos, sci->inner_rad, sci->outer_rad, sci->blast, sci->damage,
           &blast, &damage, sci->outer_rad) == -1)
        {
          continue;
        }
