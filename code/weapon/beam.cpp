...
void beam_render_muzzle_glow(beam * b)
{
  ...
  gr_set_bitmap(bwi->beam_glow.first_frame + framenum, GR_ALPHABLEND_FILTER,
  ...
  for (idx = 0; idx < b->f_collision_count; idx++)
    {
      ...
      if (draw_effects
              ...
              || (wi->flash_impact_explosion_radius > 0)))
        {
          ...
          vm_vec_sub(&temp_pos, &b->f_collisions[idx].cinfo.hit_point_world,
                     &Objects[target].pos);
                      ...
                      if (widest <=
                          Objects[target].radius * BEAM_AREA_PERCENT)
                        {
                          ...
                          vm_vec_copy_scale(&expl_vel, &fvec,
                                            base_v * frand_range(1.0f, 2.0f));
                          vm_vec_copy_scale(&expl_splash_vel, &fvec,
                                            back_v * frand_range(1.0f, 2.0f));
          ...
          switch (Objects[target].type)
            ...
            case OBJ_SHIP:
              ...
              if (!b->f_collisions[idx].is_exit_collision)
                ship_apply_local_damage(&Objects[target], &Objects[b->objnum],
                                        &b->f_collisions[idx].cinfo.
                                        hit_point_world,
  ...
  if (!(The_mission.ai_profile->flags & AIPF_FORCE_BEAM_TURRET_FOV))
    {
      ...
      beam_get_global_turret_gun_info(b->objp, b->subsys, &turret_pos,
                                      &turret_dir, 1, &temp, b->fighter_beam);
  ...
  while (lookup != END_OF_LIST(&Ships[s1].subsys_list))
    {
      ...
      if ((lookup->system_info->type == SUBSYSTEM_TURRET)
          && !subsystem_stricmp(lookup->system_info->subobj_name, "turret07"))
        {
          orion_turret = lookup;
        }
  ...
  while (lookup != END_OF_LIST(&Ships[s2].subsys_list))
    {
      // turret
      if ((lookup->system_info->type == SUBSYSTEM_TURRET)
          && !subsystem_stricmp(lookup->system_info->subobj_name, "turret03"))
        {
          fenris_turret = lookup;
        }
