...
void beam_get_cull_vals(object * objp, beam * b, float *cull_dot,
                        float *cull_dist);
...
void beam_calc_facing_pts(vec3d * top, vec3d * bot, vec3d * fvec, vec3d * pos,
                          float w, float z_add);
...
int beam_fire(beam_fire_info * fire_info) 
{
  ...
  extern bool weapon_is_used(int weapon_index);
    ...
    
    {
        ...
        
        {
            ...
            if (Ship_info[target_ship->ship_info_index].class_type > -1
                &&
                (Ship_types
                 [Ship_info[target_ship->ship_info_index].class_type].
                 weapon_bools & STI_WEAP_BEAMS_EASILY_HIT))
    ...
    objnum =
    obj_create(OBJ_BEAM, -1, new_item - Beams, &vmd_identity_matrix,
    ...
    
    {
      ...
       send_beam_fired_packet(fire_info->shooter, fire_info->turret,
                                fire_info->target, fire_info->beam_info_index,
                                &new_item->binfo,
                                (ubyte) fire_info->fighter_beam, bank_point);
...
int beam_fire_targeting(fighter_beam_fire_info * fire_info) 
{
    ...
    // create the associated object
    objnum =
    obj_create(OBJ_BEAM, OBJ_INDEX(fire_info->shooter), new_item - Beams,
               &vmd_identity_matrix, &vmd_zero_vector, 1.0f, OF_COLLIDES);
...
int beam_get_weapon_info_index(object * bm) 
{
    ...
    Assert((Beams[bm->instance].weapon_info_index > -1)
...
 void beam_get_global_turret_gun_info(object * objp, ship_subsys * ssp,
                                        vec3d * gpos, vec3d * gvec,
                                        int use_angles, vec3d * targetp,
                                        bool fighter_beam) 
{
  ship_get_global_turret_gun_info(objp, ssp, gpos, gvec, use_angles,
                                   targetp);
...
void beam_type_c_move(beam * b) 
{
  ...
  vm_vec_scale_add(&b->last_shot, &b->last_start, &b->objp->orient.vec.fvec,
                    b->range);
    ...
    num_fire_points * Weapon_info[b->weapon_info_index].energy_consumed *
    flFrametime;
...
void beam_type_e_move(beam * b) 
{
    ...
    // LEAVE THIS HERE OTHERWISE MUZZLE GLOWS DRAW INCORRECTLY WHEN WARMING UP OR DOWN
    // get the "originating point" of the beam for this frame. essentially bashes last_start
    beam_get_global_turret_gun_info(b->objp, b->subsys, &b->last_start,
                                    &turret_norm, 1, &temp, b->fighter_beam);
...
void beam_render(beam * b, float u_offset) 
{
  ...
  int cull = gr_set_cull(0);
    ...
    
    {
      ...
      beam_calc_facing_pts(&top1, &bottom1, &fvec, &b->last_start,
                            bwsi->width * scale * b->shrink, bwsi->z_add);
      beam_calc_facing_pts(&top2, &bottom2, &fvec, &b->last_shot,
                            bwsi->width * scale * scale * b->shrink,
                            bwsi->z_add);
      ...
      int framenum = 0;
        ...
        
        {
            ...
            fl2i((b->beam_secion_frame[s_idx] * bwsi->texture.num_frames) /
                 bwsi->texture.total_time);
           CLAMP(framenum, 0, bwsi->texture.num_frames - 1);
        }
       gr_set_bitmap(bwsi->texture.first_frame + framenum,
        ...
        g3_draw_poly(4, verts,
                     TMAP_FLAG_TEXTURED | TMAP_FLAG_RGB | TMAP_FLAG_GOURAUD |
                     TMAP_FLAG_TILED | TMAP_FLAG_CORRECT | TMAP_HTL_3D_UNLIT);
...
}  void beam_generate_muzzle_particles(beam * b) 
{
    ...
    
    {
        ...
        vm_vec_random_cone(&particle_dir, &turret_norm,
                        ...
                        wip->b_info.beam_muzzle_radius * frand_range(0.75f,
                                                                     0.9f));
      ...
      float p_time_ref =
        ...
        (wip->b_info.beam_muzzle_radius / p_life) * frand_range(0.85f, 1.2f);
...
void beam_render_muzzle_glow(beam * b) 
{
    ...
    
    {
        ...
        fl2i((b->beam_glow_frame * bwi->beam_glow.num_frames) /
             bwi->beam_glow.total_time);
       CLAMP(framenum, 0, bwi->beam_glow.num_frames - 1);
    }
   gr_set_bitmap(bwi->beam_glow.first_frame + framenum,
                   GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL, 0.8f * pct);
...
void beam_add_light_small(beam * bm, object * objp, vec3d * pt_override =
                          NULL) 
{
    ...
    // some noise
    noise =
    frand_range(1.0f - bwi->sections[0].flicker,
  ...
  float light_rad = beam_get_widest(bm) * blight * noise;
    ...
    
    {
      ...
      vm_vec_dist_to_line(&objp->pos, &bm->last_start, &bm->last_shot,
                           &near_pt, &dist);
  ...
  float fr = (float) wip->laser_color_1.red / 255.0f;
    ...
    light_add_point_unique(&near_pt, light_rad * 0.0001f, light_rad, 1.0f, fr,
                           fg, fb, OBJ_INDEX(objp));
}  

// call to add a light source to a large object
void beam_add_light_large(beam * bm, object * objp, vec3d * pt0,
                          vec3d * pt1) 
{
    ...
    
    {
        ...
        case 1:
            ...
            
            {
                ...
                // large ships
                if (Ship_info
                    [Ships[Objects[l->objnum].instance].ship_info_index].
...
void beam_start_warmup(beam * b) 
{
    ...
    if (!(Game_mode & GM_STANDALONE_SERVER)
        && (Weapon_info[b->weapon_info_index].b_info.beam_warmup_sound >= 0))
    
    {
      snd_play_3d(&Snds
                   [Weapon_info[b->weapon_info_index].b_info.
                    beam_warmup_sound], &b->last_start, &View_position);
...
int beam_start_firing(beam * b) 
{
    ...
    
    {
        ...
        snd_play_3d(&Snds
                    [Weapon_info[b->weapon_info_index].b_info.
                     beam_loop_sound], &b->last_start, &View_position, 0.0f,
                    NULL, 1, 1.0, SND_PRIORITY_SINGLE_INSTANCE, NULL, 1.0f,
                    1);
       
        // "shot" sound
        if (Weapon_info[b->weapon_info_index].launch_snd >= 0)
        snd_play_3d(&Snds[Weapon_info[b->weapon_info_index].launch_snd],
...
void beam_start_warmdown(beam * b) 
{
    ...
    
    {
                   ...
                   [Weapon_info[b->weapon_info_index].b_info.
                    beam_warmdown_sound], &b->last_start, &View_position);
...
void beam_recalc_sounds(beam * b) 
{
    ...
    
    {
      ...
       snd_update_3d_pos(b->beam_sound_loop, &Snds[bwi->beam_loop_sound],
                           &pos);
...
void beam_get_binfo(beam * b, float accuracy, int num_shots) 
{
    ...
    beam_get_global_turret_gun_info(b->objp, b->subsys, &turret_point,
                                    &turret_norm, 1, &p2, b->fighter_beam);
    ...
    case BEAM_TYPE_A:
        ...
        b->binfo.shot_aim[0] =
        frand_range(0.0f,
                    1.0f + bwi->beam_miss_factor[skill_level] * accuracy);
    ...
    case BEAM_TYPE_B:
        ...
        // delta angle
        b->binfo.delta_ang =
        fl_abs(vm_vec_delta_ang_norm(&b->binfo.dir_a, &b->binfo.dir_b, NULL));
    ...
    case BEAM_TYPE_D:
        ...
        
        {
          ...
          float r =
            ((float) pow(1.3f, idx)) * bwi->beam_miss_factor[skill_level] *
            accuracy;
...
void beam_aim(beam * b) 
{
    ...
    
    {
        ...
        
        {
            ...
            vm_vec_unrotate(&b->last_shot,
                            &b->target_subsys->system_info->pnt,
                            &b->target->orient);
      ...
      vm_vec_scale_add(&b->last_shot, &b->last_start,
                        &b->objp->orient.vec.fvec, b->range);
                            ...
                            vec3d * v1, vec3d * v2) 
{
    ...
    t1 =
    oct_array[oct_index][2] ? m->octants[oct_array[oct_index][0]].max : m->
    octants[oct_array[oct_index][0]].min;
  t2 =
    oct_array[oct_index][3] ? m->octants[oct_array[oct_index][1]].max : m->
    octants[oct_array[oct_index][1]].min;
...
void beam_jitter_aim(beam * b, float aim) 
{
    ...
    
    {
        ...
        subsys_strength =
        ship_get_subsystem_strength(&Ships[b->objp->instance],
                                    SUBSYSTEM_WEAPONS);
    ...
    vm_vec_random_in_circle(&circle, &b->last_shot, &m,
                            aim * b->target->radius, 0);
...
int beam_collide_ship(obj_pair * pair) 
{
  ...
  int shield_collision =
    ...
    (beam_will_tool_target(b, ship_objp)) ? model_collide(&mc_hull_exit) : 0;
    ...
    
    {
        ...
        
        {
            ...
            {
              add_shield_point(OBJ_INDEX(ship_objp),
                                mc_shield.shield_hit_tri,
                                &mc_shield.hit_point);
...
void beam_add_collision(beam * b, object * hit_object, mc_info * cinfo,
                        int quadrant_num, int exit_flag) 
{
...
void beam_handle_collisions(beam * b) 
{
    ...
    
    {
        ...
        
        {
               ...
               && (r_coll[r_coll_count].c_sig ==
                   b->r_collisions[s_idx].c_sig))
        ...
        if (draw_effects
            && ((wi->piercing_impact_explosion_radius > 0)
                || (wi->flash_impact_explosion_radius > 0)))
        
        {
            ...
            
            {
                ...
                
                {
                    ...
                    Weapon_explosions.GetAnim(wi->
                                              flash_impact_weapon_expl_index,
                                              &b->f_collisions[idx].cinfo.
                                              hit_point_world, ani_radius);
                  particle_create(&temp_local_pos, &vmd_zero_vector,
                                   0.005f * ani_radius, ani_radius,
                                   PARTICLE_BITMAP_PERSISTENT, ani_handle, -1,
                                   &Objects[target]);
                ...
                
                {
                                   ...
                                   0.005f * ani_radius, ani_radius,
                                   PARTICLE_SMOKE, 0, -1, &Objects[target]);
            ...
            
            {
                ...
                Weapon_explosions.GetAnim(wi->impact_weapon_expl_index,
                                          &b->f_collisions[idx].cinfo.
                                          hit_point_world, ani_radius);
              particle_create(&temp_local_pos, &vmd_zero_vector, 0.0f,
                               ani_radius, PARTICLE_BITMAP_PERSISTENT,
                ...
                
                {
                    ...
                    
                    {
                        ...
                        
                        {
                            ...
                            
                            {
                                   ...
                                   GetPiercingType(wi->damage_type_idx) ==
                    ...
                    
                    {
                        ...
                        
                        {
                            ...
                            wi->piercing_impact_explosion_radius *
                            frand_range(0.5f, 2.0f);
                          float base_v, back_v;
                          vec3d rnd_vec;
                           vm_vec_rand_vec_quick(&rnd_vec);
                           if (wi->piercing_impact_particle_velocity !=
                                 0.0f)
                            base_v = wi->piercing_impact_particle_velocity;
                          
                          else
                            base_v = wi->piercing_impact_explosion_radius;
                           if (wi->piercing_impact_particle_back_velocity !=
                                 0.0f)
                            back_v =
                              wi->piercing_impact_particle_back_velocity;
                          
                          else
                            back_v = base_v * (-0.2f);
                           vm_vec_copy_scale(&expl_vel, &fvec,
                                               base_v * frand_range(1.0f,
                                                                    2.0f));
                          vm_vec_copy_scale(&expl_splash_vel, &fvec,
                                             back_v * frand_range(1.0f,
                                                                  2.0f));
                          vm_vec_scale_add2(&expl_vel, &rnd_vec,
                                             base_v *
                                             wi->
                                             piercing_impact_particle_variance);
                            ...
                            
                            {
                                ...
                                Weapon_explosions.GetAnim(wi->
                                                          ...
                                                          hit_point_world,
                                                          flame_size);
                              particle_create(&b->f_collisions[idx].cinfo.
                                               hit_point_world, &expl_vel,
                                               0.0f, flame_size,
                                               PARTICLE_BITMAP_PERSISTENT,
                                               ani_handle);
                              particle_create(&b->f_collisions[idx].cinfo.
                                               hit_point_world,
                                               &expl_splash_vel, 0.0f,
                                               flame_size,
                                               PARTICLE_BITMAP_PERSISTENT,
                                               ani_handle);
            ...
            
            {
                ...
                
                {
                    ...
                    Weapon_explosions.GetAnim(wi->impact_weapon_expl_index,
                                   ...
                                   hit_point_world, &vmd_zero_vector, 0.0f,
                                   wi->impact_explosion_radius,
        ...
        
        {
            ...
            
            {
                ...
                Assert(Weapon_info
                       [Weapons[Objects[target].instance].weapon_info_index].
                       subtype == WP_MISSILE);
                ...
                ship_apply_local_damage(&Objects[target],
                                         &Objects[b->objnum],
                                         &b->f_collisions[idx].cinfo.
                                         hit_point_world,
                                         beam_get_ship_damage(b,
                                                              &Objects
                                                              [target]),
...
int beam_ok_to_fire(beam * b) 
{
    ...
    
    {
        ...
        
        {
            ...
            shipp->weapons.next_primary_fire_stamp[b->bank] =
            timestamp(shipp->weapons.next_primary_fire_stamp[b->bank] * 2);
    ...
    
    {
      ...
      beam_get_global_turret_gun_info(b->objp, b->subsys, &turret_pos,
                                       &turret_dir, 1, &temp,
                                       b->fighter_beam);
    ...
    
    {
        ...
        {
          vm_vec_unrotate(&turret_normal,
                           &b->subsys->system_info->turret_norm,
                           &b->objp->orient);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                Weapon_info[b->weapon_info_index].b_info.sections[idx].width;
...
void beam_apply_whack(beam * b, object * objp, vec3d * hit_point) 
{
  ...
  vm_vec_dist_to_line(&objp->pos, &b->last_start, &b->last_shot, &temp,
                       &dist);
...
int beam_will_tool_target(beam * b, object * objp) 
{
    ...
    
    {
           ...
           GetPiercingType(wip->damage_type_idx) == SADTF_PIERCING_NONE)
        
        {
          return 0;
        }
      hp_limit =
        Armor_types[shipp->armor_type_idx].GetPiercingLimit(wip->
                                                            damage_type_idx);
    ...
    *lookup;
    ...
    
    {
        ...
        if ((lookup->system_info->type == SUBSYSTEM_TURRET)
            && !subsystem_stricmp(lookup->system_info->subobj_name,
                                  "turret07"))
