      ...
      // a mflash of the same name exists, don't add it again
      else
        {
          if (!override_mflash)
            {
              Warning(LOCATION,
                      "Muzzle flash \"%s\" already exists!  Using existing entry instead.",
                      mflash.name);
            }
        }
    }
  ...
  for (i = 0; i < Mflash_info.size(); i++)
    {
      ...
      for (idx = 0; idx < Mflash_info[i].blobs.size(); idx++)
        {
          Mflash_info[i].blobs[idx].anim_id =
            bm_load_animation(Mflash_info[i].blobs[idx].name, &num_frames,
                              &fps, NULL, 1);
...
void mflash_create(vec3d * gun_pos, vec3d * gun_dir, physics_info * pip,
                   int mflash_type, object * local)
{
  // mflash *mflashp;
  mflash_info *mi;
