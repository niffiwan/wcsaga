...
int shockwave_create(int parent_objnum, vec3d * pos,
                     shockwave_create_info * sci, int flag, int delay) 
{
  ...
  sw->current_bitmap = -1;
    ...
    objnum =
    obj_create(OBJ_SHOCKWAVE, real_parent, i, &orient, &sw->pos,
...
void shockwave_set_framenum(int index) 
{
    ...
    fl2i(sw->time_elapsed / sw->total_time * si->num_frames + 0.5);
...
void shockwave_render(object * objp) 
{
    ...
    
    {
      ...
       model_set_warp_globals(model_Interp_scale_xyz,
                                model_Interp_scale_xyz,
                                model_Interp_scale_xyz, -1,
      ...
      model_render(sw->model_id, &Objects[sw->objnum].orient, &sw->pos,
                    MR_NO_LIGHTING | MR_NO_FOGGING | MR_NORMAL |
    ...
    
    {
      ...
       gr_set_bitmap(sw->current_bitmap, GR_ALPHABLEND_FILTER,
                       GR_BITBLT_MODE_NORMAL, 1.3f);
      g3_draw_rotated_bitmap(&p, fl_radian(sw->rot_angles.p), sw->radius,
...
int shockwave_load(char *s_name, bool shock_3D) 
{
    ...
    
    {
        ...
        bm_load_animation(si->filename, &si->num_frames, &si->fps, NULL, 1);
...
void shockwave_level_init() 
{
  ...
   Assert(((Shockwave_info[0].bitmap_id >= 0)
             || (Shockwave_info[0].model_id >= 0)));
...
 void shockwave_page_in() 
{
    ...
    
    {
        ...
        {
          bm_page_in_texture(Shockwave_info[i].bitmap_id,
                              Shockwave_info[i].num_frames);
