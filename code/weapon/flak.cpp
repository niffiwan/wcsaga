...
void flak_level_init() 
{
    ...
    {
      Flak_muzzle_flash_ani =
        bm_load_animation(MUZZLE_FLASH_FILE, &num_frames, &fps, NULL, 1);
...
void flak_pick_range(object * objp, vec3d * firing_pos,
                     ...
                     float weapon_subsys_strength) 
{
    ...
    det_range =
    Weapon_info[Weapons[objp->instance].weapon_info_index].det_range;
                     ...
                     float weapon_subsys_strength) 
{
    ...
    // error value
    error_val =
    Flak_error + (Flak_error * 0.65f * (1.0f - weapon_subsys_strength));
...
void flak_muzzle_flash(vec3d * pos, vec3d * dir, physics_info * pip,
