...
MONITOR(NumHiModelsRend) MONITOR(NumMedModelsRend) MONITOR(NumLowModelsRend)
/*
...
#define MAX_SCALE 4.7f
              ...
              if (d > 0.0f)
                {
                  ...
                  vm_vec_scale_add(&norm2, &pnt, &fvec,
                                   wVal * 2 * Interp_thrust_glow_len_factor);
  ...
  if (Interp_secondary_thrust_glow_bitmap >= 0)
    {
      gr_set_bitmap(Interp_secondary_thrust_glow_bitmap, GR_ALPHABLEND_FILTER,
                    GR_BITBLT_MODE_NORMAL, 1.0f);
  ...
  if (Interp_flags & MR_SHOW_PIVOTS)
    {
      ...
      model_draw_debug_points(pm,
                              &pm->submodel[pm->detail[Interp_detail_level]]);
...
int check_values(vec3d * N)
{
  ...
  if ((N->xyz.x * N->xyz.x) < 0 ||
      (N->xyz.y * N->xyz.y) < 0 ||
      (N->xyz.z * N->xyz.z) < 0 || !is_valid_vec(N))
    {
      N->xyz.x = 1;
      N->xyz.y = 0;
      N->xyz.z = 0;
