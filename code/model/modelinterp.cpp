...
int model_interp_sub(void *model_ptr, polymodel * pm, bsp_info * sm,
                     int do_box_check);
void model_try_cache_render(int model_num, matrix * orient, vec3d * pos,
                             uint flags, int objnum, int num_lights);
void model_really_render(int model_num, matrix * orient, vec3d * pos,
                          uint flags, int objnum = -1);
void model_interp_sortnorm_b2f(ubyte * p, polymodel * pm, bsp_info * sm,
                                int do_box_check);
void model_interp_sortnorm_f2b(ubyte * p, polymodel * pm, bsp_info * sm,
                                int do_box_check);
void (*model_interp_sortnorm) (ubyte * p, polymodel * pm, bsp_info * sm,
                                int do_box_check) = model_interp_sortnorm_b2f;
int model_should_render_engine_glow(int objnum, int bank_obj);
void model_render_buffers(bsp_info * model, polymodel * pm, bool is_child =
                           false);
void model_render_children_buffers(bsp_info * model, polymodel * pm, int mn,
                                    int detail_level);
...
 void model_allocate_interp_data(int n_verts = 0, int n_norms =
                                   0, int n_list_verts = 0) 
{
  ...
  Assert((n_verts || Num_interp_verts_allocated)
          && (n_norms
    ...
    
    {
      ...
      Interp_splode_points =
        (vertex *) vm_realloc(Interp_splode_points, n_verts * sizeof(vertex));
...
void model_interp_splode_defpoints(ubyte * p, polymodel * pm, bsp_info * sm,
                                   float dist) 
{
...
void model_interp_defpoints(ubyte * p, polymodel * pm, bsp_info * sm) 
{
  ...
  else if ((Interp_warp_scale_x != 1.0f) || (Interp_warp_scale_y != 1.0f)
...
void model_interp_flatpoly(ubyte * p, polymodel * pm) 
{
    ...
    
    {
        ...
        
        {
            ...
            {
              light_apply_rgb(&Interp_list[i]->r, &Interp_list[i]->g,
                               &Interp_list[i]->b, Interp_verts[vertnum],
                               vp(p + 8), Interp_light);
            ...
            
            {
                ...
                {
                  light_apply_rgb(&Interp_lighting->lights[norm].r,
                                   &Interp_lighting->lights[norm].g,
                                   &Interp_lighting->lights[norm].b,
                                   Interp_verts[vertnum], vp(p + 8),
                                   Interp_light);
...
 void model_interp_edge_alpha(ubyte * param_r, ubyte * param_g,
                                ubyte * param_b, vec3d * pnt, vec3d * norm,
    ...
    
    {
            ...
            
            {
                ...
                
                {
                    ...
                    
                    {
                        ...
                        &Interp_splode_points[verts[i].vertnum];
        ...
        
        {
            ...
            
            {
              ...
              model_interp_edge_alpha(&Interp_list[i]->r, &Interp_list[i]->g,
                                       &Interp_list[i]->b,
                                       Interp_verts[verts[i].vertnum],
                                       Interp_norms[verts[i].normnum], salpha,
                                       false);
            } cull = gr_set_cull(0);
          gr_set_bitmap(splodeingtexture, GR_ALPHABLEND_FILTER,
                         GR_BITBLT_MODE_NORMAL, salpha);
          
            //  gr_set_color( 255, 250, 200 );
            //  g3_draw_poly( nv, Interp_list, 0 );
            g3_draw_poly(nv, Interp_list,
                         TMAP_FLAG_TEXTURED | TMAP_FLAG_GOURAUD);
    ...
    
    {
        ...
        
        {
            ...
            if ((Interp_flags & MR_NO_LIGHTING) || (tmap->is_ambient))  //gets the ambient glow to work
            {
                ...
                {
                  model_interp_edge_alpha(&Interp_list[i]->r,
                                           &Interp_list[i]->g,
                                           &Interp_list[i]->b,
                                           Interp_verts[verts[i].vertnum],
                                           Interp_norms[verts[i].normnum],
            ...
            
            {
                ...
                
                {
                       ...
                       && !Cmdline_cell && Cmdline_spec)
                    light_apply_specular(&Interp_list[i]->spec_r,
                                          &Interp_list[i]->spec_g,
                                          &Interp_list[i]->spec_b,
                                          Interp_verts[vertnum], vp(p + 8),
                                          &View_position);
                ...
                
                {
                    ...
                    
                    {
                                         ...
                                         &Interp_lighting->lights[norm].g,
                                         &Interp_lighting->lights[norm].b,
                                         Interp_verts[vertnum],
                                         Interp_norms[norm], Interp_light);
                      if ((Detail.lighting > 2) && (Interp_detail_level < 2)
                           && !Cmdline_cell && Cmdline_spec)
                        light_apply_specular(&Interp_lighting->lights[norm].
                                              spec_r,
                                              &Interp_lighting->lights[norm].
                                              spec_g,
                                              &Interp_lighting->lights[norm].
                                              spec_b, Interp_verts[vertnum],
                                              Interp_norms[norm],
                                              &View_position);
                  ...
                   Interp_list[i]->spec_r =
                    Interp_lighting->lights[norm].spec_r;
                  Interp_list[i]->spec_g =
                    Interp_lighting->lights[norm].spec_g;
                  Interp_list[i]->spec_b =
                    Interp_lighting->lights[norm].spec_b;
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                (~(TMAP_FLAG_TEXTURED | TMAP_FLAG_TILED | TMAP_FLAG_CORRECT));
        ...
        {
          gr_set_bitmap(Interp_warp_bitmap, GR_ALPHABLEND_FILTER,
                         GR_BITBLT_MODE_NORMAL, Interp_warp_alpha);
        ...
        
        {
            ...
            
            {
                ...
                {
                  gr_set_bitmap(tbase->GetTexture(), GR_ALPHABLEND_FILTER,
                                 GR_BITBLT_MODE_NORMAL, 1.2f);
                ...
                
                {
                  ...
                  else if ((Interp_new_replacement_textures != NULL)
                           &&
                           (Interp_new_replacement_textures
                            [rt_begin_index + TM_BASE_TYPE] >= 0))
                    
                    {
                      texture_info tt =
                        texture_info(Interp_new_replacement_textures
                                     [rt_begin_index + TM_BASE_TYPE]);
                      texture =
                        model_interp_get_texture(&tt, base_frametime);
                    ...
                    
                    {
                        ...
                        texture =
                        model_interp_get_texture(tbase, base_frametime);
                        ...
                        
                        {
                            ...
                            
                            {
                                ...
                                tglow->GetTexture() +
                                shockwave_get_framenum(Objects[Interp_objnum].
                                                       instance,
                                                       tglow->GetNumFrames());
                            ...
                            
                            {
                                ...
                                model_interp_get_texture(tglow,
                                                         base_frametime);
                            }
                        }
                       if ((Detail.lighting > 2)
                             && (Interp_detail_level < 2))
                        
                        {
                          
                            // likewise, etc.
                            SPECMAP =
                            model_interp_get_texture(&tmap->
                                                     textures
                                                     [TM_SPECULAR_TYPE],
                                                     base_frametime);
                          NORMMAP =
                            model_interp_get_texture(&tmap->
                                                     textures[TM_NORMAL_TYPE],
                                                     base_frametime);
                          HEIGHTMAP =
                            model_interp_get_texture(&tmap->
                                                     textures[TM_HEIGHT_TYPE],
                                                     base_frametime);
                    ...
                    
                    {
                      ...
                      for (int tmn = TM_BASE_TYPE + 1; tmn < TM_NUM_TYPES;
                            tmn++)
                        
                        {
                          int tex =
                            Interp_new_replacement_textures[rt_begin_index +
                                                            tmn];
                            ...
                            tex =
                            model_interp_get_texture(&tinfo, base_frametime);
...
void model_draw_bay_paths_htl(int model_num) 
{
    ...
    
    {
        ...
        
        {
          ...
          vm_vec_scale_add(&v2, &v1, &pm->docking_bays[idx].norm[s_idx],
                            10.0f);
...
 extern int g3_draw_rod(int num_points, vec3d * vecs, float width,
                          uint tmap_flags);
 void interp_render_arc_segment(vec3d * v1, vec3d * v2, int depth) 
{
    ...
    
    {
      ...
       memcpy(&Arc_segment_points[Num_arc_segment_points++], v2,
                sizeof(vec3d));
...
 void interp_render_arc(vec3d * v1, vec3d * v2, color * primary,
                          color * secondary, float arc_width) 
{
    ...
    (TMAP_FLAG_RGB | TMAP_FLAG_GOURAUD | TMAP_HTL_3D_UNLIT |
     TMAP_FLAG_TRISTRIP);
  ...
   g3_draw_rod(Num_arc_segment_points, Arc_segment_points, arc_width,
                 tmap_flags);
    ...
    
    {
      ...
       g3_draw_rod(Num_arc_segment_points, Arc_segment_points,
                     arc_width * 0.33f, tmap_flags);
     ...
      void interp_render_lightning(polymodel * pm, bsp_info * sm) 
{
    ...
    
    {
        ...
        interp_render_arc(&sm->arc_pts[i][0], &sm->arc_pts[i][1], &primary,
                          &secondary, width);
...
 void model_interp_subcall(polymodel * pm, int mn, int detail_level) 
{
  ...
   g3_start_instance_matrix(&pm->submodel[mn].offset, &submodel_matrix,
                              true);
...
 void model_render_insignias(polymodel * pm, int detail_level) 
{
  ...
  &vecs[0], &vecs[1], &vecs[2]};
    ...
    TMAP_FLAG_TEXTURED | TMAP_FLAG_CORRECT | TMAP_HTL_3D_UNLIT;
   x.xyz.x = 0;
  x.xyz.y = 0;
  x.xyz.z = 0;
   
    // set the proper texture
    gr_set_bitmap(Interp_insignia_bitmap, GR_ALPHABLEND_NONE,
                  GR_BITBLT_MODE_NORMAL, 0.65f);
    ...
    
    {
        ...
        
        {
          ...
          vecs[2].v = pm->ins[idx].v[s_idx][2];
            ...
            {
              light_apply_rgb(&vecs[0].r, &vecs[0].g, &vecs[0].b,
                               &pm->ins[idx].vecs[i1], &pm->ins[idx].norm[i1],
                               1.5f);
              light_apply_rgb(&vecs[1].r, &vecs[1].g, &vecs[1].b,
                               &pm->ins[idx].vecs[i2], &pm->ins[idx].norm[i2],
                               1.5f);
              light_apply_rgb(&vecs[2].r, &vecs[2].g, &vecs[2].b,
                               &pm->ins[idx].vecs[i3], &pm->ins[idx].norm[i3],
                               1.5f);
...
//returns 1 if off screen, 0 if not
     int model_get_rotated_bitmap_points(vertex * pnt, float angle, float rad,
                                         vertex * v) 
{
...
*/ 
void model_render(int model_num, matrix * orient, vec3d * pos, uint flags,
                   int objnum, int lighting_skip, int *replacement_textures) 
{
  ...
  for (int i = 0; i < pm->n_glow_point_banks; i++)     //glow point blink code -Bobboau
    {
        ...
        
        {
            ...
            
            {
                   ...
                   ((time - bank->disp_time) % (bank->on_time +
                                                bank->off_time)))
  ...
   model_try_cache_render(model_num, orient, pos, flags, objnum,
                            num_lights);
...
float Mc_viewer_pos_factor[MAX_DETAIL_LEVEL + 1] =
  { 0.080f, 0.040f, 0.020f, 0.010f, 0.0f };
float Mc_size_factor[MAX_DETAIL_LEVEL + 1] =
  { 1.40f, 1.30f, 1.20f, 1.10f, 0.0f };
...
float Mc_framerate_hi[MAX_DETAIL_LEVEL + 1] =
  { 15.0f, 20.0f, 25.0f, 30.0f, 100000.0f };
...
float interp_closest_dist_to_box(vec3d * hitpt, vec3d * p0, vec3d * min,
                                 vec3d * max) 
{
...
float model_find_closest_point(vec3d * outpnt, int model_num,
...
 void moldel_calc_facing_pts(vec3d * top, vec3d * bot, vec3d * fvec,
                               vec3d * pos, float w, float z_add,
                               vec3d * Eyeposition) 
{
    ...
    //  Int3();
}  geometry_batcher primary_thruster_batcher, secondary_thruster_batcher,
  tertiary_thruster_batcher;
 
// maybe draw mode thruster glows
void model_render_thrusters(polymodel * pm, int objnum, ship * shipp,
...
#define MAX_SCALE 4.7f
          ...
          1.0f, 0.0f, 0.0f}}};
            ...
            gpt->radius * (scale + Interp_thrust_glow_noise * NOISE_SCALE);
            ...
            
            {
              ...
               primary_thruster_batcher.draw_bitmap(&p, 0,
                                                      (w * 0.5f *
                                                       Interp_thrust_glow_rad_factor),
                                                      (w * 0.325f));
            ...
            
            {
              ...
               tertiary_thruster_batcher.draw_bitmap(&p, 0,
                                                       (w * 0.6f *
                                                        Interp_tertiary_thrust_glow_rad_factor),
                                                       (magnitude * 4),
                                                       (-(D > 0) ? D : -D));
            ...
            
            {
                ...
                magnitude * (MAX_SCALE - (MIN_SCALE / 2)) + (MIN_SCALE / 2);
                ...
                
                {
                  ...
                   vm_vec_scale_add(&norm2, &pnt, &fvec,
                                      wVal * 2 *
                                      Interp_thrust_glow_len_factor);
                  ...
                   secondary_thruster_batcher.draw_beam(&pnt, &norm2,
                                                          wVal *
                                                          Interp_secondary_thrust_glow_rad_factor
                                                          * 0.5f, d);
            ...
            
            {
                  ...
                  (int) sip->afterburner_thruster_particles.size();
                ...
                
                {
                    ...
                    vm_vec_mag_quick(&Objects[shipp->objnum].phys_info.
                                     desired_vel);
                  ...
                  particle_emit(&pe, PARTICLE_BITMAP,
  ...
  int zbuff_save = gr_zbuffering_mode;
    ...
    {
      gr_set_bitmap(Interp_secondary_thrust_glow_bitmap,
                     GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL, 1.0f);
    ...
    {
      gr_set_bitmap(Interp_tertiary_thrust_glow_bitmap, GR_ALPHABLEND_FILTER,
                     GR_BITBLT_MODE_NORMAL, 1.0f);
                            ...
                            uint flags, int objnum) 
{
    ...
    if ((The_mission.flags & MISSION_FLAG_FULLNEB)
        && (Neb2_render_mode != NEB2_RENDER_NONE) && !(flags & MR_NO_FOGGING))
  ...
    bool is_outlines_only = (flags & MR_NO_POLYS)
    && ((flags & MR_SHOW_OUTLINE_PRESET) || (flags & MR_SHOW_OUTLINE));
  bool is_outlines_only_htl = !Cmdline_nohtl && (flags & MR_NO_POLYS)
    ...
    model_find_closest_point(&closest_pos, model_num, -1, orient, pos,
                             &Eye_position);
...
#error Code in modelInterp.cpp assumes MAX_DETAIL_LEVEL == 4
    ...
    
    {
        ...
        
        {
            ...
            -((pm->submodel[pm->detail[Interp_detail_level]].max.xyz.x +
               pm->submodel[pm->detail[Interp_detail_level]].min.xyz.x) /
              2.0f);
          auto_back.xyz.y =
            -((pm->submodel[pm->detail[Interp_detail_level]].max.xyz.y +
               pm->submodel[pm->detail[Interp_detail_level]].min.xyz.y) /
              2.0f);
          auto_back.xyz.z =
            -((pm->submodel[pm->detail[Interp_detail_level]].max.xyz.z +
               pm->submodel[pm->detail[Interp_detail_level]].min.xyz.z) /
              2.0f);
    ...
    
    {
        ...
        
        {
            ...
            {
              model_render_children_buffers(&pm->submodel[i], pm, i,
                                             Interp_detail_level);
    ...
    {
      model_render_buffers(&pm->submodel[pm->detail[Interp_detail_level]],
                            pm);
    ...
    {
      model_interp_subcall(pm, pm->detail[Interp_detail_level],
                            Interp_detail_level);
    ...
    
    {
        ...
        if (decal_z_corection.xyz.x != 0 && decal_z_corection.xyz.y != 0
            && decal_z_corection.xyz.z != 0)
    ...
    
    {
      ...
      model_draw_debug_points(pm,
                               &pm->submodel[pm->
                                             detail[Interp_detail_level]]);
                ...
                
                {
                    ...
                    
                    {
                      ...
                       if (bank->submodel_parent > 0) //this is where it rotates for the submodel parent-Bobboau
                        {
                            ...
                            pm->submodel[bank->submodel_parent].angs;
                            ...
                            pm->submodel[bank->submodel_parent].orientation;
                          vm_rotate_matrix_by_angles(&rotation_matrix,
                                                      &angs);
                           matrix inv_orientation;
                          vm_copy_transpose_matrix(&inv_orientation,
                                                    &pm->submodel[bank->
                                                                  submodel_parent].
                                                    orientation);
                           matrix submodel_matrix;
                          vm_matrix_x_matrix(&submodel_matrix,
                                              &rotation_matrix,
                                              &inv_orientation);
                           vec3d offset =
                            pm->submodel[bank->submodel_parent].offset;
                        ...
                        
                        {
                          ...
                          
                          {
                              ...
                              
                              {
                                  ...
                                  
                                  {
                                      ...
                                      (1.0f - neb2_get_fog_intensity(&npnt));
                          ...
                          
                          {
                            ...
                            vertex *verts[4] =
                              { &h1[0], &h1[1], &h1[2], &h1[3] };
                            vec3d fvec, top1, bottom1, top2, bottom2, start,
                              end;
                            ...
                             moldel_calc_facing_pts(&top1, &bottom1, &fvec,
                                                      &pnt, gpt->radius, 1.0f,
                                                      &View_position);
                            moldel_calc_facing_pts(&top2, &bottom2, &fvec,
                                                    &norm, gpt->radius, 1.0f,
                                                    &View_position);
                            ...
                            verts[3]->v = 1.0f;
                              ...
                              {
                                gr_set_bitmap(bank->glow_neb_bitmap,
                                               GR_ALPHABLEND_FILTER,
                                               GR_BITBLT_MODE_NORMAL, 1.0f);
...
  void submodel_render(int model_num, int submodel_num, matrix * orient,
                          vec3d * pos, uint flags, int objnum,
...
void submodel_get_two_random_points(int model_num, int submodel_num,
                                    vec3d * v1, vec3d * v2, vec3d * n1,
                                    vec3d * n2) 
{
                                             ...
                                             float delta) 
{
    ...
    
    {
        ...
        dist =
        fvi_ray_plane(&box_point, extremes, &ray_direction, start_point,
                      &ray_direction, 0.0f);
                                             ...
                                             int *is_inside, float delta) 
{
    ...
    get_model_closest_box_point_with_delta(closest_box_point, &box_start,
...
 void generate_vertex_buffers(bsp_info * model, polymodel * pm) 
{
    ...
    
    {
      ...
       memcpy((model_list.vert) + model_list.n_verts, polygon_list[i].vert,
                sizeof(vertex) * polygon_list[i].n_verts);
      memcpy((model_list.norm) + model_list.n_verts, polygon_list[i].norm,
              sizeof(vec3d) * polygon_list[i].n_verts);
       if (Cmdline_normal)
        
        {
          memcpy((model_list.tsb) + model_list.n_verts, polygon_list[i].tsb,
  ...
  extern IBX ibuffer_info;
    ...
    
    {
        ...
        
        {
            ...
            Warning(LOCATION,
                    "IBX: Safety Check Failure!  The file doesn't contain enough data, deleting '%s'\n",
                    ibuffer_info.name);
        ...
        
        {
            ...
            
            {
                ...
                {
                  model_list.tsb[i].tangent.xyz.x =
                    cfread_float(ibuffer_info.tsb_read);
                  model_list.tsb[i].tangent.xyz.y =
                    cfread_float(ibuffer_info.tsb_read);
                  model_list.tsb[i].tangent.xyz.z =
                    cfread_float(ibuffer_info.tsb_read);
                  model_list.tsb[i].scaler =
                    cfread_float(ibuffer_info.tsb_read);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                {
                  cfwrite_float(model_list.tsb[i].tangent.xyz.x,
                                 ibuffer_info.tsb_write);
                  cfwrite_float(model_list.tsb[i].tangent.xyz.y,
                                 ibuffer_info.tsb_write);
                  cfwrite_float(model_list.tsb[i].tangent.xyz.z,
                                 ibuffer_info.tsb_write);
                  cfwrite_float(model_list.tsb[i].scaler,
                                 ibuffer_info.tsb_write);
    ...
    (VERTEX_FLAG_POSITION | VERTEX_FLAG_NORMAL | VERTEX_FLAG_UV1);
        ...
        
        {
            ...
            
            {
                ...
                
                {
                  ...
                  new_buffer.index_buffer.ibuffer[j] =
                    cfread_uint(ibuffer_info.read);
                ...
                
                {
                  ...
                  new_buffer.index_buffer.sbuffer[j] =
                    cfread_ushort(ibuffer_info.read);
            ...
            
            {
                ...
                find_first_index_vb(&polygon_list[i], j, &model_list);
                ...
                
                {
                            ...
                            (&model_list.
                             vert[new_buffer.index_buffer.ibuffer[j]],
                             &polygon_list[i].vert[j],
                             &model_list.norm[new_buffer.index_buffer.
                                              ibuffer[j]],
                             &polygon_list[i].norm[j]));
                ...
                
                {
                            ...
                            (&model_list.
                             vert[new_buffer.index_buffer.sbuffer[j]],
                             &polygon_list[i].vert[j],
                             &model_list.norm[new_buffer.index_buffer.
                                              sbuffer[j]],
                             &polygon_list[i].norm[j]));
                ...
                
                {
                    ...
                    cfwrite_uint(new_buffer.index_buffer.ibuffer[j],
                                  ibuffer_info.write);
                  
                  else
                    cfwrite_ushort(new_buffer.index_buffer.sbuffer[j],
                                    ibuffer_info.write);
...
  void model_render_children_buffers(bsp_info * model, polymodel * pm,
                                        int mn, int detail_level) 
{
    ...
    {
      vm_vec_copy_scale(&Interp_render_box_min, &model->render_box_min,
                         Interp_box_scale);
      vm_vec_copy_scale(&Interp_render_box_max, &model->render_box_max,
                         Interp_box_scale);
       if ((-model->use_render_box +
              in_box(&Interp_render_box_min, &Interp_render_box_max,
                     &model->offset)))
                             ...
                             bool is_child) 
{
    ...
    // if using detail boxes, check that we are valid for the range
    if (!(Interp_flags & MR_FULL_DETAIL) && !is_child
        && model->use_render_box)
    ...
    
    {
        ...
        
        {
               ...
               &&
               (Interp_new_replacement_textures[rt_begin_index + TM_BASE_TYPE]
                >= 0))
        ...
        
        {
            ...
            
            {
                ...
                // shockwaves are special, their current frame has to come out of the shockwave code to get the timing correct
                if ((Interp_objnum >= 0)
                    && (Objects[Interp_objnum].type == OBJ_SHOCKWAVE)
                    && (tglow->GetNumFrames() > 1))
                
                {
                  GLOWMAP =
                    tglow->GetTexture() +
                    shockwave_get_framenum(Objects[Interp_objnum].instance,
                                           tglow->GetNumFrames());
            ...
            
            {
                ...
                // likewise, etc.
                SPECMAP =
                model_interp_get_texture(&tmap->textures[TM_SPECULAR_TYPE],
                                         base_frametime);
              NORMMAP =
                model_interp_get_texture(&tmap->textures[TM_NORMAL_TYPE],
                                         base_frametime);
              HEIGHTMAP =
                model_interp_get_texture(&tmap->textures[TM_HEIGHT_TYPE],
                                         base_frametime);
        ...
        
        {
            ...
            
            {
                ...
                Interp_new_replacement_textures[rt_begin_index + tmn];
        ...
        
        {
          ...
          gr_set_bitmap(texture, GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL,
                         Interp_xparent_alpha);
        ...
        
        {
            ...
            gr_set_bitmap(texture, GR_ALPHABLEND_NONE, GR_BITBLT_MODE_NORMAL,
                          1.0f);
        ...
        gr_render_buffer(0, model->buffer[i].n_prim,
                          model->buffer[i].index_buffer.sbuffer,
                          model->buffer[i].index_buffer.ibuffer,
                          Interp_tmap_flags);
...
int model_interp_get_texture(texture_info * tinfo, fix base_frametime) 
{
    ...
    
    {
        ...
        f2fl((game_get_overall_frametime() -
              base_frametime) % fl2f(total_time));
...
 void model_set_warp_globals(float scale_x, float scale_y, float scale_z,
                               int bitmap_id, float alpha) 
{
...
int texture_info::LoadTexture(char *filename, char *dbg_name = "<UNKNOWN>") 
{
    ...
    bm_load_either(filename, NULL, NULL, NULL, 1, CF_TYPE_MAPS);
