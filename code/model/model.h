...
void parse_triggersint(int &n_trig, queued_animation ** triggers,
                       char *props);
...
typedef struct glow_point_bank
{
  // glow bank structure -Bobboau
  int type;
  int glow_timestamp;
  int on_time;
  int off_time;
...
void model_render(int model_num, matrix * orient, vec3d * pos, uint flags =
                  MR_NORMAL, int objnum = -1, int lighting_skip =
                  -1, int *replacement_textures = NULL);

// Renders just one particular submodel on a model.
// See MR_? defines for values for flags
void submodel_render(int model_num, int submodel_num, matrix * orient,
                     vec3d * pos, uint flags = MR_NORMAL, int objnum =
                     -1, int *replacement_textures = NULL);
...
extern int model_find_2d_bound(int model_num, matrix * orient, vec3d * pos,
...
// rect.
extern int model_find_2d_bound_min(int model_num, matrix * orient,
...
extern int model_rotate_gun(int model_num, model_subsystem * turret,
                            matrix * orient, angles * base_angles,
                            angles * gun_angles, vec3d * pos, vec3d * dst,
...
// gets stuffed.
extern void submodel_rotate(model_subsystem * psub,
...
extern void model_find_submodel_offset(vec3d * outpnt, int model_num,
                                       int sub_model_num);
...
extern void model_find_world_point(vec3d * outpnt, vec3d * mpnt,
                                   int model_num, int sub_model_num,
                                   matrix * objorient, vec3d * objpos);
...
// reference, and given the object's orient and position,
// return the point in 3-space in outpnt.
extern void model_find_world_dir(vec3d * out_dir, vec3d * in_dir,
                                 int model_num, int sub_model_num,
                                 matrix * objorient, vec3d * objpos);
...
// data.
int model_which_octant_distant(vec3d * pnt, int model_num,
                               matrix * model_orient, vec3d * model_pos,
                               model_octant ** oct);
...
int model_which_octant(vec3d * pnt, int model_num, matrix * model_orient,
                       vec3d * model_pos, model_octant ** oct);
...
void model_set_warp_globals(float scale_x = 1.0f, float scale_y =
                            1.0f, float scale_z = 1.0f, int bitmap_id =
                            -1, float alpha = -1.0f);
