...
int mc_ray_boundingbox(vec3d * min, vec3d * max, vec3d * p0, vec3d * pdir,
                       vec3d * hitpos) 
{
    ...
    
    {
      ...
       return fvi_ray_boundingbox(&sphere_mod_min, &sphere_mod_max, p0,
                                    pdir, hitpos);
...
static void mc_check_face(int nv, vec3d ** verts, vec3d * plane_pnt,
                           float face_rad, vec3d * plane_norm,
                           uv_pair * uvl_list, int ntmap, ubyte * poly) 
{
    ...
    dist =
    fvi_ray_plane(NULL, plane_pnt, plane_norm, &Mc_p0, &Mc_direction, 0.0f);
...
static void mc_check_sphereline_face(int nv, vec3d ** verts,
                                     vec3d * plane_pnt, float face_rad,
                                     vec3d * plane_norm, uv_pair * uvl_list,
                                     int ntmap, ubyte * poly) 
{
  ...
  int check_edges = 1;          // assume we'll check the edges.
    ...
    if (!fvi_sphere_plane
        (&hit_point, &Mc_p0, &Mc_direction, Mc->radius, plane_norm, plane_pnt,
         &face_t, &delta_t))
    ...
    
    {
        ...
        
        {
          ...
          vm_vec_scale_add(&temp_sphere, &Mc_p0, &Mc_direction,
                            Mc->hit_dist);
...
void model_collide_flatpoly(ubyte * p) 
{
    ...
    {
      mc_check_face(nv, points, vp(p + 20), fl(p + 32), vp(p + 8), NULL, -1,
                     p);
...
 void model_collide_sortnorm(ubyte * p) 
{
    ...
    
    {
           ...
           (vp(p + 56), vp(p + 68), &Mc_p0, &Mc_direction, NULL))
    ...
    
    {
        ...
        
        {
          ...
          break;
               ...
               (vp(p + 8), vp(p + 20), &Mc_p0, &Mc_direction, NULL))
...
 bool mc_shield_check_common(shield_tri * tri) 
{
  ...
   if (!(Mc->flags & MC_CHECK_SPHERELINE))    // Don't do this test for sphere colliding against shields
    {
        ...
        // lies in
        dist =
        fvi_ray_plane(NULL, points[0], &tri->norm, &Mc_p0, &Mc_direction,
                      0.0f);
  ...
  else                          // Sphere check against shield
    {
        ...
        mc_check_sphereline_face(3, points, points[0], 10000.0f, &tri->norm,
                                 NULL, 0, NULL);
  ...
  int first_submodel;           // Which submodel gets returned as hit if MC_ONLY_SPHERE specified
    ...
    
    {
        ...
        
        {
            ...
            fvi_ray_sphere(&Mc->hit_point_world, Mc->p0, Mc->p1, Mc->pos,
                           model_radius);
        ...
        
        {
            ...
            fvi_segment_sphere(&Mc->hit_point_world, Mc->p0, Mc->p1, Mc->pos,
                               model_radius);
