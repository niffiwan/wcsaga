...
void std_multi_init_framecap_slider(HWND hwndDlg)
{
  ...
  Framecap_trackbar =
    CreateWindowEx(0, TRACKBAR_CLASS, NULL, WS_CHILD | WS_VISIBLE, 10, 10,
...
void std_multi_init_multi_controls(HWND hwndDlg)
{
  ...
  Standalone_goals = CreateWindowEx(0, WC_TREEVIEW, XSTR("Tree View", 921),
                                    ...
                                    TVS_HASLINES, GOALVIEW_X, GOALVIEW_Y,
                                    GOALVIEW_W, GOALVIEW_H, hwndDlg, NULL,
...
void std_pinfo_display_player_info(net_player * p)
{
  ...
  STD_ADDSTRING(Player_mstats[8],
                (ptr->mp_shots_fired >
                 0) ? (int) ((float) 100.0 * ((float) ptr->mp_bonehead_hits /
                                              (float) ptr->
                                              mp_shots_fired)) : 0);
  ...
  STD_ADDSTRING(Player_mstats[13],
                (ptr->ms_shots_fired >
                 0) ? (int) ((float) 100.0 * ((float) ptr->ms_bonehead_hits /
                                              (float) ptr->
                                              ms_shots_fired)) : 0);
}
...
void std_add_chat_text(char *text, int player_index, int add_id)
{
  ...
  if (Godstuff_player_messages_HDC)
    GetTextExtentPoint32(Godstuff_player_messages_HDC, format, strlen(format),
                         &text_size);
...
void std_build_title_string(char *str)
{
  ...
  if (SAGA_VERSION_BUILD == 0 && SAGA_VERSION_REVIS == 0)
    {
      snprintf(ver_str, sizeof(ver_str) - 1, "%i.%i", SAGA_VERSION_MAJOR,
               SAGA_VERSION_MINOR);
    }
  else if (SAGA_VERSION_REVIS == 0)
    {
      snprintf(ver_str, sizeof(ver_str) - 1, "%i.%i.%i", SAGA_VERSION_MAJOR,
               SAGA_VERSION_MINOR, SAGA_VERSION_BUILD);
    }
  else
    {
      snprintf(ver_str, sizeof(ver_str) - 1, "%i.%i.%i.%i",
  ...
  snprintf(temp, sizeof(temp) - 1, "%s %s", XSTR("FreeSpace Standalone", 935),
           ver_str);
...
BOOL CALLBACK std_message_handler_proc(HWND hwndDlg, UINT uMsg, WPARAM wParam,
                                       LPARAM lParam)
{
  switch (uMsg)
    {
...
BOOL std_create_standalone_window()
{
  ...
  Standalone_hwnd =
    CreateWindowEx(0, "FS2StandaloneClass", "FreeSpace2 Standalone", WS_POPUP,
                   CW_USEDEFAULT, CW_USEDEFAULT, 10, 10, NULL, NULL, hInst,
                   NULL);
