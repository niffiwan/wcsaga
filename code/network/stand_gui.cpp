...
// dialog proc for this dialog
  BOOL CALLBACK std_gen_dialog_proc(HWND hwndDlg, UINT uMsg, WPARAM wParam,
                                    LPARAM lParam) 
{
...
void std_create_gen_dialog(char *title) 
{
    ...
    Multi_gen_dialog =
    CreateDialog(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_GEN), Psht,
...
int std_connect_set_connect_count() 
{
    ...
    // set the text itself
    ctrl =
    GetDlgItem(Page_handles[CONNECT_PAGE],
               (int) MAKEINTRESOURCE(IDC_CON_COUNT));
...
void std_connect_set_host_connect_status() 
{
    ...
    ctrl =
    GetDlgItem(Page_handles[CONNECT_PAGE],
               (int) MAKEINTRESOURCE(IDC_HOST_IS));
...
void std_connect_add_ip_string(char *string) 
{
    ...
    ctrl =
    GetDlgItem(Page_handles[CONNECT_PAGE],
               (int) MAKEINTRESOURCE(IDC_CONPING));
...
void std_connect_remove_ip_string(char *string) 
{
    ...
    loc =
    SendMessage(ctrl, LB_FINDSTRING, (WPARAM) - 1, (LPARAM) (LPCTSTR) string);
...
void std_connect_set_ip_string(char *lookup, char *string) 
{
    ...
    // "prefix" which is the net_players name
    loc =
    SendMessage(ctrl, LB_FINDSTRING, (WPARAM) - 1, (LPARAM) (LPCTSTR) lookup);
...
void std_connect_handle_name_change() 
{
    ...
    
    {
        ...
        // get the new text
        SendMessage(Multi_std_name, EM_GETLINE, (WPARAM) 0,
                    (LPARAM) (LPCSTR) buf);
        ...
        strncpy(Multi_options_g.std_pname, buf,
...
void std_connect_handle_passwd_change() 
{
    ...
    // get the new text
    SendMessage(Multi_std_host_passwd, EM_GETLINE, (WPARAM) 0,
                (LPARAM) (LPCSTR) buf);
   
    // just copy it over for now. we may want to process this more later on
    strncpy(Multi_options_g.std_passwd, buf,
            sizeof(Multi_options_g.std_passwd));
...
// message handler for the connect tab
  BOOL CALLBACK connect_proc(HWND hwndDlg, UINT uMsg, WPARAM wParam,
                             LPARAM lParam) 
{
    ...
    case WM_INITDIALOG:
        ...
        Multi_std_name =
        GetDlgItem(hwndDlg, (int) MAKEINTRESOURCE(IDC_STD_NAME));
      SendMessage(Multi_std_name, EM_SETLIMITTEXT, (WPARAM) MAX_GAMENAME_LEN,
                   (LPARAM) 0);
      Multi_std_namechange_force = 1;
       
        // create the standalone host password input box
        Multi_std_host_passwd =
        GetDlgItem(hwndDlg, (int) MAKEINTRESOURCE(IDC_STD_HOST_PASSWD));
      SendMessage(Multi_std_host_passwd, EM_SETLIMITTEXT,
                   (WPARAM) STD_PASSWD_LEN, (LPARAM) 0);
...
void std_multi_update_goals() 
{
    ...
    
    {
        ...
        std_multi_get_goal_item(Mission_goals[idx].name,
                                Mission_goals[idx].type & GOAL_TYPE_MASK);
...
void std_multi_handle_framecap_scroll(HWND ctrl) 
{
    ...
    sprintf(pos_text, "%d", pos);
                 ...
                 (Page_handles[MULTIPLAYER_PAGE],
                  (int) MAKEINTRESOURCE(IDC_FRAMECAP_STATIC)), pos_text);
...
void std_multi_init_framecap_slider(HWND hwndDlg) 
{
    ...
    Framecap_trackbar =
    CreateWindowEx(0, TRACKBAR_CLASS, NULL, WS_CHILD | WS_VISIBLE, 10, 10,
                   300, 30, hwndDlg, NULL, GetModuleHandle(NULL), NULL);
...
void std_multi_init_multi_controls(HWND hwndDlg) 
{
    ...
    Standalone_FPS =
    GetDlgItem(hwndDlg, (int) MAKEINTRESOURCE(IDC_STANDALONE_FPS));
   
    // create the missiontime text box
    Standalone_missiontime =
    GetDlgItem(hwndDlg, (int) MAKEINTRESOURCE(IDC_STANDALONE_MTIME));
   
    // create the mission name text box
    Standalone_mission_name =
    GetDlgItem(hwndDlg, (int) MAKEINTRESOURCE(IDC_MISSION_NAME));
   
    // create the netgame max players text box
    Std_ng_max_players =
    GetDlgItem(hwndDlg, (int) MAKEINTRESOURCE(IDC_NG_MAXPLAYERS));
   
    // create the netgame max observers text box
    Std_ng_max_observers =
    GetDlgItem(hwndDlg, (int) MAKEINTRESOURCE(IDC_NG_MAXOBSERVERS));
   
    // create the netgame security text box
    Std_ng_security =
    GetDlgItem(hwndDlg, (int) MAKEINTRESOURCE(IDC_NG_SECURITY));
   
    // create the netgame respawns # text box
    Std_ng_respawns =
    GetDlgItem(hwndDlg, (int) MAKEINTRESOURCE(IDC_NG_RESPAWNS));
    ...
    Standalone_goals =
    CreateWindowEx(0, WC_TREEVIEW, XSTR("Tree View", 921),
...
// message handler for the multiplayer tab
  BOOL CALLBACK multi_proc(HWND hwndDlg, UINT uMsg, WPARAM wParam,
                           LPARAM lParam) 
{
...
void std_pinfo_display_player_info(net_player * p) 
{
    ...
    scoring_struct * ptr = &p->m_player->stats;
                 ...
                 (ptr->p_shots_fired >
                  0) ? (int) ((float) 100.0 * ((float) ptr->p_shots_hit /
                                               (float) ptr->
                                               p_shots_fired)) : 0);
  STD_ADDSTRING(Player_stats[8],
                 (ptr->p_shots_fired >
                   0) ? (int) ((float) 100.0 * ((float) ptr->p_bonehead_hits /
                                                (float) ptr->
                                                p_shots_fired)) : 0);
                 ...
                 (ptr->s_shots_fired >
                  0) ? (int) ((float) 100.0 * ((float) ptr->s_shots_hit /
                                               (float) ptr->
                                               s_shots_fired)) : 0);
  STD_ADDSTRING(Player_stats[13],
                 (ptr->s_shots_fired >
                   0) ? (int) ((float) 100.0 * ((float) ptr->s_bonehead_hits /
                                                (float) ptr->
                                                s_shots_fired)) : 0);
    ...
    STD_ADDSTRING(Player_mstats[0], ptr->m_score);
                 ...
                 (ptr->mp_shots_fired >
                  0) ? (int) ((float) 100.0 * ((float) ptr->mp_shots_hit /
                                               (float) ptr->
                                               mp_shots_fired)) : 0);
  STD_ADDSTRING(Player_mstats[8],
                 (ptr->mp_shots_fired >
                   0) ? (int) ((float) 100.0 *
                               ((float) ptr->mp_bonehead_hits /
                                (float) ptr->mp_shots_fired)) : 0);
                 ...
                 (ptr->ms_shots_fired >
                  0) ? (int) ((float) 100.0 * ((float) ptr->ms_shots_hit /
                                               (float) ptr->
                                               ms_shots_fired)) : 0);
  STD_ADDSTRING(Player_mstats[13],
                 (ptr->ms_shots_fired >
                   0) ? (int) ((float) 100.0 *
                               ((float) ptr->ms_bonehead_hits /
                                (float) ptr->ms_shots_fired)) : 0);
...
void std_pinfo_add_player_list_item(net_player * p) 
{
    ...
    // add the item
    SendMessage(Player_name_list, CB_ADDSTRING, (WPARAM) 0,
...
void std_pinfo_remove_player_list_item(net_player * p) 
{
    ...
    // lookup thie player
    loc =
    SendMessage(Player_name_list, CB_FINDSTRINGEXACT, (WPARAM) - 1,
                (LPARAM) (LPCTSTR) p->m_player->callsign);
    ...
    {
      SendMessage(Player_name_list, CB_DELETESTRING, (WPARAM) loc,
                   (LPARAM) 0);
...
void std_pinfo_init_player_info_controls(HWND hwndDlg) 
{
    ...
    Player_name_list =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_PLAYER_LIST));
   
    // create the player ship type text box
    Player_ship_type =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_PSHIP_TYPE));
   
    // create the player ping time text box
    Player_ping_time =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_PING_TIME));
   
    // initialize the various and sundry statistics text controls (alltime)
    Player_stats[0] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_SCORE));
  Player_stats[1] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_KILL_COUNT));
  Player_stats[2] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_KILL_COUNT_BH));
  Player_stats[3] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_ASSISTS));
  Player_stats[4] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_PSHOTS));
  Player_stats[5] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_PHITS));
  Player_stats[6] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_PBHHITS));
  Player_stats[7] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_PPCT));
  Player_stats[8] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_PBHPCT));
  Player_stats[9] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_SSHOTS));
  Player_stats[10] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_SECHITS));
  Player_stats[11] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_SBHHITS));
  Player_stats[12] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_SPCT));
  Player_stats[13] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_SBHPCT));
   
    // initialize the various and sundry statistics text controls (this mission)
    Player_mstats[0] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_MSCORE));
  Player_mstats[1] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_MKILL_COUNT));
  Player_mstats[2] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_MKILL_COUNT_BH));
  Player_mstats[3] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_MASSISTS));
  Player_mstats[4] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_MPSHOTS));
  Player_mstats[5] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_MPHITS));
  Player_mstats[6] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_MPBHHITS));
  Player_mstats[7] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_MPPCT));
  Player_mstats[8] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_MPBHPCT));
  Player_mstats[9] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_MSSHOTS));
  Player_mstats[10] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_MSECHITS));
  Player_mstats[11] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_MSBHHITS));
  Player_mstats[12] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_MSPCT));
  Player_mstats[13] =
    GetDlgItem(Page_handles[PLAYER_INFO_PAGE],
               (int) MAKEINTRESOURCE(IDC_MSBHPCT));
...
int std_pinfo_player_is_active(net_player * p) 
{
    ...
    SendMessage(Player_name_list, CB_GETLBTEXT, (WPARAM) sel,
                (LPARAM) player);
   
    // if there is a valid player selected and he's the guy we want
    return ((strlen(player) != 0)
            && (strcmp(p->m_player->callsign, player) == 0)) ? 1 : 0;
}

 
// message handler for the player info tab
  BOOL CALLBACK player_info_proc(HWND hwndDlg, UINT uMsg, WPARAM wParam,
                                 LPARAM lParam) 
{
    ...
    case WM_COMMAND:
        ...
        case CBN_SELCHANGE:
            ...
            val =
            SendMessage(Player_name_list, CB_GETCURSEL, (WPARAM) 0,
                        (LPARAM) 0);
            ...
            
            {
                ...
                
                {
                    ...
                    {
                      std_pinfo_display_player_info(&Net_players
                                                     [player_num]);
...
void std_gs_add_god_player(net_player * p) 
{
    ...
    // add the item
    SendMessage(God_player_list, CB_ADDSTRING, (WPARAM) 0,
                (LPARAM) (LPCTSTR) p->m_player->callsign);
...
void std_gs_remove_god_player(net_player * p) 
{
    ...
    // lookup the player
    loc =
    SendMessage(God_player_list, CB_FINDSTRINGEXACT, (WPARAM) - 1,
                (LPARAM) (LPCTSTR) p->m_player->callsign);
    ...
    {
      SendMessage(God_player_list, CB_DELETESTRING, (WPARAM) loc,
                   (LPARAM) 0);
...
void std_gs_send_godstuff_message() 
{
  ...
  SendMessage(Godstuff_broadcast_text, EM_GETLINE, (WPARAM) 0,
               (LPARAM) & txt[0]);
...
void std_gs_clear_controls() 
{
    ...
    SendMessage(Godstuff_player_messages, LB_RESETCONTENT, (WPARAM) 0,
                (LPARAM) 0);
...
void std_gs_init_godstuff_controls(HWND hwndDlg) 
{
    ...
    God_player_list =
    GetDlgItem(Page_handles[GODSTUFF_PAGE],
               (int) MAKEINTRESOURCE(IDC_PLAYER_GOD_LIST));
   
    // initialize the framerate text box
    Godstuff_fps =
    GetDlgItem(Page_handles[GODSTUFF_PAGE],
               (int) MAKEINTRESOURCE(IDC_GODSTUFF_FPS));
   
    // initialize the messaging edit control
    Godstuff_broadcast_text =
    GetDlgItem(Page_handles[GODSTUFF_PAGE],
               (int) MAKEINTRESOURCE(IDC_GODSTUFF_BROADCAST));
  SendMessage(Godstuff_broadcast_text, EM_SETLIMITTEXT,
               (WPARAM) CHATBOX_MAX_LEN, (LPARAM) 0);
  SendMessage(Godstuff_broadcast_text, EM_FMTLINES, (WPARAM) TRUE,
               (LPARAM) 0);
   
    // create the player chatter list box
    Godstuff_player_messages =
    GetDlgItem(Page_handles[GODSTUFF_PAGE],
               (int) MAKEINTRESOURCE(IDC_GOD_CHAT));
    ...
    Godstuff_broadcast_button =
    GetDlgItem(Page_handles[GODSTUFF_PAGE],
               (int) MAKEINTRESOURCE(IDC_GODSTUFF_SENDMESS));
...
// message handler for the godstuff tab
  BOOL CALLBACK godstuff_proc(HWND hwndDlg, UINT uMsg, WPARAM wParam,
                              LPARAM lParam) 
{
    ...
    case WM_DESTROY:
      
      {
          ...
          {
            ReleaseDC(Godstuff_player_messages,
                       Godstuff_player_messages_HDC);
...
}  void std_debug_multilog_add_line(const char *str) 
{
    ...
    
    {
      ...
       SendMessage(Standalone_multilog_string, LB_ADDSTRING, 0,
                     (LPARAM) log_str.c_str());
       
        // reset our width, using best guess, so that we have a working hscroll
        uint h_size =
        SendMessage(Standalone_multilog_string, LB_GETHORIZONTALEXTENT, 0, 0);
       if ((h_size / 6) < log_str.size())
        
        {
          SendMessage(Standalone_multilog_string, LB_SETHORIZONTALEXTENT,
                       log_str.size() * 6, 0);
...
void std_debug_init_debug_controls(HWND hwndDlg) 
{
    ...
    Standalone_state_string =
    GetDlgItem(hwndDlg, (int) MAKEINTRESOURCE(IDC_STANDALONE_STATE));
    ...
    GetDlgItem(hwndDlg, (int) MAKEINTRESOURCE(IDC_MULTILOG));
  SendMessage(Standalone_multilog_string, LB_RESETCONTENT, 0, 0);
}  

// message handler for the godstuff tab
  BOOL CALLBACK debug_proc(HWND hwndDlg, UINT uMsg, WPARAM wParam,
                           LPARAM lParam) 
{
  ...
  0, 0};
    ...
    
    {
        ...
        
        {
                   ...
                   Net_players[player_index].m_player->callsign, text);
    ...
    if (Godstuff_player_messages_HDC)
    GetTextExtentPoint32(Godstuff_player_messages_HDC, format,
                          strlen(format), &text_size);
   if (Godstuff_longest_message < text_size.cx)
    Godstuff_longest_message = (int) text_size.cx;
   SendMessage(Godstuff_player_messages, LB_SETHORIZONTALEXTENT,
    ...
    SendMessage(Godstuff_player_messages, LB_INSERTSTRING, (WPARAM) - 1,
                (LPARAM) format);
   num_items =
    SendMessage(Godstuff_player_messages, LB_GETCOUNT, (WPARAM) 0,
                (LPARAM) 0);
   if (num_items > 19)
    
    {
      ret_val =
        SendMessage(Godstuff_player_messages, LB_SETTOPINDEX,
...
void std_build_title_string(char *str) 
{
    ...
    {
      snprintf(ver_str, sizeof(ver_str) - 1, "%i.%i", FS_VERSION_MAJOR,
                FS_VERSION_MINOR);
    ...
    {
      snprintf(ver_str, sizeof(ver_str) - 1, "%i.%i.%i", FS_VERSION_MAJOR,
    ...
    {
      snprintf(ver_str, sizeof(ver_str) - 1, "%i.%i.%i.%i", FS_VERSION_MAJOR,
    ...
    memset(temp, 0, 256);
              ...
              XSTR("FreeSpace Standalone", 935), ver_str);
...
 static HMENU std_create_systray_menu() 
{
    ...
    // Type of connection:
    snprintf(tstr, sizeof(tstr) - 1, "Connection Type: %s",
    ...
    {
      snprintf(tstr, sizeof(tstr) - 1, "FS2NetD Status: %s",
    ...
    // Mission name:
    snprintf(tstr, sizeof(tstr) - 1, "Mission: %s",
...
 BOOL CALLBACK std_message_handler_proc(HWND hwndDlg, UINT uMsg,
    ...
    
    {
      ...
      
      {
          ...
          
          {
              ...
              TrackPopupMenuEx(stdPopup, TPM_RETURNCMD | TPM_NONOTIFY,
                               cur_pos.x, cur_pos.y, Standalone_hwnd, NULL);
...
 BOOL std_create_standalone_window() 
{
  ...
  HINSTANCE hInst = GetModuleHandle(NULL);
    ...
    CreateWindowEx(0, "FS2StandaloneClass", "FreeSpace2 Standalone",
...
void std_init_standalone() 
{
    ...
    Standalone_thread =
    CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) standalone_process, NULL,
