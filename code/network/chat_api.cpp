...
void ChatInit(void) 
{
  ...
  memset(Getting_user_channel_info_for, 0,
          sizeof(Getting_user_channel_info_for));
  memset(Getting_user_tracker_info_for, 0,
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
              getsockopt(Chatsock, SOL_SOCKET, SO_ERROR, (char *) &err_val,
              ...
              snprintf(signon_str, SSIZE(signon_str),
                        NOX("/USER %s %s %s :%s"), NOX("user"), NOX("user"),
                        NOX("user"), Chat_tracker_id);
              SendChatString(signon_str, 1);
              snprintf(signon_str, SSIZE(signon_str), NOX("/NICK %s"),
                        Nick_name);
...
char *SendChatString(char *line, int raw) 
{
    ...
    
    {
        ...
        {
          snprintf(szCmd, SSIZE(szCmd),
                    NOX("PRIVMSG %s :\001ACTION %s\001\n\r"), szChat_channel,
                    line + strlen(NOX("/me ")));
...
#endif  /*  */
    {
        ...
        
        {
            ...
            
            {
              ...
              strncpy(return_string, Input_chat_buffer,
...
  char *GetWordNum(int num, char *l_String) 
{
    ...
    
    {
        ...
        strncpy(strreturn, l_String + ((token - strstart) + 1),
                sizeof(strreturn) - 1);
...
 int AddChatUser(char *nickname) 
{
    ...
    
    {
      ...
      strncpy(Firstuser->nick_name, nickname,
               sizeof(Firstuser->nick_name) - 1);
    ...
    
    {
      ...
      strncpy(Curruser->nick_name, nickname,
               sizeof(Curruser->nick_name) - 1);
...
 int RemoveChatUser(char *nickname) 
{
    ...
    
    {
        ...
        
        {
          ...
          AddChatCommandToQueue(CC_USER_LEAVING, Curruser->nick_name,
...
  char *ParseIRCMessage(char *Line, int iMode) 
{
    ...
    
    {
        ...
        
        {
          ...
          strncpy(szRemLine, Line + 1 + strlen(szPrefix),
                   sizeof(szRemLine) - 1);
    ...
    strncpy(szRemLine, Line + iPrefixLen + strlen(szCmd) + 2,
            sizeof(szRemLine) - 1);
    ...
    
    {
        ...
        {
          strncpy(szCTCPCmd, GetWordNum(0, szRemLine + 1),
                   sizeof(szCTCPCmd) - 1);
        ...
        {
          strncpy(szCTCPCmd, GetWordNum(0, szRemLine),
                   sizeof(szCTCPCmd) - 1);
        ...
        
        {
            ...
            strncpy(szRemLine,
                    ...
                    strlen(szCTCPCmd) + 6, sizeof(szRemLine) - 1);
          ...
          if (stricmp(szCTCPCmd + 1, NOX("ACTION")) == 0)
            
            {
                ...
                //Posture
                snprintf(szResponse, SSIZE(szResponse), "* %s %s", szNick,
                         szRemLine);
            ...
            {
              strncpy(szHackPrefix,
                       Line + iPrefixLen + strlen(szCmd) + strlen(szTarget) +
                       4, sizeof(szHackPrefix) - 1);
              szRemLine[strlen(szRemLine) - 1] = '\0';
              snprintf(szResponse, SSIZE(szResponse),
                        NOX("** CTCP %s %s %s"), szTarget, szCTCPCmd + 1,
                        szRemLine);
            ...
            
            {
                ...
                //sprintf(szTempLine,"NOTICE %s :\001VERSION Copyright(c)\001\n",szNick);
                   ...
                   1 + GetWordNum(0,
                                  Line + iPrefixLen + strlen(szCmd) +
                                  strlen(szTarget) + 4),
                   sizeof(szRemLine) - 1);
          szRemLine[strlen(szRemLine) - 1] = '\0';
          snprintf(szResponse, SSIZE(szResponse),
                    NOX("** CTCP Message from %s (%s)"), szNick, szRemLine);
        ...
        
        {
            ...
            
            {
                        ...
                        NOX("Private Message to <%s>: %s"), szNick,
                        pszTempStr);
            ...
            
            {
                        ...
                        NOX("Private Message from <%s>: %s"), szNick,
                        pszTempStr);
        ...
        
        {
          ...
          if (stricmp(szCTCPCmd + 1, NOX("PING")) == 0)
            ...
            strncpy(szRemLine,
                    ...
                    XSTR("** CTCP Message from %s (%s)", 635), szNick,
                    szRemLine);
    ...
    
    {
        ...
        
        {
            ...
            {
              strncpy(szChat_channel, GetWordNum(0, szRemLine),
                       sizeof(szChat_channel) - 1);
              AddChatCommandToQueue(CC_YOURCHANNEL, szChat_channel,
        ...
        //strcpy_s(NewMsg.Channel,szTarget);
                ...
                XSTR("** %s has joined %s", 636), szNick, szTarget);
    ...
    
    {
        ...
        
        {
            ...
            AddChatCommandToQueue(CC_KICKED, NULL, 0);
                ...
                XSTR("*** %s has kicked %s from channel %s (%s)", 637),
                szNick, szHackPrefix, szTarget, pszTempStr);
    ...
    
    {
        ...
        
        {
            ...
            //Yup, it's me!
            strncpy(Nick_name, GetWordNum(0, szRemLine),
                    sizeof(Nick_name) - 1);
        }
      char nicks[70];
      snprintf(nicks, SSIZE(nicks), "%s %s", szNick,
                GetWordNum(0, szRemLine));
      AddChatCommandToQueue(CC_NICKCHANGED, nicks, strlen(nicks) + 1);
      RemoveChatUser(szNick);
      AddChatUser(GetWordNum(0, szRemLine));
      snprintf(szResponse, SSIZE(szResponse),
                XSTR("*** %s is now known as %s", 638), szNick, GetWordNum(0,
                                                                           szRemLine));
    ...
    
    {
        ...
        snprintf(szResponse, SSIZE(szResponse), NOX("/PONG :%s"),
                 GetWordNum(0, szRemLine));
    ...
    
    {
      ...
      strncpy(szWhoisUser, GetWordNum(1, szRemLine),
               sizeof(szWhoisUser) - 1);
      Getting_user_tracker_error = 1;
      Getting_user_channel_error = 1;
       snprintf(szResponse, SSIZE(szResponse),
                  XSTR("**Error: %s is not online!", 639), szWhoisUser);
    ...
    
    {
        ...
        //{
        strncpy(User_req_tracker_id, GetWordNum(5, szRemLine),
    ...
    
    {
        ...
        //{
        strncpy(User_req_channel, GetWordNum(2, szRemLine),
    ...
    
    {
        ...
        
        {
          ...
          strncpy(channel_list_name, GetWordNum(1, szRemLine),
                   sizeof(channel_list_name) - 1);
          AddChannel(channel_list_name,
                      (short) atoi(GetWordNum(2, szRemLine)), sztopic);
  ...
  if (stricmp(szCmd, "376") == 0)      //end of motd, trigger autojoin...
    {
        ...
        // end of motd
        strncpy(szResponse, PXO_CHAT_END_OF_MOTD_PREFIX,
                sizeof(szResponse) - 1);
    ...
    
    {
      ...
      snprintf(szResponse, SSIZE(szResponse), "%s%s", PXO_CHAT_MOTD_PREFIX,
                pszTempStr);
    ...
    
    {
        ...
        //Channel Mode info
      char new_nick[33];
      snprintf(new_nick, SSIZE(new_nick), "%s%d", Orignial_nick_name,
                Nick_variety);
...
 void AddChannel(char *channel, unsigned short numusers, char *topic) 
{
    ...
    
    {
      ...
      strncpy(Firstchannel->channel_name, channel,
    ...
    
    {
      ...
      strncpy(Currchannel->channel_name, channel,
...
  char *GetTrackerIdByUser(char *nickname) 
{
    ...
    {
      strncpy(Getting_user_tracker_info_for, nickname,
...
 char *GetChannelByUser(char *nickname) 
{
    ...
    {
      strncpy(Getting_user_channel_info_for, nickname,
