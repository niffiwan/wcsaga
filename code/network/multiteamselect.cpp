...
static int
  Multi_ts_slot_icon_coords[MULTI_TS_NUM_SHIP_SLOTS][GR_NUM_RESOLUTIONS][2] =
{
  // x,y
  {
   //
...
static int
  Multi_ts_slot_text_coords[MULTI_TS_NUM_SHIP_SLOTS][GR_NUM_RESOLUTIONS][3] =
{
  // x,y,width
  {
   // alpha
...
static int
  Multi_ts_avail_coords[MULTI_TS_AVAIL_MAX_DISPLAY][GR_NUM_RESOLUTIONS][2] = {
  // x,y coords
  {
   //
   {
...
char
  Multi_ts_ship_info_lines[MULTI_TS_SHIP_INFO_MAX_LINES]
...
int Multi_ts_deleted_objnums[MULTI_TS_MAX_TVT_TEAMS *
...
int multi_ts_can_perform(int from_type, int from_index, int to_type,
                         int to_index, int ship_class, int player_index = -1);

// determine the kind of drag and drop operation this is
int multi_ts_get_dnd_type(int from_type, int from_index, int to_type,
                          int to_index, int player_index = -1);

// swap two player positions
int multi_ts_swap_player_player(int from_index, int to_index, int *sound,
                                int player_index = -1);

// move a player
int multi_ts_move_player(int from_index, int to_index, int *sound,
                         int player_index = -1);
...
void multi_ts_common_init()
{
  ...
  memset(Multi_ts_deleted_objnums, 0,
      ...
      if (Netgame.type_flags & NG_TYPE_TEAM)
        {
          ...
          if (pl->flags & NETINFO_FLAG_TEAM_CAPTAIN)
            {
              if ((Multi_ts_team[pl->p_info.team].multi_ts_player[slot_num] !=
                   NULL)
                  &&
                  (Objects
                   [Multi_ts_team[pl->p_info.team].multi_ts_objnum[slot_num]].
...
void multi_ts_assign_players_all()
{
  ...
  multi_ts_get_team_and_slot(Ships[shipnum].ship_name, &team_index,
                             &slot_index);
  multi_assign_player_ship(NET_PLAYER_INDEX(Netgame.host),
  ...
  while (objp != END_OF_LIST(&obj_used_list))
    {
      ...
      if ((objp->flags & OF_PLAYER_SHIP)
          && stricmp(Ships[objp->instance].ship_name, name_lookup))
        {
          // determine what team and slot this ship is
          multi_ts_get_team_and_slot(Ships[objp->instance].ship_name,
                                     &team_index, &slot_index);
          ...
          if (found)
            {
              multi_assign_player_ship(idx, objp,
      ...
      for (s_idx = 0; s_idx < MULTI_TS_NUM_SHIP_SLOTS; s_idx++)
        {
          ...
          if (Multi_ts_team[idx].multi_ts_flag[s_idx] >= 0)
            {
              ...
              change_ship_type(Objects[objnum].instance,
          ...
          else if (Multi_ts_team[idx].multi_ts_flag[s_idx] ==
                   MULTI_TS_FLAG_EMPTY)
            {
              ...
              // delete the ship
              ship_add_exited_ship(&Ships
                                   [Objects
                                    [Multi_ts_deleted_objnums
                                     [Multi_ts_num_deleted]].instance],
                                   SEF_PLAYER_DELETED);
              obj_delete(Multi_ts_deleted_objnums[Multi_ts_num_deleted]);
              ship_wing_cleanup(Objects
                                [Multi_ts_deleted_objnums
                                 [Multi_ts_num_deleted]].instance,
                                &Wings[Ships
                                       [Objects
                                        [Multi_ts_team[idx].
                                         multi_ts_objnum[s_idx]].instance].
                                       wingnum]);
          ...
          if ((Multi_ts_team[idx].multi_ts_player[s_idx] != NULL) &&
              !MULTI_CONNECTED((*Multi_ts_team[idx].multi_ts_player[s_idx])))
            {
              ...
              Objects[Multi_ts_team[idx].multi_ts_objnum[s_idx]].flags &=
                ~(OF_PLAYER_SHIP);
              obj_set_flags(&Objects
                            [Multi_ts_team[idx].multi_ts_objnum[s_idx]],
                            Objects[Multi_ts_team[idx].
                                    multi_ts_objnum[s_idx]].
                            flags | OF_COULD_BE_PLAYER);
            }
        }
    }
}
...
void multi_ts_blit_locked_button()
{
  ...
  if ((Multi_ts_team[Net_player->p_info.team].multi_players_locked)
      && (Multi_ts_locked_bitmaps[2] != -1))
    {
      gr_set_bitmap(Multi_ts_locked_bitmaps[2]);
      gr_bitmap(Multi_ts_buttons[gr_screen.res][MULTI_TS_LOCK].x,
                Multi_ts_buttons[gr_screen.res][MULTI_TS_LOCK].y);
    }
...
void multi_ts_init_graphics()
{
  ...
  Multi_ts_window.create(0, 0, gr_screen.max_w_unscaled,
  ...
  for (idx = 0; idx < MULTI_TS_NUM_BUTTONS; idx++)
    {
      ...
      Multi_ts_buttons[gr_screen.res][idx].button.
        set_highlight_action(common_play_highlight_sound);

      // set the ani for the button
      Multi_ts_buttons[gr_screen.res][idx].button.
        set_bmaps(Multi_ts_buttons[gr_screen.res][idx].filename);

      // set the hotspot
      Multi_ts_buttons[gr_screen.res][idx].button.
        link_hotspot(Multi_ts_buttons[gr_screen.res][idx].hotspot);
    }

  // add some text
  Multi_ts_window.add_XSTR("Briefing", 765,
                           Multi_ts_buttons[gr_screen.res][MULTI_TS_BRIEFING].
                           xt,
                           Multi_ts_buttons[gr_screen.res][MULTI_TS_BRIEFING].
                           yt,
                           &Multi_ts_buttons[gr_screen.
                                             res][MULTI_TS_BRIEFING].button,
                           UI_XSTR_COLOR_GREEN);
  Multi_ts_window.add_XSTR("Ship Selection", 1067,
                           Multi_ts_buttons[gr_screen.
                                            res][MULTI_TS_SHIP_SELECT].xt,
                           Multi_ts_buttons[gr_screen.
                                            res][MULTI_TS_SHIP_SELECT].yt,
                           &Multi_ts_buttons[gr_screen.res]
                           [MULTI_TS_SHIP_SELECT].button,
                           UI_XSTR_COLOR_GREEN);
  Multi_ts_window.add_XSTR("Weapon Loadout", 1068,
                           Multi_ts_buttons[gr_screen.
                                            res][MULTI_TS_WEAPON_SELECT].xt,
                           Multi_ts_buttons[gr_screen.
                                            res][MULTI_TS_WEAPON_SELECT].yt,
                           &Multi_ts_buttons[gr_screen.res]
                           [MULTI_TS_WEAPON_SELECT].button,
                           UI_XSTR_COLOR_GREEN);
  Multi_ts_window.add_XSTR("Commit", 1062,
                           Multi_ts_buttons[gr_screen.res][MULTI_TS_COMMIT].
                           xt,
                           Multi_ts_buttons[gr_screen.res][MULTI_TS_COMMIT].
                           yt,
                           &Multi_ts_buttons[gr_screen.res][MULTI_TS_COMMIT].
                           button, UI_XSTR_COLOR_PINK);
  Multi_ts_window.add_XSTR("Lock", 1270,
                           Multi_ts_buttons[gr_screen.res][MULTI_TS_LOCK].xt,
                           Multi_ts_buttons[gr_screen.res][MULTI_TS_LOCK].yt,
                           &Multi_ts_buttons[gr_screen.res][MULTI_TS_LOCK].
  ...
  for (idx = 0; idx < MAX_WSS_SLOTS; idx++)
    {
      ...
      if (Multi_ts_team[Net_player->p_info.team].multi_ts_flag[idx] ==
          MULTI_TS_FLAG_EMPTY)
        {
          ss_blit_ship_icon(Multi_ts_slot_icon_coords[idx][gr_screen.res]
                            [MULTI_TS_X_COORD],
                            Multi_ts_slot_icon_coords[idx][gr_screen.
                                                           res]
                            [MULTI_TS_Y_COORD], -1, 0);
        }
      else
        {
          ss_blit_ship_icon(Multi_ts_slot_icon_coords[idx][gr_screen.res]
                            ...
                            Multi_ts_slot_icon_coords[idx][gr_screen.
                                                           res]
                            [MULTI_TS_Y_COORD], Wss_slots[idx].ship_class,
  ...
  for (idx = 0; idx < MAX_WSS_SLOTS; idx++)
    {
      ...
      if (Multi_ts_team[Net_player->p_info.team].multi_ts_player[idx] != NULL)
        {
          ...
          strcpy_s(callsign,
                   Multi_ts_team[Net_player->p_info.team].
                   multi_ts_player[idx]->m_player->callsign);
        }
      else
        {
          // determine if this is a locked AI ship
          pobj =
            mission_parse_get_arrival_ship(Ships
                                           [Objects
                                            [Multi_ts_team
                                             [Net_player->p_info.
                                              team].multi_ts_objnum[idx]].
                                            instance].ship_name);
      ...
      gr_force_fit_string(callsign, CALLSIGN_LEN,
                          Multi_ts_slot_text_coords[idx][gr_screen.
                                                         res]
                          [MULTI_TS_W_COORD]);
      ...
      gr_string(Multi_ts_slot_text_coords[idx][gr_screen.res]
                [MULTI_TS_X_COORD] +
                ((Multi_ts_slot_text_coords[idx][gr_screen.res]
                  [MULTI_TS_W_COORD] - callsign_w) / 2),
                Multi_ts_slot_text_coords[idx][gr_screen.
                                               res][MULTI_TS_Y_COORD],
                callsign);
    }
}
      ...
      if (Ss_pool[idx] > 0)
        {
          ...
          if (ship_count < Multi_ts_avail_start)
            ...
            {
              // blit the icon
              ss_blit_ship_icon(Multi_ts_avail_coords[display_count]
                                [gr_screen.res][MULTI_TS_X_COORD],
                                Multi_ts_avail_coords[display_count]
                                [gr_screen.res][MULTI_TS_Y_COORD], idx,
                                multi_ts_avail_bmap_num(display_count));

              // blit the ship count available
              sprintf(count, "%d", Ss_pool[idx]);
              gr_set_color_fast(&Color_normal);
              gr_string(Multi_ts_avail_coords[display_count][gr_screen.res]
                        [MULTI_TS_X_COORD] - 20,
                        Multi_ts_avail_coords[display_count][gr_screen.
                                                             res]
                        [MULTI_TS_Y_COORD], count);
...
void multi_ts_init_snazzy()
{
  ...
  snazzy_menu_add_region(&Multi_ts_region[Multi_ts_snazzy_regions++], "",
                         TSWING_0_SHIP_0, 0);
  snazzy_menu_add_region(&Multi_ts_region[Multi_ts_snazzy_regions++], "",
                         TSWING_0_SHIP_1, 0);
  snazzy_menu_add_region(&Multi_ts_region[Multi_ts_snazzy_regions++], "",
...
void multi_ts_blit_ship_info()
{
  ...
  gr_set_color_fast(&Color_normal);
  gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD],
            y_start, XSTR("Class", 739));
  ...
  gr_set_color_fast(&Color_normal);
  gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD],
            y_start, XSTR("Type", 740));
  if ((sip->type_str != NULL) && strlen(sip->type_str))
    {
      gr_set_color_fast(&Color_bright);
      gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] +
                150, y_start, sip->type_str);
    }
  y_start += 10;
  ...
  gr_set_color_fast(&Color_normal);
  gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD],
            y_start, XSTR("Length", 741));
  if ((sip->ship_length != NULL) && strlen(sip->ship_length))
    {
      gr_set_color_fast(&Color_bright);
      gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] +
                150, y_start, sip->ship_length);
    }
  y_start += 10;
  ...
  gr_set_color_fast(&Color_normal);
  gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD],
            y_start, XSTR("Max Velocity", 742));
  if ((sip->velocity_str != NULL) && strlen(sip->velocity_str))
    {
      gr_set_color_fast(&Color_bright);
      gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] +
                150, y_start, sip->velocity_str);
    }
  y_start += 10;
  ...
  gr_set_color_fast(&Color_normal);
  gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD],
            y_start, XSTR("Maneuverability", 744));
  if ((sip->maneuverability_str != NULL) && strlen(sip->maneuverability_str))
    {
      gr_set_color_fast(&Color_bright);
      gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] +
                150, y_start, sip->maneuverability_str);
    }
  y_start += 10;
  ...
  gr_set_color_fast(&Color_normal);
  gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD],
            y_start, XSTR("Shields", -1));
  if ((sip->shields_str != NULL) && strlen(sip->shields_str))
    {
      gr_set_color_fast(&Color_bright);
      gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] +
                150, y_start, sip->shields_str);
    }
  y_start += 10;
  ...
  gr_set_color_fast(&Color_normal);
  gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD],
            y_start, XSTR("Armor", 745));
  if ((sip->armor_str != NULL) && strlen(sip->armor_str))
    {
      gr_set_color_fast(&Color_bright);
      gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] +
                150, y_start, sip->armor_str);
    }
  y_start += 10;
  ...
  gr_set_color_fast(&Color_normal);
  gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD],
            y_start, XSTR("Gun Mounts", 746));
  if ((sip->gun_mounts != NULL) && strlen(sip->gun_mounts))
    {
      gr_set_color_fast(&Color_bright);
      gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] +
                150, y_start, sip->gun_mounts);
    }
  y_start += 10;
  ...
  gr_set_color_fast(&Color_normal);
  gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD],
            y_start, XSTR("Missile Banks", 747));
  if ((sip->missile_banks != NULL) && strlen(sip->missile_banks))
    {
      gr_set_color_fast(&Color_bright);
      gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] +
                150, y_start, sip->missile_banks);
    }
  y_start += 10;
  ...
  gr_set_color_fast(&Color_normal);
  gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD],
            y_start, XSTR("Turrets", -1));
  if ((sip->turrets_str != NULL) && strlen(sip->turrets_str))
    {
      gr_set_color_fast(&Color_bright);
      gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] +
                150, y_start, sip->turrets_str);
    }
  y_start += 10;
  ...
  gr_set_color_fast(&Color_normal);
  gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD],
            y_start, XSTR("Manufacturer", 748));
  if ((sip->manufacturer_str != NULL) && strlen(sip->manufacturer_str))
    {
      gr_set_color_fast(&Color_bright);
      gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] +
                150, y_start, sip->manufacturer_str);
    }
  y_start += 10;
  ...
  if (blit)
    {
      ...
      gr_string(Multi_ts_status_coords[gr_screen.res][MULTI_TS_X_COORD] +
                ((Multi_ts_status_coords[gr_screen.res][MULTI_TS_W_COORD] -
                  text_w) / 2),
                Multi_ts_status_coords[gr_screen.res][MULTI_TS_Y_COORD],
                text);
    }
}
...
void multi_ts_handle_mouse()
{
  ...
  snazzy_region =
    snazzy_menu_do(Multi_ts_mask_data, Multi_ts_mask_w, Multi_ts_mask_h,
                   Multi_ts_snazzy_regions, Multi_ts_region, &snazzy_action,
                   0);
  ...
  if (snazzy_region != -1)
    {
      ...
      switch (region_type)
        ...
        case MULTI_TS_SLOT_LIST:
          ...
          region_empty =
            (Multi_ts_team[Net_player->p_info.team].
             multi_ts_flag[region_index] >= 0) ? 0 : 1;
          if (!region_empty)
            {
        ...
        case MULTI_TS_PLAYER_LIST:
          ...
          region_empty =
            (Multi_ts_team[Net_player->p_info.team].
             multi_ts_player[region_index] != NULL) ? 0 : 1;
          break;
        }
    }
  ...
  switch (region_type)
    ...
    case MULTI_TS_PLAYER_LIST:
      if ((Multi_ts_hotspot_index != region_index) && (region_index >= 0)
          &&
          (Multi_ts_team[Net_player->p_info.team].multi_ts_player
           [region_index] != NULL))
        {
          gamesnd_play_iface(SND_USER_SELECT);
        }
  ...
  if (!Multi_ts_carried_flag && Multi_ts_clicked_flag &&
      mouse_down(MOUSE_LEFT_BUTTON) && ((Multi_ts_clicked_x != mouse_x)
                                        || (Multi_ts_clicked_y != mouse_y)))
    {
  ...
  if (Multi_ts_carried_flag && !mouse_down(MOUSE_LEFT_BUTTON))
    {
      ...
      multi_ts_drop(Multi_ts_carried_from_type, Multi_ts_carried_from_index,
...
// can the specified player perform the action he is attempting
int multi_ts_can_perform(int from_type, int from_index, int to_type,
                         int to_index, int ship_class, int player_index)
{
  net_player *pl;
  int op_type;
  p_object *pobj;
  ...
  op_type =
    multi_ts_get_dnd_type(from_type, from_index, to_type, to_index,
                          player_index);
          ...
          pobj =
            mission_parse_get_arrival_ship(Ships
                                           [Objects
                                            [Multi_ts_team
                                             [pl->p_info.
                                              team].multi_ts_objnum
                                             [to_index]].instance].ship_name);
          if ((pobj == NULL) || !(pobj->flags & OF_PLAYER_SHIP))
            {
              return 0;
            }
...
void multi_ts_apply(int from_type, int from_index, int to_type, int to_index,
                    int ship_class, int player_index)
{
  ...
  switch (type)
    ...
    case TS_SWAP_PLAYER_PLAYER:
      nprintf(("Network", "Apply swap player player %d %d\n", from_index,
               to_index));
      update =
        multi_ts_swap_player_player(from_index, to_index, &sound,
                                    player_index);
      break;
    case TS_MOVE_PLAYER:
      ...
      if (MULTIPLAYER_HOST)
        {
          ...
          if (type == TS_SWAP_PLAYER_PLAYER)
            ...
            {
              size =
                store_wss_data(wss_data, MAX_PACKET_SIZE - 20, sound,
                               player_index);
              ...
              send_pslot_update_packet(pl->p_info.team, TS_CODE_PLAYER_UPDATE,
                                       -1);
            }
                   ...
                   int ship_class, int player_index)
{
  ...
  if (Net_player->flags & NETINFO_FLAG_GAME_HOST)
    {
      ...
      if (multi_ts_can_perform
          (from_type, from_index, to_type, to_index, ship_class,
           player_index))
        {
          multi_ts_apply(from_type, from_index, to_type, to_index, ship_class,
                         player_index);
        }
      else
        {
  ...
  // otherwise send a request to the host
    ...
    {
      send_wss_request_packet(Net_player->player_id, from_type, from_index,
                              to_type, to_index, -1, ship_class,
                              WSS_SHIP_SELECT);
    }
}
                                ...
                                int player_index)
{
  ...
  if (Multi_ts_team[pl->p_info.team].multi_ts_player[from_index] != NULL)
    {
      Multi_ts_team[pl->p_info.team].multi_ts_player[from_index]->p_info.
        ship_index = from_index;
      Multi_ts_team[pl->p_info.team].multi_ts_player[from_index]->p_info.
        ship_class = Wss_slots_teams[pl->p_info.team][from_index].ship_class;

      multi_assign_player_ship(NET_PLAYER_INDEX
                               (Multi_ts_team[pl->p_info.team].
                                multi_ts_player[from_index]),
                               &Objects[Multi_ts_team[pl->p_info.team].
                                        multi_ts_objnum[from_index]],
                               Wss_slots_teams[pl->p_info.team][from_index].
                               ship_class);
    }
  if (Multi_ts_team[pl->p_info.team].multi_ts_player[to_index] != NULL)
    {
      Multi_ts_team[pl->p_info.team].multi_ts_player[to_index]->p_info.
        ship_index = to_index;
      Multi_ts_team[pl->p_info.team].multi_ts_player[to_index]->p_info.
        ship_class = Wss_slots_teams[pl->p_info.team][to_index].ship_class;
      ...
      multi_assign_player_ship(NET_PLAYER_INDEX
                               ...
                               Wss_slots_teams[pl->p_info.team][to_index].
                               ship_class);
    }

  // update ship flags
  Objects[Multi_ts_team[pl->p_info.team].multi_ts_objnum[to_index]].flags &=
    ~(OF_COULD_BE_PLAYER);
  Objects[Multi_ts_team[pl->p_info.team].multi_ts_objnum[to_index]].flags &=
    ~(OF_PLAYER_SHIP);
  if (Multi_ts_team[pl->p_info.team].multi_ts_player[to_index] != NULL)
  ...
  Objects[Multi_ts_team[pl->p_info.team].multi_ts_objnum[from_index]].flags &=
    ~(OF_COULD_BE_PLAYER);
  Objects[Multi_ts_team[pl->p_info.team].multi_ts_objnum[from_index]].flags &=
    ~(OF_PLAYER_SHIP);
  if (Multi_ts_team[pl->p_info.team].multi_ts_player[from_index] != NULL)
...
void multi_ts_blit_carried_icon()
{
  ...
  switch (Multi_ts_carried_from_type)
    ...
    case MULTI_TS_SLOT_LIST:
      offset_x =
        Multi_ts_slot_icon_coords[Multi_ts_carried_from_index][gr_screen.
                                                               res]
        [MULTI_TS_X_COORD] - Multi_ts_clicked_x;
      offset_y =
        Multi_ts_slot_icon_coords[Multi_ts_carried_from_index][gr_screen.
                                                               res]
        [MULTI_TS_Y_COORD] - Multi_ts_clicked_y;

      // blit the icon
      ss_blit_ship_icon(x + offset_x, y + offset_y,
                        Multi_ts_carried_ship_class, 0);
      break;
    case MULTI_TS_AVAIL_LIST:
      offset_x =
        Multi_ts_avail_coords[Multi_ts_carried_from_index][gr_screen.
                                                           res]
        [MULTI_TS_X_COORD] - Multi_ts_clicked_x;
      offset_y =
        Multi_ts_avail_coords[Multi_ts_carried_from_index][gr_screen.
    ...
    case MULTI_TS_PLAYER_LIST:
      ...
      strcpy_s(callsign,
               Multi_ts_team[Net_player->p_info.team].
               multi_ts_player[Multi_ts_carried_from_index]->m_player->
               callsign);
      gr_force_fit_string(callsign, CALLSIGN_LEN,
                          Multi_ts_slot_text_coords
                          [Multi_ts_carried_from_index][gr_screen.res]
                          [MULTI_TS_W_COORD]);
      gr_get_string_size(&callsign_w, NULL, callsign);

      // calculate the offsets
      offset_x =
        (Multi_ts_slot_text_coords[Multi_ts_carried_from_index][gr_screen.res]
         [MULTI_TS_X_COORD] - Multi_ts_clicked_x) +
        ((Multi_ts_slot_text_coords[Multi_ts_carried_from_index]
          [gr_screen.res][MULTI_TS_W_COORD] - callsign_w) / 2);
      offset_y =
      ...
      if (multi_num_players() != 1)
        {
          popup(PF_USE_AFFIRMATIVE_ICON | PF_BODY_BIG, 1, POPUP_OK,
                XSTR("Players have not yet been assigned to their ships",
                     751));
          return;
        }
      else
  ...
  for (idx = 0; idx < MULTI_TS_NUM_SHIP_SLOTS; idx++)
    {
      ...
      if ((Multi_ts_team[Net_player->p_info.team].multi_ts_objnum[idx] != -1)
          &&
          ((Objects
            [Multi_ts_team[Net_player->p_info.team].multi_ts_objnum[idx]].
            flags & OF_COULD_BE_PLAYER)
           ||
           (Objects
            [Multi_ts_team[Net_player->p_info.team].multi_ts_objnum[idx]].
            flags & OF_PLAYER_SHIP)) && !multi_ts_disabled_slot(idx))
        {
          ...
          for (s_idx = 0; s_idx < MAX_SHIP_WEAPONS; s_idx++)
            {
              ...
              if ((Wss_slots_teams[Net_player->p_info.team][idx].wep[s_idx] >=
                   0)
          ...
          if (!primary_ok
              && !(The_mission.ai_profile->
                   flags & AIPF_MULTI_ALLOW_EMPTY_PRIMARIES))
            {
              return 2;
            }
          ...
          if (!secondary_ok
              && !(The_mission.ai_profile->
                   flags & AIPF_MULTI_ALLOW_EMPTY_SECONDARIES))
            {
              return 3;
            }
        }
          ...
          if (Multi_ts_team[team].multi_ts_flag[idx] != MULTI_TS_FLAG_NONE)
            {
              ...
              if (val)
                {
                  ADD_SHORT(Multi_ts_team[team].multi_ts_player[idx]->
                            player_id);

                  // should also update his p_info settings locally
                  Multi_ts_team[team].multi_ts_player[idx]->p_info.
                    ship_class = Wss_slots_teams[team][idx].ship_class;
                  Multi_ts_team[team].multi_ts_player[idx]->p_info.
                    ship_index = idx;
                }
...
void process_pslot_update_packet(ubyte * data, header * hinfo)
{
  ...
  switch (code)
      ...
      if (Net_player->flags & NETINFO_FLAG_AM_MASTER)
        {
          ...
          if (Netgame.type_flags & NG_TYPE_TEAM)
            {
              Assert(Net_players[player_index].
                     flags & NETINFO_FLAG_TEAM_CAPTAIN);
            }
    ...
    case TS_CODE_PLAYER_UPDATE:
      ...
      while (stop != 0xff)
        {
              ...
              // if we found him, assign him to this ship
              else
                {
                  ...
                  Net_players[player_index].p_info.ship_index =
                    (int) slot_num;
                  multi_assign_player_ship(player_index, &Objects[objnum],
                                           (int) ship_class);
          ...
          switch (val)
            ...
            case 1:
              ...
              break;
            case 2:
              obj_set_flags(&Objects[objnum],
                            Objects[objnum].flags | OF_COULD_BE_PLAYER);
              break;
            }
      ...
      if ((sound != -1) && !(Game_mode & GM_STANDALONE_SERVER)
