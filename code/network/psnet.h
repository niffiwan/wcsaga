...
extern int psnet_send(net_addr * who_to, void *data, int len, int flags =
                      PSNET_FLAG_RAW, int reliable_socket = 0);
extern int psnet_get(void *data, net_addr * from_addr, int flags =
                     PSNET_FLAG_RAW);
extern int psnet_broadcast(net_addr * who_to, void *data, int len, int flags =
                           PSNET_FLAG_RAW);

// functions for reliable socket stuff
extern int psnet_rel_send(PSNET_SOCKET socket, ubyte * data, int length,
                          int flags = PSNET_FLAG_RAW);
extern int psnet_rel_get(PSNET_SOCKET socket, ubyte * buffer, int max_length,
                         int flags = PSNET_FLAG_RAW);

extern int psnet_rel_check_for_listen(net_addr * addr);
extern void psnet_rel_connect_to_server(PSNET_SOCKET * s,
