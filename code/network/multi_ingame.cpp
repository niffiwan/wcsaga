...
void multi_handle_ingame_joiners() 
{
    ...
    
    {
        ...
        if (Net_players[idx].s_info.
            ingame_join_flags & INGAME_JOIN_FLAG_FILE_XFER)
        ...
        if ((Net_players[idx].flags & NETINFO_FLAG_MISSION_OK)
            && !(Net_players[idx].s_info.
                 ingame_join_flags & INGAME_JOIN_FLAG_LOADING_MISSION))
        ...
        if ((Net_players[idx].state == NETPLAYER_STATE_MISSION_LOADED)
            && !(Net_players[idx].s_info.
                 ingame_join_flags & INGAME_JOIN_FLAG_SENDING_SHIPS))
        ...
        if ((Net_players[idx].state == NETPLAYER_STATE_INGAME_SHIPS)
            && !(Net_players[idx].s_info.
                 ingame_join_flags & INGAME_JOIN_FLAG_SENDING_POST))
        ...
        if ((Net_players[idx].state == NETPLAYER_STATE_POST_DATA_ACK)
            && !(Net_players[idx].s_info.
                 ingame_join_flags & INGAME_JOIN_FLAG_PICK_SHIP))
        ...
        if (!
            (Net_players[idx].s_info.
             ingame_join_flags & INGAME_JOIN_FLAG_SENDING_SETS))
...
#define MIJ_JOIN            1
  ...
  Multi_ingame_join_buttons[GR_NUM_RESOLUTIONS][MULTI_INGAME_JOIN_NUM_BUTTONS]
  = 
{
...
 void multi_ingame_load_icons() 
{
    ...
    
    {
        ...
        if ((strlen(Ship_info[idx].icon_filename) > 0)
        ...
        
        {
            ...
            first_frame =
            bm_load_animation(Ship_info[idx].icon_filename, &num_frames);
...
void multi_ingame_select_init() 
{
  ...
  int idx;
    ...
    Multi_ingame_window.create(0, 0, gr_screen.max_w_unscaled,
                               gr_screen.max_h_unscaled, 0);
  Multi_ingame_window.
    set_mask_bmap(Multi_ingame_join_bitmap_mask_fname[gr_screen.res]);
    ...
    
    {
        ...
        // create the object
        Multi_ingame_join_buttons[gr_screen.res][idx].button.
        create(&Multi_ingame_window, "",
        ...
        Multi_ingame_join_buttons[gr_screen.res][idx].button.
        set_bmaps(Multi_ingame_join_buttons[gr_screen.res][idx].filename);
       
        // set the hotspot
        Multi_ingame_join_buttons[gr_screen.res][idx].button.
        link_hotspot(Multi_ingame_join_buttons[gr_screen.res][idx].hotspot);
    ...
    
    {
        ...
        add_XSTR(&Multi_ingame_join_text[gr_screen.res][idx]);
    }
   
    // create the list item select button
    Multi_ingame_select_button.create(&Multi_ingame_window, "",
                                      Mi_name_field[gr_screen.
                                                    res][MI_FIELD_X],
                                      Mi_name_field[gr_screen.
                                                    res][MI_FIELD_Y],
                                      Mi_width[gr_screen.res],
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                send_ingame_ship_request_packet(INGAME_SR_REQUEST,
                                                Multi_ingame_join_sig);
...
void multi_ingame_join_display_ship(object * objp, int y_start) 
{
  ...
  gr_string(Mi_name_field[gr_screen.res][MI_FIELD_X], y_start + 10,
             Ships[objp->instance].ship_name);
   
    // blit the ship class icon
    icon_num =
    multi_ingame_get_ship_class_icon(Ships[objp->instance].ship_info_index);
    ...
    y_spacing =
    (Mi_spacing[gr_screen.res] - (wp->num_primary_banks * 10)) / 2;
  for (idx = 0; idx < wp->num_primary_banks; idx++)
    
    {
      gr_string(Mi_primary_field[gr_screen.res][MI_FIELD_X],
                 y_start + y_spacing + (idx * 10),
                 Weapon_info[wp->primary_bank_weapons[idx]].name);
    }
   
    // blit the ship's secondary weapons
    y_spacing =
    (Mi_spacing[gr_screen.res] - (wp->num_secondary_banks * 10)) / 2;
  for (idx = 0; idx < wp->num_secondary_banks; idx++)
    
    {
      gr_string(Mi_secondary_field[gr_screen.res][MI_FIELD_X],
                 y_start + y_spacing + (idx * 10),
                 Weapon_info[wp->secondary_bank_weapons[idx]].name);
    }
   
    // blit the shield/hull integrity
    hud_shield_show_mini(objp,
                         Mi_status_field[gr_screen.res][MI_FIELD_X] + 15,
                         y_start + 3, 5, 7);
...
void multi_ingame_join_display_avail() 
{
    ...
    
    {
      ...
      gr_line(Mi_name_field[gr_screen.res][MI_FIELD_X] - 1, y_start - 1,
               (Mi_name_field[gr_screen.res][MI_FIELD_X] - 1) +
               (Mi_width[gr_screen.res] + 2), y_start - 1);
      gr_line(Mi_name_field[gr_screen.res][MI_FIELD_X] - 1,
               y_start + Mi_spacing[gr_screen.res] - 2,
               (Mi_name_field[gr_screen.res][MI_FIELD_X] - 1) +
               (Mi_width[gr_screen.res] + 2),
               y_start + Mi_spacing[gr_screen.res] - 2);
      gr_line(Mi_name_field[gr_screen.res][MI_FIELD_X] - 1, y_start,
               Mi_name_field[gr_screen.res][MI_FIELD_X] - 1,
               y_start + Mi_spacing[gr_screen.res] - 2);
      gr_line((Mi_name_field[gr_screen.res][MI_FIELD_X] - 1) +
               (Mi_width[gr_screen.res] + 2), y_start,
               (Mi_name_field[gr_screen.res][MI_FIELD_X] - 1) +
               (Mi_width[gr_screen.res] + 2),
               y_start + Mi_spacing[gr_screen.res] - 2);
    ...
    
    {
           ...
           (Ships[Objects[moveup->objnum].instance].
...
void multi_ingame_handle_timeout() 
{
    ...
    {
      multi_quit_game(PROMPT_NONE, MULTI_END_NOTIFY_INGAME_TIMEOUT,
...
void process_ingame_ships_packet(ubyte * data, header * hinfo) 
{
    ...
    
    {
        ...
        
        {
            ...
            p_objp =
            mission_parse_get_arrival_ship((ushort)
                                           (net_signature -
                                            (ushort) net_sig_modify));
    ...
    
    {
        ...
        
        {
          ...
          multi_ts_get_team_and_slot(Ships[idx].ship_name, &team_val,
                                      &slot_index);
            ...
            
            {
                ...
                change_ship_type(objp->instance,
                                 ...
                                 ship_class);
              wl_bash_ship_weapons(&Ships[idx].weapons,
                                    &Wss_slots_teams[team_val][slot_index]);
...
void multi_ingame_validate_players() 
{
    ...
    
    {
        ...
        
        {
          ...
           multi_ts_get_shipname(ship_name, Net_players[i].p_info.team,
...
void process_ingame_ship_request_packet(ubyte * data, header * hinfo) 
{
    ...
    case INGAME_SR_REQUEST:
        ...
        {
          send_ingame_ship_request_packet(INGAME_SR_DENY, 0,
      ...
      Net_players[player_num].flags &= ~(NETINFO_FLAG_INGAME_JOIN);
      multi_assign_player_ship(player_num, objp,
                                Ships[objp->instance].ship_info_index);
       
        // update his ets and link status stuff
        multi_server_update_player_weapons(&Net_players[player_num],
                                           &Ships[objp->instance]);
        ...
        // ack him
        send_ingame_ship_request_packet(INGAME_SR_CONFIRM, OBJ_INDEX(objp),
                                        &Net_players[player_num]);
    ...
    case INGAME_SR_CONFIRM:
      ...
      GET_UINT(respawn_count);
      ...
      multi_assign_player_ship(MY_NET_PLAYER_NUM, objp,
                                Ships[objp->instance].ship_info_index);
        ...
        multi_ts_get_team_and_slot(Player_ship->ship_name, &team,
                                   &slot_index);
      Assert(team != -1);
      Assert(slot_index != -1);
      change_ship_type(objp->instance,
                        Wss_slots_teams[team][slot_index].ship_class);
      wl_bash_ship_weapons(&Player_ship->weapons,
                            &Wss_slots_teams[team][slot_index]);
      ...
      Assert(num_secondary_banks ==
              Player_ship->weapons.num_secondary_banks);
...
void multi_ingame_send_ship_update(net_player * p) 
{
    ...
    
    {
        ...
        
        {
            ...
            // send the update
            send_ingame_ship_update_packet(p,
                                           &Ships[Objects[moveup->objnum].
                                                  instance]);
