...
void multi_df_debrief_init()
{
  ...
  Multi_df_window.create(0, 0, gr_screen.max_w_unscaled,
  ...
  // add some text
  w = &Multi_df_window;
  w->add_XSTR("Accept", 1035,
              Multi_df_buttons[gr_screen.res][ACCEPT_BUTTON].xt,
...
void multi_df_debrief_do()
{
  ...
  gr_string(Kill_matrix_title_coords[gr_screen.res][0],
            Kill_matrix_title_coords[gr_screen.res][1], buf);
              ...
              for (idx = 0; idx < MAX_PLAYERS; idx++)
                {
                  if (MULTI_CONNECTED(Net_players[idx])
                      ...
                      && (Net_players[idx].m_player != NULL))
                    {
                      scoring_backout_accept(&Net_players[idx].m_player->
                                             stats);
                    }
                }
            }
  ...
  for (idx = 0; idx < MAX_PLAYERS; idx++)
    {
      if (MULTI_CONNECTED(Net_players[idx])
          ...
          && (Net_players[idx].m_player != NULL))
        {
          ...
          ml_printf("Dogfight debrief stats for %s",
                    Net_players[idx].m_player->callsign);
          for (s_idx = 0; s_idx < MAX_PLAYERS; s_idx++)
            {
              ml_printf("%d",
                        Net_players[idx].m_player->stats.
                        m_dogfight_kills[s_idx]);
            }
...
void multi_df_blit_kill_matrix()
{
  ...
  float max_item_width =
    ((float) Multi_df_display_coords[gr_screen.res][2] -
     40.0f) / (float) (Multi_df_score_count + 1);
  ...
  for (idx = 0; idx < Multi_df_score_count; idx++)
    {
      ...
      gr_force_fit_string(squashed_string, CALLSIGN_LEN,
                          (int) max_text_width);
      ...
      gr_string(cx + (int) ((max_item_width - (float) str_len) / 2.0f), cy,
                squashed_string);
  ...
  for (idx = 0; idx < Multi_df_score_count; idx++)
    {
      ...
      for (s_idx = 0; s_idx < Multi_df_score_count; s_idx++)
        {
          ...
          if (s_idx == idx)
          ...
          else
            {
              ...
              if (Multi_df_score[idx].np_index >= 0)
                {
                  gr_set_color_fast(Color_netplayer
                                    [Multi_df_score[idx].np_index]);
                }
            }
          ...
          gr_force_fit_string(squashed_string, CALLSIGN_LEN,
                              (int) max_text_width);
          gr_get_string_size(&str_len, NULL, squashed_string);
          gr_string(cx + (int) ((max_item_width - (float) str_len) / 2.0f),
                    cy, squashed_string);
      ...
      gr_get_string_size(&str_len, NULL, squashed_string);
      gr_string(Multi_df_display_coords[gr_screen.res][0] +
                Multi_df_display_coords[gr_screen.res][2] -
                (MULTI_DF_TOTAL_ADJUST + str_len), cy, squashed_string);
