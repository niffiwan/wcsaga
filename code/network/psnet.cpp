...
 DWORD(__stdcall * pRasEnumConnections) (LPRASCONN lprasconn, LPDWORD lpcb,
                                           LPDWORD lpcConnections) = NULL;
DWORD(__stdcall * pRasGetConnectStatus) (HRASCONN hrasconn,
                                          LPRASCONNSTATUS lprasconnstatus) =
  NULL;
DWORD(__stdcall * pRasGetProjectionInfo) (HRASCONN hrasconn,
...
void psnet_ras_status() 
{
    ...
    (DWORD(__stdcall *) (LPRASCONN, LPDWORD, LPDWORD))
    GetProcAddress(ras_handle, NOX("RasEnumConnectionsA"));
    ...
    (DWORD(__stdcall *) (HRASCONN, LPRASCONNSTATUS))
    GetProcAddress(ras_handle, NOX("RasGetConnectStatusA"));
    ...
    GetProcAddress(ras_handle, NOX("RasGetProjectionInfoA"));
    ...
    
    {
        ...
        
        {
                  ...
                  (status.rasconnstate ==
                   RASCS_Connected) ? "Connected" : "Not Connected"));
       
        // get the projection informatiom
        size = sizeof(projection);
      projection.dwSize = size;
      rval =
        pRasGetProjectionInfo(rasbuffer[i].hrasconn, RASP_PppIp, &projection,
                              &size);
  ...
  uint psnet_set_socket_mode(SOCKET sock_id, int opt, BOOL toggle) 
{
           ...
           (sock_id, SOL_SOCKET, opt, (LPSTR) & toggle, sizeof(toggle)));
...
int psnet_init_reliable(ushort port, int should_listen, int type) 
{
  ...
  SOCKADDR_IPX ipx_addr;        // IPX socket structure
    ...
    
    {
                ...
                "Unable to initialize reliable socket on port %d (%d)!\n",
    ...
    
    {
      ...
      if (bind(sock, (SOCKADDR *) & ipx_addr, sizeof(SOCKADDR_IPX)) ==
           SOCKET_ERROR)
        
        {
          nprintf(("Network",
                    "Unable to bind reliable socket on port %d (%d)!\n", port,
                    WSAGetLastError()));
    ...
    
    {
        ...
        
        {
                    ...
                    "Unable to make reliable socket non-blocking -- %d",
                    WSAGetLastError()));
        ...
        
        {
                    ...
                    "Error setting socket to reuse address -- %d\n", error));
...
 void psnet_socket_options(SOCKET sock) 
{
    ...
    broadcast = 1;
       ...
       (sock, SOL_SOCKET, SO_BROADCAST, (LPSTR) & broadcast,
        sizeof(broadcast)))
    ...
    
    {
        ...
        setsockopt(sock, SOL_SOCKET, SO_RCVBUF, (LPSTR) & trysize,
                   sizeof(trysize));
    ...
    
    {
        ...
        setsockopt(sock, SOL_SOCKET, SO_SNDBUF, (LPSTR) & trysize,
                   sizeof(trysize));
...
void psnet_init(int protocol, int port_num) 
{
    ...
    
    {
                ...
                "PSNET RELIABLE : init failed - no networking available!\n"));
    ...
    
    {
                ...
                "Couldn't initialize TCP reliable socket (OLD SCHOOL) (%d)! Invalidating TCP\n",
                Tcp_failure_code));
      return;
    }
   TCP_listen_socket =
    psnet_init_reliable((u_short) (Psnet_default_port - 1), 1, NET_TCP);
  if (TCP_listen_socket == INVALID_SOCKET)
    
    {
      Tcp_failure_code = WSAGetLastError();
      nprintf(("Network",
                "Couldn't initialize TCP listen socket (OLD SCHOOL) (%d)! Invalidating TCP\n",
                Tcp_failure_code));
    ...
    
    {
      ...
      //memcpy(&sockaddr.sin_addr.s_addr,&my_ip,sizeof(uint));
           ...
           (TCP_reliable_socket, (SOCKADDR *) & sockaddr,
...
void psnet_rel_close_socket(PSNET_SOCKET * p_sockp) 
{
    ...
    setsockopt(*sockp, SOL_SOCKET, SO_LINGER, (char *) &l_val,
               sizeof(linger));
...
int psnet_use_protocol(int protocol) 
{
    ...
    
    {
      ...
      if (getsockname(IPX_socket, (SOCKADDR *) & ipx_addr, &len) ==
           SOCKET_ERROR)
        
        {
          nprintf(("Network",
                    "Unable to get sock name for IPX unreliable socket (%d)\n",
                    WSAGetLastError()));
...
void psnet_rel_connect_to_server(PSNET_SOCKET * psocket,
    ...
    
    {
      ...
      int packet_id, id;
        ...
        
        {
            ...
            
            {
                ...
                recvfrom(socket, (char *) packet_read.data, MAX_PACKET_SIZE,
                         0, (SOCKADDR *) & ipx_addr, &from_len);
            ...
            
            {
                ...
                recvfrom(socket, (char *) &packet_read, sizeof(packet_data),
                         0, (SOCKADDR *) & ipx_addr, &from_len);
      ...
      int shave_size;
        ...
        
        {
            ...
            shave_size =
            psnet_reliable_should_process(&from_addr, packet_read.data,
                                          read_len);
            ...
            
            {
                ...
                memcpy(packet_data.data, packet_read.data + shave_size,
                       read_len);
        ...
        if (!(flags & PSNET_FLAG_RAW)
            && (packet_data.flags & PSNET_FLAG_CHECKSUM))
        
        {
            ...
            
            {
                        ...
                        "bad checksum on incoming packet -- discarding\n"));
        ...
        
        {
            ...
            
            {
                ...
                read_len - (sizeof(network_naked_packet) - MAX_PACKET_SIZE);
...
//
//
int psnet_send(net_addr * who_to, void *data, int len, int flags,
                int reliable_socket) 
{
  ...
  int ret, send_len;
    ...
    
    {
                ...
                "Network ==> destination port %d invalid in psnet_send\n",
                port));
    ...
    
    {
      ...
       Send_network_checksum_packet.checksum =
        psnet_calc_checksum(data, len);
      memcpy(Send_network_checksum_packet.data, data, len);
      send_len =
        sizeof(Send_network_checksum_packet) - MAX_CHECKSUM_PACKET_SIZE + len;
    ...
    
    {
        ...
        sendto(send_sock, (char *) send_data, send_len, 0,
               (SOCKADDR *) & ipx_addr, sizeof(ipx_addr));
      if ((ret != SOCKET_ERROR) && (ret != send_len))
        nprintf(("Network",
                  "requested to send %d bytes -- sent %d instead!!!\n",
                  send_len, ret));
...
void free_packet(int id) 
{
            ...
            && (packet_buffers[Largest_packet_index].sequence_number == -1));
...
 int psnet_rel_send(PSNET_SOCKET psocket, ubyte * data, int length,
                      int flags) 
{
  ...
  do
    
    {
        ...
        send(socket, (char *) rsend_buffer, length + sizeof(s_length), 0);
...
int psnet_rel_get(PSNET_SOCKET psocket, ubyte * buffer, int max_len,
                  int flags) 
{
  ...
  do
    
    {
        ...
        recv(socket, (char *) (&rread_buffer[total_read]),
             read_len - total_read, 0);
  ...
  do
    
    {
        ...
        recv(socket, (char *) (buffer + total_read), read_len - total_read,
             0);
...
#else   /*  */
// psnet_send_reliable sends the given data through the given reliable socket.
int psnet_send_reliable(PSNET_SOCKET psocket, ubyte * data, int length,
                        int flags) 
{
...
 int psnet_get_reliable(PSNET_SOCKET psocket, ubyte * buffer, int max_len,
                          int flags)  
{
  ...
  int from_len, error;
    ...
    
    {
        ...
        
        {
                    ...
                    "Unable to make accepted socket non-blocking -- %d",
                    WSAGetLastError()));
...
int psnet_reliable_init() 
{
    ...
    nprintf(("Network",
             "PSNET RELIABLE SIZES : \n   OUT BUFFER SIZE : %d\n   OUT BUFFER COUNT %d\n   IN BUFFER SIZE %d\n   IN BUFFER COUNT %d\n",
             PSNET_RELIABLE_MAX_OUT_BUFFER_SIZE,
             PSNET_RELIABLE_NUM_OUT_BUFFERS,
             PSNET_RELIABLE_MAX_IN_BUFFER_SIZE,
             PSNET_RELIABLE_NUM_IN_BUFFERS));
    ...
    
    {
      ...
      Psnet_reliable_out[idx] =
        (reliable_packet_out *) vm_malloc(sizeof(reliable_packet_out));
    ...
    
    {
      ...
      Psnet_reliable_in[idx] =
        (reliable_packet_in *) vm_malloc(sizeof(reliable_packet_in));
...
int psnet_reliable_send(ubyte * data, int packet_size, net_addr * addr) 
{
    ...
    
    {
                ...
                "PSNET RELIABLE : could not find player for outgoing packet!\n"));
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                if (p->player_flags & (1 << s_idx)
                    && !(p->player_acks & (1 << s_idx)))
                
                {
                  
                    // if the timestamp has elapsed
                    if ((p->player_stamps[s_idx] != -1)
                        && timestamp_elapsed(p->player_stamps[s_idx]))
...
#ifdef PSNET_RELIABLE_VERBOSE
                            nprintf(("Network",
                                     "PSNET RELIABLE : packet failed to be delivered (%d retries) !!\n",
...
#ifdef PSNET_RELIABLE_VERBOSE
                            nprintf(("Network",
                                     "PSNET RELIABLE : resending packet\n"));
...
int psnet_reliable_should_process(net_addr * addr, ubyte * data,
                                  int packet_size) 
{
...
#endif  /*  */
            Psnet_reliable_out[packet_index]->player_acks |=
            (1 << player_index);
...
int psnet_reliable_addr_index(net_addr * addr) 
{
    ...
    {
      if ((Psnet_reliable_addr_flags & (1 << idx))
           && psnet_same(addr, &Psnet_reliable_addr[idx]))
...
void psnet_reliable_kill_outgoing(int index) 
{
    ...
    
    {
        ...
        
        {
            ...
            {
              memset(Psnet_reliable_out[idx], 0,
                      sizeof(reliable_packet_out));
  ...
  ushort psnet_reliable_get_unique_id(ushort id_num, net_addr * from) 
{
    ...
    
    {
            ...
            1)
           /*(Psnet_reliable_addr_flags & (1<<idx)) && (psnet_same(from,&Psnet_reliable_addr[idx])) */
           )
...
int psnet_reliable_send_packet(reliable_packet_out * out, int force_index) 
{
    ...
    {
      if (out->player_flags & (1 << force_index)
           && !(out->player_acks & (1 << force_index)))
        
        {
          bytes_sent =
            psnet_send(&Psnet_reliable_addr[force_index], data, packet_size,
                       PSNET_FLAG_RAW, 1);
          out->player_stamps[force_index] =
            timestamp(PSNET_RELIABLE_TIMEOUT);
    ...
    
    {
        ...
        
        {
            ...
            if (out->player_flags & (1 << idx)
                && (Psnet_reliable_addr_flags & (1 << idx))
                && !(out->player_acks & (1 << idx)))
            
            {
              bytes_sent =
                psnet_send(&Psnet_reliable_addr[idx], data, packet_size,
                           PSNET_FLAG_RAW, 1);
