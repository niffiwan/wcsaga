...
void send_player_kick_packet(int player_index, int ban = 1, int reason =
                             KICK_REASON_NORM);
...
void multi_kick_player(int player_index, int ban, int reason) 
{
    ...
    
    {
        ...
        
        {
            ...
            send_leave_game_packet(Net_players[player_index].player_id,
                                   reason);
          ...
          sprintf(str, XSTR("<kicking %s ...>", 1501),
...
void multi_dcf_kick() 
{
    ...
    
    {
           ...
           && (stricmp(Net_players[idx].m_player->callsign, Dc_arg) == 0))
...
void multi_kick_get_text(net_player * pl, int reason, char *str) 
{
    ...
    
    {
               ...
               XSTR("<%s was kicked because of mission file xfer failure>",
                    1003), pl->m_player->callsign);
      break;
    case KICK_REASON_CANT_XFER:
      sprintf(str,
               XSTR("<%s was kicked for not having builtin mission %s>",
                    1004), pl->m_player->callsign,
...
void multi_kick_add_ban(net_addr * addr) 
{
    ...
    {
      memcpy(&Multi_kick_ban_slots[Multi_kick_num_ban_slots++], addr,
              sizeof(net_addr));
