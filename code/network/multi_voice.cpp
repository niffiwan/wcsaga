...
int multi_voice_process_data(ubyte * data, int player_index, int msg_mode,
                             net_player * target);
...
void multi_voice_route_data(ubyte * data, int packet_size, int player_index,
                            int mode, net_player * target);
...
void multi_voice_alg_process_data(int player_index, int stream_index,
                                  ushort chunk_index, ushort chunk_size);
...
void multi_voice_close() 
{
    ...
    
    {
        ...
        {
          rtvoice_free_playback_buffer(Multi_voice_stream[idx].
                                        stream_rtvoice_handle);
...
void multi_voice_server_process() 
{
    ...
    
    {
        ...
        default:
            ...
            {
              Assert(Multi_voice_stream[idx].token_status !=
                      MULTI_VOICE_TOKEN_INDEX_FREE);
    ...
    
    {
           ...
           && timestamp_elapsed(Net_players[idx].s_info.
                                voice_token_timestamp))
...
void multi_voice_player_process() 
{
    ...
    else if (Multi_voice_keydown && !multi_voice_keydown()
             && Multi_voice_can_record)
...
int multi_voice_keydown() 
{
    ...
    
    {
               ...
               && !(keyd_pressed[KEY_LSHIFT]
                    || keyd_pressed[KEY_RSHIFT])) ? 1 : 0;
...
void multi_voice_take_token(int stream_index) 
{
    ...
    
    {
        ...
        multi_io_send_reliable(&Net_players
                               [Multi_voice_stream[stream_index].
                                token_status], data, packet_size);
       
        // timestamp this guy so that he can't get the token back immediately
        Net_players[Multi_voice_stream[stream_index].token_status].s_info.
        voice_token_timestamp = timestamp(Netgame.options.voice_token_wait);
...
void multi_voice_player_send_stream() 
{
    ...
    // get the data
    rtvoice_get_data((unsigned char **) &Multi_voice_record_buffer,
                     &sound_size, &uncompressed_size, &d_gain, NULL, NULL);
    ...
    
    {
        ...
        
        {
            ...
            multi_find_player_by_object(&Objects[Player_ai->target_objnum]);
    ...
    
    {
        ...
        {
          multi_voice_route_data(data, packet_size, MY_NET_PLAYER_NUM,
                                  (int) msg_mode,
                                  (target_index ==
                                   -1) ? NULL : &Net_players[target_index]);
                             ...
                             net_player * target) 
{
    ...
    
    {
        ...
        {
          memcpy(Multi_voice_stream[stream_index].accum_buffer[chunk_index],
        ...
        {
          multi_voice_alg_process_data(player_index, stream_index,
...
void multi_voice_flush_old_stream(int stream_index) 
{
    ...
    memset(Multi_voice_stream[stream_index].accum_buffer_flags, 0,
...
int multi_voice_get_stream(int stream_id) 
{
    ...
    
    {
            ...
            || ((cur_time - Multi_voice_stream[idx].stream_last_heard) >
                max_diff)) && (Multi_voice_stream[idx].token_stamp != -1))
...
void multi_voice_process_next_chunk() 
{
  ...
  memcpy(str->accum_buffer[Multi_voice_current_stream_index],
...
void multi_voice_process_packet(ubyte * data, header * hinfo) 
{
    ...
    case MV_CODE_DATA:
        ...
        multi_voice_process_data(data + offset, player_index, msg_mode,
                                 (target_index ==
                                  -1) ? NULL : &Net_players[target_index]);
        ...
        {
          multi_voice_route_data(data, offset, player_index, msg_mode,
                                  (target_index ==
                                   -1) ? NULL : &Net_players[target_index]);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                rtvoice_uncompress(st->accum_buffer[idx],
                                   (int) st->accum_buffer_csize[idx],
                                   st->accum_buffer_gain[idx],
                                   (ubyte *) Multi_voice_playback_buffer +
                                   buffer_offset,
        ...
        
        {
            ...
            {
              memcpy(Multi_voice_unpack_buffer + buffer_offset,
        ...
        rtvoice_uncompress((ubyte *) Multi_voice_unpack_buffer, buffer_offset,
                           st->accum_buffer_gain[0],
                           (ubyte *) Multi_voice_playback_buffer,
        ...
        buffer_offset =
        multi_voice_mix(MULTI_VOICE_POST_SOUND, Multi_voice_playback_buffer,
        ...
        rtvoice_stop_playback(Multi_voice_stream[stream_index].
                              stream_rtvoice_handle);
        ...
        
        {
            ...
            
            {
              ...
              sprintf(voice_msg, XSTR("<%s is speaking>", 712),
                       Net_players[player_index].m_player->callsign);
        ...
        rtvoice_play_uncompressed(Multi_voice_stream[stream_index].
                                  stream_rtvoice_handle,
                                  (unsigned char *)
                                  Multi_voice_playback_buffer, buffer_offset);
...
void multi_voice_test_process_next_chunk() 
{
    ...
    rtvoice_get_data(&outbuf, &compressed_size, &uncompressed_size, &gain,
                     &outbuf_raw, &raw_size);
    ...
    options_multi_set_voice_data(outbuf_raw, raw_size, outbuf,
                                 compressed_size, uncompressed_size, gain);
