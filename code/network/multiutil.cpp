  ...
  for (idx = 0; idx < MAX_PLAYERS; idx++)
    {
      if (MULTI_CONNECTED(Net_players[idx])
          ...
          && (Objects[Net_players[idx].m_player->objnum].instance < MAX_SHIPS)
          && !stricmp(ship_name,
                        ...
                        net_addr * addr, int ship_class, short id)
{
  ...
  stuff_netplayer_info(&Net_players[net_player_num], addr, player_ship_class,
                       pl);
  ...
  else
    {
      ...
      if ((Net_players[player_num].m_player->objnum >= 0)
          ...
          && (Objects[Net_players[player_num].m_player->objnum].type ==
              OBJ_SHIP)
          && (Objects[Net_players[player_num].m_player->objnum].instance >= 0)
...
int multi_is_valid_unknown_packet(ubyte type)
{
  return (type == GAME_QUERY) || (type == JOIN) || (type == PING)
    ...
    || (type == GAME_ACTIVE) || (type == INGAME_NAK) || (type == DENY)
  ...
  if (jr->flags & JOIN_FLAG_AS_OBSERVER)
    {
      ...
      send_accept_packet(net_player_num,
                         (Net_players[net_player_num].
                          flags & NETINFO_FLAG_INGAME_JOIN) ? ACCEPT_OBSERVER
                         | ACCEPT_INGAME : ACCEPT_OBSERVER);
    }
  else
    {
...
int multi_get_connection_speed()
{
  ...
  char *connection_speed =
    os_config_read_string(NULL, "ConnectionSpeed", "Fast");
