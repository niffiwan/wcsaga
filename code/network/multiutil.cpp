...
void multi_set_network_signature(ushort signature, int what_kind) 
{
    ...
    {
      Assert((signature >= ASTEROID_SIG_MIN)
    ...
    
    {
               ...
               NPERM_SIG_MIN) /*&& (signature <= NPERM_SIG_MAX) */ );
...
// state
void stuff_netplayer_info(net_player * nplayer, net_addr * addr,
                          int ship_class, player * pplayer) 
{
...
void multi_assign_player_ship(int net_player, object * objp, int ship_class) 
{
    ...
    // works properly.
    Net_players[net_player].p_info.p_objp =
...
int multi_create_player(int net_player_num, player * pl, char *name,
                         net_addr * addr, int ship_class, short id) 
{
    ...
    stuff_netplayer_info(&Net_players[net_player_num], addr,
                         player_ship_class, pl);
...
void delete_player(int player_num, int kicked_reason) 
{
    ...
    // NETLOG
    ml_printf(NOX("Deleting player %s"),
  ...
  Net_players[player_num].m_player->flags &= ~(PLAYER_FLAGS_STRUCTURE_IN_USE);  // free up his player structure
              ...
              int found_new_host = 0;
                ...
                
                {
                    ...
                    
                    {
                        ...
                        // send a packet
                        send_host_captain_change_packet(Net_players[idx].
                                                        player_id, 0);
    ...
    
    {
        ...
        
        {
          ...
          multi_kick_get_text(&Net_players[player_num],
                               Net_players[player_num].s_info.kick_reason,
                               str);
        ...
        {
          send_leave_game_packet(Net_players[player_num].player_id,
                                  kicked_reason);
    ...
    
    {
        ...
        {
          multi_make_player_ai(&Objects
                                [Net_players[player_num].m_player->objnum]);
    ...
    {
      sprintf(notify_string, XSTR("<%s has left>", 901),
               Net_players[player_num].m_player->callsign);
...
void multi_cull_zombies() 
{
    ...
    
    {
        ...
        {
          HUD_printf(XSTR("Dumping %s after prolonged inactivity", 902),
...
//
//
void fill_net_addr(net_addr * addr, ubyte * address, ubyte * net_id,
                    ushort port) 
{
...
 void multi_maybe_send_ship_status() 
{
    ...
    if ((Multi_ship_status_stamp < 0)
        || timestamp_elapsed_safe(Multi_ship_status_stamp,
                                  MULTI_SHIP_STATUS_TIME * 2))
    
    {
        ...
        
        {
            ...
            send_ship_status_packet(Net_player, &Multi_ship_status_bi,
...
void server_verify_filesig(short player_id, ushort sum_sig, int length_sig) 
{
    ...
    if ((sum_sig != Multi_current_file_checksum)
        || (length_sig != Multi_current_file_length))
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
              pl->s_info.xfer_handle =
                multi_xfer_send_file(pl->reliable_socket,
                                     Netgame.mission_name, CF_TYPE_MISSIONS);
    ...
    
    {
        ...
        
        {
            ...
            multi_xfer_send_file(pl->reliable_socket, Netgame.mission_name,
                                 CF_TYPE_MISSIONS);
...
 void multi_untag_player_ships() 
{
    ...
    
    {
        ...
        
        {
          ...
          obj_set_flags(&Objects[moveup->objnum],
                         Objects[moveup->objnum].flags | OF_COULD_BE_PLAYER);
...
 int multi_netplayer_state_check2(int state, int state2,
...
 int multi_netplayer_state_check3(int state, int state2, int state3,
                                    int ignore_standalone) 
{
...
void multi_maybe_send_repair_info(object * dest_objp, object * source_objp,
                                  int code) 
{
...
 int multi_is_valid_unknown_packet(ubyte type) 
{
    ...
    || (type == PONG) || (type == GAME_INFO)  ||(type == ACCEPT)
    || (type == GAME_ACTIVE) || (type == INGAME_NAK)
    || (type == DENY)  ||(type == UPDATE_DESCRIPT)
    || (type == ACCEPT_PLAYER_DATA) ? 1 : 0;
...
 active_game * multi_update_active_games(active_game * ag) 
{
    ...
    if (Active_game_head != NULL)       // no games on list at all
    {
      ...
      do
        ...
        {
          if (psnet_same(&gp->server_addr, &ag->server_addr)
               /*&& (gp->game.security == game->security) */ )
...
 int multi_eval_join_request(join_request * jr, net_addr * addr) 
{
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
              send_game_chat_packet(&Net_players[MY_NET_PLAYER_NUM],
                                     knock_message, MULTI_MSG_ALL, NULL, NULL,
                                     1);
            ...
            case NETGAME_STATE_IN_MISSION:
                ...
                
                {
                                         ...
                                         knock_message, MULTI_MSG_TARGET,
                                         Netgame.host, NULL, 1);
    ...
    
    {
        ...
        
        {
            ...
            if (std_is_host_passwd()
                && strcmp(jr->passwd, Multi_options_g.std_passwd))
    ...
    if ((Netgame.mode == NG_MODE_PASSWORD)
        && strcmp(Netgame.passwd, jr->passwd))
...
void multi_warpout_all_players() 
{
    ...
    if ((Net_player->flags & NETINFO_FLAG_OBSERVER)
        ...
        || ((Player_obj->type == OBJ_SHIP)
            && (Player_ship->flags & SF_CANNOT_WARP)))
...
void multi_file_xfer_notify(int handle) 
{
    ...
    
    {
        ...
        Net_player->p_info.options.
        flags & MLO_FLAG_XFER_MULTIDATA ? CF_TYPE_MULTI_CACHE :
        CF_TYPE_MISSIONS;
    ...
    if ((strlen(filename) > 0) && !cf_access(filename, cf_type, 00)
...
void multi_process_valid_join_request(join_request * jr, net_addr * who_from,
                                      int ingame_join_team) 
{
    ...
    
    {
        ...
        if (!multi_obs_create_player
            (net_player_num, jr->callsign, who_from, &Players[player_num]))
      ...
      player_set_squad_bitmap(Net_players[net_player_num].m_player,
                               jr->squad_filename);
        ...
        send_accept_packet(net_player_num,
                           (Net_players[net_player_num].
                            flags & NETINFO_FLAG_INGAME_JOIN) ?
    ...
    
    {
        ...
        if (!multi_create_player
            (net_player_num, &Players[player_num], jr->callsign, who_from, -1,
             id_num))
        ...
        
        {
          ...
           send_accept_packet(net_player_num, ACCEPT_INGAME,
                                ingame_join_team);
    ...
    // copy in his options
    memcpy(&Net_players[net_player_num].p_info.options, &jr->player_options,
...
void multi_player_ships_available(int *team_0, int *team_1) 
{
    ...
    
    {
        ...
        
        {
            ...
            mp_team_num =
            multi_ts_get_team(Ships[Objects[moveup->objnum].instance].
                              ship_name);
...
 void multi_spew_pxo_checksums(int max_files, char *outfile) 
{
    ...
    
    {
        ...
        cf_get_file_list(max_files, file_names, CF_TYPE_MISSIONS, wild_card);
        ...
        
        {
                    ...
                    cf_add_ext(file_names[idx], FS_MISSION_FILE_EXT));
            ...
            
            {
                     ...
                     checksum, gametype, The_mission.num_players,
                     description);
...
int multi_pack_unpack_vel(int write, ubyte * data, matrix * orient,
                          vec3d * pos, physics_info * pi) 
{
...
int multi_pack_unpack_desired_vel(int write, ubyte * data, matrix * orient,
                                  vec3d * pos, physics_info * pi,
                                  ship_info * sip) 
{
...
int multi_pack_unpack_rotvel(int write, ubyte * data, matrix * orient,
                             vec3d * pos, physics_info * pi) 
{
...
int multi_pack_unpack_desired_rotvel(int write, ubyte * data, matrix * orient,
                                     vec3d * pos, physics_info * pi,
                                     ship_info * sip) 
{
    ...
    
    {
        ...
        
        {
            ...
            fl2i(pi->desired_rotvel.xyz.x * 128.0f / sip->max_rotvel.xyz.x);
        ...
        
        {
            ...
            fl2i(pi->desired_rotvel.xyz.y * 128.0f / sip->max_rotvel.xyz.y);
        ...
        
        {
            ...
            fl2i(pi->desired_rotvel.xyz.z * 128.0f / sip->max_rotvel.xyz.z);
