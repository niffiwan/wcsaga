...
// process a header
void multi_xfer_process_header(ubyte * data, PSNET_SOCKET_RELIABLE who,
                               ushort sig, char *filename, int file_size,
...
void multi_xfer_do() 
{
    ...
    
    {
        ...
        if ((Multi_xfer_entry[idx].flags & MULTI_XFER_FLAG_USED)
            && !(Multi_xfer_entry[idx].
                 flags & (MULTI_XFER_FLAG_SUCCESS | MULTI_XFER_FLAG_FAIL |
...
int multi_xfer_send_file(PSNET_SOCKET_RELIABLE who, char *filename,
                         int cfile_flags, int flags) 
{
    ...
    // set the flags
    temp_entry.flags |=
    (MULTI_XFER_FLAG_USED | MULTI_XFER_FLAG_SEND | MULTI_XFER_FLAG_PENDING);
...
// lookup a file xfer entry by player
  xfer_entry * multi_xfer_find_entry(PSNET_SOCKET_RELIABLE who, ushort sig,
                                     int sender_side) 
{
                              ...
                              PSNET_SOCKET_RELIABLE who) 
{
    ...
    case MULTI_XFER_CODE_HEADER:
        ...
        multi_xfer_process_header(xfer_data, who, sig, filename, file_size,
                                  file_checksum);
...
#ifdef MULTI_XFER_VERBOSE
        nprintf(("Network",
                 "MULTI XFER : renaming xferred file from %s to %s (chksum %d %d)\n",
                 xe->ex_filename, xe->filename, (int) xe->file_chksum,
                 (int) chksum));
