...
void multi_oo_calc_interp_splines(int ship_index, vec3d * cur_pos,
                                   matrix * cur_orient,
                       ...
                       ubyte * data_out) 
{
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                (float) subsysp->current_hits / (float) subsysp->max_hits;
  ...
  if (MULTIPLAYER_MASTER && (sip->flags & SIF_SUPPORT)
    ...
    
    {
        ...
        
        {
            ...
            Objects[Ai_info[shipp->ai_index].support_ship_objnum].
            net_signature;
...
int multi_oo_unpack_client_data(net_player * pl, ubyte * data) 
{
    ...
    
    {
        ...
        
        {
            ...
            {
              Ai_info[Ships[Objects[pl->m_player->objnum].instance].
                       ai_index].targeted_subsys =
...
int multi_oo_unpack_data(net_player * pl, ubyte * data) 
{
    ...
    if ((pobjp == NULL) || (pobjp->type != OBJ_SHIP) || (pobjp->instance < 0)
        || (pobjp->instance >= MAX_SHIPS)
        || (Ships[pobjp->instance].ship_info_index < 0)
        || (Ships[pobjp->instance].ship_info_index >= Num_ship_classes))
    ...
    
    {
        ...
        {
          memmove(&oo_arrive_time[shipp - Ships][0],
        ...
        {
          oo_arrive_time[shipp -
                          Ships][oo_arrive_time_count[shipp - Ships]++] =
            f2fl(Missiontime);
      ...
      int r3 =
        multi_pack_unpack_vel(0, data + offset, &pobjp->orient, &new_pos,
                              &new_phys_info);
    ...
    
    {
      ...
      int r5 =
        multi_pack_unpack_rotvel(0, data + offset, &new_orient, &new_pos,
                                 &new_phys_info);
    ...
    
    {
        ...
        
        {
          ...
           multi_oo_calc_interp_splines(shipp - Ships, &pobjp->pos,
                                          &pobjp->orient, &pobjp->phys_info,
                                          &new_pos, &new_orient,
                                          &new_phys_info);
...
int multi_oo_maybe_update(net_player * pl, object * obj, ubyte * data) 
{
    ...
    if ((Ships[obj->instance].np_updates[player_index].status_update_stamp ==
         -1)
        || timestamp_elapsed_safe(Ships[obj->instance].
                                  np_updates[player_index].
                                  status_update_stamp, OO_MAX_TIMESTAMP))
    ...
    if ((Ships[obj->instance].np_updates[player_index].subsys_update_stamp ==
        ...
        || timestamp_elapsed_safe(Ships[obj->instance].
                                  np_updates[player_index].
                                  subsys_update_stamp, OO_MAX_TIMESTAMP))
    ...
    cur_pos_chksum =
    cf_add_chksum_short(cur_pos_chksum, (ubyte *) (&obj->pos), sizeof(vec3d));
  cur_orient_chksum =
    cf_add_chksum_short(cur_orient_chksum, (ubyte *) (&obj->orient),
                        sizeof(matrix));
    ...
    
    {
        ...
        {
          multi_rate_add(player_index, "skp_p",
                          OO_POS_RET_SIZE + OO_VEL_RET_SIZE);
       ...
       && (shipp->np_updates[player_index].orient_chksum ==
           cur_orient_chksum))
    
    {
        ...
        {
          multi_rate_add(player_index, "skp_o",
    ...
    
    {
        ...
        
        {
            ...
            if ((Net_players[idx].s_info.rate_stamp == -1)
                || timestamp_elapsed_safe(Net_players[idx].s_info.rate_stamp,
                                          OO_MAX_TIMESTAMP)
                || (abs(timestamp_ticker - Net_players[idx].s_info.rate_stamp)
...
void multi_oo_update_server_rate() 
{
    ...
    OO_client_rate =
    (int) (((float) OO_server_rate / (float) OO_gran) /
...
void multi_oo_interp(object * objp) 
{
  ...
  float t =
    oo_arrive_time_next[objp->instance] /
