...
int RECVFROM(uint s, char *buf, int len, int flags, sockaddr * from,
...
int SENDTO(uint s, char *buf, int len, int flags, sockaddr * to, int tolen,
           int psnet_type);
...
int psnet_rel_send(PSNET_SOCKET_RELIABLE socket, ubyte * data, int length,
                   int np_index = -1);
...
int psnet_rel_get(PSNET_SOCKET_RELIABLE socket, ubyte * buffer,
                  int max_length);
...
void psnet_rel_connect_to_server(PSNET_SOCKET_RELIABLE * s,
