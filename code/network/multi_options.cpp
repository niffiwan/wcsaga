...
void multi_options_read_config()
{
  ...
  Multi_options_g.port =
    (Cmdline_network_port >=
     0) ? (ushort) Cmdline_network_port : forced_port ==
              ...
              if (SETTING("+passwd"))
                {
                  ...
                  if (tok != NULL)
                    {
                      strncpy(Multi_options_g.std_passwd, tok,
                              STD_PASSWD_LEN);
#ifdef _WIN32
                      // yuck
                      extern HWND Multi_std_host_passwd;
                      SetWindowText(Multi_std_host_passwd,
                                    Multi_options_g.std_passwd);
#else
                      // TODO: get password ?
              ...
              if (SETTING("+pxo_login"))
                {
                  ...
                  if (tok != NULL)
                    {
                      strncpy(Multi_options_g.std_pxo_login, tok,
              ...
              if (SETTING("+pxo_password"))
                {
                  ...
                  if (tok != NULL)
                    {
                      strncpy(Multi_options_g.std_pxo_password, tok,
                              MULTI_TRACKER_STRING_LEN);
                    }
                }
            }
...
void multi_options_netgame_load(multi_server_options * options)
{
  if (options != NULL)
    {
      memcpy(options, &Player->m_server_options,
  ...
  switch (code)
    ...
    case MULTI_OPTION_LOCAL:
      if (player_index == -1)
        ...
        {
          get_local_options(data, &offset,
                            &Net_players[player_index].p_info.options);
