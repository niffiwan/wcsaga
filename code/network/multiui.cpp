...
#define MULTI_COMMON_MAX_TEXT                       (MULTI_COMMON_TEXT_MAX_LINES * MULTI_COMMON_TEXT_MAX_LINE_LENGTH)
  ...
  Multi_common_text[MULTI_COMMON_TEXT_MAX_LINES]
...
 void multi_common_split_text() 
{
    ...
    split_str(Multi_common_all_text,
              Multi_common_text_coords[gr_screen.res][2], n_chars, p_str,
...
 void multi_common_render_text() 
{
    ...
    
    {
        ...
        
        {
                 ...
                 (line_count * fh), Multi_common_text[i]);
...
void multi_common_notify_do() 
{
    ...
    
    {
        ...
        
        {
            ...
            {
              gr_string((gr_screen.max_w_unscaled - w) / 2, y,
                         Multi_common_notify_text);
  ...
  UI_XSTR Multi_join_text[GR_NUM_RESOLUTIONS][MULTI_JOIN_NUM_TEXT] = 
{
  
  {
    ...
    {
    "Help", 928, 479, 436, UI_XSTR_COLOR_GREEN, -1,
  ...
  "Ping", 1307, 555, 37, UI_XSTR_COLOR_GREEN, -1, NULL} }, 
  {
    ...
    {
    "Help", 928, 782, 699, UI_XSTR_COLOR_GREEN, -1,
...
 void multi_join_notify_new_game() 
{
    ...
    // reset the # of items
    Multi_join_slider.set_numberItems(Active_game_count >
                                      Mj_max_game_items[gr_screen.
                                                        res] ?
                                      Active_game_count -
...
 void multi_join_game_init() 
{
    ...
    Multi_join_window.create(0, 0, gr_screen.max_w_unscaled,
                             gr_screen.max_h_unscaled, 0);
  Multi_join_window.
    set_mask_bmap(Multi_join_bitmap_mask_fname[gr_screen.res]);
    ...
    
    {
        ...
        // create the object
        Multi_join_buttons[gr_screen.res][idx].button.
        ...
        Multi_join_buttons[gr_screen.res][idx].button.
        set_highlight_action(common_play_highlight_sound);
       
        // set the ani for the button
        Multi_join_buttons[gr_screen.res][idx].button.
        set_bmaps(Multi_join_buttons[gr_screen.res][idx].filename);
       
        // set the hotspot
        Multi_join_buttons[gr_screen.res][idx].button.
        link_hotspot(Multi_join_buttons[gr_screen.res][idx].hotspot);
    ...
    Multi_join_select_button.create(&Multi_join_window, "",
                                    Mj_list_area_coords[gr_screen.
                                                        res][MJ_X_COORD],
                                    Mj_list_area_coords[gr_screen.
                                                        res][MJ_Y_COORD],
                                    Mj_list_area_coords[gr_screen.
    ...
    Multi_join_slider.create(&Multi_join_window,
                             Mj_slider_coords[gr_screen.res][MJ_X_COORD],
...
}  void multi_join_game_do_frame() 
{
    ...
    
    {
        ...
        
        {
            ...
            popup_till_condition(multi_join_autojoin_do, XSTR("&Cancel", 779),
...
 void multi_join_button_pressed(int n) 
{
    ...
    
    {
      ...
      else if ((Multi_join_sent_stamp != -1)
               && !timestamp_elapsed(Multi_join_sent_stamp))
        
        {
          multi_common_add_notify(XSTR
...
void multi_join_display_games() 
{
    ...
    
    {
      ...
      do
        
        {
            ...
            con_type =
            (moveup->
             flags & AG_FLAG_CONNECTION_SPEED_MASK) >> AG_FLAG_CONNECTION_BIT;
          ...
          gr_string(Mj_speed_coords[gr_screen.res][MJ_X_COORD], y_start,
                     str);
            ...
            gr_force_fit_string(str, 200,
                                Mj_game_name_coords[gr_screen.
                                                    res][MJ_W_COORD]);
          gr_string(Mj_game_name_coords[gr_screen.res][MJ_X_COORD], y_start,
                     str);
            ...
            
            {
              ...
               gr_string(Mj_ping_coords[gr_screen.res][MJ_X_COORD], y_start,
                           str);
          ...
          gr_string(Mj_players_coords[gr_screen.res][MJ_X_COORD] +
    ...
    
    {
      ...
      gr_string(Mj_game_name_coords[gr_screen.res][MJ_X_COORD] - 30, y_start,
                 XSTR("<No game servers found>", 763));
...
 void multi_join_blit_game_status(active_game * game, int y) 
{
  ...
  gr_string(Mj_status_coords[gr_screen.res][MJ_X_COORD] +
             ((Mj_status_coords[gr_screen.res][MJ_W_COORD] - str_w) / 2), y,
             status_text);
...
void multi_join_do_netstuff() 
{
    ...
    if (Net_player->
        flags & (NETINFO_FLAG_ACCEPT_INGAME | NETINFO_FLAG_ACCEPT_CLIENT |
                 NETINFO_FLAG_ACCEPT_HOST | NETINFO_FLAG_ACCEPT_OBSERVER))
    
    {
      multi_common_add_notify(XSTR
                               ("Accepted.  Waiting for player data.", 770));
...
 void multi_join_process_select() 
{
    ...
    
    {
        ...
        send_netgame_descript_packet(&Multi_join_selected_item->server_addr,
                                     0);
    ...
    
    {
        ...
        
        {
            ...
            // send a mission description request to this guy
            send_netgame_descript_packet(&Multi_join_selected_item->
                                         server_addr, 0);
...
 void multi_join_send_join_request(int as_observer) 
{
    ...
    if ((Multi_join_selected_item->flags & AG_FLAG_TEAMS)
        && (Multi_join_selected_item->
            flags & (AG_FLAG_PAUSE | AG_FLAG_IN_MISSION)))
    ...
    // pxo squad info
    strncpy(Multi_join_request.pxo_squad_name, Multi_tracker_squad_name,
            LOGIN_LEN);
    ...
    memcpy(&Multi_join_request.player_options, &Player->m_local_options,
           sizeof(multi_local_options));
   
    // set the server address for the netgame
    memcpy(&Netgame.server_addr, &Multi_join_selected_item->server_addr,
           sizeof(net_addr));
   
    // send a join request to the guy
    send_join_packet(&Multi_join_selected_item->server_addr,
                     &Multi_join_request);
...
}  void multi_join_blit_top_stuff() 
{
    ...
    
    {
        ...
        // get bitmap width
      int cd_w;
      bm_get_info(Multi_common_icons[MICON_CD], &cd_w, NULL, NULL, NULL,
                   NULL);
       gr_set_bitmap(Multi_common_icons[MICON_CD]);
      gr_bitmap((gr_screen.max_w / 2) - (cd_w / 2),
                 Mj_cd_coords[gr_screen.res]);
...
int multi_join_warn_update_low(int code) 
{
    ...
    
    {
      ...
      return popup(0, 3, XSTR("&Cancel", 779), XSTR("&Continue", 780),
...
{
  return popup(PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON | PF_TITLE_BIG
                | PF_TITLE_RED, 2, XSTR("&Back", 995), XSTR("&Continue", 780),
                XSTR
...
 void multi_start_game_init() 
{
    ...
    // create the interface window
    Multi_sg_window.create(0, 0, gr_screen.max_w_unscaled,
                           gr_screen.max_h_unscaled, 0);
    ...
    
    {
        ...
        // set the sound to play when highlighted
        Multi_sg_buttons[gr_screen.res][idx].button.
        set_highlight_action(common_play_highlight_sound);
       
        // set the ani for the button
        Multi_sg_buttons[gr_screen.res][idx].button.
        set_bmaps(Multi_sg_buttons[gr_screen.res][idx].filename);
       
        // set the hotspot
        Multi_sg_buttons[gr_screen.res][idx].button.
        link_hotspot(Multi_sg_buttons[gr_screen.res][idx].hotspot);
    ...
    Multi_sg_rank_button.create(&Multi_sg_window, "",
                                Msg_rank_list_coords[gr_screen.
                                                     res][MSG_X_COORD],
                                Msg_rank_list_coords[gr_screen.
                                                     res][MSG_Y_COORD],
                                Msg_rank_list_coords[gr_screen.
    ...
    Multi_sg_game_name.create(&Multi_sg_window,
                              Msg_title_coords[gr_screen.res][MSG_X_COORD],
                              Msg_title_coords[gr_screen.res][MSG_Y_COORD],
                              Msg_title_coords[gr_screen.res][MSG_W_COORD],
                              MAX_GAMENAME_LEN, "",
                              UI_INPUTBOX_FLAG_ESC_CLR |
                              UI_INPUTBOX_FLAG_INVIS, -1, &Color_normal);
   
    // create the netgame password input box, and disable it by default
    Multi_sg_game_passwd.create(&Multi_sg_window,
                                Msg_passwd_coords[gr_screen.res][MSG_X_COORD],
                                Msg_passwd_coords[gr_screen.res][MSG_Y_COORD],
                                Msg_passwd_coords[gr_screen.res][MSG_W_COORD],
                                16, "",
                                UI_INPUTBOX_FLAG_ESC_CLR |
                                UI_INPUTBOX_FLAG_PASSWD |
                                UI_INPUTBOX_FLAG_INVIS, -1, &Color_normal);
...
void multi_sg_init_gamenet() 
{
    ...
    
    {
      ...
      ml_printf(NOX("Starting netgame as Host on Standalone server : %s"),
  ...
  if (strlen(buf) > MAX_GAMENAME_LEN)
    ...
    multi_voice_maybe_update_vars(Netgame.options.voice_qos,
                                  Netgame.options.voice_record_time);
...
 void multi_sg_draw_radio_buttons() 
{
    ...
    case NG_MODE_RANK_ABOVE:
      Multi_sg_buttons[gr_screen.res][MSG_RANK_SET_GAME].button.
        draw_forced(2);
...
 void multi_sg_rank_display_stuff() 
{
    ...
    
    {
      ...
      gr_string(Msg_rank_list_coords[gr_screen.res][MSG_X_COORD], y,
                 rank_name);
  ...
  multi_sg_rank_build_name(Ranks[Multi_sg_netgame->rank_base].name,
                            rank_name);
  gr_string(Msg_rank_sel_coords[gr_screen.res][MSG_X_COORD],
                              ...
                              const multi_create_info & m2) 
{
    ...
    
    {
      ...
      else if ((m2.flags & Multi_create_filter)
...
 void multi_create_list_sort(int mode) 
{
    ...
    {
      std::sort(Multi_create_mission_list.begin(),
    ...
    {
      std::sort(Multi_create_campaign_list.begin(),
...
 void multi_create_setup_list_data(int mode) 
{
    ...
    
    {
        ...
        
        {
            ...
            {
              if (Multi_create_campaign_list[idx].
    ...
    // reset the slider
    Multi_create_slider.set_numberItems(Multi_create_list_count >
                                        Multi_create_list_max_display
                                        [gr_screen.
                                         res] ? Multi_create_list_count -
                                        Multi_create_list_max_display
                                        [gr_screen.res] : 0);
...
 void multi_create_game_init() 
{
    ...
    // create the interface window
    Multi_create_window.create(0, 0, gr_screen.max_w_unscaled,
                               gr_screen.max_h_unscaled, 0);
  Multi_create_window.
    set_mask_bmap(Multi_create_bitmap_mask_fname[gr_screen.res]);
    ...
    
    {
        ...
        // create the object
        b->button.create(&Multi_create_window, "", b->x, b->y, 1, 1,
                         ((idx == MC_SCROLL_LIST_UP)
                          || (idx == MC_SCROLL_LIST_DOWN)), 1);
    ...
    Multi_create_sw_checkbox.create(&Multi_create_window, "",
                                    Multi_create_sw_checkbox_coords[gr_screen.
                                                                    res][0],
                                    Multi_create_sw_checkbox_coords[gr_screen.
                                                                    res][1],
                                    0);
  Multi_create_sw_checkbox.
    set_bmaps(Multi_create_sw_checkbox_fname[gr_screen.res], 6, 0);
  ...
   Multi_create_slider.create(&Multi_create_window,
                                Mc_slider_coords[gr_screen.res][MC_X_COORD],
                                Mc_slider_coords[gr_screen.res][MC_Y_COORD],
                                Mc_slider_coords[gr_screen.res][MC_W_COORD],
                                Mc_slider_coords[gr_screen.res][MC_H_COORD],
                                -1, Mc_slider_bitmap[gr_screen.res],
                                &multi_create_list_scroll_up,
                                &multi_create_list_scroll_down, NULL);
   
    // create the player list select button
    Multi_create_player_select_button.create(&Multi_create_window, "",
    ...
    Multi_create_list_select_button.create(&Multi_create_window, "",
                                           Mc_list_coords[gr_screen.
                                                          res][MC_X_COORD],
                                           Mc_list_coords[gr_screen.
                                                          res][MC_Y_COORD],
                                           Mc_list_coords[gr_screen.
    ...
    Multi_create_buttons[gr_screen.res][MC_ACCEPT].button.
    set_hotkey(KEY_CTRLED + KEY_ENTER);
...
 void multi_create_game_do() 
{
    ...
    
    {
        ...
        
        {
          ...
          gr_bitmap(Please_wait_coords[gr_screen.res][MC_X_COORD],
                     Please_wait_coords[gr_screen.res][MC_Y_COORD]);
           
            // draw "Loading" on it
            gr_set_color_fast(&Color_normal);
          gr_set_font(FONT2);
          gr_get_string_size(&str_w, &str_h, loading_str);
          gr_string((gr_screen.max_w - str_w) / 2,
                     (gr_screen.max_h - str_h) / 2, loading_str, false);
    ...
    gr_string(Mc_players_text[gr_screen.res][MC_X_COORD],
              Mc_players_text[gr_screen.res][MC_Y_COORD], XSTR("Players",
                                                               1269));
    ...
    {
      Multi_create_buttons[gr_screen.res][MC_MISSION_FILTER].button.
        draw_forced(2);
    ...
    {
      Multi_create_buttons[gr_screen.res][MC_CAMPAIGN_FILTER].button.
        draw_forced(2);
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
              gr_bitmap(Mc_players_coords[gr_screen.res][MC_X_COORD] +
                         total_offset, y_start - 1);
          ...
          gr_force_fit_string(str, CALLSIGN_LEN,
                               Mc_players_coords[gr_screen.res][MC_W_COORD] -
                               total_offset);
          gr_string(Mc_players_coords[gr_screen.res][MC_X_COORD] +
                     total_offset, y_start, str);
        ...
        
        {
            ...
            
            {
                ...
                
                {
                    ...
                    Multi_create_buttons[gr_screen.res][MC_TEAM0].button.
                    draw_forced(2);
            ...
            
            {
                ...
                
                {
                    ...
                    Multi_common_icon_dims[MICON_TEAM0][0] + 1;
        ...
        
        {
            ...
            
            {
                ...
                
                {
                    ...
                    // be sure to blit the correct team button
                    Multi_create_buttons[gr_screen.res][MC_TEAM1].button.
                    draw_forced(2);
            ...
            
            {
                ...
                
                {
                    ...
                    Multi_common_icon_dims[MICON_TEAM1][0] + 1;
...
void multi_create_list_load_missions() 
{
    ...
    set_numberItems(int (Multi_create_mission_list.size()) >
                    Multi_create_list_max_display[gr_screen.
                                                  res] ?
                    int (Multi_create_mission_list.size()) -
                    Multi_create_list_max_display[gr_screen.res] : 0);
...
 void multi_create_list_load_campaigns() 
{
    ...
    
    {
      ...
      if (flags != CAMPAIGN_TYPE_SINGLE
           && mission_campaign_get_info(filename, title, &campaign_type,
                                        &max_players))
...
 void multi_create_list_do() 
{
  ...
  int count = 0;
    ...
    (Multi_create_list_mode ==
     MULTI_CREATE_SHOW_MISSIONS) ? Multi_create_mission_list.
    size() : Multi_create_campaign_list.size();
    ...
    
    {
        ...
        multi_create_select_to_filename(Multi_create_list_select,
                                        selected_name);
...
void multi_create_list_select_item(int n) 
{
    ...
    
    {
      ...
      int old_type = Netgame.type_flags;
        ...
        {
          multi_options_update_mission(ng,
                                        Multi_create_list_mode ==
                                        MULTI_CREATE_SHOW_CAMPAIGNS ? 1 : 0);
...
 void multi_create_list_blit_icons(int list_index, int y_start) 
{
    ...
    max_index =
    (Multi_create_list_mode ==
     MULTI_CREATE_SHOW_MISSIONS) ? Multi_create_mission_list.size() -
    ...
    
    {
        ...
        
        {
                     ...
                     y_start +
                     Mc_icon_type_coords[gr_screen.res][MC_Y_COORD]);
    ...
    
    {
        ...
        
        {
                     ...
                     y_start +
                     Mc_icon_valid_coords[gr_screen.res][MC_Y_COORD]);
    ...
    
    {
        ...
        
        {
                     ...
                     y_start +
                     Mc_icon_volition_coords[gr_screen.res][MC_Y_COORD]);
...
 void multi_create_accept_hit() 
{
    ...
    
    {
             ...
             XSTR("Please wait until all clients have finished joining",
                  788));
    ...
    
    {
        ...
        
        {
            ...
            multi_create_select_to_filename(Multi_create_list_select,
                                            selected_name);
          strncpy(Game_current_mission_filename, selected_name,
                   MAX_FILENAME_LEN);
          strncpy(Netgame.mission_name, selected_name, MAX_FILENAME_LEN);
           
            // NETLOG
            ml_printf(NOX("Starting single mission %s, with %d players"),
        ...
        
        {
            ...
            // NETLOG
            ml_printf(NOX("Starting campaign %s, with %d players"),
                      selected_name, multi_num_players());
    ...
    
    {
        ...
        
        {
            ...
            popup(0, 3, POPUP_CANCEL, POPUP_YES, XSTR(" &No", 506),
...
 void multi_create_draw_filter_buttons() 
{
    ...
    {
      Multi_create_buttons[gr_screen.res][MC_SHOW_ALL +
                                           1].button.draw_forced(2);
...
 int multi_create_ok_to_commit() 
{
    ...
    
    {
        ...
        sprintf(err_string,
                ...
                ("There are too many observers in the game\n\nMax : %d\nCurrently %d\n\nPlease dump a few",
                 791), Netgame.options.max_observers, observer_count);
    ...
    
    {
        ...
        sprintf(err_string,
                ...
                ("There are too many players in the game\n\nMax : %d\nCurrently %d\n\nPlease dump a few",
                 792), Netgame.max_players, player_count);
    ...
    
    {
        ...
        
        {
                 ...
                 XSTR("Teams and/or team captains are not assigned properly",
                      793));
    ...
    
    {
        ...
        popup(PF_BODY_BIG | PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
              "You need 1 CD for every player!");
      
#else   /*  */
#ifdef DVD_MESSAGE_HACK
        popup(PF_BODY_BIG | PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
              XSTR("You need 1 DVD for every 4 players!", 794));
        ...
        
        {
            ...
            
            {
                   ...
                   (PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON |
                    PF_TITLE_RED | PF_TITLE_BIG, 2, XSTR("&Back", 995),
                    XSTR("&Continue", 780),
                    XSTR
...
struct ho_sliders 
{
   ...
    ho_sliders(char *name, int x1, int y1, int xt1, int yt1, int h,
                int _dot_w, int _dots):filename(name), x(x1), y(y1), xt(xt1),
    yt(yt1), hotspot(h), dot_w(_dot_w), dots(_dots)
  {
} };
...
 void multi_host_options_init() 
{
    ...
    // create the interface window
    Multi_ho_window.create(0, 0, gr_screen.max_w_unscaled,
                           gr_screen.max_h_unscaled, 0);
    ...
    
    {
        ...
        // set the sound to play when highlighted
        Multi_ho_buttons[gr_screen.res][idx].button.
        set_highlight_action(common_play_highlight_sound);
       
        // set the ani for the button
        Multi_ho_buttons[gr_screen.res][idx].button.
        set_bmaps(Multi_ho_buttons[gr_screen.res][idx].filename);
       
        // set the hotspot, ignoring the skill level button
        Multi_ho_buttons[gr_screen.res][idx].button.
        link_hotspot(Multi_ho_buttons[gr_screen.res][idx].hotspot);
    ...
    
    {
        ...
        // create the object
        Multi_ho_sliders[gr_screen.res][idx].slider.create(&Multi_ho_window,
                                                           Multi_ho_sliders
                                                           [gr_screen.
                                                            res][idx].x,
                                                           Multi_ho_sliders
    ...
    Multi_ho_respawns.create(&Multi_ho_window,
                             Ho_rsp_coords[gr_screen.res][MULTI_HO_X_COORD],
                             Ho_rsp_coords[gr_screen.res][MULTI_HO_Y_COORD],
                             Ho_rsp_coords[gr_screen.res][MULTI_HO_W_COORD],
                             6, "",
                             UI_INPUTBOX_FLAG_ESC_FOC | UI_INPUTBOX_FLAG_INVIS
                             | UI_INPUTBOX_FLAG_NO_LETTERS, -1,
                             &Color_bright);
    ...
    Multi_ho_time_limit.create(&Multi_ho_window,
                               Ho_time_coords[gr_screen.
                                              res][MULTI_HO_X_COORD],
                               Ho_time_coords[gr_screen.
                                              res][MULTI_HO_Y_COORD],
                               Ho_time_coords[gr_screen.
                                              res][MULTI_HO_W_COORD], 3, "",
                               UI_INPUTBOX_FLAG_ESC_FOC |
    ...
    Multi_ho_voice_wait.create(&Multi_ho_window,
                               Ho_vw_coords[gr_screen.res][MULTI_HO_X_COORD],
                               Ho_vw_coords[gr_screen.res][MULTI_HO_Y_COORD],
                               Ho_vw_coords[gr_screen.res][MULTI_HO_W_COORD],
                               1, "",
                               UI_INPUTBOX_FLAG_ESC_FOC |
                               UI_INPUTBOX_FLAG_INVIS |
                               UI_INPUTBOX_FLAG_NO_LETTERS, -1,
    ...
    Multi_ho_kill_limit.create(&Multi_ho_window,
                               Ho_kill_coords[gr_screen.
                                              res][MULTI_HO_X_COORD],
                               Ho_kill_coords[gr_screen.
                                              res][MULTI_HO_Y_COORD],
                               Ho_kill_coords[gr_screen.
                                              res][MULTI_HO_W_COORD], 4, "",
                               UI_INPUTBOX_FLAG_ESC_FOC |
                               UI_INPUTBOX_FLAG_INVIS |
                               UI_INPUTBOX_FLAG_NO_LETTERS, -1,
                               &Color_bright);
    ...
    Multi_ho_obs.create(&Multi_ho_window,
                        Ho_obs_coords[gr_screen.res][MULTI_HO_X_COORD],
                        Ho_obs_coords[gr_screen.res][MULTI_HO_Y_COORD],
                        Ho_obs_coords[gr_screen.res][MULTI_HO_W_COORD], 1, "",
                        UI_INPUTBOX_FLAG_ESC_FOC | UI_INPUTBOX_FLAG_INVIS |
                        UI_INPUTBOX_FLAG_NO_LETTERS, -1, &Color_bright);
...
 void multi_ho_update_sliders() 
{
    ...
    if (Netgame.options.voice_qos !=
        (ubyte) (Multi_ho_sliders[gr_screen.res][MULTI_HO_SLIDER_VOICE_QOS].
                 slider.pos + 1))
    
    {
      Netgame.options.voice_qos =
        (ubyte) (Multi_ho_sliders[gr_screen.res][MULTI_HO_SLIDER_VOICE_QOS].
                 slider.pos + 1);
    ...
    if (Netgame.options.voice_record_time !=
        (int) (0.5f *
               (float) (Multi_ho_sliders[gr_screen.res]
                        [MULTI_HO_SLIDER_VOICE_DUR].slider.pos +
                        1) * 1000.0f))
...
 }  void multi_host_options_do() 
{
    ...
    {
      Multi_ho_buttons[gr_screen.res][MULTI_HO_HOST_MODIFIES].button.
        draw_forced(2);
...
 void multi_ho_button_pressed(int n) 
{
    ...
    Multi_ho_buttons[gr_screen.res][n].button.get_mouse_pos(&x_pixel,
                                                            &y_pixel);
...
 void multi_ho_accept_hit() 
{
    ...
    (int) (0.5f *
           (float) (Multi_ho_sliders[gr_screen.res]
                    [MULTI_HO_SLIDER_VOICE_DUR].slider.pos + 1) * 1000.0f);
    ...
    memcpy(&Player->m_local_options, &Net_player->p_info.options,
           sizeof(multi_local_options));
  memcpy(&Player->m_server_options, &Netgame.options,
...
 void multi_ho_get_options() 
{
    ...
    // get the voice qos options
    Assert((Netgame.options.voice_qos >= 1)
           && (Netgame.options.voice_qos <= 10));
  Multi_ho_sliders[gr_screen.res][MULTI_HO_SLIDER_VOICE_QOS].slider.pos =
    (Netgame.options.voice_qos - 1);
   
    // get the voice duration options
    Assert((Netgame.options.voice_record_time > 0)
...
void multi_ho_display_record_time() 
{
    ...
    half_seconds =
    ((((Multi_ho_sliders[gr_screen.res][MULTI_HO_SLIDER_VOICE_DUR].slider.
        pos + 1) * 500) % 1000) / 500) * 5;
   
    // format the string
    sprintf(time_str, "%d.%d", full_seconds, half_seconds);
  gr_set_color_fast(&Color_bright);
  gr_string(Ho_vd_coords[gr_screen.res][MULTI_HO_X_COORD],
    ...
    
    {
        ...
        
        {
                   ...
                   ("Warning\nRespawn count in greater than mission specified max (%d)",
                    796), Multi_ho_mission_respawn);
          popup(PF_USE_AFFIRMATIVE_ICON | PF_TITLE_RED | PF_TITLE_BIG, 1,
                 POPUP_OK, val_txt);
...
 void multi_ho_check_focus() 
{
    ...
    
    {
        ...
        if (Multi_ho_respawns.has_focus() || Multi_ho_time_limit.has_focus()
...
 void multi_ho_blit_max_respawns() 
{
  ...
  gr_string(Ho_max_rsp_coords[gr_screen.res][MULTI_HO_X_COORD],
...
 void multi_ho_display_skill_level() 
{
  ...
  gr_string(Ho_st_coords[gr_screen.res][0], Ho_st_coords[gr_screen.res][1],
             Skill_level_names(skill_level, 1));
...
 void multi_game_client_setup_init() 
{
    ...
    // create the interface window
    Multi_jw_window.create(0, 0, gr_screen.max_w_unscaled,
                           gr_screen.max_h_unscaled, 0);
    ...
    chatbox_close();
  chatbox_create(CHATBOX_FLAG_BIG | CHATBOX_FLAG_DRAW_BOX |
    ...
    
    {
        ...
        // set the sound to play when highlighted
        Multi_jw_buttons[gr_screen.res][idx].button.
        set_highlight_action(common_play_highlight_sound);
       
        // set the ani for the button
        Multi_jw_buttons[gr_screen.res][idx].button.
        set_bmaps(Multi_jw_buttons[gr_screen.res][idx].filename);
       
        // set the hotspot
        Multi_jw_buttons[gr_screen.res][idx].button.
        link_hotspot(Multi_jw_buttons[gr_screen.res][idx].hotspot);
    }
   
    // if this is a PXO game, enable the squadwar checkbox
    Multi_jw_sw_checkbox.create(&Multi_jw_window, "",
                                Multi_jw_sw_checkbox_coords[gr_screen.res][0],
                                Multi_jw_sw_checkbox_coords[gr_screen.res][1],
                                0);
  Multi_jw_sw_checkbox.set_bmaps(Multi_jw_sw_checkbox_fname[gr_screen.res],
                                  6, 0);
    ...
    Multi_jw_plist_select_button.create(&Multi_jw_window, "",
                                        Mjw_players_coords[gr_screen.
                                                           res][MJW_X_COORD],
                                        Mjw_players_coords[gr_screen.
                                                           res][MJW_Y_COORD],
                                        Mjw_players_coords[gr_screen.
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
              gr_bitmap(Mjw_players_coords[gr_screen.res][MJW_X_COORD] +
                         total_offset, y_start - 1);
            ...
            
            {
                               ...
                               Mjw_players_coords[gr_screen.
                                                  res][MJW_W_COORD] -
                               total_offset);
          gr_string(Mjw_players_coords[gr_screen.res][MJW_X_COORD] +
                     total_offset, y_start, str);
...
 void multi_sync_common_init() 
{
    ...
    // create the interface window
    Multi_sync_window.create(0, 0, gr_screen.max_w_unscaled,
                             gr_screen.max_h_unscaled, 0);
  Multi_sync_window.
    set_mask_bmap(Multi_sync_bitmap_mask_fname[gr_screen.res]);
    ...
    
    {
        ...
        // create the object
        Multi_sync_buttons[gr_screen.res][idx].button.
        create(&Multi_sync_window, "",
        ...
        
        {
            ...
            set_bmaps(Multi_sync_buttons[gr_screen.res][idx].filename, 3, 0);
        ...
        
        {
            ...
            set_bmaps(Multi_sync_buttons[gr_screen.res][idx].filename);
        }
       
        // set the hotspot
        Multi_sync_buttons[gr_screen.res][idx].button.
        link_hotspot(Multi_sync_buttons[gr_screen.res][idx].hotspot);
    ...
    Multi_sync_plist_button.create(&Multi_sync_window, "",
                                   Ms_status_coords[gr_screen.
                                                    res][MS_X_COORD],
                                   Ms_status_coords[gr_screen.
                                                    res][MS_Y_COORD],
                                   Ms_status_coords[gr_screen.
...
 void multi_sync_blit_screen_all() 
{
    ...
    
    {
        ...
        
        {
            ...
            multi_sync_display_name(Net_players[idx].m_player->callsign,
                                    count, idx);
            ...
            
            {
              ...
              multi_sync_display_status(XSTR("Ingame Ship Select", 803),
                                        count);
              ...
              multi_sync_display_status(XSTR("Ready To Enter Mission", 815),
                                         count);
              break;
            case NETPLAYER_STATE_INGAME_SHIPS:
              multi_sync_display_status(XSTR("Ingame Ships Packet Ack", 816),
                                         count);
              break;
            case NETPLAYER_STATE_INGAME_WINGS:
              multi_sync_display_status(XSTR("Ingame Wings Packet Ack", 817),
                                         count);
              break;
            case NETPLAYER_STATE_INGAME_RPTS:
              multi_sync_display_status(XSTR
                                         ("Ingame Respawn Points Ack", 818),
                                         count);
              break;
            case NETPLAYER_STATE_SLOTS_ACK:
              multi_sync_display_status(XSTR("Ingame Weapon Slots Ack", 819),
                                         count);
              ...
              multi_sync_display_status(XSTR("Weapon Slots Ack", 823),
                                         count);
                ...
                
                {
                    ...
                    
                    {
                        ...
                        multi_xfer_pct_complete(Net_players[idx].s_info.
                                                xfer_handle);
                        ...
                        {
                          sprintf(txt, XSTR("Mission file xfer %d%%", 828),
                ...
                
                {
                    ...
                    
                    {
                        ...
                        multi_xfer_pct_complete(Net_player->s_info.
                                                xfer_handle);
    ...
    if ((gameseq_get_state() == GS_STATE_MULTI_MISSION_SYNC
        ...
        || (Multi_sync_countdown != -1))
                         ...
                         Multi_sync_countdown_coords[gr_screen.
                                                     res][MS_X_COORD],
                         Multi_sync_countdown_coords[gr_screen.
                                                     res][MS_Y_COORD]);
            ...
            
            {
                ...
                
                {
                    ...
                    case MULTI_XFER_SUCCESS:
                        ...
                        multi_xfer_release_handle(Net_players[idx].s_info.
                                                  xfer_handle);
    ...
    
    {
        ...
        
        {
            ...
            case MULTI_XFER_TIMEDOUT:
                ...
                multi_quit_game(PROMPT_NONE, MULTI_END_NOTIFY_NONE,
...
 void multi_sync_post_init() 
{
    ...
    {
      generic_anim_init(&Multi_sync_countdown_anim,
        ...
        
        {
            ...
            
            {
                ...
                
                {
                    ...
                    {
                      multi_server_update_player_weapons(&Net_players[idx],
                                                          &Ships[Objects
                                                                 [Net_players
                                                                  [idx].
                                                                  m_player->
...
 void multi_sync_display_name(char *name, int index, int np_index) 
{
    ...
    {
      gr_force_fit_string(fit, CALLSIGN_LEN,
        ...
        // blit the string
        gr_string(Ms_status_coords[gr_screen.res][0] +
                  Ms_cd_icon_offset[gr_screen.res] +
                  Ms_team_icon_offset[gr_screen.res],
                  Ms_status_coords[gr_screen.res][MS_Y_COORD] + (index * 10),
                  fit);
        ...
        
        {
            ...
            
            {
                ...
                
                {
                  ...
                  gr_bitmap(Ms_status_coords[gr_screen.res][MS_X_COORD] +
                             Ms_cd_icon_offset[gr_screen.res],
    ...
    
    {
        ...
        // blit the string
        gr_string(Ms_status_coords[gr_screen.res][MS_X_COORD] +
                  Ms_cd_icon_offset[gr_screen.res],
                  Ms_status_coords[gr_screen.res][MS_Y_COORD] + (index * 10),
                  fit);
...
 void multi_sync_display_status(char *status, int index) 
{
    ...
    // make sure the string actually fits
    strcpy_s(fit, status);
  gr_force_fit_string(fit, 250,
                       Ms_status2_coords[gr_screen.res][MS_W_COORD] - 20);
  gr_set_color_fast(&Color_bright);
  gr_string(Ms_status2_coords[gr_screen.res][MS_X_COORD],
             Ms_status2_coords[gr_screen.res][MS_Y_COORD] + (index * 10),
             fit);
...
 void multi_sync_create_launch_button() 
{
    ...
    
    {
        ...
        // create the object
        Multi_sync_buttons[gr_screen.res][MS_LAUNCH].button.
        create(&Multi_sync_window, "",
        ...
        // set the ani for the button
        Multi_sync_buttons[gr_screen.res][MS_LAUNCH].button.
        set_bmaps(Multi_sync_buttons[gr_screen.res][MS_LAUNCH].filename, 3,
                  0);
       
        // set the hotspot
        Multi_sync_buttons[gr_screen.res][MS_LAUNCH].button.
        link_hotspot(Multi_sync_buttons[gr_screen.res][MS_LAUNCH].hotspot);
        ...
        Multi_sync_window.
        add_XSTR(&Multi_sync_text[gr_screen.res][MST_LAUNCH]);
...
 void multi_debrief_do_frame() 
{
    ...
    if ((Net_player->flags & NETINFO_FLAG_AM_MASTER)
        ...
        && multi_netplayer_state_check3(NETPLAYER_STATE_DEBRIEF,
...
void multi_debrief_accept_hit() 
{
    ...
    
    {
        ...
        popup(PF_TITLE | PF_BODY_BIG | PF_USE_AFFIRMATIVE_ICON |
              PF_USE_NEGATIVE_ICON | PF_IGNORE_ESC, 3, XSTR("&Cancel", 779),
              XSTR("&Accept", 844), XSTR("&Toss", 845),
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                popup(PF_TITLE | PF_BODY_BIG | PF_USE_AFFIRMATIVE_ICON |
                      PF_USE_NEGATIVE_ICON | PF_IGNORE_ESC, 3, XSTR("&Cancel",
                                                                    779),
                      XSTR("&Accept", 844), XSTR("&Toss", 845),
                      XSTR
                      ("(Continue Netgame)\nDo you wish to accept these stats?",
...
void multi_debrief_esc_hit() 
{
    ...
    
    {
        ...
        
        {
            ...
            popup(PF_BODY_BIG | PF_USE_AFFIRMATIVE_ICON |
                  PF_USE_NEGATIVE_ICON, 2, XSTR("&Cancel", 779),
                  XSTR("&Leave", 848),
                  XSTR
...
void multi_passwd_init() 
{
    ...
    // create the interface window
    Multi_pwd_window.create(0, 0, gr_screen.max_w_unscaled,
                            gr_screen.max_h_unscaled, 0);
    ...
    
    {
        ...
        // set the sound to play when highlighted
        Multi_pwd_buttons[gr_screen.res][idx].button.
        set_highlight_action(common_play_highlight_sound);
       
        // set the ani for the button
        Multi_pwd_buttons[gr_screen.res][idx].button.
        set_bmaps(Multi_pwd_buttons[gr_screen.res][idx].filename);
       
        // set the hotspot
        Multi_pwd_buttons[gr_screen.res][idx].button.
        link_hotspot(Multi_pwd_buttons[gr_screen.res][idx].hotspot);
    ...
    Multi_pwd_passwd.create(&Multi_pwd_window,
                            Mpwd_coords[gr_screen.res][MPWD_X_COORD],
                            Mpwd_coords[gr_screen.res][MPWD_Y_COORD],
                            Mpwd_coords[gr_screen.res][MPWD_W_COORD],
                            MAX_PASSWD_LEN, "",
                            UI_INPUTBOX_FLAG_ESC_CLR | UI_INPUTBOX_FLAG_INVIS,
                            -1, &Color_normal);
  Multi_pwd_passwd.set_focus();
   
    // link the enter key to ACCEPT
    Multi_pwd_buttons[gr_screen.res][MPWD_COMMIT].button.
    set_hotkey(KEY_ENTER);
