...
void multi_common_render_text()
{
  ...
  if ((Multi_common_num_text_lines - Multi_common_top_text_line) >
      Multi_common_text_max_display[gr_screen.res])
    {
      ...
      gr_string(Multi_common_text_coords[gr_screen.res][0],
                (Multi_common_text_coords[gr_screen.res][1] +
                 Multi_common_text_coords[gr_screen.res][3]) - 5, XSTR("more",
                                                                       755));
    }
}
      ...
      do
        {
          ...
          gr_force_fit_string(str, 200,
                              Mj_game_name_coords[gr_screen.res][MJ_W_COORD]);
...
void multi_create_game_init()
{
  ...
  Multi_create_slider.create(&Multi_create_window,
                             ...
                             Mc_slider_bitmap[gr_screen.res],
...
void multi_create_list_load_missions()
{
  ...
  Multi_create_slider.set_numberItems(int (Multi_create_mission_list.size()) >
                                      Multi_create_list_max_display[gr_screen.
                                                                    res] ?
...
int Multi_ho_radio_info[MULTI_HO_NUM_RADIO_BUTTONS][3] = {
  ...
  // { group #, value, button id# }
  {
   0,
   0,
   0},                          // highest ranking players can do messaging
  {
   0,
   1,
   1},                          // wing/team leaders can do messaging
  {
   0,
   2,
   2},                          // any player can do messaging
  {
   0,
   3,
   3},                          // only host can do messaging
  {
   1,
   0,
   4},                          // highest rank and host can end the mission
  {
   1,
   1,
   5},                          // team/wing leader can end the mission
  {
   1,
   2,
   6},                          // any player can end the mission
  {
   1,
   3,
   7},                          // only the host can end the mission
  {
   2,
   0,
   8},                          // voice toggled on
...
void multi_host_options_init()
{
  ...
  Multi_ho_time_limit.create(&Multi_ho_window,
                             Ho_time_coords[gr_screen.res][MULTI_HO_X_COORD],
                             Ho_time_coords[gr_screen.res][MULTI_HO_Y_COORD],
                             Ho_time_coords[gr_screen.res][MULTI_HO_W_COORD],
                             3, "",
                             UI_INPUTBOX_FLAG_ESC_FOC | UI_INPUTBOX_FLAG_INVIS
  ...
  Multi_ho_kill_limit.create(&Multi_ho_window,
                             Ho_kill_coords[gr_screen.res][MULTI_HO_X_COORD],
                             Ho_kill_coords[gr_screen.res][MULTI_HO_Y_COORD],
                             Ho_kill_coords[gr_screen.res][MULTI_HO_W_COORD],
                             4, "",
                             UI_INPUTBOX_FLAG_ESC_FOC | UI_INPUTBOX_FLAG_INVIS
                             | UI_INPUTBOX_FLAG_NO_LETTERS, -1,
                             &Color_bright);
  ...
  for (idx = 0; idx < MULTI_HO_NUM_RADIO_BUTTONS; idx++)
    {
      ...
      if (Multi_ho_radio_info[idx][1] ==
        ...
        {
          Multi_ho_buttons[gr_screen.res][Multi_ho_radio_info[idx][2]].button.
            draw_forced(2);
        }
    }
}
...
void multi_game_client_setup_init()
{
  ...
  Multi_jw_sw_checkbox.create(&Multi_jw_window, "",
                              ...
                              0);
  Multi_jw_sw_checkbox.set_bmaps(Multi_jw_sw_checkbox_fname[gr_screen.res], 6,
                                 0);
...
void multi_sync_common_init()
{
  ...
  Multi_sync_plist_button.create(&Multi_sync_window, "",
                                 ...
                                 Ms_status_coords[gr_screen.res][MS_H_COORD],
                                 0, 1);
  ...
  for (idx = 0; idx < MAX_PLAYERS; idx++)
    {
      if (MULTI_CONNECTED(Net_players[idx])
          && !MULTI_STANDALONE(Net_players[idx]))
        {
          ...
          multi_sync_display_name(Net_players[idx].m_player->callsign, count,
                                  idx);
          ...
          switch (state)
            ...
            case NETPLAYER_STATE_POST_DATA_ACK:
              multi_sync_display_status(XSTR
                                        ("Post Briefing Data Block Ack", 820),
                                        count);
              break;
            ...
            case NETPLAYER_STATE_CPOOL_ACK:
              multi_sync_display_status(XSTR("Campaign ship/weapon ack", 827),
                                        count);
              break;
...
void multi_sync_display_status(char *status, int index)
{
  ...
  gr_string(Ms_status2_coords[gr_screen.res][MS_X_COORD],
            Ms_status2_coords[gr_screen.res][MS_Y_COORD] + (index * 10), fit);
}
  ...
  if (MULTI_IS_TRACKER_GAME)
    {
      int res =
        popup(PF_TITLE | PF_BODY_BIG | PF_USE_AFFIRMATIVE_ICON |
              PF_USE_NEGATIVE_ICON | PF_IGNORE_ESC, 3,
...
void multi_debrief_server_process()
{
  ...
  if (multi_netplayer_state_check3
      ...
      && !Multi_debrief_reported_tvt && (Multi_debrief_server_framecount > 4))
    {
      multi_team_report();
