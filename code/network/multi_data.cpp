...
void multi_data_send_my_junk() 
{
    ...
    
    {
        ...
        {
          multi_data_add_new(Net_player->m_player->image_filename,
                              MY_NET_PLAYER_NUM);
        ...
        
        {
            ...
            multi_xfer_send_file(Net_player->reliable_socket,
                                 Net_player->m_player->image_filename,
                                 CF_TYPE_ANY,
                                 MULTI_XFER_FLAG_AUTODESTROY |
                                 MULTI_XFER_FLAG_QUEUE);
    ...
    
    {
        ...
        cf_add_ext(Net_player->m_player->squad_filename, NOX(".pcx"));
        ...
        {
          multi_data_add_new(Net_player->m_player->squad_filename,
                              MY_NET_PLAYER_NUM);
        ...
        
        {
            ...
            // add a file extension if necessary
            multi_xfer_send_file(Net_player->reliable_socket,
                                 Net_player->m_player->squad_filename,
                                 CF_TYPE_ANY,
                                 MULTI_XFER_FLAG_AUTODESTROY |
                                 MULTI_XFER_FLAG_QUEUE);
...
void multi_data_maybe_reload() 
{
    ...
    
    {
           ...
           && strlen(Net_players[idx].m_player->squad_filename)
           && (Net_players[idx].m_player->insignia_texture == -1))
        
        {
          Net_players[idx].m_player->insignia_texture =
            bm_load_duplicate(Net_players[idx].m_player->squad_filename);
           
            // if the bitmap loaded properly, lock it as a transparent texture
            if (Net_players[idx].m_player->insignia_texture != -1)
            
            {
              bm_lock(Net_players[idx].m_player->insignia_texture, 16,
                       BMP_TEX_XPARENT);
