...
void psnet_buffer_packet(network_packet_buffer_list * l, ubyte * data,
                         int length, net_addr * from);
 
// get the index of the next packet in order!
int psnet_buffer_get_next(network_packet_buffer_list * l, ubyte * data,
                          int *length, net_addr * from);
  ...
  fd_set rfds;
    ...
    
    {
        ...
        case NET_IPX:
            ...
            recvfrom(Unreliable_socket, (char *) packet_read.data,
                     MAX_TOP_LAYER_PACKET_SIZE, 0, (SOCKADDR *) & ipx_addr,
                     &from_len);
            ...
            recvfrom(Unreliable_socket, (char *) packet_read.data,
                     MAX_TOP_LAYER_PACKET_SIZE, 0, (SOCKADDR *) & ip_addr,
                     &from_len);
      ...
      int packet_type = packet_read.data[0];
        ...
        
        {
            ...
            // buffer the packet
            psnet_buffer_packet(&Psnet_top_buffers[packet_type],
                                packet_read.data + 1, read_len - 1,
                                &from_addr);
...
#else   /*  */
            if ((custom_address.sin_addr.s_addr =
                 inet_addr(custom_ip)) != INADDR_NONE)
...
 void psnet_rel_send_ack(SOCKADDR * raddr, unsigned int sig,
                           ubyte link_type, float time_sent) 
{
    ...
    case NET_IPX:
        ...
        SENDTO(Unreliable_socket, (char *) &ack_header,
               RELIABLE_PACKET_HEADER_ONLY_SIZE + ack_header.data_len, 0,
               raddr, sizeof(SOCKADDR), PSNET_TYPE_RELIABLE);
...
void psnet_rel_close_socket(PSNET_SOCKET_RELIABLE * sockp) 
{
  ...
  int i;
    ...
    case NET_IPX:
      ...
      SENDTO(Unreliable_socket, (char *) &diss_conn_header,
...
// send data reliably
int psnet_rel_send(PSNET_SOCKET_RELIABLE socketid, ubyte * data, int length,
                   int np_index) 
{
    ...
    
    {
        ...
        ml_printf
        ("Can't send packet because of status %d in nw_SendReliable(). socket = %d",
    ...
    
    {
        ...
        
        {
            ...
            (reliable_net_sendbuffer *)
            vm_malloc(sizeof(reliable_net_sendbuffer));
            ...
            
            {
                ...
                case NET_IPX:
                    ...
                    SENDTO(Unreliable_socket, (char *) &send_header,
                           RELIABLE_PACKET_HEADER_ONLY_SIZE +
                           rsocket->send_len[i], 0, &rsocket->addr,
...
int psnet_rel_get(PSNET_SOCKET socketid, ubyte * buffer, int max_len) 
{
    ...
    
    {
        ...
        ("Can't receive packet because it isn't connected in nw_ReceiveReliable(). socket = %d",
         socketid);
    ...
    
    {
        ...
        
        {
            ...
            case NET_IPX:
                ...
                SENDTO(Unreliable_socket, (char *) &conn_header,
                       RELIABLE_PACKET_HEADER_ONLY_SIZE, 0,
  ...
  do
    
    {
...
#else   /*  */
            udp_has_data =
            SELECT(Unreliable_socket + 1, &read_fds, NULL, NULL, &timeout,
                   PSNET_TYPE_RELIABLE);
        ...
        
        {
            ...
            RECVFROM(Unreliable_socket, (char *) &rcv_buff,
                     sizeof(reliable_header), 0, (SOCKADDR *) & rcv_addr,
        ...
        
        {
            ...
            
            {
                ...
                
                {
                    ...
                    
                    {
                        ...
                        
                        {
                            ...
                            psnet_rel_send_ack(&Reliable_sockets[i].addr,
                                               rcv_buff.seq, link_type,
                                               rcv_buff.send_time);
                ...
                
                {
                    ...
                    
                    {
                      ...
                      memcpy(&Reliable_sockets[i].m_net_addr, &d3_rcv_addr,
                              sizeof(net_addr));
                      memcpy(&Reliable_sockets[i].addr, &rcv_addr,
                              sizeof(SOCKADDR));
              ...
              psnet_rel_send_ack(&rsocket->addr, rcv_buff.seq, link_type,
                                  rcv_buff.send_time);
            ...
            
            {
                ...
                
                {
                    ...
                    
                    {
                      ...
                      psnet_rel_send_ack(&rsocket->addr, rcv_buff.seq,
            ...
            
            {
                ...
                
                {
                    ...
                    }  qsort(sort_ping, MAX_PING_HISTORY, sizeof(float),
                                psnet_rel_ping_compare);
                  rsocket->mean_ping =
                    ((sort_ping[MAX_PING_HISTORY / 2] +
                      sort_ping[(MAX_PING_HISTORY / 2) + 1])) / 2;
            ...
            
            {
              ...
              int savepacket = 1;
                ...
                
                {
                    ...
                    if (((unsigned short) (rcv_buff.seq +
                                           rsocket->oursequence)) >
                        (MAXNETBUFFERS - 1))
                ...
                
                {
                    ...
                    
                    {
                        ...
                        
                        {
                            ...
                            (reliable_net_rcvbuffer *)
                            vm_malloc(sizeof(reliable_net_rcvbuffer));
                          memcpy(rsocket->rbuffers[i]->buffer, rcv_buff.data,
                                  rsocket->recv_len[i]);
  ...
  while (udp_has_data > 0);
    ...
    
    {
        ...
        
        {
            ...
            {
              if (fl_abs((psnet_get_time() - rsocket->last_packet_received))
                   > Nettimeout)
                
                {
                  ml_printf
                    ("Reliable (but in limbo) socket (%d) timed out in nw_WorkReliable().",
                     j);
        ...
        
        {
            ...
            
            {
                ...
                
                {
                  ...
                  memcpy(send_header.data, rsocket->sbuffers[i]->buffer,
                          rsocket->send_len[i]);
  ...
  SOCKADDR rcv_addr;
    ...
    
    {
        ...
        
        {
            ...
            RECVFROM(Unreliable_socket, (char *) &ack_header,
                     sizeof(reliable_header), 0, (SOCKADDR *) & rcv_addr,
                     &addrlen, PSNET_TYPE_RELIABLE);
  ...
  do
    
    {
        ...
        
        {
            ...
            RECVFROM(typeless_sock, (char *) &ack_header,
                     sizeof(reliable_header), 0, (SOCKADDR *) & rcv_addr,
                     &addrlen, PSNET_TYPE_RELIABLE);
                    ...
                    
                    {
                        ...
                        
                        {
                            ...
                            
                            {
                                ...
                                ("Succesfully connected to server in nw_ConnectToServer().");
                                ...
                                SENDTO(typeless_sock, (char *) &conn_header,
                                       RELIABLE_PACKET_HEADER_ONLY_SIZE, 0,
...
unsigned int psnet_ras_status() 
{
    ...
    GetProcAddress(ras_handle, "RasEnumConnectionsA");
    ...
    GetProcAddress(ras_handle, "RasGetConnectStatusA");
...
void psnet_socket_options(SOCKET sock) 
{
  ...
  setsockopt(sock, SOL_SOCKET, SO_RCVBUF, (LPSTR) & bufsize,
              sizeof(bufsize));
  ...
  setsockopt(sock, SOL_SOCKET, SO_SNDBUF, (LPSTR) & bufsize,
              sizeof(bufsize));
...
int psnet_init_ipx() 
{
  ...
  if (bind(IPX_socket, (SOCKADDR *) & sockaddr, sizeof(SOCKADDR_IPX)) ==
       SOCKET_ERROR)
