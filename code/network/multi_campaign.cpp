...
void multi_campaign_start(char *filename) 
{
    ...
    
    {
        ...
        mission_parse_get_multi_mission_info(Netgame.mission_name);
                ...
                "MULTI CAMPAIGN : overriding respawn setting with mission max %d\n",
...
void multi_campaign_client_store_goals(int mission_num) 
{
    ...
    {
      strcpy_s(Campaign.missions[mission_num].goals[idx].name,
    ...
    {
      strcpy_s(Campaign.missions[mission_num].events[idx].name,
...
void multi_campaign_send_debrief_info() 
{
    ...
    
    {
        ...
        (ubyte) Campaign.missions[Campaign.current_mission].goals[idx].status;
      ADD_DATA(val);
    }
   
    // add the events
    for (idx = 0;
         idx < Campaign.missions[Campaign.current_mission].num_events; idx++)
    
    {
      val =
        (ubyte) Campaign.missions[Campaign.current_mission].events[idx].
        status;
