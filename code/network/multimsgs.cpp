...
void multi_io_send_reliable(net_player * pl, ubyte * data, int len)
{
  ...
  memcpy(pl->s_info.reliable_buffer + pl->s_info.reliable_buffer_size, data,
         len);
          ...
          if (to_player_index != MY_NET_PLAYER_NUM)
            {
              send_game_chat_packet(&Net_players[player_index], msg,
                                    (int) mode, &Net_players[to_player_index],
                                    NULL, server_msg);
            }
          break;
          // expression mode
  ...
  if (pm && !vaporized)
    {
      ...
      multi_set_network_signature((ushort)
                                  (debris_signature + pm->num_debris_objects),
                                  MULTI_SIG_DEBRIS);
  ...
  if (!message_filter_multi(id))
    {
      ...
      message_queue_message(id, priority, utiming, who_from, source, 0, delay,
                            builtin_type);
    }
}
...
void send_asteroid_hit(object * objp, object * other_objp, vec3d * hitpos,
                       float damage)
{
  int packet_size;
  ubyte data[MAX_PACKET_SIZE], packet_type;
  ...
  if ((objp != NULL) && (objp->type == OBJ_SHIP))
    {
      ...
      if (!(Net_player->flags & NETINFO_FLAG_OBSERVER) && (Player_obj != NULL)
          && (Player_obj->type == OBJ_SHIP) && (Player_obj == objp))
        {
...
void send_change_iff_color_packet(ushort net_signature, int observer_team,
                                  int observed_team, int alternate_iff_color)
{
  ubyte data[MAX_PACKET_SIZE];
  int packet_size = 0;
...
void send_NEW_countermeasure_fired_packet(object * objp, int cmeasure_count,
                                          int rand_val)
{
  ubyte data[MAX_PACKET_SIZE];
  int packet_size;
...
void send_weapon_or_ammo_changed_packet(int ship_index, int bank_type,
                                        ...
                                        int rearm_limit, int new_weapon_index)
{
  ubyte data[MAX_PACKET_SIZE];
  int packet_size = 0;
