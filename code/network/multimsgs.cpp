...
 int lzw_compress(ubyte * outputbuf, ubyte * inputbuf, int input_size) 
{
    ...
    
    {
        ...
        
        {
          ...
          output_bits(&output, (unsigned long) string_code,
                       current_code_bits);
          string_code = character;
          if (next_code > MAX_CODE)
            
            {
              output_bits(&output, (unsigned long) FLUSH_CODE,
                           current_code_bits);
            ...
            {
              output_bits(&output, (unsigned long) BUMP_CODE,
                           current_code_bits);
...
void multi_io_send(net_player * pl, ubyte * data, int len) 
{
  ...
   memcpy(pl->s_info.unreliable_buffer + pl->s_info.unreliable_buffer_size,
            data, len);
...
 void multi_io_send_force(net_player * pl) 
{
    ...
    {
      psnet_send(&pl->p_info.addr, pl->s_info.unreliable_buffer,
    ...
    {
      psnet_send(&Netgame.server_addr, pl->s_info.unreliable_buffer,
                  pl->s_info.unreliable_buffer_size, NET_PLAYER_NUM(pl));
...
void multi_io_send_reliable(net_player * pl, ubyte * data, int len) 
{
  ...
   memcpy(pl->s_info.reliable_buffer + pl->s_info.reliable_buffer_size,
            data, len);
  pl->s_info.reliable_buffer_size += len;
}

 void multi_io_send_to_all_reliable(ubyte * data, int length,
                                      net_player * ignore) 
{
...
 void multi_io_send_reliable_force(net_player * pl) 
{
    ...
    {
      psnet_rel_send(pl->reliable_socket, pl->s_info.reliable_buffer,
    ...
    {
      psnet_rel_send(Net_player->reliable_socket, pl->s_info.reliable_buffer,
                      pl->s_info.reliable_buffer_size, NET_PLAYER_NUM(pl));
...
void send_game_chat_packet(net_player * from, char *msg, int msg_mode,
                           net_player * to, char *expr, int server_msg) 
{
    ...
    
    {
        ...
        case MULTI_MSG_ALL:
            ...
            
            {
                ...
                {
                  multi_io_send_reliable(&Net_players[idx], data,
                                          packet_size);
        ...
        
        {
            ...
            
            {
                ...
                {
                  send_game_chat_packet(Net_player,
...
void process_game_chat_packet(ubyte * data, header * hinfo) 
{
    ...
    
    {
        ...
        case MULTI_MSG_TARGET:
            ...
            {
              send_game_chat_packet(&Net_players[player_index], msg,
                                     (int) mode,
                                     &Net_players[to_player_index], NULL,
                                     server_msg);
            ...
            // expression mode
        case MULTI_MSG_EXPR:
          send_game_chat_packet(&Net_players[player_index], msg, (int) mode,
                                 NULL, expr, server_msg);
          break;
          
            // all other modes
        default:
          send_game_chat_packet(&Net_players[player_index], msg, (int) mode,
                                 NULL, NULL, server_msg);
...
void process_join_packet(ubyte * data, header * hinfo) 
{
    ...
    case JOIN_QUERY_RESTRICTED:
        ...
        // NETLOG
        ml_printf(NOX("Receive restricted join request from %s"),
                  jr.callsign);
...
void process_new_player_packet(ubyte * data, header * hinfo) 
{
    ...
    
    {
        ...
        {
          HUD_sourced_printf(HUD_SOURCE_COMPUTER,
                              XSTR("%s has entered the game\n", 716),
                              new_player_name);
        ...
        {
          multi_obs_create_player(new_player_num, new_player_name, &new_addr,
                                   &Players[player_num]);
        ...
        {
          multi_create_player(new_player_num, &Players[player_num],
                               new_player_name, &new_addr, -1, new_id);
      ...
      player_set_squad_bitmap(Net_players[new_player_num].m_player,
                               new_player_squad);
       
        // copy in his pxo squad name
        strcpy_s(Net_players[new_player_num].p_info.pxo_squad_name,
        ...
        {
          sprintf(notify_string, XSTR("<%s has joined>", 717),
    ...
    // NETLOG
    ml_printf(NOX("Received notification of new player %s"),
              Net_players[new_player_num].m_player->callsign);
...
void process_accept_player_data(ubyte * data, header * hinfo) 
{
    ...
    
    {
        ...
        strcpy_s(Net_players[player_num].m_player->image_filename,
                 image_name);
        ...
        strcpy_s(Net_players[player_num].p_info.pxo_squad_name,
                 pxo_squad_name);
        ...
        
        {
            ...
            fill_net_addr(&Net_players[player_num].p_info.addr, hinfo->addr,
                          hinfo->net_id, hinfo->port);
...
void send_accept_packet(int new_player_num, int code, int ingame_join_team) 
{
    ...
    
    {
        ...
        if (!MULTI_CONNECTED(Net_players[i])
            || psnet_same(&Net_players[new_player_num].p_info.addr,
    ...
    {
      ml_printf(NOX("Server accepted %s as new client"),
                 Net_players[new_player_num].m_player->callsign);
...
 void process_accept_packet(ubyte * data, header * hinfo) 
{
    ...
    fill_net_addr(&Netgame.server_addr, hinfo->addr, hinfo->net_id,
                  hinfo->port);
...
void send_leave_game_packet(short player_id, int kicked_reason,
                            net_player * target) 
{
...
void process_leave_game_packet(ubyte * data, header * hinfo) 
{
    ...
    
    {
      ...
       sprintf(msg, XSTR("%s has left the game", 719),
                 Net_players[player_num].m_player->callsign);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                {
                  send_netgame_descript_packet(&Net_players[idx].p_info.addr,
                                                1);
  ...
  GET_INT(Netgame.security);
    ...
    
    {
        ...
        
        {
          ...
          strncpy(Game_current_mission_filename, Netgame.mission_name,
                   MAX_FILENAME_LEN);
...
void broadcast_game_query() 
{
  ...
   fill_net_addr(&addr, Psnet_my_addr.addr, Psnet_my_addr.net_id,
                   DEFAULT_GAME_PORT);
...
void send_ship_kill_packet(object * objp, object * other_objp,
                           float percent_killed, int self_destruct) 
{
    ...
    
    {
      ...
      multi_set_network_signature((ushort)
                                   (debris_signature +
    ...
    
    {
        ...
        
        {
          ...
           Assert(Net_players[pnum].m_player->killer_weapon_index <
                    SHRT_MAX);
...
void process_ship_kill_packet(ubyte * data, header * hinfo) 
{
  ...
  char killer_name[NAME_LENGTH], killer_objtype = OBJ_NONE, killer_species =
    0;
    ...
    
    {
        ...
        
        {
            ...
            killer_weapon_index;
          strcpy_s(Net_players[pnum].m_player->killer_parent_name,
                    killer_name);
...
void send_wing_create_packet(wing * wingp, int num_to_create,
...
void process_ship_depart_packet(ubyte * data, header * hinfo) 
{
    ...
    
    {
        ...
        
        {
                    ...
                    "Can not proces ship depart packed. Object with net signature %d is not a ship!\n",
                    signature));
...
void send_secondary_fired_packet(ship * shipp, ushort starting_sig,
                                 int starting_count, int num_fired,
                                 int allow_swarm) 
{
    ...
    
    {
        ...
        
        {
            ...
            ship_get_index_from_subsys(aip->targeted_subsys,
                                       aip->target_objnum);
        ...
        
        {
                  ...
                  [Weapons[Objects[aip->target_objnum].instance].
...
void process_secondary_fired_packet(ubyte * data, header * hinfo,
                                    int from_player) 
{
    ...
    shipp->weapons.next_secondary_fire_stamp[shipp->weapons.
...
void send_countermeasure_fired_packet(object * objp, int cmeasure_count,
                                      int rand_val) 
{
...
void send_turret_fired_packet(int ship_objnum, int subsys_index,
                              int weapon_objnum) 
{
    ...
    ship_get_indexed_subsys(&Ships[Objects[ship_objnum].instance],
                            subsys_index, NULL);
    ...
    multi_pack_unpack_position(1, data + packet_size, &objp->orient.vec.fvec);
...
void process_turret_fired_packet(ubyte * data, header * hinfo) 
{
    ...
    weapon_create(&pos, &orient, wid, OBJ_INDEX(objp), -1, 1);
    ...
    
    {
        ...
        {
          snd_play_3d(&Snds[Weapon_info[wid].launch_snd], &pos,
                       &View_position);
...
void process_mission_log_packet(ubyte * data, header * hinfo) 
{
  ...
   mission_log_add_entry_multi(type, pname, sname, sindex, timestamp,
                                 flags);
}  

// send a mission message packet
void send_mission_message_packet(int id, char *who_from, int priority,
                                 int timing, int source, int builtin_type,
...
void process_mission_message_packet(ubyte * data, header * hinfo) 
{
    ...
    
    {
        ...
        message_queue_message(id, priority, utiming, who_from, source, 0,
                              delay, builtin_type);
...
void send_observer_update_packet() 
{
    ...
    multi_pack_unpack_position(1, data + packet_size, &Player_obj->pos);
...
 void send_netplayer_slot_packet() 
{
    ...
    
    {
        ...
        
        {
          ...
          ADD_USHORT(Objects[Net_players[idx].m_player->objnum].
                      net_signature);
...
void send_ship_weapon_change(ship * shipp, int what, int new_bank,
                             int link_status) 
{
...
void send_player_order_packet(int type, int index, int cmd) 
{
    ...
    
    {
        ...
        ship_get_index_from_subsys(Player_ai->targeted_subsys,
...
void process_player_order_packet(ubyte * data, header * hinfo) 
{
    ...
    
    {
                                 ...
                                 &Objects[Net_players[player_num].m_player->
                                          objnum]);
    ...
    
    {
                                       ...
                                       &Objects[Net_players[player_num].
                                                m_player->objnum]);
    ...
    
    {
                ...
                "Recieved player order packet from player not allowed to give orders!!\n"));
    ...
    {
      hud_squadmsg_send_ship_command(index, command, 1,
                                      SQUADMSG_HISTORY_ADD_ENTRY, player_num);
    }
  
  else if (type == SQUAD_MSG_WING)
    
    {
      hud_squadmsg_send_wing_command(index, command, 1,
                                      SQUADMSG_HISTORY_ADD_ENTRY, player_num);
...
}  void process_file_sig_request(ubyte * data, header * hinfo) 
{
    ...
    
    {
        ...
        // reply to the server
        send_file_sig_packet(Multi_current_file_checksum,
...
void send_subsystem_destroyed_packet(ship * shipp, int index,
                                     vec3d world_hitpos) 
{
...
void process_subsystem_cargo_revealed_packet(ubyte * data, header * hinfo) 
{
    ...
    {
      send_subsystem_cargo_revealed_packet(&Ships[objp->instance],
                                            (int) uindex);
...
void process_subsystem_cargo_hidden_packet(ubyte * data, header * hinfo) 
{
    ...
    
    {
                ...
                "Could not find subsys for ship %s for cargo hidden\n",
                Ships[objp->instance].ship_name));
    ...
    {
      send_subsystem_cargo_hidden_packet(&Ships[objp->instance],
                                          (int) uindex);
...
void send_repair_info_packet(object * repaired_objp, object * repair_objp,
                             int code) 
{
    ...
    
    {
              ...
              (repair_objp ==
               NULL) ? "<none>" : Ships[repair_objp->instance].ship_name));
...
 void process_repair_info_packet(ubyte * data, header * hinfo) 
{
              ...
              (repaired_objp ==
               NULL) ? "<None>" : Ships[repaired_objp->instance].ship_name,
              (repair_objp ==
               NULL) ? "<None>" : Ships[repair_objp->instance].ship_name));
    ...
    {
       ai_do_objects_repairing_stuff(repaired_objp, repair_objp,
                                       (int) code);
        ...
        {
          ai_mission_goal_complete(&Ai_info
                                    [Ships[repair_objp->instance].ai_index]);
                                ...
                                object * other_objp) 
{
    ...
    
    {
      ...
      
      {
          ...
          Assert((aip->active_goal != AI_GOAL_NONE)
          ...
          
          {
            ...
            Assert((aigp->docker.index >= 0)
                    && (aigp->docker.index < UCHAR_MAX));
            Assert((aigp->dockee.index >= 0)
                    && (aigp->dockee.index < UCHAR_MAX));
...
void process_ai_info_update_packet(ubyte * data, header * hinfo) 
{
  ...
  objp = multi_get_network_object(net_signature);
    ...
    
    {
        ...
        ai_do_objects_docked_stuff(objp, docker_index, other_objp,
                                   dockee_index);
        ...
        
        {
            ...
            if ((mode == AI_GOAL_DESTROY_SUBSYSTEM) && (submode >= 0))
              ...
              ship_get_indexed_subsys(&Ships[other_objp->instance], submode);
...
// ALAN begin
void send_wss_request_packet(short player_id, int from_slot, int from_index,
                             int to_slot, int to_index, int wl_ship_slot,
                             int ship_class, int mode, net_player * p) 
{
...
 void process_wss_request_packet(ubyte * data, header * hinfo) 
{
  ...
  GET_INT(mode);
    ...
    {
      send_wss_request_packet(player_id, from_slot, from_index, to_slot,
                               to_index, wl_ship_slot, ship_class, mode,
                               Netgame.host);
    ...
    
    {
        ...
        
        {
          ...
          wl_drop(from_slot, from_index, to_slot, to_index, wl_ship_slot,
                   player_num);
          break;
        case WSS_SHIP_SELECT:
          multi_ts_drop(from_slot, from_index, to_slot, to_index, ship_class,
                         player_num);
...
void send_post_sync_data_packet(net_player * p, int std_request) 
{
    ...
    
    {
        ...
        
        {
            ...
            multi_find_player_by_net_signature(Objects[so->objnum].
                                               net_signature);
...
 void process_post_sync_data_packet(ubyte * data, header * hinfo) 
{
    ...
    
    {
        ...
        
        {
          ...
          ship_wing_cleanup(objp->instance,
                             &Wings[Ships[objp->instance].wingnum]);
    ...
    
    {
        ...
        {
          obj_set_flags(&Objects[shipp->objnum],
                         Objects[shipp->objnum].flags | OF_COULD_BE_PLAYER);
...
 void send_wss_slots_data_packet(int team_num, int final, net_player * p,
                                   int std_request) 
{
...
void send_shield_explosion_packet(int objnum, int tri_num, vec3d hit_pos) 
{
    ...
    
    {
        ...
        
        {
          ...
           vm_vec_normalized_dir(&eye_to_obj_vec, &Objects[objnum].pos,
                                   &eye_pos);
...
 void send_player_stats_block_packet(net_player * pl, int stats_code,
                                       net_player * target, short offset) 
{
    ...
    
    {
        ...
        {
          send_player_stats_block_packet(pl, STATS_ALLTIME_KILLS, target,
                                          (short) idx);
      ...
      break;
        ...
        {
          send_player_stats_block_packet(pl, STATS_MISSION_CLASS_KILLS,
                                          target, (short) idx);
...
 void process_player_stats_block_packet(ubyte * data, header * hinfo) 
{
    ...
    
    {
      ...
      break;
        ...
        {
          ml_printf("Dogfight stats for %s",
                     Net_players[player_num].m_player->callsign);
        ...
        
        {
            ...
            
            {
                         ...
                         Net_players[player_num].m_player->stats.
                         m_dogfight_kills[idx]);
...
void send_asteroid_create(object * new_objp, object * parent_objp,
                          int asteroid_type, vec3d * relvec) 
{
...
}  void send_asteroid_hit(object * objp, object * other_objp,
                             vec3d * hitpos, float damage) 
{
...
 void process_host_restr_packet(ubyte * data, header * hinfo) 
{
    ...
    case 0:
      ...
      HUD_printf(XSTR("Player %s has tried to join - allow (y/n) ?", 736),
                  callsign);
    ...
    case 1:
        ...
        {
          multi_process_valid_join_request(&Multi_restr_join_request,
                                            &Multi_restr_addr, (int) mode);
...
void send_client_update_packet(net_player * pl) 
{
  ...
  else if ((Game_mode & GM_IN_MISSION)
    ...
    
    {
        ...
        
        {
            ...
            (ubyte) (objp->shield_quadrant[i] / get_max_shield_quad(objp) *
                     100.0f);
...
 void process_client_update_packet(ubyte * data, header * hinfo) 
{
    ...
    
    {
      ...
      ubyte hull_percent, shield_percent[MAX_SHIELD_SECTIONS], n_subsystems,
        ...
        
        {
            ...
            
            {
                ...
                (shield_percent[i] * get_max_shield_quad(objp) / 100.0f);
            ...
            
            {
              ...
              for (subsysp = GET_FIRST(&shipp->subsys_list);
                ...
                
                {
                    ...
                    subsystem_percent[n_subsystems] * subsysp->max_hits /
                    100.0f;
...
void process_debrief_info(ubyte * data, header * hinfo) 
{
    ...
    active_stages[MAX_TVT_TEAMS][MAX_DEBRIEF_STAGES], *stages[MAX_TVT_TEAMS];
    ...
    Assert((Net_player->p_info.team >= 0)
           && (Net_player->p_info.team < Num_teams));
  debrief_set_multi_clients(stage_counts[Net_player->p_info.team],
...
void send_homing_weapon_info(int weapon_num) 
{
    ...
    
    {
        ...
        
        {
            ...
            ship_get_index_from_subsys(wp->homing_subsys,
                                       OBJ_INDEX(homing_object), 1);
...
void process_homing_weapon_info(ubyte * data, header * hinfo) 
{
    ...
    
    {
        ...
        ship_get_indexed_subsys(&Ships[homing_object->instance], h_subsys);
...
 void send_change_iff_color_packet(ushort net_signature, int observer_team,
                                     int observed_team,
...
 void process_change_iff_color_packet(ubyte * data, header * hinfo) 
{
    ...
    {
      Ships[objp->instance].ship_iff_color[observer_team][observed_team] =
        alternate_iff_color;
    }
}

 void send_change_ai_class_packet(ushort net_signature, char *subsystem,
                                    int new_ai_class) 
{
...
 void process_change_ai_class_packet(ubyte * data, header * hinfo) 
{
    ...
    
    {
        ...
        {
          ship_subsystem_set_new_ai_class(objp->instance, subsys_buf,
                                           new_ai_class);
                                                ...
                                                header * hinfo) 
{
    ...
    {
      multi_set_network_signature((ushort) rand_val,
...
 void send_beam_fired_packet(object * shooter, ship_subsys * turret,
                               object * target, int beam_info_index,
                               beam_info * override, ubyte fighter_beam,
                               int bank_point) 
{
...
}  void process_beam_fired_packet(ubyte * data, header * hinfo) 
{
  ...
  PACKET_SET_SIZE();
    ...
    
    {
        ...
        (float) cos((field_of_fire != 0.0f) ? field_of_fire : 180);
...
}  void process_event_update_packet(ubyte * data, header * hinfo) 
{
    ...
    else if ((store_flags & MEF_DIRECTIVE_SPECIAL) &
             !(Mission_events[u_event].flags & MEF_DIRECTIVE_SPECIAL))
...
 void send_weapon_or_ammo_changed_packet(int ship_index, int bank_type,
                                           int bank_number, int ammo_left,
                                           int rearm_limit,
...
 void process_weapon_or_ammo_changed_packet(ubyte * data, header * hinfo) 
{
  ...
  int ship_index, bank_type, bank_number, ammo_left, rearm_limit,
    new_weapon_index;
    ...
    
    {
        ...
        
        {
          ...
          shipp->weapons.primary_bank_weapons[bank_number] =
            new_weapon_index;
          set_primary_ammo(Player_obj->instance, bank_number, ammo_left,
                            rearm_limit);
    ...
    
    {
        ...
        {
          set_secondary_ammo(ship_index, bank_number, ammo_left,
                              rearm_limit);
        ...
        
        {
          ...
          shipp->weapons.secondary_bank_weapons[bank_number] =
            new_weapon_index;
          set_secondary_ammo(Player_obj->instance, bank_number, ammo_left,
                              rearm_limit);
...
 void process_variable_update_packet(ubyte * data, header * hinfo) 
{
    ...
    {
      send_variable_update_packet(variable_index,
                                   Sexp_variables[variable_index].text);
...
// flak fired packet
void send_flak_fired_packet(int ship_objnum, int subsys_index,
                            int weapon_objnum, float flak_range) 
{
...
// player pain packet
void send_player_pain_packet(net_player * pl, int weapon_info_index,
                             float damage, vec3d * force, vec3d * hitpos) 
{
...
}  void process_player_pain_packet(ubyte * data, header * hinfo) 
{
    ...
    Assert((windex < Num_weapon_types)
...
}  void process_bytes_recvd_packet(ubyte * data, header * hinfo) 
{
    ...
    (int) (100.0f *
           (1.0f - ((float) pl->cl_bytes_recvd / (float) pl->sv_bytes_sent)));
...
}  void process_host_captain_change_packet(ubyte * data, header * hinfo) 
{
    ...
    
    {
        ...
        
        {
            ...
            {
              HUD_printf("%s is the new captain of team %d",
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                {
                  HUD_printf("%s is the new game host",
                              Net_players[idx].m_player->callsign);
