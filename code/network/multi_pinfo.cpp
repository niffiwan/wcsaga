...
void multi_pinfo_popup_init(net_player * np) 
{
    ...
    Multi_pinfo_window.create(0, 0, gr_screen.max_w_unscaled,
    ...
    
    {
        ...
        // create the object
        Multi_pinfo_buttons[gr_screen.res][idx].button.
        create(&Multi_pinfo_window, "",
        ...
        Multi_pinfo_buttons[gr_screen.res][idx].button.
        set_bmaps(Multi_pinfo_buttons[gr_screen.res][idx].filename);
       
        // set the hotspot
        Multi_pinfo_buttons[gr_screen.res][idx].button.
        link_hotspot(Multi_pinfo_buttons[gr_screen.res][idx].hotspot);
...
void multi_pinfo_blit_pilot_image() 
{
    ...
    
    {
        ...
        else if (!(Net_player->p_info.options.flags & MLO_FLAG_ACCEPT_PIX)
      ...
      gr_string(Multi_pinfo_pilot_coords[gr_screen.res][0] +
    ...
    
    {
      ...
       gr_bitmap(Multi_pinfo_pilot_coords[gr_screen.res][0] +
...
void multi_pinfo_blit_squadron_logo() 
{
    ...
    
    {
        ...
        // center the text
        gr_get_string_size(&w, NULL, place_text);
      gr_string(Multi_pinfo_squad_coords[gr_screen.res][0] +
    ...
    
    {
      ...
       gr_bitmap(Multi_pinfo_squad_coords[gr_screen.res][0] +
...
void multi_pinfo_blit_player_stats() 
{
  ...
  gr_string(Multi_pinfo_stats_area_coords[gr_screen.res][0],
             Multi_pinfo_stats_area_coords[gr_screen.res][1],
             Multi_pinfo_popup_player->m_player->callsign);
  gr_string(Multi_pinfo_stats_x[gr_screen.res],
             Multi_pinfo_stats_area_coords[gr_screen.res][1],
    ...
    {
      gr_string(Multi_pinfo_stats_area_coords[gr_screen.res][0], y_start,
                 Multi_pinfo_stats_labels[idx]);
    ...
    {
      gr_string(Multi_pinfo_stats_x[gr_screen.res], y_start,
                 Multi_pinfo_stats_vals[idx]);
...
void multi_pinfo_build_stats() 
{
    ...
    // missions flown
    sprintf(Multi_pinfo_stats_vals[MPI_MISSIONS_FLOWN], "%d",
            (int) sc->missions_flown);
   
    // flight time
    game_format_time(fl2f((float) sc->flight_time),
    ...
    
    {
        ...
        {
          strftime(Multi_pinfo_stats_vals[MPI_LAST_FLOWN], MAX_LABEL_TEXT,
                    "%m/%d/%y %H:%M", tmr);
    ...
    // primary shots fired
    sprintf(Multi_pinfo_stats_vals[MPI_PSHOTS_FIRED], "%d",
            sc->p_shots_fired);
    ...
    {
      sprintf(Multi_pinfo_stats_vals[MPI_PSHOTS_PCT], "%d%%",
               (int) (100.0f *
                      ((float) sc->p_shots_hit / (float) sc->p_shots_fired)));
    ...
    // primary shots fired
    sprintf(Multi_pinfo_stats_vals[MPI_SSHOTS_FIRED], "%d",
            sc->s_shots_fired);
    ...
    {
      sprintf(Multi_pinfo_stats_vals[MPI_SSHOTS_PCT], "%d%%",
               (int) (100.0f *
                      ((float) sc->s_shots_hit / (float) sc->s_shots_fired)));
