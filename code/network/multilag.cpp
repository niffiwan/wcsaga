...
int multi_lag_select(int nfds, fd_set * readfds, fd_set * writefds,
                     fd_set * except_fds, const timeval * timeout)
{
  char t_buf[1024];
  int t_from_len;
  SOCKADDR_IN ip_addr;
      ...
      if (Tcp_active)
        {
          ...
          ret_val =
            recvfrom(readfds->fd_array[0], t_buf, 1024, 0,
...
int multi_lag_recvfrom(uint s, char *buf, int len, int flags,
  ...
  if ((float) rand() / (float) RAND_MAX < 0.5)
    {
      ...
      if (Multi_lag_min >= 0)
        {
          mod =
            -(int) ((float) (Multi_lag_base - Multi_lag_min) *
  ...
  else
    {
      ...
      if (Multi_lag_max >= 0)
        {
          mod =
            (int) ((float) (Multi_lag_max - Multi_lag_base) *
                   ((float) rand() / (float) RAND_MAX));
        }
    }
      ...
      if (Multi_loss_min >= 0.0f)
        {
          mod =
            -((Multi_loss_base -
               Multi_loss_min) * ((float) rand() / (float) RAND_MAX));
        }
    }
  else
    {
      ...
      if (Multi_loss_max >= 0.0f)
        {
          mod =
            ((Multi_loss_max -
              Multi_loss_base) * ((float) rand() / (float) RAND_MAX));
        }
    }
