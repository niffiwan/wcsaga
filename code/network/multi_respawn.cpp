...
void multi_respawn_player(net_player * pl, char cur_primary_bank,
                          char cur_secondary_bank, ubyte cur_link_status,
                          ushort ship_ets, ushort net_sign, char *parse_name,
                          vec3d * pos = NULL);
...
void multi_respawn_check(object * objp) 
{
    ...
    
    {
        ...
        
        {
            ...
            if ((pobjp->flags & P_OF_PLAYER_START)
    ...
    
    {
        ...
        
        {
            ...
            if (((Net_players[idx].s_info.invul_timestamp != -1)
                 &&
                 (timestamp_elapsed(Net_players[idx].s_info.invul_timestamp)))
                || ((Net_players[idx].m_player->ci.fire_primary_count > 0)
                     || (Net_players[idx].m_player->ci.fire_secondary_count >
                         0)))
...
void multi_respawn_build_points() 
{
    ...
    
    {
        ...
        if (Objects[moveup->objnum].
            flags & (OF_PLAYER_SHIP | OF_COULD_BE_PLAYER))
...
void multi_respawn_wing_stuff(ship * shipp) 
{
  ...
   hud_set_wingman_status_alive(shipp->wing_status_wing_index,
...
void multi_respawn_player(net_player * pl, char cur_primary_bank,
                          char cur_secondary_bank, ubyte cur_link_status,
                          ushort ship_ets, ushort net_sig, char *parse_name,
                          vec3d * pos) 
{
...
void multi_respawn_process_packet(ubyte * data, header * hinfo) 
{
    ...
    
    {
        ...
        // respawn the player
        multi_respawn_player(&Net_players[player_index], cur_primary_bank,
                             cur_secondary_bank, cur_link_status, ship_ets,
                             net_sig, parse_name, &v);
        ...
        
        {
                    ...
                    "Received respawn request from player who shouldn't be respawning!\n"));
        ...
        
        {
            ...
            {
              multi_respawn_player(&Net_players[player_index],
                                    Net_players[player_index].s_info.
                                    cur_primary_bank,
                                    Net_players[player_index].s_info.
                                    cur_secondary_bank,
                                    Net_players[player_index].s_info.
                                    cur_link_status,
                                    Net_players[player_index].s_info.ship_ets,
                                    0,
                                    Net_players[player_index].p_info.p_objp->
                                    name);
...
void multi_respawn_server() 
{
    ...
    // respawn me
    multi_respawn_player(Net_player, Net_player->s_info.cur_primary_bank,
                         Net_player->s_info.cur_secondary_bank,
                         Net_player->s_info.cur_link_status,
                         Net_player->s_info.ship_ets, 0,
        ...
        
        {
            ...
            
            {
                ...
                
                {
                  ...
                  multi_respawn_send_ai_respawn(Ai_respawns[i].pobjp->
                                                 net_signature);
...
void multi_respawn_place(object * new_obj, int team) 
{
    ...
    
    {
        ...
        
        {
            ...
            ship_name_lookup(Multi_respawn_priority_ships[idx].ship_name);
               ...
               && ((pri == NULL)
                   || (Ships[lookup].respawn_priority >
                       pri->respawn_priority)) && (Ships[lookup].objnum >= 0)
    ...
    
    {
        ...
        {
          vm_vec_scale_add(&new_obj->pos, &pri_obj->pos,
                            &pri_obj->orient.vec.rvec, 2000.0f);
