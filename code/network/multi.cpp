...
void multi_check_listen() 
{
    ...
    
    {
        ...
        
        {
               ...
               &&
               (!memcmp(&(addr.addr), &(Net_players[i].p_info.addr.addr), 6)))
            
            {
                ...
                send_accept_player_data(&Net_players[i],
                                        (Net_players[i].
                                         flags & NETINFO_FLAG_INGAME_JOIN) ? 1
                                        : 0);
                ...
                // NETLOG
                ml_printf(NOX("Accepted TCP connection from %s"),
                          Net_players[i].m_player ==
                          NULL ? NOX("Unknown") : Net_players[i].m_player->
                          callsign);
...
void multi_client_check_server() 
{
    ...
    
    {
        ...
        
        {
            ...
            popup_till_condition(multi_client_wait_on_server,
                                 XSTR("Cancel", 641),
                                 XSTR
...
void process_packet_normal(ubyte * data, header * header_info) 
{
    ...
    
    {
        ...
        multi_xfer_process_packet(data + HEADER_LENGTH, sock) + HEADER_LENGTH;
...
void multi_process_bigdata(ubyte * data, int len, net_addr * from_addr,
                           int reliable) 
{
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
              if ((sock_status == RNF_UNUSED) || (sock_status == RNF_BROKEN)
                ...
                
                {
                    ...
                    if (((time(NULL) -
                        ...
                        && (Net_players[idx].reliable_socket ==
                            INVALID_SOCKET))
...
void multi_process_incoming() 
{
    ...
    
    {
        ...
        
        {
            ...
            
            {
                       ...
                       psnet_rel_get(Net_players[idx].reliable_socket, data,
                                     MAX_NET_BUFFER)) > 0)
                
                {
                  multi_process_bigdata(data, size,
    ...
    
    {
        ...
        
        {
                   ...
                   psnet_rel_get(Net_player->reliable_socket, data,
                                 MAX_NET_BUFFER)) > 0)
...
 void multi_do_frame() 
{
    ...
    
    {
        ...
        
        {
          ...
          cam->get_info(&Net_player->s_info.eye_pos,
  ...
  else if ((Net_player->flags & NETINFO_FLAG_CONNECTED)
           && (Game_mode & GM_IN_MISSION))
    ...
    
    {
        ...
        
        {
            ...
            
            {
                   ...
                   || timestamp_elapsed_safe(Multi_client_update_times[idx],
                                             1000))
...
 void standalone_main_init() 
{
  ...
  extern int read_pilot_file(char *callsign, int single = 1, player * p =
                             NULL);
    ...
    Net_player->p_info.options.obj_update_level =
  ...
  DCF(netd, "change/list netgame debug flags") 
{
    ...
    if ((Dc_arg_type & ARG_INT) && (Net_player != NULL)
        && (Net_player->flags & NETINFO_FLAG_AM_MASTER) && (Dc_arg_int <= 7))
