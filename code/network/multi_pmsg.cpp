...
void multi_msg_eval_text_msg() 
{
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                send_game_chat_packet(Net_player, Multi_msg_text,
                                      Multi_msg_mode,
                                      &Net_players[player_index]);
               
                // echo the message locally
                multi_msg_display_mission_text(Multi_msg_text,
                                               MY_NET_PLAYER_NUM);
    ...
    
    {
        ...
        // send the chat packet
        send_game_chat_packet(Net_player, Multi_msg_text, Multi_msg_mode,
                              NULL);
...
void multi_msg_display_mission_text(char *msg, int player_index) 
{
    ...
    {
      HUD_sourced_printf(HUD_SOURCE_NETPLAYER, "%s %s",
                          XSTR("<SERVER>", 698), msg);
...
void multi_msg_show_squadmsg(net_player * source, int command,
                             ushort target_sig, int subsys_type) 
{
    ...
    sprintf(hud_string, XSTR("ORDER FROM <%s> : ", 699),
    ...
    case ATTACK_TARGET_ITEM:
        ...
        {
          sprintf(temp_string, XSTR("Attack %s", 700),
    ...
    case DISABLE_TARGET_ITEM:
        ...
        {
          sprintf(temp_string, XSTR("Disable %s", 701),
                   Ships[target_obj->instance].ship_name);
    ...
    case PROTECT_TARGET_ITEM:
        ...
        {
          sprintf(temp_string, XSTR("Protect %s", 702),
                   Ships[target_obj->instance].ship_name);
    ...
    case IGNORE_TARGET_ITEM:
        ...
        {
          sprintf(temp_string, XSTR("Ignore %s", 703),
                   Ships[target_obj->instance].ship_name);
    ...
    case DISARM_TARGET_ITEM:
        ...
        {
          sprintf(temp_string, XSTR("Disarm %s", 704),
                   Ships[target_obj->instance].ship_name);
    ...
    case DISABLE_SUBSYSTEM_ITEM:
        ...
        {
          sprintf(temp_string, XSTR("Disable subsystem %s on %s", 705),
...
void multi_msg_send_squadmsg_packet(net_player * target, net_player * source,
                                    int command, ushort net_sig,
                                    int subsys_type) 
{
...
int multi_msg_eval_wing_squadmsg(int wingnum, int command, ai_info * aif,
                                 int player_num) 
{
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                {
                  multi_msg_show_squadmsg(&Net_players[player_num], command,
                                           net_sig, subsys_type);
                ...
                {
                  multi_msg_send_squadmsg_packet(&Net_players[idx],
...
int multi_msg_eval_ship_squadmsg(int shipnum, int command, ai_info * aif,
                                 int player_num) 
{
...
void multi_msg_process_squadmsg_packet(unsigned char *data, header * hinfo) 
{
    ...
    multi_msg_show_squadmsg(&Net_players[source_index], command, net_sig,
                            (int) s_val);
