...
 void camera::set_fov(float in_fov, float in_fov_time,
                        ...
                        float in_fov_deceleration_time) 
{
  ...
   fov.setAVD(in_fov, in_fov_time, in_fov_acceleration_time,
                in_fov_deceleration_time, 0.0f);
...
void camera::
set_custom_position_function(void (*n_func_custom_position)
                             (camera * cam, vec3d * camera_pos)) 
{
...
//It should be replaced or added to as the custom function modifier sees fit.
void camera::
set_custom_orientation_function(void (*n_func_custom_orientation)
                                (camera * cam, matrix * camera_ori)) 
{
  func_custom_orientation = n_func_custom_orientation;
}  void camera::set_position(vec3d * in_position,
                                ...
                                float in_end_velocity) 
{
    ...
    {
      pos_x.setVD(in_translation_time, in_translation_acceleration_time,
                   in_end_velocity);
      pos_y.setVD(in_translation_time, in_translation_acceleration_time,
                   in_end_velocity);
      pos_z.setVD(in_translation_time, in_translation_acceleration_time,
                   in_end_velocity);
      return;
    }
   pos_x.setAVD(in_position->xyz.x, in_translation_time,
                  in_translation_acceleration_time,
                  in_translation_deceleration_time, in_end_velocity);
  pos_y.setAVD(in_position->xyz.y, in_translation_time,
                in_translation_acceleration_time,
                in_translation_deceleration_time, in_end_velocity);
  pos_z.setAVD(in_position->xyz.z, in_translation_time,
                in_translation_acceleration_time,
                in_translation_deceleration_time, in_end_velocity);
}

 void camera::set_translation_velocity(vec3d * in_velocity,
                                         float in_acceleration_time) 
{
  pos_x.setVD(in_acceleration_time, in_acceleration_time,
               in_velocity->xyz.x);
  pos_y.setVD(in_acceleration_time, in_acceleration_time,
               in_velocity->xyz.y);
  pos_z.setVD(in_acceleration_time, in_acceleration_time,
               in_velocity->xyz.z);
}  void camera::set_rotation(matrix * in_orientation,
                                ...
                                float in_rotation_deceleration_time) 
{
    ...
    ori[i].setAVD(in_orientation->a1d[i], in_rotation_time,
                   in_rotation_acceleration_time,
                   in_rotation_deceleration_time, 0.0f);
     ...
     */ 
}  void camera::set_rotation(angles * in_angles, float in_rotation_time,
                               float in_rotation_acceleration_time,
                               float in_rotation_deceleration_time) 
{
  matrix mtx = IDENTITY_MATRIX;
  vm_rotate_matrix_by_angles(&mtx, in_angles);
  this->set_rotation(&mtx, in_rotation_time, in_rotation_acceleration_time,
                                      ...
                                      float in_rotation_deceleration_time) 
{
  ...
   set_rotation(&temp_matrix, in_rotation_time,
                  in_rotation_acceleration_time,
                  in_rotation_deceleration_time);
...
 void camera::get_info(vec3d * position, matrix * orientation) 
{
    ...
    
    {
        ...
        
        {
            ...
            {
              model_find_world_point(&c_pos, &pt, pm->id,
                                      object_host_submodel, &objp->orient,
                                      &objp->pos);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                {
                  model_find_world_point(&target_pos, &vmd_zero_vector,
                                          pm->id, object_target_submodel,
...
void warp_camera::do_frame(float in_frametime) 
{
  ...
   apply_physics(c_damping, c_desired_vel.xyz.x, c_vel.xyz.x, in_frametime,
                   &new_vel.xyz.x, &delta_pos.xyz.x);
  apply_physics(c_damping, c_desired_vel.xyz.y, c_vel.xyz.y, in_frametime,
                 &new_vel.xyz.y, &delta_pos.xyz.y);
  apply_physics(c_damping, c_desired_vel.xyz.z, c_vel.xyz.z, in_frametime,
...
#define MAX_SUBTITLE_LINES      64
  subtitle::subtitle(int in_x_pos, int in_y_pos, char *in_text,
                     float in_display_time, char *in_imageanim,
                     float in_fade_time, color * in_text_color, bool center_x,
                     bool center_y, int in_width, bool in_post_shaded) 
{
    ...
    
    {
        ...
        split_str(in_text, in_width, text_line_lens, text_line_ptrs,
                  MAX_SUBTITLE_LINES);
...
 void subtitle::do_frame(float frametime) 
{
    ...
    {
      gr_set_bitmap(image_id, GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL,
...
 camid cam_create(char *n_name, vec3d * n_pos, vec3d * n_norm,
                    object * n_object, int n_object_host_submodel) 
{
...
 camid cam_create(char *n_name, vec3d * n_pos, matrix * n_ori,
                    object * n_object, int n_object_host_submodel) 
{
