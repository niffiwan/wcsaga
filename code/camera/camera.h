...
public:
  ...
  void
    set_custom_position_function(void (*n_func_custom_position)
                                 (camera * cam, vec3d * pos));
  void
    set_custom_orientation_function(void (*n_func_custom_orientation)
                                    (camera * cam, matrix * ori));

  void set_zoom(float in_zoom, float in_zoom_time =
                0.0f, float in_zoom_acceleration_time =
                0.0f, float in_zoom_deceleration_time = 0.0f);

  void set_position(vec3d * in_position = NULL, float in_translation_time =
                    0.0f, float in_translation_acceleration_time =
                    0.0f, float in_translation_deceleration_time =
                    0.0f, float in_end_velocity = 0.0f);
  void set_translation_velocity(vec3d * in_velocity,
                                float in_acceleration_time = 0.0f);

  void set_rotation(matrix * in_orientation = NULL, float in_rotation_time =
                    0.0f, float in_rotation_acceleration_time =
                    0.0f, float in_rotation_deceleration_time = 0.0f);
  void set_rotation(angles * in_angles, float in_rotation_time =
                    0.0f, float in_rotation_acceleration_time =
                    0.0f, float in_rotation_deceleration_time = 0.0f);
  void set_rotation_facing(vec3d * in_target, float in_rotation_time =
                           0.0f, float in_rotation_acceleration_time =
                           0.0f, float in_rotation_deceleration_time = 0.0f);
  void set_rotation_velocity(angles * in_rotation_rate,
                             float in_acceleration_time = 0.0f);
...
private:
  ...
  bool post_shaded;
public:
    subtitle(int in_x_pos, int in_y_pos, char *in_text =
             NULL, char *in_imageanim = NULL, float in_display_time =
             0, float in_fade_time = 0.0f, color * in_text_color =
             NULL, bool center_x = false, bool center_y =
             false, int in_width = 0, int in_height = 0, bool post_shaded =
             false);
   ~subtitle();
...
camid cam_create(char *n_name = NULL, vec3d * n_pos = NULL, matrix * n_ori =
                 NULL, object * n_object = NULL, int n_submodel_parent = -1);
