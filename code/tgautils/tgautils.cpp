...
//
static int targa_copy_data(char *to, char *from, int pixels, int fromsize,
                           int tosize)
{
...
//
int targa_compress(char *out, char *in, int outsize, int pixsize,
                   int bytecount)
{
      ...
      if (pixcount == 129)
        {
          *flagbyte = 127;
          ...
          ++flagbyte;
          flagbyte +=
            targa_copy_data(flagbyte, copyloc, pixcount - 1, pixsize,
                            outsize);
          pixcount = 1;
              ...
              if (pixcount > (rlcount + 1))
                {
                  ...
                  *flagbyte++ = (char) (pixcount - 2 - rlthresh);
                  ...
                  flagbyte +=
                    targa_copy_data(flagbyte, copyloc,
...
//
static void targa_read_pixel(int num_pixels, ubyte ** dst, ubyte ** src,
...
//
int targa_read_header(char *real_filename, CFILE * img_cfp, int *w, int *h,
                      int *bpp, ubyte * palette)
{
  targa_header header;
...
int targa_read_bitmap(char *real_filename, ubyte * image_data,
                      ubyte * palette, int dest_size, int cf_type)
{
  ...
  cfseek(targa_file, cfilelength(targa_file) - TARGA_FOOTER_SIZE,
         CF_SEEK_SET);
  ...
  Assert((bytes_per_pixel == 2) || (bytes_per_pixel == 3)
         || (bytes_per_pixel == 4));
  ...
  if (header.color_map_type)
    {
      // non-zero indicates palette in the file
      Int3();
      ...
      for (int i = 0; i < header.height; i++)
        {
          ...
          targa_read_pixel(header.width, &pixptr, &src_pixels,
                           bytes_per_pixel, dest_size);
        }
      ...
      for (int i = 0; i < header.height; i++)
        {
          ...
          src_pixels +=
            targa_uncompress(pixdata, src_pixels, header.width,
                             bytes_per_pixel, dest_size);
        }
...
//
int targa_write_bitmap(char *real_filename, ubyte * data, ubyte * palette,
                       int w, int h, int bpp)
{
  Assert(bpp == 24);
  char filename[MAX_FILENAME_LEN];
  ...
  compressed_data_len =
    targa_compress((char *) compressed_data, (char *) data, 3,
                   bytes_per_pixel, w * h * bytes_per_pixel);
