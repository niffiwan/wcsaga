...
CFILE *cfopen(char *filename, char *mode, int type =
              CFILE_NORMAL, int dir_type = CF_TYPE_ANY, bool localize =
              false);
...
CFILE *cfopen_special(char *file_path, char *mode, const int size,
                      const int offset, int dir_type = CF_TYPE_ANY);
...
int cf_chksum_long(char *filename, uint * chksum, int max_size =
                   -1, int cf_type = CF_TYPE_ANY);
...
void cfread_vector(vec3d * vec, CFILE * file, int ver = 0, vec3d * deflt =
                   NULL);
void cfread_angles(angles * ang, CFILE * file, int ver = 0, angles * deflt =
                   NULL);
...
int cf_get_file_list(int max, char **list, int type, char *filter, int sort =
                     CF_SORT_NONE, file_list_info * info = NULL);
...
int cf_find_file_location_ext(char *filename, const int ext_num,
                              ...
                              int max_out = 0, char *pack_filename =
                              NULL, int *size = NULL, int *offset =
                              NULL, bool localize = false);
