...
void cf_build_root_list(char *cdrom_dir)
{
  ...
  if (Cmdline_mod)
    {
      for (cur_pos = Cmdline_mod; strlen(cur_pos) != 0;
           cur_pos += (strlen(cur_pos) + 1))
        {
          ...
          strncat(str_temp, DIR_SEPARATOR_STR,
                  (CF_MAX_PATHNAME_LENGTH - strlen(str_temp) - 1));

          // truncated string check
          if ((strlen(Cfile_user_dir) + strlen(str_temp) + 1) >=
          ...
          strncat(root->path, str_temp,
                  (CF_MAX_PATHNAME_LENGTH - strlen(root->path) - 1));
          ...
          if (!_getcwd(root->path, CF_MAX_PATHNAME_LENGTH))
            {
              Error(LOCATION, "Can't get current working directory -- %d",
                    errno);
            }

          // truncated string check
          if ((strlen(root->path) + strlen(str_temp) + 1) >=
              CF_MAX_PATHNAME_LENGTH)
            {
              Int3();
            }
  ...
  if (*Home_path == '\0')
    snprintf(Home_path, sizeof(Home_path) - 1, "%s%s%s%s%s%s", detect_home(),
                      ...
                      if (is_ext_in_list(Pathtypes[i].extensions, ext))
                        {
                          ...
                          std::transform(name_ext.begin(), name_ext.end(),
                                         name_ext.begin(), tolower);
          ...
          while ((dir = readdir(dirp)) != NULL)
            {
              if (!fnmatch("*.*", dir->d_name, 0))
                {
                  char fn[MAX_PATH];
                  snprintf(fn, MAX_PATH - 1, "%s%s", search_path,
                           dir->d_name);
                  fn[MAX_PATH - 1] = 0;
...
int cf_find_file_location_fast(char *filespec, int pathtype, int max_out,
                               char *pack_filename, int *size, int *offset,
                               bool localize)
{
  std::string filespec_lower(filespec);
  std::transform(filespec_lower.begin(), filespec_lower.end(),
                 filespec_lower.begin(), tolower);

  boost::unordered_map < std::string, cf_file * >::iterator lookup_iter =
    File_lookup.find(filespec_lower);
                      ...
                      if (pack_filename)
                        {
                          ...
                          if (f->pack_offset < 1)
                            {
                              strcat_s(pack_filename, max_out,
                                       Pathtypes[f->pathtype_index].path);

                              if (pack_filename[strlen(pack_filename) - 1] !=
                                  DIR_SEPARATOR_CHAR)
                                strcat_s(pack_filename, max_out,
                                         DIR_SEPARATOR_STR);
...
int cf_find_file_location(char *filespec, int pathtype, int max_out,
                          char *pack_filename, int *size, int *offset,
                          bool localize)
{
  ...
  if (cf_find_file_location_fast
      (filespec, pathtype, max_out, pack_filename, size, offset, localize))
    {
      return 1;
    }
      ...
      for (int tested_root = 0; tested_root < Num_roots; ++tested_root)
        {
          ...
          cf_create_default_path_string_for_root(tested_root, longname,
                                                 sizeof(longname) - 1,
                                                 search_order[i], filespec,
                                                 localize);
...
int cf_find_file_location_ext(char *filename, const int ext_num,
                              const char **ext_list, int pathtype,
                              int max_out, char *pack_filename, int *size,
                              int *offset, bool localize)
{
  ...
  for (cur_ext = 0; cur_ext < ext_num; cur_ext++)
    {
      ...
      if (cf_find_file_location_fast
          (filespec, pathtype, max_out, pack_filename, size, offset,
           localize))
        {
          return cur_ext;
        }
...
int cf_matches_spec(char *filespec, char *filename)
{
  ...
  dst_ext =
    filename + strlen(filename) - ((filespec + strlen(filespec)) - src_ext);
  if (!dst_ext)
    return 1;
...
int cf_get_file_list(int max, char **list, int pathtype, char *filter,
...
#if defined _WIN32
  cf_create_default_path_string(filespec, sizeof(filespec) - 1, pathtype,
                                filter);
...
int cf_get_file_list_preallocated(int max, char arr[][MAX_FILENAME_LEN],
                                  char **list, int pathtype, char *filter,
                                  int sort, file_list_info * info)
{
...
int cf_create_default_path_string(char *path, uint path_max, int pathtype,
                                  char *filename, bool localize)
{
  return cf_create_default_path_string_for_root(0, path, path_max, pathtype,
                                                filename, localize);
}
...
int cf_create_default_path_string_for_root(int root_n, char *path,
                                           uint path_max, int pathtype,
                                           char *filename, bool localize)
{
#ifdef SCP_UNIX
