...
void opengl_kill_render_target(int slot);
                         ...
                         int num_mipmaps, ubyte * image_data = NULL);
                            ...
                            float *u_scale, float *v_scale, int stage = 0);
 ...
 */ 
    class texture_pool 
  {
  private:static SCP_multimap < std::pair < int, int >,
      texture * >pool;
    ...
    static texture *acquire(int w, int h) 
    {
      ...
      SCP_multimap < std::pair < int, int >, texture * >::iterator itr =
        pool.find(std::pair < int, int >(w, h));
    ...
    static void release(texture * tex) 
    {
      ...
      pool.insert(std::pair < std::pair < int, int >,
                   texture * >(std::pair < int,
                               int >(tex->get_width(), tex->get_height()),
                               tex));
 ...
 */ 
    class render_target 
  {
    ...
    void use_depth_rb() 
    {
      ...
      vglFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT,
                                     GL_DEPTH_ATTACHMENT_EXT,
                                     GL_RENDERBUFFER_EXT, depth_rb->get_id());
    ...
    void set_depth(texture * tex) 
    {
      ...
      vglFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT,
                                  GL_TEXTURE_2D, depth_t->get_id(), 0);
    ...
    static void release(render_target * rt) 
    {
                   ...
                   render_target * >(std::pair < int,
                                     int >(rt->get_width(), rt->get_height()),
                                     rt));
