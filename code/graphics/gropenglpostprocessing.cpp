...
texture *bloom::high_pass(texture * image, int size)
{
  ...
  special_shader *bright_sdr =
    shader_manager::get()->get_special_shader(special_shader::bright_pass);
  ...
  target =
    render_target::acquire(image->get_width() / size,
                           image->get_width() / size);

  Assert(target);

  // Acquire two textures
  texture *tex =
    texture_pool::acquire(target->get_width(), target->get_height());
...
texture *bloom::blur(texture * image, int size)
{
  ...
  // Blur
  special_shader *blur_sdr =
    shader_manager::get()->get_special_shader(special_shader::blur);
...
void bloom::set_uniforms(post_shader * sdr)
{
  ...
  if (Neb2_render_mode == NEB2_RENDER_NONE)
    sdr->set_uniform(post_shader::bloom, intensity,
                     MIN(config::get_integer(config::bloom_int),
                         200) / 100.0f);
  else
    sdr->set_uniform(post_shader::bloom, intensity,
                     MIN(config::get_integer(config::bloom_int),
...
bool depth_of_field::begin(texture * pp_img)
{
  ...
  special_shader *blur_sdr =
    shader_manager::get()->apply_special_shader(special_shader::blur);
...
void depth_of_field::set_uniforms(post_shader * sdr)
{
  ...
  sdr->set_texture(post_shader::dof, depth_tex,
                   post_processing::get()->get_target()->get_depth());
}
...
{
  if (!resources::text_file::
      file_exist("post_processing.tbl", resources::text_file::table_file))
    return;
...
post_processing::post_processing()
{
  ...
  if (!target->check_status())
  ...
  else
    {
      ...
      depth =
        texture::create(target->get_width(), target->get_height(),
                        texture::fmt_depth);
#endif
    }
...
void post_processing::after()
{
  ...
  if (gr_screen.max_h != gr_screen.clip_height)
    {
      ...
      glScissor(0, (gr_screen.max_h - gr_screen.clip_height) / 2,
                gr_screen.max_w, gr_screen.clip_height);
    }
  else if (Cutscene_bar_size)
    {
      glEnable(GL_SCISSOR_TEST);
      glScissor(0, Cutscene_bar_size, gr_screen.max_w,
                gr_screen.max_h - Cutscene_bar_size * 2);
    }
  ...
  sdr->set_uniform(post_shader::timer,
                   static_cast < float >(timer_get_milliseconds() % 100 + 1));
...
void gr_opengl_post_process_init()
{
  ...
  if (!opengl::config::is_enabled(opengl::config::glsl)
      || opengl::config::is_enabled(opengl::config::no_fbo))
    {
