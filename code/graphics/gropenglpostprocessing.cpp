...
 texture * bloom::high_pass(texture * image, int size) 
{
    ...
    shader_manager::get()->apply_special_shader(special_shader::bright_pass);
  ...
  bool depth_of_field::begin(texture * pp_img) 
{
    ...
    render_target::acquire(gr_screen.max_w / 4, gr_screen.max_h / 4);
...
  void post_processing::before() 
{
  ...
   target->clear(render_target::clr_color | render_target::clr_depth,
                   false);
...
}  void post_processing::after() 
{
    ...
    sdr->set_texture(post_shader::texture, image);
                    ...
                    static_cast <
                    float >(timer_get_milliseconds() % 100 + 1));
