...
  void opengl_go_fullscreen() 
{
    ...
    os_config_read_uint(NULL, NOX("OGL_RefreshRate"), 0);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                       ...
                       "Unable to go fullscreen on second attempt!");
    ...
    
    {
        ...
        
        {
                ...
                (gr_screen.max_w, gr_screen.max_h, 0, SDL_OPENGL)) == NULL)
...
 void opengl_minimize() 
{
    ...
    {
      SDL_SetGammaRamp(GL_original_gamma_ramp,
                        (GL_original_gamma_ramp + 256),
...
}  void gr_opengl_print_screen(char *filename) 
{
    ...
    
    {
      ...
      vglBufferDataARB(GL_PIXEL_PACK_BUFFER_ARB,
                        (gr_screen.max_w * gr_screen.max_h * 4), NULL,
                        GL_STATIC_READ);
        ...
        pixels =
        (GLubyte *) vglMapBufferARB(GL_PIXEL_PACK_BUFFER_ARB, GL_READ_ONLY);
    ...
    
    {
        ...
        (GLubyte *) vm_malloc_q(gr_screen.max_w * gr_screen.max_h * 4);
...
 void gr_opengl_cleanup(int minimize) 
{
    ...
    
    {
        ...
        
        {
          ...
          MessageBox(wnd, "Unable to delete rendering context", "error",
                      MB_OK);
...
 void opengl_save_mouse_area(int x, int y, int w, int h) 
{
    ...
    
    {
        ...
        
        {
          ...
          vglBufferDataARB(GL_PIXEL_PACK_BUFFER_ARB, cursor_size * 4, NULL,
                            GL_STATIC_READ);
        }
       vglBindBufferARB(GL_PIXEL_PACK_BUFFER_ARB, GL_cursor_pbo);
      glReadBuffer(GL_BACK);
      glReadPixels(x, gr_screen.max_h - y - 1 - h, w, h, GL_read_format,
                    GL_UNSIGNED_INT_8_8_8_8_REV, NULL);
    ...
    
    {
        ...
        if (GL_saved_mouse_data == NULL)
                    ...
                    GL_UNSIGNED_INT_8_8_8_8_REV, GL_saved_mouse_data);
...
 int gr_opengl_save_screen() 
{
    ...
    
    {
        ...
        
        {
            ...
            (GLubyte *) vglMapBufferARB(GL_PIXEL_PACK_BUFFER_ARB,
                                        GL_READ_ONLY);
           sptr = (ubyte *) pixels;
          dptr =
            (ubyte *) &
            GL_saved_screen[(GL_mouse_saved_x1 +
        ...
        bm_create(32, gr_screen.max_w, gr_screen.max_h, GL_saved_screen, 0);
...
}  void gr_opengl_dump_frame_start(int first_frame,
                                      int frames_between_dumps) 
{
...
}  int opengl_init_display_device() 
{
  ...
  int PixelFormat;
    ...
    {
      MessageBox(wnd, "Unable to get device context for OpenGL W32!",
    ...
    {
      MessageBox(wnd, "Unable to choose pixel format for OpenGL W32!",
                  "error", MB_ICONERROR | MB_OK);
    ...
    {
      DescribePixelFormat(GL_device_context, PixelFormat,
                           sizeof(PIXELFORMATDESCRIPTOR), &pfd_test);
       
        // make sure that we are hardware accelerated and not using the generic implementation
        if (!Fred_running && (pfd_test.dwFlags & PFD_GENERIC_FORMAT)
            && !(pfd_test.dwFlags & PFD_GENERIC_ACCELERATED))
        
        {
            ...
            
            {
                          ...
                          "Unable to choose pixel format for OpenGL W32!",
                          "error", MB_ICONERROR | MB_OK);
            ...
            DescribePixelFormat(GL_device_context, PixelFormat,
                                sizeof(PIXELFORMATDESCRIPTOR), &pfd_test);
           if ((pfd_test.dwFlags & PFD_GENERIC_FORMAT)
                 && !(pfd_test.dwFlags & PFD_GENERIC_ACCELERATED))
    ...
    {
      MessageBox(wnd, "Unable to set pixel format for OpenGL W32!", "error",
                  MB_ICONERROR | MB_OK);
    ...
    {
      MessageBox(wnd, "Unable to create rendering context for OpenGL W32!",
                  "error", MB_ICONERROR | MB_OK);
    ...
    {
      MessageBox(wnd, "Unable to make current thread for OpenGL W32!",
                  "error", MB_ICONERROR | MB_OK);
  ...
  int r = 0, g = 0, b = 0, depth = 0, db = 1;
  ...
   if (SDL_SetVideoMode(gr_screen.max_w, gr_screen.max_h, bpp, flags) ==
         NULL)
    ...
    {
      SDL_GetGammaRamp(GL_original_gamma_ramp,
                        (GL_original_gamma_ramp + 256),
                        (GL_original_gamma_ramp + 512));
...
  void opengl_setup_function_pointers() 
{
  ...
  gr_screen.gf_start_angles_instance_matrix =
    gr_opengl_start_instance_angles;
...
}   bool gr_opengl_init() 
{
  ...
   if ((major < REQUIRED_GL_MAJOR_VERSION)
         || ((major <= REQUIRED_GL_MAJOR_VERSION)
    ...
    
    {
             ...
             major, minor, REQUIRED_GL_MAJOR_VERSION,
             REQUIRED_GL_MINOR_VERSION);
...
 DCF(ogl_anisotropy, "toggles anisotropic filtering") 
{
    ...
    
    {
         ...
         && !Is_Extension_Enabled(OGL_EXT_TEXTURE_FILTER_ANISOTROPIC))
