...
const geometry_batcher & geometry_batcher::
operator=(const geometry_batcher & geo)
{
  if (this != &geo)
    {
      clone(geo);
...
3----2
*/
void geometry_batcher::draw_bitmap(vertex * pnt, int orient, float rad,
                                   float depth)
{
...
void geometry_batcher::draw_beam(vec3d * start, vec3d * end, float width,
                                 float intensity)
{
  vec3d p[4];
  vertex *P = &vert[n_to_render * 3];
...
float geometry_batcher::draw_laser(vec3d * p0, float width1, vec3d * p1,
...
float batch_add_laser(int texture, vec3d * p0, float width1, vec3d * p1,
                      float width2, int r, int g, int b)
{
  if (texture < 0)
    {
...
int batch_add_bitmap(int texture, int tmap_flags, vertex * pnt, int orient,
                     float rad, float alpha, float depth)
{
  if (texture < 0)
    {
      Int3();
