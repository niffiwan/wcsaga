...
public:
    opengl_texture_state():active_texture_unit(0),
    shader_mode(GL_FALSE),
    units(NULL), Current_texture_source(TEXTURE_SOURCE_NONE)
  {
  }
...
inline void opengl_texture_state::SetEnvCombineMode(GLenum cmode,
                                                    GLenum cfunc)
{
...
{
  glTexParameteri(units[active_texture_unit].texture_target,
...
void opengl_setup_render_states(int &r, int &g, int &b, int &alpha,
                                int &tmap_type, int flags, int is_scaler = 0);
