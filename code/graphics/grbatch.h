...
public:
    geometry_batcher():n_to_render(0),
    n_allocated(0), vert(NULL), vert_list(NULL)
  {
  };
  ~geometry_batcher();
  ...
  void draw_bitmap(vertex * position, int orient, float rad, float angle,
                   float depth);
  ...
  // draw a beam
  void draw_beam(vec3d * start, vec3d * end, float width, float intensity =
                 1.0f);

  //draw a laser
  float draw_laser(vec3d * p0, float width1, vec3d * p1, float width2, int r,
                   int g, int b);
...
float batch_add_laser(int texture, vec3d * p0, float width1, vec3d * p1,
                      float width2, int r = 255, int g = 255, int b = 255);
int batch_add_bitmap(int texture, int tmap_flags, vertex * pnt, int orient,
                     float rad, float alpha = 1.0f, float depth = 0.0f);
