...
  static inline int is_power_of_two(int w, int h) 
{
    ...
    return (((w >= GL_min_texture_width) && !(w & (w - 1)))
...
int gr_opengl_bm_load(ubyte type, int n, char *filename, CFILE * img_cfp,
                      int *w, int *h, int *bpp, ubyte * c_type, int *mm_lvl,
                      int *size) 
{
    ...
    
    {
        ...
        
        {
          ...
          Error(LOCATION, "Bad DDS file compression! Not using DXT1,3,5: %s",
                 filename);
...
extern int opengl_compress_image(ubyte ** out_data, ubyte * in_data,
                                  int width, int height, int alpha =
                                  1, int num_mipmaps = 1);
 static int opengl_bm_lock_ani_compress(int handle, int bitmapnum,
                                          bitmap_entry * be, bitmap * bmp,
                                          ubyte bpp, ubyte flags) 
{
    ...
    
    {
              ...
              "Attempting to compress '%s' with %d frames, original size %.3fM ... ",
              bm_bitmaps[first_frame].filename, nframes,
              ((float) (size * nframes) / 1024.0f) / 1024.0f));
    ...
    
    {
        ...
        anim_get_next_raw_buffer(the_anim_instance, 0, 0, bmp->bpp);
       Assert(frame_data != NULL);
       out_size =
        opengl_compress_image(&compressed_data, frame_data, bmp->w, bmp->h,
                              alpha, num_mipmaps);
        ...
        
        {
            ...
            anim_get_next_raw_buffer(the_anim_instance, 0, 0, bm->bpp);
...
 static int opengl_bm_lock_compress(int handle, int bitmapnum,
                                      bitmap_entry * be, bitmap * bmp,
                                      int bpp, int flags) 
{
    ...
    
    {
              ...
              "Attempting to compress '%s', original size %.3fM ... ",
              be->filename,
              ((float) (bmp->w * bmp->h * byte_size) / 1024.0f) / 1024.0f));
    ...
    out_size =
    opengl_compress_image(&compressed_data, data, bmp->w, bmp->h, alpha,
                          num_mipmaps);
...
int gr_opengl_bm_lock(char *filename, int handle, int bitmapnum, ubyte bpp,
                      ubyte flags) 
{
    ...
    
    {
      ...
       try_compress = (Cmdline_img2dds && Texture_compression_available
                         && is_power_of_two(bmp->w, bmp->h)
        ...
        
        {
            ...
            {
              if (!opengl_bm_lock_compress
                   (handle, bitmapnum, be, bmp, true_bpp, flags))
            ...
            {
              if (!opengl_bm_lock_ani_compress
                   (handle, bitmapnum, be, bmp, true_bpp, flags))
...
 void gr_opengl_bm_save_render_target(int n) 
{
    ...
    opengl_export_image(n, bmp->w, bmp->h, (bmp->true_bpp == 32),
                        be->num_mipmaps, (ubyte *) bmp->data);
    ...
    {
      dds_save_image(bmp->w, bmp->h, bmp->true_bpp, be->num_mipmaps,
                      (ubyte *) bmp->data, (bmp->flags & BMP_FLAG_CUBEMAP));
    }
}

 int gr_opengl_bm_make_render_target(int n, int *width, int *height,
                                       ubyte * bpp, int *mm_lvl, int flags) 
{
    ...
    
    {
         ...
         && !Is_Extension_Enabled(OGL_ARB_TEXTURE_CUBE_MAP))
    ...
    
    {
         ...
         (bm_bitmaps[n].handle, n, width, height, bpp, mm_lvl, flags))
