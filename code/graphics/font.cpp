          ...
          if ((letter2 >= 0) && (letter2 < Current_font->num_chars))
            {
              ...
              while ((k->c1 == (char) letter) && (k->c2 < (char) letter2)
      ...
      for (int i = 0; i < fnt->num_chars; i++)
        {
          ...
          fnt->char_data[i].byte_width =
            INTEL_INT(fnt->char_data[i].byte_width);
          fnt->char_data[i].offset = INTEL_INT(fnt->char_data[i].offset);
          fnt->char_data[i].kerning_entry =
            INTEL_INT(fnt->char_data[i].kerning_entry);
          fnt->char_data[i].user_data =
            INTEL_SHORT(fnt->char_data[i].user_data);
        }
    }
  else
    {
                     ...
                     size_t only_parse_first_font_size)
{
  ...
  while (required_string_either("#End", "$Font:"))
    {
      ...
      if (only_parse_first_font != NULL)
        {
          strcpy_s(only_parse_first_font, only_parse_first_font_size,
                   font_filename);
          return;
        }
      ...
      if (font_id < 0)
        {
          Warning(LOCATION, "Could not create font from typeface '%s'!",
                  font_filename);
        }
    }
  ...
  if (Num_fonts < 3)
    {
      Error(LOCATION, "There must be at least three fonts in %s!",
            (filename) ? filename : NOX("<default fonts.tbl>"));
    }
}
