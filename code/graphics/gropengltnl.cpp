...
#define DO_RENDER() {   \
  if (sbuffer)
  {
    \vglDrawRangeElements(GL_TRIANGLES, start, end, count, GL_UNSIGNED_SHORT,
                           sbuffer + start);
  \}
else if (ibuffer)
  {
    \vglDrawRangeElements(GL_TRIANGLES, start, end, count, GL_UNSIGNED_INT,
                           ibuffer + start);
  \}
else
  {
...
 static void opengl_render_pipeline_fixed(int start, int n_prim,
                                            ushort * sbuffer, uint * ibuffer,
                                            int flags);
 static void opengl_render_pipeline_program(int start, int n_prim,
                                              ushort * sbuffer,
                                              uint * ibuffer, int flags) 
{
    ...
    
    {
      ...
       gr_opengl_tcache_set(gr_screen.current_bitmap, tmap_type, &u_scale,
                              &v_scale, render_pass);
    ...
    
    {
      ...
       gr_opengl_tcache_set(GLOWMAP, tmap_type, &u_scale, &v_scale,
                              render_pass);
    ...
    
    {
      ...
       gr_opengl_tcache_set(SPECMAP, tmap_type, &u_scale, &v_scale,
                              render_pass);
        ...
        
        {
          ...
          sdr->set_uniformMatrix4f(opengl::main_shader::envMatrix,
                                    &GL_env_texture_matrix[0]);
          sdr->set_uniform(opengl::main_shader::sEnvmap, render_pass);
           vglClientActiveTextureARB(GL_TEXTURE0_ARB + render_pass);
           gr_opengl_tcache_set(ENVMAP, TCACHE_TYPE_CUBEMAP, &u_scale,
                                  &v_scale, render_pass);
    ...
    
    {
      ...
       gr_opengl_tcache_set(NORMMAP, tmap_type, &u_scale, &v_scale,
                              render_pass);
        ...
        
        {
          ...
           gr_opengl_tcache_set(HEIGHTMAP, tmap_type, &u_scale, &v_scale,
                                  render_pass);
                                            ...
                                            int flags) 
{
    ...
    
    {
        ...
        
        {
          ...
           gr_opengl_tcache_set(gr_screen.current_bitmap, tmap_type,
                                  &u_scale, &v_scale, render_pass);
    ...
    
    {
      ...
       gr_opengl_tcache_set(ENVMAP, TCACHE_TYPE_CUBEMAP, &u_scale, &v_scale,
                              render_pass);
...
void gr_opengl_render_buffer(int start, int n_prim, ushort * sbuffer,
                             uint * ibuffer, int flags) 
{
...
void gr_opengl_set_projection_matrix(float fov, float aspect, float z_near,
                                     float z_far) 
{
  ...
   glViewport(gr_screen.offset_x,
                (gr_screen.max_h - gr_screen.offset_y -
                 gr_screen.clip_height), gr_screen.clip_width,
  ...
   glFrustum(-clip_width, clip_width, -clip_height, clip_height, z_near,
               z_far);
...
 void gr_opengl_set_view_matrix(vec3d * pos, matrix * orient) 
{
    ...
    if (!memcmp(pos, &last_view_pos, sizeof(vec3d))
        && !memcmp(orient, &last_view_orient, sizeof(matrix)))
...
 void gr_opengl_start_clip_plane() 
{
    ...
    (GLdouble) (G3_user_clip_normal.xyz.x *
                G3_user_clip_point.xyz.x)  +(GLdouble) (G3_user_clip_normal.
                                                         xyz.y *
                                                         G3_user_clip_point.
                                                         xyz.
                                                         y) 
