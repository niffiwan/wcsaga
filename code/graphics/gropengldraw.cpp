...
}  void opengl_aabitmap_ex_internal(int x, int y, int w, int h, int sx,
                                       int sy, bool resize, bool mirror) 
{
    ...
    
    {
         ...
         (gr_screen.current_bitmap, TCACHE_TYPE_AABITMAP, &u_scale, &v_scale))
                                 ...
                                 bool resize, bool mirror) 
{
  ...
  do
    ...
    opengl_aabitmap_ex_internal(dx1, dy1, (dx2 - dx1 + 1), (dy2 - dy1 + 1),
                                sx, sy, resize, mirror);
...
  void gr_opengl_string(int sx, int sy, char *s, bool resize) 
{
    ...
    {
      glColor3ub(gr_screen.current_color.red, gr_screen.current_color.green,
                  gr_screen.current_color.blue);
    ...
    
    {
        ...
        xc + ((do_resize) ? gr_screen.offset_x_unscaled : gr_screen.offset_x);
      y1 =
        yc + ((do_resize) ? gr_screen.offset_y_unscaled : gr_screen.offset_y);
...
 void gr_opengl_line(int x1, int y1, int x2, int y2, bool resize) 
{
    ...
    ((do_resize) ? gr_screen.offset_y_unscaled : gr_screen.offset_y);
    INT_CLIPLINE(x1, y1, x2, y2, clip_left, clip_top, clip_right,
                    clip_bottom, return, clipped = 1, swapped = 1);
...
 void gr_opengl_line_htl(vec3d * start, vec3d * end) 
{
  ...
   gr_zbuffer_type zbuffer_state =
    (gr_zbuffering) ? ZBUFFER_TYPE_FULL : ZBUFFER_TYPE_NONE;
...
 void gr_opengl_aaline(vertex * v1, vertex * v2) 
{
  ...
    FL_CLIPLINE(x1, y1, x2, y2, (float) gr_screen.clip_left,
                   (float) gr_screen.clip_top, (float) gr_screen.clip_right,
                   (float) gr_screen.clip_bottom, return, clipped =
                   1, swapped = 1);
...
 void gr_opengl_gradient(int x1, int y1, int x2, int y2, bool resize) 
{
    ...
    
    {
                  ...
                  gr_screen.clip_right, gr_screen.clip_bottom, return,
                  clipped = 1, swapped = 1);
    ...
    
    {
        ...
        
        {
              ...
              gr_screen.current_color.blue, ba);
  glVertex2f(sx2, sy2);
   glColor4ub(gr_screen.current_color.red, gr_screen.current_color.green,
                gr_screen.current_color.blue, aa);
...
 void opengl_draw_primitive(int nv, vertex ** verts, uint flags,
                              float u_scale, float v_scale, int r, int g,
                              int b, int a, int override_primary = 0) 
{
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
              glColor4ub(gr_palette[pal * 3 + 0], gr_palette[pal * 3 + 1],
                          gr_palette[pal * 3 + 2], alpha);
...
 void opengl_tmapper_internal(int nv, vertex ** verts, uint flags,
                                int is_scaler = 0) 
{
    ...
    
    {
           ...
           (gr_screen.current_bitmap, tmap_type, &u_scale, &v_scale, stage))
        ...
        
        {
               ...
               (GLOWMAP, tmap_type, &u_scale, &v_scale, stage))
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                ((va->sx * 16.0f) +
                 ((float) gr_screen.offset_x * 16.0f)) / 16.0f;
              sy =
                ((va->sy * 16.0f) +
                 ((float) gr_screen.offset_y * 16.0f)) / 16.0f;
  ...
   opengl_draw_primitive(nv, verts, flags, u_scale, v_scale, r, g, b,
                           alpha);
...
void opengl_tmapper_internal3d(int nv, vertex ** verts, uint flags) 
{
    ...
    
    {
           ...
           (gr_screen.current_bitmap, tmap_type, &u_scale, &v_scale))
...
void gr_opengl_scaler(vertex * va, vertex * vb, bool bw_bitmap = false) 
{
  ...
  v1 = vb->v;
    ...
    {
      opengl_tmapper_internal(4, vl,
                               (TMAP_FLAG_TEXTURED | TMAP_FLAG_BW_TEXTURE),
                               1);
...
// cross fade
void gr_opengl_cross_fade(int bmap1, int bmap2, int x1, int y1, int x2,
                          int y2, float pct) 
{
  gr_set_bitmap(bmap1, GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL,
                 1.0f - pct);
...
}  void gr_opengl_shade(int x, int y, int w, int h, bool resize) 
{
  ...
   glColor4ub((GLubyte) gr_screen.current_shader.r,
                (GLubyte) gr_screen.current_shader.g,
                (GLubyte) gr_screen.current_shader.b,
                (GLubyte) gr_screen.current_shader.c);
...
}  void gr_opengl_fade_out(int instantaneous) 
{
    ...
    // Empty - DDOI
}  void opengl_bitmap_ex_internal(int x, int y, int w, int h, int sx,
                                    int sy, bool resize) 
{
    ...
    
    {
         ...
         (gr_screen.current_bitmap, TCACHE_TYPE_INTERFACE, &u_scale,
          &v_scale))
                         ...
                         bool resize) 
{
  ...
  do
    ...
    // We now have dx1,dy1 and dx2,dy2 and sx, sy all set validly within clip regions.
    opengl_bitmap_ex_internal(dx1, dy1, (dx2 - dx1 + 1), (dy2 - dy1 + 1), sx,
                              sy, resize);
...
void opengl_render_timer_bar(int colour, float x, float y, float w,
                              float h) 
{
