...
void opengl_aabitmap_ex_internal(int x, int y, int w, int h, int sx, int sy,
                           ...
                           bool resize, bool mirror)
{
  ...
  opengl_aabitmap_ex_internal(dx1, dy1, (dx2 - dx1 + 1), (dy2 - dy1 + 1), sx,
                              sy, resize, mirror);
}
...
void gr_opengl_line(int x1, int y1, int x2, int y2, bool resize)
{
  ...
  INT_CLIPLINE(x1, y1, x2, y2, clip_left, clip_top, clip_right, clip_bottom,
...
void gr_opengl_aaline(vertex * v1, vertex * v2)
{
  ...
  FL_CLIPLINE(x1, y1, x2, y2, (float) gr_screen.clip_left,
              ...
              (float) gr_screen.clip_bottom, return, clipped = 1, swapped =
              1);
...
void gr_opengl_gradient(int x1, int y1, int x2, int y2, bool resize)
{
  ...
  INT_CLIPLINE(x1, y1, x2, y2, gr_screen.clip_left, gr_screen.clip_top,
               gr_screen.clip_right, gr_screen.clip_bottom, return, clipped =
               1, swapped = 1);
...
void opengl_draw_primitive(int nv, vertex ** verts, uint flags, float u_scale,
                           float v_scale, int r, int g, int b, int a,
  ...
  if (!bw_bitmap)
  ...
  else
    {
      opengl_tmapper_internal(4, vl,
                              (TMAP_FLAG_TEXTURED | TMAP_FLAG_BW_TEXTURE), 1);
    }
}
...
void opengl_bitmap_ex_internal(int x, int y, int w, int h, int sx, int sy,
                               bool resize)
{
  if ((w < 1) || (h < 1))
    {
  ...
  if (!gr_opengl_tcache_set
      (gr_screen.current_bitmap, TCACHE_TYPE_INTERFACE, &u_scale, &v_scale))
    {
      return;
    }
