...
void opengl_tcache_get_adjusted_texture_size(int w_in, int h_in, int *w_out,
                                             int *h_out);
int opengl_create_texture_sub(int bitmap_handle, int bitmap_type, int bmap_w,
                              int bmap_h, int tex_w, int tex_h, ubyte * data =
                              NULL, tcache_slot_opengl * t =
                              NULL, int base_level = 0, int resize =
                              0, int reload = 0);
int opengl_create_texture(int bitmap_handle, int bitmap_type,
...
void opengl_tcache_init()
{
  ...
  if (Textures == NULL)
    {
      Textures =
        (tcache_slot_opengl *) vm_malloc_q(MAX_BITMAPS *
  ...
  if (Is_Extension_Enabled(OGL_EXT_TEXTURE_FILTER_ANISOTROPIC))
    {
      ...
      char *plevel =
        os_config_read_string(NULL, NOX("OGL_AnisotropicFilter"), NOX("0.0"));
  ...
  if ( /*t->bitmap_handle > -1 */ t->texture_id)
    {
      ...
      if ((t->bitmap_handle >= 0)
          && Tex_used_this_frame[t->bitmap_handle %
                                 MAX_BITMAPS] /*&& (t->bpp == 8) */ )
        {
          return 0;
        }
                                             ...
                                             int *h_out)
{
  ...
  if (Is_Extension_Enabled(OGL_ARB_TEXTURE_NON_POWER_OF_TWO)
      || (GL_texture_target == GL_TEXTURE_RECTANGLE_ARB))
    {
      *w_out = w_in;
      *h_out = h_in;
...
int opengl_create_texture_sub(int bitmap_handle, int bitmap_type, int bmap_w,
                              int bmap_h, int tex_w, int tex_h, ubyte * data,
                              tcache_slot_opengl * t, int base_level,
                              int resize, int reload)
{
  int ret_val = 1;
  int byte_mult = 0;
        ...
        if (block_size > 0)
          {
            ...
            if (!reload)
              {
                vglCompressedTexImage2D(GL_TEXTURE_2D, 0, intFormat, mipmap_w,
                                        mipmap_h, 0, dsize,
                                        bmp_data + doffset);
              }
            else
              {
                vglCompressedTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, mipmap_w,
                                           mipmap_h, intFormat, dsize,
                                           bmp_data + doffset);
              }
            ...
            for (i = 1; i < (mipmap_levels - base_level); i++)
              {
                ...
                if (!reload)
                  {
                    vglCompressedTexImage2D(GL_TEXTURE_2D, i, intFormat,
                                            mipmap_w, mipmap_h, 0, dsize,
                                            bmp_data + doffset);
                  }
                else
                  {
                    vglCompressedTexSubImage2D(GL_TEXTURE_2D, i, 0, 0,
                                               mipmap_w, mipmap_h, intFormat,
                                               dsize, bmp_data + doffset);
                  }
              }
          }
        else
    ...
    case TCACHE_TYPE_AABITMAP:
      {
        ...
        if (!reload
            || (GL_state.Texture.GetTarget() == GL_TEXTURE_RECTANGLE_ARB))
          {
            glTexImage2D(GL_state.Texture.GetTarget(), 0, GL_ALPHA, tex_w,
                         tex_h, 0, GL_ALPHA, GL_UNSIGNED_BYTE, texmem);
          }
        else
          {
            // faster anis
            glTexSubImage2D(GL_state.Texture.GetTarget(), 0, 0, 0, tex_w,
                            tex_h, GL_ALPHA, GL_UNSIGNED_BYTE, texmem);
          }
    ...
    case TCACHE_TYPE_INTERFACE:
      {
        ...
        if (!reload
            || (GL_state.Texture.GetTarget() == GL_TEXTURE_RECTANGLE_ARB))
          {
            glTexImage2D(GL_state.Texture.GetTarget(), 0, intFormat, mipmap_w,
                         mipmap_h, 0, glFormat, texFormat,
                         (resize) ? texmem : bmp_data);
          }
        else
          {
            // faster anis
            glTexSubImage2D(GL_state.Texture.GetTarget(), 0, 0, 0, mipmap_w,
                            mipmap_h, glFormat, texFormat,
                            (resize) ? texmem : bmp_data);
          }
            ...
            if (block_size > 0)
              {
                ...
                if (!reload)
                  {
                    vglCompressedTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X +
                                            i, 0, intFormat, mipmap_w,
                                            mipmap_h, 0, dsize,
                                            bmp_data + doffset);
                  }
                else
                  {
                    vglCompressedTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X
                                               + i, 0, 0, 0, mipmap_w,
                                               mipmap_h, intFormat, dsize,
                                               bmp_data + doffset);
                  }
                ...
                for (j = 1; j < mipmap_levels; j++)
                  {
                    ...
                    // size of data block (4x4)
                    dsize =
                      ((mipmap_h + 3) / 4) * ((mipmap_w +
                                               3) / 4) * block_size;

                    if (!reload)
                      {
                        vglCompressedTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X
                                                + i, j, intFormat, mipmap_w,
                                                mipmap_h, 0, dsize,
                                                bmp_data + doffset);
                      }
                    else
                      {
                        vglCompressedTexSubImage2D
                          (GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, j, 0, 0,
                           mipmap_w, mipmap_h, intFormat, dsize,
                           bmp_data + doffset);
                      }
                  }
              }
                ...
                if (!reload)
                  {
                    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0,
                                 intFormat, mipmap_w, mipmap_h, 0, glFormat,
                                 texFormat, bmp_data + doffset);
                  }
                else
                  {
                    // faster anis
                    glTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, 0,
                                    0, mipmap_w, mipmap_h, glFormat,
                                    texFormat, bmp_data + doffset);
                  }
                ...
                for (j = 1; j < mipmap_levels; j++)
                  {
                    ...
                    if (!reload)
                      {
                        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, j,
                                     intFormat, mipmap_w, mipmap_h, 0,
                                     glFormat, texFormat, bmp_data + doffset);
                      }
                    else
                      {
                        glTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, j,
                                        0, 0, mipmap_w, mipmap_h, glFormat,
                                        texFormat, bmp_data + doffset);
                      }
                  }
              }
    ...
    default:
      {
                ...
                for (i = 0; i < tex_w; i++)
                  {
                    for (k = 0; k < byte_mult; k++)
                      {
                        *texmemp++ =
                          bmp_data[(f2i(v) * bmap_w + f2i(utmp)) * byte_mult +
                                   k];
                      }
                    utmp += du;
                  }
        ...
        if (!reload)
          {
            glTexImage2D(GL_TEXTURE_2D, 0, intFormat, mipmap_w, mipmap_h, 0,
                         glFormat, texFormat,
          ...
          {
            // faster anis
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, mipmap_w, mipmap_h,
                            glFormat, texFormat,
                            (texmem != NULL) ? texmem : bmp_data + doffset);
          }
        ...
        for (i = 1; i < (mipmap_levels - base_level); i++)
          {
            ...
            if (!reload)
              {
                glTexImage2D(GL_TEXTURE_2D, i, intFormat, mipmap_w, mipmap_h,
                             0, glFormat, texFormat, bmp_data + doffset);
              }
            else
              {
                glTexSubImage2D(GL_TEXTURE_2D, i, 0, 0, mipmap_w, mipmap_h,
                                glFormat, texFormat, bmp_data + doffset);
              }
          }
  ...
  t->size =
    (dsize) ? ((doffset + dsize) - skip_size) : (tex_w * tex_h * byte_mult);
  t->w = (ushort) tex_w;
  t->h = (ushort) tex_h;
                          ...
                          tcache_slot_opengl * tslot)
{
  ...
  // call the helper
  int ret_val =
    opengl_create_texture_sub(bitmap_handle, bitmap_type, bmp->w, bmp->h,
                              final_w,
                              final_h, (ubyte *) bmp->data, tslot, base_level,
                              resize, reload);
...
int gr_opengl_tcache_set_internal(int bitmap_handle, int bitmap_type,
      ...
      if (t->mipmap_levels > 1)
        {
          ...
          GL_state.Texture.
            SetMinFilter((GL_mipmap_filter) ? GL_LINEAR_MIPMAP_LINEAR :
...
int gr_opengl_tcache_set(int bitmap_handle, int bitmap_type, float *u_scale,
                         float *v_scale, int stage)
{
  int rc = 0;
  ...
  if ((bitmap_type == TCACHE_TYPE_INTERFACE)
      && Is_Extension_Enabled(OGL_ARB_TEXTURE_RECTANGLE)
  ...
  rc =
    gr_opengl_tcache_set_internal(bitmap_handle, bitmap_type, u_scale,
                                  v_scale, stage);
...
int gr_opengl_preload(int bitmap_num, int is_aabitmap)
{
  ...
  retval =
    gr_opengl_tcache_set(bitmap_num,
                         (is_aabitmap) ? TCACHE_TYPE_AABITMAP :
                         TCACHE_TYPE_NORMAL, &u_scale, &v_scale);
...
int opengl_compress_image(ubyte ** compressed_data, ubyte * in_data,
                          int width, int height, int alpha, int num_mipmaps)
{
  Assert(in_data != NULL);
  ...
  glTexImage2D(GL_PROXY_TEXTURE_2D, 0, intFormat, width, height, 0, glFormat,
               texFormat, in_data);

  glGetTexLevelParameteriv(GL_PROXY_TEXTURE_2D, 0, GL_TEXTURE_COMPRESSED_ARB,
                           &compressed);
  ...
  if (compressed == GL_TRUE)
    {
      glGetTexLevelParameteriv(GL_PROXY_TEXTURE_2D, 0,
                               GL_TEXTURE_INTERNAL_FORMAT, &compressed);
    }
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, intFormat, width, height, 0, glFormat,
               texFormat, in_data);
  ...
  for (i = 0; i < num_mipmaps; i++)
    {
      glGetTexLevelParameteriv(GL_TEXTURE_2D, i,
                               GL_TEXTURE_COMPRESSED_IMAGE_SIZE_ARB,
                               &testing);
      vglGetCompressedTexImageARB(GL_TEXTURE_2D, i,
                                  out_data + compressed_size);
      compressed_size += testing;
    }
...
int opengl_export_image(int slot, int width, int height, int alpha,
                        int num_mipmaps, ubyte * image_data)
{
  ...
  for (int i = 0; i < faces; i++)
    {
      for (int j = 0; j < ts->mipmap_levels; j++)
        {
          glGetTexImage(target + i, j, (alpha) ? GL_BGRA : GL_BGR,
                        (alpha) ? GL_UNSIGNED_INT_8_8_8_8_REV :
...
struct fbo_t
{
  ...
  int is_static;
    ...
    fbo_t():renderbuffer_id(0),
    framebuffer_id(0),
    width(0), height(0), ref_count(0), working_slot(-1), is_static(0)
  {
  }
};
...
static fbo_t *opengl_get_fbo(int width, int height)
{
  ...
  for (uint i = 0; i < rt_size; i++)
    {
      if ((RenderTarget[i].width == width)
          && (RenderTarget[i].height == height))
        {
...
void opengl_kill_render_target(int slot)
{
  ...
  for (idx = 0; idx < RenderTarget.size(); idx++)
    {
      if ((RenderTarget[idx].width == ts->w)
          && (RenderTarget[idx].height == ts->h))
        {
          break;
        }
    }
...
int opengl_set_render_target(int slot, int face, int is_static)
{
  ...
  if (slot < 0)
    {
      ...
      vglFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT,
                                 GL_TEXTURE_2D, 0, 0);
  ...
  if (!vglIsFramebufferEXT(fbo->framebuffer_id)
      /*|| !vglIsRenderbufferEXT(fbo->renderbuffer_id) */ )
    {
      Int3();
      return 0;
    }
  ...
  if (ts->texture_target == GL_TEXTURE_CUBE_MAP)
  ...
  else
    {
      Assert(face <= 0);
      vglFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT,
                                 ts->texture_target, ts->texture_id, 0);
    }
...
int opengl_make_render_target(int handle, int slot, int *w, int *h,
                              ubyte * bpp, int *mm_lvl, int flags)
{
  ...
  if ((fbo != NULL) && (fbo->width == *w) && (fbo->height == *h))
    {
      ...
      Assert(vglIsFramebufferEXT(fbo->framebuffer_id)
             /*&& vglIsRenderbufferEXT(fbo->renderbuffer_id) */ );
      ...
      if (flags & BMP_FLAG_CUBEMAP)
        {
          ...
          for (int i = 0; i < 6; i++)
            {
              glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB8, *w,
                           *h, 0, GL_BGR, GL_UNSIGNED_BYTE, NULL);
            }
      ...
      else
        {
          ...
          glTexImage2D(GL_state.Texture.GetTarget(), 0, GL_RGB8, *w, *h, 0,
  ...
  if (flags & BMP_FLAG_CUBEMAP)
    {
      ...
      for (int i = 0; i < 6; i++)
        {
          glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB8, *w, *h,
  ...
  if (flags & BMP_FLAG_CUBEMAP)
    ...
    {
      vglFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT,
                                 GL_state.Texture.GetTarget(), ts->texture_id,
                                 0);
    }
...
namespace opengl
{
  ...
  texture *texture::create(int w, int h, format fmt)
  {
    ...
    switch (fmt)
      ...
      case (fmt_color):
        ...
        break;
      case (fmt_depth):
        glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32, t->width,
                     t->height, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_BYTE,
                     NULL);
        break;
      default:
  ...
  render_buffer::render_buffer(int w, int h):width(w), height(h)
  {
    ...
    vglRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_DEPTH_COMPONENT, width,
                              height);
  }
  ...
  render_target::render_target(int w, int h):depth_rb(NULL),
    depth_t(NULL), use_rb(false), target(0), width(w), height(h)
  {
    Assert(w > 0 && h > 0);
  ...
  void render_target::attach_texture(unsigned int slot, const texture * tex)
  {
    ...
    tex->bind();
    vglFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT,
                               GL_COLOR_ATTACHMENT0_EXT + slot, GL_TEXTURE_2D,
                               tex->get_id(), 0);
