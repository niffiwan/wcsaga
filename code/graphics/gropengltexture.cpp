...
int opengl_free_texture(tcache_slot_opengl * t);
                               ...
                               int bmap_h, int tex_w, int tex_h,
                               ubyte * data = NULL, tcache_slot_opengl * t =
...
 void opengl_tcache_get_adjusted_texture_size(int w_in, int h_in,
                                                int *w_out, int *h_out) 
{
    ...
    
    {
      ...
      
      {
          ...
          
          {
              ...
              {
                vglCompressedTexImage2D(GL_TEXTURE_2D, 0, intFormat,
      ...
      
      {
          ...
          {
            glTexImage2D(GL_state.Texture.GetTarget(), 0, intFormat,
                          mipmap_w, mipmap_h, 0, glFormat, texFormat,
          ...
          
          {
              ...
              
              {
                  ...
                  
                  {
                      ...
                      {
                        vglCompressedTexImage2D
                          (GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, j, intFormat,
                           mipmap_w, mipmap_h, 0, dsize, bmp_data + doffset);
                             ...
                             tcache_slot_opengl * tslot) 
{
  ...
  int ret_val =
    opengl_create_texture_sub(bitmap_handle, bitmap_type, bmp->w, bmp->h,
                              final_w, final_h, (ubyte *) bmp->data, tslot,
                              base_level, resize, reload);
...
 int gr_opengl_tcache_set(int bitmap_handle, int bitmap_type,
                            float *u_scale, float *v_scale, int stage) 
{
                             ...
                             int num_mipmaps) 
{
    ...
    glTexImage2D(GL_PROXY_TEXTURE_2D, 0, intFormat, width, height, 0,
                 glFormat, texFormat, in_data);
...
struct fbo_t 
{
  ...
  int working_slot;
    ...
    fbo_t():renderbuffer_id(0), framebuffer_id(0), width(0), height(0),
    ...
    
    {
        ...
        
        {
            ...
            
            {
                            ...
                            *w, *h, 0, GL_BGR, GL_UNSIGNED_BYTE, NULL);
  ...
  namespace opengl 
{
  ...
   render_buffer::render_buffer(int w, int h):width(w), height(h) 
  {
                               ...
                               height);
  }  SCP_multimap < std::pair < int, int >,
    render_buffer * >render_buffer::pool;
   SCP_multimap < std::pair < int, int >,
    render_target * >render_target::pool;
   render_target::render_target(int w, int h):width(w), height(h),
    depth_t(NULL), depth_rb(NULL), use_rb(false) 
  {
  ...
  }  void render_target::attach_texture(unsigned int slot,
                                           const texture * tex) 
  {
