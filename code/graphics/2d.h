...
struct poly_list 
{
  poly_list():n_prim(0), n_verts(0), vert(NULL), norm(NULL), tsb(NULL),
...
 extern void gr_set_palette(char *name, ubyte * palette,
...
__inline void gr_gradient(int x1, int y1, int x2, int y2, bool resize =
                          true) 
{
...
{
  return (*gr_screen.gf_tcache_set) (bitmap_id, bitmap_type, u_scale,
                                      v_scale, stage);
...
__inline int gr_bm_load(ubyte type, int n, char *filename, CFILE * img_cfp =
                        ...
                        0, ubyte * c_type = 0, int *mm_lvl = 0, int *size =
                        0) 
{
