...
{
  poly_list():n_prim(0),
    n_verts(0), vert(NULL), norm(NULL), tsb(NULL), currently_allocated(0)
  {
  }
   ~poly_list();
...
typedef struct screen
{
  ...
  int (*gf_bm_load) (ubyte type, int n, char *filename, CFILE * img_cfp,
                     int *w, int *h, int *bpp, ubyte * c_type, int *mm_lvl,
                     int *size);
  void (*gf_bm_init) (int n);
  void (*gf_bm_page_in_start) ();
  int (*gf_bm_lock) (char *filename, int handle, int bitmapnum, ubyte bpp,
                     ubyte flags);

  int (*gf_bm_make_render_target) (int n, int *width, int *height,
...
extern bool gr_init(int d_mode = GR_DEFAULT, int d_width =
                    GR_DEFAULT, int d_height = GR_DEFAULT, int d_depth =
                    GR_DEFAULT);
...
extern std::pair < int, int >gr_get_alignment_offset(bool useForClipping =
                                                     false);
...
extern void gr_set_palette(char *name, ubyte * palette, int restrict_to_128 =
                           0);
...
__inline void gr_set_clip(int x, int y, int w, int h, bool resize =
                          true, bool change_fov = true)
{
...
void gr_set_bitmap(int bitmap_num, int alphablend =
                   GR_ALPHABLEND_NONE, int bitbltmode =
                   GR_BITBLT_MODE_NORMAL, float alpha = 1.0f);
...
__inline void gr_aabitmap(int x, int y, bool resize = true, bool mirror =
                          false)
{
...
__inline void gr_aabitmap_ex(int x, int y, int w, int h, int sx, int sy,
...
__inline void gr_fog_set(int fog_mode, int r, int g, int b, float fog_near =
                         -1.0f, float fog_far = -1.0f)
{
...
__inline int gr_tcache_set(int bitmap_id, int bitmap_type, float *u_scale,
                           float *v_scale, int stage = 0)
{
...
__inline int gr_bm_load(ubyte type, int n, char *filename, CFILE * img_cfp =
                        NULL, int *w = 0, int *h = 0, int *bpp =
                        0, ubyte * c_type = 0, int *mm_lvl = 0, int *size = 0)
{
  return (*gr_screen.gf_bm_load) (type, n, filename, img_cfp, w, h, bpp,
                                  c_type, mm_lvl, size);
}
...
__inline void gr_render_buffer(int start, int n_prim, ushort * sbuffer,
                               uint * ibuffer = NULL, int flags =
                               TMAP_FLAG_TEXTURED)
{
...
void gr_init_alphacolor(color * clr, int r, int g, int b, int alpha,
