...
{
  float offsetLeft =
    ((gr_screen.max_w_unscaled * Gr_resize_X) -
     (gr_screen.max_w_unscaled * Gr_resize_Y)) * 0.5f;
  ...
  return std::make_pair(static_cast < int >(offsetLeft + 0.5f),
                        static_cast < int >(offsetRight + 0.5f));
}
...
void gr_set_palette_internal(char *name, ubyte * palette,
  ...
  if (Gr_inited)
    {
      if (gr_screen.gf_set_palette)
        {
          (*gr_screen.gf_set_palette) (Gr_current_palette,
                                       restrict_font_to_128);
...
void gr_screen_resize(int width, int height)
{
  ...
  if (gr_screen.custom_size)
    {
      gr_unsize_screen_pos(&gr_screen.max_w_unscaled,
...
bool gr_init(int d_mode, int d_width, int d_height, int d_depth)
{
  ...
  bool has_sparky_hi = (cf_exists_full("2_ChoosePilot-m.pcx", CF_TYPE_ANY)
                        && cf_exists_full("2_TechShipData-m.pcx",
                                          CF_TYPE_ANY));
  ...
  if (dst->screen_sig != gr_screen.signature)
    {
      if (dst->is_alphacolor)
        gr_init_alphacolor(dst, dst->red, dst->green, dst->blue, dst->alpha,
                           dst->ac_type);
      else
  ...
  for (int i = 0; i < n_bm; i++)
    {
      ...
      if ((l->w <= 0) || (l->h <= 0))
        {
          bm_get_info(gr_screen.current_bitmap, &l->w, &l->h, NULL, NULL,
                      NULL);
        }
      ...
      if (idx > 0)
        {
          ...
          g3_draw_poly_constant_sw(3, verts,
                                   TMAP_FLAG_GOURAUD | TMAP_FLAG_RGB, 0.1f);
        }
...
inline bool same_vert(vertex * v1, vertex * v2, vec3d * n1, vec3d * n2)
{
  return ((v1->x == v2->x) &&
          ...
          (v1->v == v2->v) &&
          (n1->xyz.x == n2->xyz.x) &&
          (n1->xyz.y == n2->xyz.y) && (n1->xyz.z == n2->xyz.z));
}
...
void gr_set_bitmap(int bitmap_num, int alphablend_mode, int bitblt_mode,
                   float alpha)
{
...
KeepAspectRatio::KeepAspectRatio(bool keep):previousValue(Keeping_aspect_ratio)
{
...
ActivateAlignment::ActivateAlignment(Alignment newAlignment):previousAlignment
  (Active_alignment)
{
