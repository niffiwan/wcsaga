...
void opengl_set_light(int light_num, opengl_light * ltp) 
{
  ...
  glLightf(GL_LIGHT0 + light_num, GL_CONSTANT_ATTENUATION,
            ltp->ConstantAtten);
  glLightf(GL_LIGHT0 + light_num, GL_LINEAR_ATTENUATION, ltp->LinearAtten);
  glLightf(GL_LIGHT0 + light_num, GL_QUADRATIC_ATTENUATION,
            ltp->QuadraticAtten);
...
 void opengl_pre_render_init_lights() 
{
    ...
    qsort(opengl_lights, Num_active_gl_lights, sizeof(opengl_light),
          opengl_sort_active_lights);
...
 void opengl_change_active_lights(int pos, int d_offset) 
{
  ...
   if (!memcmp(&Eye_position, &last_view_pos, sizeof(vec3d))
         && !memcmp(&Eye_matrix, &last_view_orient, sizeof(matrix)))
...
}  void gr_opengl_set_center_alpha(int type) 
{
    ...
    // first light
    memcpy(&opengl_lights[Num_active_gl_lights], &glight,
           sizeof(opengl_light));
...
 void opengl_light_init() 
{
    ...
    Verify(GL_max_lights > 0);
      ...
      (opengl_light *) vm_malloc_q(MAX_LIGHTS * sizeof(opengl_light));
...
 void gr_opengl_set_lighting(bool set, bool state) 
{
    ...
    
    {
        ...
        { gr_screen.current_alpha, gr_screen.current_alpha,
gr_screen.current_alpha, gr_screen.current_alpha };
