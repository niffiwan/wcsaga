...
void opengl_texture_state::init(GLuint n_units)
{
  Assert(n_units > 0);
  units =
    (opengl_texture_unit *) vm_malloc(n_units * sizeof(opengl_texture_unit));
...
void opengl_texture_state::default_values(GLint unit, GLenum target)
{
  ...
  glTexParameteri((target == GL_INVALID_ENUM) ? GL_TEXTURE_2D : target,
...
void opengl_texture_state::Enable(GLuint tex_id)
{
  ...
  if (!shader_mode
      && (active_texture_unit < (uint) GL_supported_texture_units))
    {
      ...
      if (Is_Extension_Enabled(OGL_EXT_TEXTURE_FILTER_ANISOTROPIC))
        {
          ...
          CLAMP(units[active_texture_unit].aniso_filter, 1.0f,
                GL_max_anisotropy);

          glTexParameterf(units[active_texture_unit].texture_target,
...
{
  Assert((num >= 0)
         || (num < (int) (sizeof(clipplane_Status) / sizeof(GLboolean))));
...
void opengl_state::SetAlphaBlendMode(gr_alpha_blend ab)
{
  ...
  switch (ab)
    ...
    case ALPHA_BLEND_ALPHA_BLEND_SRC_COLOR:
      GL_state.BlendFunc( /*GL_SRC_COLOR */ GL_SRC_ALPHA,
...
void opengl_setup_render_states(int &r, int &g, int &b, int &alpha,
                                int &tmap_type, int flags, int is_scaler)
{
  ...
  if (gr_screen.current_alphablend_mode == GR_ALPHABLEND_FILTER)
    {
      if ((gr_screen.current_bitmap >= 0)
          && bm_has_alpha_channel(gr_screen.current_bitmap))
        {
