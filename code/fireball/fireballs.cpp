...
 void fireball_parse_tbl() 
{
    ...
    
    {
        ...
        {
          strcpy_s(Fireball_info[i].lod[0].filename,
                    LOD_checker[i].filename);
    ...
    
    {
        ...
        if ((LOD_checker[i].override >= 0)
            && (LOD_checker[i].override < Num_fireball_types))
        
        {
          strcpy_s(Fireball_info[LOD_checker[i].override].lod[0].filename,
                    LOD_checker[i].filename);
          Fireball_info[LOD_checker[i].override].lod_count =
            LOD_checker[i].num_lods;
           if (LOD_color[i].alpha == 255)
            
            {
              Fireball_info[LOD_checker[i].override].exp_color[0] =
                (LOD_color[i].red / 255.0f);
              Fireball_info[LOD_checker[i].override].exp_color[1] =
                (LOD_color[i].green / 255.0f);
              Fireball_info[LOD_checker[i].override].exp_color[2] =
    ...
    
    {
        ...
        {
          sprintf(Fireball_info[i].lod[j].filename, "%s_%d",
...
  void fireball_load_data() 
{
    ...
    
    {
        ...
        
        {
          ...
           fd->lod[idx].bitmap_id =
            bm_load_animation(fd->lod[idx].filename, &fd->lod[idx].num_frames,
                              &fd->lod[idx].fps, NULL, 1);
     ...
      MONITOR(NumFireballsRend)  void fireball_render(object * obj) 
{
    ...
    
    {
      ...
      g3_draw_bitmap(&p, fb->orient, obj->radius,
        ...
        gr_set_bitmap(Fireballs[num].current_bitmap, GR_ALPHABLEND_FILTER,
                      GR_BITBLT_MODE_NORMAL, 1.3f);
      g3_draw_rotated_bitmap(&p, (i2fl(fb->orient) * PI) / 180.0f,
                              obj->radius,
                              TMAP_FLAG_TEXTURED | TMAP_HTL_3D_UNLIT);
      ...
      
      {
        ...
        float t = fb->time_elapsed;
          ...
          
          {
              ...
              (float) pow((fb->total_time - t) / WARPHOLE_GROW_TIME,
                          0.4f) * obj->radius;
          ...
          //rad = obj->radius;
          //              obj->alt_rad = rad;
           warpin_render(obj, &obj->orient, &obj->pos,
                           Fireballs[num].current_bitmap, rad, percent_life,
...
 void fireball_set_framenum(int num) 
{
    ...
    
    {
        ...
        fl2i(fb->time_elapsed / fb->total_time * fl->num_frames + 0.5);
...
int fireball_create(vec3d * pos, int fireball_type, int render_type,
                    int parent_obj, float size, int reverse, vec3d * velocity,
                    float warp_lifetime, int ship_class,
                    matrix * orient_override, int low_res, int extra_flags,
                    int warp_open_sound, int warp_close_sound) 
{
    ...
    obj_create(OBJ_FIREBALL, parent_obj, n, &orient, pos, size, OF_RENDERS);
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
               bm_page_in_texture(fd->lod[idx].bitmap_id,
...
 int fireball_ship_explosion_type(ship_info * sip) 
{
    ...
    
    {
        ...
        Ship_types[sip->class_type].explosion_bitmap_anims[rand() %
                                                           objecttype_fireballs];
