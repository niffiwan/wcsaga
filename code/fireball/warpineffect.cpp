...
  void draw_face(vertex * v1, vertex * v2, vertex * v3) 
{
  ...
   vm_vec_perp(&norm, (vec3d *) & v1->x, (vec3d *) & v2->x,
                 (vec3d *) & v3->x);
...
 void warpin_render(object * obj, matrix * orient, vec3d * pos,
                      int texture_bitmap_num, float radius,
                      float life_percent, float max_radius, int warp_3d) 
{
  ...
   vm_vec_scale_add(&center, pos, &orient->vec.fvec,
                      -(max_radius / 2.5f) / 3.0f);
...
#define IN_PERCENT2 0.20f
        ...
        
        {
            ...
            
            {
                ...
                (OUT_PERCENT2 - life_percent) / (OUT_PERCENT2 - OUT_PERCENT1);
        ...
        
        {
          ...
          int noise_frame = fl2i(Missiontime / 15.0f) % NOISE_NUM_FRAMES;
            ...
            
            {
                ...
                powf((2.0f * life_percent) - 1.0f, 24.0f) * max_radius * 1.5f;
          ...
          verts[4].v = 0.5f;
            ...
            (The_mission.flags & MISSION_FLAG_FULLNEB) ? (1.0f -
                                                          neb2_get_fog_intensity
                                                          (obj)) : 1.0f;
          gr_set_bitmap(Warp_glow_bitmap, GR_ALPHABLEND_FILTER,
                         GR_BITBLT_MODE_NORMAL, alpha);
           g3_draw_bitmap(&verts[4], 0, r,
    ...
    
    {
      ...
      model_set_warp_globals(scale, scale, scale, texture_bitmap_num,
                              (radius / max_radius));
       float dist = vm_vec_dist_quick(pos, &Eye_position);
      model_set_detail_level((int) (dist / (radius * 10.0f)));
       model_render(Warp_model, orient, pos,
                      MR_LOCK_DETAIL | MR_NO_LIGHTING | MR_NORMAL |
                      MR_NO_FOGGING | MR_NO_CULL);
    ...
    {
      flash_ball warp_ball(20, .1f, .25f, &vmd_z_vector, &vmd_zero_vector,
                            4.0f, 0.5f);
        ...
        
        {
          ...
           gr_set_bitmap(Warp_ball_bitmap, GR_ALPHABLEND_FILTER,
                           GR_BITBLT_MODE_NORMAL, 0.9999f);
           warp_ball.render(max_radius * pct * 0.5f, adg * adg,
                              adg * adg * 6.0f);
