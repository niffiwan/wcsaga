...
void ai_big_chase_attack(ai_info * aip, ship_info * sip, vec3d * enemy_pos,
...
 extern int model_which_octant_distant_many(vec3d * pnt, int model_num,
                                              matrix * model_orient,
                                              vec3d * model_pos,
                                              polymodel ** pm, int *octs);
extern void compute_desired_rvec(vec3d * rvec, vec3d * goal_pos,
                                  vec3d * cur_pos);
...
void ai_bpap(object * objp, vec3d * attacker_objp_pos,
             vec3d * attacker_objp_fvec, vec3d * attack_point,
             vec3d * local_attack_point, float fov, float weapon_travel_dist,
             vec3d * surface_normal, ship_subsys * ss) 
{
  ...
   model_which_octant_distant_many(attacker_objp_pos, sip->model_num,
                                     &objp->orient, &objp->pos, &pm, octs);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                vm_vec_normalized_dir(&v2p, &result_point, attacker_objp_pos);
...
void ai_big_pick_attack_point_turret(object * objp, ship_subsys * ssp,
                                     vec3d * gpos, vec3d * gvec,
                                     vec3d * attack_point, float fov,
                              ...
                              vec3d * attack_point, float fov) 
{
    ...
    
    {
      ...
      
      {
          ...
          
          {
            ...
             vm_vec_unrotate(&result_point, &wp->big_attack_point,
                               &objp->orient);
  ...
  ai_bpap(objp, &attacker_objp->pos, &attacker_objp->orient.vec.fvec,
...
void ai_big_evade_ship() 
{
    ...
    get_world_closest_box_point_with_delta(&box_vec, En_objp, &player_pos,
                                           &is_inside,
...
int ai_big_maybe_follow_subsys_path(int do_dot_check) 
{
    ...
    
    {
        ...
        model_get(Ship_info[Ships[Pl_objp->instance].ship_info_index].
                  model_num);
        ...
        
        {
            ...
            ep =
            &(pm->view_positions[Ships[Pl_objp->instance].current_viewpoint]);
          model_find_world_point(&geye, &ep->pnt, pm->id, 0,
        ...
        min_dot =
        (target_objp->phys_info.fspeed >
         5.0f ? MIN_DOT_TO_ATTACK_MOVING_SUBSYS : MIN_DOT_TO_ATTACK_SUBSYS);
        ...
        
        {
            ...
            
            {
                ...
                
                {
                  ...
                  ai_find_path(Pl_objp, aip->target_objnum, subsys_path_num,
                                0, 1);
        ...
        
        {
            ...
            vm_vec_dist_quick(&Path_points
                              [aip->path_start + aip->path_length - 2].pos,
                              &Pl_objp->pos);
            ...
            (target_objp->phys_info.fspeed >
             5.0f ? MIN_DOT_TO_ATTACK_MOVING_SUBSYS :
...
int ai_big_maybe_start_strafe(ai_info * aip, ship_info * sip) 
{
    ...
    
    {
        ...
        
        {
          ...
           aip->scan_for_enemy_timestamp =
            timestamp(SCAN_FIGHTERS_INTERVAL);
            ...
            
            {
                ...
                {
                  if (Ship_info[test_sp->ship_info_index].
                       flags & SIF_SMALL_SHIP)
                    
                    {
                      dist_squared =
                        vm_vec_dist_squared(&Pl_objp->pos, &test_objp->pos);
                         ...
                         float dist_to_enemy, int modelnum) 
{
    ...
    
    {
                  ...
                  
                  {
                      ...
                      
                      {
                          ...
                          
                          {
                                 ...
                                 (vm_vec_dist_quick
                                  (&objp->pos,
                                   &Objects[objp->parent].pos) < 300.0f))
        ...
        vm_vec_normalized_dir(&vec_to_enemy, enemy_pos, &Pl_objp->pos);
                                ...
                                sip->srotation_time, NULL, rel_pos, 0.0f, 0,
                                rvec);
            ...
            
            {
                ...
                accelerate_ship(aip,
                                MAX(1.0f - dot_to_enemy,
                                    Objects[aip->target_objnum].phys_info.
                                    fspeed / sip->max_speed));
...
void ai_big_chase_ct() 
{
                                       ...
                                       weapon_info * swip, bool burst);
                                        ...
                                        object * en_objp);
extern int maybe_avoid_big_ship(object * objp, object * ignore_objp,
                                 ai_info * aip, vec3d * goal_point,
                                 float delta_time);
...
//
void ai_big_maybe_fire_weapons(float dist_to_enemy, float dot_to_enemy,
                               vec3d * firing_pos, vec3d * enemy_pos,
                               vec3d * enemy_vel) 
{
    ...
    
    {
        ...
        if (dot_to_enemy >
            MAX(0.5f,
                0.90f + aip->ai_accuracy / 10.0f - En_objp->radius / MAX(1.0f,
                                                                         dist_to_enemy)))
                          ...
                          extern int Nebula_sec_range;
                            ...
                            
                            {
                                ...
                                
                                {
                                        ...
                                        ai_flags & AIF_UNLOAD_SECONDARIES)
                                       || (swip->
                                           burst_flags & WBF_FAST_FIRING))
...
void ai_big_chase() 
{
    ...
    {
      set_predicted_enemy_pos(&predicted_enemy_pos, Pl_objp, &En_objp->pos,
  ...
  dot_to_enemy = vm_vec_dot(&vec_to_enemy, &Pl_objp->orient.vec.fvec);
    ...
    {
      update_aspect_lock_information(aip, &vec_to_enemy,
                                      dist_to_enemy - En_objp->radius,
                                      En_objp->radius);
  ...
  else if (En_objp->flags & OF_PROTECTED)       //  If protected and we're not attacking a subsystem, stop attacking!
    {
           ...
           (Pl_objp - Objects, MAX_ENEMY_DISTANCE,
    ...
    
    {
      ...
      float dist_normal_to_enemy;
        ...
        
        {
            ...
            fl_abs((dist_to_enemy *
                    vm_vec_dot(&vec_to_enemy,
        ...
        
        {
          ...
          float time_to_enemy;
            ...
            MAX(0.0f, (Pl_objp->phys_info.speed - 50) * 2);
            ...
            
            {
                ...
                //              accelerate_ship(aip, -1.0f);
                big_ship_collide_recover_start(Pl_objp, En_objp,
                                               &Pl_objp->pos, NULL);
    ...
    
    {
        ...
        // DA 4/20/98  can never get here attacking a big ship
      {
          ...
          get_world_closest_box_point_with_delta(NULL, En_objp, &Pl_objp->pos,
                                                 &is_inside, 0.0f);
...
void ai_big_attack_get_data(vec3d * enemy_pos, float *dist_to_enemy,
                            float *dot_to_enemy) 
{
    ...
    vm_vec_scale_add2(&player_pos, &Pl_objp->orient.vec.fvec,
                      Pl_objp->radius);
    ...
    
    {
      ...
       set_predicted_enemy_pos_turret(&predicted_enemy_pos, &gun_pos,
                                        Pl_objp, enemy_pos,
                                        &En_objp->phys_info.vel, weapon_speed,
                                        aip->time_enemy_in_range);
    }  *dist_to_enemy =
    vm_vec_normalized_dir(&vec_to_enemy, &predicted_enemy_pos, &player_pos);
...
int ai_big_strafe_maybe_retreat(float dist, vec3d * target_pos) 
{
    ...
    
    {
        ...
        -vm_vec_dotprod(&vec_to_target, &aip->big_attack_surface_normal);
    ...
    MAX(0.2f * dist_to_target, dist_normal_to_target);
  ...
  float speed_to_dist_penalty = MAX(0.0f, (Pl_objp->phys_info.speed - 50));
    ...
    
    {
        ...
        
        {
            ...
            get_world_closest_box_point_with_delta(&goal_point, En_objp,
                                                   &Pl_objp->pos, &is_inside,
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
              vm_vec_scale_add(&rand_vec, &Pl_objp->orient.vec.fvec,
...
void ai_big_strafe_glide_attack() 
{
  ...
  flight_dot_to_enemy = vm_vec_dot(&target_objp->pos, &norm_vel_vec);   //Angle between flight path and target ship
    ...
    
    {
      ...
      vm_vec_random_in_circle(&tangentPoint, &target_objp->pos, &orient,
                               target_objp->radius + GLIDE_STRAFE_DISTANCE,
                               1);
...
void ai_big_strafe_retreat1() 
{
    ...
    
    {
        ...
        
        {
          ...
          vm_vec_add(&rand_vec, &Pl_objp->orient.vec.fvec,
