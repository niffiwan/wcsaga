...
void ai_goal_purge_all_invalid_goals(ai_goal * aigp)
{
  ...
  if ((aigp->ai_mode & PURGE_GOALS_DISARM_DISABLE)
      &&
      (!(The_mission.ai_profile->flags & AIPF_GLOBAL_DISARM_DISABLE_EFFECTS)))
    return;
...
void ai_add_goal_sub_scripting(int type, int mode, int submode, int priority,
                               char *shipname, ai_goal * aigp)
{
  Assert((type == AIG_TYPE_PLAYER_WING) || (type == AIG_TYPE_PLAYER_SHIP));
...
void ai_process_mission_orders(int objnum, ai_info * aip)
{
  ...
  bool ship_is_idle_at_chase_any = (aip->goals[0].ai_mode == AI_GOAL_CHASE_ANY
                                    && aip->mode == AIM_NONE);

  //  If goal didn't change, return.
  if ((aip->active_goal != -1)
      && (aip->goals[0].signature == original_signature)
      && !ship_is_idle_at_chase_any)
    return;
