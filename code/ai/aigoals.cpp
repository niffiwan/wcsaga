...
void ai_maybe_add_form_goal(wing * wingp) 
{
    ...
    
    {
        ...
        
        {
            ...
            ai_add_ship_goal_player(AIG_TYPE_PLAYER_SHIP,
                                    AI_GOAL_FORM_ON_WING, -1, wing_leader,
                                    aip);
...
void ai_goal_purge_all_invalid_goals(ai_goal * aigp) 
{
    ...
    if (!
        ((aigp->ai_mode & PURGE_GOALS_ALL_SHIPS)
         || (aigp->ai_mode & PURGE_GOALS_DISARM_DISABLE)))
    return;
   if ((aigp->ai_mode & PURGE_GOALS_DISARM_DISABLE)
         &&
         (!(The_mission.ai_profile->
            flags & AIPF_GLOBAL_DISARM_DISABLE_EFFECTS)))
...
int ai_goal_find_dockpoint(int shipnum, int dock_type) 
{
    ...
    
    {
        ...
        model_find_dock_index(Ship_info[shipp->ship_info_index].model_num,
                              dock_type, dock_index + 1);
    ...
    if (loop_count >= 100)
             ...
             "Too many iterations while looking for a dockpoint on %s.  Either there was a bug or this is an übership.\n",
             shipp->ship_name);
   
    // if we're here, just return the first dockpoint
    return model_find_dock_index(Ship_info[shipp->ship_info_index].model_num,
                                 dock_type);
...
void ai_add_goal_sub_player(int type, int mode, int submode, char *shipname,
                             ai_goal * aigp) 
{
  ...
  aigp->ai_submode = submode;   // could mean different things depending on mode
    ...
    
    {
      ...
      ai_get_goal_ship_name(shipname, &aigp->ship_name_index);
...
  void ai_add_goal_sub_scripting(int type, int mode, int submode,
                                    int priority, char *shipname,
                                    ai_goal * aigp) 
{
...
 void ai_add_ship_goal_scripting(int mode, int submode, int priority,
                                   char *shipname, ai_info * aip) 
{
  ...
  ai_add_goal_sub_scripting(AIG_TYPE_PLAYER_SHIP, mode, submode, priority,
                             shipname, aigp);
...
void ai_add_ship_goal_player(int type, int mode, int submode, char *shipname,
                             ai_info * aip) 
{
...
// ships in the wing
void ai_add_wing_goal_player(int type, int mode, int submode, char *shipname,
                             int wingnum) 
{
                          ...
                          char *actor_name) 
{
    ...
    
    {
      ...
      
      {
        ...
        ai_get_goal_ship_name(CTEXT(CDR(node)), &aigp->ship_name_index);
        ...
        aigp->docker.name =
        ai_get_goal_ship_name(CTEXT(CDR(CDR(node))), &dummy);
    ...
    case OP_AI_WARP:
      ...
      aigp->wp_index = -1;
        ...
        if (CDR(CDR(node)) != -1)
          ...
          ai_get_goal_ship_name(CTEXT(CDR(CDR(node))),
 ...
 */ 
int ai_find_goal_index(ai_goal * aigp, int mode, int submode =
                       -1, int priority = -1) 
{
...
int ai_remove_goal_sexp_sub(int sexp, ai_goal * aigp) 
{
  ...
  char *op_text = NULL;
    ...
    
    {
        ...
        (CDR(CDR(CDR(node))) >= 0) ? atoi(CTEXT(CDR(CDR(CDR(node))))) : -1;
        ...
        (CDR(CDR(CDR(CDR(node)))) >=
         0) ? atoi(CTEXT(CDR(CDR(CDR(CDR(node)))))) : -1;
...
void ai_remove_wing_goal_sexp(int sexp, int wingnum) 
{
    ...
    
    {
        ...
        
        {
            ...
            ai_remove_goal_sexp_sub(sexp, Ai_info[Ships[num].ai_index].goals);
...
void ai_add_goal_ship_internal(ai_info * aip, int goal_type, char *name,
...
void ai_add_goal_wing_internal(wing * wingp, int goal_type, char *name,
                               int immediate) 
{
    ...
    
    {
        ...
        continue;
      ai_add_goal_ship_internal(&Ai_info[Ships[num].ai_index], goal_type,
                                 name, -1, -1, immediate);
...
int ai_mission_goal_achievable(int objnum, ai_goal * aigp) 
{
    ...
    
    {
      ...
      
      {
          ...
          mission_log_get_time(LOG_SHIP_SUBSYS_DESTROYED, aigp->ship_name,
                               ssp->system_info->subobj_name, NULL);
         break;
      }
     case AI_GOAL_DISABLE_SHIP:
      status =
        mission_log_get_time(LOG_SHIP_DISABLED, aigp->ship_name, NULL, NULL);
      break;
     case AI_GOAL_DISARM_SHIP:
      status =
        mission_log_get_time(LOG_SHIP_DISARMED, aigp->ship_name, NULL, NULL);
    ...
    case AI_GOAL_CHASE:
      ...
      
      {
          ...
          
          {
              ...
              mission_log_get_time(LOG_SHIP_DEPARTED, aigp->ship_name, NULL,
                                   NULL);
            if (!status)
              
              {
                status =
                  mission_log_get_time(LOG_SHIP_DESTROYED, aigp->ship_name,
                                       NULL, NULL);
      ...
      
      {
          ...
          mission_log_get_time(LOG_WING_DEPARTED, aigp->ship_name, NULL,
                               NULL);
        if (!status)
          
          {
            status =
              mission_log_get_time(LOG_WING_DESTROYED, aigp->ship_name, NULL,
                                   NULL);
    ...
    
    {
        ...
        if (Ships[ship_name_lookup(aigp->ship_name)].
            subsys_info[SUBSYSTEM_TURRET].num == 0)
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                ai_add_goal_ship_internal(aip, AI_GOAL_UNDOCK,
                                           Ships[obstacle_objp->instance].
                                           ship_name, -1, -1, 0);
    ...
    
    {
        ...
        
        {
            ...
            if (aip->goal_objnum !=
                Ships[ship_name_lookup(aigp->ship_name)].objnum)
    ...
    if (((aigp->ai_mode == AI_GOAL_IGNORE)
         || (aigp->ai_mode == AI_GOAL_IGNORE_NEW))
        && (status == SHIP_STATUS_ARRIVED))
    ...
    case AI_GOAL_REARM_REPAIR:
      
      {
          ...
          if (!
              (Ai_info[Ships[sindex].ai_index].
               ai_flags & AIF_AWAITING_REPAIR))
...
int ai_goal_priority_compare(const void *a, const void *b) 
{
  ...
  else if (!(ga->flags & AIGF_GOAL_ON_HOLD)
           && (gb->flags & AIGF_GOAL_ON_HOLD))
...
void prioritize_goals(int objnum, ai_info * aip) 
{
    ...
    insertion_sort(aip->goals, MAX_AI_GOALS, sizeof(ai_goal),
...
void ai_process_mission_orders(int objnum, ai_info * aip) 
{
    ...
    case AI_GOAL_WAYPOINTS_ONCE:
      ...
      
      {
        ...
        ai_dock_with_object(objp, current_goal->docker.index, other_obj,
    ...
    case AI_GOAL_DESTROY_SUBSYSTEM:
      ...
      ai_rearm_repair(objp, current_goal->docker.index, other_obj,
...
 void ai_update_goal_references(ai_goal * goals, int type, char *old_name,
                                  char *new_name) 
{
