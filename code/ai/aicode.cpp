  ...
  for (i = 0; i < (int) Ai_huge_fire_info.size(); i++)
    {
      ...
      if ((signature == target_signature) && (hfi->weapon_index == weapon_num)
          && (hfi->team == firer_team))
        break;
    }
  ...
  if (aip->ai_flags & AIF_FORMATION)
  ...
  else
    {
      ...
      ai_turn_towards_vector(point, objp, flFrametime,
                             ...
                             srotation_time, slide_vec, NULL, bank_override,
                             0);
    }
}
  ...
  if (attacked == NULL)
    {
      ...
      for (int max_attackers = 4;
           (objnum < 0) && (max_attackers < 2 * MAX_SHIPS);
           max_attackers *= 2)
        {
      ...
      if (The_mission.flags & MISSION_FLAG_USE_AP_CINEMATICS)
      ...
      else
        {
          ...
          ai_turn_towards_vector(&perp, Pl_objp, flFrametime,
                                 sip->srotation_time * 3.0f * scale, slop_vec,
                                 NULL, 0.0f, 0);
        }
    }
  else
              ...
              (MIN_DIST_TO_WAYPOINT_GOAL + fl_sqrt(Pl_objp->radius))))
        {
          ...
          if (treat_as_ship)
          ...
          else
            {
              ...
              ai_mission_wing_goal_complete(Ships[Pl_objp->instance].wingnum,
...
void ai_waypoints()
{
  ...
  if ((dist_to_goal < Pl_objp->radius * 0.5f)
      || (vm_vec_dist_quick(&Pl_objp->last_pos, &Pl_objp->pos) > 0.1f))
    {
      vec3d nearest_point;
      float r;
      ...
      if ((vm_vec_dist_quick(&Pl_objp->pos, wp_cur) <
           (Pl_objp->radius * 0.5f) + vm_vec_dist_quick(&Pl_objp->pos,
                                                        &Pl_objp->last_pos))
          || ((r >= 0.0f) && (r <= 1.0f))
      ...
      if (dot_to_enemy > 0.75f)
        {
          ...
          vm_vec_scale_add(&goal_point, &enemy_pos, &Pl_objp->orient.vec.rvec,
                           1000.0f);
          if (dist < 100.0f)
      ...
      if (aip->ai_evasion > myrand() * 100.0f / 32767.0f)
        {
          ...
          vm_vec_scale_add(&goal_point, &enemy_pos, &En_objp->orient.vec.rvec,
                           scale);
...
int ai_fire_primary_weapon(object * objp)
{
  ...
  float burstFireProb =
    ((0.6f * percentAmmoLeft) +
     (0.4f * distanceFactor)) * dotToTarget * aip->ai_primary_ammo_burst_mult;
...
void ai_select_secondary_weapon(object * objp, ship_weapon * swp,
                                int priority1 = -1, int priority2 =
                                -1, int wif2_priority1 =
                                -1, int wif2_priority2 = -1)
{
          ...
          if (Ships[A->instance].team == firing_ship_team)
            {
              ...
              if (!object_is_docked(A)
                  && !(aip->ai_flags & (AIF_REPAIRING | AIF_BEING_REPAIRED)))
                {
                  ...
                  if (percent_left > 30.0f + ((Pl_objp - Objects) & 0x0f))
                    {
                      ...
                      if (aip->
                          ...
                          AIPF_SMART_AFTERBURNER_MANAGEMENT)
                        {
                          ...
                          aip->afterburner_stop_time =
                            (fix) (Missiontime + F1_0 * time_to_enemy);
      ...
      if (Ship_info[Ships[big_objp->instance].ship_info_index].
          flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP))
        {
          ...
          if (will_collide_with_big_ship
              (objp, goal_point, big_objp, &cur_collision_point, delta_time))
            {
  ...
  if (num_weapon_types && (wip->wi_flags & WIF_LOCKED_HOMING))
    {
      ...
      if (dot_to_enemy > needed_dot &&
          (wip->wi_flags & WIF_HOMING_ASPECT ||
           ...
           (tshpp == NULL ||
            ship_get_closest_subsys_in_sight(tshpp, SUBSYSTEM_ENGINE,
                                             &aiobjp->pos))))
        {
                                ...
                                object * en_objp)
{
  ...
  int is_big_ship, wif_priority1, wif_priority2, wif2_priority1,
    wif2_priority2;
  ship_weapon *swp;
  ship_info *esip;
...
void ai_chase_big_approach_set_goal(vec3d * goal_pos, object * attack_objp,
...
void ai_chase_big_parallel_set_goal(vec3d * goal_pos, object * attack_objp,
                                    object * target_objp, float *accel)
{
  int opposing;
  float temp, r_target, r_attacker, h_attacker, h_target;
  ...
  vm_vec_scale_add(goal_pos, &target_objp->pos, &target_objp->orient.vec.fvec,
                   dist);
  ...
  if (Ship_info[Ships[attack_objp->instance].ship_info_index].max_vel.xyz.z !=
      0.0f)
    {
      match_accel =
                                  ...
                                  else
                                    {
                                      float secondary_range_mult =
                                        (aip->ai_secondary_range_mult ==
                                         FLT_MIN) ? (float) (Game_skill_level
                                                             + 1 +
                                                             (3 *
                                                              aip->ai_class /
                                                              (Num_ai_classes
                                  ...
                                  int dock_index)
{
  ...
  // use the static dockpoints
  else
    {
      ...
      vm_vec_unrotate(global_p0_norm, &pm->docking_bays[dock_index].norm[0],
                      &objp->orient);
    }
}
          ...
          if (attacking_aip->target_objnum == target_objnum)
            {
              ...
              bool isPlayerShip =
                (attacking_objp->flags & OF_PLAYER_SHIP) != 0;
              bool isChasing = attacking_aip->mode == AIM_CHASE;

              if ((isMultiplayer && isPlayerShip)
                  || (isChasing && !isPlayerShip))
                {
                  count++;
                }
            }
                  ...
                  if (aip->guard_objnum == hit_objp - Objects)
                    {
                      guard_object_was_hit(objp, hitter_objp);
                           ...
                           && (aip->guard_wingnum ==
                               Ai_info[Ships[hit_objp->instance].ai_index].
                               wing))
                    {
                      guard_object_was_hit(objp, hitter_objp);
                    }
                                   ...
                                   object * repair_objp, int how)
{
  ...
  switch (how)
    ...
    case REPAIR_INFO_KILLED:
      ...
      if ((repair_objp != NULL)
          && dock_check_find_direct_docked_object(repair_objp, repaired_objp))
        {
                      ...
                      if (percent_left > 30.0f + ((Pl_objp - Objects) & 0x0f))
                        {
                          ...
                          aip->afterburner_stop_time =
                            (fix) (Missiontime + F1_0 * time_to_goal);
                          aip->ai_flags |= AIF_CLOSING_DISTANCE_WITH_AB;
                        }
                    }
                }
            ...
            if (dist < 1.0f)
              {
                ...
                ai_do_objects_docked_stuff(Pl_objp, docker_index, goal_objp,
                                           dockee_index);
          ...
          if (psub->turret_num_firing_points > 0)
            {
              ...
              if ((pss->system_info->flags & MSS_FLAG_TURRET_RESET_IDLE)
                  || enemies_present == 1 || pss->turret_enemy_objnum >= 0)
  ...
  if (aip->ai_flags & AIF_BIG_SHIP_COLLIDE_RECOVER_1)
    {
      ai_turn_towards_vector(&aip->big_recover_pos_1, objp, flFrametime,
                             Ship_info[Ships[objp->instance].ship_info_index].
                             srotation_time, NULL, NULL, 0.0f, 0, NULL);
      ...
      if (aip->ai_flags & AIF_CLOSING_DISTANCE_WITH_AB)
        {
          ...
          if (aip->mode == AIM_STAY_NEAR)
            {
              object *goal_objp =
                &Objects[Ai_info[Ships[Pl_objp->instance].ai_index].
                         goal_objnum];
            ...
            {
              dist_to_enemy =
                vm_vec_dist(&Pl_objp->pos, &aip->last_predicted_enemy_pos);
          ...
          if (((aip->mode != AIM_CHASE) && (aip->mode != AIM_STAY_NEAR))
              || (dist_to_enemy < closing_dist)
...
void init_ai_object(int objnum)
{
  ...
  init_aip_from_class_and_profile(aip,
                                  &Ai_classes[Ship_info[ship_type].ai_class],
                                     ...
                                     ai_profile_t * profile)
{
  ...
  aip->ai_primary_ammo_burst_mult =
    (aicp->ai_primary_ammo_burst_mult[Game_skill_level] ==
     FLT_MIN) ? profile->primary_ammo_burst_mult[Game_skill_level] : aicp->
  ...
  aip->ai_link_energy_levels_maybe =
    (aicp->ai_link_energy_levels_maybe[Game_skill_level] ==
     FLT_MIN) ? profile->link_energy_levels_maybe[Game_skill_level] : aicp->
  ...
  aip->ai_link_energy_levels_always =
    (aicp->ai_link_energy_levels_always[Game_skill_level] ==
     FLT_MIN) ? profile->link_energy_levels_always[Game_skill_level] : aicp->
  ...
  aip->ai_predict_position_delay =
    (aicp->ai_predict_position_delay[Game_skill_level] ==
     INT_MIN) ? profile->predict_position_delay[Game_skill_level] : aicp->
    ai_predict_position_delay[Game_skill_level];
  aip->ai_shield_manage_delay =
    (aicp->ai_shield_manage_delay[Game_skill_level] ==
     FLT_MIN) ? profile->shield_manage_delay[Game_skill_level] : aicp->
  ...
  aip->ai_max_aim_update_delay =
    (aicp->ai_max_aim_update_delay[Game_skill_level] ==
     FLT_MIN) ? profile->max_aim_update_delay[Game_skill_level] : aicp->
  ...
  if (dead_aip->ai_flags & (AIF_AWAITING_REPAIR | AIF_BEING_REPAIRED))
    {
      if (dead_aip->support_ship_objnum >= 0)
        ai_do_objects_repairing_stuff(&Objects[objnum],
                                      &Objects[dead_aip->support_ship_objnum],
                                      REPAIR_INFO_END);
      else
