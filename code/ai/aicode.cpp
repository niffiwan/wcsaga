...
float dock_orient_and_approach(object * docker_objp, int docker_index,
                               object * dockee_objp, int dockee_index,
                               int dock_mode,
...
void ai_good_secondary_time(int team, int weapon_index, int max_fire_count,
                            char *shipname) 
{
...
int is_preferred_weapon(int weapon_num, object * firer_objp,
                        object * target_objp) 
{
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
               Assert((aip->path_cur >= 0)
                        && (aip->path_cur < MAX_PATH_POINTS));
...
 void parse_ai_class() 
{
    ...
    parse_float_list(aicp->ai_ship_fire_delay_scale_friendly,
                      NUM_SKILL_LEVELS);
   if (optional_string("$Hostile AI Fire Delay Scale:"))
    parse_float_list(aicp->ai_ship_fire_delay_scale_hostile,
                      NUM_SKILL_LEVELS);
   if (optional_string("$Friendly AI Secondary Fire Delay Scale:"))
    parse_float_list(aicp->ai_ship_fire_secondary_delay_scale_friendly,
                      NUM_SKILL_LEVELS);
   if (optional_string("$Hostile AI Secondary Fire Delay Scale:"))
    parse_float_list(aicp->ai_ship_fire_secondary_delay_scale_hostile,
                      NUM_SKILL_LEVELS);
  ...
   set_aic_flag(aicp, "$smart primary weapon selection:",
                  AIPF_SMART_PRIMARY_WEAPON_SELECTION);
   set_aic_flag(aicp, "$smart secondary weapon selection:",
                  AIPF_SMART_SECONDARY_WEAPON_SELECTION);
   set_aic_flag(aicp, "$smart shield management:",
                  AIPF_SMART_SHIELD_MANAGEMENT);
   set_aic_flag(aicp, "$smart afterburner management:",
                  AIPF_SMART_AFTERBURNER_MANAGEMENT);
   set_aic_flag(aicp, "$allow rapid secondary dumbfire:",
                  AIPF_ALLOW_RAPID_SECONDARY_DUMBFIRE);
   set_aic_flag(aicp, "$huge turret weapons ignore bombs:",
                  AIPF_HUGE_TURRET_WEAPONS_IGNORE_BOMBS);
   set_aic_flag(aicp, "$don't insert random turret fire delay:",
                  AIPF_DONT_INSERT_RANDOM_TURRET_FIRE_DELAY);
   set_aic_flag(aicp, "$prevent turrets targeting too distant bombs:",
                  AIPF_PREVENT_TARGETING_BOMBS_BEYOND_RANGE);
   set_aic_flag(aicp, "$smart subsystem targeting for turrets:",
                  AIPF_SMART_SUBSYSTEM_TARGETING_FOR_TURRETS);
   set_aic_flag(aicp, "$allow turrets target weapons freely:",
...
void parse_aitbl() 
{
    ...
    
    {
        ...
        
        {
            ...
            (ai_class *) vm_realloc(Ai_classes,
                                    Num_alloced_ai_classes *
                                    sizeof(ai_class));
...
 int ai_is_stealth_visible(object * viewer_objp, object * stealth_objp) 
{
    ...
    
    {
        ...
        vm_vec_dotprod(&viewer_objp->orient.vec.fvec,
                       &vec_to_stealth) / dist_to_stealth;
      ...
      float needed_dot_to_stealth;
        ...
        {
          needed_dot_to_stealth =
            get_skill_stealth_dot_scaler() * float (STEALTH_VIEW_CONE_DOT) *
...
float compute_dots(object * objp, object * other_objp, float *to_dot,
                   float *from_dot) 
{
...
void ai_turn_towards_vector(vec3d * dest, object * objp, float frametime,
                            float turn_time, vec3d * slide_vec,
                            vec3d * rel_pos, float bank_override, int flags,
                            vec3d * rvec, int sexp_flags) 
{
  ...
  objp->orient = objp_orient_copy;
    ...
    {
      vm_forward_interpolate(&desired_fvec, &curr_orient, &vel_in,
                              delta_time, delta_bank, &objp->orient, &vel_out,
                              &vel_limit, &acc_limit);
...
 int ai_select_primary_weapon(object * objp, object * other_objp,
                                int flags);
 
//  Make new_subsys the targeted subsystem of ship *aip.
  ship_subsys * set_targeted_subsys(ai_info * aip, ship_subsys * new_subsys,
                                    int parent_objnum) 
{
...
void adjust_accel_for_docking(ai_info * aip) 
{
    ...
    
    {
        ...
        if ((ratio < 0.8f)
            && !(The_mission.ai_profile->flags & AIPF_NO_MIN_DOCK_SPEED_CAP))
...
void project_point_to_perimeter(vec3d * perim_point, vec3d * pos,
                                float radius, vec3d * vp) 
{
...
void get_tangent_point(vec3d * tan1, vec3d * p0, vec3d * centerp, vec3d * p1,
                       float radius) 
{
...
// approach behavior.
void turn_towards_point(object * objp, vec3d * point, vec3d * slide_vec,
                        float bank_override) 
{
    ...
    
    {
        ...
        
        {
          ...
          ai_turn_towards_vector(point, objp, flFrametime,
                                  Ship_info[Ships[objp->instance].
                                            ship_info_index].srotation_time,
...
void turn_away_from_point(object * objp, vec3d * point, float bank_override) 
{
  ...
   ai_turn_towards_vector(&opposite_point, objp, flFrametime,
                            Ship_info[Ships[objp->instance].ship_info_index].
                            srotation_time, NULL, NULL, bank_override,
                            AITTV_FAST);
...
 float turn_toward_tangent_with_axis(object * objp, object * center_objp,
                                       float radius) 
{
    ...
    vm_vec_scale_add(&center_vec, &center_objp->pos,
                     &center_objp->orient.vec.fvec, center_obj_z);
...
void get_tangent_point(vec3d * goal_point, object * objp, vec3d * point,
                       float radius) 
{
...
//      FALSE   otherwise
int is_ignore_object_sub(int *ignore_objnum, int *ignore_signature,
                         int objnum) 
{
...
int get_nearest_bbox_point(object * ship_obj, vec3d * start, vec3d * box_pt) 
{
    ...
    model_get(Ship_info[Ships[ship_obj->instance].ship_info_index].model_num);
        ...
        
        {
            ...
            
            {
                ...
                
                {
                  ...
                  int inside =
                    get_nearest_bbox_point(eno->trial_objp,
                                           &Objects[eno->objnum].pos,
                                           &box_pt);
                    ...
                    
                    {
                        ...
                        vm_vec_dist_quick(&Objects[eno->objnum].pos, &box_pt);
                ...
                
                {
                    ...
                    vm_vec_dist_quick(&Objects[eno->objnum].pos,
                                      &eno->trial_objp->pos);
                ...
                if ((Ship_info[shipp->ship_info_index].
                     flags & (SIF_FIGHTER | SIF_BOMBER)))
                
                {
                  dist = dist * 0.5f;
                }
               num_attacking =
                num_enemies_attacking(eno->trial_objp - Objects);
...
int get_nearest_objnum(int objnum, int enemy_team_mask, int enemy_wing,
                       float range, int max_attackers) 
{
            ...
            
            {
                ...
                
                {
                    ...
                    
                    {
                        ...
                        if (Weapon_info
                            [Weapons[danger_weapon_objp->instance].
                             weapon_info_index].subtype == WP_LASER)
...
int find_nearby_threat(int objnum, int enemy_team_mask, float range,
                       int *count) 
{
    ...
    
    {
        ...
        
        {
                 ...
                 flags & (SIF_NO_SHIP_TYPE | SIF_NAVBUOY))
            ...
            
            {
                ...
                vm_vec_dist_quick(&Objects[objnum].pos,
                                  &objp->pos) - objp->radius * 0.75f;
...
int find_enemy(int objnum, float range, int max_attackers) 
{
    ...
    
    {
      ...
       return get_nearest_objnum(objnum, enemy_team_mask, aip->enemy_wing,
                                   range, max_attackers);
...
void ai_attack_object(object * attacker, object * attacked,
                      ship_subsys * ssp) 
{
    ...
    if (!
        (Ship_info[Ships[attacker->instance].ship_info_index].
         flags & SIF_SMALL_SHIP) && (attacked != NULL))
...
void ai_attack_wing(object * attacker, int wingnum) 
{
  ...
  aip->submode_start_time = Missiontime;        // for AIM_CHASE... it may have been not set correctly here
    ...
    
    {
      ...
       set_target_objnum(aip,
                           Ships[Wings[wingnum].ship_index[index]].objnum);
...
void add_path_point(vec3d * pos, int path_num, int path_index,
                    int modify_index) 
{
...
void create_path_to_point(vec3d * curpos, vec3d * goalpos, object * curobjp,
                                  ...
                                  int randomize_pnt) 
{
  ...
  int start_index, finish_index;
    ...
    
    {
      ...
       model_find_submodel_offset(&submodel_offset, modelnum,
                                    mp->parent_submodel);
    ...
    
    {
        ...
        
        {
          ...
          model_find_world_point(&v1, &local_vert, modelnum,
                                  mp->parent_submodel, &objp->orient,
                                  &objp->pos);
...
void create_model_path(object * pl_objp, object * mobjp, int path_num,
                       int subsys_path) 
{
    ...
    polymodel * pm =
    model_get(Ship_info[Ships[mobjp->instance].ship_info_index].model_num);
    ...
    
    {
        ...
        {
          project_point_to_perimeter(&perim_point2, &mobjp->pos,
                                      mobjp->radius, &pl_objp->pos);
        ...
        {
          project_point_to_perimeter(&perim_point1, &mobjp->pos,
                                      mobjp->radius, &gp0);
...
//  Calls pp_collide
int pp_collide_any(vec3d * curpos, vec3d * goalpos, float radius,
...
void ai_find_path(object * pl_objp, int objnum, int path_num, int exit_flag,
                  int subsys_path) 
{
    ...
    
    {
        ...
        
        {
                   ...
                   "ai_find_path tring to find a path (%d) that doesn't exist, on ship %s",
...
 extern int vector_object_collision(vec3d * start_pos, vec3d * end_pos,
...
int maybe_avoid_player(object * objp, vec3d * goal_pos) 
{
    ...
    
    {
        ...
        {
          vm_vec_copy_scale(&avoid_vec, &objp->orient.vec.rvec,
                             frand() - 0.5f);
          vm_vec_scale_add2(&avoid_vec, &objp->orient.vec.uvec,
                             frand() - 0.5f);
        ...
        vm_vec_scale_add(&aip->avoid_goal_point, &player_objp->pos,
                         &avoid_vec, 400.0f);
...
void ai_stay_still(object * still_objp, vec3d * view_pos) 
{
  ...
  Assert((still_objp->instance >= 0)
          && (still_objp->instance < MAX_OBJECTS));
    ...
    if (view_pos != NULL)
                      ...
                      &still_objp->orient.vec.fvec, 100.0f);
...
void ai_do_objects_docked_stuff(object * docker, int docker_point,
                                object * dockee, int dockee_point,
                                bool update_clients) 
{
    ...
    
    {
        ...
        if ((Ship_info[Ships[docker->instance].ship_info_index].
             flags & SIF_SUPPORT)  ||(Ship_info[Ships[dockee->instance].
                                                 ship_info_index].
                                       flags & SIF_SUPPORT))
...
void ai_dock_with_object(object * docker, int docker_index, object * dockee,
                         int dockee_index, int dock_type) 
{
    ...
    
    {
        ...
        model_get(Ship_info[Ships[dockee->instance].ship_info_index].
                  model_num);
    ...
    {
      dock_orient_and_approach(docker, docker_index, dockee, dockee_index,
                                DOA_DOCK_STAY);
...
void ai_start_waypoints(object * objp, int waypoint_list_index,
                        int wp_flags) 
{
    ...
    aip = &Ai_info[Ships[objp->instance].ai_index];
         ...
         && (aip->wp_index == waypoint_list_index))
...
float compute_collision_time(vec3d * targpos, vec3d * targvel,
                             vec3d * attackpos, float weapon_speed) 
{
    ...
    pos_dot_vel * pos_dot_vel - vel_sqr * vm_vec_dot(&vec_to_target,
                                                     &vec_to_target);
                            ...
                            object * eobjp) 
{
    ...
    MIN((max_laser_speed * Weapon_info[weapon_num].lifetime),
    ...
    
    {
        ...
        
        {
            ...
            (float) (dist_to_enemy /
                     (pl_speed * scale + max_laser_speed * (1.0f - scale)));
...
void fds_aux(float *dot, float *tts, vec3d * pos, float dtime,
             object * objp) 
{
...
//  --------------------------------------------------------------------------
void ai_set_positions(object * pl_objp, object * en_objp, ai_info * aip,
                      vec3d * player_pos, vec3d * enemy_pos) 
{
...
void ai_update_aim(ai_info * aip, object * En_Objp) 
{
    ...
    
    {
        ...
        Missiontime + fl2f(frand_range(0.0f, aip->ai_max_aim_update_delay));
    ...
    
    {
        ...
        vm_vec_scale_add2(&aip->last_aim_enemy_pos, &aip->last_aim_enemy_vel,
                          flFrametime);
...
int find_nearest_waypoint(object * objp) 
{
    ...
    
    {
        ...
        vm_vec_dot_to_point(&objp->orient.vec.fvec, &objp->pos,
                            &wpl->waypoints[i]);
...
int get_base_path_info(int path_cur, int goal_objnum, model_path ** pmp,
                       mp_vert ** pmpv) 
{
...
float maybe_recreate_path(object * objp, ai_info * aip,
                          int force_recreate_flag, int override_hash = 0) 
{
    ...
    
    {
        ...
        
        {
            ...
            ai_matrix_dist(&path_objp->orient,
                           &aip->path_create_orient) * 25.0f;
...
void set_accel_for_docking(object * objp, ai_info * aip, float dot,
                           float dot_to_next, float dist_to_next,
                           float dist_to_goal, ship_info * sip) 
{
    ...
    
    {
        ...
        
        {
                                      ...
                                      sip->max_speed * MAX(dist_to_next /
                                                           500.0f, 1.0f));
...
float ai_path() 
{
    ...
    
    {
        ...
        ai_return_path_num_from_dockbay(&Objects[aip->goal_objnum],
                                        aigp->dockee.index);
    ...
    
    {
      ...
      vm_vec_sub(&delvec, cvp,
                  &Path_points[aip->path_cur - aip->path_dir].pos);
    ...
    
    {
        ...
        find_nearest_point_on_line(&nearest_point, &Pl_objp->last_pos,
                                   &Pl_objp->pos, &gcvp);
        ...
        if ((aip->mode == AIM_DOCK)
            && (aip->path_cur >= aip->path_length - 2))
...
int get_enemy_team_range(object * my_objp, float range, int enemy_team_mask,
                         vec3d * min_vec, vec3d * max_vec) 
{
    ...
    
    {
        ...
        
        {
               ...
               flags & (SIF_FIGHTER | SIF_BOMBER | SIF_CRUISER | SIF_CAPITAL |
              ...
              
              {
                  ...
                  {
                    update_min_max(objp->pos.xyz.x, &min_vec->xyz.x,
                                    &max_vec->xyz.x);
                    update_min_max(objp->pos.xyz.y, &min_vec->xyz.y,
                                    &max_vec->xyz.y);
                    update_min_max(objp->pos.xyz.z, &min_vec->xyz.z,
                                    &max_vec->xyz.z);
...
float ai_safety_goto_spot(object * objp) 
{
    ...
    
    {
                                ...
                                sip->max_speed * dot_val * (dist / 200.0f +
                                                            0.2f));
...
 void ai_safety_circle_spot(object * objp) 
{
                              ...
                              0.5f * (1.0f + dot) * sip->max_speed / 4.0f);
...
void ai_fly_to_ship() 
{
    ...
    {
      if (aip->goals[i].ai_mode == AIM_FLY_TO_SHIP
        ...
        
        {
            ...
            
            {
                   ...
                   && !stricmp(aip->goals[i].ship_name, Ships[j].ship_name))
  ...
  int scale;
  ...
   bool carry_flag = ((shipp->flags2 & SF2_NAVPOINT_CARRY)
                        || ((shipp->wingnum >= 0)
                            && (Wings[shipp->wingnum].flags & WF_NAV_CARRY)));
   if (AutoPilotEngaged && timestamp_elapsed(LockAPConv)
         && carry_flag 
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                
                {
                    ...
                    get_absolute_wing_pos_autopilot(&goal_point, wing_leader,
                                                    wing_index,
                                                    aip->
                                                    ai_flags &
                ...
                
                {
                    ...
                    1 +
                    int ((float)
                         floor(double (autopilot_wings[aip->wing] - 1) /
                               2.0));
                   switch (autopilot_wings[aip->wing] % 2)
                    
                    {
                    case 1:   // back-left
                      vm_vec_copy_normalize(&perp,
          ...
           vm_vec_sub(&perp, Navs[CurrentNav].GetPosition(),
                        &Player_obj->pos);
        ...
        {
          vm_vec_scale_add(&perp, &Pl_objp->pos, &wing_leader->phys_info.vel,
                            1000.0f);
          ai_turn_towards_vector(&perp, Pl_objp, flFrametime,
                                  sip->srotation_time * 3.0f * scale,
                                  slop_vec, NULL, 0.0f, 0);
  ...
  float max_allowed_speed = 9999.9f;
    ...
    
    {
        ...
        find_nearest_point_on_line(&nearest_point, &Pl_objp->last_pos,
                                   &Pl_objp->pos, target_pos);
       if ((vm_vec_dist_quick(&Pl_objp->pos, target_pos) <
              ...
              
              {
                  ...
                  
                  {
                      ...
                      ai_mission_wing_goal_complete(Ships[Pl_objp->instance].
                                           ...
                                           Wings[Ships[Pl_objp->instance].
                                                 wingnum].name, wpl->name,
                                           -1);
...
void avoid_ship() 
{
    ...
    
    {
      ...
       vm_vec_scale_add(&goal_pos, &En_objp->pos, &En_objp->orient.vec.fvec,
                          -100.0f);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                vm_vec_dist_quick(&objp->pos,
                                  &Player_obj->pos) - Player_obj->radius -
                objp->radius;
                ...
                
                {
                  ...
                   vm_vec_normalized_dir(&v2p, &Player_obj->pos,
                                           &objp->pos);
...
void maybe_afterburner_after_ship_hit(object * objp, ai_info * aip,
                                      object * en_objp) 
{
...
void evade_weapon() 
{
    ...
    
    {
        ...
        
        {
               ...
               1.5f * vm_vec_dist_quick(&unlocked_weapon_objp->pos,
                                        &Pl_objp->pos))
    ...
    
    {
        ...
        
        {
          ...
           if (aip->ai_profile_flags & AIPF_ALLOW_VERTICAL_DODGE
                 && abs(udot) > abs(rdot))
            
            {
              if ((udot < -0.5f) || (udot > 0.5f))
                vm_vec_scale_add(&goal_point, &Pl_objp->pos,
                                  &Pl_objp->orient.vec.uvec, -200.0f);
            ...
            
            {
                                  ...
                                  &Pl_objp->orient.vec.rvec, -200.0f);
...
void slide_face_ship() 
{
  ...
   vm_vec_scale_add(&goal_pos, &En_objp->pos, &En_objp->orient.vec.rvec,
                      right * 200.0f);
  vm_vec_scale_add(&goal_pos, &En_objp->pos, &En_objp->orient.vec.uvec,
                    up * 200.0f);
...
void evade_ship() 
{
    ...
    
    {
        ...
        
        {
            ...
            
            {
                     ...
                     ai_profile_flags & AIPF_SMART_AFTERBURNER_MANAGEMENT)
                
                {
                  aip->afterburner_stop_time =
                    (fix) (Missiontime + F1_0 +
                           static_randf(Pl_objp - Objects) * F1_0 / 4);
                ...
                
                {
                    ...
                    Missiontime + F1_0 + static_rand(Pl_objp - Objects) / 4;
    ...
    
    {
        ...
        vm_vec_scale_add(&goal_point, &enemy_pos, &En_objp->orient.vec.fvec,
                         -1000.0f);
    ...
    
    {
        ...
        
        {
            ...
            vm_vec_scale_add(&goal_point, &enemy_pos,
                             &Pl_objp->orient.vec.rvec, 1000.0f);
        ...
        
        {
            ...
            if ((aip->prev_goal_point.xyz.x != 0.0f)
                ...
                || (aip->prev_goal_point.xyz.z != 0.0f))
                              ...
                              &En_objp->orient.vec.rvec, 100.0f);
...
int ai_select_primary_weapon_OLD(object * objp, object * other_objp,
                                 int flags) 
{
    ...
    Assert(shipp->ship_info_index >= 0
           && shipp->ship_info_index < Num_ship_classes);
...
int ai_select_primary_weapon(object * objp, object * other_objp, int flags) 
{
    ...
    if (!
        (Ai_info[shipp->ai_index].
         ai_profile_flags & AIPF_SMART_PRIMARY_WEAPON_SELECTION))
    ...
    if ((flags & WIF_PUNCTURE)
        && (Ai_info[shipp->ai_index].goals[0].
            ai_mode & (AI_GOAL_DISARM_SHIP | AI_GOAL_DISABLE_SHIP)))
  ...
   if (Ship_info[Ships[other_objp->instance].ship_info_index].
         flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP))
...
void set_primary_weapon_linkage(object * objp) 
{
    ...
    
    {
        ...
        
        {
         ...
         || sip->flags2 & SIF2_NO_PRIMARY_LINKING)
    ...
    {
      if (aip->goals[aip->active_goal].
           ai_mode & (AI_GOAL_DISABLE_SHIP | AI_GOAL_DISARM_SHIP))
        
        {
            ...
            
            {
                   ...
                   (Weapon_info[swp->primary_bank_weapons[0]].
                    wi_flags & WIF_PUNCTURE))
                return;
              if (!
                   (Weapon_info[swp->primary_bank_weapons[1]].
                    wi_flags & WIF_PUNCTURE))
  ...
  float dot;
    ...
    
    {
        ...
        
        {
            ...
            {
              vm_vector_2_matrix(&objp->orient, &v2t, &objp->orient.vec.uvec,
                                  NULL);
        ...
        Weapon_info[swp->primary_bank_weapons[swp->current_primary_bank]].wi_flags2 & WIF2_BALLISTIC)   //Current weapon must be ballistic
    {
        ...
        //Combine factors
      float burstFireProb =
        ((0.6f * percentAmmoLeft) +
         (0.4f * distanceFactor)) * dotToTarget *
...
void ai_select_secondary_weapon(object * objp, ship_weapon * swp,
                                int priority1 = -1, int priority2 = -1) 
{
    ...
    num_weapon_types =
    get_available_secondary_weapons(objp, weapon_id_list, weapon_bank_list);
    ...
    
    {
        ...
        wi_flags;
      int ignore_mask_to_use =
        ((aip->ai_profile_flags & AIPF_SMART_SECONDARY_WEAPON_SELECTION)
         && (wi_flags & WIF_BOMBER_PLUS)) ? ignore_mask_without_huge :
        ignore_mask;
      ...
      if (priority2_index == -1)
        
        {
            ...
            
            {
                ...
                ((aip->
                  ai_profile_flags & AIPF_SMART_SECONDARY_WEAPON_SELECTION)
                 && (wi_flags & WIF_BOMBER_PLUS)) ? ignore_mask_without_huge :
                ignore_mask;
    ...
    if ((aip->ai_profile_flags & AIPF_ALLOW_RAPID_SECONDARY_DUMBFIRE)
        && !(wip->wi_flags & WIF_HOMING) && (wip->fire_wait < .5f))
        ...
        
        {
            ...
            
            {
                ...
                if (!object_is_docked(A)
                    && !(aip->
                         ai_flags & (AIF_REPAIRING | AIF_BEING_REPAIRED)))
                ...
                
                {
                  ...
                  int t =
                    ...
                    (NUM_SKILL_LEVELS + 2);
                       ...
                       Ai_info[Ships[tobjp->instance].ai_index].
...
//  collide with obj2.
int might_collide_with_ship(object * obj1, object * obj2, float dot_to_enemy,
...
int might_hit_teammate(object * firing_objp) 
{
    ...
    
    {
        ...
        
        {
            ...
            vm_vec_dot(&firing_objp->orient.vec.fvec, &vec_to_objp) / dist;
...
void render_all_ship_bay_paths(object * objp) 
{
    ...
    
    {
        ...
        {
          vm_vec_unrotate(&global_path_point, &mp->verts[j].pos,
                           &objp->orient);
...
 void render_path_points(object * objp) 
{
    ...
    model_get(Ship_info[Ships[dobjp->instance].ship_info_index].model_num);
    ...
    
    {
      ...
      model_find_world_point(&global_dock_point, &dock_point, pm->id, 0,
...
float ai_get_weapon_dist(ship_weapon * swp) 
{
    ...
    MIN((Weapon_info[weapon_num].max_speed *
         Weapon_info[weapon_num].lifetime),
        Weapon_info[weapon_num].weapon_range);
...
void set_predicted_enemy_pos_turret(vec3d * predicted_enemy_pos,
                                    vec3d * gun_pos, object * pobjp,
                                    vec3d * enemy_pos, vec3d * enemy_vel,
                                    float weapon_speed,
                                    float time_enemy_in_range) 
{
    ...
    
    {
        ...
        compute_collision_time(enemy_pos, enemy_vel, gun_pos, weapon_speed);
        ...
        
        {
            ...
            (1.0f - aip->ai_accuracy) * 4.0f * (1.0f +
                                                4.0f * (1.0f -
                                                        time_enemy_in_range /
                                                        (2 * range_time)));
        }
       static_randvec(((pobjp - Objects) ^ (Missiontime >> 16)) & 7,
                        &rand_vec);
...
void set_predicted_enemy_pos(vec3d * predicted_enemy_pos, object * pobjp,
                             vec3d * enemy_pos, vec3d * enemy_vel,
                             ai_info * aip) 
{
    ...
    
    {
          ...
          The_mission.ai_profile->in_range_time[Game_skill_level];
    ...
    
    {
        ...
        compute_collision_time(enemy_pos, &target_moving_direction, &gun_pos,
                               weapon_speed);
        ...
        
        {
                          ...
                          &target_moving_direction, collision_time);
  ...
  float scale = 0.0f;
    ...
    
    {
        ...
        3.0f * (1.4f - dot) * (1.0f +
                               dist / (get_skill_stealth_dist_scaler() *
                                       STEALTH_MAX_VIEW_DIST)) * (1 -
                                                                  aip->
                                                                  ai_accuracy);
...
void ai_chase_ct() 
{
  ...
   ai_turn_towards_vector(&tvec, Pl_objp, flFrametime, sip->srotation_time,
                            NULL, NULL, 0.0f, 0);
...
void ai_chase_eb(ai_info * aip, ship_info * sip, vec3d * predicted_enemy_pos,
                 float dist_to_enemy) 
{
    ...
    vm_vec_scale_add(&_pep, &Pl_objp->pos, &En_objp->orient.vec.fvec,
                      100.0f);
   ai_turn_towards_vector(&_pep, Pl_objp, flFrametime, sip->srotation_time,
                            NULL, NULL, 0.0f, 0);
...
float ai_endangered_by_weapon(ai_info * aip) 
{
  ...
   return ai_endangered_time(&Objects[Ships[aip->shipnum].objnum],
                               weapon_objp);
...
void attack_set_accel(ai_info * aip, float dist_to_enemy, float dot_to_enemy,
                      float dot_from_enemy) 
{
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                //nprintf(("AI", " slowly "));
                accelerate_ship(aip,
                                MAX(1.0f - (dist_to_enemy - 200.0f) / 600.0f,
                                    0.1f));
    ...
    
    {
      ...
      int strafeHoldDirAmount = 3;
        ...
        side_vec.x =
        static_randf_range((((Missiontime +
                              static_rand(aip->shipnum)) >> 16) /
                            strafeHoldDirAmount), -1.0f, 1.0f);
      side_vec.y =
        static_randf_range((((Missiontime +
                              static_rand(aip->shipnum)) >> 16) /
                            strafeHoldDirAmount) * 2, -1.0f, 1.0f);
        ...
        vm_vec_boxscale(&side_vec, 1.0f);
           ...
           200.0f +
           vm_vec_mag_quick(&En_objp->phys_info.vel) * dot_from_enemy +
                    ...
                    
                    {
                        ...
                        
                        {
                            ...
                            
                            {
                                ...
                                max_ab_vel =
                                sip->afterburner_max_vel.xyz.z >
                                0.0f ? sip->afterburner_max_vel.xyz.z : sip->
                                max_vel.xyz.z;
                                ...
                                // Get minimum
                                ab_time =
                                MIN(time_to_exhaust_25pct_fuel,
                                    time_to_fly_75pct_of_distance);
                            ...
                            
                            {
                                ...
                                Missiontime + F1_0 + static_rand(Pl_objp -
                                                                 Objects) / 4;
...
int will_collide_pp(vec3d * p0, vec3d * p1, float radius, object * big_objp,
                    vec3d * collision_point) 
{
  mc_info mc;
  polymodel * pm =
    model_get(Ship_info[Ships[big_objp->instance].ship_info_index].model_num);
...
int will_collide_with_big_ship(object * objp, vec3d * goal_point,
                               object * big_objp, vec3d * collision_point,
                               float delta_time) 
{
...
int will_collide_with_big_ship_all(object * objp, object * ignore_objp,
                                   vec3d * goal_point,
                                   vec3d * collision_point, float *distance,
                                   float delta_time) 
{
...
void mabs_pick_goal_point(object * objp, object * big_objp,
                          vec3d * collision_point, vec3d * avoid_pos) 
{
    ...
    
    {
        ...
        
        {
          ...
          float kr =
            ...
            will_collide_pp(&objp->pos, &p, objp->radius, big_objp,
                            collision_point);
  ...
  vm_vec_scale_add(avoid_pos, &objp->pos, &away_vec,
                    big_objp->radius * 1.5f);
 }

 
//  Return true if a large ship is being ignored.
int maybe_avoid_big_ship(object * objp, object * ignore_objp, ai_info * aip,
                         vec3d * goal_point, float delta_time) 
{
    ...
    
    {
            ...
            will_collide_with_big_ship_all(Pl_objp, ignore_objp, goal_point,
                                           &collision_point, &distance,
                                           delta_time)) != -1)
...
void ai_stealth_find() 
{
  ...
  int delta_time = (timestamp() - aip->stealth_last_visible_stamp);
    ...
    
    {
        ...
        // do turn around)
        vm_vec_scale_add(&aip->goal_point, &Pl_objp->pos,
                         &Pl_objp->orient.vec.fvec, -300.0f);
      aip->submode_parm0 = SM_SF_BEHIND;
      vm_vec_sub(&vec_to_enemy, &new_pos, &Pl_objp->pos);
      dist_to_enemy = vm_vec_normalize_quick(&vec_to_enemy);
      dot_to_enemy =
        vm_vec_dotprod(&vec_to_enemy, &Pl_objp->orient.vec.fvec);
    ...
    -vm_vec_dotprod(&vec_to_enemy, &En_objp->orient.vec.fvec);
...
void ai_stealth_sweep() 
{
    ...
    
    {
        ...
        vm_vec_scale_add(&goal_pt, &aip->stealth_last_pos,
                         &aip->stealth_velocity, (0.001f * lost_time));
    ...
    case SM_SS_LR:
      ...
      vm_vec_scale_add2(&goal_pt, &forward,
                         0.5f * aip->stealth_sweep_box_size);
  ...
   ai_turn_towards_vector(&goal_pt, Pl_objp, flFrametime,
                            sip->srotation_time, NULL, NULL, 0.0f, 0);
...
//  ATTACK submode handler for chase mode.
void ai_chase_attack(ai_info * aip, ship_info * sip,
                     vec3d * predicted_enemy_pos, float dist_to_enemy,
                     int modelnum) 
{
  ...
  float bank_override = 0.0f;
    ...
    
    {
        ...
        dist_to_enemy / (dist_to_enemy + En_objp->radius) * En_objp->radius;
    ...
    
    {
                              ...
                              sip->srotation_time, NULL, rel_pos,
...
void ai_chase_es(ai_info * aip, ship_info * sip) 
{
    ...
    vm_vec_scale_add2(&tvec, &Pl_objp->orient.vec.rvec,
                       f2fl(scale ^ 0x10000));
  if (timeslice & 0x02)
    vm_vec_scale_sub2(&tvec, &Pl_objp->orient.vec.rvec, f2fl(scale));
  if (timeslice & 0x04)
    vm_vec_scale_add2(&tvec, &Pl_objp->orient.vec.uvec,
                       f2fl(scale ^ 0x10000));
  ...
   ai_turn_towards_vector(&tvec, Pl_objp, flFrametime / 2,
                            sip->srotation_time, NULL, NULL, bank_override,
                            0);
...
void ai_chase_ga(ai_info * aip, ship_info * sip) 
{
    ...
    
    {
        ...
        
        {
            ...
            100.0f * Ships[Pl_objp->instance].afterburner_fuel /
            sip->afterburner_fuel_capacity;
...
void update_aspect_lock_information(ai_info * aip, vec3d * vec_to_enemy,
                                    float dist_to_enemy, float enemy_radius) 
{
    ...
    get_available_secondary_weapons(Pl_objp, weapon_id_list,
                                    weapon_bank_list);
...
void ai_chase_fly_away(object * objp, ai_info * aip) 
{
    ...
    
    {
      ...
       vm_vec_normalized_dir(&v2e, &Objects[aip->target_objnum].pos,
                               &objp->pos);
...
int has_preferred_secondary(object * objp, object * en_objp,
                            ship_weapon * swp) 
{
...
void ai_choose_secondary_weapon(object * objp, ai_info * aip,
                                object * en_objp) 
{
...
float set_secondary_fire_delay(ai_info * aip, ship * shipp,
                               weapon_info * swip, bool burst) 
{
...
 void ai_chase_big_circle_set_goal(vec3d * goal_pos, object * attack_objp,
                                     object * target_objp, float *accel) 
{
...
void ai_chase_big_get_separations(object * attack_objp, object * target_objp,
                                  ...
                                  float *cur_separation) 
{
    ...
    polymodel * pm =
    model_get(Ship_info[Ships[attack_objp->instance].ship_info_index].
              model_num);
    ...
    vm_vec_scale_add(horz_vec_to_target, &vec_to_target,
                     &target_objp->orient.vec.fvec, -perp_dist);
                                          ...
                                          float *accel) 
{
    ...
    opposing =
    (vm_vec_dotprod
     (&attack_objp->orient.vec.fvec, &target_objp->orient.vec.fvec) < 0);
   ai_chase_big_get_separations(attack_objp, target_objp,
                                  &horz_vec_to_target, &optimal_separation,
                                  &separation);
  ...
  float dist = 2000;
    ...
    vm_vec_scale_add(goal_pos, &target_objp->pos,
                     &target_objp->orient.vec.fvec, dist);
    ...
    vm_vec_dotprod(&vec_to_target, &target_objp->orient.vec.fvec);
   float match_accel = 0.0f;
  float length_scale = attack_objp->radius;
   if (Ship_info[Ships[attack_objp->instance].ship_info_index].max_vel.xyz.
         z != 0.0f)
    
    {
      match_accel =
        target_objp->phys_info.vel.xyz.z /
        Ship_info[Ships[attack_objp->instance].ship_info_index].max_vel.xyz.z;
...
void ai_cruiser_chase_set_goal_pos(vec3d * goal_pos, object * pl_objp,
                                   object * en_objp) 
{
    ...
    
    {
      ...
      float accel = 0.0f;
        ...
        
        {
            ...
            // do approach stuff;
            ai_chase_big_approach_set_goal(&goal_pos, Pl_objp, En_objp,
                                           &accel);
            ...
            // do parallel stuff
            ai_chase_big_parallel_set_goal(&goal_pos, Pl_objp, En_objp,
                                           &accel);
        ...
        // now move as desired
        ai_turn_towards_vector(&goal_pos, Pl_objp, flFrametime, turn_time,
                               NULL, NULL, 0.0f, 0, rvecp);
        ...
        
        {
            ...
            
            {
                ...
                if (vm_vec_dotprod(&vec_to_enemy, &En_objp->orient.vec.fvec) >
                    0)
            ...
            // we're opposing
            if (vm_vec_dotprod
                (&Pl_objp->orient.vec.fvec, &En_objp->orient.vec.fvec) < 0)
...
void ai_chase() 
{
    ...
    
    {
        ...
        &Ship_info[Ships[En_objp->instance].ship_info_index];
    ...
    
    {
         ...
         && (Ship_types[sip->class_type].ai_bools & STI_AI_ATTEMPT_BROADSIDE))
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                {
                  get_subsystem_pos(&enemy_pos, En_objp,
                                     aip->targeted_subsys);
    ...
    
    {
        ...
        if (aip->ai_stalemate_time_thresh > 0.0f
            ...
            && (dot_to_enemy <
                 0.95f - 0.5f * En_objp->radius / MAX(1.0f,
                                                      En_objp->radius +
                                                      dist_to_enemy)))
        
        {
            ...
            if (static_randf((Missiontime + static_rand(aip->shipnum)) >> 16)
                > (aip->ai_patience * .01))
        ...
        
        {
            ...
            if (aip->submode != AIS_CHASE_CIRCLESTRAFE
                && might_collide_with_ship(Pl_objp, En_objp, dot_to_enemy,
                                           dist_to_enemy, 4.0f))
    ...
    
    {
      ...
      else if (ai_near_full_strength(Pl_objp)
        ...
        
        {
            ...
            ? (float) (aip->ai_class + Game_skill_level) / (Num_ai_classes +
                                                            NUM_SKILL_LEVELS)
        ...
        
        {
            ...
            ? (aip->ai_class + Game_skill_level) / (Num_ai_classes +
                                                    NUM_SKILL_LEVELS)  :
            aip->ai_get_away_chance;
        ...
        
        {
            ...
            
            {
                ...
                
                {
                  ...
                  Assert(tswp->current_primary_bank <
                          tswp->num_primary_banks);
                  weapon_info * pwip =
                    &Weapon_info[tswp->
                                 primary_bank_weapons[tswp->
                                                      current_primary_bank]];
                   
                    //  Less likely to fire if far away and moving.
                    scale =
                    pwip->max_speed / (En_objp->phys_info.speed +
                                       pwip->max_speed);
                    ...
                    
                    {
                        ...
                        
                        {
                            ...
                            
                            {
                                   ...
                                   (swp->
                                    next_secondary_fire_stamp[current_bank]) >
                            ...
                            
                            {
                                ...
                                
                                {
                                      ...
                                      MIN((swip->max_speed * swip->lifetime *
                                           0.75f), swip->weapon_range);
...
//  Return distance.
float dock_move_towards_point(object * objp, vec3d * start, vec3d * finish,
                              float speed_scale, float other_obj_speed =
...
void set_goal_dock_orient(matrix * dom, vec3d * docker_p0, vec3d * docker_p1,
                          vec3d * docker_p0_norm, matrix * docker_orient,
                          vec3d * dockee_p0, vec3d * dockee_p1,
...
// Goober5000
void find_adjusted_dockpoint_info(vec3d * global_p0, vec3d * global_p1,
                                  vec3d * global_p0_norm, object * objp,
                                  polymodel * pm, int modelnum, int submodel,
                                  int dock_index) 
{
    ...
    
    {
      ...
      vm_vec_sub(&local_p0, &pm->docking_bays[dock_index].pnt[0],
                  &submodel_offset);
      vm_vec_sub(&local_p1, &pm->docking_bays[dock_index].pnt[1],
                  &submodel_offset);
       
        // find the dynamic positions of the dockpoints
        model_find_world_point(global_p0, &local_p0, modelnum, submodel,
                               &objp->orient, &objp->pos);
      model_find_world_point(global_p1, &local_p1, modelnum, submodel,
                              &objp->orient, &objp->pos);
       
        // find the normal of the first dockpoint
        model_find_world_dir(global_p0_norm,
                             &pm->docking_bays[dock_index].norm[0], modelnum,
    ...
    {
      vm_vec_unrotate(global_p0, &pm->docking_bays[dock_index].pnt[0],
                       &objp->orient);
      vm_vec_add2(global_p0, &objp->pos);
       vm_vec_unrotate(global_p1, &pm->docking_bays[dock_index].pnt[1],
                         &objp->orient);
      vm_vec_add2(global_p1, &objp->pos);
       vm_vec_unrotate(global_p0_norm,
                         &pm->docking_bays[dock_index].norm[0],
                         &objp->orient);
                               ...
                               rotating_dockpoint_info * rdinfo) 
{
  ...
  int dockee_rotating_submodel =
    ...
    find_adjusted_dockpoint_info(&docker_p0, &docker_p1, &docker_p0_norm,
                                 docker_objp, pm0, sip0->model_num, -1,
                                 docker_index);
  find_adjusted_dockpoint_info(&dockee_p0, &dockee_p1, &dockee_p0_norm,
                                dockee_objp, pm1, sip1->model_num,
    ...
    
    {
        ...
        // get submodel center
        model_find_submodel_offset(&submodel_offset, sip1->model_num,
                                   dockee_rotating_submodel);
        ...
        
        {
            ...
            pm1->submodel[dockee_rotating_submodel].sii->cur_turn_rate;
    ...
    
    {
      ...
      int force_recreate = (dockee_rotating_submodel >= 0)
        && ((dock_mode == DOA_APPROACH) || (dock_mode == DOA_UNDOCK_1));
       if (maybe_recreate_path
             (docker_objp, aip, force_recreate, force_recreate) > 5.0f)
    ...
    
    {
      ...
      
      {
          ...
          set_goal_dock_orient(&dom, &docker_p0, &docker_p1, &docker_p0_norm,
                               &docker_objp->orient, &dockee_p0, &dockee_p1,
                               &dockee_p0_norm, &dockee_objp->orient);
          ...
          vm_matrix_interpolate(&dom, &docker_objp->orient, &omega_in,
                                flFrametime, &nm, &omega_out, &vel_limit,
                                &acc_limit, 1);
          ...
          dock_move_towards_point(docker_objp, &docker_objp->pos, goal_point,
                                  speed_scale, dockee_objp->phys_info.speed,
                                  rdinfo);
      ...
      
      {
          ...
          
          {
              ...
              dock_move_towards_point(docker_objp, &docker_point,
                                      &dockee_point, speed_scale,
      ...
      
      {
          ...
          dock_move_towards_point(docker_objp, &docker_objp->pos,
                                  &Path_points[aip->path_start +
      ...
      
      {
          ...
          vm_vec_normalized_dir(&away_vec, &docker_objp->pos,
                                &dockee_objp->pos);
        vm_vec_scale_add(&goal_point, &dockee_objp->pos, &away_vec,
                          goal_dist);
          ...
          
          {
              ...
              srotation_time;
            ai_turn_towards_vector(&goal_point, docker_objp, flFrametime,
                                    turn_time, NULL, NULL, 0.0f, 0);
...
void guard_object_was_hit(object * guard_objp, object * hitter_objp) 
{
    ...
    
    {
        ...
        
        {
               ...
               (Ship_info[Ships[guard_objp->instance].ship_info_index].
                flags & SIF_SMALL_SHIP))
                                   ...
                                   object * guarded_objp) 
{
  ...
   object * closest_asteroid_objp = NULL, *danger_asteroid_objp =
    NULL, *asteroid_objp;
  float dist_to_self, closest_danger_asteroid_dist =
    999999.0f, closest_asteroid_dist = 999999.0f;
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                vm_vec_dist_quick(&asteroid_objp->pos, &guarding_objp->pos);
...
float get_cylinder_points(object * other_objp, object * cyl_objp,
                          vec3d * axis_pt, vec3d * r_vec, float *radius) 
{
    ...
    polymodel * pm =
    model_get(Ship_info[Ships[cyl_objp->instance].ship_info_index].model_num);
  ...
  float extended_cylinder_z =
    vm_vec_dotprod(&r_sph, &cyl_objp->orient.vec.fvec);
   
    // get pt on axis of extended cylinder
    vm_vec_scale_add(axis_pt, &cyl_objp->pos, &cyl_objp->orient.vec.fvec,
                     extended_cylinder_z);
    ...
    
    {
      ...
      
      {
        ...
        float objval = static_randf(Pl_objp - Objects);
          ...
          extended_z =
          get_cylinder_points(Pl_objp, guard_objp, &axis_pt, &r_vec, &radius);
        ...
        float min_z, max_z, length;
          ...
          model_get(Ship_info[Ships[guard_objp->instance].ship_info_index].
                    model_num);
        ...
        int time_choose = int (floor(log(length * 0.001f) / log(2.0f)));
          ...
          min_z + length * static_randf(Pl_objp -
                                        Objects ^ (Missiontime >>
                                                   (22 + time_choose)));
        ...
        int inside = (extended_z > min_z) && (extended_z < min_z + length);
          ...
          
          {
              ...
              
              {
                  ...
                  
                  {
                      ...
                      vm_vec_scale_add(&goal_pt, &axis_pt, &r_vec,
                                       desired_guard_dist);
                    vm_vec_scale_add2(&goal_pt, &theta_vec,
                                       desired_guard_dist);
              ...
              
              {
                ...
                vm_vec_scale_add2(&goal_pt, &guard_objp->phys_info.vel,
                                   time);
                ...
                 vm_vec_scale_add2(&goal_pt, &guard_objp->orient.vec.fvec,
                                     v_z * time);
          ...
          
          {
            ...
            float delta_z = desired_z - extended_z;
                              ...
                              &guard_objp->orient.vec.fvec, delta_z);
          ...
          ai_turn_towards_vector(&goal_pt, Pl_objp, flFrametime,
                                 Ship_info[Ships[Pl_objp->instance].
                                           ship_info_index].srotation_time,
                                 NULL, NULL, 0.0f, 0);
...
void ai_guard() 
{
    ...
    
    {
        ...
        //  Stay near ship
        dist_to_guardobj =
        vm_vec_normalized_dir(&vec_to_guardobj, &guard_objp->pos,
                              &Pl_objp->pos);
      dot_to_guardobj =
        vm_vec_dot(&Pl_objp->orient.vec.fvec, &vec_to_guardobj);
        ...
        
        {
            ...
            compute_desired_rvec(&rvec, &goal_point, &Pl_objp->pos);
                                  ...
                                  srotation_time, NULL, NULL, 0.0f, 0, &rvec);
           accelerate_ship(aip,
                             accel_scale * (0.25f +
                                            dist_to_goal_point / 700.0f));
        ...
        
        {
          ...
          float speed = guard_objp->phys_info.speed;
            ...
            
            {
                ...
                
                {
                    ...
                    
                    {
                        ...
                        {
                          turn_towards_point(Pl_objp, &goal_point, NULL,
                                              0.0f);
            ...
            
            {
              ...
              float dot =
                turn_towards_tangent(Pl_objp, &guard_objp->pos,
                                     (1.0f +
                                      objval / 2) * guard_objp->radius);
                                   ...
                                   object * repair_objp, int how) 
{
    ...
    
    {
        ...
        if ((repair_objp != NULL)
            && dock_check_find_direct_docked_object(repair_objp,
                                                    repaired_objp))
        ...
        
        {
            ...
            
            {
                ...
                {
                  message_send_builtin_to_player(MESSAGE_SUPPORT_KILLED,
                                                  NULL, MESSAGE_PRIORITY_HIGH,
        ...
        {
          mission_log_add_entry(LOG_PLAYER_ABORTED_REARM,
                                 Ships[repaired_objp->instance].ship_name,
                                 NULL);
...
void ai_cleanup_rearm_mode(object * objp) 
{
    ...
    
    {
      ...
      ai_do_objects_repairing_stuff(&Objects[aip->goal_objnum], objp,
                                     REPAIR_INFO_KILLED);
    ...
    
    {
      ...
      ai_do_objects_repairing_stuff(objp, &Objects[aip->support_ship_objnum],
                                     REPAIR_INFO_ABORT);
    ...
    
    {
        ...
        
        {
            ...
            ai_do_objects_repairing_stuff(objp,
                                           &Objects[aip->support_ship_objnum],
                                           REPAIR_INFO_ABORT);
...
int maybe_dock_obstructed(object * cur_objp, object * goal_objp,
                          int big_only_flag) 
{
    ...
    pp_collide_any(curpos, goalpos, radius, cur_objp, goal_objp,
                   big_only_flag);
...
void ai_dock() 
{
  ...
  else if ((aip->submode == AIS_UNDOCK_0) || (aip->submode == AIS_UNDOCK_1)
    ...
    case AIS_DOCK_0:
      
      {
          ...
          
          {
              ...
              &Ship_info[Ships[goal_objp->instance].ship_info_index];
              ...
              model_get(goal_sip->model_num)->paths[aip->mp_index].name;
                       ...
                       goal_ship_class_name, aip->path_length,
                       goal_dock_path_name);
    ...
    case AIS_DOCK_1:
      
      {
          ...
          
          {
                  ...
                  maybe_avoid_big_ship(Pl_objp, goal_objp, aip,
    ...
    case AIS_DOCK_2:
      
      {
          ...
          
          {
              ...
              //nprintf(("AI", "Time = %7.3f, submode = %i\n", f2fl(Missiontime), aip->submode));
              dist =
              dock_orient_and_approach(Pl_objp, docker_index, goal_objp,
                                       dockee_index, DOA_APPROACH);
            Assert(dist != UNINITIALIZED_VALUE);
             if (dist == DOCK_BACKUP_RETURN_VAL)
              
              {
                Assert(aip->goal_objnum >= 0
                        && aip->goal_objnum < MAX_OBJECTS);
                int path_num;
                 aip->submode = AIS_DOCK_1;
                aip->submode_start_time = Missiontime;
                path_num =
                  ai_return_path_num_from_dockbay(goal_objp, dockee_index);
          ...
          
          {
            ...
            float dist =
              ...
              
              {
                  ...
                  dist =
                  dock_orient_and_approach(Pl_objp, docker_index, goal_objp,
                                           dockee_index, DOA_DOCK);
                Assert(dist != UNINITIALIZED_VALUE);
                 physics_ship_init(Pl_objp);
                 ai_do_objects_docked_stuff(Pl_objp, docker_index,
                                              goal_objp, dockee_index);
                  ...
                  
                  {
                      ...
                      if ((Pl_objp == Player_obj)
                           || (goal_objp == Player_obj))
    ...
    case AIS_DOCK_4A:
      
      {
          ...
          {
            mission_log_add_entry(LOG_SHIP_DOCKED,
                                   Ships[Pl_objp->instance].ship_name,
      ...
      
      {
        ...
        float dist;
          ...
          dock_orient_and_approach(Pl_objp, docker_index, goal_objp,
                                   dockee_index, DOA_UNDOCK_1, &rdinfo);
      ...
      
      {
          ...
          if ((dist < 2.0f)
              ...
              || ((goal_objp->phys_info.speed + 0.005f) >
                  MAX_UNDOCK_ABORT_SPEED))
          
          {
              ...
              {
                ai_do_objects_repairing_stuff(&Objects
                                               [aip->support_ship_objnum],
                                               Pl_objp, REPAIR_INFO_END);
            ...
            aip->submode_start_time = Missiontime;
              ...
              {
                mission_log_add_entry(LOG_SHIP_UNDOCKED,
                                       Ships[Pl_objp->instance].ship_name,
                                       Ships[goal_objp->instance].ship_name);
...
 void ai_debug_render_stuff() 
{
    ...
    
    {
      ...
       ship_get_global_turret_info(&Objects[parent_objnum], tp, &gpos,
                                     &gvec);
...
void get_wing_delta(vec3d * _delta_vec, int wing_index) 
{
    ...
    ((float) column -
     (float) row / 2.0f) * DEFAULT_WING_X_DELTA / DEFAULT_WING_MAG;
  _delta_vec->xyz.y =
    ((float) row +
     (float) bank * 2.25f) * DEFAULT_WING_Y_DELTA / DEFAULT_WING_MAG;
  _delta_vec->xyz.z =
    -((float) row +
      0.5f * (float) bank) * DEFAULT_WING_Z_DELTA / DEFAULT_WING_MAG;
...
void get_absolute_wing_pos(vec3d * result_pos, object * leader_objp,
                           int wing_index, int formation_object_flag) 
{
    ...
    MAX(50.0f,
        3.0f * get_wing_largest_radius(leader_objp,
                                       formation_object_flag) + 15.0f);
  ...
   vm_vec_scale(&wing_delta,
                  wing_spread_size * (1.0f +
                                      leader_objp->phys_info.speed / 70.0f));
...
void get_absolute_wing_pos_autopilot(vec3d * result_pos, object * leader_objp,
                                     int wing_index,
...
int formation_is_leader_chaotic(object * objp) 
{
    ...
    
    {
        ...
        5.0f * (1.0f -
                vm_vec_dot(&objp->orient.vec.fvec,
                           &objp->last_orient.vec.fvec)) * flFrametime;
      udot =
        8.0f * (1.0f -
                vm_vec_dot(&objp->orient.vec.uvec,
                           &objp->last_orient.vec.uvec)) * flFrametime;
...
 void ai_most_massive_object_of_its_wing_of_all_docked_objects_helper(object
                                                                        *
                                                                        objp,
                                                                        dock_function_info
                                                                        *
...
int ai_formation() 
{
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
               turn_towards_point(Pl_objp, &future_goal_point_2, NULL,
                                    0.0f);
            ...
            
            {
                ...
                turn_towards_point(Pl_objp, &future_goal_point_x, NULL,
                                    0.0f);
  ...
  float up_dot =
    ...
    
    {
      ...
       vm_vec_copy_scale(&angular_accel, &Pl_objp->phys_info.max_rotvel,
                           0.2f);
      vm_matrix_interpolate(&leader_objp->orient, &Pl_objp->orient,
                             &Pl_objp->phys_info.rotvel, flFrametime,
                             &new_orient, &w_out,
                             &Pl_objp->phys_info.max_rotvel, &angular_accel,
                             1);
...
void ai_do_repair_frame(object * objp, ai_info * aip, float frametime) 
{
    ...
    
    {
        ...
        Assert(Objects[support_objnum].signature ==
               aip->support_ship_signature);
...
void call_doa(object * child, object * parent) 
{
  ...
  int parent_index = dock_find_dockpoint_used_by_object(parent, child);
    ...
    dock_orient_and_approach(child, child_index, parent, parent_index,
                             DOA_DOCK_STAY);
    ...
    
    {
        ...
        
        {
            ...
            if (shipp->team == Player_ship->team)
              ...
              The_mission.ai_profile->cmeasure_fire_chance[NUM_SKILL_LEVELS /
                                                           2] *
              Ai_classes[aip->ai_class].
              ai_cmeasure_fire_chance[NUM_SKILL_LEVELS / 2];
...
void ai_transfer_shield(object * objp, int quadrant_num) 
{
  ...
   if (objp->shield_quadrant[quadrant_num] +
         (MAX_SHIELD_SECTIONS - 1) * transfer_delta > max_quadrant_strength)
    transfer_delta =
      (max_quadrant_strength -
       objp->shield_quadrant[quadrant_num]) / (MAX_SHIELD_SECTIONS - 1);
...
 void ai_balance_shield(object * objp) 
{
    ...
    if (Ships[objp->instance].ship_max_shield_strength ==
        shield_get_strength(objp))
...
void ai_manage_shield(object * objp, ai_info * aip) 
{
    ...
    
    {
        ...
        
        {
            ...
            The_mission.ai_profile->shield_manage_delay[NUM_SKILL_LEVELS /
                                                        2] *
            Ai_classes[aip->ai_class].
            ai_shield_manage_delay[NUM_SKILL_LEVELS / 2];
        ...
        if (aip->ai_class_autoscale)
          ...
          delay +
          delay * (float) (3 * (Num_ai_classes - aip->ai_class - 1) /
                           (Num_ai_classes - 1));
...
void ai_manage_bay_doors(object * pl_objp, ai_info * aip, bool done) 
{
    ...
    
    {
        ...
        
        {
          ...
          parent_ship->bay_doors_anim_done_time =
            model_anim_get_time_type(parent_ship, TRIGGER_TYPE_DOCK_BAY_DOOR,
    ...
    {
      model_anim_start_type(parent_ship, TRIGGER_TYPE_DOCK_BAY_DOOR,
                             shipp->bay_doors_launched_from, -1);
...
//               0      => success
int ai_acquire_emerge_path(object * pl_objp, int parent_objnum,
                           int allowed_path_mask, vec3d * pos, vec3d * fvec) 
{
    ...
    create_model_exit_path(pl_objp, parent_objp, path_index,
                           ...
                           int allowed_path_mask) 
{
    ...
    model_get(Ship_info[Ships[parent_objp->instance].ship_info_index].
              model_num);
...
void ai_bay_depart() 
{
    ...
    anchor_shipnum =
    ship_name_lookup(Parse_names[Ships[Pl_objp->instance].departure_anchor]);
    ...
    
    {
        ...
        model_get(Ship_info
                  [Ships[Objects[aip->goal_objnum].instance].ship_info_index].
                  model_num);
...
void ai_execute_behavior(ai_info * aip) 
{
    ...
    
    {
        ...
        
        {
          ...
          vm_vec_scale_add(&tvec, &Pl_objp->pos, &Pl_objp->orient.vec.rvec,
                            100.0f);
...
int num_allies_rearming(object * objp) 
{
    ...
    
    {
        ...
        
        {
               ...
               ai_flags & (AIF_REPAIRING | AIF_AWAITING_REPAIR))
  ...
  desire += mrs_subsystem(shipp, SUBSYSTEM_COMMUNICATION);
    ...
    
    {
        ...
        
        {
            ...
            r =
            (float) swp->secondary_bank_ammo[i] /
    ...
    
    {
      ...
      else if (desire >= 3)     //  >= 3 means having a single subsystem fully blown will cause repair.
        {
            ...
            find_nearby_threat(OBJ_INDEX(objp),
                               iff_get_attacker_mask(obj_team(objp)), 2000.0f,
                               &count);
    ...
    
    {
      ...
      if (timestamp_elapsed(aip->force_warp_time)
        ...
        
        {
          ...
          vm_vec_scale_add(&goal_point, &objp->pos, &objp->orient.vec.uvec,
                            100.0f);
      ...
      float speed, goal_speed;
        ...
        speed =
        goal_speed * flFrametime + objp->phys_info.speed * (1.0f -
                                                            flFrametime);
...
void ai_announce_ship_dying(object * dying_objp) 
{
    ...
    
    {
        ...
        
        {
                 ...
                 flags & (SIF_SMALL_SHIP | SIF_FREIGHTER | SIF_TRANSPORT))
    ...
    
    {
      ...
      int pos_set = 0;
        ...
        
        {
            ...
            vm_vec_dotprod(&vec_from_exp, &weapon_objp->orient.vec.fvec);
            ...
            vm_vec_scale_add(safe_pos, &weapon_objp->pos,
                             &weapon_objp->orient.vec.fvec, -50000.0f * dir);
...
void ai_maybe_self_destruct(object * objp, ai_info * aip) 
{
    ...
    if ((Ship_info[Ships[objp->instance].ship_info_index].
         flags & SIF_SMALL_SHIP) && (Ships[objp->instance].wingnum != -1))
    
    {
           ...
           ||
             ...
             (&Ships[objp->instance], SUBSYSTEM_WEAPONS) <= 0.0f))
...
int maybe_big_ship_collide_recover_frame(object * objp, ai_info * aip) 
{
    ...
    
    {
                              ...
                              Ship_info[Ships[objp->instance].
                                        ship_info_index].srotation_time, NULL,
                              NULL, 0.0f, 0, NULL);
...
void ai_frame(int objnum) 
{
    ...
    
    {
        ...
        
        {
            ...
            ~(AIF_BIG_SHIP_COLLIDE_RECOVER_1 |
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                ship_return_next_subsys(&Ships[En_objp->instance],
...
void init_ai_object(int objnum) 
{
    ...
    init_aip_from_class_and_profile(aip,
                                    &Ai_classes[Ship_info[ship_type].
                                                ai_class],
  ...
   aip->choose_enemy_timestamp =
    timestamp(3 * (NUM_SKILL_LEVELS - Game_skill_level) *
              ((rand_alt() % 500) + 500));
...
void init_aip_from_class_and_profile(ai_info * aip, ai_class * aicp,
                                     ai_profile_t * profile) 
{
    ...
    (aicp->ai_cmeasure_fire_chance[Game_skill_level] ==
     FLT_MIN) ? profile->cmeasure_fire_chance[Game_skill_level] : aicp->
    ai_cmeasure_fire_chance[Game_skill_level];
  aip->ai_in_range_time =
    (aicp->ai_in_range_time[Game_skill_level] ==
     FLT_MIN) ? profile->in_range_time[Game_skill_level] : aicp->
    ai_in_range_time[Game_skill_level];
  aip->ai_link_ammo_levels_maybe =
    (aicp->ai_link_ammo_levels_maybe[Game_skill_level] ==
     FLT_MIN) ? profile->link_ammo_levels_maybe[Game_skill_level] : aicp->
    ai_link_ammo_levels_maybe[Game_skill_level];
  aip->ai_link_ammo_levels_always =
    (aicp->ai_link_ammo_levels_always[Game_skill_level] ==
     FLT_MIN) ? profile->link_ammo_levels_always[Game_skill_level] : aicp->
    ai_link_ammo_levels_always[Game_skill_level];
  aip->ai_primary_ammo_burst_mult =
    (aicp->ai_primary_ammo_burst_mult[Game_skill_level] ==
     FLT_MIN) ? profile->primary_ammo_burst_mult[Game_skill_level] : aicp->
    ai_primary_ammo_burst_mult[Game_skill_level];
  aip->ai_link_energy_levels_maybe =
    (aicp->ai_link_energy_levels_maybe[Game_skill_level] ==
     FLT_MIN) ? profile->link_energy_levels_maybe[Game_skill_level] : aicp->
    ai_link_energy_levels_maybe[Game_skill_level];
  aip->ai_link_energy_levels_always =
    (aicp->ai_link_energy_levels_always[Game_skill_level] ==
     FLT_MIN) ? profile->link_energy_levels_always[Game_skill_level] : aicp->
    ai_link_energy_levels_always[Game_skill_level];
  aip->ai_predict_position_delay =
    (aicp->ai_predict_position_delay[Game_skill_level] ==
     INT_MIN) ? profile->predict_position_delay[Game_skill_level] : aicp->
    ai_predict_position_delay[Game_skill_level];
  aip->ai_shield_manage_delay =
    (aicp->ai_shield_manage_delay[Game_skill_level] ==
     FLT_MIN) ? profile->shield_manage_delay[Game_skill_level] : aicp->
    ai_shield_manage_delay[Game_skill_level];
  aip->ai_ship_fire_delay_scale_friendly =
    (aicp->ai_ship_fire_delay_scale_friendly[Game_skill_level] ==
     FLT_MIN) ? profile->
    ship_fire_delay_scale_friendly[Game_skill_level] : aicp->
    ai_ship_fire_delay_scale_friendly[Game_skill_level];
  aip->ai_ship_fire_delay_scale_hostile =
    (aicp->ai_ship_fire_delay_scale_hostile[Game_skill_level] ==
     FLT_MIN) ? profile->
    ship_fire_delay_scale_hostile[Game_skill_level] : aicp->
    ai_ship_fire_delay_scale_hostile[Game_skill_level];
  aip->ai_ship_fire_secondary_delay_scale_friendly =
    (aicp->ai_ship_fire_secondary_delay_scale_friendly[Game_skill_level] ==
     FLT_MIN) ? profile->
    ship_fire_secondary_delay_scale_friendly[Game_skill_level] : aicp->
    ...
    (aicp->ai_ship_fire_secondary_delay_scale_hostile[Game_skill_level] ==
     FLT_MIN) ? profile->
    ship_fire_secondary_delay_scale_hostile[Game_skill_level] : aicp->
    ...
    (aicp->ai_turn_time_scale[Game_skill_level] ==
     FLT_MIN) ? profile->turn_time_scale[Game_skill_level] : aicp->
    ai_turn_time_scale[Game_skill_level];
  aip->ai_glide_attack_percent =
    (aicp->ai_glide_attack_percent[Game_skill_level] ==
     FLT_MIN) ? profile->glide_attack_percent[Game_skill_level] : aicp->
    ai_glide_attack_percent[Game_skill_level];
  aip->ai_circle_strafe_percent =
    (aicp->ai_circle_strafe_percent[Game_skill_level] ==
     FLT_MIN) ? profile->circle_strafe_percent[Game_skill_level] : aicp->
    ai_circle_strafe_percent[Game_skill_level];
  aip->ai_glide_strafe_percent =
    (aicp->ai_glide_strafe_percent[Game_skill_level] ==
     FLT_MIN) ? profile->glide_strafe_percent[Game_skill_level] : aicp->
    ai_glide_strafe_percent[Game_skill_level];
  aip->ai_stalemate_time_thresh =
    (aicp->ai_stalemate_time_thresh[Game_skill_level] ==
     FLT_MIN) ? profile->stalemate_time_thresh[Game_skill_level] : aicp->
    ai_stalemate_time_thresh[Game_skill_level];
  aip->ai_stalemate_dist_thresh =
    (aicp->ai_stalemate_dist_thresh[Game_skill_level] ==
     FLT_MIN) ? profile->stalemate_dist_thresh[Game_skill_level] : aicp->
    ai_stalemate_dist_thresh[Game_skill_level];
  aip->ai_chance_to_use_missiles_on_plr =
    (aicp->ai_chance_to_use_missiles_on_plr[Game_skill_level] ==
     INT_MIN) ? profile->
    chance_to_use_missiles_on_plr[Game_skill_level] : aicp->
    ai_chance_to_use_missiles_on_plr[Game_skill_level];
  aip->ai_max_aim_update_delay =
    (aicp->ai_max_aim_update_delay[Game_skill_level] ==
     FLT_MIN) ? profile->max_aim_update_delay[Game_skill_level] : aicp->
    ai_max_aim_update_delay[Game_skill_level];
...
 }  void ai_do_default_behavior(object * obj) 
{
    ...
    
    {
        ...
        find_enemy(OBJ_INDEX(obj), 1000.0f,
                   The_mission.ai_profile->max_attackers[Game_skill_level]);
...
 extern void ship_set_subsystem_strength(ship * shipp, int type,
                                           float strength);
 
//  Object *objp_weapon, fired by *objp_hitter, hit object *objp_ship.
void maybe_process_friendly_hit(object * objp_hitter, object * objp_hit,
                                object * objp_weapon) 
{
    ...
    
    {
        ...
        // wacky stuff here
        ship_info * sip =
        &Ship_info[Ships[objp_hit->instance].ship_info_index];
      ...
      float accredited_damage =
        MIN(MAX_BURST_DAMAGE,
            pp->damage_this_burst + damage) - pp->damage_this_burst;
        ...
        nprintf(("AI",
                 ...
                 pp->friendly_damage,
                 FRIENDLY_DAMAGE_THRESHOLD * (1.0f +
                                              (float) (NUM_SKILL_LEVELS + 1 -
                                                       Game_skill_level) /
                                              3.0f), pp->damage_this_burst,
                 MAX_BURST_DAMAGE));
...
{
  Assert(Ship_info[Ships[aip->shipnum].ship_info_index].
          flags & (SIF_FIGHTER | SIF_BOMBER));
...
void big_ship_collide_recover_start(object * objp, object * big_objp,
                                    vec3d * collide_pos,
...
 void ai_update_lethality(object * ship_obj, object * other_obj,
                            float damage) 
{
        ...
        
        {
            ...
            
            {
                ...
                
                {
                    ...
                    if (Ship_info
                        [Ships[Objects[parent].instance].ship_info_index].
                        flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP))
...
void ai_ship_hit(object * objp_ship, object * hit_objp, vec3d * hitpos,
                 int shield_quadrant, vec3d * hit_normal) 
{
    ...
    
    {
        ...
        
        {
            ...
            if (!
                (Ship_info[Ships[objp_ship->instance].ship_info_index].
                 flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP)))
    ...
    
    {
        ...
        
        {
               ...
               (objp_ship, OBJ_INDEX(hit_objp), 1))
        ...
        
        {
               ...
               (&objp_ship->pos, &Objects[aip->guard_objnum].pos) > 500.0f)
...
void ai_ship_destroy(int shipnum, int method) 
{
  ...
  Assert((Ships[shipnum].ai_index >= 0)
...
void ai_deathroll_start(object * dying_objp) 
{
    ...
    
    {
      ...
       dock_dead_dock_objects(dying_objp, docker_index, docked_objp,
                                dockee_index);
...
int ai_issue_rearm_request(object * requester_objp) 
{
    ...
    
    {
                ...
                &Ships[Objects[requester_aip->support_ship_objnum].instance].
                ship_name));
...
void ai_rearm_repair(object * objp, int docker_index, object * goal_objp,
                     int dockee_index) 
{
    ...
    ai_dock_with_object(objp, docker_index, goal_objp, dockee_index,
                         AIDO_DOCK);
  ...
   goal_aip->abort_rearm_timestamp =
    timestamp(NEXT_REARM_TIMESTAMP * 3 / 2);
...
int ai_return_path_num_from_dockbay(object * dockee_objp, int dockbay_index) 
{
    ...
    
    {
        ...
        model_get(Ship_info[Ships[dockee_objp->instance].ship_info_index].
                  model_num);
              ...
              time -= 2 * 60;   //  Subtract off two minutes.
                ...
                
                {
                    ...
                    
                    {
                        ...
                        num_nearby_fighters(iff_get_attackee_mask
                                            (obj_team(objp)), &objp->pos,
                                            1500.0f);
