...
int bomb_headed_towards_ship(object * bomb_objp, object * ship_objp)
{
  ...
  vm_vec_normalized_dir(&bomb_to_ship_vector, &ship_objp->pos,
                        &bomb_objp->pos);
...
weapon_info *get_turret_weapon_wip(ship_weapon * swp, int weapon_num)
{
  ...
  if (weapon_num >= MAX_SHIP_PRIMARY_BANKS)
    return &Weapon_info[swp->
                        secondary_bank_weapons[weapon_num -
...
int get_turret_weapon_next_fire_stamp(ship_weapon * swp, int weapon_num)
{
  ...
  if (weapon_num >= MAX_SHIP_PRIMARY_BANKS)
    return swp->next_secondary_fire_stamp[weapon_num -
                                          MAX_SHIP_PRIMARY_BANKS];
  else
  ...
  if ((objp->type == OBJ_SHIP))
    {
      ...
      if ((sip->class_type >= 0)
          && !(Ship_types[sip->class_type].ai_bools & STI_AI_TURRETS_ATTACK))
        {
          return 0;
        }
  ...
  if (objp->type == OBJ_WEAPON)
    {
      ...
      if ((!(wip->wi_flags & WIF_BOMB)
           && !(Ai_info[Ships[turret_parent->instance].ai_index].
                ai_profile_flags & AIPF_ALLOW_TURRETS_TARGET_WEAPONS_FREELY)))
        {
          return 0;
        }
...
void evaluate_obj_as_target(object * objp, eval_enemy_obj_struct * eeo)
{
  ...
  if ((Ai_info[Ships[turret_parent_obj->instance].ai_index].
       ai_profile_flags & AIPF_PREVENT_TARGETING_BOMBS_BEYOND_RANGE)
          ...
          if (dist_comp < eeo->nearest_homing_bomb_dist)
            {
              if ((eeo->current_enemy == -1)
                  || object_in_turret_fov(objp, ss, eeo->tvec, eeo->tpos,
                                          dist + objp->radius))
                {
      ...
      if (eeo->turret_parent_objnum == asteroid_collide_objnum(objp))
        {
          ...
          dist_comp *=
            0.9f + (0.01f * Asteroids[objp->instance].collide_time_to_impact);
...
int get_nearest_turret_objnum(int turret_parent_objnum,
                              ship_subsys * turret_subsys,
                              int enemy_team_mask, vec3d * tpos, vec3d * tvec,
                              int current_enemy, bool big_only_flag,
                              bool small_only_flag, bool tagged_only_flag,
                              bool beam_flag)
{
  ...
  if (ship_get_subsystem_strength
      (&Ships[Objects[turret_parent_objnum].instance], SUBSYSTEM_WEAPONS) > 0)
    {
      weapon_system_ok = 1;
    }
      ...
      for (int i = 0; i < n_tgt_priorities; i++)
        {
          ...
          if (priority_weapon_idx == -1)
            ...
            tt =
              &Ai_tp_list[Weapon_info[priority_weapon_idx].
          ...
          while (ptr != END_OF_LIST(&obj_used_list))
            {
              ...
              if (((tt->wif2_flags != 0) || (tt->wif_flags != 0))
                  && (ptr->type == OBJ_WEAPON))
                {
                  if (((Weapon_info[Weapons[ptr->instance].weapon_info_index].
                        wi_flags & tt->wif_flags) == tt->wif_flags)
                      &&
                      ((Weapon_info[Weapons[ptr->instance].weapon_info_index].
                        wi_flags2 & tt->wif2_flags) == tt->wif2_flags))
                    {
                      found_something = true;
                    }
                }

              if (((tt->sif_flags != 0) || (tt->sif2_flags != 0))
                  && (ptr->type == OBJ_SHIP))
                {
                  if (((Ship_info[Ships[ptr->instance].ship_info_index].
                        flags & tt->sif_flags) == tt->sif_flags)
                      &&
                      ((Ship_info[Ships[ptr->instance].ship_info_index].
                        flags2 & tt->sif2_flags) == tt->sif2_flags))
                    {
                      found_something = true;
                    }
                }
      ...
      for (int i = 0; i < NUM_TURRET_ORDER_TYPES; i++)
        {
          ...
          switch (turret_subsys->turret_targeting_order[i])
            ...
            case 0:
              ...
              if (!
                  ((aip->
                    ai_profile_flags & AIPF_HUGE_TURRET_WEAPONS_IGNORE_BOMBS)
                   && big_only_flag))
                {
                  // Missile_obj_list
                  for (mo = GET_FIRST(&Missile_obj_list);
                       ...
                       mo = GET_NEXT(mo))
                    {
                      ...
                      if (Weapon_info
                          [Weapons[objp->instance].weapon_info_index].
                          wi_flags & WIF_BOMB)
                        {
            ...
            case 1:
              ...
              Assert(eeo.nearest_attacker_objnum < 0
                     || is_target_beam_valid(swp,
                                             &Objects[eeo.
                                                      nearest_attacker_objnum]));
            ...
            case 2:
              ...
              // - AAA beams
              if (!all_turret_weapons_have_flags
                  (swp, WIF_HUGE | WIF_FLAK | WIF_HOMING | WIF_SPAWN))
                {
                  // Asteroid_obj_list
...
//              tvec                => forward vector of turret (world coords)
//              current_enemy   =>  objnum of current turret target
     int find_turret_enemy(ship_subsys * turret_subsys, int objnum,
                           vec3d * tpos, vec3d * tvec, int current_enemy)
{
  ...
  bool big_only_flag =
    all_turret_weapons_have_flags(&turret_subsys->weapons, WIF_HUGE);
  bool small_only_flag =
    all_turret_weapons_have_flags2(&turret_subsys->weapons, WIF2_SMALL_ONLY);
  bool tagged_only_flag =
    all_turret_weapons_have_flags2(&turret_subsys->weapons, WIF2_TAGGED_ONLY)
    || (turret_subsys->weapons.flags & SW_FLAG_TAGGED_ONLY);
  bool beam_flag =
    all_turret_weapons_have_flags(&turret_subsys->weapons, WIF_BEAM);
  ...
  if ((Ship_types[sip->class_type].ship_bools & STI_TURRET_TGT_SHIP_TGT)
      && (aip->target_objnum != -1))
    {
      ...
      if (Objects[target_objnum].signature == aip->target_signature)
        {
          if (iff_matches_mask
              (Ships[Objects[target_objnum].instance].team, enemy_team_mask))
            {
  ...
  enemy_objnum =
    get_nearest_turret_objnum(objnum, turret_subsys, enemy_team_mask, tpos,
                              tvec, current_enemy, big_only_flag,
                              small_only_flag, tagged_only_flag, beam_flag);
  if (enemy_objnum >= 0)
    {
      Assert(!
             ((Objects[enemy_objnum].flags & OF_BEAM_PROTECTED)
              && beam_flag));
...
//  in global space.
void ship_get_global_turret_info(object * objp, model_subsystem * tp,
                                 vec3d * gpos, vec3d * gvec)
{
...
void ship_get_global_turret_gun_info(object * objp, ship_subsys * ssp,
                                     vec3d * gpos, vec3d * gvec,
                                     int use_angles, vec3d * targetp)
{
  vec3d *gun_pos;
  ...
  gun_pos =
    &tp->turret_firing_point[ssp->turret_next_fire_pos %
...
int aifft_rotate_turret(ship * shipp, int parent_objnum, ship_subsys * ss,
                        object * objp, object * lep,
                        vec3d * predicted_enemy_pos, vec3d * gvec)
{
  ...
  if (ss->turret_enemy_objnum != -1)
    {
          ...
          if (ss->turret_enemy_objnum != -1)
            {
              ...
              vm_vec_add2(&enemy_point,
                          &Objects[ss->turret_enemy_objnum].pos);
            }
        }
      else
        {
          if ((lep->type == OBJ_SHIP)
              && (Ship_info[Ships[lep->instance].ship_info_index].
                  flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP)))
            {
              ai_big_pick_attack_point_turret(lep, ss, &gun_pos, &gun_vec,
                                              &enemy_point, tp->turret_fov,
                                              MIN(wip->max_speed *
                                                  wip->lifetime,
                                                  wip->weapon_range));
            }
      ...
      set_predicted_enemy_pos_turret(predicted_enemy_pos, &gun_pos, objp,
                                     &enemy_point, &target_moving_direction,
                                     wip->max_speed,
      ...
      if (weapon_system_strength < 0.7f)
        {
          ...
          vm_vec_scale_add2(predicted_enemy_pos, &rand_vec,
                            (1.0f -
                             weapon_system_strength) * 1.5f * lep->radius);
        }
      ...
      if (in_fov)
        {
          ret_val =
            model_rotate_gun(Ship_info[shipp->ship_info_index].model_num, tp,
                             &Objects[parent_objnum].orient,
                             &ss->submodel_info_1.angs,
                             &ss->submodel_info_2.angs,
                             &Objects[parent_objnum].pos, predicted_enemy_pos,
                             shipp->objnum);
        }
      else if ((tp->flags & MSS_FLAG_TURRET_RESET_IDLE)
  ...
  else if ((ss->system_info->flags & MSS_FLAG_TURRET_RESET_IDLE)
           && (timestamp_elapsed(ss->rotation_timestamp)))
    {
      ret_val =
        model_rotate_gun(Ship_info[shipp->ship_info_index].model_num,
...
float aifft_compute_turret_dot(object * objp, object * enemy_objp,
                               vec3d * abs_gunposp,
                               ...
                               ship_subsys * enemy_subsysp)
{
  ...
  if (ship_subsystem_in_sight
       ...
       &vector_out))
    {
      ...
      vm_vec_unrotate(&turret_norm, &turret_subsysp->system_info->turret_norm,
                      &objp->orient);
      float dot_return = vm_vec_dot(&turret_norm, &vector_out);

      if (Ai_info[Ships[objp->instance].ai_index].
          ai_profile_flags & AIPF_SMART_SUBSYSTEM_TARGETING_FOR_TURRETS)
        {
...
ship_subsys *aifft_find_turret_subsys(object * objp, ship_subsys * ssp,
                                      object * enemy_objp, float *dot_out)
{
  ship *eshipp, *shipp;
  ship_info *esip;
  ...
  *dot_out = best_dot;
  ...
  int stride =
    aifft_list_size >
    aifft_max_checks ? aifft_list_size / aifft_max_checks : 1;
  if (stride <= 0)
    {
      stride = 1;
    }
  ...
  for (idx = offset; idx < aifft_list_size; idx += stride)
    {
      dot =
        aifft_compute_turret_dot(objp, enemy_objp, &abs_gun_pos, ssp,
                                 aifft_list[idx]);
...
void turret_set_next_fire_timestamp(int weapon_num, weapon_info * wip,
                                    ship_subsys * turret, ai_info * aip)
{
  Assert(weapon_num < MAX_SHIP_WEAPONS);
  float wait = 1000.0f;
  ...
  if (weapon_num < MAX_SHIP_PRIMARY_BANKS)
    ...
    fs_dest =
      &turret->weapons.next_secondary_fire_stamp[weapon_num -
                                                 MAX_SHIP_PRIMARY_BANKS];

  //Check for the new cooldown flag
  if (!
      ((wip->wi_flags2 & WIF2_SAME_TURRET_COOLDOWN)
       || ((wip->burst_shots > 0) && (wip->burst_flags & WBF_FAST_FIRING))))
    {
...
int turret_should_fire_aspect(ship_subsys * turret, float dot,
                              weapon_info * wip)
{
                        ...
                        NULL, float flak_range_override = 100.0f)
{
  ...
  if ((!timestamp_elapsed
       (get_turret_weapon_next_fire_stamp(&turret->weapons, weapon_num)))
      && last_shot_in_salvo)
    return false;
  ...
  if (check_ok_to_fire(parent_objnum, turret->turret_enemy_objnum, wip))
    {
      ...
      // now do anything else
        ...
        {
          int new_group_id = (Detail.lighting > 3
                              && wip->render_type ==
                              WRT_LASER) ? weapon_create_group_id() : -1;
          ...
          for (int i = 0; i < wip->shots; i++)
            {
              ...
              weapon_objnum =
                weapon_create(turret_pos, &turret_orient, turret_weapon_class,
                              parent_objnum, new_group_id, 1);
                  ...
                  if (wip->wi_flags & WIF_FLAK)
                    {
                      ...
                      flak_muzzle_flash(turret_pos, turret_fvec,
                                        &Objects[parent_ship->objnum].
                                        phys_info, turret_weapon_class);

                      if (predicted_pos != NULL)
                        {
                          // pick a firing range so that it detonates properly
                          flak_pick_range(objp, turret_pos, predicted_pos,
                    ...
                    {
                      mflash_create(turret_pos, turret_fvec,
                  ...
                  if (MULTIPLAYER_MASTER && (weapon_objnum != -1))
                    {
                      ...
                      if (wip->wi_flags & WIF_FLAK)
                        {
                          send_flak_fired_packet(parent_objnum, subsys_index,
                                                 weapon_objnum, flak_range);
                        }
                      else
                        {
                          send_turret_fired_packet(parent_objnum,
                                                   subsys_index,
                                                   weapon_objnum);
                        }
                    }
                  ...
                  if (wip->launch_snd != -1)
                    {
                      ...
                      if (parent_objnum != OBJ_INDEX(Player_obj))
                        {
                          snd_play_3d(&Snds[wip->launch_snd], turret_pos,
                                      &View_position);
                        }
                    }
                }
  ...
  //Not useful -WMC
    ...
    if (!
        (parent_aip->
         ai_profile_flags & AIPF_DONT_INSERT_RANDOM_TURRET_FIRE_DELAY)
        && last_shot_in_salvo)
    {
...
void turret_swarm_fire_from_turret(turret_swarm_info * tsi)
{
  ...
  //  change firing point
  ship_get_global_turret_gun_info(&Objects[tsi->parent_objnum], tsi->turret,
                                  &turret_pos, &turret_fvec, 1, NULL);
  ...
  Assert((Weapon_info[tsi->weapon_class].wi_flags & WIF_SWARM)
         || (Weapon_info[tsi->weapon_class].wi_flags & WIF_CORKSCREW));
  ...
  if ((Weapon_info[tsi->weapon_class].subtype == WP_LASER)
      ||
      ((The_mission.ai_profile->
        flags & AIPF_HACK_IMPROVE_NON_HOMING_SWARM_TURRET_FIRE_ACCURACY)
  ...
  int new_group_id = (Detail.lighting > 3
                      && Weapon_info[tsi->weapon_class].render_type ==
                      WRT_LASER) ? weapon_create_group_id() : -1;

  weapon_objnum =
    weapon_create(&turret_pos, &turret_orient, tsi->weapon_class,
                  tsi->parent_objnum, new_group_id, 1);
  weapon_set_tracking_info(weapon_objnum, tsi->parent_objnum,
                           tsi->target_objnum, 1, tsi->target_subsys);
      ...
      if (Weapon_info[tsi->weapon_class].launch_snd != -1)
        {
          ...
          if (tsi->parent_objnum != OBJ_INDEX(Player_obj))
            {
              snd_play_3d(&Snds[Weapon_info[tsi->weapon_class].launch_snd],
      ...
      if (MULTIPLAYER_MASTER && (weapon_objnum != -1))
        {
          ...
          Assert(subsys_index != -1);
          send_turret_fired_packet(tsi->parent_objnum, subsys_index,
                                   weapon_objnum);
        }
    }
}
...
void ai_fire_from_turret(ship * shipp, ship_subsys * turret,
                         int parent_objnum)
{
  ...
  for (checked_weapon = 0;
       (checked_weapon < turret_weapon->num_primary_banks)
       && !has_valid_weapon; checked_weapon++)
    {
  ...
  if (!has_valid_weapon)
    {
      for (checked_weapon = 0;
           (checked_weapon < turret->weapons.num_secondary_banks)
           && !has_valid_weapon; checked_weapon++)
        {
  ...
  if (all_turret_weapons_have_flags(turret_weapon, WIF_BEAM)
      && !(turret_weapon->flags & SW_FLAG_BEAM_FREE))
    {
      return;
    }
  ...
  if (turret->turret_animation_position == MA_POS_NOT_SET)
    {
      if (model_anim_start_type
          (shipp, TRIGGER_TYPE_TURRET_FIRING, turret->system_info->subobj_num,
           1))
        {
          turret->turret_animation_done_time =
            model_anim_get_time_type(shipp, TRIGGER_TYPE_TURRET_FIRING,
  ...
  bool has_invalid_signatrue =
    (turret->turret_enemy_sig !=
     Objects[turret->turret_enemy_objnum].signature);
  ...
  if (!(turret_system_info->flags & MSS_FLAG_TURRET_MATRIX))
    {
      if (!
          (turret_system_info->turret_gun_sobj ==
           turret_system_info->subobj_num))
        {
          model_make_turret_matrix(Ship_info[shipp->ship_info_index].
                                   model_num, turret_system_info);
        }
    }

  // Use the turret info for all guns, not one gun in particular.
  vec3d global_turret_fvec, global_turret_pos;
  ship_get_global_turret_info(&Objects[parent_objnum], turret_system_info,
                              &global_turret_pos, &global_turret_fvec);

  if (turret_system_info->flags & MSS_FLAG_TURRET_ALT_MATH)
    {
      vm_matrix_x_matrix(&turret->world_to_turret_matrix,
                         &Objects[parent_objnum].orient,
                         &turret_system_info->turret_matrix);
    }

  // Rotate the turret even if time hasn't elapsed, since it needs to turn to face its target.
  int use_angles =
    aifft_rotate_turret(shipp, parent_objnum, turret, objp, last_enemy,
                        &predicted_enemy_pos, &global_turret_fvec);

  if ((turret_system_info->flags & MSS_FLAG_FIRE_ON_TARGET)
      && (turret->points_to_target >= 0.0f))
    {
  ...
  if (last_enemy != NULL)
    {
      if (The_mission.ai_profile->flags2 & AIPF2_TURRETS_IGNORE_TARGET_RADIUS)
        dist_to_enemy =
          MAX(0,
              vm_vec_normalized_dir(&vector_to_enemy, &predicted_enemy_pos,
                                    &global_turret_pos));
      else
        dist_to_enemy =
          MAX(0,
              vm_vec_normalized_dir(&vector_to_enemy, &predicted_enemy_pos,
                                    &global_turret_pos) - last_enemy->radius);
    }
  ...
  int num_ships_nearby =
    num_nearby_fighters(iff_get_attackee_mask
                        (obj_team(&Objects[parent_objnum])),
  ...
  for (i = 0; i < (MAX_SHIP_WEAPONS); i++)
    {
      ...
      if (num_valid_weapons > 0
          && !(turret_system_info->flags & MSS_FLAG_USE_MULTIPLE_GUNS))
        break;
      ...
      if (i < MAX_SHIP_PRIMARY_BANKS)
      ...
      else
        {
          ...
          if (!timestamp_elapsed
              (turret_weapon->next_secondary_fire_stamp[secnum]))
            continue;
      ...
      if ((wip->wi_flags & WIF_SPAWN) && !(wip->wi_flags2 & WIF2_SMART_SPAWN))
        {
          if ((num_ships_nearby >= 3)
            ...
            {
              turret_fire_weapon(i, turret, parent_objnum, &global_turret_pos,
                                 &turret->turret_last_fire_direction);
            }
          else
            {
              turret->turret_next_fire_stamp = timestamp(1000); //  Regardless of firing rate, don't check whether should fire for awhile.
            }
      ...
      if (last_enemy != NULL)
        {
          if ((dist_to_enemy <= weapon_firing_range)
              && (dist_to_enemy >= weapon_minimum_range))
            {
              if (wip->wi_flags & WIF_HUGE)
                {
                  ...
                  if (!
                      (Ship_info[Ships[last_enemy->instance].ship_info_index].
                       flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP)))
                    continue;
                }
              ...
              if (last_enemy->type == OBJ_SHIP)
                {
                  ...
                  if ((last_enemy->flags & OF_BEAM_PROTECTED)
                      && (wip->wi_flags & WIF_BEAM))
                    {
                      turret->turret_enemy_objnum = -1;
                      turret->turret_time_enemy_in_range = 0.0f;
                      continue;
              ...
              else
                {
                  ...
                  if ((wip->wi_flags2 & WIF2_TAGGED_ONLY)
                      || (turret->weapons.flags & SW_FLAG_TAGGED_ONLY))
                    continue;
                }
  ...
  if (turret_should_pick_new_target(turret))
    {
      ...
      int obj_possible_target =
        find_turret_enemy(turret, parent_objnum, &global_turret_pos,
                          &global_turret_fvec, turret->turret_enemy_objnum);

      bool has_found_target = obj_possible_target >= 0;
      bool is_beam_valid =
        is_target_beam_valid(&turret->weapons,
                             &Objects[obj_possible_target]) != 0;
          ...
          if (turret->turret_enemy_objnum == -1)
            {
              ...
              turret->turret_enemy_sig =
                Objects[obj_possible_target].signature;
              // Why return?
              return;
            }
          else
      ...
      if (turret->turret_enemy_objnum != -1)
        {
          ...
          if ((last_enemy->type == OBJ_SHIP)
            ...
            {
              turret->targeted_subsys =
                aifft_find_turret_subsys(objp, turret, last_enemy, &dot);
            }

          turret->turret_next_enemy_check_stamp =
            timestamp((int) (MAX(dot, 0.5f) * 1000.0f) + 1000);
        }
      else
        {
          // Check every second.
          turret->turret_next_enemy_check_stamp =
            timestamp((int) (1000.0f * frand_range(0.9f, 1.1f)));
        }
    }
  ...
  if (turret->turret_enemy_objnum != -1)
    {
      ...
      if (dock_check_find_docked_object
          (objp, &Objects[turret->turret_enemy_objnum]))
        {
          turret->turret_enemy_objnum = -1;
          return;
        }
      ...
      if ((aip->mode == AIM_DOCK)
          && (aip->goal_objnum == turret->turret_enemy_objnum))
        {
          turret->turret_enemy_objnum = -1;
          return;
        }
      ...
      for (i = 0; i < number_of_firings; i++)
        {
          if (turret_system_info->flags & MSS_FLAG_TURRET_FIXED_FP)
            {
              ...
              if (ffp_bank < MAX_SHIP_PRIMARY_BANKS)
                ...
                {
                  ffp_pos =
                    ffp_bank - MAX_SHIP_PRIMARY_BANKS +
                    turret->weapons.num_primary_banks;
                }

              turret->turret_next_fire_pos = ffp_pos;
            }

          ship_get_global_turret_gun_info(&Objects[parent_objnum], turret,
                                          &global_turret_pos,
                                          &global_turret_fvec, use_angles,
                                          &predicted_enemy_pos);

          // Fire in the direction the turret is facing, not right at the target regardless of turret dir.
          vm_vec_sub(&vector_to_enemy, &predicted_enemy_pos,
                     &global_turret_pos);
          ...
          if (wip->wi_flags & WIF_FLAK)
            {
              flak_jitter_aim(&tv2e, dist_to_enemy,
          ...
          if (The_mission.ai_profile->
              flags & AIPF_USE_ONLY_SINGLE_FOV_FOR_TURRETS)
            {
          ...
          if (!(wip->wi_flags & WIF_HOMING))
          ...
          else if (wip->wi_flags & WIF_HOMING_JAVELIN)
            {
              ...
              if (turret_should_fire_aspect(turret, dot, wip) &&
                  ship_get_closest_subsys_in_sight(&Ships
                                                   [last_enemy->signature],
                                                   SUBSYSTEM_ENGINE,
                                                   &global_turret_pos))
                {
                  ok_to_fire = true;
          ...
          if (ok_to_fire
              && (turret_system_info->flags & MSS_FLAG_TURRET_HULL_CHECK))
            {
              ...
              hull_check.submodel_num =
                model_get(hull_check.model_num)->detail[0];
              ...
              if (model_collide(&hull_check))
                {
                  float hit_dist =
                    vm_vec_dist(hull_check.p0, &hull_check.hit_point_world);
          ...
          if (ok_to_fire)
            {
              ...
              if (turret_system_info->flags & MSS_FLAG_TURRET_SALVO)
                turret_fire_weapon(valid_weapons[0], turret,
                                   parent_objnum, &global_turret_pos, &tv2e,
                                   &predicted_enemy_pos);
              else
              ...
              if (i < MAX_SHIP_PRIMARY_BANKS)
                {
                  ...
                  if (turret_weapon->next_primary_fire_stamp[i] <
                      turret->turret_next_fire_stamp)
                    {
                      turret->turret_next_fire_stamp =
                        turret_weapon->next_primary_fire_stamp[i];
                    }
                }
              else
                {
                  //timestamp range is 2 to INT_MAX/2
                  if (turret_weapon->
                      next_secondary_fire_stamp[i - MAX_SHIP_PRIMARY_BANKS] <
                      2)
                    continue;
                  ...
                  if (turret_weapon->
                    ...
                    {
                      turret->turret_next_fire_stamp =
                        turret_weapon->next_secondary_fire_stamp[i -
                                                                 MAX_SHIP_PRIMARY_BANKS];
                    }
                }
            }
...
bool turret_std_fov_test(ship_subsys * ss, vec3d * gvec, vec3d * v2e,
                         float size_mod)
{
  model_subsystem *tp = ss->system_info;
...
bool turret_adv_fov_test(ship_subsys * ss, vec3d * gvec, vec3d * v2e,
                         float size_mod)
{
  model_subsystem *tp = ss->system_info;
  float dot = vm_vec_dot(v2e, gvec);
...
bool turret_fov_test(ship_subsys * ss, vec3d * gvec, vec3d * v2e,
                     float size_mod)
{
  bool in_fov = false;
