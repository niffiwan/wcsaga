    ...
    
    {
        ...
        if ((sip->class_type >= 0)
            && !(Ship_types[sip->class_type].
                 ai_bools & STI_AI_TURRETS_ATTACK))
    ...
    
    {
      ...
      float active_lethality = aip->lethality;
        ...
        
        {
                       ...
                       0.01f * Lethality_range_const * active_lethality);
        ...
        
        {
            ...
            
            {
                ...
                
                {
                         ...
                         [Weapons[ptr->instance].weapon_info_index].
                         wi_flags & tt->wif_flags) ==
                        tt->
                        wif_flags) 
                       &&((Weapon_info
                           [Weapons[ptr->instance].weapon_info_index].
                               ...
                               ship_subsys * enemy_subsysp) 
{
    ...
    
    {
      ...
       vm_vec_unrotate(&turret_norm,
                         &turret_subsysp->system_info->turret_norm,
                         &objp->orient);
...
void ai_fire_from_turret(ship * shipp, ship_subsys * ss, int parent_objnum) 
{
  ...
  int weap_check;
    ...
    
    {
           ...
           (shipp, TRIGGER_TYPE_TURRET_FIRING, ss->system_info->subobj_num,
            1))
    ...
    Num_ai_firing++;
         ...
         || (ss->turret_enemy_sig !=
             Objects[ss->turret_enemy_objnum].signature))
    ...
    {
      vm_matrix_x_matrix(&ss->world_to_turret_matrix,
                          &Objects[parent_objnum].orient, &tp->turret_matrix);
    }
   
    // Rotate the turret even if time hasn't elapsed, since it needs to turn to face its target.
  int use_angles =
    aifft_rotate_turret(shipp, parent_objnum, ss, objp, lep,
                        &predicted_enemy_pos, &gvec);
   if ((tp->flags & MSS_FLAG_FIRE_ON_TARGET)
  ...
  float dist_to_enemy = 0.0f;
    ...
    
    {
           ...
           flags2 & AIPF2_TURRETS_IGNORE_TARGET_RADIUS)
        dist_to_enemy =
          MAX(0, vm_vec_normalized_dir(&v2e, &predicted_enemy_pos, &gpos));
      
      else
        dist_to_enemy =
          MAX(0,
              vm_vec_normalized_dir(&v2e, &predicted_enemy_pos,
                                    &gpos) - lep->radius);
    ...
    num_nearby_fighters(iff_get_attackee_mask
                        (obj_team(&Objects[parent_objnum])), &gpos, 1500.0f);
    ...
    
    {
        ...
        if ((wip->wi_flags & WIF_SPAWN)
            && !(wip->wi_flags2 & WIF2_SMART_SPAWN))
        
        {
            ...
            {
              turret_fire_weapon(i, ss, parent_objnum, &gpos,
            ...
            
            {
                ...
                
                {
                    ...
                    if ((wip->wi_flags2 & WIF2_TAGGED_ONLY)
                        || (ss->weapons.flags & SW_FLAG_TAGGED_ONLY))
    ...
    
    {
        ...
        find_turret_enemy(ss, parent_objnum, &gpos, &gvec,
    ...
    
    {
      ...
      int number_of_firings;
        ...
        
        {
          ...
           ship_get_global_turret_gun_info(&Objects[parent_objnum], ss,
                                             &gpos, &gvec, use_angles,
                                             &predicted_enemy_pos);
            ...
            
            {
                ...
                if (turret_should_fire_aspect(ss, dot, wip)
                    &&
                    ship_get_closest_subsys_in_sight(&Ships[lep->signature],
                                                      SUBSYSTEM_ENGINE,
                                                      &gpos))
            ...
            
            {
              ...
               vm_vec_scale_add(&end, &gpos, &gvec,
                                  model_get_radius(model_num));
            ...
            
            {
                ...
                if (tp->flags & MSS_FLAG_TURRET_SALVO)
                turret_fire_weapon(valid_weapons[0], ss, parent_objnum,
                                    &gpos, &tv2e, &predicted_enemy_pos);
              
              else
                turret_fire_weapon(valid_weapons[i], ss, parent_objnum,
                                    &gpos, &tv2e, &predicted_enemy_pos);
        ...
        
        {
            ...
            turret_update_enemy_in_range(ss,
                                         -4 *
                                         Weapon_info[ss->turret_best_weapon].
                                         fire_wait);
        ...
        
        {
            ...
            
            {
                ...
                
                {
                    ...
                    //timestamp range is 2 to INT_MAX/2
                    if (swp->
                        next_secondary_fire_stamp[i -
                                                  MAX_SHIP_PRIMARY_BANKS] < 2)
                    continue;
