...
int demo_do_frame_start() 
{
    ...
    Assert(((Game_mode & GM_DEMO_RECORD) && !(Game_mode & GM_DEMO_PLAYBACK))
...
int demo_should_sim(object * objp) 
{
    ...
    if ((objp->type == OBJ_SHIP)
        || ((objp->type == OBJ_WEAPON) && (objp->instance >= 0)
...
int demo_write_header() 
{
    ...
    full_name =
    cf_add_ext(Game_current_mission_filename, FS_MISSION_FILE_EXT);
...
void demo_POST_object_dump() 
{
    ...
    obj_count = 0;
  for (sobjp = GET_FIRST(&Ship_obj_list);
        sobjp != END_OF_LIST(&Ship_obj_list); sobjp = GET_NEXT(sobjp))
...
void demo_POST_unique_message(char *id, char *who_from, int m_source,
                              int priority) 
{
  
    // sanity
    if ((id == NULL) || (who_from == NULL) || (strlen(id) <= 0)
        || (strlen(who_from) <= 0))
...
void demo_POST_builtin_message(int type, ship * shipp, int priority,
                               int timing) 
{
...
int demo_read_header() 
{
    ...
    cfread_string_len(Game_current_mission_filename, MAX_FILENAME_LEN,
                      Demo_file);
    ...
    
    {
          ...
          
          {
              ...
              
              {
                  ...
                  
                  {
                    ...
                    Objects[Ships[ship_index].objnum].phys_info.
                      forward_thrust = obj_fthrust;
        ...
        case DE_PRIMARY:
          
          {
              ...
              {
                Ships[ship_index].weapons.current_primary_bank =
                  (int) (fire_info & ~(1 << 7));
        ...
        case DE_UNIQUE_MESSAGE:
          
          {
              ...
              // send the message
              message_send_unique_to_player(id, who_from, m_source, priority,
                                            0, 0);
        ...
        case DE_SHIP_KILL:
          
          {
              ...
              
              {
                ...
                ship_generic_kill_stuff(&Objects[Ships[ship_index].objnum],
                                         1.0f);
