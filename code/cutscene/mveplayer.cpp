...
extern void opengl_tcache_get_adjusted_texture_size(int w_in, int h_in,
                                                    int *w_out, int *h_out);
extern GLenum GL_previous_texture_target;

// the decoders
extern void decodeFrame16(ubyte * pFrame, ubyte * pMap, int mapRemain,
                          ubyte * pData, int dataRemain);
extern void decodeFrame8(ubyte * pFrame, ubyte * pMap, int mapRemain,
                         ubyte * pData, int dataRemain);
...
void mve_audio_createbuf(ubyte minor, ubyte * data)
{
  ...
  OpenAL_ErrorCheck(alGenSources(1, &mas->source_id),
                    {
                    mve_audio_canplay = 0;
                    return;
                    }
  ...
  if (mve_audio_canplay)
    {
      ...
      OpenAL_ErrorCheck(alGetSourcei
                        (mas->source_id, AL_SOURCE_STATE, &status), return);

      OpenAL_ErrorCheck(alGetSourcei
                        (mas->source_id, AL_BUFFERS_QUEUED, &bqueued),
                        return);
...
static void mve_audio_stop()
{
  ...
  OpenAL_ErrorPrint(alSourceUnqueueBuffers
                    (mas->source_id, p, mas->audio_buffer));
      ...
      if (chan & selected_chan)
        {
          ...
          OpenAL_ErrorCheck(alGetSourcei
                            (mas->source_id, AL_BUFFERS_PROCESSED,
                             &bprocessed), return 0);
          ...
          OpenAL_ErrorCheck(alGetSourcei
                            (mas->source_id, AL_SOURCE_STATE, &status),
                            return 0);
          ...
          if (bqueued < MVE_AUDIO_BUFFERS)
            {
              ...
              if (!mas->audio_buffer[mve_audio_buffer_tail])
                {
                  OpenAL_ErrorCheck(alGenBuffers
                                    (1,
                                     &mas->
                                     audio_buffer[mve_audio_buffer_tail]),
                                    {
                                    vm_free(buf);
                                    return 0;
                                    }
              ...
              OpenAL_ErrorCheck(alBufferData
                                (mas->audio_buffer[mve_audio_buffer_tail],
                                 mas->format, buf, nsamp, mas->sample_rate),
                                {
                                vm_free(buf);
                                return 0;
                                }
  ...
  if (gr_screen.mode == GR_OPENGL)
    {
      ...
      if (mve_scale_video)
        {
          g_screenX =
            ((fl2i(gr_screen.max_w / scale_by + 0.5f) - g_width) / 2);
          g_screenY =
            ((fl2i(gr_screen.max_h / scale_by + 0.5f) - g_height) / 2);
        }
      else
        {
...
void mve_video_display()
{
  ...
  if (gr_screen.mode == GR_OPENGL)
    {
      glTexSubImage2D(GL_state.Texture.GetTarget(), 0, 0, 0, g_width,
                      g_height, GL_BGRA, GL_UNSIGNED_SHORT_1_5_5_5_REV,
                      pixelbuf);
  ...
  if (gr_screen.mode == GR_OPENGL)
    {
      ...
      opengl_tcache_get_adjusted_texture_size(g_screenWidth, g_screenHeight,
                                              &wp2, &hp2);
      ...
      glTexImage2D(GL_state.Texture.GetTarget(), 0, GL_RGB5_A1, wp2, hp2, 0,
                   GL_BGRA_EXT, GL_UNSIGNED_SHORT_1_5_5_5_REV, NULL);
...
void mve_video_data(ubyte * data, int len)
{
  ...
  if (g_truecolor)
    decodeFrame16((ubyte *) g_vBackBuf1, g_pCurMap, g_nMapLength, data + 14,
                  len - 14);
  else
    decodeFrame8((ubyte *) g_vBackBuf1, g_pCurMap, g_nMapLength, data + 14,
                 len - 14);
}
