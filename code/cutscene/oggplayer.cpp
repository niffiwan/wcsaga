...
 static void OGG_audio_write(vorbis_info * vorbis, bool * ready) 
{
                       ...
                       (audio_sid, AL_BUFFERS_PROCESSED, &processed), return);
    ...
    
    {
        ...
        (vorbis->channels == 2) ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16;
       OpenAL_ErrorCheck(alBufferData
                           (audio_buffers[audio_buffer_tail], format,
                            audiobuf, audiobuf_fill, vorbis->rate), return);
       OpenAL_ErrorCheck(alSourceQueueBuffers
                           (audio_sid, 1, &audio_buffers[audio_buffer_tail]),
                           return);
...
static void OGG_video_init(theora_info * tinfo) 
{
    ...
    
    {
        ...
        glTexImage2D(GL_state.Texture.GetTarget(), 0, GL_RGB8, wp2, hp2, 0,
    ...
    
    {
        ...
        ((fl2i(gr_screen.max_w / scale_by + 0.5f) - g_screenWidth) / 2);
      g_screenY =
        ((fl2i(gr_screen.max_h / scale_by + 0.5f) - g_screenHeight) / 2);
...
 static void OGG_video_draw(theora_state * tstate) 
{
    ...
    {
      glTexSubImage2D(GL_state.Texture.GetTarget(), 0, 0, 0, g_screenWidth,
                       g_screenHeight, GL_BGR, GL_UNSIGNED_BYTE, pixelbuf);
  ...
  THEORAFILE * theora_open(char *filename) 
{
    ...
    *p = 0;
        ...
        
        {
            ...
            if (!movie->theora_p
                ...
                (theora_decode_header
                 (&movie->tinfo, &movie->tcomment, &movie->opacket) >= 0))
            
            {
                ...
                memcpy(&movie->t_osstate, &test, sizeof(test));
                   ...
                   (vorbis_synthesis_headerin
                    (&movie->vinfo, &movie->vcomment, &movie->opacket) >= 0))
    ...
    while ((movie->theora_p < 3)
           || (movie->vorbis_p && (movie->vorbis_p < 3)))
    
    {
        ...
        
        {
               ...
               || theora_decode_header(&movie->tinfo, &movie->tcomment,
                                       &movie->opacket))
        ...
        
        {
               ...
               || vorbis_synthesis_headerin(&movie->vinfo, &movie->vcomment,
                                            &movie->opacket))
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                (audiofd_fragsize -
                 audiobuf_fill) / 2 / movie->vinfo.channels;
            ...
            
            {
                ...
                if (ogg_stream_packetout(&movie->v_osstate, &movie->opacket) >
                    0)
        ...
        
        {
            ...
            theora_granule_time(&movie->tstate, movie->tstate.granulepos);
