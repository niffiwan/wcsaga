...
static void OGG_audio_init(vorbis_info * vinfo)
{
  ...
  OpenAL_ErrorPrint(alSourcef
                    (audio_sid, AL_GAIN, Master_voice_volume /*1.0f */ ));
  ...
  if (audiobuf_fill && (queued < OGG_AUDIO_BUFFERS))
    {
      ...
      OpenAL_ErrorCheck(alBufferData
                        (audio_buffers[audio_buffer_tail], format, audiobuf,
                         audiobuf_fill, vorbis->rate), return);
      ...
      if (!hw_yuv)
        {
          glTexSubImage2D(GL_state.Texture.GetTarget(), 0, 0, 0,
                          g_screenWidth, g_screenHeight, GL_BGR,
      ...
      else
        {
          ...
          glTexSubImage2D(GL_state.Texture.GetTarget(), 0, 0, 0,
                          g_screenWidth, g_screenHeight, GL_YCRCB_422_SGIX,
                          GL_UNSIGNED_BYTE, pixelbuf);
        }
  ...
  while (playing)
    {
      ...
      while (movie->vorbis_p
             && !audiobuf_ready /*&& !movie->reader->cfeof() */ )
        {
          int ret;
          float **pcm;
              ...
              if (movie->reader->popAudioPacket(&movie->opacket) > 0)
                {
                  if (vorbis_synthesis(&movie->vblock, &movie->opacket) == 0)
                    {
                      vorbis_synthesis_blockin(&movie->vstate,
                                               &movie->vblock);
