...
static void dispatchDecoder8(ubyte ** pFrame, ubyte codeType, ubyte ** pData,
                             int *pDataRemain, int *curXb, int *curYb);

void decodeFrame8(ubyte * pFrame, ubyte * pMap, int mapRemain, ubyte * pData,
                  int dataRemain)
{
  int i, j;
  int xb, yb;
  ...
  for (j = 0; j < yb; j++)
    {
      for (i = 0; i < (xb / 2); i++)
        {
          dispatchDecoder8(&pFrame, (*pMap) & 0xf, &pData, &dataRemain, &i,
                           &j);

          dispatchDecoder8(&pFrame, (*pMap) >> 4, &pData, &dataRemain, &i,
                           &j);
...
static void patternRow4Pixels(ubyte * pFrame, ubyte pat0, ubyte pat1,
                              ubyte * p)
{
  ushort mask = 0x0003;
  ushort shift = 0;
...
static void patternQuadrant4Pixels(ubyte * pFrame, ubyte pat0, ubyte pat1,
                                   ubyte pat2, ubyte pat3, ubyte * p)
{
...
static void patternQuadrant2Pixels(ubyte * pFrame, ubyte pat0, ubyte pat1,
                                   ubyte * p)
{
  ubyte pel;
  ushort mask = 0x0001;
                             ...
                             int *pDataRemain, int *curXb, int *curYb)
{
  ...
  switch (codeType)
    ...
    case 0x0:
      copyFrame(*pFrame,
                *pFrame + ((ubyte *) g_vBackBuf2 - (ubyte *) g_vBackBuf1));
      *pFrame += 8;
      break;
