...
void brief_maybe_create_new_grid(grid * gridp, vec3d * pos, matrix * orient,
                                  int force = 0);
grid * brief_create_grid(grid * gridp, vec3d * forward, vec3d * right,
                          vec3d * center, int nrows, int ncols,
                          float square_size);
...
void brief_parse_icon_tbl() 
{
    ...
    
    {
        ...
        
        {
                   ...
                   "Too many icons in icons.tbl; only the first %d will be used",
                   max_icons);
    ...
    
    {
        ...
        {
          Species_info[species].icon_bitmaps[icon] =
            temp_icon_bitmaps[current_icon];
          Species_info[species].icon_fade_anims[icon] =
            temp_icon_fade_anims[current_icon];
          Species_info[species].icon_highlight_anims[icon] =
            temp_icon_highlight_anims[current_icon];
    ...
    
    {
      ...
      for (species = num_species_covered;
            species < (int) Species_info.size(); species++)
...
void mission_brief_common_init() 
{
  ...
  gr_init_alphacolor(&Brief_color_legacy_neutral, 255, 0, 0,
                      iff_get_alpha_value(false));
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                {
                  Briefings[i].stages[j].new_text =
                    (char *) vm_malloc(MAX_BRIEF_LEN);
        ...
        
        {
            ...
            
            {
                ...
                {
                  Debriefings[i].stages[j].new_text =
                    (char *) vm_malloc(MAX_DEBRIEF_LEN);
                  Assert(Debriefings[i].stages[j].new_text != NULL);
                  memset(Debriefings[i].stages[j].new_text, 0,
                          MAX_DEBRIEF_LEN);
                }
               if (Debriefings[i].stages[j].new_recommendation_text == NULL)
                
                {
                  Debriefings[i].stages[j].new_recommendation_text =
                    (char *) vm_malloc(MAX_RECOMMENDATION_LEN);
                  Assert(Debriefings[i].stages[j].new_recommendation_text !=
                          NULL);
                  memset(Debriefings[i].stages[j].new_recommendation_text, 0,
...
void brief_init_screen(int multiplayer_flag) 
{
    ...
    Brief_grid_coords[gr_screen.res][0] + Brief_grid_coords[gr_screen.res][2];
  bscreen.map_y1 = Brief_grid_coords[gr_screen.res][1];
  bscreen.map_y2 =
    Brief_grid_coords[gr_screen.res][1] + Brief_grid_coords[gr_screen.res][3];
...
float brief_icon_get_dist_moved(icon_move_info * mi, float elapsed_time) 
{
    ...
    (mi->total_dist / 2.0f) + (mi->peak_speed * time) -
...
void brief_render_icon_line(int stage_num, int line_num) 
{
  ...
   gr_line(fl2i(icon_x[0]), fl2i(icon_y[0]), fl2i(icon_x[1]),
             fl2i(icon_y[1]), false);
...
void brief_render_icon(int stage_num, int icon_num, float frametime,
                       int selected, float w_scale_factor,
  ...
  vec3d *pos = NULL;
  ...
   if (!(tv.flags & PF_OVERFLOW))     // make sure point projected before drawing text
    {
        ...
        //gr_aabitmap(bx, by);
             ...
             || (by > gr_screen.max_h_unscaled) && !Fred_running)
        ...
        
        {
            ...
            
            {
                ...
                
                {
                    ...
                    bi->hold_y - fl2i(gr_get_font_height() / 2.0f + 0.5) - 2;
                ...
                
                {
                    ...
                    
                    {
                        ...
                        snd_play(&Snds_iface[SND_ICON_HIGHLIGHT]);
        ...
        
        {
            ...
            
            {
                ...
                brief_set_icon_color(bi->team);
                     ...
                     (ha, frametime, 1, 0, 0, 1, true, mirror_icon) == 0)
...
void brief_start_highlight_anims(int stage_num) 
{
    ...
    
    {
        ...
        
        {
          ...
           bm_get_info(bi->highlight_anim.first_frame, &anim_w, &anim_h,
                         NULL);
...
void brief_render_map(int stage_num, float frametime) 
{
  ...
   brief_maybe_create_new_grid(The_grid, &Current_cam_pos,
                                 &Current_cam_orient);
...
void brief_blit_stage_num(int stage_num, int stage_max) 
{
    ...
    {
      gr_printf(Brief_stage_text_coords_multi[gr_screen.res][0],
                 Brief_stage_text_coords_multi[gr_screen.res][1], buf);
...
void brief_render_line(int line_num, int x, int y, int instance) 
{
          ...
          && instance < (sizeof(Colored_stream) / sizeof(*Colored_stream)));
   SCP_vector < colored_char > *src =
    &Colored_stream[instance].at(line_num);
  ...
  int truncate_len =
    fl2i(Brief_text_wipe_time_elapsed / BRIEF_TEXT_WIPE_TIME *
  ...
  } }  
  {
      ...
      char_seq_pos = 0;
    for (int current_pos = truncate_len;
...
void brief_set_camera_target(vec3d * pos, matrix * orient, int time) 
{
  ...
   vm_vec_scale_add(&Start_lookat_pos, &Start_cam_pos,
                      &Start_cam_orient.vec.fvec, LOOKAT_DIST);
  vm_vec_scale_add(&Target_lookat_pos, &Target_cam_pos,
                    &Target_cam_orient.vec.fvec, LOOKAT_DIST);
...
  ubyte brief_return_color_index(char c) 
{
    ...
    case 'w':
      ...
      Warning(LOCATION,
              "Unrecognized or undefined case character: '$%c' used in Briefing in mission: '%s'. Tell Zacam.",
              c, Mission_filename);
...
int brief_set_move_list(int new_stage, int current_stage, float time) 
{
    ...
    
    {
        ...
        
        {
            ...
            Species_info[species].icon_fade_anims[cb->icons[i].type];
    ...
    
    {
        ...
        
        {
            ...
            Species_info[species].icon_fade_anims[newb->icons[i].type];
                          ...
                          int stage_num) 
{
    ...
    
    {
        ...
        brief_set_move_list(stage_num, Last_new_stage, new_time / 1000.0f);
    ...
    
    {
        ...
        
        {
               ...
               (&Briefing->stages[stage_num].camera_orient.vec.fvec,
                &Briefing->stages[Last_new_stage].camera_orient.vec.fvec))
    ...
    
    {
                ...
                XSTR("Please review your objectives for this mission.", 395));
    ...
    
    {
        ...
        brief_color_text_init(msg, MAX_BRIEF_LINE_W_640);
    ...
    
    {
        ...
        // GR_1024
        Num_brief_text_lines[0] =
        brief_color_text_init(msg, MAX_BRIEF_LINE_W_1024);
...
// and goal.
void interpolate_matrix(matrix * result, matrix * goal, matrix * start,
...
float brief_camera_get_dist_moved(float elapsed_time) 
{
    ...
    (Total_dist / 2.0f) + (Peak_speed * time) -
...
void brief_camera_move(float frametime, int stage_num) 
{
    ...
    
    {
        ...
        vm_matrix_interpolate(&Target_cam_orient, &Current_cam_orient,
                              &W_init, frametime, &result, &w_out, &Vel_limit,
                              &Acc_limit);
...
{
  brief_create_grid(gridp, &gridp->gmatrix.vec.fvec,
                     &gridp->gmatrix.vec.rvec, &gridp->center, gridp->nrows,
                     gridp->ncols, gridp->square_size);
                                      ...
                                      int ship_class) 
{
    ...
    *w = 0;
    ...
    {
      bm_get_info(Species_info[species].icon_bitmaps[type].first_frame, w, h,
                   NULL);
