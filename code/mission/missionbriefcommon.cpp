  ...
  if (num_species_covered < (int) Species_info.size())
    {
      ...
      for (species = num_species_covered; species < (int) Species_info.size();
           species++)
        {
...
void brief_render_map(int stage_num, float frametime)
{
  ...
  g3_start_frame(0);
  g3_set_view_matrix(&Current_cam_pos, &Current_cam_orient,
                     Viewer_zoom_target);
  ...
  if ((Elapsed_time < Total_move_time))
    {
      ...
      vm_matrix_interpolate(&Target_cam_orient, &Current_cam_orient, &W_init,
                            frametime, &result, &w_out, &Vel_limit,
                            &Acc_limit);
...
void brief_modify_grid(grid * gridp)
{
  brief_create_grid(gridp, &gridp->gmatrix.vec.fvec, &gridp->gmatrix.vec.rvec,
