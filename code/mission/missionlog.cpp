...
void mission_log_add_entry(int type, char *pname, char *sname, int info_index)
{
      ...
      if (index >= 0)
        {
          ...
          if (shipp->callsign_index >= 0)
            ...
            {
              end_string_at_first_hash_symbol(entry->pdisplayname,
                                              (shipp->wingnum != -1));
            }
        }
    }
      ...
      if (index >= 0)
        {
          ...
          if (shipp->callsign_index >= 0)
            ...
            {
              end_string_at_first_hash_symbol(entry->sdisplayname,
                                              (shipp->wingnum != -1));
            }
        }
    }
  ...
  switch (type)
    ...
    case LOG_SELF_DESTRUCTED:
      ...
      if (((type == LOG_SHIP_ARRIVED)
           && ((Ships[index].flags2 & SF2_NO_ARRIVAL_LOG) != 0))
          || ((type == LOG_SHIP_DEPARTED)
              && ((Ships[index].flags2 & SF2_NO_DEPARTURE_LOG) != 0)))
        {
          entry->flags |= MLF_HIDDEN;
        }
          ...
          if (sname)
              ...
              if ((Game_mode & GM_MULTIPLAYER)
                  && (multi_find_player_by_callsign(sname) >= 0))
                {
                  ...
                  if (np_ship < 0)
                    ...
                    {
                      team =
                        Ships[Objects[Net_players[np_index].m_player->objnum].
                              instance].team;
                    }
                }
              else
          ...
          else
            {
              nprintf(("missionlog",
               ...
               && (Ship_info[Ships[index].ship_info_index].
    ...
    case LOG_WING_ARRIVED:
      ...
      if (type == LOG_WING_ARRIVED)
      ...
      else
        {
          ...
          if ((Iff_info[entry->primary_team].flags & IFFF_WING_NAME_HIDDEN) !=
              0)
            {
              entry->flags |= MLF_HIDDEN;
            }
      ...
      if (((type == LOG_WING_ARRIVED)
           && ((Wings[index].flags & WF_NO_ARRIVAL_LOG) != 0))
          || ((type == LOG_WING_DEPARTED)
              && ((Wings[index].flags & WF_NO_DEPARTURE_LOG) != 0)))
        {
          entry->flags |= MLF_HIDDEN;
        }
...
void mission_log_add_entry_multi(int type, char *pname, char *sname,
                                 int index, fix timestamp, int flags)
{
  log_entry *entry;
...
int mission_log_get_time_indexed(int type, char *pname, char *sname,
                                 int count, fix * time)
{
  int i, found;
  log_entry *entry;
  ...
  for (i = 0; i < last_entry; i++)
    {
      ...
      Log_line_timestamps[Num_log_lines] =
        (int) (f2fl(entry->timestamp) * 1000.0f);
      ...
      if ((Lcl_gr)
        ...
        {
          message_log_add_seg(Num_log_lines, OBJECT_X, c,
                              entry->pdisplayname);
        }
      ...
      switch (entry->type)
        ...
        case LOG_SHIP_DOCKED:
          if ((entry->flags & MLF_SECONDARY_HIDDEN) != 0)
            ...
            {
              message_log_add_segs(XSTR("Docked with ", 409),
                                   LOG_COLOR_NORMAL);
              message_log_add_segs(entry->sdisplayname, c);
            }
          break;
        ...
        case LOG_SHIP_UNDOCKED:
          if ((entry->flags & MLF_SECONDARY_HIDDEN) != 0)
            ...
            {
              message_log_add_segs(XSTR("Undocked with ", 412),
                                   LOG_COLOR_NORMAL);
              message_log_add_segs(entry->sdisplayname, c);
            }
          break;
        ...
        case LOG_CARGO_REVEALED:
          ...
          message_log_add_segs(XSTR("Cargo revealed: ", 418),
                               LOG_COLOR_NORMAL);
        ...
        case LOG_GOAL_FAILED:
          {
            ...
            message_log_add_segs(text,
                                 LOG_COLOR_BRIGHT,
                                 (entry->type ==
                                  LOG_GOAL_SATISFIED ? LOG_FLAG_GOAL_TRUE :
...
void mission_log_scrollback(int line, int list_x, int list_y, int list_w,
                            int list_h)
{
  char buf[256];
  int y;
