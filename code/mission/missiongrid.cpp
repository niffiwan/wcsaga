...
void maybe_create_new_grid(grid * gridp, vec3d * pos, matrix * orient,
                           int force) 
{
...
//  Stuffs grid in *gridp.  If gridp == NULL, mallocs and returns a grid.
  grid * create_grid(grid * gridp, vec3d * forward, vec3d * right,
...
{
  create_grid(gridp, &gridp->gmatrix.vec.fvec, &gridp->gmatrix.vec.rvec,
               &gridp->center, gridp->nrows, gridp->ncols,
               gridp->square_size);
