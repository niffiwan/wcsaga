...
int message_select_persona(ship * shipp, int persona_needed,
                           bool use_only_autoassign_personas);
...
void messages_init()
{
  ...
  Message_avis.erase((Message_avis.begin() + Num_builtin_avis),
                     Message_avis.end());
  Message_waves.erase((Message_waves.begin() + Num_builtin_waves),
                      Message_waves.end());
}
...
void message_remove_from_queue(message_q * q)
{
  ...
  if (MessageQ_num > 0)
    {
      insertion_sort(MessageQ, MAX_MESSAGE_Q, sizeof(message_q),
...
int message_calc_anim_start_frame(int time, anim * ani, int reverse)
{
  ...
  if (reverse)
    {
      start_frame =
        (ani->total_frames - 1) - fl2i(ani->fps * wave_time + 0.5f);
    }
  else
    {
  ...
  if (start_frame < 0)
    {
      mprintf(("animation '%s' is %i frames too short for the corresponding death message wave. Start frame reset to 0.\n", ani->name, -start_frame));
      start_frame = 0;
    }
...
void message_play_anim(message_q * q)
{
  ...
  if ((q->message_num < Num_builtin_messages)
      ||
      !(_strnicmp
        (HEAD_PREFIX_STRING, ani_name, strlen(HEAD_PREFIX_STRING) - 1)))
    {
      ...
      if (persona_index >= 0)
        {
          if (Personas[persona_index].
              flags & (PERSONA_FLAG_WINGMAN | PERSONA_FLAG_SUPPORT |
                       PERSONA_FLAG_LARGE))
            {
              // get a random head
  ...
  if ((anim_info->anim_data != NULL)
      && !strstr(anim_info->anim_data->name, ani_name))
    {
  ...
  if (m->avi_info.index >= 0)
    {
      ...
      anim_start_frame =
        message_calc_anim_start_frame(Message_wave_duration,
                                      anim_info->anim_data, is_death_scream);
          ...
          if ((Playing_messages[i].anim != NULL)
              && (Playing_messages[i].wave == -1))
            {
              ...
              int length_in_frames =
                (int) ((length * frames_per_character) + 0.5f);
              int frames_played =
                (Playing_messages[i].anim->frame_num -
                 Playing_messages[i].anim->start_at);
  ...
  if (q->builtin_type == MESSAGE_WINGMAN_SCREAM)
    {
      if (!((Ship_info[Ships[Message_shipnum].ship_info_index].flags &
             SIF_SMALL_SHIP)
            || (Ships[Message_shipnum].flags2 & SF2_ALWAYS_DEATH_SCREAM)))
        {
          goto all_done;
        }
    }
  ...
  for (int i = 0; i < Num_personas; i++)
    {
      ...
      if (tempPersonas[i].first->species !=
...
void message_send_unique_to_player(char *id, void *data, int m_source,
                                   int priority, int group, int delay)
{
  int i, source;
  char *who_from;
      ...
      if (!stricmp(id, Messages[i].name))
        {
          ...
          if (m_source == MESSAGE_SOURCE_COMMAND)
          ...
          else if (m_source == MESSAGE_SOURCE_WINGMAN)
            {
              ...
              ship_index =
                ship_get_random_player_wing_ship(SHIP_GET_NO_PLAYERS, 0.0f,
                                                 m_persona, 1,
          ...
          if (!message_filter_multi(i))
            {
              message_queue_message(i, priority, MESSAGE_TIME_ANYTIME,
                                    who_from, source, group, delay);
            }
          ...
          if (MULTIPLAYER_MASTER)
            {
              send_mission_message_packet(i, who_from, priority,
                                          MESSAGE_TIME_SOON, source, -1, -1,
                                          -1, delay);
            }
...
{
  int i, persona_index = -1, persona_species = -1, message_index =
    -1, random_selection = -1;
  int source;
  int num_matching_builtins = 0;
  char *who_from;
  int best_match = -1;

  std::shared_ptr < matching_builtin >
    current_builtin(new matching_builtin());
  ...
  Assertion(random_selection == 0,
            "unable to randomly select built in message correctly, still have %d selections left",
            random_selection);
  ...
  if (!(Game_mode & GM_MULTIPLAYER)
      || ((multi_target == -1) || (multi_target == MY_NET_PLAYER_NUM)))
    {
      ...
      if ((multi_team_filter < 0) || !(Netgame.type_flags & NG_TYPE_TEAM)
        ...
        {
          message_queue_message(message_index, priority, timing, who_from,
                                source, group, delay, type);
      ...
      if (multi_target == -1)
        {
          if (multi_message_should_broadcast(type))
            {
              send_mission_message_packet(message_index, who_from, priority,
                                          timing, source, type, -1,
                                          multi_team_filter);
            }
        }
      else
        {
          send_mission_message_packet(message_index, who_from, priority,
                                      timing, source, type, multi_target,
                                      multi_team_filter);
        }
    }
}
  ...
  for (i = 0; i < Num_messages_playing; i++)
    {
      ...
      if ((hud_communications_state(Player_ship) != COMM_OK)
          || emp_active_local() || Sexp_Messages_Scrambled)
        {
          was_muted = Message_wave_muted;
          if (timestamp_elapsed(Next_mute_time))
            {
              Next_mute_time =
                fl2i(Distort_patterns[Distort_num][Distort_next++] *
...
bool change_message(char *name, char *message, int persona_index,
                    int multi_team)
{
