...
MONITOR(NumShipArrivals) MONITOR(NumShipDepartures)
  ...
  if (flags & MPF_IMPORT_FSM)
    {
      ...
      strcpy_s(pm->event_music_name,
               Soundtracks[rand() % num_soundtracks].name);
    ...
    done_event_music:
      ...
      strcpy_s(pm->briefing_music_name,
               Spooled_music[rand() % num_soundtracks].name);
...
#ifndef _DEBUG
  if ((shipp->wingnum != -1)
      && (Wings[shipp->wingnum].flags & WF_NO_ARRIVAL_LOG))
    shipp->flags2 |= SF2_NO_ARRIVAL_LOG;

  if ((shipp->wingnum != -1)
      && (Wings[shipp->wingnum].flags & WF_NO_DEPARTURE_LOG))
    shipp->flags2 |= SF2_NO_DEPARTURE_LOG;
#else
  if (Fred_running && (shipp->wingnum != -1)
      && (Wings[shipp->wingnum].flags & WF_NO_ARRIVAL_LOG))
    shipp->flags2 |= SF2_NO_ARRIVAL_LOG;
              ...
              if (Fred_running)
              ...
              else
                {
                  ...
                  wp->secondary_bank_ammo[j] =
                    fl2i(capacity /
                         Weapon_info[wp->
                                     secondary_bank_weapons[j]].cargo_size +
                         0.5f);
                }
            }
          ...
          if (shipp->wingnum == -1)
            {
              ...
              mission_set_arrival_location(p_objp->arrival_anchor, location,
                                           p_objp->arrival_distance, objnum,
...
void fix_old_special_hits(p_object * p_objp, int variable_index)
{
  ...
  Assertion(((p_objp->special_hitpoints == 0)
             && (p_objp->special_shield == -1)),
            "Mission appears to be using both the new and old method of special hitpoints for %s",
            p_objp->name);
...
int parse_object(mission * pm, int flag, p_object * p_objp)
{
  ...
  find_and_stuff("$Class:", &p_objp->ship_class, F_NAME, Ship_class_names,
                 Num_ship_classes, "ship class");
  ...
  find_and_stuff("$Team:", &p_objp->team, F_NAME, temp_team_names, Num_iffs,
                 "team name");
      ...
      while ((p_objp->num_texture_replacements < MAX_REPLACEMENT_TEXTURES)
             && (optional_string("+old:")))
        {
          ...
          p_objp->replacement_textures[p_objp->num_texture_replacements].
            new_texture_id =
            bm_load_either(p_objp->replacement_textures
                           [p_objp->num_texture_replacements].new_texture);

          if (p_objp->replacement_textures[p_objp->num_texture_replacements].
              new_texture_id < 0)
            {
              Warning(LOCATION,
  ...
  while (optional_string("+Subsystem:"))
    {
      ...
      if (stricmp("Pilot", Subsys_status[i].name))
        {
          ...
          if (j == sip->n_subsystems)
            Warning(LOCATION,
                    "Ship \"%s\", class \"%s\"\nUnknown subsystem \"%s\" found in mission!",
                    objp->name, sip->name, Subsys_status[i].name);
        }
          ...
          if (index == -1)
            {
              if (Num_cargo < MAX_CARGO)
                ...
                {
                  WarningEx(LOCATION,
                            "Maximum number of cargo names (%d) exceeded, defaulting to Nothing!",
                            MAX_CARGO);
                  index = 0;
                }
            }
      ...
      if (optional_string("+Primary Banks:"))
        stuff_int_list(Subsys_status[i].primary_banks, MAX_SHIP_PRIMARY_BANKS,
                       WEAPON_LIST_TYPE);
      ...
      for (int i = 0; i < wingp->current_count; i++)
        {
          ...
          bool ship_type_harmless =
            (Ship_info[Ships[ship_num].ship_info_index].
             flags & SIF_HARMLESS) != 0;
          bool ship_harmless =
            (Ships[ship_num].flags2 & SF2_IS_HARMLESS) != 0;
          bool is_harmless = (ship_type_harmless || ship_harmless);
          all_harmless &= is_harmless;
        }

      bool wing_has_no_arrival_music_flag =
        (wingp->flags & WF_NO_ARRIVAL_MUSIC) != 0;
              ...
              if (!
                  (stricmp
                   (Sexp_variables[j].variable_name,
                    Player->player_variables[i].variable_name)))
                {
...
void post_process_mission()
{
  ...
  for (p_object * p_objp = GET_FIRST(&Ship_arrival_list);
       p_objp != END_OF_LIST(&Ship_arrival_list); p_objp = GET_NEXT(p_objp))
    {
      ...
      int num = (p_objp->wingnum == -1) ? 1 : Wings[p_objp->wingnum].num_waves - 1;     // subtract one since we already counted the first wave
  ...
  if (should_arrive)
    {
      ...
      bool no_arrival_music_flag =
        (Ships[Objects[object_num].instance].flags & SF_NO_ARRIVAL_MUSIC) !=
        0;
      bool ship_type_is_harmless =
        (Ship_info[Ships[Objects[object_num].instance].ship_info_index].
         flags & SIF_HARMLESS) != 0;
      bool ship_is_harmless =
        (Ships[Objects[object_num].instance].flags2 & SF2_IS_HARMLESS) != 0;

      if (!no_arrival_music_flag && !ship_type_is_harmless
          && !ship_is_harmless)
        {
          if (timestamp_elapsed(Allow_arrival_music_timestamp))
            {
...
{
  // command briefings
  conv_fix_punctuation_section(Mission_text, "#Command Briefing", "#Briefing",
                               "$Stage Text:", "$end_multi_text");

  // briefings
  conv_fix_punctuation_section(Mission_text, "#Briefing", "#Debriefing_info",
                               "$multi_text", "$end_multi_text");
