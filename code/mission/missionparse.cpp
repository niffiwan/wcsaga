...
"Largeship Wing", "Capital", "Planet", "Asteroid Field", "Waypoint", "Support Ship", "Freighter(no cargo)",
"Freighter(has cargo)", "Freighter Wing(no cargo)", "Freighter Wing(has cargo)", "Installation", "Bomber",
"Bomber Wing", "Cruiser", "Cruiser Wing", "Unknown", "Unknown Wing", "Player Fighter", "Player Fighter Wing",
"Player Bomber", "Player Bomber Wing", "Knossos Device", "Transport Wing", "Corvette", "Gas Miner", "Awacs",
"Supercap", "Sentry Gun", "Jump Node", "Transport" 
};
...
void parse_player_info2(mission * pm);
                                  ...
                                  int objnum, int path_mask, vec3d * new_pos,
                                  matrix * new_orient);
...
void restore_one_primary_bank(int *ship_primary_weapons,
                               int *default_primary_weapons);
void restore_one_secondary_bank(int *ship_secondary_weapons,
                                 int *default_secondary_weapons);
       MONITOR(NumShipArrivals)  MONITOR(NumShipDepartures)   void
       parse_mission_info(mission * pm, bool basic = false) 
{
  ...
  pm->support_ships.ship_class = -1;
    ...
    
    {
        ...
        {
          if ((Ship_info[ship_class].flags & SIF_SUPPORT)
               && (Ship_info[ship_class].species == species))
    ...
    {
      if (!Fred_running && (Player != NULL) && (strlen(pm->squad_name) > 0)
    ...
    
    {
               ...
               "Mission: %s\nhas a 640x480 loading screen but no 1024x768 loading screen!",
               pm->name);
...
 void parse_player_info2(mission * pm) 
{
    ...
    
    {
        ...
        stuff_loadout_list(list, MAX_SHIP_CLASSES * 4,
        ...
        stuff_loadout_list(list2, MAX_WEAPON_TYPES * 4,
        ...
        
        {
            ...
            
            {
                ...
                if ((Weapon_info[list2[i]].wi_flags & WIF_PLAYER_ALLOWED)
                    || Fred_running)
...
void parse_music(mission * pm, int flags) 
{
    ...
    index =
    event_music_get_spooled_music_index(pm->substitute_briefing_music_name);
  if ((index >= 0) && (Spooled_music[index].flags & SMF_VALID)
       && !Fred_running)
    
    {
      event_music_set_score(SCORE_BRIEFING,
...
 void parse_cmd_brief(mission * pm) 
{
    ...
    
    {
      ...
      Cur_cmd_brief->stage[stage].text =
        stuff_and_malloc_string(F_MULTITEXT, NULL, CMD_BRIEF_TEXT_MAX);
      Assert(Cur_cmd_brief->stage[stage].text);
       required_string("$Ani Filename:");
      stuff_string(Cur_cmd_brief->stage[stage].ani_filename, F_FILESPEC,
                    MAX_FILENAME_LEN);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                stuff_and_malloc_string(F_MULTITEXT, NULL, MAX_BRIEF_LEN);
            ...
            
            {
                ...
                
                {
                    ...
                    
                    {
                        ...
                        (brief_line *) vm_malloc(sizeof(brief_line) *
                                                 bs->num_lines);
            ...
            
            {
                ...
                
                {
                    ...
                    (brief_icon *) vm_malloc(sizeof(brief_icon) *
                                             bs->num_icons);
          ...
          static char *temp_team_names[MAX_IFFS];
            ...
            
            {
              ...
               find_and_stuff("$team:", &bi->team, F_NAME, temp_team_names,
                                Num_iffs, "team name");
               find_and_stuff("$class:", &bi->ship_class, F_NAME,
                                Ship_class_names, Num_ship_classes,
                                "ship class");
                ...
                
                {
                    ...
                    if (!strnicmp
                        (Ship_info[bi->ship_class].name, "GTSC Faustus", 12))
                    ...
                    if (!strnicmp
                        (Ship_info[bi->ship_class].name, "SD Demon", 8))
                    ...
                    if (!strnicmp
                        (Ship_info[bi->ship_class].name, "GTD Hades", 9))
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                stuff_and_malloc_string(F_MULTITEXT, NULL, MAX_DEBRIEF_LEN);
            ...
            {
              stuff_string(dbs->new_recommendation_text, F_MULTITEXT,
...
 void position_ship_for_knossos_warpin(p_object * p_objp) 
{
    ...
    fvi_ray_plane(&new_point, &knossos_objp->pos,
                  &knossos_objp->orient.vec.fvec, &p_objp->pos,
                  &p_objp->orient.vec.fvec, 0.0f);
  float desired_dist = -pm->mins.xyz.z;
  vm_vec_scale_add2(&objp->pos, &objp->orient.vec.fvec,
                     (dist - desired_dist));
...
void parse_dock_one_docked_object(p_object * pobjp, p_object * parent_pobjp) 
{
    ...
    dockpoint =
    model_find_dock_name_index(Ship_info
                               [Ships[objp->instance].ship_info_index].
                               model_num, dockpoint_name);
  parent_dockpoint =
    model_find_dock_name_index(Ship_info
                               [Ships[parent_objp->instance].ship_info_index].
                               model_num, parent_dockpoint_name);
  ...
  ai_dock_with_object(objp, dockpoint, parent_objp, parent_dockpoint,
                       AIDO_DOCK_NOW);
...
int parse_create_object(p_object * pobjp) 
{
    ...
    
    {
        ...
        dock_evaluate_all_docked_objects(pobjp, &dfi,
                                         parse_create_docked_object_helper);
...
int parse_create_object_sub(p_object * p_objp) 
{
    ...
    objnum =
    ship_create(&p_objp->orient, &p_objp->pos, p_objp->ship_class,
                p_objp->name);
    ...
    if ((Game_mode & GM_MULTIPLAYER)
        && (Netgame.type_flags & NG_TYPE_DOGFIGHT) && (p_objp->wingnum >= 0))
  ...
  if (The_mission.ai_profile->flags & AIPF_FIX_AI_CLASS_BUG)
    ...
    
    {
        ...
        
        {
            ...
            tmap->FindTexture(p_objp->replacement_textures[i].old_texture);
          if (tnum > -1)
            shipp->ship_replacement_textures[j * TM_NUM_TYPES + tnum] =
              p_objp->replacement_textures[i].new_texture_id;
    ...
    if (p_objp->ship_max_shield_strength == 0.0f
        || (!Fred_running && !(p_objp->flags2 & P2_OF_FORCE_SHIELDS_ON)
            && (sip->flags2 & SIF2_INTRINSIC_NO_SHIELDS)))
    Objects[objnum].flags |= OF_NO_SHIELDS;
  
  else if ((p_objp->ship_max_shield_strength > 0.0f)
           && (p_objp->flags2 & P2_OF_FORCE_SHIELDS_ON))
    ...
    
    {
        ...
        
        {
            ...
            ship_get_default_orders_accepted(&Ship_info
    ...
    // flag for the ship
    if ((shipp->wingnum != -1)
        && (Wings[shipp->wingnum].flags & WF_NO_ARRIVAL_WARP))
    shipp->flags |= SF_NO_ARRIVAL_WARP;
   if ((shipp->wingnum != -1)
         && (Wings[shipp->wingnum].flags & WF_NO_DEPARTURE_WARP))
        ...
        
        {
            ...
            
            {
                ...
                
                {
                    ...
                    fl2i(sssp->primary_ammo[j] / 100.0f *
                         sip->primary_bank_ammo_capacity[j] + 0.5f);
                  wp->primary_bank_ammo[j] =
                    fl2i(capacity /
            ...
            
            {
                ...
                
                {
                    ...
                    fl2i(sssp->secondary_ammo[j] / 100.0f *
                         sip->secondary_bank_ammo_capacity[j] + 0.5f);
                  wp->secondary_bank_ammo[j] =
                    fl2i(capacity /
                         Weapon_info[wp->secondary_bank_weapons[j]].
                         cargo_size + 0.5f);
        ...
        
        {
            ...
            
            {
                ...
                
                {
                    ...
                    ptr->system_info->max_subsys_strength *
                    (shipp->ship_max_hull_strength / sip->max_hull_strength);
                   float new_hits;
                  new_hits =
                    ptr->max_hits * (100.0f - sssp->percent) / 100.f;
                  shipp->subsys_info[ptr->system_info->type].current_hits -=
    ...
    
    {
        ...
        
        {
            ...
            submodel_get_two_random_points(sip->model_num, pm->detail[0], &v1,
                                           &v2);
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
               mission_set_arrival_location(p_objp->arrival_anchor,
                                              location,
                                              p_objp->arrival_distance,
                                              objnum,
                                              p_objp->arrival_path_mask, NULL,
                                              NULL);
        ...
        mission_hotkey_mf_add(shipp->hotkey, shipp->objnum,
                               HOTKEY_MISSION_FILE_ADDED);
      
      else if ((shipp->wingnum != -1) && (Wings[shipp->wingnum].hotkey != -1))
        mission_hotkey_mf_add(Wings[shipp->wingnum].hotkey, shipp->objnum,
                               HOTKEY_MISSION_FILE_ADDED);
    ...
    
    {
             ...
             && (p_objp->wingnum == -1))
        send_ship_create_packet(&Objects[objnum],
...
void resolve_parse_flags(object * objp, int parse_flags, int parse_flags2) 
{
            ...
            ((parse_flags & P_OF_NO_SHIELDS)
             && (parse_flags2 & P2_OF_FORCE_SHIELDS_ON)));
...
int parse_object(mission * pm, int flag, p_object * p_objp) 
{
  ...
    find_and_stuff("$Class:", &p_objp->ship_class, F_NAME,
                      Ship_class_names, Num_ship_classes, "ship class");
  ...
  char alt_ship_class[TOKEN_LENGTH >
                      NAME_LENGTH ? TOKEN_LENGTH : NAME_LENGTH];
    ...
    
    {
        ...
        
        {
          ...
          new_alt_class.ship_class =
            ship_info_lookup(Sexp_variables[new_alt_class.variable_index].
                             text);
        ...
        
        {
            ...
            
            {
                ...
                
                {
                           ...
                           "Ship \"%s\" has an invalid Alternate Ship Class type. Skipping this entry",
                           p_objp->name);
    ...
    if ((Game_mode & GM_MULTIPLAYER)
        ...
        && (Ship_info[p_objp->ship_class].flags & SIF_SUPPORT))
  ...
   find_and_stuff("$Team:", &p_objp->team, F_NAME, temp_team_names,
                    Num_iffs, "team name");
  ...
   find_and_stuff("$AI Behavior:", &p_objp->behavior, F_NAME,
                    Ai_behavior_names, Num_ai_behaviors, "AI behavior");
  p_objp->ai_goals = -1;
   if (optional_string("+AI Class:"))
    
    {
      p_objp->ai_class =
        match_and_stuff(F_NAME, Ai_class_names, Num_ai_classes, "AI class");
  ...
  count = 0;
    ...
    
    {
      ...
       find_and_stuff("$Status Description:", &p_objp->status_type[count],
                        F_NAME, Status_desc_names, Num_status_names,
                        "Status Description");
      find_and_stuff("$Status:", &p_objp->status[count], F_NAME,
                      Status_type_names, Num_status_names, "Status Type");
      find_and_stuff("$Target:", &p_objp->target[count], F_NAME,
  ...
  find_and_stuff("$Arrival Location:", &p_objp->arrival_location, F_NAME,
                  Arrival_location_names, Num_arrival_names,
                  "Arrival Location");
  ...
  find_and_stuff("$Departure Location:", &p_objp->departure_location, F_NAME,
  ...
  if (Ship_info[p_objp->ship_class].
       flags & SIF_SHIP_CLASS_DONT_COLLIDE_INVIS)
    ...
    
    {
              ...
              [p_objp->special_hitpoint_index +
               SHIELD_STRENGTH].type & SEXP_VARIABLE_SET)
             &&
             (Sexp_variables
               [p_objp->special_hitpoint_index +
                SHIELD_STRENGTH].type & SEXP_VARIABLE_BLOCK))
        
        {
          p_objp->ship_max_shield_strength =
            (float)
            atoi(Sexp_variables
                 [p_objp->special_hitpoint_index + SHIELD_STRENGTH].text);
        ...
        
        {
                   ...
                   "Special shield hitpoints used for variable number %d. But no variable with this number exists!",
                   p_objp->special_hitpoint_index + SHIELD_STRENGTH);
          reset_index = true;
        }
       if ((Sexp_variables[p_objp->special_hitpoint_index + HULL_STRENGTH].
              type & SEXP_VARIABLE_SET)
             &&
             (Sexp_variables[p_objp->special_hitpoint_index + HULL_STRENGTH].
        ...
        
        {
            ...
            (float)
            atoi(Sexp_variables
                 [p_objp->special_hitpoint_index + HULL_STRENGTH].text);
        ...
        
        {
                   ...
                   "Special hitpoints used for variable number %d. But no variable with this number exists!",
                   p_objp->special_hitpoint_index + HULL_STRENGTH);
    ...
    
    {
        ...
        strcpy_s(Initially_docked[Total_initially_docked].docker,
                 p_objp->name);
      strcpy_s(Initially_docked[Total_initially_docked].dockee, docked_with);
      strcpy_s(Initially_docked[Total_initially_docked].docker_point,
                docker_point);
      strcpy_s(Initially_docked[Total_initially_docked].dockee_point,
                dockee_point);
    ...
    
    {
      ...
       while ((p_objp->num_texture_replacements < MAX_REPLACEMENT_TEXTURES)
                && (optional_string("+old:")))
        
        {
          stuff_string(p_objp->
                        replacement_textures[p_objp->
                                             num_texture_replacements].
                        old_texture, F_NAME, MAX_FILENAME_LEN);
          required_string("+new:");
          stuff_string(p_objp->
                        replacement_textures[p_objp->
                                             num_texture_replacements].
                        new_texture, F_NAME, MAX_FILENAME_LEN);
           
            // get rid of extensions
            p =
            strchr(p_objp->
                   replacement_textures[p_objp->num_texture_replacements].
                   old_texture, '.');
            ...
            // load the texture
            p_objp->replacement_textures[p_objp->num_texture_replacements].
            new_texture_id =
            bm_load_either(p_objp->
                           replacement_textures[p_objp->
                                                num_texture_replacements].
                           new_texture);
            ...
            
            {
                       ...
                       p_objp->replacement_textures[p_objp->
                                                    num_texture_replacements].
                       new_texture, p_objp->name);
            ...
            
            {
              ...
              strcpy_s(Fred_texture_replacements
                        [Fred_num_texture_replacements].ship_name,
                        p_objp->name);
              strcpy_s(Fred_texture_replacements
                        [Fred_num_texture_replacements].old_texture,
                        p_objp->replacement_textures[p_objp->
                                                     num_texture_replacements].
                        old_texture);
              strcpy_s(Fred_texture_replacements
                        [Fred_num_texture_replacements].new_texture,
                        p_objp->replacement_textures[p_objp->
                                                     num_texture_replacements].
                        new_texture);
              Fred_texture_replacements[Fred_num_texture_replacements].
                new_texture_id = -1;
...
void mission_parse_maybe_create_parse_object(p_object * pobjp) 
{
    ...
    if ((object_is_docked(pobjp) && !(pobjp->flags & P_SF_DOCK_LEADER))
        || (!Fred_running
            && (!eval_sexp(pobjp->arrival_cue)
                || !timestamp_elapsed(pobjp->arrival_delay)
                || (pobjp->flags & P_SF_REINFORCEMENT))))
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
              shipfx_blow_up_model(objp,
                                    Ship_info[Ships[objp->instance].
                                              ship_info_index].model_num, 0,
                                    0, &objp->pos);
...
 void parse_common_object_data(p_object * objp) 
{
    ...
    
    {
        ...
        Subsys_status[i].ai_class =
          match_and_stuff(F_NAME, Ai_class_names, Num_ai_classes, "AI class");
       if (optional_string("+Primary Banks:"))
        stuff_int_list(Subsys_status[i].primary_banks,
                        MAX_SHIP_PRIMARY_BANKS, WEAPON_LIST_TYPE);
       
        // Goober5000
        if (optional_string("+Pbank Ammo:"))
        stuff_int_list(Subsys_status[i].primary_ammo, MAX_SHIP_PRIMARY_BANKS,
                        RAW_INTEGER_TYPE);
       if (optional_string("+Secondary Banks:"))
        stuff_int_list(Subsys_status[i].secondary_banks,
                        MAX_SHIP_SECONDARY_BANKS, WEAPON_LIST_TYPE);
       if (optional_string("+Sbank Ammo:"))
        stuff_int_list(Subsys_status[i].secondary_ammo,
  ...
  bool is_ship_assignable(p_object * p_objp) 
{
    ...
    
    {
        ...
        
        {
            ...
            get_reassigned_index(data_for_team,
                                 p_objp->alt_classes[i].ship_class);
                            ...
                            int specific_instance) 
{
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                num_remaining =
                ((wingp->num_waves -
                  wingp->current_wave) * wingp->wave_count);
        ...
        
        {
            ...
            
            {
                ...
                (int) (frand() *
                       (wingp->wave_delay_max - wingp->wave_delay_min));
  ...
  // are ignored, then *all* ships must be in the Ship_arrival_list.
    ...
    
    {
        ...
        {
          sprintf(p_objp->name, NOX("%s %d"), wingp->name,
                   wingp->total_arrived_count);
    ...
    if ((objnum != -1) && (Game_mode & GM_IN_MISSION))  // if true, we have created at least one new ship.
    {
         ...
         */ 
        mission_log_add_entry(LOG_WING_ARRIVED, wingp->name, NULL,
                               wingp->current_wave);
  ...
  wingp->flags = 0;
    ...
    
    {
        ...
        wingp->net_signature + (wingp->wave_count * wingp->num_waves);
               ...
               "Too many total ships in mission (%d) for network signature assignment",
               SHIP_SIG_MAX);
...
void resolve_path_masks(int anchor, int *path_mask) 
{
    ...
    
    {
        ...
        modelnum =
        model_load(Ship_info[parent_pobjp->ship_class].pof_file, 0, NULL);
...
void post_process_path_stuff() 
{
    ...
    
    {
      ...
      resolve_path_masks(pobjp->departure_anchor,
                          &pobjp->departure_path_mask);
    ...
    
    {
      ...
      resolve_path_masks(wingp->departure_anchor,
                          &wingp->departure_path_mask);
...
 void parse_bitmaps(mission * pm) 
{
    ...
    
    {
        ...
        {
          nebula_init(Nebula_filenames[Nebula_index], Nebula_pitch,
    ...
    
    {
        ...
        {
          Warning(LOCATION, "Too many backgrounds in mission!  Max is %d.",
                   MAX_BACKGROUNDS);
        ...
        
        {
            ...
            
            {
                ...
                
                {
                    ...
                    if (!
                        (stricmp
                         (Sexp_variables[k].variable_name,
                          Campaign.missions[i].saved_variables[j].
                          variable_name))  &&!(Campaign.missions[i].
                                                saved_variables[j].
                                                type & SEXP_VARIABLE_BLOCK))
                    
                    {
                      Sexp_variables[k].type =
                        Campaign.missions[i].saved_variables[j].type;
                      strcpy_s(Sexp_variables[k].text,
                                Campaign.missions[i].saved_variables[j].text);
                ...
                if (!
                    (stricmp
                     (Sexp_variables[j].variable_name,
                      Player->player_variables[i].
                      variable_name))  &&!(Player->player_variables[i].
                                            type & SEXP_VARIABLE_BLOCK))
                
                {
                  Sexp_variables[j].type = Player->player_variables[i].type;
                  strcpy_s(Sexp_variables[j].text,
                            Player->player_variables[i].text);
  ...
  parse_cutscenes(pm);
    ...
    if ((Num_unknown_ship_classes > 0)
        || (Num_unknown_loadout_classes >
            0) /*|| (Num_unknown_weapon_classes > 0) */ )
        ...
        
        {
          ...
          char text[1024];
            ...
            
            {
                       ...
                       (Game_mode & GM_CAMPAIGN_MODE) ? "campaign" :
                       "mission");
            ...
            {
              for (char *mod_token = Cmdline_mod; strlen(mod_token) != 0;
          ...
          int popup_rval =
            popup(PF_TITLE_BIG | PF_TITLE_RED, 2, POPUP_NO, POPUP_YES, text);
...
 void post_process_mission() 
{
  ...
  Player_ship = &Ships[Player_start_shipnum];
    ...
    Player->ci.forward_cruise_percent =
      Player_obj->phys_info.vel.xyz.z / Player_ship->current_max_speed *
      100.0f;
    ...
    
    {
        ...
        
        {
          ...
          result =
            check_sexp_syntax(i, query_operator_return_type(op), 1,
                              &bad_node);
...
void mission_set_wing_arrival_location(wing * wingp, int num_to_set) 
{
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                get_absolute_wing_pos(&objp->pos, leader_objp, wing_index++,
                                      0);
    ...
    
    {
        ...
        {
          shipfx_warpin_start(&Objects
                               [Ships[wingp->ship_index[index]].objnum]);
...
void parse_object_clear_all_handled_flags() 
{
    ...
    
    {
        ...
        dock_evaluate_all_docked_objects(pobjp, &dfi,
                                         parse_object_clear_handled_flag_helper);
...
void mission_parse_set_up_initial_docks() 
{
    ...
    
    {
        ...
        dock_evaluate_all_docked_objects(pobjp, &dfi,
                                         parse_object_mark_dock_leader_helper);
        ...
        dock_evaluate_all_docked_objects(pobjp, &dfi,
                                         parse_object_set_handled_flag_helper);
...
int mission_set_arrival_location(int anchor, int location, int dist,
                                 int objnum, int path_mask, vec3d * new_pos,
                                 matrix * new_orient) 
{
    ...
    
    {
      ...
      int iff_index = anchor;
        ...
        // get ship
        shipnum =
        ship_get_random_team_ship(iff_get_mask(iff_index),
                                  get_players ? SHIP_GET_ONLY_PLAYERS :
                                  SHIP_GET_ANY_SHIP);
    ...
    
    {
        ...
        if (ai_acquire_emerge_path
            (&Objects[objnum], anchor_objnum, path_mask, &pos, &fvec) == -1)
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                // same information.
                r1 =
                static_rand(Objects[objnum].net_signature) <
                RAND_MAX_2 ? -1 : 1;
              r2 =
                static_rand(Objects[objnum].net_signature + 1) <
                RAND_MAX_2 ? -1 : 1;
            }
           vm_vec_copy_scale(&t1, &(Objects[anchor_objnum].orient.vec.fvec),
                               x);
          vm_vec_copy_scale(&t2, &(Objects[anchor_objnum].orient.vec.rvec),
                             (1.0f - x) * r1);
          vm_vec_copy_scale(&t3, &(Objects[anchor_objnum].orient.vec.uvec),
                             (1.0f - x) * r2);
...
int mission_did_ship_arrive(p_object * objp) 
{
  ...
   if (should_arrive)         // has the arrival criteria been met?
    {
        ...
        if (!
            (Ships[Objects[object_num].instance].flags & SF_NO_ARRIVAL_MUSIC))
...
void mission_parse_mark_non_arrivals() 
{
    ...
    
    {
        ...
        {
          if (Sexp_nodes[Wings[p_objp->wingnum].arrival_cue].value ==
               SEXP_KNOWN_FALSE)
    ...
    
    {
        ...
        
        {
          ...
          int created = parse_wing_create_ships(wingp, wingp->wave_count);
            ...
            
            {
                ...
                
                {
                    ...
                    
                    {
                        ...
                        timestamp_rand(ARRIVAL_MESSAGE_DELAY_MIN,
            ...
            if (iff_x_attacks_y
                (Player_ship->team, Ships[wingp->ship_index[0]].team))
            ...
            
            {
                ...
                
                {
                    ...
                    
                    {
                        ...
                        {
                          message_send_builtin_to_player(j, &Ships[rship],
                                                          MESSAGE_PRIORITY_LOW,
                                                          MESSAGE_TIME_SOON,
                                                          0, 0, -1, -1);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                shipp->departure_location =
                Wings[shipp->wingnum].departure_location;
              shipp->departure_anchor =
                Wings[shipp->wingnum].departure_anchor;
              shipp->departure_path_mask =
                Wings[shipp->wingnum].departure_path_mask;
...
}  int allocate_subsys_status() 
{
    ...
    
    {
        ...
        (subsys_status *) vm_realloc(Subsys_status,
...
int insert_subsys_status(p_object * pobjp) 
{
    ...
    {
      memcpy(&Subsys_status[i], &Subsys_status[i - 1],
              sizeof(subsys_status));
...
void mission_add_to_arriving_support(object * requester_objp) 
{
  ...
   strcpy_s(Arriving_repair_targets[Num_arriving_repair_targets],
              Ships[requester_objp->instance].ship_name);
  Num_arriving_repair_targets++;
   if (MULTIPLAYER_MASTER)
    
    {
      multi_maybe_send_repair_info(requester_objp, NULL,
                                    REPAIR_INFO_WARP_ADD);
    }
}

 extern int pp_collide_any(vec3d * curpos, vec3d * goalpos, float radius,
...
 int mission_parse_add_alt(char *name) 
{
    ...
    
    {
        ...
        // stuff the name
        strncpy(Mission_alt_types[Mission_alt_type_count++], name,
                NAME_LENGTH);
...
 int mission_parse_add_callsign(char *name) 
{
    ...
    
    {
        ...
        // stuff the name
        strncpy(Mission_callsigns[Mission_callsign_count++], name,
                NAME_LENGTH);
...
void conv_fix_punctuation_section(char *str, char *section_start,
                                  char *section_end, char *text_start,
                                  char *text_end) 
{
    ...
    
    {
        ...
        return;
       replace_all(t1, "\"", "$quote",
                     MISSION_TEXT_SIZE - (str - Mission_text), (t2 - t1));
...
void conv_fix_punctuation() 
{
    ...
    // command briefings
    conv_fix_punctuation_section(Mission_text, "#Command Briefing",
                                 "#Briefing", "$Stage Text:",
                                 "$end_multi_text");
   
    // briefings
    conv_fix_punctuation_section(Mission_text, "#Briefing",
                                 "#Debriefing_info", "$multi_text",
                                 "$end_multi_text");
   
    // debriefings
    conv_fix_punctuation_section(Mission_text, "#Debriefing_info", "#Players",
                                 "$Multi text", "$end_multi_text");
   
    // messages
    conv_fix_punctuation_section(Mission_text, "#Messages", "#Reinforcements",
                                 "$Message:", "\n");
...
void restore_default_weapons(char *ships_tbl) 
{
    ...
    
    {
                  ...
                  "This is not a ships.tbl file.  Aborting conversion...",
                  "Error", MB_OK);
    ...
    
    {
        ...
        
        {
          ...
          restore_one_primary_bank(Ships[i].weapons.primary_bank_weapons,
                                    sip->primary_bank_weapons);
           
            // check sbanks (capital ships have these specified but empty)
            Mp = strstr(ch, "$Default SBanks");
          Mp = strchr(Mp, '(');
          restore_one_secondary_bank(Ships[i].weapons.secondary_bank_weapons,
                                      sip->secondary_bank_weapons);
           
            // see if we have any turrets
            ch = strstr(ch, "$Subsystem");
          for (ss = GET_FIRST(&Ships[i].subsys_list);
                ss != END_OF_LIST(&Ships[i].subsys_list); ss = GET_NEXT(ss))
            
            {
                ...
                
                {
                    ...
                    
                    {
                      ...
                      restore_one_primary_bank(ss->weapons.
                                                primary_bank_weapons,
                                                sip->subsystems[si_subsys].
                                                primary_banks);
                    ...
                    
                    {
                      ...
                      restore_one_secondary_bank(ss->weapons.
