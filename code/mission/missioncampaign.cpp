...
int mission_campaign_load(char *filename, player * pl, int load_savefile)
{
  ...
  strncpy(strippedFilename,
          filenametemp.substr(filenameStart,
                              filenametemp.length() - filenameStart).c_str(),
          MAX_FILENAME_LEN - 1);
          ...
          for (j = 0; j < Campaign.missions[i].num_saved_variables; j++)
            {
              ...
              cfwrite_string_len(Campaign.missions[i].saved_variables[j].text,
                                 fp);
          ...
          if (Campaign.missions[num].num_saved_variables > 0)
            {
              Campaign.missions[num].saved_variables =
                (sexp_variable *) vm_malloc(Campaign.
                                            missions[num].num_saved_variables
              ...
              for (j = 0; j < Campaign.missions[num].num_saved_variables; j++)
                {
                  Campaign.missions[num].saved_variables[j].type =
                    cfread_int(fp);
                  cfread_string_len(Campaign.missions[num].saved_variables[j].
                                    text, TOKEN_LENGTH, fp);
                  cfread_string_len(Campaign.missions[num].saved_variables[j].
                                    variable_name, TOKEN_LENGTH, fp);
                }
            }
        }
