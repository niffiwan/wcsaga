...
void mission_campaign_build_list(bool desc, bool sort, bool multiplayer) 
{
    ...
    rc =
    cf_get_file_list(MAX_CAMPAIGNS, Campaign_file_names, CF_TYPE_MISSIONS,
                     wild_card, CF_SORT_NONE);
...
int mission_campaign_load(char *filename, player * pl, int load_savefile) 
{
    ...
    
    {
        ...
        
        {
          ...
          stuff_and_malloc_string(F_MULTITEXT, NULL, MISSION_DESC_LENGTH);
        ...
        
        {
            ...
            {
              cm->mission_loop_brief_anim =
                stuff_and_malloc_string(F_MULTITEXT, NULL, MAX_FILENAME_LEN);
            }
           cm->mission_loop_brief_sound = NULL;
          if (optional_string("+Mission Loop Brief Sound:"))
            
            {
              cm->mission_loop_brief_sound =
                stuff_and_malloc_string(F_MULTITEXT, NULL, MAX_FILENAME_LEN);
...
int mission_campaign_savefile_save() 
{
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
              cfwrite_string_len(Campaign.missions[i].saved_variables[j].
                                  text, fp);
          ...
          memcpy(&stats_tmp, &Campaign.missions[i].stats,
                  sizeof(scoring_struct));
           
            // swap values if needed
            for (j = 0; j < Num_ship_classes; j++)
            stats_tmp.kills[j] =
              INTEL_INT(Campaign.missions[i].stats.kills[j]);
          ...
          stats_tmp.kill_count =
            INTEL_INT(Campaign.missions[i].stats.kill_count);
          stats_tmp.kill_count_ok =
            INTEL_INT(Campaign.missions[i].stats.kill_count_ok);
          stats_tmp.p_shots_fired =
            INTEL_INT(Campaign.missions[i].stats.p_shots_fired);
          stats_tmp.s_shots_fired =
            INTEL_INT(Campaign.missions[i].stats.s_shots_fired);
          stats_tmp.p_shots_hit =
            INTEL_INT(Campaign.missions[i].stats.p_shots_hit);
          stats_tmp.s_shots_hit =
            INTEL_INT(Campaign.missions[i].stats.s_shots_hit);
          stats_tmp.p_bonehead_hits =
            INTEL_INT(Campaign.missions[i].stats.p_bonehead_hits);
          stats_tmp.s_bonehead_hits =
            INTEL_INT(Campaign.missions[i].stats.s_bonehead_hits);
          stats_tmp.bonehead_kills =
            INTEL_INT(Campaign.missions[i].stats.bonehead_kills);
           for (j = 0; j < MAX_MEDALS; j++)
            stats_tmp.medals[j] =
              INTEL_INT(Campaign.missions[i].stats.medals[j]);
    ...
    
    {
        ...
        (Ship_info[i].flags & SIF_IN_TECH_DATABASE) ? (ubyte) 1 : (ubyte) 0;
    ...
    
    {
        ...
        (Weapon_info[i].
         wi_flags & WIF_IN_TECH_DATABASE) ? (ubyte) 1 : (ubyte) 0;
    ...
    
    {
        ...
        (Intel_info[i].flags & IIF_IN_TECH_DATABASE) ? (ubyte) 1 : (ubyte) 0;
...
int mission_campaign_savefile_load(char *cfilename, player * pl) 
{
  ...
  extern int Player_select_screen_active;
  if (!Player_select_screen_active
       && !strcmp(cfilename, pl->current_campaign))
    ...
    Assert(((Game_mode & GM_MULTIPLAYER)
            && (type_sig == (int) CAMPAIGN_MULTI_PLAYER_SIG))
           || (!(Game_mode & GM_MULTIPLAYER)
               && (type_sig == (int) CAMPAIGN_SINGLE_PLAYER_SIG)));
   Campaign.type =
    type_sig ==
    (int) CAMPAIGN_SINGLE_PLAYER_SIG ? CAMPAIGN_TYPE_SINGLE :
  ...
  int ship_count, weapon_count;
    ...
    
    {
        ...
        
        {
            ...
            (mgoal *) vm_malloc(Campaign.missions[num].num_goals *
                                sizeof(mgoal));
            ...
            {
              cfread_string_len(Campaign.missions[num].goals[j].name,
                                 NAME_LENGTH, fp);
        ...
        
        {
            ...
            (mevent *) vm_malloc(Campaign.missions[num].num_events *
                                 sizeof(mevent));
            ...
            {
              cfread_string_len(Campaign.missions[num].events[j].name,
                                 NAME_LENGTH, fp);
        ...
        
        {
            ...
            {
              Campaign.missions[num].saved_variables =
                (sexp_variable *) vm_malloc(Campaign.missions[num].
                                            num_saved_variables *
                                            sizeof(sexp_variable));
               Assert(Campaign.missions[num].saved_variables != NULL);
               if (Campaign.missions[num].saved_variables != NULL)
                memset(Campaign.missions[num].saved_variables, 0,
                        sizeof(sexp_variable) *
                        Campaign.missions[num].num_saved_variables);
               
                // now read in the variable information for this mission
                for (j = 0; j < Campaign.missions[num].num_saved_variables;
                     j++)
                
                {
                  Campaign.missions[num].saved_variables[j].type =
                    cfread_int(fp);
                  cfread_string_len(Campaign.missions[num].
                                     saved_variables[j].text, TOKEN_LENGTH,
                                     fp);
                  cfread_string_len(Campaign.missions[num].
                                     saved_variables[j].variable_name,
                                     TOKEN_LENGTH, fp);
        ...
        {
          cfread(&Campaign.missions[num].stats, sizeof(scoring_struct), 1,
                  fp);
            ...
            // swap values
            Campaign.missions[num].stats.score =
            INTEL_INT(Campaign.missions[num].stats.score);
          Campaign.missions[num].stats.rank =
            INTEL_INT(Campaign.missions[num].stats.rank);
          Campaign.missions[num].stats.assists =
            INTEL_INT(Campaign.missions[num].stats.assists);
          Campaign.missions[num].stats.kill_count =
            INTEL_INT(Campaign.missions[num].stats.kill_count);
          Campaign.missions[num].stats.kill_count_ok =
            INTEL_INT(Campaign.missions[num].stats.kill_count_ok);
          Campaign.missions[num].stats.p_shots_fired =
            INTEL_INT(Campaign.missions[num].stats.p_shots_fired);
          Campaign.missions[num].stats.s_shots_fired =
            INTEL_INT(Campaign.missions[num].stats.s_shots_fired);
          Campaign.missions[num].stats.p_shots_hit =
            INTEL_INT(Campaign.missions[num].stats.p_shots_hit);
          Campaign.missions[num].stats.s_shots_hit =
            INTEL_INT(Campaign.missions[num].stats.s_shots_hit);
          Campaign.missions[num].stats.p_bonehead_hits =
            INTEL_INT(Campaign.missions[num].stats.p_bonehead_hits);
          Campaign.missions[num].stats.s_bonehead_hits =
            INTEL_INT(Campaign.missions[num].stats.s_bonehead_hits);
          Campaign.missions[num].stats.bonehead_kills =
            INTEL_INT(Campaign.missions[num].stats.bonehead_kills);
           for (j = 0; j < MAX_MEDALS; j++)
            
            {
              Campaign.missions[num].stats.medals[j] =
                INTEL_INT(Campaign.missions[num].stats.medals[j]);
    ...
    
    {
      ...
      char trash[MAX_FILENAME_LEN + DATE_TIME_LENGTH];
        ...
        
        {
          ...
          cfread_string_len(Player_loadout.last_modified, DATE_TIME_LENGTH,
                             fp);
    ...
    
    {
        ...
        
        {
                   ...
                   "Campaign stats save file has invalid signature");
        ...
        
        {
            ...
            if (MAX_SHIP_CLASSES > ORIG_MAX_SHIP_CLASSES)
              ...
              (MAX_SHIP_CLASSES - ORIG_MAX_SHIP_CLASSES) * sizeof(int) * 3;
        ...
        
        {
            ...
            read_size -=
            (sizeof(int) - sizeof(ushort)) * 3 * ORIG_MAX_SHIP_CLASSES;
            ...
            if (MAX_SHIP_CLASSES > ORIG_MAX_SHIP_CLASSES)
              ...
              (MAX_SHIP_CLASSES - ORIG_MAX_SHIP_CLASSES) * (sizeof(int) -
                                                            sizeof(ushort)) *
              3;
        ...
        
        {
            ...
            
            {
                ...
                for (j = 0; j < MAX_SHIP_CLASSES; j++)
                  ...
                  INTEL_INT(Campaign.missions[num].stats.kills[j]);
...
void mission_campaign_store_goals_and_events_and_variables() 
{
    ...
    {
      mission->saved_variables =
        (sexp_variable *) vm_malloc(sizeof(sexp_variable) *
                                    mission->num_saved_variables);
    ...
    
    {
        ...
        
        {
          ...
          strcpy_s(mission->saved_variables[j].variable_name,
...
int mission_campaign_get_filenames(char *filename, char dest[][NAME_LENGTH],
                                   int *num) 
{
...
void read_mission_goal_list(int num) 
{
    ...
    
    {
        ...
        
        {
            ...
            {
              sprintf(events[event_count], NOX("Event #%d"),
                       event_count + 1);
...
int mission_load_up_campaign(player * pl) 
{
    ...
    
    {
        ...
        
        {
          ...
           if ((Campaign_file_names[0] != NULL)
                 && strlen(Campaign_file_names[0]))
...
void mission_campaign_skip_to_next(int start_game) 
{
    ...
    Campaign.missions[Campaign.current_mission].flags |=
    CMISSION_FLAG_SKIPPED;
        ...
        
        {
            ...
            
            {
                ...
                if (!
                    (stricmp
                     (Sexp_variables[i].variable_name,
                      Player->player_variables[j].variable_name)))
            ...
            
            {
                ...
                
                {
                         ...
                         Player->player_variables[Player->num_variables -
                                                  1].variable_name,
          ...
          strcpy_s(Player->player_variables[index].text,
                    Sexp_variables[i].text);
          strcpy_s(Player->player_variables[index].variable_name,
                    Sexp_variables[i].variable_name);
