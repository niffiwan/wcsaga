...
int bm_load_sub_slow(char *real_filename, const int num_ext,
                     const char **ext_list, CFILE ** img_cfp =
                     NULL, int dir_type = CF_TYPE_ANY) 
{
  char full_path[MAX_PATH];
  int size = 0, offset = 0;
  int rval = -1;
   rval =
    cf_find_file_location_ext(real_filename, num_ext, ext_list, dir_type,
                              sizeof(full_path) - 1, full_path, &size,
                              &offset, 0);
...
int bm_load_sub_fast(char *real_filename, int *handle, int dir_type =
                     CF_TYPE_ANY, bool animated_type = false) 
{
    ...
    
    {
      ...
       bool animated = ((bm_bitmaps[i].type == BM_TYPE_EFF)
                          || (bm_bitmaps[i].type == BM_TYPE_ANI));
...
int bm_load(char *real_filename) 
{
    ...
    // Lets find out what type it is
  {
      ...
      { BM_TYPE_DDS, BM_TYPE_TGA, BM_TYPE_PNG, BM_TYPE_JPG, BM_TYPE_PCX };
    ...
    gr_bm_load(type, free_slot, filename, img_cfp, &w, &h, &bpp, &c_type,
               &mm_lvl, &bm_size);
...
 int bm_load_and_parse_eff(char *filename, int dir_type, int *nframes,
                             int *nfps, int *key, ubyte * type) 
{
...
//
int bm_load_animation(char *real_filename, int *nframes, int *fps,
                      int *keyframe, int can_drop_frames, int dir_type) 
{
    ...
    // Lets find out what type it is
  {
      ...
      // if we are still here then we need to fall back to a file-based search
    int rval =
      bm_load_sub_slow(filename, NUM_TYPES, ext_list, &img_cfp, dir_type);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                INTEL_INT(the_anim.keys[i].frame_num);
    ...
    
    {
        ...
        
        {
          ...
          sprintf(bm_bitmaps[n + i].info.ani.eff.filename, "%s_%.4d",
                   clean_name, i);
           
            // gr_bm_load() returns non-0 on failure
            if (gr_bm_load
                (eff_type, n + i, bm_bitmaps[n + i].info.ani.eff.filename,
                 NULL, &anim_width, &anim_height, &bpp, &c_type, &mm_lvl,
                 &img_size))
...
 int bm_load_either(char *filename, int *nframes, int *fps, int *keyframe,
                      int can_drop_frames, int dir_type) 
{
    ...
    bm_load_animation(filename, nframes, fps, keyframe, can_drop_frames,
                      dir_type);
...
int bm_get_info(int handle, int *w, int *h, ubyte * flags, int *nframes,
                int *fps) 
{
    ...
    *flags = bmp->flags;
         ...
         || (bm_bitmaps[bitmapnum].type == BM_TYPE_EFF))
...
static void bm_convert_format(int bitmapnum, bitmap * bmp, ubyte bpp,
                               ubyte flags) 
{
...
void bm_swizzle_8bit_for_fred(bitmap_entry * be, bitmap * bmp, ubyte * data,
                              ubyte * palette) 
{
  
    /* 2004/10/17 - taylor - no longer needed since FRED is OGL now */ 
}  void bm_lock_pcx(int handle, int bitmapnum, bitmap_entry * be,
                      bitmap * bmp, ubyte bpp, ubyte flags) 
{
    ...
    pcx_read_bitmap(filename, data, NULL, (bpp >> 3), (flags & BMP_AABITMAP),
                    0, be->dir_type);
...
 void bm_lock_ani(int handle, int bitmapnum, bitmap_entry * be,
                    bitmap * bmp, ubyte bpp, ubyte flags) 
{
          ...
          anim_load(bm_bitmaps[first_frame].filename,
                    bm_bitmaps[first_frame].dir_type)) == NULL)
  ...
   if (the_anim->total_frames !=
         bm_bitmaps[first_frame].info.ani.num_frames)
    ...
    
    {
        ...
        anim_get_next_raw_buffer(the_anim_instance, 0,
                                 flags & BMP_AABITMAP ? 1 : 0, bm->bpp);
...
  void bm_lock_user(int handle, int bitmapnum, bitmap_entry * be,
                       bitmap * bmp, ubyte bpp, ubyte flags) 
{
...
 void bm_lock_tga(int handle, int bitmapnum, bitmap_entry * be,
                    bitmap * bmp, ubyte bpp, ubyte flags) 
{
    ...
    targa_read_bitmap(filename, data, NULL, d_size, be->dir_type);
  ...
  bitmap * bm_lock(int handle, ubyte bpp, ubyte flags) 
{
    ...
    
    {
      ...
      else if ((flags & BMP_TEX_DXT1) || (flags & BMP_TEX_DXT3)
               || (flags & BMP_TEX_DXT5))
...
int bm_release(int handle, int clear_render_targets) 
{
    ...
    {
      int i, first = be->info.ani.first_frame, total =
        bm_bitmaps[first].info.ani.num_frames;
...
void bm_page_in_texture(int bitmapnum, int nframes) 
{
  ...
   Assertion((bm_bitmaps[n].handle == bitmapnum),
               "bitmapnum = %i, n = %i, bm_bitmaps[n].handle = %i", bitmapnum,
               n, bm_bitmaps[n].handle);
   if (nframes <= 0)
    
    {
      if ((bm_bitmaps[n].type == BM_TYPE_ANI)
...
 void bm_page_in_stop() 
{
  ...
  int n = 0;
    ...
    
    {
           ...
           && (bm_bitmaps[i].type != BM_TYPE_RENDER_TARGET_DYNAMIC)
        ...
        
        {
            ...
            
            {
                ...
                
                {
                       ...
                       (bm_bitmaps[i].handle, (bm_bitmaps[i].preloaded == 2)))
                ...
                {
                  bm_lock(bm_bitmaps[i].handle,
                           (bm_bitmaps[i].used_flags ==
                            BMP_AABITMAP) ? 8 : 16, bm_bitmaps[i].used_flags);
              ...
               if ((bm_bitmaps[i].info.ani.first_frame == 0)
                     || (bm_bitmaps[i].info.ani.first_frame == i))
...
 int bm_get_cache_slot(int bitmap_id, int separate_ani_frames) 
{
  ...
   if ((!separate_ani_frames)
         && ((be->type == BM_TYPE_ANI) || (be->type == BM_TYPE_EFF)))
    
    {
                              ...
                              ubyte * a) = NULL;
void (*bm_set_components_32) (ubyte * pixel, ubyte * r, ubyte * g, ubyte * b,
                               ubyte * a) = NULL;
 void bm_set_components_argb_16_screen(ubyte * pixel, ubyte * rv,
                                         ubyte * gv, ubyte * bv, ubyte * av) 
{
    ...
    (unsigned short) (((int) *rv / Gr_current_red->scale) << Gr_current_red->
                      shift);
  *((unsigned short *) pixel) |=
    (unsigned short) (((int) *gv / Gr_current_green->scale) <<
                      Gr_current_green->shift);
  *((unsigned short *) pixel) |=
    (unsigned short) (((int) *bv / Gr_current_blue->scale) <<
                      Gr_current_blue->shift);
}  void bm_set_components_argb_32_screen(ubyte * pixel, ubyte * rv,
                                            ubyte * gv, ubyte * bv,
                                            ubyte * av) 
{
    ...
    (unsigned int) (((int) *rv / Gr_current_red->scale) << Gr_current_red->
                    shift);
  *((unsigned int *) pixel) |=
    (unsigned int) (((int) *gv / Gr_current_green->scale) <<
                    Gr_current_green->shift);
  *((unsigned int *) pixel) |=
    (unsigned int) (((int) *bv / Gr_current_blue->scale) << Gr_current_blue->
                    shift);
}  void bm_set_components_argb_16_tex(ubyte * pixel, ubyte * rv,
                                         ubyte * gv, ubyte * bv, ubyte * av) 
{
...
}  void bm_set_components_argb_32_tex(ubyte * pixel, ubyte * rv,
                                         ubyte * gv, ubyte * bv, ubyte * av) 
{
...
void bm_get_components(ubyte * pixel, ubyte * r, ubyte * g, ubyte * b,
                       ubyte * a) 
{
    ...
    
    {
        ...
        
        {
            ...
            ubyte(((*((unsigned int *) pixel) & Gr_current_red->
                    mask) >> Gr_current_red->shift) * Gr_current_red->scale);
        ...
        
        {
            ...
            ubyte(((((unsigned short *) pixel)[0] & Gr_current_red->
                    mask) >> Gr_current_red->shift) * Gr_current_red->scale);
    ...
    
    {
        ...
        
        {
            ...
            ubyte(((*((unsigned int *) pixel) & Gr_current_green->
                    mask) >> Gr_current_green->shift) *
        ...
        
        {
            ...
            ubyte(((((unsigned short *) pixel)[0] & Gr_current_green->
                    mask) >> Gr_current_green->shift) *
                  Gr_current_green->scale);
    ...
    
    {
        ...
        
        {
            ...
            ubyte(((*((unsigned int *) pixel) & Gr_current_blue->
                    mask) >> Gr_current_blue->shift) *
        ...
        
        {
            ...
            ubyte(((((unsigned short *) pixel)[0] & Gr_current_blue->
                    mask) >> Gr_current_blue->shift) *
                  Gr_current_blue->scale);
...
int bm_convert_color_index_to_BGR(int num, ubyte ** out_data) 
{
  ...
   if ((bmp->bpp != 8) || !(bmp->data)
         || ((be->type != BM_TYPE_DDS) && (be->type != BM_TYPE_PCX)))
  ...
  Assert(palette != NULL);
    ...
    {
      dds_read_header(filename, NULL, NULL, NULL, &bpp, NULL, NULL, &size,
                       palette);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                        ...
                        bm_bitmaps[i].data_size ? NOX("*LOCKED*") : NOX(""),
                        ((float) bm_bitmaps[i].data_size / 1024.0f) /
                        1024.0f));
...
int bm_make_render_target(int width, int height, int flags) 
{
  ...
  int w = width, h = height;
    ...
    (flags & BMP_FLAG_RENDER_TARGET_STATIC) ? BM_TYPE_RENDER_TARGET_STATIC :
...
 int bm_is_render_target(int bitmap_id) 
{
         ...
         ((bm_bitmaps[n].type == BM_TYPE_RENDER_TARGET_STATIC)
          || (bm_bitmaps[n].type == BM_TYPE_RENDER_TARGET_DYNAMIC)))
...
 int bm_set_render_target(int handle, int face) 
{
    ...
    
    {
      ...
       if ((bm_bitmaps[n].type != BM_TYPE_RENDER_TARGET_STATIC)
             && (bm_bitmaps[n].type != BM_TYPE_RENDER_TARGET_DYNAMIC))
