  ...
  if (type == BM_TYPE_EFF)
    {
      if (bm_load_and_parse_eff
          (filename, dir_type, &anim_frames, &anim_fps, &key, &eff_type) != 0)
        {
      ...
      if (type == BM_TYPE_EFF)
        {
          ...
          if (gr_bm_load
              (eff_type, n + i, bm_bitmaps[n + i].info.ani.eff.filename, NULL,
               &anim_width, &anim_height, &bpp, &c_type, &mm_lvl, &img_size))
            {
...
int bm_load_eff_frame(char *filename, int frame, int dir_type, ubyte eff_type,
                      int *bitmap_id, int *width, int *height)
{
  int anim_width;
  int anim_height;
  int bpp;
...
MONITOR(NumBitmapPage) MONITOR(SizeBitmapPage)
...
void bm_page_in_texture(int bitmapnum, int nframes)
{
  ...
  Assertion((bm_bitmaps[n].handle == bitmapnum),
            "bitmapnum = %i, n = %i, bm_bitmaps[n].handle = %i", bitmapnum, n,
            bm_bitmaps[n].handle);
              ...
              if (bm_preloading)
              ...
              else
                {
                  bm_lock(bm_bitmaps[i].handle,
                          (bm_bitmaps[i].used_flags == BMP_AABITMAP) ? 8 : 16,
...
void bm_set_components_argb_16_screen(ubyte * pixel, ubyte * rv, ubyte * gv,
                                      ubyte * bv, ubyte * av)
{
  if (*av == 0)
    {
...
void bm_set_components_argb_32_screen(ubyte * pixel, ubyte * rv, ubyte * gv,
                                      ubyte * bv, ubyte * av)
{
  if (*av == 0)
    {
...
void bm_set_components_argb_16_tex(ubyte * pixel, ubyte * rv, ubyte * gv,
                                   ubyte * bv, ubyte * av)
{
  if (*av == 0)
    {
...
void bm_set_components_argb_32_tex(ubyte * pixel, ubyte * rv, ubyte * gv,
                                   ubyte * bv, ubyte * av)
{
  if (*av == 0)
    {
