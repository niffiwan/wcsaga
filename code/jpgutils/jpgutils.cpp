...
//
int jpeg_read_header(char *real_filename, CFILE * img_cfp, int *w, int *h,
...
//
int jpeg_read_bitmap(char *real_filename, ubyte * image_data, ubyte * palette,
  ...
  if ((rc = setjmp(FSJpegError) == 0) != FALSE)
    {
      ...
      int size =
        jpeg_info.output_width * jpeg_info.output_components *
