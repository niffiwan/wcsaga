  ...
  if (Viewer_mode & VM_EXTERNAL)
    {
      if (!(Viewer_mode & VM_EXTERNAL_CAMERA_LOCKED))
        {
          t =
            t + (check_control_timef(YAW_LEFT) -
                 check_control_timef(YAW_RIGHT));
          u =
            u + (check_control_timef(PITCH_BACK) -
...
void do_view_track_target(float frame_time)
{
  ...
  vm_vec_rotate(&forwardvec_rotated, &Player_obj->orient.vec.fvec,
                &Player_obj->orient);
...
void do_view_chase(float frame_time)
{
  ...
  t =
    check_control_timef(VIEW_DIST_INCREASE) -
...
{
  ci->forward =
    check_control_timef(FORWARD_THRUST) - check_control_timef(REVERSE_THRUST);
  ...
  for (int axn = 0; axn < 2; ++axn)
    {
      ...
      if (Mouse_sensitivity <= 4)
        {
          percent[axn] =
            pow(percent[axn], (3.0f - ((float) Mouse_sensitivity / 2.0f)));
        }
      else
        {
...
void read_keyboard_controls(control_info * ctrl_info, float frame_time,
                            physics_info * pi)
{
  ...
  if (ok_to_read_ci_pitch_yaw)
    {
      ...
      if (check_control(BANK_WHEN_PRESSED))
        {
          ctrl_info->bank =
            check_control_timef(BANK_LEFT) + check_control_timef(YAW_LEFT) -
      ...
      else
        {
          kh =
            (check_control_timef(YAW_RIGHT) -
             check_control_timef(YAW_LEFT)) / 8.0f;
          if (kh == 0.0f)
            {
              ctrl_info->heading = 0.0f;
          ...
          ctrl_info->bank =
            check_control_timef(BANK_LEFT) - check_control_timef(BANK_RIGHT);
        }

      ctrl_info->heading += kh;
      kh =
        (check_control_timef(PITCH_FORWARD) -
         check_control_timef(PITCH_BACK)) / 8.0f;
      if (kh == 0.0f)
        {
  ...
  chase_angles_to_value(&Viewer_slew_angles, &chase_slew_angles,
                        centering_speed);

  if (!(Game_mode & GM_DEAD))
    {
      if (button_info_query(&Player->bi, ONE_THIRD_THROTTLE)
          && !object_get_gliding(Player_obj))
        {
              ...
              if (!(Objects[Player->objnum].phys_info.flags & PF_GLIDING))
                {
                  ...
                  if (pmax_speed > 0.0f)
                    {
                      ctrl_info->forward_cruise_percent =
            ...
            if (Warpout_forced)
              {
                ...
                Player->ci.forward =
                  ((target_warpout_speed +
                    diff) / Ships[objp->instance].current_max_speed);
              }
            else
              {
                // check if warp ability has been disabled
                if (Ships[objp->instance].
                    flags & (SF_WARP_BROKEN | SF_WARP_NEVER))
                  {
                ...
                if (Warpout_time > MINIMUM_PLAYER_WARPOUT_TIME)
                  {
                    ...
                    if (target_warpout_speed != 0.0f)
                      {
                        diffSpeed =
                          fl_abs(objp->phys_info.fspeed -
                                 target_warpout_speed) / target_warpout_speed;
                      }
...
void player_match_target_speed(char *no_target_text, char *match_off_text,
                               char *match_on_text)
{
      ...
      if (Ships[objp->instance].team == Player_ship->team)
        {
          ...
          if (ship_type != -1
              && (Ship_types[ship_type].
                  message_bools & STI_MSG_COUNTS_FOR_ALONE))
            {
              return;
            }
        }
  ...
  if (Player->flags & PLAYER_FLAGS_AUTO_MATCH_SPEED)
    {
      // multiplayer observers can't match target speed
      if (!
          ((Game_mode & GM_MULTIPLAYER) && (Net_player != NULL)
           && ((Net_player->flags & NETINFO_FLAG_OBSERVER)
               || (Player_obj->type == OBJ_OBSERVER))))
        {
...
void player_restore_target_and_weapon_link_prefs()
{
  ...
  if (Player->flags & PLAYER_FLAGS_LINK_PRIMARY
      && !(player_sip->flags2 & SIF2_NO_PRIMARY_LINKING))
    {
          ...
          if (get_hull_pct(&Objects[Ships[ship_index].objnum]) > 0.5f)
            {
              if (Player->stats.m_kill_count_ok > 10)
                {
                  ...
                  message_send_builtin_to_player(MESSAGE_HIGH_PRAISE,
                                                 &Ships[ship_index],
                ...
                {
                  message_send_builtin_to_player(MESSAGE_PRAISE,
                                                 &Ships[ship_index],
                                                 MESSAGE_PRIORITY_HIGH,
                                                 MESSAGE_TIME_SOON, 0, 0, -1,
                                                 -1);
              ...
              Player->allow_praise_timestamp =
                timestamp(PLAYER_ALLOW_PRAISE_INTERVAL *
                          (Game_skill_level + 1));
...
int player_inspect_cargo(float frametime, char *outstr)
{
  ...
  if (Player_ai->current_target_distance <
      MAX(CARGO_REVEAL_MIN_DIST, (cargo_objp->radius + CARGO_RADIUS_DELTA)))
    {
      ...
      vm_vec_normalized_dir(&vec_to_cargo, &cargo_objp->pos,
                            &Player_obj->pos);
...
int player_inspect_cap_subsys_cargo(float frametime, char *outstr)
{
  ...
  get_subsystem_world_pos(cargo_objp, Player_ai->targeted_subsys,
                          &subsys_pos);
  ...
  if (cargo_sip->flags & SIF_HUGE_SHIP)
    ...
    {
      scan_dist =
        MAX(CARGO_REVEAL_MIN_DIST, (subsys_rad + CARGO_RADIUS_DELTA));
    }
...
void player_generate_killer_weapon_name(int weapon_info_index,
                                        int killer_species, char *weapon_name)
{
...
void player_generate_death_message(player * player_p)
{
  ...
  if (ship_index >= 0)
    {
      ...
      if (shipp->callsign_index >= 0)
        ...
        {
          end_string_at_first_hash_symbol(killer_parent_name,
                                          (shipp->wingnum != -1));
        }
    }
  else
  ...
  switch (player_p->killer_objtype)
    ...
    case OBJ_SHOCKWAVE:
      if (weapon_name[0])
        {
          sprintf(msg, XSTR("%s was killed by a missile shockwave", 92),
                  player_p->callsign);
        }
      else
        {
    ...
    case OBJ_WEAPON:
      ...
      if ((ship_index >= 0) && (Player_ship != NULL)
        ...
        {
          sprintf(msg, XSTR("%s was killed by friendly fire from %s", 1338),
                  player_p->callsign, killer_parent_name);
        }
      else
        {
          sprintf(msg, XSTR("%s was killed by %s", 94), player_p->callsign,
                  killer_parent_name);
        }
      break;
    ...
    case OBJ_SHIP:
      if (player_p->flags & PLAYER_FLAGS_KILLED_BY_EXPLOSION)
        {
          sprintf(msg, XSTR("%s was killed by a blast from %s exploding", 95),
                  player_p->callsign, killer_parent_name);
        }
      else if (player_p->flags & PLAYER_FLAGS_KILLED_BY_ENGINE_WASH)
        {
          sprintf(msg, XSTR("%s was killed by engine wash from %s", 1494),
                  player_p->callsign, killer_parent_name);
        }
      else
        {
          sprintf(msg, XSTR("%s was killed by a collision with %s", 96),
                  player_p->callsign, killer_parent_name);
        }
      break;

    case OBJ_DEBRIS:
      sprintf(msg, XSTR("%s was killed by a collision with debris", 97),
              player_p->callsign);
      break;

    case OBJ_ASTEROID:
      sprintf(msg, XSTR("%s was killed by a collision with an asteroid", 98),
              player_p->callsign);
      break;
      ...
      if (strlen(player_p->killer_parent_name) <= 0)
        {
          Int3();
          sprintf(msg,
                  XSTR("%s was killed by a beam from an unknown source",
                       1081), player_p->callsign);
        }
      else
        {
          if ((ship_index >= 0) && (Player_ship != NULL)
              && (Player_ship->team == Ships[ship_index].team))
            {
              sprintf(msg,
                      XSTR("%s was destroyed by friendly beam fire from %s",
                           1339), player_p->callsign, killer_parent_name);
            }
          else
            {
              sprintf(msg, XSTR("%s was destroyed by a beam from %s", 1082),
                      player_p->callsign, killer_parent_name);
            }
        }
      break;
  ...
  if (Viewer_mode & VM_PADLOCK_UP)
    {
      ...
      vm_vec_copy_scale(&eye_orient->vec.uvec, &old_eye_orient.vec.fvec,
                        -1.0f);
    }
    ...
    {
      vm_vec_copy_scale(&eye_orient->vec.fvec, &old_eye_orient.vec.rvec,
                        -1.0f);
  ...
  else if (Viewer_mode & VM_PADLOCK_RIGHT)
    {
      ...
      vm_vec_copy_scale(&eye_orient->vec.rvec, &old_eye_orient.vec.fvec,
                        -1.0f);
    }
  else
    {
