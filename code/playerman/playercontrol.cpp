...
 void do_view_track_target(float frame_time) 
{
    ...
    vm_vec_rotate(&targetpos_rotated, &target_pos, &Player_obj->orient);
                 ...
                 &Player_obj->orient);
   vm_vec_normalized_dir(&view_vector, &targetpos_rotated,
                           &playerpos_rotated);
...
}  void read_keyboard_controls(control_info * ci, float frame_time,
                                  physics_info * pi) 
{
                                        ...
                                        char *weapon_name) 
{
    ...
    if (Show_killer_weapon
        || (killer_species ==
            Ship_info[Player_ship->ship_info_index].species))
