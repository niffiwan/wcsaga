...
 static int lua_fs2_getState(lua_State * L) 
{
  ...
   return script_return_args(L, "s",
                               GS_state_text[gameseq_get_state(depth)]);
...
 static lua_msn_ship_setSpeed(lua_State * L) 
{
         ...
         (L, "offf", l_ship.Parse(&shipp), &vel.xyz.x, &vel.xyz.y,
          &vel.xyz.z))
...
void script_state::OutputLuaMeta(FILE * fp) 
{
  ...
  const script_lua_lib_list *lib;
    ...
    
    {
      ...
       fprintf(fp, "<dt><a href=\"#%s\">%s</a> - %s</dt>",
                 lib->library_name, lib->library_name, lib->description);
