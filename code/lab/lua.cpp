...
struct script_lua_opdata
{
  ...
  void **buf;
    ...
    script_lua_opdata(char *in_meta, void **in_buf)
  {
    meta = in_meta;
    buf = in_buf;
  }
...
struct script_lua_odata
{
  ...
  int size;
    ...
    script_lua_odata(char *in_meta, void *in_buf, int in_size)
  {
    meta = in_meta;
    buf = in_buf;
    size = in_size;
...
public:
  lua_cvar(char *in_meta)
  {
    sprintf(meta, "fs2.%s", in_meta);
  }
  ...
  script_lua_odata Return(StoreType * obj)
  {
    return script_lua_odata(meta, obj, sizeof(StoreType));
  }
  script_lua_odata Parse(StoreType * ptr)
  {
    return script_lua_odata(meta, ptr, sizeof(StoreType));
  }
  script_lua_opdata ParsePtr(StoreType ** ptr)
  {
    return script_lua_opdata(meta, ptr);
  }
};
  ...
  for (int argnum = 1; argnum <= stacksize; argnum++)
    {
      ...
      switch (type)
        ...
        case LUA_TLIGHTUSERDATA:
          ...
          break;
        default:
          sprintf(buf, "<UNKNOWN>: %s (%d) (%s)", lua_typename(L, type),
      ...
      switch (*fmt++)
        ...
        case 'b':
          if (lua_isboolean(L, nargs))
            ...
            {
              Warning(LOCATION,
                      "Argument %d is an invalid type; boolean expected");
              if (!optional_args)
                return 0;
            }
          break;
        case 'd':
          if (lua_isnumber(L, nargs))
            ...
            {
              Warning(LOCATION,
                      "Argument %d is an invalid type; float expected",
                      nargs);
              if (!optional_args)
                return 0;
            }
        ...
        case 'i':
          if (lua_isnumber(L, nargs))
            ...
            {
              Warning(LOCATION,
                      "Argument %d is an invalid type; int expected", nargs);
              if (!optional_args)
                return 0;
            }
          break;
        case 's':
          if (lua_isstring(L, nargs))
            ...
            {
              Warning(LOCATION,
                      "Argument %d is an invalid type; string expected",
                      nargs);
              if (!optional_args)
                return 0;
            }
          ...
          if (lua_isuserdata(L, nargs))
            {
              ...
              if (ptr == NULL)
                {
                  Warning(LOCATION,
                          "Argument %d is the wrong type of userdata; %s expected",
                          nargs, od.meta);
                }
            ...
            {
              Warning(LOCATION,
                      "Argument %d is an invalid type %d; type '%s' expected",
                      nargs, lua_type(L, nargs), va_arg(vl,
                                                        script_lua_odata).
                      meta);
              if (!optional_args)
                return 0;
        ...
        case '|':
          ...
          break;
        default:
          Error(LOCATION, "Bad character passed to script_parse_args; (%c)",
                *(fmt - 1));
          break;
        }
      nargs++;
  ...
  while (*fmt)
    {
      switch (*fmt++)
        ...
        default:
          Error(LOCATION, "Bad character passed to script_return_args; (%c)",
                *fmt);
        }
      nargs++;
    }
...
static const script_lua_obj_func_list lua_fs2_lvec_funcs[] = {
  ...
  {LUA_OPER_SUBTRACTION, lua_fs2_vec__subtraction,
   "Subtracts one vector from another"},
  {LUA_OPER_MULTIPLICATION, lua_fs2_vec__multiplication,
   "Multiplies two vectors, or a vector times a number"},
...
static int lua_msn_newShip(lua_State * L)
{
  ...
  if (!script_parse_args
      (L, "ssfff|fff", &ship_name, &class_name, &pos.xyz.x, &pos.xyz.y,
       &pos.xyz.z, &ang.p, &ang.b, &ang.h))
    return 0;
...
static lua_msn_ship_setSpeed(lua_State * L)
{
  ...
  if (!script_parse_args
      (L, "offf", l_ship.Parse(&shipp), &vel.xyz.x, &vel.xyz.y, &vel.xyz.z))
    return LUA_RETURN_NOTHING;
...
static const script_lua_obj_func_list lua_msn_ship_funcs[] = {
  {"setSpeed", lua_msn_ship_setSpeed,
   "Sets ship speed: (x speed, y speed, z speed)"},
  {"setName", lua_msn_ship_setName, "Changes ship name: (new name)"},
  {"getTarget", lua_msn_ship_getTarget,
   "Returns handle to ship's targetted ship"},
  {SCRIPT_END_LIST},
};
...
static const script_lua_func_list lua_msn_lib[] = {
  {"newShip", lua_msn_newShip,
   "Creates a new ship and returns a handle to it: (ship name, class name, x pos, y pos, z pos; x rot, y rot, z rot)"},
...
static int lua_grpc_model_getEyepointPos(lua_State * L)
{
  ...
  if (idx > -1 && idx < pm->n_view_positions)
    {
      return script_return_args(L, "o",
                                l_lvec.Return(&pm->view_positions[idx].pnt));
    }
...
static const script_lua_obj_func_list lua_grpc_model_funcs[] = {
  {
   ...
   NULL, "string"},
  {
   "getnumEyepoints", lua_grpc_model_getnumEyepoints,
   "Gets number of eyepoints on ship",
   NULL, "number"},
  {
   "getEyepointPos", lua_grpc_model_getEyepointPos,
   "Gets selected eye position",
   "number", "lvec"},
  {SCRIPT_END_LIST},
};
...
static const script_lua_func_list lua_grpc_lib[] = {
  {
   "loadModel", lua_grpc_loadModel,
   "Loads the given model and returns a handle to it",
   "string", "model"},
  {"setColor", lua_grpc_setColor,
   "Sets the current drawing color: (red, green, blue; alpha)"},
  {"setFont", lua_grpc_setFont,
   "Sets the current drawing font: (font number)"},
  {"drawPixel", lua_grpc_drawPixel,
   "Draws a pixel using the current color: (x, y; resize)"},
  {"drawLine", lua_grpc_drawLine,
   "Draws a line using the current color: (x1, y1, x2, y2; resize)"},
  {"drawGradientLine", lua_grpc_drawGradientLine,
   "Draws a line using the current color that gradually fades out: (x1, y1, x2, y2; resize)"},
  {"drawCircle", lua_grpc_drawCircle,
   "Draws a circle using the current color: (x,y,radius; resize)"},
  {"drawCurve", lua_grpc_drawCurve,
   "Draws a curve using the current color: (x,y,radius,direction)"},
  {"drawText", lua_grpc_drawText,
   "Draws text using the current font and color: (x, y, text; resize)"},
  {"flashScreen", lua_grpc_flashScreen,
   "Flashes the screen with the specified color: (red,green,blue)"},
  {SCRIPT_END_LIST},
};
  ...
  for (; lib->library_funcs != NULL; lib++)
    {
      ...
      if (lib->library_objects != NULL)
        {
          for (obj = (script_lua_obj_list *) lib->library_objects;
               obj->object_funcs != NULL; obj++)
            {
              if (!luaL_newmetatable(L, obj->object_name))
                {
                  Warning(LOCATION,
                          "Couldn't create metatable for object '%s'",
                          obj->object_name);
                  continue;
                }
              ...
              for (ofunc = (script_lua_obj_func_list *) obj->object_funcs;
                   ofunc->name || ofunc->func; ofunc++)
                {
                      ...
                      if (!stricmp(ofunc->name, "__index"))
                        {
                          if (!index_meth_already)
                            ...
                            {
                              Error(LOCATION,
                                    "Attempt to set both an indexing operator and methods for Lua class '%s'; get a coder",
                                    obj->object_name);
                            }
                        }
  ...
  if (lib->library_objects != NULL)
    {
      ...
      for (obj = lib->library_objects; obj->object_name != NULL; obj++)
        {
          fprintf(fp, "<dt id=\"%s.%s\"><h3>%s</h3></dt>", lib->library_name,
          ...
          for (ofunc = obj->object_funcs; ofunc->name != NULL; ofunc++)
            {
              ...
              if (ofunc->retvals != NULL)
                {
                  fprintf(fp, "<dd><b>Return values: %s</dd>",
                          ofunc->retvals);
                }
              else
                {
      ...
      if (lib->library_objects != NULL)
        {
          for (obj = lib->library_objects; obj->object_name != NULL; obj++)
            {
              fprintf(fp, "<dd><a href=\"#%s.%s\">%s</a> - %s</dd>",
                      lib->library_name, obj->object_name, obj->object_name,
                      obj->description);
            }
        }
    }
  ...
  for (lib = LuaLibs; lib->library_funcs != NULL; lib++)
    {
      ...
      fprintf(fp, "<dt><a href=\"#%s\">%s</a> - %s</dt>", lib->library_name,
              lib->library_name, lib->description);
      if (lib->library_objects != NULL)
        {
          for (obj = lib->library_objects; obj->object_name != NULL; obj++)
            {
  ...
  for (lib = LuaLibs; lib->library_funcs != NULL; lib++)
    {
      ...
      fprintf(fp, "<dt id=\"%s\"><h2>%s - %s</h2></dt>", lib->library_name,
              lib->library_name, lib->description);
