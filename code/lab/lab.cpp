      ...
      for (i = 0; i < pm->num_debris_objects; i++)
        {
          ...
          model_find_world_point(&world_point,
                                 ...
                                 Lab_model_num, -1, &Lab_viewer_orient,
                                 &vmd_zero_vector);

          submodel_render(Lab_model_num, pm->debris_objects[i],
                          &Lab_viewer_orient, &world_point, Lab_model_flags,
                          -1);
        }
    }
  ...
  if (!(Lab_model_flags & MR_NO_GLOWMAPS)
      && (wip->laser_glow_bitmap.first_frame >= 0))
    {
      ...
      gr_set_bitmap(wip->laser_glow_bitmap.first_frame + framenum,
                    ...
                    weapon_glow_alpha);
      g3_draw_laser_rgb(&headp2, wip->laser_head_radius * weapon_glow_scale_f,
                        &tailp, wip->laser_tail_radius * weapon_glow_scale_r,
                        c.red, c.green, c.blue,
                        TMAP_FLAG_TEXTURED | TMAP_FLAG_XPARENT | TMAP_FLAG_RGB
                        | TMAP_HTL_3D_UNLIT);
    }
  ...
  Lab_description_window =
    ...
    Add(new
        Window("Description", gr_screen.max_w - gr_screen.max_w / 3 - 50,
               ...
               gr_screen.max_w / 3, gr_screen.max_h / 6));
  ...
  if (Lab_viewer_flags & LAB_FLAG_FULLY_LOAD)
    {
      ...
      int test_idx =
        ship_create(&vmd_identity_matrix, &vmd_zero_vector, ship_index);
      obj_delete(test_idx);
    }
