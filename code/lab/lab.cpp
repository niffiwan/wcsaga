...
void labviewer_setup_subsys_rotation() 
{
    ...
    (ship_subsys *) vm_malloc(n_subsystems * sizeof(ship_subsys));
    ...
    {
      Lab_ship_model_subsys.push_back(Ship_info[Lab_selected_index].
                                       subsystems[idx]);
...
 void labviewer_change_model(char *model_fname, int lod = 0, int sel_index =
                               -1) 
{
    ...
    
    {
        ...
        {
          memcpy(&Lab_viewer_pos, &Ship_info[sel_index].closeup_pos,
                  sizeof(vec3d));
        ...
        
        {
            ...
            
            {
              ...
               for (int idx = 0; idx < Ship_info[sel_index].n_subsystems;
                      idx++)
                
                {
                  model_set_instance_info(&Lab_ship_subsys[idx].
                                           submodel_info_1,
                                           Lab_ship_model_subsys[idx].
                                           turn_rate, 0.5f);
...
 void labviewer_add_model_arcs() 
{
    ...
    
    {
           ...
           && timestamp_elapsed(Lab_arc_timestamp[i]))
            ...
            
            {
              ...
              int mr = myrand();
                ...
                
                {
                  ...
                  submodel_get_two_random_points(Lab_model_num, -1, &v1,
                                                  &v2);
    ...
    
    {
        ...
        {
          model_add_arc(Lab_model_num, -1, &Lab_arc_pts[i][0],
                         &Lab_arc_pts[i][1], Lab_arc_type[i]);
...
 void labviewer_add_model_thrusters(ship_info * sip) 
{
  ...
  extern void ship_init_thrusters();
      ...
      glow_anim = &species->thruster_info.glow.normal;  // select normal glow
        ...
        
        {
          ...
          secondary_glow_bitmap =
            ...
            sip->thruster_tertiary_glow_info.normal.bitmap_id;
    ...
    
    {
        ...
        fl2i((thruster_frame * flame_anim->num_frames) /
    ...
    
    {
        ...
        fl2i((thruster_glow_frame * glow_anim->num_frames) /
...
 void labviewer_render_model(float frametime) 
{
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                dx * (double) model_get_radius(Lab_model_num) * 0.005;
              double scale_y =
                dy * (double) model_get_radius(Lab_model_num) * 0.005;
    ...
    
    {
      ...
       g3_set_view_matrix(&Lab_viewer_pos, &vmd_identity_matrix,
                            Lab_viewer_zoom);
    }
   if (!Cmdline_nohtl)
    
    {
      gr_set_proj_matrix(Proj_fov, gr_screen.clip_aspect, 1.0f,
                          Max_draw_distance);
    ...
    
    {
        ...
        
        {
          ...
           model_find_world_point(&world_point,
                                    &pm->submodel[pm->debris_objects[i]].
                                    offset, Lab_model_num, -1,
                                    &Lab_viewer_orient, &vmd_zero_vector);
           submodel_render(Lab_model_num, pm->debris_objects[i],
                             &Lab_viewer_orient, &world_point,
                             Lab_model_flags, -1);
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
               model_set_instance(Lab_model_num,
                                    Lab_ship_model_subsys[i].subobj_num,
                                    &Lab_ship_subsys[i].submodel_info_1);
               
                // if we got this far, we can rotate - so choose which method to use
                if (Lab_ship_model_subsys[i].flags & MSS_FLAG_STEPPED_ROTATE)
                
                {
                  submodel_stepped_rotate(&Lab_ship_model_subsys[i],
                                           &Lab_ship_subsys[i].
                                           submodel_info_1);
                ...
                {
                  submodel_rotate(&Lab_ship_model_subsys[i],
                                   &Lab_ship_subsys[i].submodel_info_1);
                }
            }
        }
       model_render(Lab_model_num, &Lab_viewer_orient, &vmd_zero_vector,
                      /*Lab_model_flags */ flagggs, -1, -1);
...
 void labviewer_render_bitmap(float frametime) 
{
    ...
    if ((Lab_selected_index < 0) || (Lab_mode != LAB_MODE_WEAPON)
        || (Lab_bitmap_id < 0))
    ...
    
    {
        ...
        fl2i((current_frame * wip->laser_bitmap.num_frames) /
             wip->laser_bitmap.total_time);
       CLAMP(framenum, 0, wip->laser_bitmap.num_frames - 1);
    }
   vec3d headp;
  vm_vec_scale_add(&headp, &vmd_zero_vector, &Lab_viewer_orient.vec.fvec,
                    wip->laser_length);
   gr_set_bitmap(wip->laser_bitmap.first_frame + framenum,
                   GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL, 0.99999f);
  g3_draw_laser(&headp, wip->laser_head_radius, &vmd_zero_vector,
    ...
    if (!(Lab_model_flags & MR_NO_GLOWMAPS)
        && (wip->laser_glow_bitmap.first_frame >= 0))
    
    {
        ...
        // get the laser color
        if ((wip->laser_color_2.red == 0) && (wip->laser_color_2.green == 0)
            && (wip->laser_color_2.blue == 0))
        ...
        
        {
          ...
          float pct = 1.0f;
            ...
            gr_init_color(&c,
                          (int) ((float) wip->laser_color_1.red +
                                 (((float) wip->laser_color_2.red -
                                   (float) wip->laser_color_1.red) * pct)),
                          (int) ((float) wip->laser_color_1.green +
                                  (((float) wip->laser_color_2.green -
                                    (float) wip->laser_color_1.green) * pct)),
                          (int) ((float) wip->laser_color_1.blue +
                                  (((float) wip->laser_color_2.blue -
                                    (float) wip->laser_color_1.blue) * pct)));
        }  vm_vec_scale_add(&headp2, &vmd_zero_vector,
                               &Lab_viewer_orient.vec.fvec,
        ...
        
        {
            ...
            fl2i((current_glow_frame * wip->laser_glow_bitmap.num_frames) /
                 wip->laser_glow_bitmap.total_time);
           CLAMP(framenum, 0, wip->laser_glow_bitmap.num_frames - 1);
        }
       gr_set_bitmap(wip->laser_glow_bitmap.first_frame + framenum,
                       ...
                       weapon_glow_alpha);
                         ...
                         TMAP_FLAG_RGB | TMAP_HTL_3D_UNLIT);
...
 void labviewer_do_render(float frametime) 
{
    ...
    
    {
      ...
      gr_string(gr_screen.clip_right - w, gr_screen.clip_bottom - h,
                 "Viewer off", false);
  ...
  extern void game_get_framerate();
    ...
    {
      gr_printf_no_resize(gr_screen.clip_left + 2,
                           gr_screen.clip_bottom - gr_get_font_height(),
                           "FPS: %i", fl2i(Framerate + 0.5f));
    }
  
  else
    
    {
      gr_string(gr_screen.clip_left + 10,
                 gr_screen.clip_bottom - gr_get_font_height(), "FPS: ?",
                 false);
...
 void labviewer_set_class_window(int mode) 
{
    ...
    
    {
        ...
        (Window *) Lab_screen->Add(new Window("Class Window", 50, 50));
    ...
    
    {
        ...
        (Window *) Lab_screen->
        Add(new
            Window("Class Toolbar", 0, Lab_toolbar->GetHeight(), -1, -1,
...
 void labviewer_flags_add(int *X, int *Y, char *flag_name, int flag,
                            bool flags2 = false) 
{
...
 void labviewer_populate_flags_window() 
{
    ...
    
    {
      ...
      labviewer_flags_add(NULL, &y, "NO COLLIDE INVISIBLE",
      ...
      labviewer_flags_add(NULL, &y, "DEFAULT PLAYER SHIP",
                           SIF_DEFAULT_PLAYER_SHIP);
      labviewer_flags_add(NULL, &y, "BALLISTIC PRIMARIES",
                           SIF_BALLISTIC_PRIMARIES);
      labviewer_flags_add(NULL, &y, "FLASH", SIF2_FLASH, true);
      labviewer_flags_add(NULL, &y, "SURFACE SHIELDS", SIF2_SURFACE_SHIELDS,
                           true);
      labviewer_flags_add(NULL, &y, "SHOW SHIP MODEL", SIF2_SHOW_SHIP_MODEL,
                           true);
      labviewer_flags_add(NULL, &y, "IN TECH DATABASE",
                           SIF_IN_TECH_DATABASE);
      labviewer_flags_add(NULL, &y, "IN TECH DATABASE MULTI",
    ...
    
    {
      ...
      labviewer_flags_add(NULL, &y, "PIERCE SHIELDS", WIF2_PIERCE_SHIELDS,
                           true);
      labviewer_flags_add(NULL, &y, "CYCLE", WIF2_CYCLE, true);
      labviewer_flags_add(NULL, &y, "NO LIGHTING", WIF2_MR_NO_LIGHTING,
                           true);
      labviewer_flags_add(NULL, &y, "TRANSPARENT", WIF2_TRANSPARENT, true);
      labviewer_flags_add(NULL, &y, "IN TECH DATABASE",
                           WIF_IN_TECH_DATABASE);
...
}  void labviewer_make_flags_window(Button * caller) 
{
    ...
    
    {
        ...
        (Window *) Lab_screen->
        Add(new Window("Flags Window", gr_screen.max_w - 205, 200));
...
#define VAR_ADD_HEADER(name) {  \
  ntp =
  (Text *) Lab_variables_window->
AddChild(new
         Text((name), (name), VAR_POS_RIGHTX / 2, y + 8, VAR_POS_RIGHTWIDTH));
\y += ntp->GetHeight() + 10;
\}
...
 void labviewer_variables_add(int *Y, char *var_name) 
{
    ...
    // variable
    Lab_variables_window->
    AddChild(new Text((var_name), (var_name), 0, y, VAR_POS_LEFTWIDTH));
  
    // edit box
    new_text =
    (Text *) Lab_variables_window->
    AddChild(new
             Text(std::string((var_name)) + std::string("Editbox"), "",
...
 void labviewer_update_variables_window() 
{
    ...
    
    {
      ...
      VAR_SET_VALUE_SAVE(sip->shield_icon_index,
                          Hud_shield_filename_count - 1);
...
 void labviewer_make_variables_window(Button * caller) 
{
    ...
    Add(new
        Window("Class Variables",
               gr_screen.max_w - (VAR_POS_RIGHTX + VAR_POS_RIGHTWIDTH + 25),
               200));
...
#define ADD_RENDER_FLAG(text, flag, var) {  \
  cbp =
  (Checkbox *) Lab_render_options_window->
AddChild(new Checkbox((text), 2, y));
     ...
      void labviewer_make_render_options_window(Button * caller) 
{
  ...
   Lab_render_options_window =
    (Window *) Lab_screen->
    Add(new Window("Render Options", gr_screen.max_w - 300, 200));
  Assert(Lab_render_options_window != NULL);
   
    // add all of the flags that we want/need...
    
    // viewer flags
    ADD_RENDER_FLAG("Disable Model Rotation", Lab_viewer_flags,
                    LAB_FLAG_NO_ROTATION);
  ADD_RENDER_FLAG("Show Insignia", Lab_viewer_flags, LAB_FLAG_SHOW_INSIGNIA);
  ADD_RENDER_FLAG("Show Damage Lightning", Lab_viewer_flags,
                   LAB_FLAG_LIGHTNING_ARCS);
  ADD_RENDER_FLAG("Rotate Subsystems", Lab_viewer_flags,
                   LAB_FLAG_SUBMODEL_ROTATE);
    ...
    ADD_RENDER_FLAG("Wireframe", Lab_model_flags,
                    ((Cmdline_nohtl) ? MR_SHOW_OUTLINE : MR_SHOW_OUTLINE_HTL)
                    | MR_NO_POLYS);
    ...
    AddChild(new
             Tree("Detail Options Tree", 0, y + 2, NULL,
    ...
    // set close function
    Lab_render_options_window->
    SetCloseFunction(labviewer_close_render_options_window);
...
}  void labviewer_update_desc_window() 
{
    ...
    
    {
        ...
        {
          Lab_description_window->SetCaption(Ship_info[Lab_selected_index].
                                              name);
           if (Ship_info[Lab_selected_index].tech_desc != NULL)
            
            {
              Lab_description_text->SetText(Ship_info[Lab_selected_index].
                                             tech_desc);
        ...
        {
          Lab_description_window->SetCaption(Weapon_info[Lab_selected_index].
                                              name);
           if (Weapon_info[Lab_selected_index].tech_desc != NULL)
            
            {
              Lab_description_text->SetText(Weapon_info[Lab_selected_index].
                                             tech_desc);
...
 void labviewer_make_desc_window(Button * caller) 
{
  ...
   Lab_description_window =
    (Window *) Lab_screen->
    Add(new
        Window("Description", gr_screen.max_w - gr_screen.max_w / 3 - 50,
               gr_screen.max_h - gr_screen.max_h / 6 - 50,
               gr_screen.max_w / 3, gr_screen.max_h / 6));
  Lab_description_text =
    (Text *) Lab_description_window->
    AddChild(new Text("Description Text", "No ship selected.", 0, 0));
...
void labviewer_make_ship_window(Button * caller) 
{
    ...
    Lab_class_toolbar->
    AddChild(new
             Button("Class Description", x, 0, labviewer_make_desc_window));
   x += cbp->GetWidth() + 10;
  cbp =
    Lab_class_toolbar->
    AddChild(new Button("Class Options", x, 0, labviewer_make_flags_window));
   x += cbp->GetWidth() + 10;
  cbp =
    Lab_class_toolbar->
    AddChild(new
             Button("Class Variables", x, 0,
                    labviewer_make_variables_window));
    ...
    {
      if ((Ship_info[idx].species >= 0)
           && (Ship_info[idx].species < (int) Species_info.size()))
        ...
        cmp->AddItem(stip, Ship_info[idx].name, idx, false,
        ...
        
        {
            ...
            
            {
              ...
               cmp->AddItem(ctip, lod_name, j, false,
                              labviewer_change_ship_lod);
            }  cmp->AddItem(ctip, "Debris", 99, false,
                               labviewer_change_ship_lod);
...
 void labviewer_change_ship_lod(Tree * caller) 
{
    ...
    (int) (caller->GetSelectedItem()->GetParentItem()->GetData());
                            ...
                            caller->GetSelectedItem()->GetData(), ship_index);
...
void labviewer_show_tech_model(Tree * caller) 
{
  ...
   labviewer_change_model(Weapon_info[weap_index].tech_model,
...
}  void labviewer_change_weapon(Tree * caller) 
{
    ...
    
    {
        ...
        
        {
          ...
          labviewer_change_model(Weapon_info[weap_index].pofbitmap_name, 0,
                                  weap_index);
...
void labviewer_make_weap_window(Button * caller) 
{
    ...
    
    {
        ...
        {
          Warning(LOCATION, "Invalid weapon subtype found on weapon %s",
                   Weapon_info[i].name);
        ...
        cmp->AddItem(stip, Weapon_info[i].name, i, false,
...
void lab_init() 
{
    ...
    (Window *) Lab_screen->
    Add(new Window("Toolbar", 0, 0, -1, -1, WS_NOTITLEBAR | WS_NONMOVEABLE));
   
    // start filling the main toolbar
    x = 0;
  cbp =
    Lab_toolbar->
    AddChild(new Button("Ships", x, 0, labviewer_make_ship_window));
   x += cbp->GetWidth() + 10;
  cbp =
    Lab_toolbar->
    AddChild(new Button("Weapons", x, 0, labviewer_make_weap_window));
    ...
    
    {
        ...
        Lab_toolbar->
        AddChild(new
                 Button("Render Options", x, 0,
                        labviewer_make_render_options_window));
...
void lab_do_frame(float frametime) 
{
    ...
    (GUI_system.
     OnFrame(frametime, !(Trackball_active) ? true : false,
             false) == GSOF_NOTHINGPRESSED);
