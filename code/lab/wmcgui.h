...
class GUIObject:public LinkedList 
{
...
private:class GUISystem * OwnerSystem;
                                        //What system this object is associated with
  ...
  int GetCIEImageHandle(int id, int handleid = 0)
  {
    ...
    GUIObject(std::string in_Name = "", int x_coord = 0, int y_coord =
              0, int x_width = -1, int y_height = -1, int in_style = 0);
...
 public:Window(std::string in_caption, int x_coord, int y_coord, int x_width =
          -1, int y_height = -1, int in_style = 0);
...
public:Button(std::string in_caption, int x_coord, int y_coord,
          void (*in_function) (Button * caller) = NULL, int x_width =
          -1, int y_height = -1, int in_style = 0);
...
public:Tree(std::string in_name, int x_coord, int y_coord,
        void *in_associateditem = NULL, int x_width = -1, int y_width =
        -1, int in_style = 0);
   
    //void LoadItemList(TreeItem *in_list, unsigned int count);
  TreeItem * AddItem(TreeItem * parent, std::string in_name, int in_data =
                       0, bool in_delete_data =
                       true, void (*in_function) (Tree * caller) = NULL);
...
public:Text(std::string in_name, std::string in_content, int x_coord, int y_coord,
        int x_width = -1, int y_width = -1, int in_style = 0);
   
    //Set
  void SetText(std::string in_content);
  void SetText(int the_int);
  void SetText(float the_float);
  void SetSaveLoc(int *ptr, int save_method, int max_value =
                   INT_MAX, int min_value = INT_MIN);
  void SetSaveLoc(short int *sint_ptr, int save_method, short int max_value =
                   SHRT_MAX, short int min_value = SHRT_MIN);
  void SetSaveLoc(float *ptr, int save_method, float max_value =
                   INT_MAX, float min_value = INT_MIN);
  void SetSaveLoc(char *ptr, int save_method, uint max_len =
                   UINT_MAX, uint min_len = 0);
  void SetSaveLoc(ubyte * ptr, int save_method, int max_value =
                   UCHAR_MAX, int min_value = 0);
  void SetSaveStringAlloc(char **ptr, int save_method, int mem_flags,
                           uint max_len = UINT_MAX, uint min_len = 0);
...
 public:Checkbox(std::string in_label, int x_coord, int y_coord,
            void (*in_function) (Checkbox * caller) = NULL, int x_width =
            -1, int y_height = DEFAULT_BUTTON_HEIGHT, int in_style = 0);
...
public:ImageAnim(std::string in_name, std::string in_imagename, int x_coord,
             int y_coord, int x_width = -1, int y_width = -1, int in_style =
             0);
