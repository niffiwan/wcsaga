...
 ObjectClassInfoEntry * GUIScreen::GetObjectClassInfo(GUIObject * cgp) 
{
    ...
    
    {
        ...
        
        {
               ...
               && OwnerSystem->GetClassInfo()->Entries[i].Object ==
               cgp->Type)
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
              GetCIECoords(WCI_CAPTION, &CaptionCoords[0],
                            &CaptionCoords[1]);
...
 int Window::DoMouseDown(float frametime) 
{
         ...
         &&((OwnerSystem->GetMouseX() < CaptionCoords[2])
            || ((Style & WS_NOTITLEBAR)
                && OwnerSystem->GetMouseX() <=
                Coords[2]))  &&OwnerSystem->GetMouseY() >=
...
 void Window::DoDraw(float frametime) 
{
    ...
    
    {
        ...
        //Caption text
        gr_set_color_fast(&Color_text_normal);
       gr_string(CaptionCoords[0], CaptionCoords[1],
                   (char *) Caption.c_str(), false);
...
 Window::Window(std::string in_caption, int x_coord, int y_coord,
                  int x_width, int y_width,
                  int in_style) :GUIObject(in_caption, x_coord, y_coord,
                                            x_width, y_width, in_style) 
{
...
}  TreeItem * Tree::AddItem(TreeItem * parent, std::string in_name,
                               int in_data, bool in_delete_data,
...
GUIObject(in_name, x_coord, y_coord, x_width, y_width,
          in_style) 
{
