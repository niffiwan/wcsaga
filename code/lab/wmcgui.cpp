      ...
      if (in_type == CIE_IMAGE || in_type == CIE_IMAGE_NMCSD)
        {
          ...
          if (IMG_HANDLE_IS_VALID(Handles[CIE_HANDLE_N].Image) && num_frames)
            {
              if (num_frames > 1)
                IMG_HANDLE_SET_FRAME(Handles[CIE_HANDLE_N].Image,
...
GUIObject *GUIScreen::Add(GUIObject * new_gauge)
{
  ...
  new_gauge->GetOIECoords(&new_gauge->Coords[0], &new_gauge->Coords[1],
                          &new_gauge->Coords[2], &new_gauge->Coords[3]);
...
ScreenClassInfoEntry *GUISystem::GetScreenClassInfo(const std::
                                                    string & screen_name)
{
              ...
              if (Status & GraspingButton)
                {
                  ...
                  GraspedGuiobject->SetPosition(MouseX - GraspedDiff[0],
                                                MouseY - GraspedDiff[1]);
...
//*****************************GUIObject*******************************
GUIObject::GUIObject(std::string in_Name, int x_coord, int y_coord,
                     int x_width, int y_height, int in_style)
{
  //General stuff
  LastStatus = Status = 0;
  OwnerSystem = NULL;
...
GUIObject *GUIObject::AddChildInternal(GUIObject * cgp)
{
  ...
  cgp->SetCIPointer();
  //Check position
  cgp->GetOIECoords(&cgp->Coords[0], &cgp->Coords[1], &cgp->Coords[2],
                    &cgp->Coords[3]);
  //In case we need to resize
  cgp->OnRefreshSize();
...
int Window::DoRefreshSize()
{
  ...
  if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_ML)))
    IMG_INFO(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_ML), &BorderSizes[0],
             NULL);
  else
  ...
  if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_MR)))
    IMG_INFO(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_MR), &BorderSizes[2],
             NULL);
  else
  ...
  if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_TL)))
    IMG_INFO(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_TL), NULL,
             &BorderSizes[1]);
  else if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_TR)))
    IMG_INFO(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_TR), NULL,
             &BorderSizes[1]);
  else if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_TM)))
    IMG_INFO(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_TM), NULL,
             &BorderSizes[1]);
  else
    {
      custom_top = false;
  ...
  if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_BL)))
    IMG_INFO(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_BL), NULL,
             &BorderSizes[3]);
  else if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_BR)))
    IMG_INFO(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_BR), NULL,
             &BorderSizes[3]);
  else if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_BM)))
    IMG_INFO(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_BM), NULL,
             &BorderSizes[3]);
  else
  ...
  if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_TL)))
    IMG_INFO(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_BL), &CornerWidths[0],
             NULL);
  else
    CornerWidths[0] = BorderSizes[0];

  if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_TR)))
    IMG_INFO(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_TR), &CornerWidths[1],
             NULL);
  else
    CornerWidths[1] = BorderSizes[2];

  if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_BL)))
    IMG_INFO(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_BL), &CornerWidths[2],
             NULL);
  else
    CornerWidths[2] = BorderSizes[0];

  if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_BR)))
    IMG_INFO(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_BL), &CornerWidths[3],
             NULL);
  else
          ...
          if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_CAPTION)))
            {
              ...
              num = (float) w / cw;
              CaptionRectList =
                bitmap_rect_list(Coords[0] + CornerWidths[2], Coords[1], w,
                                 ch, 0, 0, 1.0f, 1.0f);
              h = ch;
            }
          else
  ...
  if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_TM)))
    {
      ...
      num =
        (float) ((Coords[2] - CornerWidths[3]) -
                 (Coords[0] + CornerWidths[2])) / (float) w;
      BorderRectLists[CIE_HANDLE_TM] =
        bitmap_rect_list(Coords[0] + CornerWidths[2], Coords[1],
                         fl2i(w * num), h, 0, 0, num, 1.0f);
  ...
  if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_BM)))
    {
      ...
      num =
        (float) ((Coords[2] - CornerWidths[3]) -
                 (Coords[0] + CornerWidths[2])) / (float) w;
      BorderRectLists[CIE_HANDLE_BM] =
        bitmap_rect_list(Coords[0] + CornerWidths[2], Coords[3] - h,
                         fl2i(w * num), h, 0, 0, num, 1.0f);
      //gr_bitmap_list(&BorderRectLists[CIE_HANDLE_BM], 1, false);
    }
  if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_ML)))
    {
      ...
      num =
        (float) ((Coords[3] - BorderSizes[3]) -
                 (Coords[1] + BorderSizes[1])) / (float) h;
      BorderRectLists[CIE_HANDLE_ML] =
        bitmap_rect_list(Coords[0], Coords[1] + BorderSizes[1], w,
                         fl2i(h * num), 0, 0, 1.0f, num);
    }
  if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_MR)))
    {
      ...
      num =
        (float) ((Coords[3] - BorderSizes[3]) -
                 (Coords[1] + BorderSizes[1])) / (float) h;
      BorderRectLists[CIE_HANDLE_MR] =
        bitmap_rect_list(Coords[2] - w, Coords[1] + BorderSizes[1], w,
                         fl2i(h * num), 0, 0, 1.0f, num);
    }
  ...
  if (HideHighlight)
    {
      if (Style & GS_HIDDEN)
      ...
      else
        {
          ...
          Coords[3] =
            Coords[1] + (CaptionCoords[3] - CaptionCoords[1]) +
...
void Window::DoDraw(float frametime)
{
  ...
  if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_TM)))
    ...
    {
      gr_line(Coords[0] + BorderSizes[0], Coords[1],
              Coords[2] - BorderSizes[2], Coords[1], false);
    }
  ...
  if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_BM)))
    ...
    {
      gr_line(Coords[0] + BorderSizes[0], Coords[3],
              Coords[2] - BorderSizes[2], Coords[3], false);
    }
  ...
  if (!(Style & GS_HIDDEN))
    {
      if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_ML)))
        ...
        {
          gr_line(Coords[0], Coords[1] + BorderSizes[1], Coords[0],
                  Coords[3] - BorderSizes[3], false);
        }
      ...
      if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_BORDER, CIE_HANDLE_MR)))
        ...
        {
          gr_line(Coords[2], Coords[1] + BorderSizes[1], Coords[2],
                  Coords[3] - BorderSizes[3], false);
        }
    }
  ...
  if (!(Style & WS_NOTITLEBAR))
    {
      ...
      if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_CLOSE)))
        {
          if (CloseHighlight
              &&
              IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_CLOSE, CIE_HANDLE_M)))
      ...
      if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_HIDE)))
        {
          if (HideHighlight
              &&
              IMG_HANDLE_IS_VALID(GetCIEImageHandle(WCI_HIDE, CIE_HANDLE_M)))
      ...
      gr_string(CaptionCoords[0], CaptionCoords[1], (char *) Caption.c_str(),
                false);
    }
}
...
Window::Window(std::string in_caption, int x_coord, int y_coord, int x_width,
               int y_width, int in_style):GUIObject(in_caption, x_coord,
                                                    y_coord, x_width, y_width,
                                                    in_style)
{
  Caption = in_caption;
...
Button::Button(std::string in_caption, int x_coord, int y_coord,
               void (*in_function) (Button * caller), int x_width,
               int y_height, int in_style):GUIObject(in_caption, x_coord,
                                                     y_coord, x_width,
                                                     y_height, in_style)
{
  Caption = in_caption;
  ...
  if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(BCI_BUTTON)))
    {
      ...
      if ((Status & GST_MOUSE_OVER)
          && IMG_HANDLE_IS_VALID(GetCIEImageHandle(BCI_BUTTON, CIE_HANDLE_M)))
        IMG_SET(GetCIEImageHandle(BCI_BUTTON, CIE_HANDLE_M));
      else if ((Status & GST_MOUSE_LEFT_BUTTON)
               &&
               IMG_HANDLE_IS_VALID(GetCIEImageHandle
                                   (BCI_BUTTON, CIE_HANDLE_C)))
        IMG_SET(GetCIEImageHandle(BCI_BUTTON, CIE_HANDLE_C));
      else if ((Style & BS_STICKY) && IsDown
               &&
               IMG_HANDLE_IS_VALID(GetCIEImageHandle
                                   (BCI_BUTTON, CIE_HANDLE_S)))
...
Tree::Tree(std::string in_name, int x_coord, int y_coord, void *in_associateditem, int x_width, int y_height, int in_style):GUIObject(in_name, x_coord, y_coord, x_width, y_height,
          in_style)
{
      ...
      if (tip->ShowThis)
        {
          ...
          gr_string(tip->Coords[0], tip->Coords[1],
                    (char *) tip->Name.c_str(), false);
...
TreeItem *Tree::AddItem(TreeItem * parent, std::string in_name, int in_data,
                        bool in_delete_data,
...
int Text::DoRefreshSize()
{
  ...
  NumLines =
    split_str((char *) Content.c_str(), width, LineLengths, LineStartPoints,
              MAX_TEXT_LINES);
  ...
  for (int i = 0; i < NumLines; i++)
    {
      gr_get_string_size(&width_test, NULL, LineStartPoints[i],
                         LineLengths[i]);
...
void Text::DoDraw(float frametime)
{
  ...
  for (int i = 0; i < NumLines; i++)
    {
      gr_string(ChildCoords[0], ChildCoords[1] + (i * font_height),
                (char *) Content.substr(LineStartPoints[i] - Content.c_str(),
                                        LineLengths[i]).c_str(), false);
    }
}
...
Text::Text(std::string in_name, std::string in_content, int x_coord, int y_coord, int x_width, int y_height, int in_style):GUIObject(in_name, x_coord, y_coord, x_width, y_height,
          in_style)
{
  Content = in_content;
...
void Text::SetSaveLoc(int *int_ptr, int save_method, int max_value,
                      int min_value)
{
...
void Text::SetSaveLoc(short int *sint_ptr, int save_method,
                      short int max_value, short int min_value)
{
...
void Text::SetSaveLoc(float *ptr, int save_method, float max_value,
                      float min_value)
{
...
void Text::SetSaveLoc(char *ptr, int save_method, uint max_length,
                      uint min_length)
{
  Assert(ptr != NULL);          //Naughty.
...
void Text::SetSaveStringAlloc(char **ptr, int save_method, int mem_flags,
                              uint max_length, uint min_length)
{
  Assert(ptr != NULL);          //Naughty.
...
void Text::SetSaveLoc(ubyte * ptr, int save_method, int max_value,
                      int min_value)
{
  Assert(ptr != NULL);          //Naughty.
  Assert(min_value < max_value);        //Mmm-hmm.
...
Checkbox::Checkbox(std::string in_label, int x_coord, int y_coord,
                   void (*in_function) (Checkbox * caller), int x_width,
                   int y_height, int in_style):GUIObject(in_label, x_coord,
                                                         y_coord, x_width,
                                                         y_height, in_style)
{
  Label = in_label;
...
void Checkbox::DoDraw(float frametime)
{
  ...
  draw_open_rect(CheckCoords[0], CheckCoords[1], CheckCoords[2],
                 CheckCoords[3], false);
  ...
  gr_set_color_fast(&Color_text_normal);
  gr_string(CheckCoords[2] + CB_TEXTCHECKDIST, CheckCoords[1],
            (char *) Label.c_str(), false);
}
...
//*****************************ImageAnim*******************************
ImageAnim::ImageAnim(std::string in_name, std::string in_imagename, int x_coord, int y_coord, int x_width, int y_width, int in_style):GUIObject(in_name, x_coord, y_coord, x_width, y_width,
          in_style)
{
  //Load the image
  IsSet = false;
...
void ImageAnim::DoDraw(float frametime)
{
  ...
  gr_set_bitmap(ImageHandle + CurrentFrame, GR_ALPHABLEND_FILTER,
...
void ImageAnim::SetImage(std::string in_imagename)
{
  if (in_imagename.size())
    {
      ImageHandle =
        IMG_LOAD_ANIM((char *) in_imagename.c_str(), &TotalFrames, &FPS);
