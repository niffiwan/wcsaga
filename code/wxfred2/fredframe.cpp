...
EVT_MENU(XRCID("mnuFileExit"), FREDFrame::OnFileExit)  
  // editor menu
  EVT_MENU(XRCID("mnuEditorsAsteroidField"),
         FREDFrame::
         OnEditorsAsteroidField)  EVT_MENU(XRCID("mnuEditorsMissionSpecs"),
                                            FREDFrame::
                                            OnEditorsMissionSpecs) 
EVT_MENU(XRCID("mnuEditorsDebriefing"),
         FREDFrame::
         OnEditorsDebriefing)  EVT_MENU(XRCID("mnuEditorsShieldSystem"),
                                         FREDFrame::
                                         OnEditorsShieldSystem) 
EVT_MENU(XRCID("mnuEditorsCommandBriefing"),
         FREDFrame::
         OnEditorsCommandBriefing) 
EVT_MENU(XRCID("mnuEditorsSetGlobalShipFlags"),
         FREDFrame::
         OnEditorsSetGlobalShipFlags) 
EVT_MENU(XRCID("mnuEditorsVoiceActingManager"),
         FREDFrame::
         OnEditorsVoiceActingManager)  EVT_MENU(XRCID("mnuEditorsCampaign"),
                                                 FREDFrame::
                                                 OnEditorsCampaign)  
  // help menu
EVT_MENU(XRCID("mnuHelpAboutFRED2"), FREDFrame::OnHelpAboutFRED2) 
END_EVENT_TABLE() 
 FREDFrame::FREDFrame(const wxChar * title, int xpos, int ypos, int width, int height) :wxFrame(NULL, -1, title, wxPoint(xpos, ypos),
                                 wxSize(width, height)) 
{
...
}  void FREDFrame::OnFileOpen(wxCommandEvent & WXUNUSED(event)) 
{
    ...
    wxFileDialog * dlg =
    new wxFileDialog(this, "Open", "", "",
                     _T
                     ("FreesSpace2 Missions (*.fs2)|*.fs2|All files (*.*)|*.*"),
...
}  void FREDFrame::OnFileSaveAs(wxCommandEvent & WXUNUSED(event)) 
{
    ...
    wxFileDialog * dlg =
    new wxFileDialog(this, "Save As", "", currentFilename,
...
OnEditorsAsteroidField(wxCommandEvent & WXUNUSED(event)) 
{
  dlgAsteroidFieldEditor * dlg = new dlgAsteroidFieldEditor(this);
  dlg->ShowModal();
  dlg->Destroy();
}  void FREDFrame::
OnEditorsMissionSpecs(wxCommandEvent & WXUNUSED(event)) 
{
...
OnEditorsShieldSystem(wxCommandEvent & WXUNUSED(event)) 
{
  dlgShieldSystemEditor * dlg = new dlgShieldSystemEditor(this);
  dlg->ShowModal();
  dlg->Destroy();
}  void FREDFrame::
OnEditorsCommandBriefing(wxCommandEvent & WXUNUSED(event)) 
{
  dlgCommandBriefingEditor * dlg = new dlgCommandBriefingEditor(this);
  dlg->ShowModal();
  dlg->Destroy();
}  void FREDFrame::
OnEditorsSetGlobalShipFlags(wxCommandEvent & WXUNUSED(event)) 
{
  dlgSetGlobalShipFlagsEditor * dlg = new dlgSetGlobalShipFlagsEditor(this);
  dlg->ShowModal();
  dlg->Destroy();
}  void FREDFrame::
OnEditorsVoiceActingManager(wxCommandEvent & WXUNUSED(event)) 
{
