...
EVT_MENU(XRCID("mnuFileExit"), FREDFrame::OnFileExit)
  ...
  EVT_MENU(XRCID("mnuEditorsAsteroidField"),
         FREDFrame::
         OnEditorsAsteroidField) EVT_MENU(XRCID("mnuEditorsMissionSpecs"),
                                          FREDFrame::
                                          OnEditorsMissionSpecs)
EVT_MENU(XRCID("mnuEditorsDebriefing"),
         FREDFrame::
         OnEditorsDebriefing) EVT_MENU(XRCID("mnuEditorsShieldSystem"),
                                       FREDFrame::
...
EVT_MENU(XRCID("mnuEditorsVoiceActingManager"),
         FREDFrame::
         OnEditorsVoiceActingManager) EVT_MENU(XRCID("mnuEditorsCampaign"),
                                               FREDFrame::OnEditorsCampaign)
  // help menu
EVT_MENU(XRCID("mnuHelpAboutFRED2"), FREDFrame::OnHelpAboutFRED2)
END_EVENT_TABLE()FREDFrame::FREDFrame(const wxChar * title, int xpos,
                                        int ypos, int width,
                                        int height):wxFrame(NULL, -1, title,
                                                            wxPoint(xpos,
                                                                    ypos),
                                                            wxSize(width,
                                                                   height))
