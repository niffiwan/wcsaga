...
 bool quick_find_array[QUICKFIND_SUBDIVISION] =
{
false};
...
void check_ship_decals() 
{
    ...
    
    {
        ...
        
        {
            ...
            Assert(!list->d_list);      //if there are no decals there should be no list
          if (!list->d_list)
            Assert(!list->n_decals);    //if there are no list there should be no decals
          if (list->n_decals)
            Assert(list->d_list);       //if there are decals there should be a list
          if (list->d_list)
            Assert(list->n_decals);     //if there are list there should be some decals
            ...
            
            {
                ...
                Assert(dec->poly == NULL);      //if there are no polys there should be no polys
              int n = 0;
...
#ifndef NDEBUG
    if (dec == NULL)
    return (poly->next == NULL) ? true : false;
...
 bool decal_is_in_proper_list(decal * dec) 
{
    ...
    {
      if (dec->n_poly || dec->poly)
        Error(LOCATION, "decal list is null");
...
void check_poly_decals() 
{
  ...
  int i;
    ...
    
    {
        ...
        for (i = q * QUICKFIND_SEGMENT_LENGTH;
             i < (q + 1) * QUICKFIND_SEGMENT_LENGTH; i++)
        ...
        Assert(i != (q + 1) * QUICKFIND_SEGMENT_LENGTH);
    ...
    
    {
              ...
              (big_ol_decal_poly_array[i].parent,
               &big_ol_decal_poly_array[i]));
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                if (big_ol_decal_poly_array[j].parent == NULL)
                break;
            ...
            
            {
              ...
              mprintf(("highest poly used: %d; highest decal used: %d",
                        highest_poly_index, highest_decal_index));
...
void decal_list_controle::trim() 
{
    ...
    //hack fix
    if (!d_list)
    Error(LOCATION, "decal list is null in trim");      //return;
  decal_poly *poly = d_list->dec.poly;
  if (poly)
    free_all_polys(&d_list->dec);
    ...
    else if (d_list->dec.n_poly)
    Error(LOCATION, "poly list is missing in trim");
  ...
  SAFEPOINT("decal_list_controle::trim; polys cleared");
    ...
    if (!d_list)
    Error(LOCATION, "decal list is null in trim 1");    //return;
  d_list->dec.poly = NULL;
  
    //poly should have made it to the end of the list, make sure that it did
    if (!d_list)
    Error(LOCATION, "decal list is null in trim 2");    //return;
  //  Assert(poly == NULL);
  d_list->dec.n_poly = 0;
  if (!d_list)
    Error(LOCATION, "decal list is null in trim 3");    //return;
  //the end is the first
  end->next = d_list;
  if (!d_list)
    Error(LOCATION, "decal list is null in trim 4");    //return;
  //the end is pushed back
  end = end->next;
  if (!d_list)
    Error(LOCATION, "decal list is null in trim 5");    //return;
  //the first is the second
  d_list = d_list->next;
  
    //the end is NULLed off
    if (!d_list)
    Error(LOCATION, "decal list is null in trim 6");    //return;
  end->next = NULL;
  if (!d_list)
    Error(LOCATION, "decal list is null in trim 7");    //return;
  check_poly_decals();
  modifyed = true;             //mark this decal list as haveing been modifyed
  if (!d_list)
    Error(LOCATION, "decal list is null at the end of trim");   //return;
...
 decal_list_controle::decal_list_controle():n_decals(0), buffer(-1), texture(-1), modifyed(false), d_list(NULL),
end(NULL) 
{
...
//find a decal list with the texture we want from an array of decal lists
  decal_list_controle * find_list(decal_list_controle * list, int n_textures,
                                  int texture) 
{
...
 decal * decal_find_next(decal_system * system, int texture,
                           int glow_texture = -1, int burn_texture = -1) 
{
    ...
    decal_list_controle * list =
    find_list(system->decals, system->n_decal_textures, texture);
    ...
    
    {
        ...
        system->decals =
        (decal_list_controle *) vm_realloc(system->decals,
                                           sizeof(decal_list_controle) *
                                           ++system->n_decal_textures);
       if (!system->decals)
        Error(LOCATION, "unable to malloc new decal list");
    ...
    Error(LOCATION, "decal list lost");
  ...
  if (!system->max_decals)
    system->max_decals = 10;    //this should never happen, but it is and I'm to lazy to fix it properly
...
 void clear_decals(decal_system * system) 
{
  ...
  if (!system->n_decal_textures)
    return;
  for (i = 0; i < system->n_decal_textures; i++)
    system->decals[i].clear();
  for (i = 0; i < system->n_decal_textures; i++)
    Assert(!system->decals[i].n_decals);
...
int decal_create_simple(object * obj, decal_point * point, int texture) //makes a simple non-clipped decal
{
            ...
            point->orient.vec.fvec.xyz.x, point->orient.vec.fvec.xyz.y,
            point->orient.vec.fvec.xyz.z));
    ...
    decal_find_next(&shipp->ship_decal_system, texture, -1, -1);
...
void setup_decal_cube(matrix m, vec3d cube_point[8]) 
{
    ...
    decal_cube_point[i] = cube_point[i];
...
int decal_create(object * obj, decal_point * point, int subobject,
                  int texture, int backfaced_texture, int glow_texture,
                  int burn_texture, int burn_time) 
{
    ...
    
    {
      ...
      vm_vec_scale_add(&cube_point[i + 4], &cube_point[i],
                        &point->orient.vec.fvec, rad * 2);
  ...
  mprintf(("decal defined\n"));
  ...
   if (!hit_main_hull && !psub)
    return -1;
   decal_system * system =
    (hit_main_hull) ? &shipp->ship_decal_system : &psub->model_decal_system;
    ...
    //check_poly_decals();
    decal * sdec =
    decal_find_next(system, texture, glow_texture, burn_texture);
...
void decal_create_sortnorm(ubyte * p) 
{
  ...
   if (!fvi_ray_boundingbox
         (&min, &max, &decal_hit_point, &decal_hit_vect, &h))
  ...
   float decal_plane_dist =
    fvi_point_dist_plane(point, normal, &decal_hit_point);
  bool within = fl_abs(decal_plane_dist) < max_rad;
   if (prelist)
    decal_create_sub(p + prelist);
   if (within)
    if (onlist)
      decal_create_sub(p + onlist);
   if (decal_plane_dist > 0.0f)
    
    {
      if (frontlist)
        decal_create_sub(p + frontlist);
      if (backlist)
        decal_create_sub(p + backlist);
...
{
  if (fl_abs(test->xyz.x) > rad)
    return false;
  if (fl_abs(test->xyz.y) > rad)
    return false;
  if (fl_abs(test->xyz.z) > rad)
    return false;
  return true;
}

 void decal_clip_line(vertex * out, vertex * p1, vertex * p2, float test,
                        bool U) 
{
  ...
  float iterp = 0.0;
    ...
    iterp = (p1->u - test) / (p1->u - p2->u);
  
  else
    iterp = (p1->v - test) / (p1->v - p2->v);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                decal_clip_line(&out[n_verts++], &work[i],
                                &work[(i + 1) % old_n_verts], 1.0f, true);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                //if the end point is bad, clip between them
                decal_clip_line(&out[n_verts++], &work[i],
                                &work[(i + 1) % old_n_verts], 0.0f, true);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                //if the end point is bad, clip between them
                decal_clip_line(&out[n_verts++], &work[i],
                                &work[(i + 1) % old_n_verts], 1.0f, false);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                //if the end point is bad, clip between them
                decal_clip_line(&out[n_verts++], &work[i],
                                &work[(i + 1) % old_n_verts], 0.0f, false);
...
 float fvi_point_dist_plane(vec3d * pl_pnt, vec3d * pl_norm,
                              vertex * vert) 
{
...
 int decal_plane_clip_tri(vertex in[3], vertex out[20]) 
{
    ...
    
    {
        ...
        
        {
            ...
            //for each edge, in[i], in[(i+1)%3]
            if (fvi_point_dist_plane
                (&decal_cube_plane[p][0], &decal_cube_plane[p][1],
                 &work[i]) > 0.0)
            
            {
                ...
                out[n_verts++] = work[i];
                     ...
                     (fvi_point_dist_plane
                      (&decal_cube_plane[p][0], &decal_cube_plane[p][1],
                       &work[(i + 1) % old_n_verts]) > 0.0))
                
                {
                  ...
                  fvi_segment_plane(&temp, &decal_cube_plane[p][0],
                                     &decal_cube_plane[p][1], &temp3, &temp2,
                                     0.0f);
...
void decal_create_tmappoly(ubyte * p) 
{
     ...
     */ 
  float decal_plane_dist =
    fl_abs(fvi_point_dist_plane(pcenter, pnorm, &decal_hit_point));
    ...
    
    {
        ...
        return;
         ...
         && !fvi_point_face(&closest, nv, points, &norm, NULL, NULL, NULL))
    
    {
      ...
      float distance = FLT_MAX, test_dist = FLT_MAX;
        ...
        
        {
                   ...
                   (&closest, decal_point_list[decal__poly[i]],
                    decal_point_list[decal__poly[(i + 1) % nv]],
                    &test_nearest, &test_dist))
  ...
   bool defenately = !clip
    || (decal_tmap_bbox(&local_closest, decal_min_hit_radius));
   if (!defenately
         && max_rad * 1.5 < vm_vec_dist(&closest, &decal_hit_point))
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
               vm_vec2vert(decal_point_list[decal__poly[0]],
                             &LAST_POLY->point[0]);
              vm_vec2vert(decal_point_list[decal__poly[i % nv]],
                           &LAST_POLY->point[1]);
              vm_vec2vert(decal_point_list[decal__poly[(i + 1) % nv]],
                           &LAST_POLY->point[2]);
              ...
              int k;
                ...
                
                {
                    ...
                    (fvi_point_dist_plane
                     (&decal_cube_plane[4][0], &decal_cube_plane[4][1],
                      &temp) / (max_rad * 3));
                  a[k] =
                    (fvi_point_dist_plane
                     (&decal_cube_plane[5][0], &decal_cube_plane[5][1],
                      &temp) / (max_rad * 3));
                ...
                
                {
                    ...
                    (fvi_point_dist_plane
                     (&decal_cube_plane[0][0], &decal_cube_plane[0][1],
                      &temp) / (min_rad * 2));
                  LAST_POLY->point[k].v =
                    (fvi_point_dist_plane
                     (&decal_cube_plane[2][0], &decal_cube_plane[2][1],
                      &temp) / (min_rad * 2));
                ...
                /*if(uv_passed) */ 
              {
                  ...
                  
                  {
                      ...
                      decal_plane_clip_tri(LAST_POLY->point, work_poly);
                  ...
                  
                  {
                    ...
                    else if (uv_passed && IS_VEC_NULL(&local_closest))
                    ...
                    else if (uv_passed && check_ab(a, b))
        ...
        
        {
            ...
            
            {
                ...
                (fvi_point_dist_plane
                 (&decal_cube_plane[2][0], &decal_cube_plane[2][1],
                  &temp) / (min_rad * 2));
               vm_vec2vert(decal_point_list[decal__poly[i]],
                             &LAST_POLY->point[1]);
...
  int decal_create_sub(void *model_ptr) 
{
    ...
    
    {
        ...
        
        {
          ...
          decal_create_defpoints(p);
          break;
        case DP_FLATPOLY:
          break;                // I don't want any of these anyway-Bobboau decal_create_flatpoly(p); break;
        case DP_TMAPPOLY:
          decal_create_tmappoly(p);
          break;
        case DP_SORTNORM:
          decal_create_sortnorm(p);
          break;
...
int find_decal_tricounts(decal_system * system, int i) 
{
  ...
  int n = list->n_decals;
    ...
    
    {
      ...
      Assert((decal->dec.n_poly < MAX_DECAL_POLY)
              && (decal->dec.n_poly > -1));
...
void rebuild_decal_buffer(decal_system * system) 
{
    ...
    
    {
      ...
       Assert(list->end->dec.vertex_buffer_start +
                (list->end->dec.n_poly * 3) <= dec_list.n_verts);
       if (list->buffer > -1)
        gr_destroy_buffer(list->buffer);
      if (dec_list.n_verts)
        list->buffer =
          gr_make_buffer(&dec_list,
                         VERTEX_FLAG_POSITION | VERTEX_FLAG_NORMAL |
                         VERTEX_FLAG_UV1);
...
void decal_render_system(decal_system * system) 
{
    ...
    //return;
  int i;
   if (system->decals_modified)
    rebuild_decal_buffer(system);
  gr_zbias(4);
  for (i = 0; i < system->n_decal_textures; i++)
    
    {
      GLOWMAP = system->decals[i].glow_texture;
      gr_set_bitmap(system->decals[i].texture, GR_ALPHABLEND_NONE,
                     GR_BITBLT_MODE_NORMAL, 1.0);
        ...
        
        {
            ...
            
            {
                ...
                
                {
                    ...
                    Assert(list->dec.n_poly > 0
                           && list->dec.vertex_buffer_start > -1);
                  Assert(list->dec.vertex_buffer_start ==
                          3 * (list->dec.vertex_buffer_start / 3));
                  if (!list->dec.n_poly)
                    continue;
                  if (list->dec.vertex_buffer_start < 0)
                    continue;
                  float a =
                    1.0f -
                    ((float (timestamp()) -
                      float (list->dec.timestamp)) /float (list->dec.
                                                           burn_time));
                  gr_set_bitmap(system->decals[i].burn_texture,
...
void set_submodel_instance(int model_num, int sub_model_num) 
{
  ...
  vec3d offset[8];
    ...
    {
       orient[decal_submodel_instance_depth] =
        pm->submodel[sub_model_num].angs;
      offset[decal_submodel_instance_depth] =
        pm->submodel[sub_model_num].offset;
      decal_submodel_instance_depth++;
       sub_model_num = pm->submodel[sub_model_num].parent;
      if (decal_submodel_instance_depth == 7)
        break;
