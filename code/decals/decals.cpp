...
decal *decal_find_next(decal_system * system, int texture, int glow_texture =
                       -1, int burn_texture = -1)
{
  check_list_decals();
  check_poly_decals();
  check_ship_decals();
...
bool check_uv(decal_poly * poly)
{
  if (poly->point[0].u < 0.0f
      && poly->point[1].u < 0.0f && poly->point[2].u < 0.0f)
    return false;
  if (poly->point[0].v < 0.0f
      && poly->point[1].v < 0.0f && poly->point[2].v < 0.0f)
    return false;
  if (poly->point[0].u > 1.0f
      && poly->point[1].u > 1.0f && poly->point[2].u > 1.0f)
    return false;
  if (poly->point[0].v > 1.0f
      && poly->point[1].v > 1.0f && poly->point[2].v > 1.0f)
    return false;
      ...
      for (i = 0; i < nv; i++)
        {
          switch (vm_vec_dist_to_line
                  (&closest, decal_point_list[decal__poly[i]],
                   decal_point_list[decal__poly[(i + 1) % nv]], &test_nearest,
                   &test_dist))
            {
            case -1:
