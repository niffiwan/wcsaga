...
void error_display(int error_level, char *format, ...) 
{
    ...
    
    {
  ...
   nprintf((error_text, "%s(line %i:%s: ", Current_filename, get_line_num(),
              error_text));
             ...
             get_line_num(), error_text, buffer);
...
int required_string(char *pstr) 
{
    ...
    
    {
                     ...
                     "Missing required token: [%s]. Found [%.32s] %s instead.\n",
...
int required_string_either(char *str1, char *str2) 
{
    ...
    
    {
        ...
        
        {
            ...
            // Mp += strlen(str2);
            diag_printf("Found required string [%s]\n%s", token_found =
                        str2, parse_error_text);
          return 1;
        }
       error_display(1,
                       "Required token = [%s] or [%s], found [%.32s] %s.\n",
...
int required_string_3(char *str1, char *str2, char *str3) 
{
    ...
    
    {
        ...
        
        {
                       ...
                       "Required token = [%s], [%s] or [%s], found [%.32s].\n",
...
 int required_string_4(char *str1, char *str2, char *str3, char *str4) 
{
    ...
    
    {
        ...
        
        {
                       ...
                       "Required token = [%s], [%s], [%s], or [%s], found [%.32s].\n",
...
void copy_to_eoln(char *outstr, char *more_terminators, char *instr,
                  int max) 
{
...
char *alloc_block(char *startstr, char *endstr, int extra_chars) 
{
  ...
  int level = 1;
    ...
    
    {
             ...
             "Unclosed pair of \"%s\" and \"%s\" on line %d in file",
...
int get_string_or_variable(char *str) 
{
    ...
    
    {
        ...
        Assert(Sexp_variables[sexp_variable_index].
...
#endif*/ 
int parse_get_line(char *lineout, int max_line_len, char *start,
                    int max_size, char *cur) 
{
...
void read_file_text(char *filename, int mode, char *processed_text,
                    char *raw_text) 
{
...
// Goober5000
void read_file_text_from_array(char *array, char *processed_text,
                               char *raw_text) 
{
...
void process_raw_file_text(char *processed_text, char *raw_text) 
{
  ...
  int num_chars_read = 0;
           ...
           parse_get_line(outbuf, PARSE_BUF_SIZE, raw_text, raw_text_len,
                          mp_raw)) != 0)
...
void stuff_boolean(bool * b, bool a_to_eol) 
{
    ...
    
    {
        ...
        
        {
          ...
          Warning(LOCATION, "Boolean '%s' type unknown; assuming 'no/false'",
                   token);
...
 int parse_string_flag_list(int *dest, flag_def_list defs[],
                              int defs_size) 
{
  Assert(dest != NULL);        //wtf?
  char (*slp)[NAME_LENGTH] =
    (char (*)[32]) new char[defs_size * NAME_LENGTH];
                ...
                
                {
                    ...
                    
                    {
                        ...
                        
                        {
                                   ...
                                   "Unable to find WEAPON_LIST_TYPE string \"%s\" %s.\n",
...
int stuff_loadout_list(int *ilp, int max_ints, int lookup_type) 
{
    ...
    
    {
        ...
        if (index < 0
            && (lookup_type == MISSION_LOADOUT_SHIP_LIST
        ...
        
        {
            ...
            Warning(LOCATION,
                    "Invalid type \"%s\" found in loadout of mission file...skipping",
                    str);
        ...
        if ((lookup_type == MISSION_LOADOUT_SHIP_LIST)
            && (!(Ship_info[index].flags & SIF_PLAYER_SHIP)))
...
int string_lookup(char *str1, char *strlist[], int max, char *description,
                  int say_errors) 
{
...
// match.
void find_and_stuff(char *id, int *addr, int f_type, char *strlist[], int max,
                    char *description) 
{
    ...
    
    {
          ...
          string_lookup(Ship_info[idx].name, strlist, max, description, 0);
...
 void find_and_stuff_optional(char *id, int *addr, int f_type,
                                char *strlist[], int max, char *description) 
{
...
 void find_and_stuff_or_add(char *id, int *addr, int f_type,
                              char *strlist[], int *total, int max,
                              char *description) 
{
...
int split_str(char *src, int max_pixel_w, int *n_chars, char **p_str,
              int max_lines, char ignore_char) 
{
...
 int split_str(char *src, int max_pixel_w, SCP_vector < int >*n_chars,
                 SCP_vector < char *>*p_str, char ignore_char) 
{
...
// Goober5000
int replace_one(char *str, char *oldstr, char *newstr, uint max_len,
                int range) 
{
...
// Goober5000
int replace_all(char *str, char *oldstr, char *newstr, uint max_len,
                int range) 
{
...
// Goober5000
void format_integer_with_commas(char *buf, int integer,
...
int scan_fso_version_string(const char *text, int *major, int *minor,
                            int *build, int *revis) 
{
...
int parse_modular_table(char *name_check,
                        void (*parse_callback) (char *filename),
                        int path_type, int sort_type) 
{
    ...
    cf_get_file_list_preallocated(MAX_TBL_PARTS, tbl_file_arr, tbl_file_names,
                                  path_type, name_check, sort_type);
