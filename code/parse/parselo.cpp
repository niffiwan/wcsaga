...
int required_string_5(char *str1, char *str2, char *str3, char *str4,
                      char *str5)
{
  int count = 0;
  ...
  while (count < RS_MAX_TRIES)
    {
      ...
      error_display(1,
                    "Required token = [%s], [%s], [%s], [%s], or [%s], found [%.32s].\n",
...
char *alloc_block(char *startstr, char *endstr, int extra_chars)
{
  ...
  if (level > 0)
    {
      Error(LOCATION, "Unclosed pair of \"%s\" and \"%s\" on line %d in file",
...
int parse_get_line(char *lineout, int max_line_len, char *start, int max_size,
                   char *cur)
{
  char *t = lineout;
...
void find_and_stuff_optional(char *id, int *addr, int f_type, char *strlist[],
...
void find_and_stuff_or_add(char *id, int *addr, int f_type, char *strlist[],
