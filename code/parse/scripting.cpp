  ...
  if (Output_scripting_meta)
    {
      ...
      snprintf(cmdname, MAX_PATH_LEN - 1, "%s%s%s%s%s%s%s", detect_home(),
               DIR_SEPARATOR_STR, Osreg_company_name, DIR_SEPARATOR_STR,
               Osreg_user_dir, DIR_SEPARATOR_STR, "scripting.html");
      ...
      switch (scp->condition_type)
        ...
        case CHC_WEAPONCLASS:
          if (objp == NULL || objp->type != OBJ_WEAPON)
            return false;
          if (stricmp
              (Weapon_info[Weapons[objp->instance].weapon_info_index].name,
               scp->data.name))
            return false;
          break;
        case CHC_OBJECTTYPE:
        ...
        case CHC_VERSION:
          {
            ...
            char buf[32];
            sprintf(buf, "%i.%i.%i", FS_VERSION_MAJOR, FS_VERSION_MINOR,
                    FS_VERSION_BUILD);
        ...
        case CHC_APPLICATION:
          {
            if (Fred_running)
              {
                if (stricmp("FRED2_Open", scp->data.name)
                    && stricmp("FRED2_Open", scp->data.name)
                    && stricmp("FRED 2", scp->data.name)
                    && stricmp("FRED", scp->data.name))
                  return false;
              }
            else
              {
                if (stricmp("FS2_Open", scp->data.name)
                    && stricmp("FS2Open", scp->data.name)
                    && stricmp("Freespace 2", scp->data.name)
                    && stricmp("Freespace", scp->data.name))
                  return false;
              }
          }
        default:
...
bool ConditionedHook::Run(script_state * sys, int action, char format,
                          void *data)
{
  Assert(sys != NULL);
  ...
  if (LuaState != NULL)
    {
      ...
      if (this->OpenHookVarTable())
        ...
        {
          LuaError(LuaState,
                   "Could not get HookVariable library to set hook variable '%s' - get a coder",
                   name);
          if (data_ldx)
            lua_pop(LuaState, 1);
        }
  ...
  if (LuaState != NULL)
    {
      ...
      if (this->OpenHookVarTable())
        ...
        {
          LuaError(LuaState,
                   "Could not get HookVariable library to get hook variable '%' - get a coder",
                   name);
        }
    }
...
int script_state::RunBytecodeSub(int in_lang, int in_idx, char format,
                                 void *data)
{
  if (in_idx >= 0)
    {
      ...
      if (in_lang == SC_LUA)
        {
          ...
          if (lua_isfunction(GetLuaSession(), hook_ldx))
            {
              if (lua_pcall
                  (GetLuaSession(), 0, format != '\0' ? 1 : 0, err_ldx) != 0)
                {
          ...
          else
            {
              ...
              LuaError(GetLuaSession(),
                       "RunBytecodeSub tried to call a non-function value!");
            }
...
int script_state::RunCondition(int action, char format, void *data,
                               object * objp)
{
...
bool script_state::EvalString(char *string, char *format, void *rtn,
                              char *debug_str)
{
...
void script_state::ParseChunkSub(int *out_lang, int *out_index,
                                 char *debug_str)
{
...
bool script_state::ParseCondition(char *filename)
{
  ...
  if (!actions_added)
    {
      Warning(LOCATION,
              "No actions specified for conditional hook in file '%s'",
              filename);
