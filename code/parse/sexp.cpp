...
void add_block_variable(const char *text, const char *var_name, int type,
                        int index);
...
{
  boost::unordered_map < std::string, int >::const_iterator found_operator =
    operator_indices.find(token);
...
int check_sexp_syntax(int node, int return_type, int recursive, int *bad_node,
                      int mode)
{
...
bool has_special_explosion_block_index(ship * shipp, int *index)
{
  ...
  for (current_index = (MAX_SEXP_VARIABLES - BLOCK_EXP_SIZE);
       current_index >=
       (MAX_SEXP_VARIABLES - (BLOCK_EXP_SIZE * Num_special_expl_blocks));
       current_index = (current_index - BLOCK_EXP_SIZE))
    {
      if ((atoi(Block_variables[current_index + INNER_RAD].text) ==
           shipp->special_exp_inner)
          && (atoi(Block_variables[current_index + OUTER_RAD].text) ==
              shipp->special_exp_outer)
          && (atoi(Block_variables[current_index + DAMAGE].text) ==
              shipp->special_exp_damage)
          && (atoi(Block_variables[current_index + BLAST].text) ==
              shipp->special_exp_blast)
          && (atoi(Block_variables[current_index + PROPAGATE].text) ==
              (shipp->use_shockwave ? 1 : 0))
          && (atoi(Block_variables[current_index + SHOCK_SPEED].text) ==
              shipp->special_exp_shockwave_speed))
        {
          *index = current_index;
          return true;
        }
...
bool generate_special_explosion_block_variables()
{
  ...
  for (sop = GET_FIRST(&Ship_obj_list); sop != END_OF_LIST(&Ship_obj_list);
       sop = GET_NEXT(sop))
    {
      ...
      already_added =
        has_special_explosion_block_index(shipp, &current_index);
      ...
      if (!already_added)
        {
          sprintf(Block_variables[current_index + INNER_RAD].text, "%d",
                  shipp->special_exp_inner);
          sprintf(Block_variables[current_index + OUTER_RAD].text, "%d",
                  shipp->special_exp_outer);
          sprintf(Block_variables[current_index + DAMAGE].text, "%d",
                  shipp->special_exp_damage);
          sprintf(Block_variables[current_index + BLAST].text, "%d",
                  shipp->special_exp_blast);
          sprintf(Block_variables[current_index + PROPAGATE].text, "%d",
                  (shipp->use_shockwave ? 1 : 0));
          sprintf(Block_variables[current_index + SHOCK_SPEED].text, "%d",
                  shipp->special_exp_shockwave_speed);

          // add the names
          for (i = current_index; i < (current_index + BLOCK_EXP_SIZE); i++)
            {
              sprintf(Block_variables[i].variable_name, "%s",
                      shipp->ship_name);
            }
...
int sexp_has_docked(int n)
{
  ...
  if (!mission_log_get_time_indexed
      (LOG_SHIP_DOCKED, docker, dockee, count, NULL))
    {
      ...
      else if (mission_log_get_time(LOG_SHIP_DEPARTED, docker, NULL, NULL)
               || mission_log_get_time(LOG_SHIP_DEPARTED, dockee, NULL, NULL))
        return SEXP_KNOWN_FALSE;
      else
        return SEXP_FALSE;
    }
...
int sexp_distance_subsystem(int n)
{
  ...
  if (mission_log_get_time
      ...
      || mission_log_get_time(LOG_SHIP_DEPARTED, ship_with_subsys_name, NULL,
                              NULL))
    return SEXP_NAN_FOREVER;
  ...
  if (subjective)
    {
      ...
      vm_vec_rotate(&subjective_desired_vel, &objp->phys_info.desired_vel,
                    &objp->orient);
      ...
      vm_vec_unrotate(&objp->phys_info.desired_vel, &subjective_desired_vel,
                      &objp->orient);
    }
  else
    {
  ...
  if (turn_time)
    {
      ...
      ai_turn_towards_vector(location, objp, flFrametime,
                             float (turn_time) / (1000.0f), NULL, NULL, 0.0f,
                             0, NULL, (AITTV_VIA_SEXP | bank));
...
void sexp_set_oswpt_facing(object_ship_wing_point_team * oswpt,
                           vec3d * location, int turn_time = 0, int bank = 0)
{
...
void sexp_set_ship_lat_maneuver(int n)
{
  ...
  sexp_set_oswpt_maneuver(&oswpt, duration, 0.0f, 0.0f, 0.0f, 0, up, sideways,
                          forward, force_control);
}
      ...
      if (wingnum != -1)
        {
          if (Wings[wingnum].flags & WF_NEVER_EXISTED)
            ...
            if (mission_log_get_time
                (LOG_WING_DEPARTED, name, NULL, &time_gone))
            {
              count++;
            }
          else
            if (mission_log_get_time
                (LOG_WING_DESTROYED, name, NULL, &time_gone))
            {
              count++;
            }
        }
                                     ...
                                     int *num_known_false)
{
  ...
  Assert((num_true != NULL) && (num_false != NULL) && (num_known_true != NULL)
         && (num_known_false != NULL));
...
void sexp_ingame_ship_change_iff(ship * shipp, int new_team)
{
  ...
  if ((Game_mode & GM_MULTIPLAYER) && (Net_player != NULL)
      && (Net_player->flags & NETINFO_FLAG_AM_MASTER) && (shipp->objnum >= 0))
...
void sexp_parse_ship_change_iff_color(p_object * parse_obj, int observer_team,
                                      int observed_team,
                                      int alternate_iff_color)
{
  Assert(parse_obj);
...
void sexp_trigger_arrival_track(int n)
{
  ...
  if (n == -1)
    {
      Warning(LOCATION,
              "Detected missing team parameter in sexp_trigger_arrival_track");
      return;
    }
  ...
  if (team == -1)
    {
      Warning(LOCATION,
              "Detected invalid team parameter in sexp_trigger_arrival_track");
      return;
    }
  ...
  if (explosion_size && inner_radius && outer_radius)
    {
      if (fireball_type == FIREBALL_EXPLOSION_MEDIUM)
        fireball_create(&origin, fireball_type, FIREBALL_MEDIUM_EXPLOSION, -1,
...
*/
{
  ...
  fireball_create(&origin, fireball_type, FIREBALL_WARP_EFFECT, -1,
                  (float) radius, 0, NULL, (float) duration, -1, &m_orient, 0,
                  extra_flags, warp_open_sound_index, warp_close_sound_index);
}
...
void sexp_transfer_cargo(int n)
{
  ...
  if (!
    ...
    {
      if (stricmp
          (Cargo_names[Ships[shipnum2].cargo1 & CARGO_INDEX_MASK], "nothing"))
        {
          Warning(LOCATION,
                  ...
                  if (Player_ai->target_objnum == shipp->objnum)
                    {
                      ...
                      if (Players[Player_num].
                          flags & PLAYER_FLAGS_AUTO_TARGETING)
                        {
  ...
  while (node != -1)
    {
      ...
      // If it isn't command then assume that we're dealing with a ship
      else
        {
          if (enable_messages)
            {
              sexp_deal_with_ship_flag(node, 0, 0, 0, SF2_NO_BUILTIN_MESSAGES,
                                       0, P2_SF2_NO_BUILTIN_MESSAGES, 0);
            }
          else
            {
              sexp_deal_with_ship_flag(node, 0, 0, 0, SF2_NO_BUILTIN_MESSAGES,
                                       0, P2_SF2_NO_BUILTIN_MESSAGES, 1);
            }
        }
...
{
  sexp_deal_with_ship_flag(n, OF_BEAM_PROTECTED, 0, 0, 0, P_OF_BEAM_PROTECTED,
                           0, flag);
}
                      ...
                      int rearm_limit, bool update)
{
  ...
  maximum_allowed =
    fl2i((Ship_info[shipp->ship_info_index].
          primary_bank_ammo_capacity[requested_bank] /
          Weapon_info[shipp->weapons.primary_bank_weapons[requested_bank]].
          cargo_size) + 0.5);
                        ...
                        int requested_ammo, int rearm_limit, bool update)
{
  ...
  // Is the number requested larger than the maximum allowed for that particular bank?
  maximum_allowed =
    fl2i(shipp->weapons.secondary_bank_capacity[requested_bank] /
         Weapon_info[shipp->weapons.secondary_bank_weapons[requested_bank]].
         cargo_size);
  if (maximum_allowed < requested_ammo)
    {
      ...
      if (ship_num == -1)
        {
          ...
          for (pobj = GET_FIRST(&Ship_arrival_list);
               pobj != END_OF_LIST(&Ship_arrival_list); pobj = GET_NEXT(pobj))
            {
              ...
              for (j = 0; j < num_groups; j++)
                {
                  if (!stricmp(Ai_tp_list[j].name, CTEXT(node)))
                    {
                      turret->target_priority[turret->num_target_priorities] =
                        j;
                      turret->num_target_priorities++;
                    }
                }
          ...
          if (rotate->system_info->rotation_snd >= 0)
            {
              obj_snd_assign(Ships[ship_num].objnum,
                             ...
                             &rotate->system_info->pnt, 0, OS_SUBSYS_ROTATION,
                             rotate);
              rotate->subsys_snd_flags |= SSSF_ROTATE;
            }
        }
...
void sexp_set_camera_facing_object(int n)
{
  ...
  switch (oswpt.type)
    ...
    case OSWPT_TYPE_EXITED:
      {
        Warning(LOCATION, "Camera tried to face destroyed/departed object %s",
                object_name);
        return;
      }
...
void sexp_show_subtitle(int node)
{
  ...
  subtitle new_subtitle(x_pos, y_pos, text, imageanim, display_time,
                        fade_time, &new_color, center_x, center_y, width, 0,
                        post_shaded);
...
void sexp_show_subtitle_text(int node)
{
  ...
  // add the subtitle
  subtitle new_subtitle(x_pos, y_pos, text, NULL, display_time, fade_time,
                        &new_color, center_x, center_y, width, 0,
                        post_shaded);
  Subtitles.push_back(new_subtitle);
}
...
void sexp_show_subtitle_image(int node)
{
  ...
  // add the subtitle
  subtitle new_subtitle(x_pos, y_pos, NULL, image, display_time, fade_time,
                        NULL, center_x, center_y, width, height, post_shaded);
  Subtitles.push_back(new_subtitle);
}
  ...
  for (n = CDR(n); n != -1; n = CDR(n))
    {
      ...
      // check to see if this ship/wing has departed.  If so, then function is known false
      if (mission_log_get_time
          (LOG_SHIP_DEPARTED, destroyed_ship_name, NULL, NULL))
        return SEXP_KNOWN_FALSE;

      // check the mission log.  If ship/wing not destroyed, immediately return SEXP_FALSE.
      if (mission_log_get_time
          (LOG_SHIP_DESTROYED, destroyed_ship_name, destroyer_ship_name,
           &time))
        {
          num_destroyed++;
          if (latest_time && (time > *latest_time))
      ...
      if (ship_num != -1)
        {
          ...
          Objects[shipp->objnum].phys_info.forward_decel_time_const =
            new_deceleration_time_const;
        }
    }
}
        ...
        case OSWPT_TYPE_WING:
          {
            ...
            for (int i = 0; i < oswpt.wingp->current_count; i++)
              {
                ai_info *aip =
                  &Ai_info[Ships[oswpt.wingp->ship_index[i]].ai_index];
...
{
  insertion_sort((void *) Sexp_variables, (size_t) (MAX_SEXP_VARIABLES),
...
sexp_help_struct Sexp_help[] = {
  ...
  {OP_NAV_SELECT,
   "Takes 1 Argument: NavPoint Name, it then sets it as the currently selected one.\r\n"}
  ,
  ...
  // Karajorma
  {
   ...
   "\tPerforms specified actions once for each valid " SEXP_ARGUMENT_STRING
   " in the parent conditional.\r\n"
  ...
  {
   OP_MATCHING_SPEED,
   "Checks whether the player is currently matching speed with the selected target.\r\n"
   "Takes no arguments..."}
  ,
...

  {
   ...
   "\t\t" SEXP_HUD_GAUGE_WARPOUT
   " - the \"Subspace drive active\" box that appears above the viewscreen.\r\n"}
  ,
