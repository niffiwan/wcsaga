...
extern int skip_to_start_of_string_either(char *pstr1, char *pstr2,
                                          char *end = NULL);
...
extern int required_string_5(char *str1, char *str2, char *str3, char *str4,
                             char *str5);
...
extern void stuff_string(char *pstr, int type, int len, char *terminators =
                         NULL);
...
extern char *stuff_and_malloc_string(int type, char *terminators =
                                     NULL, int len = 0);
extern void stuff_malloc_string(char **dest, int type, char *terminators =
                                NULL, int len = 0);
...
extern int parse_string_flag_list(int *dest, flag_def_list defs[],
                                  int defs_size);
...
extern void find_and_stuff_optional(char *id, int *addr, int f_type,
                                    char *strlist[], int max,
                                    char *description);
...
extern void read_file_text(char *filename, int mode =
                           CF_TYPE_ANY, char *processed_text =
                           NULL, char *raw_text = NULL);
extern void read_file_text_from_array(char *array, char *processed_text =
                                      NULL, char *raw_text = NULL);
extern void read_raw_file_text(char *filename, int mode =
                               CF_TYPE_ANY, char *raw_text = NULL);
extern void process_raw_file_text(char *processed_text =
                                  NULL, char *raw_text = NULL);
...
extern int optional_string_fred(char *pstr, char *end = NULL, char *end2 =
                                NULL);

extern char parse_error_text[128];

// Goober5000 - returns position of replacement or -1 for exceeded length
extern int replace_one(char *str, char *oldstr, char *newstr,
                       unsigned int max_len, int range = 0);

// Goober5000 - returns number of replacements or -1 for exceeded length
extern int replace_all(char *str, char *oldstr, char *newstr,
                       unsigned int max_len, int range = 0);
...
extern void format_integer_with_commas(char *buf, int integer,
                                       bool use_comma_with_four_digits);

// Goober5000
extern int scan_fso_version_string(const char *text, int *major, int *minor,
                                   int *build, int *revis);
...
extern int parse_modular_table(char *name_check,
                               void (*parse_callback) (char *filename),
                               int path_type = CF_TYPE_TABLES, int sort_type =
                               CF_SORT_REVERSE);
