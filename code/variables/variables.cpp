...
operate(variable * op1, variable * op2):variable(VAR_OPP)
  {
    operand1 = op1;
    operand2 = op2;
  } virtual ~ operate() 
...
 class mod:public operate 
{
  ...
  {
    return i2fl((int) operand1->value(data) % (int) operand2->value(data));
  };
...
 class power:public operate 
{
  ...
  {
    return (float) pow(operand1->value(data), operand2->value(data));
  };
...
class root:public operate 
{
  ...
  {
    return (float) pow(operand1->value(data), 1.0f / operand2->value(data));
  };
...
class logb:public operate 
{
  ...
  {
    return (float) log(operand2->value(data)) / log(operand1->value(data));
  };
...
class comp_equal:public operate 
{
  ...
  {
    if (operand1->value(data) == operand2->value(data))
      return 1.0;
    else
      return 0.0f;
  };
...
unary(variable * op):variable(VAR_OPP)
  {
    operand = op;
  } virtual ~ unary() 
  {
...
 class hypsin_op:public unary 
{
  ...
  {
    return sinh(operand->value(data));
  }
...
 class hypcos_op:public unary 
{
  ...
  {
    return cosh(operand->value(data));
  }
...
 class hyptan_op:public unary 
{
  ...
  {
    return tanh(operand->value(data));
  }
...
 class arcsin_op:public unary 
{
  ...
  {
    return asin(operand->value(data));
  }
...
 class arccos_op:public unary 
{
  ...
  {
    return acos(operand->value(data));
  }
...
 class arctan_op:public unary 
{
  ...
  {
    return atan(operand->value(data));
  }
...
 class degree_op:public unary 
{
  ...
  {
    return (float) (operand->value(data) * 3.14159265358979323846264338327 /
                    180.0f);
} };
...
 class radian_op:public unary 
{
  ...
  {
    return (float) (operand->value(data) * 180.0f /
...
 class factorial_op:public unary 
{
  ...
  {
    return evaluate(operand->value(data));
  }
...
 class log_op:public unary 
{
  ...
  {
    return log10(operand->value(data));
  }
...
 class sqrt_op:public unary 
{
  ...
  {
    return sqrt(operand->value(data));
  }
...
{
"hypsin", "hyptan", "hyptan", "arcsin", "arccos", "arctan", "sin", "cos",
    "tan", "log", "ln", "sqrt", "deg", "rad", "!", "abs", "ipart", "fpart",
    "sign", "not"};
  ...
  variable * expression::parse_next(char *&in_str) 
{
    ...
    //kill any noise
    while ((*in_str == ' ' || *in_str == '	') && *in_str != char (0))
    in_str++;
    ...
    
    {
      ...
      int un = wich_unary(in_str);      //if it's a unary opperator lets get the value now
      if (strchr(op_char, *in_str) && *(in_str + 1) == ',')    //all items are followed by commas
        {
          ...
          if (*in_str == ',')
            in_str++;           //move the input string to the next item
            ...
            
            {
              ...
              c = new comp_equal(a, b);
              break;
            case '>':
              c = new comp_greater(a, b);
              break;
...
 variable * expression::get_object_variable(char **str) 
{
    ...
    
    {
      ...
      else if (!strncmp(*str, shield_id, strlen(shield_id)))
        ...
        (*str)++;               //move the input string to the next item
      return ret;
    ...
    
    {
      ...
      else if (!strncmp(*str, fps_id, strlen(fps_id)))
...
void fix_implied_mult(string & str) 
{
  ...
  for (uint i = 0; i < str.length(); i++)
    
    {
        ...
        
        {
            ...
            
            {
              ...
              for (int k = i + 2; (get_value(str[k]) == -1 || p) && str[k];
                    k++)
    ...
    str += ')';                 //add any missing parenthises
}
...
 void reverse(string & s) 
{
    ...
    
    {
        ...
        
        {
          ...
          if (!is_value(s[i]) || !is_value(s[i + 1]))
            break;
...
 void postfix(string & str) 
{
    ...
    
    {
        ...
        
        {
            ...
            while (is_value(s[i]) || prefix_unary(s.c_str() + i))
            out += s[i++];
