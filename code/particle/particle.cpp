...
void particle_init()
{
  ...
  if (Anim_bitmap_id_fire == -1)
    {
      Anim_bitmap_id_fire =
        bm_load_animation("particleexp01", &Anim_num_frames_fire, &fps, NULL,
                          0);
    }
  ...
  if (Anim_bitmap_id_smoke == -1)
    {
      Anim_bitmap_id_smoke =
        bm_load_animation("particlesmoke01", &Anim_num_frames_smoke, &fps,
                          NULL, 0);
    }
  ...
  if (Anim_bitmap_id_smoke2 == -1)
    {
      Anim_bitmap_id_smoke2 =
        bm_load_animation("particlesmoke02", &Anim_num_frames_smoke2, &fps,
                          NULL, 0);
    }
}
    ...
    case PARTICLE_BITMAP_3D:
      {
        ...
        bm_get_info(pinfo->optional_data, NULL, NULL, NULL,
                    &new_particle.nframes, &fps);
...
void particle_create(vec3d * pos, vec3d * vel, float lifetime, float rad,
                     int type, int optional_data, float tracer_length,
                     struct object *objp, bool reverse)
{
  particle_create(pos, &vmd_z_vector, vel, lifetime, rad, type, optional_data,
                  255, 255, 255, tracer_length, objp, reverse);
}

void particle_create(vec3d * pos, vec3d * norm, vec3d * vel, float lifetime,
                     float rad, int type, int optional_data, int r, int g,
                     int b, float tracer_length, struct object *objp,
                     bool reverse)
{
  particle_info pinfo;
     ...
     void particle_move_all(float frametime)
{
  ...
  for (SCP_vector < particle >::iterator p = Particles.begin();
       p != Particles.end();)
    {
      if (p->age == 0.0f)
        {
...
void particle_render_all()
{
  ...
  for (SCP_vector < particle >::iterator p = Particles.begin();
       p != Particles.end(); ++p)
    {
      ...
      if (p->attached_objnum >= 0)
        {
          vm_vec_unrotate(&p_pos, &p->pos,
                          &Objects[p->attached_objnum].orient);
          vm_vec_unrotate(&p_norm, &p->norm,
                          &Objects[p->attached_objnum].orient);
      ...
      if (p->type != PARTICLE_BITMAP_3D
          && vm_vec_dot_to_point(&Eye_matrix.vec.fvec, &Eye_position,
                                 &p_pos) <= 0.0f)
        {
          continue;
        }
      ...
      if (p->type == PARTICLE_DEBUG)
        ...
        {
          gr_set_bitmap(p->optional_data + cur_frame, GR_ALPHABLEND_FILTER,
                        GR_BITBLT_MODE_NORMAL, alpha);
          int flags = tmap_flags | TMAP_FLAG_RGB | TMAP_FLAG_GOURAUD;
          g3_draw_polygon(&p_pos, &p_norm, 2.0f * p->radius, 2.0f * p->radius,
                          flags, p->color[0], p->color[1], p->color[2],
                          (ubyte) (alpha * 255));
        }
      else
        {
          ...
          if (p->tracer_length > 0.0f)
            {
              batch_add_laser(framenum + cur_frame, &ts, p->radius, &te,
                              p->radius);
            }
          // draw as a regular bitmap
          else
            {
              batch_add_bitmap(framenum + cur_frame, tmap_flags, &pos,
                               p->particle_index % 8, p->radius, alpha);
            }
...
void particle_emit(particle_emitter * pe, int type, int optional_data,
                   float range)
{
  int i, n;
