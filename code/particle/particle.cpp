...
 void particle_render_all() 
{
    ...
    
    {
        ...
        if (vm_vec_dot_to_point(&Eye_matrix.vec.fvec, &Eye_position, &p->pos)
            <= 0.0f)
                   ...
                   float range) 
{
  ...
  float min_dist = 125.0f;
    ...
    
    {
        ...
        ((pe->max_life - pe->min_life) * frand()) + pe->min_life;
      ...
       particle_create(&pe->pos, &tmp_vel, life, radius, type,
                         optional_data);
