...
void particle_create(vec3d * pos, vec3d * vel, float lifetime, float rad,
                     int type, int optional_data = -1, float tracer_length =
                     -1.0f, struct object *objp = NULL, bool reverse = false);
void particle_create(vec3d * pos, vec3d * norm, vec3d * vel, float lifetime,
                     float rad, int type, int optional_data = -1, int r =
                     255, int g = 255, int b = 255, float tracer_length =
                     -1.0f, struct object *objp = NULL, bool reverse = false);
