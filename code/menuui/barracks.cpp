...
 struct barracks_buttons 
{
 ...
  barracks_buttons(char *name, int x1, int y1, int x2, int y2, int h, int r = 0):filename(name), x(x1), y(y1), text_x(x2), text_y(y2), hotspot(h),
    repeat
    (r)
  {
} };
  ...
   void barracks_init_stats(scoring_struct * stats) 
{
  ...
  STRCPY1(Stat_labels[Num_stat_lines],
           XSTR("Secondary friendly hit %:", 61));
...
 void barracks_set_callsign_enter_mode(bool set_callsign_enter_mode) 
{
    ...
    
    {
        ...
        {
          Buttons[gr_screen.res][idx].button.
...
 void barracks_delete_pilot() 
{
    ...
    popup(PF_TITLE_BIG | PF_TITLE_RED, 2, POPUP_NO, POPUP_YES,
...
void barracks_squad_change_popup() 
{
    ...
    popup(PF_USE_AFFIRMATIVE_ICON | PF_NO_NETWORKING, 1, POPUP_OK,
          XSTR("You cannot change your squadron in Single Player mode.",
               1445));
      ...
      
      {
        ...
        char temp[256], *str;
          ...
          
          {
                       ...
                       XSTR("This will overwrite your %s pilot.  Proceed?",
                            70), str);
...
void barracks_init() 
{
  ...
  int i;
    ...
    
    {
        ...
        // set the hotspot
        Buttons[gr_screen.res][i].button.
        link_hotspot(Buttons[gr_screen.res][i].hotspot);
...
void barracks_do_frame(float frametime) 
{
  ...
  int prospective_pilot = -1;
    ...
    
    {
      if (Pilot_squad_images[Pic_squad_number] == BARRACKS_IMAGE_NOT_LOADED)   // haven't tried loading it yet
        {
            ...
            
            {
              ...
               bm_get_info(Pilot_squad_images[Pic_squad_number], &w, &h,
                             NULL);
    ...
    {
      Buttons[gr_screen.res][B_PILOT_MULTI_MODE_BUTTON].button.
        draw_forced(2);
