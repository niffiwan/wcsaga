...
struct barracks_buttons
{
  ...
  barracks_buttons(char *name, int x1, int y1, int x2, int y2, int h, int r = 0):filename(name),
    x(x1),
    y(y1), text_x(x2), text_y(y2), hotspot(h), repeat(r)
  {
  }
};
...
namespace
{
  ...
  {
  public:
    KillClassEqual(const std::string & ship_class_):ship_class(ship_class_)
    {
    }
...
void barracks_init_stats(scoring_struct * stats)
{
  ...
  if (stats->p_shots_fired > 0)
    {
      f =
        (float) stats->p_bonehead_hits * 100.0f /
  ...
  if (stats->s_shots_fired > 0)
    {
      f =
        (float) stats->s_bonehead_hits * 100.0f /
  ...
  for (i = 0; i < Num_ship_classes; i++)
    {
      ...
      strcpy(buffer,
             (Ship_info[i].alt_name[0]) ? Ship_info[i].
             alt_name : Ship_info[i].name);
      end_string_at_first_hash_symbol(buffer);
      std::string label(buffer);

      std::vector < Kill_pair >::iterator kill_pair =
        std::find_if(kills.begin(), kills.end(), KillClassEqual(label));
  ...
  for (std::vector < Kill_pair >::const_iterator kill = kills.begin();
       kill != kills.end(); ++kill)
    {
      if (kill->second)
        {
          ...
          sprintf(Stat_labels[Num_stat_lines], NOX("%s:"),
                  kill->first.c_str());
  ...
  for (i = 0; i < Num_stat_lines; i++)
    {
      gr_force_fit_string(Stat_labels[i], Stat_column1_w[gr_screen.res],
                          Barracks_stats_coords[gr_screen.
                                                res][BARRACKS_W_COORD]);
      gr_force_fit_string(Stats[i], Stat_column2_w[gr_screen.res],
                          Barracks_stats2_coords[gr_screen.
                                                 res][BARRACKS_W_COORD]);
    }
}
...
{
  Buttons[gr_screen.res][B_PILOT_SCROLL_UP_BUTTON].button.
    set_hotkey(pilot_text_enter_mode ? KEY_UP : -1);
  Buttons[gr_screen.res][B_PILOT_SCROLL_DOWN_BUTTON].button.
    set_hotkey(pilot_text_enter_mode ? KEY_DOWN : -1);

  Buttons[gr_screen.res][B_PILOT_CREATE_BOTTON].button.
    set_hotkey(pilot_text_enter_mode ? KEY_C : -1);
  Buttons[gr_screen.res][B_PILOT_SET_ACTIVE_BUTTON].button.
    set_hotkey(pilot_text_enter_mode ? KEY_ENTER : -1);
  Buttons[gr_screen.res][B_PILOT_DELETE_BUTTON].button.
    set_hotkey(pilot_text_enter_mode ? KEY_DELETE : -1);

  Buttons[gr_screen.res][B_PIC_PREV_PILOT_BUTTON].button.
    set_hotkey(pilot_text_enter_mode ? KEY_LEFT : -1);
  Buttons[gr_screen.res][B_PIC_NEXT_PILOT_BUTTON].button.
    set_hotkey(pilot_text_enter_mode ? KEY_RIGHT : -1);

  Buttons[gr_screen.res][B_ACCEPT_BUTTON].button.
    set_hotkey(pilot_text_enter_mode ? KEY_CTRLED | KEY_ENTER : -1);
}
...
void barracks_create_new_pilot()
{
  ...
  Inputbox.
    update_dimensions(Barracks_list_coords[gr_screen.res][BARRACKS_X_COORD],
...
void barracks_prev_squad_pic()
{
  ...
  if ((Pic_squad_number >= 0) && (Pic_squad_number < Num_pilot_squad_images))
    {
      strcpy_s(Cur_pilot->squad_filename,
...
void barracks_delete_pilot()
{
  ...
  del_rval =
    delete_pilot_file(buf,
                      (Player_sel_mode == PLAYER_SELECT_MODE_SINGLE) ? 1 : 0);
...
int barracks_pilot_filter(char *filename)
{
  ...
  r =
    verify_pilot_file(filename, Player_sel_mode == PLAYER_SELECT_MODE_SINGLE,
                      &rank);
  ...
  if (mode == PLAYER_SELECT_MODE_SINGLE)
    {
      ...
      Num_pilots =
        cf_get_file_list_preallocated(MAX_PILOTS, Pilots_arr, Pilots,
                                      CF_TYPE_SINGLE_PLAYERS, NOX("*.pl2"),
                                      CF_SORT_TIME);
      old_pilot_num =
        cf_get_file_list_preallocated(MAX_PILOTS, old_pilots_arr, old_pilots,
                                      CF_TYPE_SINGLE_PLAYERS, NOX("*.plr"),
                                      CF_SORT_TIME);
      ...
      Buttons[gr_screen.res][B_SQUAD_PREV_BUTTON].button.
        set_disabled_action(barracks_squad_change_popup);
      Buttons[gr_screen.res][B_SQUAD_NEXT_BUTTON].button.hide();
      Buttons[gr_screen.res][B_SQUAD_NEXT_BUTTON].button.disable();
      Buttons[gr_screen.res][B_SQUAD_NEXT_BUTTON].button.
        set_disabled_action(barracks_squad_change_popup);
    }
      ...
      if (barracks_new_pilot_selected())
        {
          ...
          if (Player_sel_mode == PLAYER_SELECT_MODE_SINGLE)
            ...
            {
              popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                    XSTR("You must create a multi player pilot.", 67));
            }
        }
      else
        {
        ...
        if (!barracks_new_pilot_selected())
          {
            ...
            sprintf(temp,
                    XSTR("This will overwrite your %s pilot.  Proceed?", 70),
                    str);
            if (!verify_pilot_file
                (Cur_pilot->callsign,
                 Player_sel_mode == PLAYER_SELECT_MODE_MULTI))
              {
  ...
  while (y + font_height <=
         Barracks_list_coords[gr_screen.res][BARRACKS_H_COORD])
    {
      ...
      if (!stricmp(Cur_pilot->callsign, Pilots[cur_pilot_idx])
          && (is_pilot_multi(Cur_pilot) == multi))
        {
      ...
      if (*str)
        {
          gr_printf(Barracks_stats2_coords[gr_screen.res][BARRACKS_X_COORD],
                    Barracks_stats_coords[gr_screen.res][BARRACKS_Y_COORD] +
                    y, "%s", str);
        }
      ...
      if (Pilot_images[Pic_number] >= 0)
        {
          ...
          char buf[40];
          sprintf(buf, XSTR("%d of %d", 71), Pic_number + 1,
                  Num_pilot_images);
          gr_printf(Barracks_image_number_coords[gr_screen.res]
                    [BARRACKS_X_COORD],
                    Barracks_image_number_coords[gr_screen.
                                                 res][BARRACKS_Y_COORD], buf);
        }
    }
  else
    {
      ...
      if (Pilot_squad_images[Pic_squad_number] >= 0)
        {
          ...
          if (Player_sel_mode != PLAYER_SELECT_MODE_SINGLE)
            {
              sprintf(buf, XSTR("%d of %d", 71), Pic_squad_number + 1,
                      Num_pilot_squad_images);
              gr_printf(Barracks_squad_number_coords[gr_screen.res]
                        [BARRACKS_X_COORD],
                        Barracks_squad_number_coords[gr_screen.
                                                     res][BARRACKS_Y_COORD],
                        buf);
            }
        }
    }
...
void barracks_init()
{
  ...
  // create interface
  Ui_window.create(0, 0, gr_screen.max_w_unscaled, gr_screen.max_h_unscaled,
                   0);
  ...
  for (i = 0; i < BARRACKS_NUM_BUTTONS; i++)
    {
      // create the object
      Buttons[gr_screen.res][i].button.create(&Ui_window, "",
                                              Buttons[gr_screen.res][i].x,
                                              Buttons[gr_screen.res][i].y, 60,
                                              30,
                                              Buttons[gr_screen.res][i].
                                              repeat, 1);

      // set the sound to play when highlighted
      Buttons[gr_screen.res][i].button.
        set_highlight_action(common_play_highlight_sound);

      // set the ani for the button
      Buttons[gr_screen.res][i].button.set_bmaps(Buttons[gr_screen.res][i].
                                                 filename);

      // set the hotspot
      Buttons[gr_screen.res][i].button.link_hotspot(Buttons[gr_screen.res][i].
                                                    hotspot);
    }
  ...
  w->add_XSTR("Select", 1552, Buttons[gr_screen.res][12].text_x,
              ...
              &Buttons[gr_screen.res][12].button, UI_XSTR_COLOR_PINK);
  ...
  w->add_XSTR("Single", 1041, Buttons[gr_screen.res][14].text_x,
              ...
              &Buttons[gr_screen.res][14].button, UI_XSTR_COLOR_PINK);
  w->add_XSTR("Multi", 1042, Buttons[gr_screen.res][15].text_x,
              Buttons[gr_screen.res][15].text_y,
              &Buttons[gr_screen.res][15].button, UI_XSTR_COLOR_PINK);
...
void barracks_do_frame(float frametime)
{
  ...
  if (Player_sel_mode == PLAYER_SELECT_MODE_SINGLE)
    {
      Buttons[gr_screen.res][B_PILOT_SINGLE_MODE_BUTTON].button.
        draw_forced(2);
    }
  else
    {
