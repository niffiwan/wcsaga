...
struct techroom_buttons
{
  ...
  techroom_buttons(char *name, int x1, int y1, int xt1, int yt1, int h, int t, int f = 0):filename(name),
    x(x1),
    y(y1), xt(xt1), yt(yt1), hotspot(h), tab(t), flags(f)
  {
  }
};
...
void techroom_select_new_entry()
{
  ...
  if (Tab == SHIPS_DATA_TAB)
  ...
  else
    {
      ...
      generic_anim_init(&Current_list[Cur_entry].animation,
                        Current_list[Cur_entry].tech_anim_filename);
      Current_list[Cur_entry].animation.ani.bg_type =
        bm_get_type(Tech_background_bitmap);
  ...
  while (y + font_height <= Tech_list_coords[gr_screen.res][SHIP_H_COORD])
    {
      ...
      List_buttons[z -
                   List_offset].update_dimensions(Tech_list_coords[gr_screen.
                                                                   res]
                                                  [SHIP_X_COORD],
                                                  Tech_list_coords[gr_screen.
                                                                   res]
                                                  [SHIP_Y_COORD] + y,
                                                  Tech_list_coords[gr_screen.
                                                                   res]
...
void techroom_ships_render(float frametime)
{
  ...
  model_render(Techroom_ship_modelnum, &Techroom_ship_orient,
               &vmd_zero_vector, MR_LOCK_DETAIL | MR_AUTOCENTER);
...
void techroom_anim_render(float frametime)
{
  ...
  if (Current_list[Cur_entry].animation.num_frames > 0)
    {
      //grab dimensions
      bm_get_info((Current_list[Cur_entry].animation.
                   streaming) ? Current_list[Cur_entry].animation.
                  bitmap_id : Current_list[Cur_entry].animation.first_frame,
                  &x, &y, NULL, NULL, NULL);
      //get the centre point - adjust
      x = Tech_ani_centre_coords[gr_screen.res][0] - x / 2;
      y = Tech_ani_centre_coords[gr_screen.res][1] - y / 2;
      generic_anim_render(&Current_list[Cur_entry].animation, frametime, x,
                          y);
    }
  // if our active item has a bitmap instead of an animation, draw it
  else if ((Cur_entry >= 0) && (Current_list[Cur_entry].bitmap >= 0))
...
void techroom_change_tab(int num)
{
  ...
  switch (Tab)
    ...
    case SHIPS_DATA_TAB:
      ...
      mask2 =
        multi ? SIF2_DEFAULT_IN_TECH_DATABASE_M :
        SIF2_DEFAULT_IN_TECH_DATABASE;
      ...
      Tech_slider.set_numberItems(Current_list_size >
                                  max_num_entries_viewable ? Current_list_size
          ...
          for (i = 0; i < Num_weapon_types; i++)
            {
              if (Techroom_show_all || (Weapon_info[i].wi_flags & mask)
                  || (Weapon_info[i].wi_flags2 & mask2))
                {
                  ...
                  strncpy(Weapon_list[Weapon_list_size].tech_anim_filename,
                  ...
                  || (Intel_info[i].flags & IIF_DEFAULT_IN_TECH_DATABASE))
                {
                  ...
                  if (!strcmp(Intel_info[i].anim_filename, "none"))
                  ...
                  else
                    {
                      ...
                      strncpy(Intel_list[Intel_list_size].tech_anim_filename,
  ...
  while (optional_string("$Entry:"))
    {
      ...
      stuff_string(Intel_info[Intel_info_size].desc, F_MULTITEXT,
                   TECH_INTEL_DESC_LEN);
...
void techroom_init()
{
  ...
  Tech_slider.create(&Ui_window,
                     ...
                     Num_ship_classes, Tech_slider_filename[gr_screen.res],
