      ...
      if (Multi_options_g.protocol == NET_TCP)
        {
          popup(PF_NO_NETWORKING, 1, POPUP_OK,
                XSTR
                ("TCP/IP protocol not found.  This protocol is required for multiplayer FreeSpace.",
                 -1));
        }
      else
        {
          ...
          popup(PF_NO_NETWORKING, 1, POPUP_OK,
                XSTR
                ("IPX protocol not found.  This protocol is required for multiplayer FreeSpace.",
                 -1));
        }
      break;
...
void main_hall_init(int main_hall_num)
{
  ...
  Main_hall->region_descript[1] =
    XSTR("Barracks - Manage yourWing Commander Saga pilots", 354);
  Main_hall->region_descript[2] =
    XSTR("Ready room - Start or continue a campaign", 355);
  Main_hall->region_descript[3] =
    XSTR
    ("Tech room - View specifications of Wing Commander Saga ships and weaponry",
     356);
  Main_hall->region_descript[4] =
    XSTR("Options - Change your Wing Commander Saga options", 357);
  Main_hall->region_descript[5] =
    XSTR("Campaign Room - View all available campaigns", 358);
  Main_hall->region_descript[6] =
  ...
  for (idx = 0; idx < Main_hall->num_misc_animations; idx++)
    {
      generic_anim_init(&Main_hall_misc_anim[idx],
      ...
      if (generic_anim_stream(&Main_hall_misc_anim[idx]) == -1)
      ...
      else
        {
          ...
          if (Main_hall->misc_anim_modes[idx] == MISC_ANIM_MODE_HOLD)
            Main_hall_misc_anim[idx].direction |=
  ...
  for (idx = 0; idx < Main_hall->num_door_animations; idx++)
    {
      generic_anim_init(&Main_hall_door_anim[idx],
      ...
      if (generic_anim_stream(&Main_hall_door_anim[idx]) == -1)
        ...
        Main_hall_door_anim[idx].direction =
...
void main_hall_exit_game()
{
  ...
  choice =
    popup(PF_NO_NETWORKING | PF_BODY_BIG, 2, POPUP_NO, POPUP_YES,
          XSTR("Exit Game?", 365));
  if (choice == 1)
    {
...
void main_hall_do(float frametime)
{
  ...
  snazzy_action = -1;
  code =
    snazzy_menu_do(Main_hall_mask_data, Main_hall_mask_w, Main_hall_mask_h,
                   Main_hall_num_options, Main_hall_region, &snazzy_action, 1,
                   &key);
    ...
    case SNAZZY_CLICKED:
      switch (code)
        ...
        case READY_ROOM_REGION:
          ...
          else if (!(Player->flags & PLAYER_FLAGS_IS_MULTI)
                   && !strlen(Campaign.filename))
            {
              // no campaign loaded...
              popup(PF_NO_NETWORKING, 1, POPUP_OK,
                    XSTR
                    ("No active campaign is available.  Please choose one in the Campaign Room.",
                     -1));
              break;
            }
        ...
        case CAMPAIGN_ROOM_REGION:
          ...
          gamesnd_play_iface(SND_IFACE_MOUSE_CLICK);
          {
            //game_feature_not_in_demo_popup();
            int reset_campaign =
              popup(PF_USE_AFFIRMATIVE_ICON | PF_BODY_BIG, 2, "Exit",
                    "Restart Campaign",
          ...
          if (Player->flags & PLAYER_FLAGS_IS_MULTI)
            {
              gamesnd_play_iface(SND_IFACE_MOUSE_CLICK);
              main_hall_set_notify_string(XSTR
              ...
              if (Num_recent_missions > 0)
              ...
              else
                {
                  if (mission_load_up_campaign())
                    main_hall_set_notify_string(XSTR
                                                ("Campaign file is currently unavailable",
                                                 -1));

                  strncpy(Game_current_mission_filename,
                          Campaign.missions[0].name, MAX_FILENAME_LEN);
                }
  ...
  if (!(Player->flags & PLAYER_FLAGS_IS_MULTI) && Campaign_file_missing
    ...
    {
      int rc =
        popup(0, 3, XSTR("Go to Campaign Room", -1),
              XSTR("Select another pilot", -1), XSTR("Exit Game", -1),
              XSTR
...
void main_hall_start_music()
{
  ...
  // get music
  index =
    main_hall_get_music_index(Main_hall - Main_hall_defines[gr_screen.res]);
  if (index < 0)
    {
      nprintf(("Warning",
  ...
  for (idx = 0; idx < MAX_MISC_ANIMATIONS; idx++)
    {
      ...
      if (Main_hall_misc_anim[idx].num_frames > 0)
        {
          //animation is paused
          if (Main_hall_misc_anim[idx].
              direction & GENERIC_ANIM_DIRECTION_PAUSED)
            {
              ...
              if (Main_hall->misc_anim_delay[idx][0] == -1)
                {
                  Main_hall->misc_anim_delay[idx][0] =
                    timestamp(Main_hall->misc_anim_delay[idx][1] +
                              (int) (((float) rand() / (float) RAND_MAX) *
                                     (float) (Main_hall->
                                              misc_anim_delay[idx][2] -
                                              Main_hall->
                                              misc_anim_delay[idx][1])));
            ...
            {
              for (s_idx = Main_hall->misc_anim_special_sounds[idx][0];
                   s_idx > 0; s_idx--)
                {
                  // if we've passed the trigger point, then play the sound and break out of the loop
                  if ((Main_hall_misc_anim[idx].current_frame >=
                      ...
                      && !Main_hall->misc_anim_sound_flag[idx][s_idx])
                    {
                      ...
                      if (snd_is_playing
                        ...
                        {
                          snd_stop(Main_hall->
                                   misc_anim_sound_handles[idx][s_idx]);
                          Main_hall->misc_anim_sound_handles[idx][s_idx] = -1;
                        }
                      // play the sound
                      Main_hall->misc_anim_sound_handles[idx][s_idx] =
              ...
              if (Main_hall_misc_anim[idx].current_frame ==
                  Main_hall_misc_anim[idx].num_frames - 1)
                {
                  Main_hall->misc_anim_delay[idx][0] = -1;
          ...
          if (Main_hall_frame_skip || Main_hall_paused)
            frametime = 0;
          generic_anim_render(&Main_hall_misc_anim[idx], frametime,
      ...
      if (Main_hall_door_anim[idx].num_frames > 0)
        {
          // first pair : coords of where to play a given door anim
          // second pair : center of a given door anim in windowed mode
          generic_anim_render(&Main_hall_door_anim[idx], frametime,
                              Main_hall->door_anim_coords[idx][0],
...
void main_hall_mouse_release_region(int region)
{
  ...
  Main_hall_door_anim[region].direction =
    GENERIC_ANIM_DIRECTION_BACKWARDS | GENERIC_ANIM_DIRECTION_NOLOOP;
  ...
  if ((Main_hall_door_anim[region].num_frames > 0))
    {
      ...
      Main_hall_door_sound_handles[region] =
        snd_play(&Snds_iface[Main_hall->door_sounds[region][1]],
                 Main_hall->door_sound_pan[region]);

      //TODO: track current frame
      snd_set_pos(Main_hall_door_sound_handles[region],
                  &Snds_iface[SND_MAIN_HALL_DOOR_CLOSE],
                  (float) ((Main_hall_door_anim[region].
                            keyframe) ? Main_hall_door_anim[region].
                           keyframe : Main_hall_door_anim[region].num_frames -
                           Main_hall_door_anim[region].current_frame) /
...
void main_hall_mouse_grab_region(int region)
{
  ...
  Main_hall_door_sound_handles[region] =
    snd_play(&Snds_iface[Main_hall->door_sounds[region][0]],
             Main_hall->door_sound_pan[region]);

  // start the sound playing at the right spot relative to the completion of the animation
  if ((Main_hall_door_anim[region].num_frames > 0)
      && (Main_hall_door_anim[region].current_frame != -1))
    {
      snd_set_pos(Main_hall_door_sound_handles[region],
                  &Snds_iface[SND_MAIN_HALL_DOOR_OPEN],
                  (float) Main_hall_door_anim[region].current_frame /
                  (float) Main_hall_door_anim[region].num_frames, 1);
    }
}
...
void main_hall_handle_random_intercom_sounds()
{
  ...
  if ((Main_hall_next_intercom_sound_stamp == -1)
      && (Main_hall_intercom_sound_handle == -1))
    {
      Main_hall_next_intercom_sound_stamp =
        timestamp((int)
                  (((float) rand() / (float) RAND_MAX) *
                   (float) (Main_hall->intercom_delay
                            [Main_hall_next_intercom_sound][1] -
                            Main_hall->intercom_delay
                            [Main_hall_intercom_sound_handle][0])));
    }
  ...
  if (play_ambient_loop)
    {
      Main_hall_ambient_loop =
        snd_play_looping(&Snds_iface[SND_MAIN_HALL_AMBIENT]);
    }
}
...
void main_hall_blit_version()
{
  ...
  gr_shade(0, version_y - padding, w + 2 * padding,
           Current_font->h + 2 * padding);
  ...
  if (!help_overlay_active(Main_hall_overlay_id))
    {
      ...
      gr_set_shader(&Main_hall_tooltip_shader);
      gr_shade(0, shader_y, gr_screen.clip_width_unscaled,
               (gr_screen.clip_height_unscaled - shader_y));

      gr_set_color_fast(&Color_bright_white);
      gr_string((gr_screen.max_w_unscaled - w) / 2, Main_hall->region_yval,
...
void main_hall_process_help_stuff()
{
  ...
  gr_set_color_fast(&Color_bright_white);
  gr_shade(0, 0, gr_screen.max_w_unscaled,
           (2 * Main_hall_tooltip_padding[gr_screen.res]) + h -
           y_anim_offset);
  gr_string((gr_screen.max_w_unscaled - w) / 2,
            Main_hall_tooltip_padding[gr_screen.res] /*- y_anim_offset*/ ,
            str);
}
  ...
  if (Vasudan_funny)
    {
      ...
      Main_hall_defines[GR_640][hall].door_sounds[OPTIONS_REGION][0] =
        SND_MAIN_HALL_PA_18;
      Main_hall_defines[GR_640][hall].door_sounds[OPTIONS_REGION][1] =
        SND_MAIN_HALL_PA_18;
      Main_hall_defines[GR_1024][hall].door_sounds[OPTIONS_REGION][0] =
        SND_MAIN_HALL_PA_18;
      Main_hall_defines[GR_1024][hall].door_sounds[OPTIONS_REGION][1] =
        SND_MAIN_HALL_PA_18;

      // set head anim. hehe
      strcpy_s(Main_hall_defines[GR_640][hall].door_anim_name[OPTIONS_REGION],
               "vhallheads");
      strcpy_s(Main_hall_defines[GR_1024][hall].
               door_anim_name[OPTIONS_REGION], "2_vhallheads");
...
int main_hall_is_vasudan()
{
  ...
  return (!stricmp(Main_hall->music_name, "Psampik")
          || !stricmp(Main_hall->music_name, "Psamtik"));
}
