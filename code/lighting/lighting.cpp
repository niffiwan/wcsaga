...
void light_add_directional(vec3d * dir, float intensity, float r, float g,
                           float b, float spec_r, float spec_g, float spec_b,
                           bool specular)
{
  if (!specular)
    {
...
void light_add_point(vec3d * pos, float r1, float r2, float intensity,
                     float r, float g, float b, int light_ignore_objnum,
                     float spec_r, float spec_g, float spec_b, bool specular)
{
  Assert(r1 > 0.0f);
  Assert(r2 > 0.0f);
...
void light_add_point_unique(vec3d * pos, float r1, float r2, float intensity,
...
void light_add_tube(vec3d * p0, vec3d * p1, float r1, float r2,
                    float intensity, float r, float g, float b,
                    int affected_objnum, float spec_r, float spec_g,
  ...
  for (i = 0; i < Num_relevent_lights[n1]; i++)
    {
      ...
      switch (l->type)
        ...
        case LT_TUBE:
          if (is_inside(min, max, &l->local_vec, l->radb)
              || is_inside(min, max, &l->local_vec2, l->radb))
            {
              Relevent_lights[Num_relevent_lights[n2]++][n2] = l;
            }
          break;
      ...
      for (idx = 0; idx < (int) Static_light.size(); idx++)
        {
          ...
          float sunFactor =
            Static_light.size() > 0 ? (2.0f / Static_light.size()) : 2.0f;
          ltmp = -vm_vec_dot(&Static_light[idx]->local_vec, norm) * Static_light[idx]->intensity * Reflective_light * sunFactor;        // reflective light
...
void light_apply_specular(ubyte * param_r, ubyte * param_g, ubyte * param_b,
                          vec3d * pos, vec3d * norm, vec3d * cam)
{
...
void light_apply_rgb(ubyte * param_r, ubyte * param_g, ubyte * param_b,
                     vec3d * pos, vec3d * norm, float static_light_level)
{
  int idx;
