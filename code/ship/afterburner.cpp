...
void afterburners_start(object * objp)
{
      ...
      if ((now - Player_afterburner_start_time) < 1300)
        {
          if (Viewer_mode & VM_NOT_COCKPIT)
            {
              snd_play_3d(&Snds[SND_ABURN_FAIL], &objp->pos, &View_position,
                          objp->radius);
            }
          else
            {
  ...
  //Do anim
  model_anim_start_type(shipp, TRIGGER_TYPE_AFTERBURNER,
                        ANIMATION_SUBTYPE_ALL, 1);
  ...
  if (objp == Player_obj)
    {
      ...
      if (Viewer_mode & VM_NOT_COCKPIT)
        ...
        {
          snd_play(&Snds[SND_ABURN_ENGAGE], 0.0f, 1.0f,
      ...
      if (!timestamp_elapsed(Player_disengage_timer))
        {
          float remaining;
          remaining =
            timestamp_until(Player_disengage_timer) / i2fl(DISENGAGE_TIME);
          if (remaining <= 0)
            {
              afterburner_stop_sounds();
            }
          ...
          if (shipp->afterburner_fuel < sip->afterburner_fuel_capacity)
            {
              ...
              recharge_scale =
                ...
                The_mission.
                ai_profile->afterburner_recharge_scale[Game_skill_level];
          ...
          if (Player_afterburner_loop_id == -1)
            {
              Player_afterburner_loop_id =
                snd_play_looping(&Snds[SND_ABURN_LOOP], 0.0f, -1, -1,
      ...
      if (timestamp_elapsed(volume_chg_timer))
        {
          ...
          if (percent_afterburner_left < AFTERBURNER_PERCENT_VOL_ATTENUATE)
            {
              Player_afterburner_vol =
                percent_afterburner_left * (1 /
                                            AFTERBURNER_PERCENT_VOL_ATTENUATE)
