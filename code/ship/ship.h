...
typedef struct exited_ship
{
  ...
  int damage_ship_id[MAX_DAMAGE_SLOTS];
    ...
    exited_ship():team(0),
    flags(0), time(0), hull_strength(0), time_cargo_revealed(0), cargo1(0)
  {
    ship_name[0] = '\0';
    obj_signature = -1;
    ship_class = -1;
...
typedef struct ship_type_info
{
  ...
  //Regen values - need to be converted after all types have loaded
    ...
    ship_type_info():message_bools(0),
    hud_bools(0),
    ship_bools(0),
    weapon_bools(0),
    ...
    ai_valid_goals(0),
    ai_player_orders(0),
    ai_bools(0), ai_active_dock(0), ai_passive_dock(0), vaporize_chance(0.f)
  {
    name[0] = '\0';
  }
} ship_type_info;
...
typedef struct man_thruster
{
  ...
  vec3d pos, norm;
    man_thruster():use_flags(0),
    tex_nframes(0), tex_fps(0), length(0.), radius(0.)
  {
    tex_id = -1;
    start_snd = -1;
    loop_snd = -1;
...
//returns -1 if failed
extern int ship_create(matrix * orient, vec3d * pos, int ship_type,
                       char *ship_name = NULL);
...
extern void ship_actually_depart(int shipnum, int method =
                                 SHIP_DEPARTED_WARP);

extern int ship_fire_primary_debug(object * objp);      //  Fire the debug laser.
extern int ship_stop_fire_primary(object * obj);
extern int ship_fire_primary(object * objp, int stream_weapons, int force =
                             0);
...
// Goober5000
extern int get_available_primary_weapons(object * objp, int *outlist,
                                         int *outbanklist);
...
extern int get_subsystem_pos(vec3d * pos, object * objp,
                             ship_subsys * subsysp);
...
extern void create_shield_explosion(int objnum, int model_num,
                                    matrix * orient, vec3d * centerp,
                                    vec3d * tcp, int tr0);

//  Initialize shield hit system.
extern void do_shield_effect(object * ship_objp, mc_info * mc,
                             bool shield_collision = true);
...
extern ship_subsys *ship_get_closest_subsys_in_sight(ship * sp,
                                                     int subsys_type,
                                                     vec3d * attacker_pos);
...
int ship_return_subsys_path_normal(ship * sp, ship_subsys * ss,
                                   vec3d * gsubpos, vec3d * norm);
int ship_subsystem_in_sight(object * objp, ship_subsys * subsys,
                            vec3d * eye_pos, vec3d * subsys_pos,
                            int do_facing_check = 1, float *dot_out =
                            NULL, vec3d * vec_out = NULL);
...
extern int ship_get_random_team_ship(int team_mask, int flags =
                                     SHIP_GET_ANY_SHIP, float max_dist =
                                     0.0f);
extern int ship_get_random_player_wing_ship(int flags =
                                            SHIP_GET_ANY_SHIP,
                                            float max_dist =
                                            0.0f, int persona_index =
                                            -1, int get_first =
                                            0, int multi_team = -1);
extern int ship_get_random_ship_in_wing(int wingnum, int flags =
                                        SHIP_GET_ANY_SHIP, float max_dist =
                                        0.0f, int get_first = 0);
...
extern void ship_do_cap_subsys_cargo_revealed(ship * shipp,
                                              ship_subsys * subsys,
                                              int from_network = 0);
extern void ship_do_cap_subsys_cargo_hidden(ship * shipp,
                                            ship_subsys * subsys,
                                            int from_network = 0);
...
extern int ship_explode_area_calc_damage(vec3d * pos1, vec3d * pos2,
                                         float inner_rad, float outer_rad,
                                         float max_damage, float max_blast,
...
extern void ship_subsystem_set_new_ai_class(int ship_num, char *subsystem,
                                            int new_ai_class);
...
extern void ship_do_submodel_rotation(ship * shipp, model_subsystem * psub,
                                      ship_subsys * pss);
