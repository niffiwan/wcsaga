...
 class ArmorType 
{
  ...
  {
    return (strnicmp(in_name, Name, strlen(Name)) == 0);
  }
...
typedef struct ship_type_info 
{
  ...
  float vaporize_chance;
    ...
    ship_type_info() :message_bools(0), hud_bools(0), ship_bools(0),
    weapon_bools(0),  debris_max_speed(0.f), ff_multiplier(0.f),
    emp_multiplier(0.f),  fog_start_dist(0.f), fog_complete_dist(0.f),
     ai_valid_goals(0), ai_player_orders(0), ai_bools(0), ai_active_dock(0),
    ai_passive_dock(0),  vaporize_chance(0.f)  
  {
...
typedef struct man_thruster 
{
   ...
    man_thruster() :use_flags(0), tex_nframes(0), tex_fps(0), length(0.),
    radius(0.) 
  {
