          ...
          if ((ship_system->system_info != NULL)
              && (ship_system->system_info->flags & MSS_FLAG_AWACS))
            {
              ...
              Awacs_team[shipp->team] +=
                ship_system->awacs_intensity * (ship_system->current_hits /
  ...
  if (target->type == OBJ_SHIP)
    {
      ...
      friendly_invisible = (shipp->flags2 & SF2_STEALTH)
        && (shipp->flags2 & SF2_FRIENDLY_STEALTH_INVIS);
    }

  int stealth_ship = (target->type == OBJ_SHIP) && (shipp != NULL)
    && (shipp->flags2 & SF2_STEALTH);
  int nebula_enabled = (The_mission.flags & MISSION_FLAG_FULLNEB);
  int check_huge_ship = (target->type == OBJ_SHIP) && (sip != NULL)
    && (sip->flags & SIF_HUGE_SHIP);

  // ships on the same team are always viewable
  // not necessarily now! :) -- Goober5000
  if ((target->type == OBJ_SHIP) && (shipp != NULL)
      && (shipp->team == viewer->team) && (!friendly_invisible))
  ...
  // all other ships
  else
    {
      ...
      float scan_nebula_range =
        Neb2_awacs *
        Species_info[Ship_info[viewer->ship_info_index].
                     species].awacs_multiplier;
              ...
              for (idx = 0; idx < cur_count; idx++)
                {
                  ...
                  if (awacs_get_level
                    ...
                    {
                      Ship_visibility_by_team[cur_team][en_team_ships[en_idx]]
                        = 1;
                      break;
                    }
                }
