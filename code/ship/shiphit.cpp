      ...
      for (i = 0; i < num_fireballs; i++)
        {
          ...
          fireball_create(&temp_vec, fireball_type, FIREBALL_MEDIUM_EXPLOSION,
...
float subsys_get_range(object * other_obj, ship_subsys * subsys)
{
  ...
  if ((other_obj) && (other_obj->type == OBJ_SHOCKWAVE))
    {
      // Goober5000 - check for NULL when via sexp
  ...
  for (j = 0; j < count; j++)
    {
      ...
      if ((dist < 10.0f)
          && ((other_obj) && (other_obj->type == OBJ_SHOCKWAVE)))
        {
      ...
      if (damage_to_apply > 0.1f && !(MULTIPLAYER_CLIENT)
          && !(Game_mode & GM_DEMO_PLAYBACK))
        {
          ...
          if (damage_left <= 0)
            {
              // no more damage to distribute, so stop checking
              damage_left = 0.0f;
              break;
            }
        }
  ...
  if (Ship_info[sp->ship_info_index].death_roll_time >= 0)
    {
      sp->death_time = sp->final_death_time =
        timestamp(Ship_info[sp->ship_info_index].death_roll_time);
    }
  else
    {
          ...
          if (Game_mode & GM_MULTIPLAYER)
          ...
          else
            {
              ...
              mission_log_add_entry(LOG_SHIP_DESTROYED, sp->ship_name,
                                    killer_ship_name, killer_damage_percent);
              //}
            }
        }
...
MONITOR(ShipHits) MONITOR(ShipNumDied)
     int maybe_shockwave_damage_adjust(object * ship_obj, object * other_obj,
                                       float *damage)
{
  ship_subsys *subsys;
  ship *shipp;
      ...
      if (other_obj->type == OBJ_WEAPON)
        {
          ...
          if (((Weapon_info[Weapons[other_obj->instance].weapon_info_index].
                subtype != WP_LASER) || special_check) && (Player_obj != NULL)
              && (ship_obj == Player_obj))
            {
              ship_hit_pain(damage, quadrant);
            }
                  ...
                  if ((other_obj->parent < 0)
                  ...
                  else
                    {
                      scoring_add_damage(ship_obj,
                                         &Objects[other_obj->parent], damage);
                    }
                  break;
                              ...
                              if (other_obj->type == OBJ_BEAM)
                                {
                                  ...
                                  if ((beam_weapon_info_index > -1)
                                      &&
                                      (Weapon_info
                                       [beam_weapon_info_index].wi_flags &
                                       (WIF_HUGE)))
                                    {
                                      // Flag as vaporized
