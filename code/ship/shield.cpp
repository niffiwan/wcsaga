...
extern void create_shield_from_triangle(int trinum, matrix * orient,
                                        shield_info * shieldp, vec3d * tcp,
                                        vec3d * centerp, float radius,
...
void do_shield_effect(object * ship_objp, mc_info * mc, bool shield_collision)
{
  ...
  if (shield_collision)
    ...
    {
      model_rot_sub_into_obj(&impact_pos, &mc->hit_point, pm,
                             mc->hit_submodel);
      impact_normal = mc->hit_normal;
      vm_vec_scale_add2(&impact_pos, &impact_normal, 1.0f);
    }

  particle_create(&impact_pos, &impact_normal, &vmd_zero_vector, 0.0f, radius,
                  PARTICLE_BITMAP_3D, anim_id, sip->shield_color[0],
                  sip->shield_color[1], sip->shield_color[2], -1.0f,
                  ship_objp);
}
...
void load_shield_hit_bitmap()
{
  ...
  for (i = 0; i < Species_info.size(); i++)
    {
      Species_info[i].shield_anim.first_frame =
        bm_load_animation(Species_info[i].shield_anim.filename,
                          &Species_info[i].shield_anim.num_frames, NULL, NULL,
                          1);
...
void create_low_detail_poly(int global_index, vec3d * tcp, vec3d * rightv,
                            vec3d * upv)
{
  float scale;
  gshield_tri *trip;
...
void rs_compute_uvs(shield_tri * stp, shield_vertex * verts, vec3d * tcp,
                    float radius, vec3d * rightv, vec3d * upv)
{
  int i;
  shield_vertex *sv;
...
void render_low_detail_shield_bitmap(gshield_tri * trip, matrix * orient,
                                     vec3d * pos, ubyte r, ubyte g, ubyte b)
{
  int j;
  vec3d pnt;
  vertex verts[4];
...
void render_shield_triangle(gshield_tri * trip, matrix * orient, vec3d * pos,
                            ubyte r, ubyte g, ubyte b)
{
  ...
  vec3d norm;
  Poly_count++;
  vm_vec_perp(&norm, (vec3d *) & verts[0]->x, (vec3d *) & verts[1]->x,
              (vec3d *) & verts[2]->x);
     ...
     void render_shield(int shield_num) //, matrix *orient, vec3d *centerp)
{
  ...
  if (Poly_count > 50)
    {
      if (Shield_hits[shield_num].start_time +
          (SHIELD_HIT_DURATION * 50) / Poly_count < Missiontime)
        {
          Shield_hits[shield_num].type = SH_UNUSED;
  ...
  if (sa->first_frame >= 0)
    {
      frame_num =
        fl2i(f2fl(Missiontime - Shield_hits[shield_num].start_time) *
             sa->num_frames);
      ...
      gr_set_bitmap(bitmap_id, GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL,
                    alpha);
      ...
      if ((Detail.shield_effects == 1) || (Detail.shield_effects == 2))
        {
          if (bitmap_id != -1)
            {
              render_low_detail_shield_bitmap(&Global_tris
                                              [Shield_hits[shield_num].
                                               tri_list[0]], orient, centerp,
                                              Shield_hits[shield_num].rgb[0],
              ...
              for (i = 0; i < Shield_hits[shield_num].num_tris; i++)
                {
                  ...
                  render_shield_triangle(&Global_tris
                                         [Shield_hits[shield_num].
                                          tri_list[i]], orient, centerp,
                                         Shield_hits[shield_num].rgb[0],
                                         Shield_hits[shield_num].rgb[1],
                                         Shield_hits[shield_num].rgb[2]);
                }
...
// -----------------------------------------------------------------------------------------------------
void create_tris_containing(vec3d * vp, matrix * orient,
                            shield_info * shieldp, vec3d * tcp,
                            vec3d * centerp, float radius, vec3d * rvec,
                            vec3d * uvec)
{
  int i, j;
  shield_vertex *verts;
          ...
          for (j = 0; j < 3; j++)
            {
              ...
              if ((vp->xyz.x == v.xyz.x) && (vp->xyz.y == v.xyz.y)
                  && (vp->xyz.z == v.xyz.z))
                create_shield_from_triangle(i, orient, shieldp, tcp, centerp,
                                            radius, rvec, uvec);
            }
        }
    }
...
void visit_children(int trinum, int vertex_index, matrix * orient,
                    shield_info * shieldp, vec3d * tcp, vec3d * centerp,
                    float radius, vec3d * rvec, vec3d * uvec)
{
  ...
  if ((sv->u > 0.0f) && (sv->u < UV_MAX) && (sv->v > 0.0f)
      && (sv->v < UV_MAX))
    create_tris_containing(&sv->pos, orient, shieldp, tcp, centerp, radius,
                           rvec, uvec);
}
...
void create_shield_from_triangle(int trinum, matrix * orient,
                                 shield_info * shieldp, vec3d * tcp,
                                 vec3d * centerp, float radius, vec3d * rvec,
                                 vec3d * uvec)
{
  ...
  rs_compute_uvs(&shieldp->tris[trinum], shieldp->verts, tcp, radius, rvec,
                 uvec);
  ...
  //mprintf(("%i ", trinum));
  visit_children(trinum, 0, orient, shieldp, tcp, centerp, radius, rvec,
                 uvec);
  visit_children(trinum, 1, orient, shieldp, tcp, centerp, radius, rvec,
                 uvec);
  visit_children(trinum, 2, orient, shieldp, tcp, centerp, radius, rvec,
                 uvec);
}
...
void copy_shield_to_globals(int objnum, shield_info * shieldp)
{
  ...
  if ((objnum >= 0) && (objnum < MAX_OBJECTS)
      ...
      && (Ships[Objects[objnum].instance].ship_info_index < Num_ship_classes))
    {
      ship_info *sip =
...
void create_shield_low_detail(int objnum, int model_num, matrix * orient,
                              vec3d * centerp, vec3d * tcp, int tr0,
                              shield_info * shieldp)
{
  matrix tom;
  int gi;
...
void create_shield_explosion(int objnum, int model_num, matrix * orient,
                             vec3d * centerp, vec3d * tcp, int tr0)
{
  ...
  create_shield_from_triangle(tr0, orient, shieldp, tcp, centerp,
                              Objects[objnum].radius, &tom.vec.rvec,
                              &tom.vec.uvec);
  //nprintf(("AI", "\n"));
  for (i = 0; i < 3; i++)
    {
      create_shield_from_triangle(shieldp->tris[tr0].neighbors[i], orient,
                                  shieldp, tcp, centerp,
                                  Objects[objnum].radius, &tom.vec.rvec,
                                  &tom.vec.uvec);
      ...
      for (int j = 0; j < 3; j++)
        {
          create_shield_from_triangle(shieldp->
                                      tris[shieldp->tris[tr0].neighbors[i]].
                                      neighbors[j], orient, shieldp, tcp,
                                      centerp, Objects[objnum].radius,
          ...
          for (j = 0; j < 3; j++)
            {
              ...
              // Rotate point into world coordinates
              vm_vec_unrotate(&pnt, &pm->shield.verts[tri->verts[j]].pos,
                              &objp->orient);
  ...
  if ((quadrant >= 0) && (quadrant < MAX_SHIELD_SECTIONS))
    {
      ...
      if (obj->shield_quadrant[quadrant] >
          MAX(2.0f, 0.1f * get_max_shield_quad(obj)))
        {
          return 1;
        }
    }
  else
    {
      ...
      if (strength >
          MAX(2.0f * 4.0f,
              0.1f * Ships[obj->instance].ship_max_shield_strength))
        {
          return 1;
        }
    }
