                                  ...
                                  float *warp_time, vec3d * warp_pos)
{
  ...
  if (-vm_vec_dotprod(&objp->orient.vec.fvec, &facing_normal) <
      max_warpout_angle)
    {
      // within allowed angle
      Int3();
                         ...
                         int weapon_info_index)
{
  ...
  if ((Weapon_info[weapon_info_index].wi_flags & WIF_MFLASH)
      && !(Weapon_info[weapon_info_index].wi_flags & WIF_FLAK))
    {
      ...
      mflash_create(gun_pos, &real_dir, &objp->phys_info,
          ...
          if (half_ship->explosion_vel > 0)
            {
              if (half_ship->cur_clip_plane_pt >
                  tmp1.xyz.z + pm->submodel[pm->debris_objects[i]].max.xyz.z -
            ...
            {
              if (half_ship->cur_clip_plane_pt <
                  tmp1.xyz.z + pm->submodel[pm->debris_objects[i]].min.xyz.z -
                  0.1f * half_ship->explosion_vel)
                {
                  create_debris = 1;
                }
                  ...
                  if (debris_obj)
                    {
                      ...
                      shipfx_debris_limit_speed(&Debris[debris_obj->instance],
                                                shipp);
                      /* } else {
                  ...
                  if (dist_sqr < max_wash_dist * max_wash_dist)
                    {
                      ...
                      if (dist_sqr <
                          ((radius_mult * radius_mult) *
                           (bank->points[j].radius * bank->points[j].radius)))
                        {
...
//********************-----CLASS: WE_Default-----********************//
WE_Default::WE_Default(object * n_objp, int n_direction):WarpEffect(n_objp,
           n_direction)
{
  if (!this->isValid())
    return;
...
//********************-----CLASS: WE_BSG-----********************//
WE_BSG::WE_BSG(object * n_objp, int n_direction):WarpEffect(n_objp,
           n_direction)
{
...
//********************-----CLASS: WE_Homeworld-----********************//
WE_Homeworld::WE_Homeworld(object * n_objp, int n_direction):WarpEffect(n_objp,
           n_direction)
{
  if (!this->isValid())
    return;
...
//********************-----CLASS: WE_Hyperspace----********************//
WE_Hyperspace::WE_Hyperspace(object * n_objp, int n_direction):WarpEffect(n_objp,
           n_direction)
{
  total_duration = 0;
  if (direction == WD_WARP_IN)
