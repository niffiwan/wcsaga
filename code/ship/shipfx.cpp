...
void shipfx_subsystem_maybe_create_live_debris(object * ship_obj,
                                               ship * ship_p,
                                               ship_subsys * subsys,
                                               vec3d * exp_center,
                                               float exp_mag) 
  ...
  0.0f, 0.0f, 0.0f};
    ...
    
    {
      ...
      void model_get_rotating_submodel_axis(vec3d * model_axis,
                                            vec3d * world_axis, int modelnum,
                                            int submodel_num, object * obj);
      model_get_rotating_submodel_axis(&model_axis, &world_axis, pm->id,
                                ...
                                submodel_num, &ship_obj->orient,
                                &ship_obj->pos);
       vm_quaternion_rotate(&m_rot, vm_vec_mag((vec3d *) & sii->angs),
                              &model_axis);
    ...
    
    {
        ...
        vm_vec_zero(&start_world_pos);
                              ...
                              pm->id, live_debris_submodel, &ship_obj->orient,
                              &ship_obj->pos);
        ...
        fireball_create(&end_world_pos, fireball_type,
                        FIREBALL_MEDIUM_EXPLOSION, OBJ_INDEX(ship_obj),
                        pm->submodel[live_debris_submodel].rad);
       
        // create debris
        live_debris_obj =
        debris_create(ship_obj, pm->id, live_debris_submodel, &end_world_pos,
        ...
        
        {
            ...
            
            {
                ...
                vm_vec_mag_quick(&radial_vel) * 1.3f * (0.9f +
                                                        0.2f * frand());
          ...
           shipfx_debris_limit_speed(&Debris[live_debris_obj->instance],
                                       ship_p);
    ...
    
    {
        ...
        
        {
          ...
          int model_get_parent_submodel_for_live_debris(int model_num,
                                                        int
                                                        live_debris_model_num);
          int parent =
            model_get_parent_submodel_for_live_debris(pm->id,
                                                      live_debris_submodel);
            ...
            
            {
                ...
                
                {
                    ...
                    
                    {
                      ...
                      model_find_world_point(&exp_center, &tmp, pm->id,
                                              parent, &ship_obj->orient,
                                              &ship_obj->pos);
                       
                        // if not blown off, blow it off
                        shipfx_subsystem_maybe_create_live_debris(ship_obj,
                                                                  shipp, pss,
                                                                  &exp_center,
                                                                  3.0f);
                      ...
                      set_ship_submodel_as_blown_off(&Ships
                                                      [ship_obj->instance],
                                                      pss->system_info->
                                                      subobj_name);
...
 void shipfx_blow_off_subsystem(object * ship_obj, ship * ship_p,
                                  ship_subsys * subsys, vec3d * exp_center) 
{
    ...
    shipfx_blow_up_model(ship_obj, model_num, psub->subobj_num, 50,
                         &subobj_pos);
   
    // create live debris objects, if any
    // TODO:  some MULITPLAYER implcations here!!
    shipfx_subsystem_maybe_create_live_debris(ship_obj, ship_p, subsys,
                                              exp_center, 1.0f);
    ...
    fireball_create(&subobj_pos, fireball_type, FIREBALL_MEDIUM_EXPLOSION,
                    OBJ_INDEX(ship_obj), psub->radius);
...
  void shipfx_blow_up_hull(object * obj, int model, vec3d * exp_center) 
{
    ...
    
    {
                                   ...
                                   (Ships[obj->instance].arrival_distance),
                                   MULTI_SIG_DEBRIS);
    ...
    
    {
        ...
        
        {
          ...
          debris_create(obj, model, pm->debris_objects[i], &tmp, exp_center,
                         1, 3.0f);
...
// ship's model.
void shipfx_blow_up_model(object * obj, int model, int submodel, int ndebris,
                          vec3d * exp_center) 
{
...
static float shipfx_calculate_effect_radius(object * objp, int warp_dir) 
{
    ...
    
    {
        ...
        dock_calc_max_cross_sectional_radius_perpendicular_to_axis(objp,
                                                                   Z_AXIS);
...
float shipfx_calculate_warp_time(object * objp, int warp_dir) 
{
  ...
  float rad_percent =
    ...
    rad_percent * (LARGEST_RAD_TIME - SMALLEST_RAD_TIME) + SMALLEST_RAD_TIME;
...
int shipfx_special_warp_objnum_valid(int objnum) 
{
    ...
    // must be a knossos
    if (!
        (Ship_info[Ships[special_objp->instance].ship_info_index].
         flags & SIF_KNOSSOS_DEVICE))
...
int compute_special_warpout_stuff(object * objp, float *speed,
                                  float *warp_time, vec3d * warp_pos) 
{
    ...
    dist_to_plane =
    fvi_ray_plane(warp_pos, &sp_objp->pos, &facing_normal, &center_pos,
...
  void compute_warpout_stuff(object * objp, float *speed, float *warp_time,
                                vec3d * warp_pos) 
{
  ...
   vm_vec_scale_add(warp_pos, &center_pos, &objp->orient.vec.fvec,
                      warp_dist);
...
// it can see the sun.
int shipfx_point_in_shadow(vec3d * p0, matrix * src_orient, vec3d * src_pos,
                           float radius) 
{
...
// accordingly.
void shipfx_flash_create(object * objp, int model_num, vec3d * gun_pos,
                         vec3d * gun_dir, int is_primary,
                         int weapon_info_index) 
{
    ...
    
    {
      ...
      mflash_create(&real_pos, gun_dir, &objp->phys_info,
    ...
    
    {
        ...
        
        {
            ...
            (Ship_flash[i].max_life -
             Ship_flash[i].life) / Ship_flash[i].max_life;
...
DCF(particle_num, "Multiplier for the number of particles created") 
{
    ...
    
    {
        ...
        
        {
            ...
            ("Illegal value for particle num. (Must be from 0-5) \n\n");
...
DCF(particle_life, "Multiplier for the lifetime of particles created") 
{
    ...
    
    {
        ...
        
        {
            ...
            ("Illegal value for particle life. (Must be from 0-5) \n\n");
...
static void split_ship_init(ship * shipp, split_ship * split_ship) 
{
    ...
    snd_play_3d(&Snds[SND_SHOCKWAVE_EXPLODE], &parent_ship_obj->pos,
                &View_position, 0.0f, NULL, 0, 1.0f,
  ...
  split_ship->explosion_flash_timestamp =
  ...
  vm_vec_copy_scale(&split_ship->front_ship.local_pivot, &orient->vec.fvec,
                     dist);
  ...
  vm_vec_copy_scale(&split_ship->back_ship.local_pivot, &orient->vec.fvec,
                     dist);
    ...
    
    {
        ...
        model_find_world_point(&tmp, &tmp1, pm->id, -1, &vmd_identity_matrix,
                               &temp_pos);
  ...
  float front_length =
    pm->maxs.xyz.z - split_ship->front_ship.cur_clip_plane_pt;
  float back_length =
    split_ship->back_ship.cur_clip_plane_pt - pm->mins.xyz.z;
  ...
  float speed_reduction_factor = (1.0f + 0.001f * parent_ship_obj->radius);
    ...
    (3.0f + expl_length_scale + (frand() - 0.5f)) * speed_reduction_factor;
    ...
    vec3d rotvel;
                     ...
                     -(front_length * front_length) / (back_length *
                                                       back_length));
  split_ship->front_ship.phys_info.rotvel.xyz.z =
    parent_ship_obj->phys_info.rotvel.xyz.z;
  split_ship->back_ship.phys_info.rotvel.xyz.z =
    parent_ship_obj->phys_info.rotvel.xyz.z;
  ...
  maybe_fireball_wipe(&split_ship->back_ship,
                       (int *) &split_ship->sound_handle);
  vm_vec_scale_add2(&split_ship->front_ship.phys_info.vel, &orient->vec.fvec,
                     front_vel);
  vm_vec_scale_add2(&split_ship->back_ship.phys_info.vel, &orient->vec.fvec,
                     back_vel);
   
    // HANDLE LIVE DEBRIS - blow off if not already gone
    shipfx_maybe_create_live_debris_at_ship_death(parent_ship_obj);
}   static void half_ship_render_ship_and_debris(clip_ship * half_ship,
                                                     ship * shipp) 
{
    ...
    
    {
        ...
        
        {
          ...
          int is_live_debris =
            ...
            {
              model_find_world_point(&tmp, &tmp1, pm->id, -1,
                                      &half_ship->orient, &temp_pos);
              submodel_render(pm->id, pm->debris_objects[i],
                               &half_ship->orient, &tmp, render_flags, -1,
            ...
            
            {
              ...
              int debris_count = 0;
                ...
                
                {
                    ...
                    debris_create(half_ship->parent_obj, pm->id,
                    ...
                    
                    {
                      ...
                      vm_vec_scale_add2(&debris_vel, &radial_vel,
                                         radial_mag);
                      debris_obj->phys_info.vel = debris_vel;
                      shipfx_debris_limit_speed(&Debris
                                                 [debris_obj->instance],
                                                 shipp);
  ...
  model_render(pm->id, &half_ship->orient, &orig_ship_world_center,
                render_flags, -1, -1, shipp->ship_replacement_textures);
...
 void shipfx_debris_limit_speed(debris * db, ship * shipp) 
{
    ...
    
    {
        ...
        {
          vm_vec_copy_scale(&pi->vel, &ship_obj->orient.vec.fvec,
                             debris_speed);
  ...
  float currotvel = vm_vec_mag(&pi->rotvel);
    ...
    
    {
        ...
        ((sip->debris_max_rotspeed - sip->debris_min_rotspeed) * frand()) +
        ...
        {
          vm_vec_copy_scale(&pi->rotvel, &ship_obj->orient.vec.uvec,
                             debris_rotspeed);
...
void do_sub_expl_sound(float radius, vec3d * sound_pos, int *sound_handle) 
{
  ...
  float sound_range = 1.0f + 0.0043f * radius;
    ...
    
    {
        ...
        sound_handle[handle_index] =
        snd_play_3d(&Snds[sound_index], sound_pos, &View_position, 0.0f, NULL,
                    0, 0.6f, SND_PRIORITY_MUST_PLAY, NULL, sound_range);
    ...
    
    {
        ...
        
        {
            ...
            snd_play_3d(&Snds[sound_index], sound_pos, &View_position, 0.0f,
                        NULL, 0, 0.6f, SND_PRIORITY_MUST_PLAY, NULL,
                        sound_range);
...
static void maybe_fireball_wipe(clip_ship * half_ship, int *sound_handle) 
{
    ...
    
    {
        ...
        
        {
            ...
            &Ship_info[Ships[half_ship->parent_obj->instance].
                       ship_info_index];
            ...
            get_model_cross_section_at_z(half_ship->cur_clip_plane_pt, pm);
            ...
            
            {
                ...
                rand() % FIREBALL_NUM_LARGE_EXPLOSIONS;
                           ...
                           &half_ship->parent_obj->phys_info.vel, 0.0f, -1,
                           NULL, low_res_fireballs);
          ...
          int time_low, time_high;
            ...
            // do sound
            do_sub_expl_sound(half_ship->parent_obj->radius,
                              &model_clip_plane_pt, sound_handle);
...
int shipfx_large_blowup_do_frame(ship * shipp, float frametime) 
{
  ...
   split_ship * the_split_ship =
    &Split_ships[shipp->large_ship_blowup_index];
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                vm_vec_dist(&Player_obj->pos,
                            &objp->pos) - 2.0f * objp->radius -
                Player_obj->radius;
                ...
                
                {
                     ...
                     && Ship_info[shipp->ship_info_index].flags2 & SIF2_FLASH)
    ...
    MAX(the_split_ship->front_ship.length_left,
        the_split_ship->back_ship.length_left);
      ...
      float factor = 1.0f + 0.0025f * obj->radius;
        ...
        
        {
            ...
            
            {
                ...
                // 1.00 second effect
                snd_play_3d(&Snds[SND_DEBRIS_ARC_05], &snd_pos,
                            &View_position, obj->radius);
            ...
            
            {
                ...
                // 0.75 second effect
                snd_play_3d(&Snds[SND_DEBRIS_ARC_04], &snd_pos,
                            &View_position, obj->radius);
            ...
            
            {
                ...
                // 0.50 second effect
                snd_play_3d(&Snds[SND_DEBRIS_ARC_03], &snd_pos,
                            &View_position, obj->radius);
            ...
            
            {
                ...
                // 0.25 second effect
                snd_play_3d(&Snds[SND_DEBRIS_ARC_02], &snd_pos,
                            &View_position, obj->radius);
            ...
            
            {
                ...
                // 0.10 second effect
                snd_play_3d(&Snds[SND_DEBRIS_ARC_01], &snd_pos,
                            &View_position, obj->radius);
...
void shipfx_do_shockwave_stuff(ship * shipp, shockwave_create_info * sci) 
{
    ...
    
    {
        ...
        (sci->blast / (float) sip->shockwave_count) * frand_range(0.75f,
                                                                  1.25f);
      sci2.damage =
        (sci->damage / (float) sip->shockwave_count) * frand_range(0.75f,
                                                                   1.25f);
     ...
     void engine_wash_ship_process(ship * shipp) 
{
  ...
   vec3d world_thruster_pos, world_thruster_norm, apex, thruster_to_ship,
    apex_to_ship, temp;
  ...
  int do_damage = 0;
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                if ((wash_sip->flags & SIF_HUGE_SHIP)
                    && Engine_wash_info.size())
            ...
            
            {
                ...
                vm_vec_unrotate(&world_thruster_pos, &bank->points[j].pnt,
                                &wash_objp->orient);
              vm_vec_add2(&world_thruster_pos, &wash_objp->pos);
               
                // get world norm of thruster;
                vm_vec_unrotate(&world_thruster_norm, &bank->points[j].norm,
                                &wash_objp->orient);
               
                // get vector from thruster to ship
                vm_vec_sub(&thruster_to_ship, &objp->pos,
                           &world_thruster_pos);
               
                // check if on back side of thruster
                dot_to_ship =
                vm_vec_dotprod(&thruster_to_ship, &world_thruster_norm);
                ...
                
                {
                    ...
                    
                    {
                        ...
                        
                        {
                            ...
                            if (vm_vec_dotprod
                                (&apex_to_ship,
                                 &world_thruster_norm) > cos(half_angle))
    ...
    
    {
        ...
        
        {
            ...
            {
              obj_snd_assign(shipp->objnum, SND_ENGINE_WASH,
                              &vmd_zero_vector, 1);
            ...
            {
              Player_engine_wash_loop =
                snd_play_looping(&Snds[SND_ENGINE_WASH], 0.0f, -1, -1, 1.0f);
...
 void shipfx_cloak_frame(ship * shipp, float frametime) 
{
    ...
    
    {
      ...
      {
         if (timestamp_elapsed(shipp->time_until_uncloak)
               && (shipp->time_until_uncloak != 0))
          ...
          fl2i(shipfx_calc_visibility(obj, &Eye_position) * 255.0f);
    ...
    vm_vec_scale_add2(&shipp->current_translation,
                      &shipp->texture_translation_key, frametime);
...
static const size_t Warp_variables_num =
  sizeof(Warp_variables) / sizeof(flag_def_list);
...
 int WE_Default::warpStart() 
{
    ...
    
    {
        ...
        // for ship to go thru, and 'SHIPFX_WARP_DELAY' (1.5 secs) to go away.
        warp_time =
        shipfx_calculate_warp_time(objp,
                                   WD_WARP_IN) + SHIPFX_WARP_DELAY +
        SHIPFX_WARP_DELAY;
    ...
    
    {
        ...
        // maybe special warpout
      int fireball_type = ((portal_objp != NULL)
                           || Cmdline_tbp) ? FIREBALL_KNOSSOS : FIREBALL_WARP;
       
        // create fireball
        warp_objnum =
        fireball_create(&pos, fireball_type, FIREBALL_WARP_EFFECT,
                        OBJ_INDEX(objp), radius, 1, NULL, warp_time,
                        shipp->ship_info_index, NULL, 0, 0,
    ...
    
    {
        ...
        // maybe special warpin
      int fireball_type =
        (portal_objp != NULL) ? FIREBALL_KNOSSOS : FIREBALL_WARP;
       
        // create fireball
        warp_objnum =
        fireball_create(&pos, fireball_type, FIREBALL_WARP_EFFECT,
                        OBJ_INDEX(objp), radius, 0, NULL, warp_time,
                        shipp->ship_info_index, NULL, 0, 0,
    ...
    
    {
        ...
        
        {
            ...
            if ((warp_pos > objp->radius)
                || (shipp->flags & SF_NO_DEPARTURE_WARP) || timed_out)
           ...
           n_direction) 
{
  ...
  char tmp_name[MAX_FILENAME_LEN];
    ...
    
    {
        ...
        //Load shockwave
        shockwave =
        bm_load_either(tmp_name, &shockwave_nframes, &shockwave_fps, NULL, 1);
      if (shockwave > -1)
        
        {
          shockwave_total_time =
            fl2i(((float) shockwave_nframes / (float) shockwave_fps) *
                 1000.0f);
    ...
    
    {
        ...
        if (tube_radius <= 0.0f)
          ...
          MAX((pm->maxs.xyz.y - pm->mins.xyz.y),
              (pm->maxs.xyz.x - pm->mins.xyz.x)) / 2.0f;
...
 int WE_BSG::warpStart() 
{
  ...
  int gs_start_index = -1;
    ...
    
    {
        ...
        snd_play_3d(snd_start_gs, &objp->pos, &View_position, 0.0f, NULL, 0,
                    1, SND_PRIORITY_SINGLE_INSTANCE, NULL, snd_range_factor);
...
 int WE_BSG::warpFrame(float frametime) 
{
    ...
    snd_update_3d_pos(snd_start, snd_start_gs, &objp->pos, 0.0f,
                       snd_range_factor);
...
 int WE_BSG::warpShipRender() 
{
  ...
  int saved_zbuffer_mode = gr_zbuffer_get();
    ...
    
    {
      ...
      int anim_frame =
        fl2i(((float) (timestamp() - total_time_start) / 1000.0f) *
             (float) anim_fps);
        ...
        
        {
            ...
            //Do warpout geometry
            vec3d start, end;
          vm_vec_scale_add(&start, &pos, &objp->orient.vec.fvec,
                            z_offset_min);
    ...
    
    {
      ...
      int shockwave_frame =
        fl2i(((float) (timestamp() - stage_time_start) / 1000.0f) *
             (float) shockwave_fps);
        ...
        
        {
          ...
          gr_set_bitmap(shockwave + shockwave_frame, GR_ALPHABLEND_FILTER,
                         GR_BITBLT_MODE_NORMAL, 1.0f);
          g3_draw_bitmap(&p, 0, tube_radius,
...
 int WE_BSG::warpEnd() 
{
      ...
      snd_play_3d(snd_end_gs, &objp->pos, &View_position, 0.0f, NULL, 0, 1.0f,
           ...
           n_direction) 
{
    ...
    if (direction == WD_WARP_IN)
      ...
      bm_load_either(sip->warpin_anim, &anim_nframes, &anim_fps, NULL, 1);
  
  else if (direction == WD_WARP_OUT)
    anim =
      bm_load_either(sip->warpout_anim, &anim_nframes, &anim_fps, NULL, 1);
    ...
    snd_range_factor =
    sqrt(width_full * width_full + height_full * height_full) / 141.421356f;
...
 int WE_Homeworld::warpStart() 
{
    ...
    
    {
        ...
        snd_play_3d(snd_gs, &pos, &View_position, 0.0f, NULL, 0, 1,
                    SND_PRIORITY_SINGLE_INSTANCE, NULL, snd_range_factor);
...
 int WE_Homeworld::warpFrame(float frametime) 
{
  ...
  float progress =
    ((float) timestamp() -
     (float) stage_time_start) / ((float) stage_time_end -
    ...
    
    {
                        ...
                        z_offset_max - progress * (z_offset_max -
                                                   z_offset_min));
...
 int WE_Homeworld::warpShipRender() 
{
      ...
      fl2i((int)
           (((float) (timestamp() - (float) total_time_start) / 1000.0f) *
            (float) anim_fps) % anim_nframes);
   
    //Set the correct frame
    gr_set_bitmap(anim + frame, GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL,
                  1.0f);
  g3_draw_polygon(&pos, &objp->orient, width, height,
                   TMAP_FLAG_TEXTURED | TMAP_HTL_3D_UNLIT);
...
 int WE_Hyperspace::warpFrame(float frametime) 
{
    ...
    
    {
        ...
        ((float) timestamp() -
         (float) total_time_start) / (float) total_duration;
      ...
      vm_vec_scale_add(&objp->pos, &pos_final, &objp->orient.vec.fvec,
                        scale);
