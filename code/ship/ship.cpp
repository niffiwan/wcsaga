...
static boost::unordered_map < std::string,
  ship_subsys * >ship_subsys_map[MAX_SHIPS];
...
void init_ship_entry(ship_info * sip)
{
  ...
  sip->type_str = sip->velocity_str = sip->maneuverability_str =
    sip->shields_str = sip->armor_str = sip->manufacturer_str = NULL;
  sip->desc = NULL;
  sip->tech_desc = NULL;
      ...
      if (!mission_parse_get_arrival_ship(name))
        {
          ...
          bool mother_ship_departed =
            (mission_log_get_time(LOG_SHIP_DEPARTED, name, NULL, NULL) != 0);
          bool mother_ship_destroyed =
            (mission_log_get_time(LOG_SELF_DESTRUCTED, name, NULL, NULL) != 0)
            || (mission_log_get_time(LOG_SHIP_DESTROYED, name, NULL, NULL) !=
                0);

          if (mother_ship_not_in_mission || mother_ship_destroyed
              || mother_ship_departed)
            {
                      ...
                      bool replace)
{
  ...
  if (optional_string("$Texture Replace:"))
    {
      ...
      while ((PLACEHOLDER_num_texture_replacements < MAX_REPLACEMENT_TEXTURES)
             && (optional_string("+old:")))
        {
  ...
  if (optional_string("$POF target file:"))
    {
      ...
      if (valid)
        ...
        WarningEx(LOCATION, "Ship \"%s\" POF target file \"%s\" invalid!",
                  sip->name, temp);
    }
  ...
  find_and_stuff_optional("$AI Class:", &sip->ai_class, F_NAME,
                          Ai_class_names, Num_ai_classes, "AI class names");
  ...
  if (has_afterburner == 1)
    {
      ...
      if (!(sip->afterburner_fuel_capacity))
        {
          Warning(LOCATION,
                  "Ship class %s has an afterburner but has no afterburner fuel. Setting fuel to 1",
                  sip->name);
            ...
            if (optional_string("$Engine Wash:"))
              {
                ...
                if (sp->engine_wash_pointer == NULL)
                  Warning(LOCATION,
                          "Invalid engine wash name %s specified for subsystem %s in ship class %s",
  ...
  for (i = 0; i < sinfo->n_subsystems; i++)
    {
      ...
      ship_subsys_map[Objects[objnum].instance][ship_system->system_info->
                                                subobj_name] = ship_system;
      ...
      for (k = 0; k < MAX_SHIP_SECONDARY_BANKS; k++)
        {
          ship_system->weapons.secondary_bank_ammo[k] =
            (Fred_running ? 100 : ship_system->
             weapons.secondary_bank_capacity[k]);
      ...
      for (k = 0; k < MAX_SHIP_PRIMARY_BANKS; k++)
        {
          ship_system->weapons.primary_bank_ammo[k] =
            (Fred_running ? 100 : ship_system->
             weapons.primary_bank_capacity[k]);
        }
...
DCF_BOOL(ship_shadows, Ship_shadows) MONITOR(NumShipsRend)
...
void batch_render_man_thrusters()
{
  ...
  for (uint i = 0; i < mant_size; i++)
    {
      mtr = &Man_thrusters[i];
      gr_set_bitmap(mtr->bmap_id, GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL,
                    1.0f);
...
void ship_render_cockpit(object * objp)
{
  ...
  model_render(sip->cockpit_model_num, &eye_ori, &pos,
               MR_LOCK_DETAIL | MR_NO_FOGGING /*| MR_NO_LIGHTING */ , -1, -1);
...
void maybe_handle_destroyed_or_departed(wing * wingp, int team)
{
  ...
  bool are_all_ships_gone =
    wingp->total_destroyed + wingp->total_departed + wingp->total_vanished ==
    wingp->total_arrived_count;
      ...
      for (objp = GET_FIRST(&obj_used_list);
           objp != END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp))
        {
          ...
          if (ship_explode_area_calc_damage
              (&exp_objp->pos, &objp->pos, inner_rad, outer_rad, max_damage,
  ...
  while (object_is_dead_docked(dying_objp))
    {
      ...
      physics_apply_whack(&impulse_vec, &pos, &docked_objp->phys_info,
                          &docked_objp->orient, docked_objp->phys_info.mass);
          ...
          for (int zz = 0; zz < 6; zz++)
            {
              ...
              fireball_create(&outpnt, fireball_type,
                              FIREBALL_MEDIUM_EXPLOSION, OBJ_INDEX(objp), rad,
                              0, &objp->phys_info.vel);
            }
        }
  ...
  if (Ship_info[shipp->ship_info_index].draw_primary_models[bank_to_stop])
    {
      if (shipp->primary_rotate_rate[bank_to_stop] > 0.0f)
        shipp->primary_rotate_rate[bank_to_stop] -=
          Weapon_info[swp->
                      primary_bank_weapons
                      [bank_to_stop]].weapon_submodel_rotate_accell *
          flFrametime;
      if (shipp->primary_rotate_rate[bank_to_stop] < 0.0f)
                          ...
                          if (!(sip->flags2 & SIF2_GUN_CONVERGENCE))
                            {
                              if ((sip->aiming_flags & AIM_FLAG_AUTOAIM) &&
                                  aip->target_objnum != -1 &&
                                  Players[Player_num].lead_indicator_active ==
                                  1)
                                {
                                  // Fire weapon in target direction
                                  vec3d target_position, target_velocity_vec,
                          ...
                          if (winfo_p->wi_flags & WIF_FLAK)
                            {
                              ...
                              if (target != NULL)
                                {
                                  set_predicted_enemy_pos(&predicted_pos, obj,
                                                          &target->pos,
                                                          &target->phys_info.
                                                          vel, aip);
                                  range_to_target =
      ...
      if (ship_select_next_valid_secondary_bank(swp))
        {
          ...
          if (timestamp_elapsed
              (shipp->weapons.
               next_secondary_fire_stamp[shipp->weapons.
                                         current_secondary_bank]))
            {
  ...
  if (swp->num_primary_banks == 0)
  ...
  else if (swp->num_primary_banks == 1)
    {
      if (objp == Player_obj)
        {
          HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                             XSTR("This ship has only one primary weapon: %s",
                                  491),
                             Weapon_info[swp->
...
float ship_calculate_rearm_duration(object * objp)
{
  ...
  return shield_rep_time + hull_rep_time + subsys_rep_time + prim_rearm_time +
    sec_rearm_time + 1.2f;
}
...
int ship_do_rearm_frame(object * objp, float frametime)
{
  ...
  if ((subsys_all_ok && shields_full
       && (The_mission.flags & MISSION_FLAG_SUPPORT_REPAIRS_HULL) && hull_ok)
          ...
          if ((moveup->system_info->rotation_snd != -1)
              && (moveup->system_info->flags & MSS_FLAG_ROTATES))
            {
              obj_snd_assign(sp->objnum, moveup->system_info->rotation_snd,
                             &moveup->system_info->pnt, 0, OS_SUBSYS_ROTATION,
                             moveup);
...
void ship_maybe_praise_self(ship * deader_sp, ship * killer_sp)
{
  ...
  message_send_builtin_to_player(MESSAGE_PRAISE_SELF, killer_sp,
                                 MESSAGE_PRIORITY_HIGH, MESSAGE_TIME_SOON, 0,
                                 0, -1, -1);
...
{
  if (stristr(subsys, "nav") || stristr(subsys, "comm") || stristr(subsys, "engine") || stristr(subsys, "fighter")      // fighter bays
      || stristr(subsys, "sensors") || stristr(subsys, "weapons"))
    {
...
ship_subsys *ship_get_subsys(ship * shipp, char *subsys_name)
{
  ...
  boost::unordered_map < std::string, ship_subsys * >*map =
    &ship_subsys_map[Objects[shipp->objnum].instance];
  boost::unordered_map < std::string, ship_subsys * >::iterator subsys_iter =
    map->find(subsys_name);
             ...
             (Weapon_info
              [shipp->weapons.
               primary_bank_weapons[shipp->weapons.
                                    current_primary_bank]].b_info.beam_type ==
              BEAM_TYPE_C));
      if (!
          (Weapon_info
           [shipp->weapons.
          ...
          (Weapon_info
           [shipp->weapons.
            primary_bank_weapons[shipp->weapons.current_primary_bank]].b_info.
      ...
      if (aip->artillery_lock_time >= 2.0f)
        {
          ...
          ssm_create(&Objects[aip->artillery_objnum], &cinfo->hit_point_world,
                     0, NULL, shipp->team);
                                         ...
                                         float delta_box)
{
  ...
  return ((ship_pt.xyz.x > pm->mins.xyz.x - delta_box)
