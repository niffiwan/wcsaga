...
 void stars_load_debris_vclips(debris_vclip * vclips) 
{
    ...
    
    {
        ...
        bm_load_animation(vclips[i].name, &vclips[i].nframes, NULL, NULL, 1);
...
void stars_project_2d_onto_sphere(vec3d * pnt, float rho, float phi,
                                  float theta);
static void starfield_create_perspective_bitmap_buffer(angles * a,
                                                        float scale_x,
                                                        float scale_y,
                                                        int div_x, int div_y,
                                                        uint tmap_flags,
                                                        ...
                                                        index_buffer) 
{
  ...
   vec3d s_points[MAX_PERSPECTIVE_DIVISIONS + 1][MAX_PERSPECTIVE_DIVISIONS +
                                                   1];
  vec3d t_points[MAX_PERSPECTIVE_DIVISIONS + 1][MAX_PERSPECTIVE_DIVISIONS +
                                                 1];
    ...
    // cap division values
    div_x =
    div_x > MAX_PERSPECTIVE_DIVISIONS ? MAX_PERSPECTIVE_DIVISIONS : div_x;
  ...
  float b_save = a->b;
    ...
    
    {
        ...
        
        {
            ...
            vm_vec_rotate(&t_points[idx][s_idx], &s_points[idx][s_idx],
                          &m_bank);
    ...
    
    {
        ...
        
        {
          ...
          perspective_bitmap_list.vert[perspective_bitmap_list.n_verts++] =
            v;
...
 static void starfield_update_index_buffers(int index, int nverts) 
{
                  ...
                  || (nverts !=
                      (Starfield_bitmap_instances[index].n_prim * 3)));
    ...
    {
      Starfield_bitmap_instances[index].buffer =
        (ushort *) vm_malloc(nverts * sizeof(ushort));
...
static void starfield_generate_bitmap_instance_buffers() 
{
    ...
    
    {
        ...
        Starfield_bitmap_instances[idx].div_y * 6;
    ...
    
    {
        ...
        {
          starfield_create_perspective_bitmap_buffer
            (&Starfield_bitmap_instances[idx].ang,
  ...
   perspective_bitmap_buffer =
    gr_make_buffer(&perspective_bitmap_list,
                   VERTEX_FLAG_POSITION | VERTEX_FLAG_UV1);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                
                {
                             ...
                             "Starfield bitmap '%s' listed more than once!!  Only using the first entry!",
                             sbm.filename);
                ...
                
                {
                           ...
                           (rc) ? "xparent" : "non-xparent",
                           (rc) ? "xparent" : "non-xparent", sbm.filename);
        ...
        
        {
            ...
            
            {
              ...
              stuff_string(sbm.flare_bitmaps[0].filename, F_NAME,
                            MAX_FILENAME_LEN);
                ...
                
                {
                    ...
                    
                    {
                      ...
                      stuff_string(sbm.flare_bitmaps[idx].filename, F_NAME,
                                    MAX_FILENAME_LEN);
            ...
            
            {
                         ...
                         "Sun bitmap '%s' listed more than once!!  Only using the first entry!",
                         sbm.filename);
        ...
        
        {
            ...
            
            {
                       ...
                       "Could not load normal motion debris '%s'; maximum of %d exceeded.",
        ...
        
        {
            ...
            
            {
                       ...
                       "Could not load nebula motion debris '%s'; maximum of %d exceeded.",
                       name, MAX_DEBRIS_VCLIPS);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                bm_load_animation(sb->filename, &sb->n_frames, &sb->fps, NULL,
                                  1);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                bm_load_animation(sb->glow_filename, &sb->glow_n_frames,
                                  &sb->glow_fps, NULL, 1);
...
void stars_post_level_init() 
{
  ...
   stars_load_debris(((The_mission.flags & MISSION_FLAG_FULLNEB)
                        || Nebula_sexp_used));
    ...
    
    {
      ...
       
      {
        ...
        gr_init_alphacolor(&Stars[i].col, red, green, blue, alpha,
                            AC_TYPE_BLEND);
  ...
  int num_suns = (int) Suns.size();
    ...
    
    {
        ...
        if (!Rendering_to_env)
                               ...
                               bm->spec_r, bm->spec_g, bm->spec_b, true);
        ...
        {
          gr_set_bitmap(bm->bitmap_id +
                         ((timestamp() / (int) (bm->fps) % bm->n_frames)),
                         GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL, 0.999f);
        ...
        
        {
             ...
             (&sun_vex, 0, 0.05f * Suns[idx].scale_x * local_scale,
              TMAP_FLAG_TEXTURED))
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
              g3_draw_bitmap(&flare_vex, 0, 0.05f * bm->flare_infos[i].scale,
                              TMAP_FLAG_TEXTURED);
...
void stars_draw_sun_glow(int sun_n) 
{
    ...
    {
      gr_set_bitmap(bm->glow_bitmap +
                     ((timestamp() / (int) (bm->glow_fps) %
                       bm->glow_n_frames)), GR_ALPHABLEND_FILTER,
  ...
  int saved_zbuffer_mode = gr_zbuffer_get();
    ...
    
    {
        ...
        
        {
            ...
            {
              g3_draw_perspective_bitmap(&Starfield_bitmap_instances[idx].
                                          ang,
                                          Starfield_bitmap_instances[idx].
                                          scale_x,
                                          Starfield_bitmap_instances[idx].
                                          scale_y,
                                          Starfield_bitmap_instances[idx].
            ...
            {
              gr_render_buffer(0, Starfield_bitmap_instances[idx].n_prim,
                                Starfield_bitmap_instances[idx].buffer);
...
void subspace_render() 
{
  ...
  0.0f, 0.0f, 0.0f};
    ...
    gr_set_texture_panning(Interp_subspace_offset_v,
                            Interp_subspace_offset_u, true);
...
 class star_point_list 
{
  ...
  colored_vector * get_buffer()
  {
    return &point_list->p1;
  };
...
 void stars_page_in() 
{
        ...
        
        {
            ...
            
            {
                ...
                
                {
                    ...
                    bm_load_animation(sb->filename, &sb->n_frames, &sb->fps,
                                      NULL, 1);
    ...
    {
      bm_page_in_xparent_texture(Debris_vclips[idx].bm,
                                  Debris_vclips[idx].nframes);
...
int stars_add_sun_entry(starfield_list_entry * sun) 
{
    ...
    
    {
               ...
               "Trying to add a sun that does not exist in stars.tbl!");
    ...
    
    {
        ...
        {
          Sun_bitmaps[idx].bitmap_id =
            bm_load_animation(Sun_bitmaps[idx].filename,
                              &Sun_bitmaps[idx].n_frames,
                              &Sun_bitmaps[idx].fps, NULL, 1);
        ...
        
        {
            ...
            
            {
                ...
                bm_load_animation(Sun_bitmaps[idx].glow_filename,
                                  &Sun_bitmaps[idx].glow_n_frames,
                                  &Sun_bitmaps[idx].glow_fps, NULL, 1);
        ...
        
        {
            ...
            
            {
                ...
                {
                  Sun_bitmaps[idx].flare_bitmaps[i].bitmap_id =
                    bm_load(Sun_bitmaps[idx].flare_bitmaps[i].filename);
...
int stars_add_bitmap_entry(starfield_list_entry * sle) 
{
    ...
    
    {
        ...
        
        {
            ...
            bm_load_animation(Starfield_bitmaps[idx].filename,
                              &Starfield_bitmaps[idx].n_frames,
                              &Starfield_bitmaps[idx].fps, NULL, 1);
  ...
  starfield_bitmap * stars_get_bitmap_entry(int index, bool sun) 
{
    ...
    (sun) ? (int) Suns.size() : (int) Starfield_bitmap_instances.size();
...
const char *stars_get_name_FRED(int index, bool sun) 
{
    ...
    (sun) ? (int) Sun_bitmaps.size() : (int) Starfield_bitmaps.size();
...
void stars_modify_entry_FRED(int index, const char *name,
                             starfield_list_entry * sbi_new, bool sun) 
{
...
void stars_delete_instance_FRED(int index, bool sun) 
{
    ...
    {
      Starfield_bitmap_instances.erase(Starfield_bitmap_instances.begin() +
                                        index);
...
int stars_get_first_valid_background() 
{
    ...
    
    {
        ...
        if ((total_suns >= (background->suns.size() + 1) / 2)
            && (total_bitmaps >= (background->bitmaps.size() + 1) / 2))
...
void stars_load_background(int background_idx) 
{
    ...
    
    {
        ...
        
        {
               ...
               && !Fred_running)
            Warning(LOCATION, "Failed to add sun '%s' to the mission!",
...
{
  return !(Backgrounds[idx].suns.size() > 0
            || Backgrounds[idx].bitmaps.size() > 0);
