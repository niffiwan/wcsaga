...
 int FS2NetD_ConnectToServer(const char *host, const char *port) 
{
    ...
    
    {
...
#ifdef SCP_UNIX
            my_error = errno;
          ml_printf("FS2NetD ERROR: Couldn't bind socket (\"%s\")!",
                     strerror(my_error));
      ...
      long m_port = strtol(port, (char **) NULL, 10);
               ...
               (mySocket, (sockaddr *) & FS2NetD_addr,
        ...
        
        {
            ...
            
            {
              ...
              int errv = errno;
                ...
                ("FS2NetD ERROR: Couldn't connect to remote system at %s (\"%s\")!",
    ...
    
    {
        ...
        
        {
          ...
          getsockopt(mySocket, SOL_SOCKET, SO_ERROR, (char *) &err_val,
...
 int FS2NetD_GetData(char *buffer, int blen) 
{
  ...
   return recvfrom(mySocket, buffer, blen, flags,
                     (sockaddr *) & FS2NetD_addr, &from_len);
...
 int FS2NetD_SendData(char *buffer, int blen) 
{
  ...
   return sendto(mySocket, buffer, blen, flags, (sockaddr *) & FS2NetD_addr,
                   to_len);
