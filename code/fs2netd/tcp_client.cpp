...
  int FS2NetD_CheckSingleMission(const char *m_name, uint crc32,
                                    bool do_send) 
{
                             ...
                             bool can_create, bool do_send) 
{
    ...
    
    {
      ...
      do
        ...
        while ((rc_total < (uint) buffer_size) && (rc_total <= sizeof(buffer))
        ...
        
        {
            ...
            
            {
                ...
                FS2NetD_GetData(buffer + rc_total, sizeof(buffer) - rc_total);
...
 int FS2NetD_GetBanList(SCP_vector < std::string > &mask_list,
                          bool do_send) 
{
...
}  void FS2NetD_RequestServerList() 
{
  ...
  char buffer[BASE_PACKET_SIZE + sizeof(int) + sizeof(int) + sizeof(int) +
               MAX_PATH];
    ...
    
    {
      ...
      do
        ...
        
        {
            ...
            ("FS2NetD WARNING: Table list contains %i tables, but we only requested %i!  Invalid data!",
...
}  void FS2NetD_CheckDuplicateLogin() 
{
  ...
  char buffer[BASE_PACKET_SIZE + sizeof(int) + sizeof(ubyte) +
