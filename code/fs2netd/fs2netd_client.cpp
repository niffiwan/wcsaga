...
bool fs2netd_check_mission ( char *mission_name )
{
	...
	if ( Is_standalone )
	{
		do { rc = fs2netd_check_mission_do(); }
		while ( !rc );
	}
	else
	{
...
void fs2netd_update_ban_list()
{
	...
	if ( Is_standalone )
	{
		do { rc = fs2netd_update_ban_list_do(); }
		while ( !rc );
	}
	else
	{
...
bool fs2netd_get_valid_missions()
{
	...
	if ( Is_standalone )
	{
		do { rc = fs2netd_get_valid_missions_do(); }
		while ( !rc );
	}
	else
	{
...
int fs2netd_update_valid_tables()
{
	...
	if ( Is_standalone )
	{
		do { rc = fs2netd_update_valid_tables_do(); }
		while ( !rc );
	}
	else
	{
