          ...
          if (!Is_connected)
            {
              if (!Is_standalone)
                {
                  gamesnd_play_iface(SND_GENERAL_FAIL);
                  popup(PF_USE_AFFIRMATIVE_ICON | PF_TITLE_BIG | PF_TITLE_RED,
                        1, POPUP_OK,
                        "ERROR:\nLost connection to the FS2NetD server!");
                }
  ...
  if (Is_standalone)
  ...
  else
    {
      rc = popup_till_condition(fs2netd_send_player_do, XSTR("&Cancel", 779),
                                XSTR("Sending player stats requests ...",
                                     676));
    }
...
static void fs2netd_store_stats_results()
{
  ...
  multi_display_chat_msg(XSTR("<PXO stats store process complete>", 1001), 0,
                         0);
...
void fs2netd_store_stats()
{
  ...
  switch (spd_ret)
    ...
    default:
      multi_display_chat_msg(XSTR("Unknown Stats Store Request Reply", -1), 0,
                             0);
      break;
    }
