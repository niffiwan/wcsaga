...
BOOL SCP_mspdbcs_ResolveSymbol(HANDLE hProcess, UINT_PTR dwAddress,
                                SCP_mspdbcs_SDumpStackSymbolInfo & siSymbol);
LPVOID __stdcall SCP_mspdbcs_FunctionTableAccess(HANDLE hProcess,
                                                  DWORD64 dwPCAddress);
DWORD64 __stdcall SCP_mspdbcs_GetModuleBase(HANDLE hProcess,
  ...
  
{
  ...
   __try 
  {
    ...
    {
      CHAR rgchSymbol[sizeof(IMAGEHLP_SYMBOL64) +
                       SCP_MSPDBCS_MAX_SYMBOL_LENGTH];
    ...
    sym;
...
#endif  /*  */
      {
               ...
               (sym.sym.Name, szUndec, sizeof(szUndec) / sizeof(szUndec[0]),
          ...
          if (SymUnDName64
              (&sym.sym, szUndec, sizeof(szUndec) / sizeof(szUndec[0])))
          ...
          {
            sprintf_s(szWithOffset, SCP_MSPDBCS_MAX_SYMBOL_LENGTH,
                                               ...
                                               DWORD64 returnAddress) 
{
    ...
    
    {
        ...
        
        {
            ...
            GetModuleFileNameA((HINSTANCE) memoryBasicInfo.AllocationBase,
                               szFile, MAX_PATH);
...
#else   /*  */
                         NULL,
                         (DWORD) (DWORD_PTR) memoryBasicInfo.AllocationBase,
                         0);
...
#error UNKNOWN ARCHITECTURE
  ...
  SCP_vector < void *>addresses;
    ...
    
    {
           ...
           (dwMachType, pdsti->hProcess, pdsti->hThread, &stackFrame,
            &context, NULL, SCP_mspdbcs_FunctionTableAccess,
        ...
        
        {
            ...
            addresses.push_back((void *) (DWORD_PTR) stackFrame.AddrPC.
                                 Offset);
          
          else
            pdsti->pIDS->OnEntry((void *) (DWORD_PTR) stackFrame.AddrPC.
                                  Offset, NULL, NULL);
  ...
  HRESULT SCP_DumpStack(SCP_IDumpHandler * pIDH) 
{
    ...
    CreateThread(NULL, 0, SCP_mspdbcs_DumpStackThread, &info, 0, &dwID);
