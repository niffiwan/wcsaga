...
template < typename T > class SCP_vm_allocator
{
public:
  ...
  template < class T2 > SCP_vm_allocator(const SCP_vm_allocator < T2 > &)
  {
  }
};

template < typename T > class SCP_vector:public std::vector < T,
  SCP_vm_allocator < T > >
{
};

typedef std::basic_string < char, std::char_traits < char >,
  SCP_vm_allocator < char >>SCP_string;

typedef std::basic_stringstream < char, std::char_traits < char >,
  SCP_vm_allocator < char >>SCP_stringstream;

template < typename T, typename U > class SCP_map:public std::map < T, U, std::less < T >,
  SCP_vm_allocator < std::pair < const T,
  U > >>
{
};

template < typename T, typename U > class SCP_multimap:public std::multimap < T, U, std::less < T >,
  SCP_vm_allocator < std::pair < const T,
  U > >>
{
};

template < typename T > class SCP_queue:public std::queue < T, std::deque < T,
  SCP_vm_allocator < T > > >
{
};

template < class T1, class T2 >
  bool operator==(const SCP_vm_allocator < T1 > &,
                  const SCP_vm_allocator < T2 > &) throw()
{
  return true;
}
