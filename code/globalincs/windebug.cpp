...
}   void DumpBuffer::SetWindowText(HWND hWnd) const const 
{
...
class SCP_DebugCallStack:public SCP_IDumpHandler 
{
  ...
  {
  }  virtual void OnEntry(void *address, const char *module,
                             const char *symbol) 
  {
  ...
  }  std::string DumpToString() 
  {
      ...
      
      {
          ...
          m_stackFrames[i].module + "! " + m_stackFrames[i].symbol + "\n";
  ...
  class PE_Debug 
{
  ...
  int DumpDebugInfo(DumpBuffer & dumpBuffer, const BYTE * caller,
                     HINSTANCE hInstance);
  void Display();
private:
    // Report data
    enum
  { MAX_MODULENAME_LEN = 512, VA_MAX_FILENAME_LEN = 256 };
...
  void PE_Debug::DumpLineNumber(DumpBuffer & dumpBuffer,
                                   DWORD relativeAddress) 
{
  PIMAGE_LINENUMBER line =
    BasedPtr(PIMAGE_LINENUMBER, COFFDebugInfo,
...
#endif  /*  */
 void PE_Debug::DumpSymbolInfo(DumpBuffer & dumpBuffer,
                                DWORD relativeAddress) 
{
      ...
      if (isFunction
        ...
        
        {
            ...
            
            {
              ...
               Add_Symbol(currentSym->SectionNumber, currentSym->Value,
                            symName, pretty_module);
...
  PIMAGE_COFF_SYMBOLS_HEADER PE_Debug::GetDebugHeader() 
{
    ...
    
    {
        ...
        (PIMAGE_DEBUG_DIRECTORY) (debugHeader->PointerToRawData +
                                  (DWORD) fileBase);
      ...
      // Microsoft debug directory is in the .rdata section
    {
        ...
        BasedPtr(PIMAGE_DEBUG_DIRECTORY, fileBase,
      ...
      
      {
          ...
          
          {
              ...
              if (NT_Header->FileHeader.PointerToSymbolTable)
                ...
                BasedPtr(PIMAGE_SYMBOL, fileBase,
...
} }   void PE_Debug::MapFileInMemory(const char *module) 
{
    ...
    CreateFile(module, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
    ...
    
    {
        ...
        CreateFileMapping(hFile, NULL, PAGE_READONLY, 0, 0, NULL);
          ...
          (PIMAGE_DOS_HEADER) MapViewOfFile(hFileMapping, FILE_MAP_READ, 0, 0,
                                            0);
...
  int PE_Debug::DumpDebugInfo(DumpBuffer & dumpBuffer, const BYTE * caller,
                                 HINSTANCE hInstance) 
{
...
  void _cdecl WinAssert(char *text, char *filename, int linenum) 
{
    ...
    MessageBox(NULL, assertString.c_str(), "Assertion Failed!",
               MB_OKCANCEL | flags);
    ...
    MessageBox(NULL, dumpBuffer.buffer, "Assertion Failed!",
               MB_OKCANCEL | flags);
  
#else   /*  */
    val =
    MessageBox(NULL, AssertText1, "Assertion Failed!", MB_OKCANCEL | flags);
                         ...
                         const char *format, ...) 
{
    ...
    // this stuff migt be really useful for solving bug reports and user errors. We should output it!
    mprintf(("ASSERTION: \"%s\" at %s:%d\n %s\n", text,
             strrchr(filename, '\\') + 1, linenum, AssertText2));
...
void LuaError(struct lua_State *L, char *format, ...) 
{
    ...
    MessageBox(NULL, dumpBuffer.buffer, "Error!", flags | MB_YESNOCANCEL);
                     ...
                     ...) 
{
    ...
    MessageBox(NULL, assertString.c_str(), "Error!",
               flags | MB_DEFBUTTON2 | MB_OKCANCEL);
    ...
    MessageBox(NULL, dumpBuffer.buffer, "Error!",
               flags | MB_DEFBUTTON2 | MB_OKCANCEL);
    ...
    MessageBox(NULL, AssertText2, "Error!",
               flags | MB_DEFBUTTON2 | MB_OKCANCEL);
...
 void _cdecl Warning(char *filename, int line, const char *format, ...) 
{
    ...
    MessageBox(NULL, assertString.c_str(), "Warning!",
               MB_YESNOCANCEL | MB_DEFBUTTON2 | MB_ICONWARNING | flags);
    ...
    MessageBox((HWND) os_get_window(), dumpBuffer.buffer, "Warning!",
               MB_YESNOCANCEL | MB_DEFBUTTON2 | MB_ICONWARNING | flags);
    ...
    MessageBox((HWND) os_get_window(), AssertText2, "Warning!",
               MB_YESNOCANCEL | MB_DEFBUTTON2 | MB_ICONWARNING | flags);
...
{
  insertion_sort(mem_block_list, MAX_MEM_MODULES, sizeof(MemBlockInfo),
                  memblockinfo_sort_compare);
...
 void memblockinfo_output_memleak() 
{
    ...
    
    {
        ...
        
        {
                 ...
                 mem_ptr_list[f].filename, mem_ptr_list[f].line,
                 mem_ptr_list[f].size);
