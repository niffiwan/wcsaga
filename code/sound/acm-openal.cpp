...
230, 230 
    ...
    (header->iDelta * AdaptionTable[nib]) / FIXED_POINT_ADAPTION_BASE;
...
 static int decode_adpcm_sample_frame(HMMIO rw, adpcm_fmt_t * fmt) 
{
    ...
    
    {
        ...
        ((fmt->header[i].iSamp1 * iCoef1) +
         (fmt->header[i].iSamp2 * iCoef2)) / FIXED_POINT_COEF_BASE;
...
 static uint read_sample_fmt_adpcm(ubyte * data, HMMIO rw,
                                     adpcm_fmt_t * fmt) 
{
                             ...
                             unsigned short dest_bps) 
{
    ...
    fmt->adpcm.aCoef =
    (ADPCMCOEFSET *) vm_malloc(sizeof(ADPCMCOEFSET) * fmt->adpcm.wNumCoef);
    ...
    fmt->header =
    (ADPCMBLOCKHEADER *) vm_malloc(sizeof(ADPCMBLOCKHEADER) *
    ...
    fmt->adpcm.wav.wBitsPerSample =
      INTEL_SHORT(fmt->adpcm.wav.wBitsPerSample);
...
 int ACM_stream_open(WAVEFORMATEX * pwfxSrc, WAVEFORMATEX * pwfxDest,
                       void **stream, int dest_bps) 
{
