...
{ DSPROPERTY_VMANAGER_STATE_PLAYING3DHW =
...
//
//
int ds3d_update_buffer(int channel, float min, float max, vec3d * pos,
                       vec3d * vel) 
{
    ...
    OpenAL_ErrorPrint(alSourcef
                      (Channels[channel].source_id, AL_REFERENCE_DISTANCE,
                       min));
   
    // set the max distance
    //  OpenAL_ErrorPrint( alSourcef(Channels[channel].source_id, AL_MAX_DISTANCE, max) );
    OpenAL_ErrorPrint(alSourcef
                      (Channels[channel].source_id, AL_MAX_DISTANCE,
                       40000.0f));
   
    // set rolloff factor
    OpenAL_ErrorPrint(alSourcef
                      (Channels[channel].source_id, AL_ROLLOFF_FACTOR, 1.0f));
    ...
    
    {
      ...
      OpenAL_ErrorPrint(alSourcefv
                         (Channels[channel].source_id, AL_POSITION, alpos));
    ...
    
    {
      ...
      {
      vel->xyz.x, vel->xyz.y, vel->xyz.z};
      OpenAL_ErrorPrint(alSourcefv
                         (Channels[channel].source_id, AL_VELOCITY, alvel));
    ...
    
    {
        ...
        pds3db->SetPosition(pos->xyz.x, pos->xyz.y, pos->xyz.z,
                            DS3D_DEFERRED);
    ...
    
    {
        ...
        pds3db->SetVelocity(vel->xyz.x, vel->xyz.y, vel->xyz.z,
                            DS3D_DEFERRED);
...
int ds3d_update_listener(vec3d * pos, vec3d * vel, matrix * orient) 
{
    ...
    {
      OpenAL_ErrorPrint(alListener3f
                         (AL_POSITION, pos->xyz.x, pos->xyz.y, pos->xyz.z));
    ...
    
    {
                         ...
                         (AL_VELOCITY, vel->xyz.x, vel->xyz.y, vel->xyz.z));
    ...
    
    {
        ...
        ALfloat list_orien[] =
      {
      orient->vec.fvec.xyz.x, orient->vec.fvec.xyz.y,
          orient->vec.fvec.xyz.z, orient->vec.uvec.xyz.x,
          orient->vec.uvec.xyz.y, orient->vec.uvec.xyz.z };
    ...
    
    {
        ...
        pDS3D_listener->SetPosition(pos->xyz.x, pos->xyz.y, pos->xyz.z,
                                    DS3D_DEFERRED);
    ...
    
    {
        ...
        pDS3D_listener->SetVelocity(vel->xyz.x, vel->xyz.y, vel->xyz.z,
                                    DS3D_DEFERRED);
    ...
    
    {
        ...
        pDS3D_listener->SetOrientation(orient->vec.fvec.xyz.x,
    ...
    
    {
                ...
                "Error in pDS3D_listener->CommitDeferredSettings(): %s\n",
                get_DSERR_text(hr)));
...
int ds3d_init_listener() 
{
    ...
    pPrimaryBuffer->QueryInterface(IID_IDirectSound3DListener,
                                   (void **) &pDS3D_listener);
  if (hr != DS_OK)
    
    {
      nprintf(("Sound",
                "SOUND => Fatal error calling pPrimaryBuffer->QueryInterface(): %s\n",
                get_DSERR_text(hr)));
...
int ds3d_init(int voice_manager_required) 
{
    ...
    
    {
        ...
        pset->QuerySupport(DSPROPSETID_VoiceManager_Def,
                           DSPROPERTY_VMANAGER_MODE, &driver_support);
      ...
       if ((driver_support & KSPROPERTY_SUPPORT_SET |
        ...
        pset->Set(DSPROPSETID_VoiceManager_Def, DSPROPERTY_VMANAGER_MODE,
                  NULL, 0, &vmode, sizeof(float));
