...
bool VOICEREC_init(HWND hWnd, int event_id, int grammar_id,
  ...
  while (1)
    {
      ...
      // recognition.
      hr =
        p_recogContext->SetInterest(SPFEI(SPEI_RECOGNITION),
      ...
      hr =
        p_grammarObject->LoadCmdFromResource(NULL,
                                             MAKEINTRESOURCEW
                                             (command_resource), L"SRGRAMMAR",
  ...
  if (SUCCEEDED(pPhrase->GetPhrase(&pElements)))
    {
      if (DEBUG_ON)
        {
          WCHAR *pwszText;
          pPhrase->GetText(SP_GETWHOLEPHRASE, SP_GETWHOLEPHRASE, TRUE,
                           &pwszText, NULL);
            ...
            if (Player->flags & PLAYER_FLAGS_MSG_MODE)
              {
                switch (action)
                  ...
                  case VID_IgnoreTarget:
                    ...
                    break;
                  case VID_FormWing:
                    Msg_shortcut_command = FORMATION_ITEM;
                    break;
                  case VID_CoverMe:
                    Msg_shortcut_command = COVER_ME_ITEM;
                    break;
                  case VID_EngageEnemy:
                    Msg_shortcut_command = ENGAGE_ENEMY_ITEM;
                    break;
                  case VID_Depart:
                    Msg_shortcut_command = DEPART_ITEM;
                    break;
                  default:
                ...
                if (Msg_instance == MESSAGE_ALL_FIGHTERS
                ...
                else if (Squad_msg_mode == SM_MODE_SHIP_COMMAND)
                  {
                    ...
                    hud_squadmsg_send_ship_command(Msg_instance,
                                                   Msg_shortcut_command, 1);
                  }
                else if (Squad_msg_mode == SM_MODE_WING_COMMAND)
                  {
                    //  nprintf(("warning", "VOICER msg wing %d\n", Msg_instance));
                    hud_squadmsg_send_wing_command(Msg_instance,
                                                   Msg_shortcut_command, 1);
                  }
