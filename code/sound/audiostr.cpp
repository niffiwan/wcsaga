    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                DSBCAPS_STATIC | DSBCAPS_CTRLPAN | DSBCAPS_CTRLVOLUME;
               hr =
                (m_pass->GetPDS())->CreateSoundBuffer(&m_dsbd, &m_pdsb, NULL);
                ...
                
                {
                    ...
                    nprintf(("Sound",
                             "SOUND => Error, unable to create DirectSound buffer\n\r"));
                  if (hr == DSERR_BADFORMAT)
                    
                    {
                      nprintf(("Sound",
                                "SOUND => Bad format (probably ADPCM)\n\r"));
                                  ...
                                  int service) 
{
    ...
    hr =
    m_pdsb->Lock(m_cbBufOffset, size, (void **) (&lpbuf1), &dwsize1,
    ...
    
    {
        ...
        m_pwavefile->Read(uncompressed_wave_data, dwsize1 + dwsize2, service);
        ...
        
        {
            ...
            
            {
              ...
              memcpy(lpbuf2, uncompressed_wave_data + dwsize1,
                      num_bytes_read - dwsize1);
        ...
        m_cbBufOffset =
        (m_cbBufOffset + dwbyteswritten1 + dwbyteswritten2) % m_cbBufSize;
    ...
    
    {
        ...
        nprintf(("SOUND",
                 "SOUND ==> Error, unable to lock sound buffer in AudioStr\n"));
...
void AudioStream::Play(long volume, int looping) 
{
    ...
    
    {
        ...
        
        {
            ...
            m_timer.constructor();
           m_timer.Create(m_nBufService, m_nBufService, DWORD(this),
                            TimerCallback);
        ...
        
        {
            ...
            
            {
                        ...
                        "Sound => Play failed with return value %s\n",
                        get_DSERR_text(hr)));
                     ...
                     TIMERCALLBACK pfnCallback) 
{
          ...
          timeSetEvent(m_nPeriod, m_nRes, TimeProc, (DWORD) this,
                       TIME_PERIODIC)) == 0)
  ...
  BOOL WaveFile::Open(LPSTR pszFilename) 
{
  ...
  PCMWAVEFORMAT pcmwf;
         ...
         (pszFilename, CF_TYPE_ANY, sizeof(fullpath) - 1, fullpath, &FileSize,
          &FileOffset))
    ...
    if ((rc =
         ov_open_callbacks(&m_snd_info, &m_snd_info.vorbis_file, NULL, 0,
                           mmio_callbacks)) == 0)
    ...
    
    {
        ...
        {
          if (mmioRead(m_snd_info.cfp, (char *) &tag, sizeof(uint)) !=
               sizeof(uint))
            break;
           if (mmioRead(m_snd_info.cfp, (char *) &size, sizeof(uint)) !=
                 sizeof(uint))
            ...
            
            {
              ...
              mmioRead(m_snd_info.cfp, (char *) &pcmwf,
...
int WaveFile::Read(BYTE * pbDest, UINT cbSize, int service) 
{
    ...
    -1, byte_order = 0;
  unsigned int src_bytes_used, convert_len, num_bytes_desired =
    0, num_bytes_read;
    ...
    
    {
        ...
        
        {
            ...
            ACM_convert((void *) m_hStream, dest_buf, num_bytes_read,
...
 void audiostream_init() 
{
    ...
    
    {
        ...
        (unsigned char *) vm_malloc(COMPRESSED_BUFFER_SIZE);
