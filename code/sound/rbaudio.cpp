...
 UINT MakeCDDiscID(int tracks, unsigned long msflen,
                     unsigned long msftrack1);
...
  void RBAInit(void) 
{
    ...
    mciSendCommand(NULL, MCI_OPEN, MCI_OPEN_TYPE | MCI_OPEN_SHAREABLE,
                   (DWORD) (LPVOID) & mciOpenParms);
  if (retval)
    
    {
      if (retval == MCIERR_MUST_USE_SHAREABLE)
        RBCDROM_State = -1;
    ...
    
    {
        ...
        mciSendCommand(CDDeviceID, MCI_SET, MCI_SET_TIME_FORMAT,
...
  void RBARegisterCD(void) 
{
    ...
    
    {
        ...
        mciSendCommand(CDDeviceID, MCI_STATUS, MCI_STATUS_ITEM | MCI_TRACK,
                       (DWORD) (LPVOID) & mciStatusParms);
...
  int get_track_time(int first, int last) 
{
    ...
    msf_sub(msf_add(CDTrackInfo[last - 1].msf, CDTrackInfo[last - 1].length),
            CDTrackInfo[first - 1].msf);
...
  int RBAPlayTrack(int track) 
{
    ...
    //  Play the track
    min =
    min + MCI_MSF_MINUTE(CDTrackInfo[track - 1].msf) +
    MCI_MSF_MINUTE(CDTrackInfo[track - 1].length);
  mciPlayParms.dwFrom = CDTrackInfo[track - 1].msf;    // MCI_MAKE_TMSF(track, 0, 0, 0);
  mciPlayParms.dwTo =
    msf_add(CDTrackInfo[track - 1].msf, CDTrackInfo[track - 1].length);
  retval =
    mciSendCommand(CDDeviceID, MCI_PLAY, MCI_FROM | MCI_TO,
...
  int RBAPlayTracks(int start, int end) 
{
  ...
  mciPlayParms.dwTo =
    msf_add(CDTrackInfo[end - 1].msf, CDTrackInfo[end - 1].length);
...
  int RBAResume() 
{
  ...
  mciPlayParms.dwTo =
    msf_add(CDTrackInfo[Playback_last_track - 1].msf,
...
}   long RBAGetHeadLoc(int *min, int *sec, int *frame) 
{
    ...
    mciSendCommand(CDDeviceID, MCI_STATUS, MCI_STATUS_ITEM,
                   (DWORD) (LPVOID) & mciStatusParms);
...
  int RBAGetTrackNum() 
{
    ...
    f2i(timer_get_fixed_seconds() - Playback_Start_Time + f1_0 / 2);
   for (track = Playback_first_track;
          track <= Playback_last_track && delta_time > 0; track++)
...
  int RBAPeekPlayStatus() 
{
    ...
    return 0;
    if ((timer_get_fixed_seconds() - Playback_Start_Time) >
           Playback_Length)
    return 0;
...
int RBACheckMediaChange() 
{
    ...
    return 0;
   if (CDDiscID != GetCDDiscID() || !CDDiscID)
    return 1;
...
  unsigned long msf_sub(unsigned long msf1, unsigned long msf2) 
{
    ...
    sec2 = sec1 - (int) MCI_MSF_SECOND(msf2);
