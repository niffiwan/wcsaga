...
int dscap_init() 
{
    ...
    (HRESULT(__stdcall *)
     (LPGUID lpGuid, LPDIRECTSOUNDCAPTURE * lplpDSC,
      IUnknown FAR * pUnkOuter)) GetProcAddress(Dscap_dll_handle,
                                                "DirectSoundCaptureCreate");
   if (!pfn_DirectSoundCaptureCreate)
    
    {
      nprintf(("Sound",
                "SOUND ==> Could not get DirectSoundCaptureCreate function pointer\n"));
    ...
    
    {
                ...
                "SOUND ==> Error '%s' initializing DirectSoundCapture.\n",
                        ...
                        int nseconds) 
{
    ...
    
    {
                ...
                "SOUND ==> Error '%s' creating a DirectSoundCapture buffer.\n",
                get_DSERR_text(dsrval)));
...
int dscap_get_raw_data(unsigned char *outbuf, unsigned int max_size) 
{
  ...
  unsigned long capture_offset, read_offset, num_bytes_captured, size1,
    size2;
    ...
    
    {
        ...
        max_size - Dscap_last_capture_offset + read_offset;
    ...
    pDSCB->Lock(Dscap_last_capture_offset, num_bytes_captured, &data1, &size1,
                &data2, &size2, 0);
