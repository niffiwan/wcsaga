...
extern HRESULT(__stdcall * pfn_DirectSoundCaptureCreate) (LPGUID lpGUID,
...
DEFINE_GUID(DSPROPSETID_EAXBUFFER_ReverbProperties,
            0x4a4e6fc0,
            0xc341, 0x11d1, 0xb7, 0x3a, 0x44, 0x45, 0x53, 0x54, 0x00, 0x00);
...
int ds_parse_sound(CFILE * fp, ubyte ** dest, uint * dest_size,
                   WAVEFORMATEX ** header, bool ogg =
                   false, OggVorbis_File * ovf = NULL);
...
int ds_eax_set_all(unsigned long id, float volume, float damping,
                   float decay);
