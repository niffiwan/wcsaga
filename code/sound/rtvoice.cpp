...
// RECORD/ENCODE
/////////////////////////////////////////////////////////////////////////////////////////////////
void CALLBACK TimeProc(unsigned int id, unsigned int msg,
                        unsigned long userdata, unsigned long dw1,
                        unsigned long dw2) 
{
...
int rtvoice_init_recording(int qos) 
{
    ...
    
    {
        ...
        Rtv_formats[Rtv_recording_format].frequency * (RTV_BUFFER_TIME) *
        ...
        
        {
             ...
             (Rtv_formats[Rtv_recording_format].frequency,
              Rtv_formats[Rtv_recording_format].bits_per_sample, 1,
              RTV_BUFFER_TIME))
        ...
        (unsigned char *) vm_malloc(Rtv_capture_raw_buffer_size);
      ...
      Rtv_capture_compressed_buffer =
        (unsigned char *) vm_malloc(Rtv_capture_compressed_buffer_size);
...
#else   /*  */
        Rtv_record_timer_id =
        timeSetEvent(callback_time, callback_time, TimeProc, 0,
                     TIME_PERIODIC);
...
int rtvoice_compress(unsigned char *data_in, int size_in,
                     unsigned char *data_out, int size_out) 
{
    ...
    
    {
        ...
        Encode(data_in, data_out, size_in, size_out, &Rtv_code_info);
       nprintf(("SOUND",
                  "RTVOICE => Sound compressed to %d bytes (%0.2f percent)\n",
...
void rtvoice_get_data(unsigned char **outbuf, int *compressed_size,
                      int *uncompressed_size, double *gain,
                      unsigned char **outbuf_raw, int *outbuf_size_raw) 
{
    ...
    
    {
        ...
        rtvoice_compress(Rtv_capture_raw_buffer, raw_size,
                         Rtv_capture_compressed_buffer,
...
void rtvoice_uncompress(unsigned char *data_in, int size_in, double gain,
                        unsigned char *data_out, int size_out) 
{
...
int rtvoice_init_playback() 
{
    ...
    
    {
        ...
        rtvf->frequency * (RTV_BUFFER_TIME) * fl2i(rtvf->bits_per_sample /
                                                   8.0f);
...
int rtvoice_create_playback_buffer() 
{
  ...
   Rtv_output_buffers[index].ds_handle =
    ds_create_buffer(rtvf->frequency, rtvf->bits_per_sample, 1,
                     RTV_BUFFER_TIME);
...
int rtvoice_play_compressed(int index, unsigned char *data, int size,
                            int uncompressed_size, double gain) 
{
    ...
    {
      rtvoice_uncompress(data, size, gain, Rtv_playback_uncompressed_buffer,
                          uncompressed_size);
    }
   
    // lock the data in
    if (ds_lock_data
        (ds_handle, Rtv_playback_uncompressed_buffer, uncompressed_size))
    ...
    // play the voice
    rval =
    ds_play(ds_handle, -1, -100, DS_MUST_PLAY,
            ds_convert_volume(Master_voice_volume), 0, 0);
