...
  static int dbg_print_ogg_error(const char *filename, int rc) 
{
    ...
    
    {
      ...
      strncpy(err_msg, "Attempted to use a feature that's not supported",
               99);
      ...
      strncpy(err_msg, "Invalid stream section supplied, or corrupt link",
               99);
...
 class Timer 
{
  ...
  bool Create(uint nPeriod, uint nRes, ptr_u dwUser,
...
 class WaveFile 
{
  ...
  uint m_total_uncompressed_bytes_read;
                                ...
                                // number of bytes to actual wave data
...
 class AudioStream 
{
  ...
  void Set_Default_Volume(int converted_volume)
  {
...
// Create
  bool Timer::Create(uint nPeriod, uint nRes, ptr_u dwUser,
                     TIMERCALLBACK pfnCallback) 
{
...
#ifndef SCP_UNIX
    if ((m_nIDTimer =
         timeSetEvent((UINT) m_nPeriod, (UINT) m_nRes, TimeProc, (DWORD) this,
                      TIME_PERIODIC)) == NULL)
...
#else   /*  */
    if ((m_nIDTimer =
         SDL_AddTimer(m_nPeriod, TimeProc, (void *) this)) == NULL)
...
//
#ifndef SCP_UNIX
void CALLBACK Timer::TimeProc(UINT uID, UINT uMsg, DWORD dwUser, DWORD dw1,
                              DWORD dw2) 
#else   /*  */
  ...
  bool WaveFile::Open(char *pszFilename, bool keep_ext) 
{
  ...
  PCMWAVEFORMAT pcmwf;
    ...
    
    {
      ...
       cf_find_file_location(pszFilename, CF_TYPE_ANY, sizeof(fullpath) - 1,
    ...
    
    {
           ...
           (&m_snd_info, &m_snd_info.vorbis_file, NULL, 0,
            mmio_callbacks) == 0)
        
        {
          ...
          m_wfmt.nAvgBytesPerSec =
            m_wfmt.nSamplesPerSec * m_wfmt.nBlockAlign;
            ...
            
            {
                ...
                (m_wfmt.nChannels == 2) ? AL_FORMAT_STEREO8 : AL_FORMAT_MONO8;
              break;
             case 16:
              m_ALformat =
                (m_wfmt.nChannels ==
                 2) ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16;
            ...
            m_data_offset = 0;
          m_nDataSize = m_data_bytes_left =
            ((int) ov_pcm_total(&m_snd_info.vorbis_file, -1) *
             m_wfmt.nBlockAlign);
    ...
    
    {
        ...
        
        {
            ...
            
            {
            case 0x20746d66:  // The 'fmt ' tag
              {
                ...
                audiostr_read_dword(m_snd_info.cfp,
                  ...
                  
                  {
                      ...
                      if (cbExtra != 0)
                                ...
                                ((char *) (m_pwfmt_original) +
                                 sizeof(WAVEFORMATEX)), cbExtra);
        ...
        
        {
            ...
            (m_wfmt.nChannels == 2) ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16;
...
int WaveFile::Read(ubyte * pbDest, uint cbSize, int service) 
{
  ...
  int rc, uncompressed_bytes_written, section, last_section =
    -1, byte_order = 0;
    ...
    
    {
        ...
        
        {
               ...
               (m_pwfmt_original, &m_wfxDest, (void **) &m_hStream,
        ...
        
        {
            ...
            ACM_query_source_size((void *) m_hStream, cbSize);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                ov_read(&m_snd_info.vorbis_file,
                        (char *) dest_buf + actual_read,
                        num_bytes_read - actual_read, byte_order,
                        m_wfmt.wBitsPerSample / 8, 1, &section);
        ...
        
        {
            ...
            mmioRead(m_snd_info.cfp, (char *) dest_buf, num_bytes_read);
    ...
    
    {
        ...
        
        {
            ...
            ACM_convert((void *) m_hStream, (ubyte *) dest_buf,
                        num_bytes_read, (ubyte *) uncompressed_wave_data,
        ...
        
        {
            ...
            mmioSeek(m_snd_info.cfp, src_bytes_used - num_bytes_read,
                     SEEK_CUR);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                m_cbBufSize =
                (m_pwavefile->GetUncompressedAvgDataRate() * m_nBufLength) /
                1000;
                ...
                OpenAL_ErrorCheck(alGenSources(1, &m_source_id),
                                  ...
                                  goto ErrorExit;
              ...
               OpenAL_ErrorCheck(alGenBuffers
                                   (MAX_STREAM_BUFFERS, m_buffer_ids),
                                   {
                                   fRtn = false;
                                   goto ErrorExit;
                                   }
                                   ...
                                   (m_source_id, AL_ROLLOFF_FACTOR, 0));
               OpenAL_ErrorPrint(alSourcei
                                   (m_source_id, AL_SOURCE_RELATIVE,
                                    AL_TRUE));
...
// ServiceBuffer; it's not exposed to users of the AudioStream class.
  bool AudioStream::WriteWaveData(uint size, uint * num_bytes_written,
                                  int service) 
{
    ...
    num_bytes_read =
    m_pwavefile->Read(uncompressed_wave_data, m_cbBufSize, service);
    ...
    
    {
        ...
        ALint p = 0;
                           ...
                           (m_source_id, AL_BUFFERS_PROCESSED, &p));
        ...
        
        {
                           ...
                           (m_buffer_ids[m_play_buffer_id],
                            m_pwavefile->m_ALformat, uncompressed_wave_data,
                            num_bytes_read,
      ...
       OpenAL_ErrorCheck(alSourceQueueBuffers
                           (m_source_id, 1, &m_buffer_ids[m_play_buffer_id]),
                           {
                           fRtn = false;
                           goto ErrorExit;
                           }
  ...
  uint AudioStream::GetMaxWriteSize(void) 
{
  ...
   OpenAL_ErrorCheck(alGetSourcei(m_source_id, AL_BUFFERS_PROCESSED, &n),
                       return 0);
   OpenAL_ErrorCheck(alGetSourcei(m_source_id, AL_BUFFERS_QUEUED, &q),
                       return 0);
  ...
  bool AudioStream::ServiceBuffer(void) 
{
    ...
    
    {
        ...
        
        {
            ...
            m_lCutoffVolume =
            MAX(vol - VOLUME_ATTENUATION_BEFORE_CUTOFF, -10000);
    ...
    
    {
        ...
        
        {
            ...
            OpenAL_ErrorCheck(alGetSourcei
                              (m_source_id, AL_BUFFERS_PROCESSED, &n),
                              m_bReadingDone = true);
...
void AudioStream::Play(int volume, int looping) 
{
    ...
    
    {
        ...
        m_timer.constructor();
       m_timer.Create(m_nBufService, m_nBufService, ptr_u(this),
                        TimerCallback);
...
 void AudioStream::Set_Sample_Cutoff(unsigned int sample_cutoff) 
{
    ...
    ((sample_cutoff * m_pwavefile->m_wfmt.wBitsPerSample) / 8);
...
 uint AudioStream::Get_Samples_Committed(void) 
{
  ...
   return ((m_pwavefile->m_total_uncompressed_bytes_read * 8) /
             m_pwavefile->m_wfmt.wBitsPerSample);
...
  void audiostream_init() 
{
    ...
    {
      Compressed_service_buffer =
        (ubyte *) vm_malloc(COMPRESSED_BUFFER_SIZE);
