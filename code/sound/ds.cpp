...
int ds_load_buffer(int* sid, int* hid, int* final_size, void* header, sound_info* si, int flags)
{
	...
	*final_size = size;
	...
	case OGG_FORMAT_VORBIS:
		...
		while (src_bytes_used < si->size)
		{
			rc = ov_read(&si->ogg_info, (char*)convert_buffer + src_bytes_used, si->size - src_bytes_used, byte_order,
				si->bits / 8, 1, &section);
	...
	*sid = ds_get_sid();
	...
	if (DSReturn == DS_OK && ds_software_buffers[*sid].pdsb != NULL)
	{
		...
		if ((ds_software_buffers[*sid].pdsb)->Lock(0,
			final_sound_size, (void**)(&pData), &DataSize, (void**)(&pData2), &DataSize2, 0) == DS_OK)
		{
...
{
	pfn_DirectSoundCreate = (HRESULT(__stdcall *)(LPGUID lpGuid, LPDIRECTSOUND *ppDS, IUnknown FAR *pUnkOuter))
	GetProcAddress(Ds_dll_handle, "DirectSoundCreate");
	pfn_DirectSoundCaptureCreate = (HRESULT(__stdcall *)(LPGUID lpGuid, LPDIRECTSOUNDCAPTURE *lplpDSC, IUnknown FAR *pUnkOuter))
