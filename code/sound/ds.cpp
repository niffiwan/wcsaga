  ...
  for (i = 0; i < MAX_CHANNELS; i++)
    {
      ...
      OpenAL_ErrorCheck(alGetSourcei
                        (chp->source_id, AL_SOURCE_STATE, &status), continue);
            ...
            int looping, bool is_voice_msg)
{
  ...
  OpenAL_ErrorPrint(alSource3f
                    (Channels[ch_idx].source_id, AL_VELOCITY, 0.0, 0.0, 0.0));
  ...
  if (state == AL_PLAYING)
    {
      ...
      OpenAL_ErrorPrint(alSource3f
                        (Channels[channel].source_id, AL_POSITION, alpan, 0.0,
                         1.0));
    }
#else
  HRESULT hr;
...
int ds_get_pitch(int channel)
{
  ...
  OpenAL_ErrorCheck(alGetSourcei
                    (Channels[channel].source_id, AL_SOURCE_STATE, &status),
                    return -1);
