...
//              0       => init failed
//
int snd_init(int use_a3d, int use_eax, unsigned int sample_rate,
  ...
  while (1)
    {
      ...
      if (rval == -2)
        ...
        {
          nprintf(("Sound",
                   "SOUND ==> Error initializing audio device, trying again in 1 second.\n"));
          Sleep(1000);
        }
      else
        {
...
int snd_load(game_snd * gs, int allow_hardware_load)
{
  ...
  if (ds_parse_sound
      (fp, &si->data, &si->size, &header, (rc == 0), &si->ogg_info) == -1)
    {
  ...
  snd->duration =
    fl2i(1000.0f *
         ((si->size / (si->bits / 8.0f)) / si->sample_rate / si->n_channels));
  ...
  rc =
    ds_load_buffer(&snd->sid, &snd->hid, &snd->uncompressed_size, header, si,
                   type);
...
MONITOR(NumSoundsStarted) MONITOR(NumSoundsLoaded)
// ---------------------------------------------------------------------------------------
//  snd_play()
//
...
//                  n       =>      handle for instance of sound
//
     int snd_play(game_snd * gs, float pan, float vol_scale, int priority,
                  bool is_voice_msg)
{
  float volume;
  sound *snd;
  ...
  if (volume > MIN_SOUND_VOLUME)
    {
      handle =
        ds_play(snd->sid, snd->hid, gs->id_sig, ds_priority(priority),
...
MONITOR(Num3DSoundsStarted) MONITOR(Num3DSoundsLoaded)
// ---------------------------------------------------------------------------------------
// snd_play_3d()
//
//  NOTE: vol_scale parameter is the multiplicative scaling applied to the default volume
...
//                  n       =>      handle for instance of sound
//
     int snd_play_3d(game_snd * gs, vec3d * source_pos, vec3d * listen_pos,
                     float radius, vec3d * source_vel, int looping,
                     float vol_scale, int priority, vec3d * sound_fvec,
                     float range_factor, int force)
{
  ...
  if (play_using_ds3d)
    {
      ...
      handle =
        ds3d_play(snd->sid, snd->hid, gs->id_sig, source_pos, source_vel,
  ...
  else
    {
      ...
      if (looping)
        {
          handle =
            snd_play_looping(gs, pan, -1, -1, volume / gs->default_volume,
                             priority, force);
        }
      else
        {
...
void snd_update_3d_pos(int soundnum, game_snd * gs, vec3d * new_pos,
                       float radius, float range_factor)
{
  float vol, pan;
...
//
int snd_get_3d_vol_and_pan(game_snd * gs, vec3d * pos, float *vol, float *pan,
                           float radius, float range_factor)
{
  vec3d vector_to_sound;
  ...
  *pan = 0.0f;
  ...
  if (distance <= min_range)
    ...
    {
      *vol =
        max_volume - (distance - min_range) * max_volume / (max_range -
                                                            min_range);
    }
...
//                  n       =>      handle for instance of sound
//
int snd_play_looping(game_snd * gs, float pan, int start_loop, int stop_loop,
