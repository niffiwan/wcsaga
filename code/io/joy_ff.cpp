...
void init_periodic_effect_struct(di_periodic_effect_struct * effect, int type,
                                 int dur, int per, int ang = 0, int mag =
  ...
  if (ff_enabled)
    {
      ...
      hr =
        SWFF_OpenDefaultFFJoystick((HWND) os_get_window(), &pDi, &pDiDevice);
      if (FAILED(hr))
        {
...
int joy_ff_create_std_periodic(LPDIRECTINPUTEFFECT * eff, int type, int dur,
                               int per, int ang = 0, int mag =
                               10000, int att = 0, int fade = 0)
{
  joy_ff_handle_error(SWFF_CreatePeriodicEffect
                      (pDiDevice, eff, type, dur, per, ang, mag, 0, att, 0,
                       fade, 0, -1));
  if (!*eff)
    return -1;
...
int joy_ff_create_effects()
{
  joy_ff_handle_error(SWFF_CreateConstantForceEffect(pDiDevice, &pHitEffect1, 300000,   // Duration
                                                     0, // Angle
                                                     10000,     // Magnitude
                                                     0, // Attack time
                                                     10000,     // Attack level
  ...
  if (pHitEffect1)
    ...
    {
      // bail out early if we can't even load this (because rest probably won't either and failing is slow.
      nprintf(("Joystick", "FF: Hit effect 1 failed to load\n"));
      return -1;
    }

  joy_ff_create_std_periodic(&pHitEffect2, SINE, 300000,        // Duration
                             100000,    // Period
                             9000,      // Angle
                             10000,     // Magnitude
                             100000,    // Attack time
  ...
  joy_ff_handle_error(SWFF_CreatePeriodicEffect(pDiDevice, &pShootEffect, SAWTOOTH_DOWN, 160000,        // Duration
                                                20000,  // Period
                                                0,      // Angle
                                                10000,  // Magnitude
                                                0,
  ...
  joy_ff_handle_error(SWFF_CreateConstantForceEffect(pDiDevice, &pSecShootEffect, 200000,       // Duration
                                                     0, // Angle
                                                     10000,     // Magnitude
                                                     50000,     // Attack time
                                                     10000,     // Attack level
  ...
  joy_ff_handle_error(SWFF_CreateConditionEffectStruct(&Spring_cond_effect, pDiDevice, &pSpring, SPRING, INFINITE,      // uS
                                                       100,     // X Coefficient
                                                       0,       // X Offset
                                                       100,     // Y Coefficient
                                                       0,       // Y Offset
  ...
  init_periodic_effect_struct(&Struct_afterburn1, SINE, INFINITE,       // Duration
                              20000,    // Period
                              0,        // Angle
                              8000);    // Magnitude

  pDiDevice->CreateEffect(Struct_afterburn1.guid, &Struct_afterburn1.effect,
                          &pAfterburn1, NULL);
  if (pAfterburn1)
  ...
  init_periodic_effect_struct(&Struct_afterburn2, SINE, INFINITE,       // Duration
                              120000,   // Period
                              9000,     // Angle
                              4400);    // Magnitude

  pDiDevice->CreateEffect(Struct_afterburn2.guid, &Struct_afterburn2.effect,
                          &pAfterburn2, NULL);
  if (pAfterburn2)
  ...
  init_periodic_effect_struct(&Struct_dock, SQUARE_HIGH, 125000,        // Duration
                              100000,   // Period
                              9000,     // Angle
                              4000);    // Magnitude

  pDiDevice->CreateEffect(Struct_dock.guid, &Struct_dock.effect, &pDock,
                          NULL);
  if (pDock)
  ...
  init_periodic_effect_struct(&Struct_explode, SAWTOOTH_DOWN, 500000,   // Duration
                              20000,    // Period
                              9000,     // Angle
                              10000,    // Magnitude
                              0,        // Attack time
  ...
  pDiDevice->CreateEffect(Struct_explode.guid, &Struct_explode.effect,
                          &pExplode, NULL);
  if (pExplode)
  ...
  init_periodic_effect_struct(&Struct_deathroll1, SINE, INFINITE,       // Duration
                              200000,   // Period
                              0,        // Angle
                              10000,    // Magnitude
                              2000000,  // Attack time
  ...
  pDiDevice->CreateEffect(Struct_deathroll1.guid, &Struct_deathroll1.effect,
                          &pDeathroll1, NULL);
  if (pDeathroll1)
  ...
  init_periodic_effect_struct(&Struct_deathroll2, SINE, INFINITE,       // Duration
                              200000,   // Period
                              9000,     // Angle
                              10000,    // Magnitude
                              2000000,  // Attack time
  ...
  pDiDevice->CreateEffect(Struct_deathroll2.guid, &Struct_deathroll2.effect,
                          &pDeathroll2, NULL);
  if (pDeathroll2)
      ...
      if (Spring_cond_effect.DIConditionStruct[0].lPositiveCoefficient != v)
        {
          ...
          hr =
            joy_ff_handle_error(pSpring->
                                SetParameters(&Spring_cond_effect.
                                              DIEffectStruct,
...
void joy_ff_change_effect(di_periodic_effect_struct * s,
                          LPDIRECTINPUTEFFECT eff, int gain = -1, int dur =
                          0, int flags = -1)
{
  int reload = 0;
  ...
  if (flags == -1)
    {
      flags =
        DIEP_DURATION | DIEP_SAMPLEPERIOD | DIEP_GAIN | DIEP_DIRECTION |
...
void joy_ff_docked()
{
  if (pDock)
    {
      pDock->Stop();
      if (joy_ff_handle_error(SWFF_SetGain(pDock, 10000), "Dock") ==
          DIERR_INPUTLOST)
        {
          joy_reacquire_ff();
...
void joy_ff_play_reload_effect()
{
  if (pDock)
    {
      pDock->Stop();
      if (joy_ff_handle_error(SWFF_SetGain(pDock, 5000), "Dock (Reload)") ==
          DIERR_INPUTLOST)
        {
          joy_reacquire_ff();
...
void joy_ff_afterburn_on()
{
  if (pAfterburn1)
    {
      pAfterburn1->Stop();
      joy_ff_change_effect(&Struct_afterburn1, pAfterburn1, 5000, INFINITE,
  ...
  if (pAfterburn2)
    {
      pAfterburn2->Stop();
      joy_ff_change_effect(&Struct_afterburn2, pAfterburn2, 5000, INFINITE,
                           DIEP_DURATION | DIEP_GAIN);
...
void joy_ff_fly_by(int mag)
{
  ...
  if (pAfterburn1)
    {
      pAfterburn1->Stop();
      joy_ff_change_effect(&Struct_afterburn1, pAfterburn1, gain,
                           6000 * mag + 400000, DIEP_DURATION | DIEP_GAIN);
  ...
  if (pAfterburn2)
    {
      pAfterburn2->Stop();
      joy_ff_change_effect(&Struct_afterburn2, pAfterburn2, gain,
                           6000 * mag + 400000, DIEP_DURATION | DIEP_GAIN);
  ...
  if (Joy_ff_directional_hit_effect_enabled)
    {
      ...
      if (joy_ff_handle_error
          (SWFF_SetDirectionGain(pHitEffect1, idegs, imag),
           "HitEffect1") == DIERR_INPUTLOST)
        {
          joy_reacquire_ff();
          joy_ff_handle_error(SWFF_SetDirectionGain(pHitEffect1, idegs, imag),
                              "HitEffect1");
        }
      ...
      if (joy_ff_handle_error
          (SWFF_SetDirectionGain(pHitEffect2, idegs, imag),
           "HitEffect2") == DIERR_INPUTLOST)
        {
          joy_reacquire_ff();
          joy_ff_handle_error(SWFF_SetDirectionGain(pHitEffect2, idegs, imag),
                              "HitEffect2");
        }
    }
...
void joy_ff_play_secondary_shoot(int gain)
{
  ...
  if (gain != secondary_ff_level)
    {
      if (joy_ff_handle_error
          (SWFF_SetGain(pSecShootEffect, gain),
           "SecShootEffect") == DIERR_INPUTLOST)
        {
          joy_reacquire_ff();
          joy_ff_handle_error(SWFF_SetGain(pSecShootEffect, gain),
                              "SecShootEffect");
        }

      if (joy_ff_handle_error
          (SWFF_SetDuration(pSecShootEffect, 150000 + gain * 25),
           "SecShootEffect") == DIERR_INPUTLOST)
        {
          joy_reacquire_ff();
          joy_ff_handle_error(SWFF_SetDuration
                              (pSecShootEffect, 150000 + gain * 25),
                              "SecShootEffect");
        }
...
void joy_ff_play_primary_shoot(int gain)
{
  ...
  if (gain != primary_ff_level)
    {
      if (joy_ff_handle_error(SWFF_SetGain(pShootEffect, gain), "ShootEffect")
          == DIERR_INPUTLOST)
        {
          joy_reacquire_ff();
          joy_ff_handle_error(SWFF_SetGain(pShootEffect, gain),
                              "ShootEffect");
        }
...
void init_periodic_effect_struct(di_periodic_effect_struct * effect, int type,
                                 int dur, int per, int ang, int mag, int att,
                                 int fade)
{
  // type-specific stuff
  DWORD dwPhase = 0;
