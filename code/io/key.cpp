...
void key_mark(uint code, int state, uint latency) 
{
    ...
    
    {
      ...
      Script_system.SetHookVar("Key", 's',
                                textify_scancode(Current_key_down));
...
 DWORD di_process(DWORD lparam) 
{
    ...
    
    {
        ...
        
        {
            ...
            Di_keyboard->GetDeviceData(sizeof(DIDEVICEOBJECTDATA), rgdod,
                                       &dwItems, 0);
...
  int di_init() 
{
     ...
     */ 
    hr =
    Di_keyboard->SetCooperativeLevel((HWND) os_get_window(),
  ...
  hdr.dwData = 16;              //MAX_BUFFERED_KEYBOARD_EVENTS;
    ...
    CreateThread(NULL, 1024, (LPTHREAD_START_ROUTINE) di_process, NULL, 0,
                 &Di_thread_id);
