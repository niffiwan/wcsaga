...
void process_debug_keys ( int k )
{
	...
	case KEY_DEBUGGED + KEY_V:
		...
		message_send_unique_to_player ( Messages[Message_debug_index].name,
		                                Message_waves[Messages[Message_debug_index].wave_info.index].name, MESSAGE_SOURCE_SPECIAL,
	...
	case KEY_DEBUGGED1 + KEY_SHIFTED + KEY_K:
		if ( ( Player_ai->target_objnum != -1 ) && ( Player_ai->targeted_subsys != NULL ) )
		{
			...
			if ( objp->type == OBJ_SHIP )
			{
				...
				do_subobj_hit_stuff ( objp, Player_obj, &g_subobj_pos, ( float ) - Player_ai->targeted_subsys->system_info->
				                      type, NULL ); //100.0f);
	...
	case KEY_DEBUGGED1 + KEY_I:
		...
		HUD_sourced_printf ( HUD_SOURCE_HIDDEN, XSTR ( "You are %s", 10 ),
		                     Player_obj->flags & OF_INVULNERABLE ? XSTR ( "now INVULNERABLE!", 11 ) : XSTR ( "no longer invulnerable...",
		                             12 ) );
		break;
	...
	case KEY_DEBUGGED1 + KEY_SHIFTED + KEY_I:
		if ( Player_ai->target_objnum != -1 )
		{
			...
			HUD_sourced_printf ( HUD_SOURCE_HIDDEN, XSTR ( "Player's target [%s] is %s", 13 ),
			                     Ships[objp->instance].ship_name, objp->flags & OF_INVULNERABLE ? XSTR ( "now INVULNERABLE!",
	...
	case KEY_F12 + KEY_SHIFTED:
		...
		else
		{
			...
			HUD_sourced_printf ( HUD_SOURCE_HIDDEN, XSTR ( "%s added to set %d. (F%d)", 27 ),
			                     Ships[Objects[Player_ai->target_objnum].instance].ship_name, hotkey_set, 4 + hotkey_set + 1 );
		}
...
int button_function_demo_valid ( int n )
{
	...
	case VIEW_EXTERNAL_TOGGLE_CAMERA_LOCK:
		...
		if ( Viewer_mode & VM_EXTERNAL )
		{
			...
			{
				HUD_sourced_printf ( HUD_SOURCE_HIDDEN,
				                     XSTR ( "External camera is free, controls will move the camera, not the ship", 37 ) );
			}
		}
		ret = 1;
		break;
