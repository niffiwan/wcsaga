  ...
  while (1)
    {
      ...
      if (WaitForSingleObject(Joy_tell_thread_to_end_event, joy_pollrate) ==
          WAIT_OBJECT_0)
        {
          break;
        }
...
void joy_close()
{
  ...
  // Wait for it to end
  if (WaitForSingleObject(Joy_thread_says_its_done_event, 5000) ==
      WAIT_TIMEOUT)
    {
      //INFINITE );
      ...
      if (rs == JOYERR_NOERROR)
        {
          ...
          Joy_tell_thread_to_end_event =
            CreateEvent(NULL, FALSE, FALSE, NULL);
          Joy_thread_says_its_done_event =
            CreateEvent(NULL, FALSE, FALSE, NULL);

          joy_thread = CreateThread(NULL,
                                    1024 * 32,
                                    (LPTHREAD_START_ROUTINE) joy_process,
                                    NULL, 0, &joy_thread_id);
...
int joy_get_scaled_reading(int raw, int axn)
{
  ...
  dead_zone =
    (joystick.axis_max[axn] - joystick.axis_min[axn]) * Dead_zone_size / 100;
...
int joy_di_init()
{
  ...
  Joy_di_inited = 0;
  hr =
    DirectInputCreate(GetModuleHandle(NULL), 0x500, &Di_joystick_obj, NULL);
  if (FAILED(hr))
    {
  ...
  Di_joy_guid_valid = 0;
  hr =
    Di_joystick_obj->EnumDevices(DIDEVTYPE_JOYSTICK, joy_di_enum,
                                 Di_joystick_obj, DIEDFL_ATTACHEDONLY);
  if (FAILED(hr))
    {
  ...
  hr =
    pdev->SetCooperativeLevel((HWND) os_get_window(),
  ...
  hr =
    pdev->QueryInterface(IID_IDirectInputDevice2, (LPVOID *) & Di_joystick);
  if (FAILED(hr))
    {
      pdev->Release();
      return -1;
