...
BOOL CALLBACK DIEnumAndDestroyCreatedEffectsProc(LPDIRECTINPUTEFFECT
                                                 pDIEffect, LPVOID lpvRef);
                                   ...
                                   LPDIRECTINPUTDEVICE2 * ppDIDevice)
{
  ...
  hResult =
    DirectInputCreate(GetModuleHandle(NULL), DIRECTINPUT_VERSION, ppDI, NULL);
  if (FAILED(hResult))
    return hResult;
  ...
  hResult =
    (*ppDI)->EnumDevices(DIDEVTYPE_JOYSTICK, DIEnumDevicesProc,
  ...
  hResult =
    (*ppDI)->CreateDevice(DIDeviceInstance.guidInstance, &pDIDevice, NULL);
  if (FAILED(hResult))
    {
      (*ppDI)->Release();
      *ppDI = NULL;
  ...
  hResult =
    pDIDevice->QueryInterface(IID_IDirectInputDevice2, (void **) ppDIDevice);
  if (FAILED(hResult))
    {
      pDIDevice->Release();
      pDIDevice = NULL;
  ...
  hResult =
    (*ppDIDevice)->SetCooperativeLevel(hWnd,
                                     ...
                                     IN DWORD dwFlags)
{
  ...
  hResult =
    (*ppDIDevice)->SetProperty(DIPROP_AUTOCENTER, &DIPropAutoCenter.diph);
  if (FAILED(hResult))
    {
      (*ppDIDevice)->Release();
      *ppDIDevice = NULL;
...
BOOL CALLBACK DIEnumDevicesProc(LPCDIDEVICEINSTANCE lpddi, LPVOID lpvContext)
{
  ...
  if (GET_DIDEVICE_TYPE(lpddi->dwDevType) == DIDEVTYPE_JOYSTICK)
    {
      memcpy((LPVOID) pDIDeviceInstance, (LPVOID) lpddi,
...
HRESULT SWFF_DestroyAllEffects(IN LPDIRECTINPUTDEVICE2 pDIDevice)
{
  ...
  hResult =
    pDIDevice->EnumCreatedEffectObjects(DIEnumAndDestroyCreatedEffectsProc,
                                        NULL, 0);
...
HRESULT SWFF_SetDirectionGain(IN LPDIRECTINPUTEFFECT pDIEffect,
                              IN DWORD dwAngle, IN DWORD dwGain)
{
  if (pDIEffect == NULL)
    return SFERR_INVALID_PARAM;
                         ...
                         IN LONG lMagnitude, IN DWORD dwDirection)
{
  ...
  return pDIEffect->SetParameters(&thisEffect,
...
HRESULT SWFF_CreateROMEffect(IN LPDIRECTINPUTDEVICE2 pDIDevice,
                             ...
                             IN DWORD dwDuration,
                             IN DWORD dwGain,
                             IN DWORD dwDirection, IN LONG lButton)
{
  if (pDIDevice == NULL || ppDIEffect == NULL)
    return SFERR_INVALID_PARAM;
  // Always clear return IPtr
  *ppDIEffect = NULL;
  ...
  DIEffect.dwTriggerButton =
    lButton == -1 ? DIEB_NOTRIGGER : FIELD_OFFSET(DIJOYSTATE,
                                                  rgbButtons) + lButton;
  DIEffect.dwTriggerRepeatInterval = 0;
  DIEffect.cAxes = 2;
  DIEffect.rgdwAxes = rgdwAxes;
  ...
  HRESULT hRet =
    pDIDevice->CreateEffect(refGUID, &DIEffect, &pROMEffect, NULL);
  if (FAILED(hRet))
    return hRet;
...
BOOL SWFF_RegisterVFXObject(IN LPCTSTR pszVFXPath)
{
  ...
  return SWFF_WriteRegString("\\VFX1.0", "VFX Object")
    ...
    && SWFF_WriteRegString("\\CLSID\\" GUID_VFX_Object
                           "\\VersionIndependentProgID", "VFX")
    && SWFF_WriteRegString("\\CLSID\\" GUID_VFX_Object "\\InprocServer32",
                           pszVFXPath)
                                     ...
                                     IN const TCHAR * pszFileName)
{
  ...
  *ppDIEffect = NULL;
  ...
  HRESULT hResult;
  hResult =
    pDIDevice->CreateEffect(GUID_VFXEffect, &DIEffectStruct, &pVFXEffect,
                            NULL);
  if (FAILED(hResult))
    return hResult;
...
HRESULT SWFF_CreateVFXEffectFromBuffer(IN LPDIRECTINPUTDEVICE2 pDIDevice,
                                       IN OUT LPDIRECTINPUTEFFECT *
                                       ppDIEffect, IN const LPVOID pBuffer,
                                    ...
                                    IN const TCHAR * pszFileName)
{
  ...
  *ppDIEffect = NULL;
  ...
  hResult = pIVFX->CreateEffectFromFile(pDIDevice, ppDIEffect,
                                        0, pszFileName, NULL, NULL, dwInFlags,
                                        NULL);
                                      ...
                                      IN OUT PDWORD pdwOutFlags)
{
  ...
  *pdwOutFlags = 0;
  ...
  hResult = pIVFX->CreateEffectFromFile(NULL, NULL,
                                        &dwEffectCount, pszFileName, NULL,
                                        &dwBufferSize, dwInFlags, NULL);
  if (FAILED(hResult))
    {
      pIVFX->Release();
      CoUninitialize();
  ...
  hResult = pIVFX->CreateEffectFromFile(pDIDevice, ppDIEffect,
                                        &dwEffectCount, pszFileName,
                                        pUDBuffer, &dwBufferSize, dwInFlags,
                                        &dwOutFlags);
                                        ...
                                        IN OUT PDWORD pdwOutFlags)
{
  ...
  *pdwOutFlags = 0;
  ...
  hResult = pIVFX->CreateEffectFromBuffer(NULL, NULL,
                                          &dwEffectCount, pBuffer,
                                          dwBufferSize, NULL, &dwUDBufferSize,
                                          dwInFlags, NULL);
  if (FAILED(hResult))
    {
      pIVFX->Release();
  ...
  hResult = pIVFX->CreateEffectFromBuffer(pDIDevice, ppDIEffect,
                                          &dwEffectCount, pBuffer,
                                          dwBufferSize, pUDBuffer,
                                          &dwUDBufferSize, dwInFlags,
                                          &dwOutFlags);
...
HRESULT SWFF_CreatePeriodicEffect(IN LPDIRECTINPUTDEVICE2 pDIDevice,
                                  ...
                                  IN DWORD dwFadeTime,
                                  IN DWORD dwFadeLevel, IN LONG lButton)
{
  if (pDIDevice == NULL || ppDIEffect == NULL)
    return SFERR_INVALID_PARAM;
  // Always clear return IPtr
  *ppDIEffect = NULL;
  ...
  DIEffectStruct.dwTriggerButton =
    lButton == -1 ? DIEB_NOTRIGGER : FIELD_OFFSET(DIJOYSTATE,
                                                  rgbButtons) + lButton;
  DIEffectStruct.dwTriggerRepeatInterval = 0;
  DIEffectStruct.cAxes = 2;
...
HRESULT SWFF_CreateSpringEffect(IN LPDIRECTINPUTDEVICE2 pDIDevice,
                                ...
                                IN LONG lCenterx,
                                IN LONG lKy,
                                IN LONG lCentery, IN LONG lButton)
{
  if (pDIDevice == NULL || ppDIEffect == NULL)
    return SFERR_INVALID_PARAM;
...
HRESULT SWFF_CreateFrictionEffect(IN LPDIRECTINPUTDEVICE2 pDIDevice,
                                  IN OUT LPDIRECTINPUTEFFECT * ppDIEffect,
                                  IN DWORD dwDuration,
                                  IN LONG lFx, IN LONG lFy, IN LONG lButton)
{
  if (pDIDevice == NULL || ppDIEffect == NULL)
    return SFERR_INVALID_PARAM;
...
HRESULT SWFF_CreateConditionEffectStruct(di_condition_effect_struct * ptr,
                                         ...
                                         IN OUT LPDIRECTINPUTEFFECT *
                                         ppDIEffect, IN DWORD dwType,
                                         IN DWORD dwDuration,
                                         IN LONG lXCoefficient,
                                         IN LONG lXOffset,
                                         IN LONG lYCoefficient,
                                         IN LONG lYOffset, IN LONG lButton)
{
  if (pDIDevice == NULL || ppDIEffect == NULL)
    return SFERR_INVALID_PARAM;
  ...
  *ppDIEffect = NULL;
  ...
  ptr->DIEffectStruct.dwTriggerButton =
    lButton == -1 ? DIEB_NOTRIGGER : FIELD_OFFSET(DIJOYSTATE,
                                                  rgbButtons) + lButton;
  ...
  HRESULT hResult;
  hResult =
    pDIDevice->CreateEffect(guid, &ptr->DIEffectStruct, ppDIEffect, NULL);
                              ...
                              IN DWORD dwFadeLevel, IN LONG lButton)
{
  ...
  *ppDIEffect = NULL;
  ...
  HRESULT hResult;
  hResult =
    pDIDevice->CreateEffect(GUID_RampForce, &DIEffectStruct, ppDIEffect,
                            NULL);
...
HRESULT SWFF_CreateConstantForceEffect(IN LPDIRECTINPUTDEVICE2 pDIDevice,
                                       IN OUT LPDIRECTINPUTEFFECT *
                                       ppDIEffect, IN DWORD dwDuration,
                                       IN DWORD dwDirection,
                                       IN LONG lMagnitude,
                                       IN DWORD dwAttackTime,
                                       IN DWORD dwAttackLevel,
                                       ...
                                       IN DWORD dwFadeLevel, IN LONG lButton)
{
  ...
  *ppDIEffect = NULL;
  ...
  HRESULT hResult;
  hResult =
    pDIDevice->CreateEffect(GUID_ConstantForce, &DIEffectStruct, ppDIEffect,
                            NULL);
                              ...
                              IN LONG lWallCoefficient, IN LONG lButton)
{
  ...
  *ppDIEffect = NULL;
  ...
  HRESULT hResult;
  hResult =
    pDIDevice->CreateEffect(GUID_Wall, &DIEffectStruct, ppDIEffect, NULL);
