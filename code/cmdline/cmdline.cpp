...
void parm_stuff_args(cmdline_parm * parm, char *cmdline)
{
  ...
  while ((*cmdline != '\0') && strncmp(cmdline, " -", 2)
         && ((size_t) (dest - buffer) < sizeof(buffer)))
    {
      *dest++ = *cmdline++;
    }
...
void os_init_cmdline(char *cmdline)
{
  ...
  snprintf(cmdname, MAX_PATH - 1, "%s%s%s%s%s%s%s%s%s", detect_home(),
           DIR_SEPARATOR_STR, Osreg_company_name, DIR_SEPARATOR_STR,
           Osreg_user_dir, DIR_SEPARATOR_STR, "data", DIR_SEPARATOR_STR,
           "cmdline_fso.cfg");
  fp = fopen(cmdname, "rt");
  ...
  if (get_flags_arg.found())
    {
      char cmdname[MAX_PATH];
      memset(cmdname, 0, sizeof(cmdname));
      snprintf(cmdname, MAX_PATH - 1, "%s%s%s%s%s%s%s", detect_home(),
               DIR_SEPARATOR_STR, Osreg_company_name, DIR_SEPARATOR_STR,
               Osreg_user_dir, DIR_SEPARATOR_STR, "flags.lch");
  ...
  if (output_sexp_arg.found())
    {
      ...
      snprintf(cmdname, MAX_PATH - 1, "%s%s%s%s%s%s%s", detect_home(),
               DIR_SEPARATOR_STR, Osreg_company_name, DIR_SEPARATOR_STR,
               Osreg_user_dir, DIR_SEPARATOR_STR, "sexps.html");
