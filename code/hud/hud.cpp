			...
			if (image_ids[i].first_frame != -1)
			{
				if (current_hud->custom_gauge_moveflags[i])
				{
					hud_aabitmap(image_ids[i].first_frame + current_hud->custom_gauge_frames[i],
						current_hud->custom_gauge_coords[i][0] + HUD_nose_x, current_hud->custom_gauge_coords[i][1] +
						HUD_nose_y);
				}
				else
				{
...
void hud_show_text_flash_icon(char* txt, int y, int bright)
{
	...
	// draw the box	
	gr_rect((int)((((float)gr_screen.max_w_unscaled / 2.0f) - ((float)w / 2.0f)) - 1.0f) + HUD_nose_x, (int)((float)y -
																											 1.0f) +
		HUD_nose_y, w + 2, h + 1);
