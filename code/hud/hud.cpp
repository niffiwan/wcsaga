...
void HUD_init_colors() 
{
  ...
   gr_init_alphacolor(&HUD_color_debug, 128, 255, 128,
                        HUD_color_alpha * 16);
...
void hud_maybe_show_damage() 
{
    ...
    
    {
      ...
       if ((Player_ship->ship_max_hull_strength -
...
void hud_maybe_blit_head_border() 
{
    ...
    
    {
        ...
        gr_string(Head_message_coords[gr_screen.res][0],
                  Head_message_coords[gr_screen.res][1], XSTR("message",
                                                              217));
    ...
    
    {
      ...
      int i;
        ...
        
        {
            ...
            if (current_hud->custom_gauge_color_parents[i] >= 0
                && current_hud->custom_gauge_color_parents[i] <
                NUM_HUD_GAUGES)
            
            {
              gauge_color =
                &HUD_config.clr[current_hud->custom_gauge_color_parents[i]];
          ...
           if (gauge_color->red != 0 || gauge_color->green != 0
                 || gauge_color->blue != 0)
            
            {
              
                //No custom alpha gauge color...
                gr_init_alphacolor(gauge_color, gauge_color->red,
                                   gauge_color->green, gauge_color->blue,
            ...
            
            {
                ...
                {
                  GR_AABITMAP(image_ids[i].first_frame +
...
void hud_show_damage_popup() 
{
    ...
    
    {
        ...
        
        {
                     ...
                     hud_targetbox_truncate_subsys_name(hud_subsys_list
                                                        [best_index].name));
...
 void hud_show_text_flash_icon(char *txt, int y, int bright) 
{
    ...
    // draw the box
    gr_rect((int)
            ((((float) gr_screen.max_w_unscaled / 2.0f) -
              ((float) w / 2.0f)) - 1.0f) + HUD_nose_x,
            (int) ((float) y - 1.0f) + HUD_nose_y, w + 2, h + 1);
...
void hud_show_kills_gauge() 
{
  ...
  char num_kills_string[32];
    ...
    
    {
                 ...
                 Kills_text_val_coords_gr[gr_screen.res][1],
                 num_kills_string);
...
void hud_add_objective_messsage(int type, int status) 
{
    ...
    {
      mission_goal_fetch_num_resolved(type,
...
 void hud_show_voice_status() 
{
    ...
    case MULTI_VOICE_STATUS_DENIED:
        ...
        gr_string(Voice_coords[gr_screen.res][0],
                  Voice_coords[gr_screen.res][1], XSTR("[voice denied]",
                                                       243));
    ...
    case MULTI_VOICE_STATUS_PLAYING:
                 ...
                 Voice_coords[gr_screen.res][1], XSTR("[playing voice]",
                                                      245));
