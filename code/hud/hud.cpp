...
void HUD_init_hud_color_array()
{
  ...
  for (i = 0; i < HUD_NUM_COLOR_LEVELS; i++)
    {
      gr_init_alphacolor(&HUD_color_defaults[i], HUD_color_red,
                         HUD_color_green, HUD_color_blue, (i + 1) * 16);
    }
}
...
void hud_init_talking_head_gauge()
{
  ...
  if (!Head_frame_gauge_loaded)
    {
      Head_frame_gauge.first_frame =
        bm_load_animation(Head_fname[gr_screen.res],
...
void hud_init_mission_time_gauge()
{
  ...
  if (!Mission_time_gauge_loaded)
    {
      Mission_time_gauge.first_frame =
        bm_load_animation(Mission_time_fname[gr_screen.res],
  ...
  if (!
       ...
       (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY)))
    {
      ...
      if (hud_gauge_active(HUD_MISSILE_WARNING_ARROW)
          && !hud_disabled_except_messages())
        {
          hud_show_homing_missiles();
        }
      if (!(Viewer_mode & VM_OTHER_SHIP))
        {
          // Added to prevent the offscreen crosshair indicator from drawing when viewing other ships (Swifty)
  ...
  if (hud_gauge_active(HUD_DAMAGE_GAUGE))
    {
      ...
      if ((Player_ship->ship_max_hull_strength - Player_obj->hull_strength) >
          1.0f)
        {
          show_gauge_flag = 1;
        }
...
void hud_show_mission_time()
{
  ...
  if (Mission_time_gauge.first_frame >= 0)
    {
      hud_aabitmap(Mission_time_gauge.first_frame,
  ...
  if (time_comp < 1)
    ...
    {
      gr_printf(Mission_time_text_val_coords[gr_screen.res][0],
                Mission_time_text_val_coords[gr_screen.res][1], XSTR("x%.2f",
                                                                     216),
                time_comp);
    }
}
  ...
  if (message_anim_is_playing())
    {
      ...
      hud_aabitmap(Head_frame_gauge.first_frame,
      ...
      if ((Netgame.server != NULL)
          && (Netgame.server->s_info.ping.ping_avg > 0))
        {
          ...
          if (Netgame.server->s_info.ping.ping_avg >= 1000)
            ...
            {
              sprintf(ping_str, XSTR("%d ms", 629),
                      Netgame.server->s_info.ping.ping_avg);
            }

          // blit the string out
          hud_set_default_color();
          gr_string(Ping_coords[gr_screen.res][0],
                    Ping_coords[gr_screen.res][1], ping_str);
        }
    }
}
  ...
  if (Player_ai->target_objnum > -1)
    {
      if (Objects[Player_ai->target_objnum].type == OBJ_WEAPON)
        {
          if (Weapons[Objects[Player_ai->target_objnum].instance].
              lssm_stage == 3)
            {
          ...
          for (i = 0; i < Num_custom_gauges; i++)
            {
              if (strlen(current_hud->custom_gauge_images[i]))
                {
                  image_ids[i].first_frame =
                    bm_load_animation(current_hud->custom_gauge_images[i],
                                      &image_ids[i].num_frames);
                  if (image_ids[i].first_frame != -1)
                    {
                      bm_page_in_aabitmap(image_ids[i].first_frame,
                                          image_ids[i].num_frames);
                    }
                  else
                    {
                      image_ids[i].first_frame =
                        bm_load(current_hud->custom_gauge_images[i]);
                    }
                }
              else
                {
          ...
          if (strlen(current_hud->custom_gauge_text[i]))
            {
              ...
              gr_string(current_hud->custom_gauge_coords[i][0],
          ...
          if (image_ids[i].first_frame != -1)
            {
              if (current_hud->custom_gauge_moveflags[i])
                {
                  hud_aabitmap(image_ids[i].first_frame +
                               current_hud->custom_gauge_frames[i],
                               current_hud->custom_gauge_coords[i][0] +
                               HUD_nose_x,
                               current_hud->custom_gauge_coords[i][1] +
                               HUD_nose_y);
                }
              ...
              if (Player_engine_snd_loop == -1)
                {
                  Player_engine_snd_loop =
                    snd_play_looping(&Snds[SND_ENGINE], 0.0f, -1, -1,
                                     percent_throttle);
                }
              else
                {
                  ...
                  if (snd_is_playing(Player_engine_snd_loop))
                    {
                      snd_set_volume(Player_engine_snd_loop,
                                     percent_throttle *
                                     Snds[SND_ENGINE].default_volume);
                    }
                  else
                    {
                      Player_engine_snd_loop = -1;
  ...
  if (!Damage_gauges_loaded)
    {
      for (i = 0; i < NUM_DAMAGE_GAUGES; i++)
        {
          Damage_gauges[i].first_frame =
            bm_load_animation(Damage_gauge_fnames[gr_screen.res][i],
...
void hud_show_damage_popup()
{
  ...
  hud_aabitmap(Damage_gauges[0].first_frame,
  ...
  if (screen_integrity < 100)
    {
      ...
      gr_string(Hull_integ_coords[gr_screen.res][0],
                Hull_integ_coords[gr_screen.res][1], XSTR("Hull Integrity",
                                                          220));
      ...
      if (!timestamp_elapsed
          (Pl_hud_subsys_info[type].flash_duration_timestamp))
        {
          if (timestamp_elapsed(Pl_hud_next_flash_timestamp))
            {
              Pl_hud_is_bright ^= 1;
              Pl_hud_next_flash_timestamp =
                timestamp(SUBSYS_DAMAGE_FLASH_INTERVAL);
            }
...
int hud_anim_load(hud_anim * ha)
{
  ...
  if (ha->first_frame == -1)
    {
      Warning(LOCATION, "Couldn't load hud animation for file '%s'",
              ha->filename);
      return -1;
    }
...
int hud_anim_render(hud_anim * ha, float frametime, int draw_alpha, int loop,
                    int hold_last, int reverse, bool resize, bool mirror)
{
  int framenum;
...
void hud_show_text_flash_icon(char *txt, int y, int bright)
{
  ...
  // draw the box
  gr_rect((int)
          ((((float) gr_screen.max_w_unscaled / 2.0f) - ((float) w / 2.0f)) -
           1.0f) + HUD_nose_x, (int) ((float) y - 1.0f) + HUD_nose_y, w + 2,
          h + 1);

  // string
  hud_set_gauge_color(HUD_TEXT_FLASH, HUD_C_BRIGHT);
  gr_string(fl2i((gr_screen.max_w_unscaled - w) / 2.0f) + HUD_nose_x,
            y + HUD_nose_y, txt);
...
void hud_show_kills_gauge()
{
  ...
  // draw background
  hud_aabitmap(Kills_gauge.first_frame, Kills_gauge_coords[gr_screen.res][0],
  ...
  if (Lcl_gr)
    {
      gr_string(Kills_text_val_coords_gr[gr_screen.res][0] - w,
                Kills_text_val_coords_gr[gr_screen.res][1], num_kills_string);
    }
  else
    {
...
void hud_maybe_show_netlag_icon()
{
  ...
  if (emp_should_blit_gauge())
    {
      gr_aabitmap(Netlag_coords[gr_screen.res][0],
                  Netlag_coords[gr_screen.res][1]);
    }
}
  ...
  if (!Kills_gauge_loaded)
    {
      Kills_gauge.first_frame =
        bm_load_animation(Kills_fname[gr_screen.res],
  ...
  if (!Netlag_icon_loaded)
    {
      Netlag_icon.first_frame =
        bm_load_animation(Netlag_fname[gr_screen.res],
                          &Netlag_icon.num_frames);
      if (Netlag_icon.first_frame == -1)
        {
          Warning(LOCATION,
                  "Could not load in the netlag ani: Netlag_fname[gr_screen.res]\n");
          return;
        }
...
void hud_support_view_init()
{
  ...
  if (!Support_view_gauge_loaded)
    {
      Support_view_gauge.first_frame =
        bm_load_animation(Support_fname[gr_screen.res],
...
int hud_get_dock_time(object * docker_objp)
{
  ...
  vm_vec_sub(&rel_vel, &docker_objp->phys_info.vel,
             &dockee_objp->phys_info.vel);
  ...
  if (rel_speed <= docker_speed / 2.0f)
    {
      //  This means the player is moving away fast from the support ship.
  ...
  while (sop != END_OF_LIST(&Ship_obj_list))
    {
      if (Ship_info[Ships[Objects[sop->objnum].instance].ship_info_index].
          flags & SIF_SUPPORT)
        {
          ...
          if (!
              (Ships[Objects[sop->objnum].instance].
               flags & (SF_DYING | SF_EXPLODED)))
            {
...
void hud_support_view_blit()
{
  ...
  hud_aabitmap(Support_view_gauge.first_frame,
      ...
      if (objp->type == OBJ_ASTEROID)
        {
          if (OBJ_INDEX(objp) == Player_ai->target_objnum)
            {
              use_color =
                iff_get_color_by_team(Iff_traitor, Player_ship->team,
                                      is_bright);
            }
          else
            {
        ...
        {
          use_color =
            iff_get_color_by_team_and_object(obj_team(objp),
                                             Player_ship->team, is_bright,
                                             objp);
        }
    }
...
void hud_set_custom_gauge_color(int gauge_index, int bright_index)
{
  ...
  if (current_hud->custom_gauge_color_parents[gauge_index] >= 0 &&
    ...
    {
      gauge_color =
        &HUD_config.clr[current_hud->custom_gauge_color_parents[gauge_index]];
      flash_status =
        hud_gauge_maybe_flash(current_hud->
                              custom_gauge_color_parents[gauge_index]);
    }
  else
    {
...
void hud_set_gauge_color_specific(int flash_status, color * use_color,
                                  int bright_index)
{
  int alpha;
  ...
  if (bright_index != HUD_C_NONE)
    {
      switch (bright_index)
        ...
        case HUD_C_BRIGHT:
          ...
          // intensity
        default:
          Assert((bright_index >= 0)
                 && (bright_index < HUD_NUM_COLOR_LEVELS));
          if (bright_index < 0)
            {
              bright_index = 0;
            }
  ...
  if (!Objective_display_gauge_inited)
    {
      Objective_display_gauge.first_frame =
        bm_load_animation(Objective_fname[gr_screen.res],
...
void hud_maybe_display_subspace_notify()
{
  ...
  hud_aabitmap(Objective_display_gauge.first_frame,
...
void hud_show_voice_status()
{
  ...
  switch (multi_voice_status())
    ...
    case MULTI_VOICE_STATUS_DENIED:
      ...
      gr_string(Voice_coords[gr_screen.res][0],
                Voice_coords[gr_screen.res][1], XSTR("[voice denied]", 243));
      break;

      // the player is currently recording
    case MULTI_VOICE_STATUS_RECORDING:
      gr_string(Voice_coords[gr_screen.res][0],
                Voice_coords[gr_screen.res][1], XSTR("[recording voice]",
                                                     244));
      break;
    ...
    case MULTI_VOICE_STATUS_PLAYING:
      gr_string(Voice_coords[gr_screen.res][0],
                Voice_coords[gr_screen.res][1], XSTR("[playing voice]", 245));
      break;
  ...
  if (multi_msg_message_text(txt))
    {
      ...
      gr_string(Multi_msg_coords[gr_screen.res][0],
                Multi_msg_coords[gr_screen.res][1], txt);
    }
}
  ...
  if ((viewer_obj == Player_obj) && wiggedy_wack)
    {
      ...
      HUD_offset_x -=
        0.45f * (i2fl(gr_screen.clip_width_unscaled) * 0.5f - pt.sx);
      HUD_offset_y -=
        0.45f * (i2fl(gr_screen.clip_height_unscaled) * 0.5f - pt.sy);
...
void HUD_get_nose_coordinates(int *x, int *y)
{
  ...
  *y = 0;
  ...
  vm_vec_scale_add(&p0, &Player_obj->pos, &Player_obj->orient.vec.fvec,
                   10000.0f);
  g3_rotate_vertex(&v0, &p0);
...
void hud_aabitmap_ex(int bitmap_num, int x, int y, int w, int h, int sx,
                     int sy, bool resize, bool mirror)
{
  if (emp_should_blit_gauge())
    {
      gr_set_bitmap(bitmap_num);
