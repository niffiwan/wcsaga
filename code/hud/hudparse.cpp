...
int stuff_coords(hud_info * dest_hud, gauge_info * cg, bool required = false)
{
  ...
  if (optional_string("+Alignment:"))
    ...
    {
      *HUD_INT(dest_hud, cg->alignment_dest) =
        *HUD_INT(dest_hud, cg->parent->alignment_dest);
    }
...
static void parse_resolution_gauges(hud_info * dest_hud)
{
  ...
  while (!required_string_5
         ("$Gauge:", "$Default:", "$Resolution:", "$Ship:", "#End"))
    {
  ...
  if (Num_gauge_types < MAX_HUD_GAUGE_TYPES)
    {
      ...
      cg->alignment_dest =
        HUD_VAR(custom_gauge_alignments[0]) +
        (Num_custom_gauges * sizeof(Alignment));
      ...
      if (optional_string("+Parent:"))
        {
          ...
          if (cg->parent == NULL)
            {
              WarningEx(LOCATION, "\"+Parent:\" HUD gauge \"%s\" not found!",
                        buffer);
            }
        }
