...
extern void draw_brackets_square_quick(int x1, int y1, int x2, int y2,
                                        int thick);
...
void HUD_Draw_Navigation() 
{
    ...
    
    {
      ...
      vec3d *target_pos = Navs[CurrentNav].GetPosition();
        ...
        vm_vec_dist_quick(&Objects[Player_ship->objnum].pos, target_pos);
        ...
        
        {
          ...
           draw_brackets_square(x - box_scale, y - box_scale, x + box_scale,
                                  y + box_scale);
           gr_set_color_fast(&NavColor);
          
            // draw the nav name
            gr_string(x - (box_scale + 10), y - (box_scale + 20),
                      Navs[CurrentNav].NavName);
          
            // draw distance to target in lower right corner of box
            hud_target_show_dist_on_bracket(x + (box_scale + 10),
                                            y + (box_scale + 10), dist);
