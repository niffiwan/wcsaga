...
void ssm_get_random_start_pos(vec3d * out, vec3d * start, matrix * orient,
                              int ssm_index)
{
  vec3d temp;
...
void ssm_create(object * target, vec3d * start, int ssm_index,
          ...
          if (!moveup->done_flags[idx])
            {
              ...
              if (moveup->fireballs[idx] >= 0)
                {
                  ...
                  if ((1.0f -
                       fireball_lifeleft_percent(&Objects
                                                 [moveup->fireballs[idx]])) >=
                      0.5f)
                    {
                      // get an orientation
                      vec3d temp;
                      ...
                      vm_vec_sub(&temp, &moveup->sinfo.target->pos,
                      ...
                      Weapons[Objects[weapon_objnum].instance].homing_object =
                        moveup->sinfo.target;
                      Weapons[Objects[weapon_objnum].instance].target_sig =
              ...
              // maybe create his warpin effect
                       ...
                       && timestamp_elapsed(moveup->sinfo.delay_stamp[idx]))
                {
                  ...
                  vm_vector_2_matrix(&orient, &temp, NULL, NULL);
                  moveup->fireballs[idx] =
                    fireball_create(&moveup->sinfo.start_pos[idx],
                                    FIREBALL_WARP, FIREBALL_WARP_EFFECT, -1,
                                    si->warp_radius, 0, &vmd_zero_vector,
                                    si->warp_time, 0, &orient);
                }
            }
        }
      if (finished)
