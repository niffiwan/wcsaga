  ...
  for (i = 0; i < NUM_ESCORT_FRAMES; i++)
    {
      if (strlen(current_hud->Escort_filename[i]))
        {
          Escort_gauges[i].first_frame =
            bm_load_animation(current_hud->Escort_filename[i],
              ...
              if (Ships[objp->instance].team == Player_ship->team)
                {
                  ...
                  if (Ships[objp->instance].
                      flags2 & SF2_FRIENDLY_STEALTH_INVIS)
                    {
                      continue;
                    }
                }
...
void hud_setup_escort_list(int level)
{
  ...
  insertion_sort(complete_escorts, num_complete_escorts, sizeof(escort_info),
                 escort_compare_func);
      ...
      for (i = 0; i < Num_escort_ships; i++)
        {
          ...
          if (Objects[objnum].flags & OF_SHOULD_BE_DEAD
              || Ships[Objects[objnum].instance].
              flags & SF_HIDDEN_FROM_SENSORS)
            {
  ...
  if (!timestamp_elapsed(shi->shield_hit_timers[HULL_HIT_OFFSET]))
    {
      if (timestamp_elapsed(shi->shield_hit_next_flash[HULL_HIT_OFFSET]))
        {
          shi->shield_hit_next_flash[HULL_HIT_OFFSET] =
...
void hud_escort_show_icon_dogfight(int x, int y, int index)
{
  ...
  emp_hud_string(x + current_hud->Escort_name[0],
                 y + current_hud->Escort_name[1], EG_ESCORT1 + index, buf);
  ...
  if ((Net_players[np_index].m_player->objnum >= 0)
      ...
      && (Objects[Net_players[np_index].m_player->objnum].type == OBJ_SHIP))
    {
      ...
      hull_integrity =
        (int) (((float) objp->hull_strength /
                (float) Ships[objp->instance].ship_max_hull_strength) *
               100.0f);
      if (hull_integrity < 0)
        {
          hull_integrity = 0;
  ...
  if (objp == NULL)
    {
      emp_hud_printf(x + current_hud->Escort_integrity[0] - stat_shift,
                     y + current_hud->Escort_integrity[1], EG_NULL, "%d",
                     Net_players[np_index].m_player->stats.m_kill_count_ok);
    }
  else
    {
      emp_hud_printf(x + current_hud->Escort_integrity[0] - stat_shift,
                     y + current_hud->Escort_integrity[1], EG_NULL,
                     "(%d%%) %d", hull_integrity,
                     Net_players[np_index].m_player->stats.m_kill_count_ok);
    }
}
...
void hud_escort_show_icon(int x, int y, int index)
{
  ...
  if ((sp->flags & SF_DISABLED)
    ...
    {
      emp_hud_string(x + current_hud->Escort_status[0],
                     y + current_hud->Escort_status[1], EG_NULL, XSTR("D",
                                                                      284));
    }
  ...
  if (sp->callsign_index >= 0)
    ...
    if (((Iff_info[sp->team].flags & IFFF_WING_NAME_HIDDEN)
         && (sp->wingnum != -1)) || (sp->flags2 & SF2_HIDE_SHIP_NAME))
    {
  ...
  emp_hud_printf(x + current_hud->Escort_integrity[0] + offset,
                 y + current_hud->Escort_integrity[1], EG_NULL, "%d",
                 screen_integrity);
...
void hud_display_escort()
{
  ...
  hud_aabitmap(Escort_gauges[0].first_frame, current_hud->Escort_coords[0],
               current_hud->Escort_coords[1]);
  gr_string(current_hud->Escort_htext_coords[0],
            current_hud->Escort_htext_coords[1], current_hud->Escort_htext);
