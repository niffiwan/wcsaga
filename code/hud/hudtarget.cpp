...
int TARGET_SHIP_IGNORE_FLAGS =
  (SF_EXPLODED | SF_DEPART_WARP | SF_DYING | SF_ARRIVING_STAGE_1 |
...
int Weapon_gauge_primary_coords[NUM_HUD_SETTINGS][GR_NUM_RESOLUTIONS][3][2] = {
  {
   // normal HUD
   {
    ...
    {
     509,
     273},                      // top of weapon gauge, first frame, always
    {
     497,
     293},                      // for the first primary
    {
     497,
     305}                       // for the second primary
    },
   {
    // GR_1024
    // based on the # of primaries
    {
     892,
     489},                      // top of weapon gauge, first frame, always
    {
     880,
     509},                      // for the first primary
    {
     880,
     521}                       // for the second primary
    }
   },
  {
...
int Weapon_gauge_secondary_coords[NUM_HUD_SETTINGS][GR_NUM_RESOLUTIONS][5][2]
  = {
  {
   // normal HUD
   {
    ...
    {
     497,
     318},                      // bottom of gauge, 0 secondaries
    {
     497,
     318},                      // bottom of gauge, 1 secondaries
    {
     497,
    ...
    {
     497,
     335}                       // bottom of gauge, 3 secondaries
    },
   {
    // GR_1024
    // based on the # of secondaries
    {
     880,
     534},                      // bottom of gauge, 0 secondaries
    {
     880,
     534},                      // bottom of gauge, 1 secondaries
    {
     880,
     533},                      // middle of gauge, 2 secondaries AND middle of gauge, 3 secondaries
    {
     880,
     542},                      // bottom of gauge, 2 secondaries AND middle of gauge, 3 secondaries
    {
     880,
     551}                       // bottom of gauge, 3 secondaries
    }
   },
  {
   // ballistic HUD - slightly different alignment
...
int Weapon_plink_coords[GR_NUM_RESOLUTIONS][2][2] = {
  {
   ...
   {
    530,
    285},                       // fire-linked thingie, for the first primary
   {
    530,
    295}                        // fire-linked thingie, for the second primary
   },
  {
   // GR_1024
   {
    913,
    501},                       // fire-linked thingie, for the first primary
   {
    913,
    511}                        // fire-linked thingie, for the second primary
   }
};
...
int Weapon_pname_coords[GR_NUM_RESOLUTIONS][2][2] = {
  {
   ...
   {
    536,
    285},                       // weapon name, first primary
   {
    536,
    295}                        // weapon name, second primary
   },
  {
   // GR_1024
   {
    919,
    501},                       // weapon name, first primary
   {
    919,
    511}                        // weapon name, second primary
   }
};
...
char
  *Weapon_gauge_fnames[NUM_HUD_SETTINGS][GR_NUM_RESOLUTIONS]
  [NUM_WEAPON_GAUGES] = {
  //XSTR:OFF
  {
   // normal HUD
...
void hud_stuff_reticle_list(reticle_list * rl, object * objp, float measure,
                            int dot_flag)
{
  if (dot_flag)
    {
...
void hud_make_shader(shader * sh, int r, int g, int b, float dimmer = 1000.0f)
{
  ...
  A =
    static_cast < ubyte >
    (fl2i((i2fl(r) / dimmer) * (i2fl(HUD_color_alpha) / 15.0f) * 255.0f));
...
{
  Aburn_bar_gauge.first_frame =
    bm_load_animation(current_hud->Aburn_fname, &Aburn_bar_gauge.num_frames);
  ...
  if (Aburn_bar_gauge.first_frame < 0)
    ...
    {
      bm_page_in_aabitmap(Aburn_bar_gauge.first_frame,
                          Aburn_bar_gauge.num_frames);
    }

  Wenergy_bar_gauge.first_frame =
    bm_load_animation(current_hud->Wenergy_fname,
                      &Wenergy_bar_gauge.num_frames);
  ...
  if (Wenergy_bar_gauge.first_frame < 0)
    ...
    {
      bm_page_in_aabitmap(Wenergy_bar_gauge.first_frame,
                          Wenergy_bar_gauge.num_frames);
    }
}
  ...
  if (!Weapon_gauges_loaded)
    {
      ...
      for (hud_index = 0; hud_index < NUM_HUD_SETTINGS; hud_index++)
        {
          for (gauge_index = 0; gauge_index < NUM_WEAPON_GAUGES;
               gauge_index++)
            {
              Weapon_gauges[hud_index][gauge_index].first_frame =
                bm_load_animation(Weapon_gauge_fnames[hud_index]
                                  [gr_screen.res][gauge_index],
                                  &Weapon_gauges[hud_index][gauge_index].
                                  num_frames);
                  ...
                  if (hud_index)
                    {
                      ...
                      Weapon_gauges[hud_index][gauge_index].first_frame =
                        bm_load_animation(Weapon_gauge_fnames[0]
                                          [gr_screen.res][gauge_index],
                                          &Weapon_gauges[hud_index]
                                          [gauge_index].num_frames);

                      // and test again
                      if (Weapon_gauges[hud_index][gauge_index].first_frame <
                          0)
                        {
                          Warning(LOCATION, "Cannot load hud ani: %s\n",
                                  Weapon_gauge_fnames[hud_index][gr_screen.
                                                                 res]
                                  [gauge_index]);
                        }
                    }
                  ...
                  for (k = 0; k < 2; k++)
                    {
                      for (j = 0; j < 3; j++)
                        {
                          Weapon_gauge_primary_coords[hud_index][i][j][k] =
                            Weapon_gauge_primary_coords[0][i][j][k];
                        }
                      for (j = 0; j < 5; j++)
                        {
                          Weapon_gauge_secondary_coords[hud_index][i][j][k] =
                            Weapon_gauge_secondary_coords[0][i][j][k];
                        }
                      Weapon_title_coords[hud_index][i][k] =
                        Weapon_title_coords[0][i][k];
                    }
                }
            }
        }
...
void hud_init_targeting()
{
  ...
  if (!Lead_indicator_gauge_loaded)
    {
      Lead_indicator_gauge.first_frame =
        bm_load_animation(Lead_fname[Hud_reticle_style][gr_screen.res],
  ...
  if (!Toggle_gauge_loaded)
    {
      Toggle_gauge.first_frame =
        bm_load_animation(Toggle_fname[gr_screen.res],
  ...
  if (!Cmeasure_gauge_loaded)
    {
      Cmeasure_gauge.first_frame =
        bm_load_animation(Cm_fname[gr_screen.res],
                          &Cmeasure_gauge.num_frames);
  ...
  for (A = start2; A != start; A = advance_fb(A, next_flag))
    {
      ...
      if (A->type == OBJ_WEAPON)
        {
          if (!
              (Weapon_info[Weapons[A->instance].weapon_info_index].
               wi_flags2 & WIF2_CAN_BE_TARGETED))
            if (!
                (Weapon_info[Weapons[A->instance].weapon_info_index].
                 wi_flags & WIF_BOMB))
              continue;
      ...
      if (target_objp->type == OBJ_WEAPON
          ...
          subtype == WP_MISSILE)
        {
          // must be a missile
            ...
            missile_obj_return_address(Weapons[target_objp->instance].
                                       missile_list_index);
        }
    }
...
int hud_target_ship_can_be_scanned(ship * shipp)
{
  ...
  if ((sip->class_type < 0)
      || !(Ship_types[sip->class_type].ship_bools & STI_SHIP_SCANNABLE))
    return 0;
...
void hud_target_live_turret(int next_flag, int auto_advance,
                            int only_player_target)
{
                  ...
                  || A->weapons.num_secondary_banks > 0))
            {
              if (!only_player_target
                  || (A->turret_enemy_objnum == OBJ_INDEX(Player_obj)))
                {
                  ...
                  distance =
                    vm_vec_normalized_dir(&vec_to_subsys, &gsubpos,
                                          &View_position);

                  // check if facing and in view
                  int facing =
                    ship_subsystem_in_sight(objp, A, &View_position, &gsubpos,
                                            0);
                  ...
                  if (turret_weapon_has_flags(&A->weapons, WIF_BEAM))
                  ...
                  else
                    {
                      if (turret_weapon_has_subtype(&A->weapons, WP_MISSILE))
                        ...
                        if (turret_weapon_has_subtype(&A->weapons, WP_LASER))
                        {
  ...
  if (!use_straight_ahead_turret)
    {
      insertion_sort(ent, num_live_turrets, sizeof(eval_next_turret),
                     turret_compare_func);
    }
...
float hud_find_target_distance(object * targetee, object * targeter)
{
  ...
  if (model_num > -1)
    {
      dist =
        model_find_closest_point(&tmp_pnt, model_num, -1, &targetee->orient,
                                 &targetee->pos, &targeter->pos);
    }
  else
    {
      // Old way, that uses radius.
      dist =
        vm_vec_dist_quick(&targetee->pos, &targeter->pos) - targetee->radius;
      if (dist < 0.0f)
        {
          dist = 0.0f;
        }
...
void evaluate_ship_as_closest_target(esct * esct)
{
  ...
  // bail if harmless
  if (Ship_info[esct->shipp->ship_info_index].class_type > -1
      && !(Ship_types[Ship_info[esct->shipp->ship_info_index].class_type].
           hud_bools & STI_HUD_TARGET_AS_THREAT))
    {
      return;
    }
  ...
  if (esct->filter)
    {
      if (!
          (Ship_info[esct->shipp->ship_info_index].
  ...
  if (Ship_info[esct->shipp->ship_info_index].
      flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP))
    {
...
int hud_target_closest(int team_mask, int attacked_objnum, int play_fail_snd,
                       int filter, int get_closest_turret_attacking_player)
{
  object *A;
  ...
  if ((attacked_objnum >= 0) && (attacked_objnum != player_obj_index))
    {
      ...
      if (Ships[Objects[attacked_objnum].instance].
...
void hud_target_in_reticle_new()
{
  ...
  vm_vec_scale_add(&terminus, &Eye_position, &Player_obj->orient.vec.fvec,
        ...
        case OBJ_ASTEROID:
          {
            ...
            mc.model_num =
              Asteroid_info[Asteroids[A->instance].asteroid_type].
              model_num[pof];
          }
          break;
        case OBJ_JUMP_NODE:
...
void hud_target_subsystem_in_reticle()
{
  ...
  if (nearest_subsys != NULL)
    {
      set_targeted_subsys(Player_ai, nearest_subsys,
                          Player_ai->target_objnum);
      ...
      HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                         XSTR("Targeting subsystem %s.", 323), r_name);
...
//  On entry:
//      color set
void hud_render_orientation_tee(object * from_objp, object * to_objp,
                                matrix * from_orientp)
{
  float dot_product;
  vec3d target_to_obj;
  ...
  y1 =
    (float) sin(dot_product) * (Outer_circle_radius[gr_screen.res] -
                                T_OFFSET_FROM_CIRCLE);
  x1 =
    (float) cos(dot_product) * (Outer_circle_radius[gr_screen.res] -
                                T_OFFSET_FROM_CIRCLE);
...
void hud_render_tail_missile_triangle(float ang, float xpos, float ypos,
                                      float cur_dist, int draw_solid,
                                      int draw_inside)
{
  float x1 = 0.0f;
  float x2 = 0.0f;
...
void hud_render_split_missile_triangle(float ang, float xpos, float ypos,
                                       float cur_dist, int draw_solid,
                                       int draw_inside)
{
  ...
  if (cur_dist < Min_warning_missile_dist)
    ...
    {
      triangle_sep =
        (cur_dist / Max_warning_missile_dist) *
        (Max_offscreen_tri_seperation[gr_screen.res] +
         Max_front_seperation[gr_screen.res]);
    }
...
void hud_render_triangle(vec3d * hostile_pos, int aspect_flag,
                         int show_interior, int split_tri)
{
  ...
  if (hostile_vertex.codes == 0)
    {
      ...
      if (!(hostile_vertex.flags & PF_OVERFLOW))
        {
          // make sure point projected
          float mag_squared;
          ...
          mag_squared =
            (projected_x -
             Hud_reticle_center[gr_screen.res][0]) * (projected_x -
                                                      Hud_reticle_center
                                                      [gr_screen.res][0]) +
            (projected_y -
             Hud_reticle_center[gr_screen.res][1]) * (projected_y -
                                                      Hud_reticle_center
                                                      [gr_screen.res][1]);
  ...
  if (draw_inside)
    {
      xpos =
        Hud_reticle_center[gr_screen.res][0] +
      ...
      if (wp->homing_object == Player_obj)
        {
          hud_render_triangle(&A->pos,
                              ...
                              wi_flags & WIF_LOCKED_HOMING, 1, 1);
  ...
  if (nearest_dist < Homing_beep.max_cycle_dist)
    {
      ...
      cycle_time =
        Homing_beep.min_cycle_time + (nearest_dist -
                                      Homing_beep.min_cycle_dist) *
          ...
          if (closest_is_aspect)
            {
              Homing_beep.snd_handle =
                snd_play(&Snds[SND_PROXIMITY_ASPECT_WARNING], 0.0f, 1.0f,
                         SND_PRIORITY_MUST_PLAY);
            }
          else
            {
              Homing_beep.snd_handle =
                snd_play(&Snds[SND_PROXIMITY_WARNING], 0.0f, 1.0f,
                         SND_PRIORITY_MUST_PLAY);
            }
        }
    }
              ...
              if (!(target_point.flags & PF_OVERFLOW))
                {
                  ...
                  switch (mobjp->type)
                    ...
                    case OBJ_WEAPON:
                      modelnum =
                        Weapon_info[Weapons[mobjp->instance].
                                    weapon_info_index].model_num;
                      bound_rval =
                        model_find_2d_bound_min(modelnum, &mobjp->orient,
                                                &mobjp->pos, &x1, &y1, &x2,
                                                &y2);
                      break;
                  ...
                  if (bound_rval == 0)
                    {
                      ...
                      draw_bounding_brackets(x1 - 5, y1 - 5, x2 + 5, y2 + 5,
                                             0, 0, distance,
                                             OBJ_INDEX(mobjp));
                    }
  ...
  if (hud_gauge_active(HUD_OFFSCREEN_INDICATOR))
    {
      if (target_point.codes != 0)
        {
          // target center is not on screen
          ...
          if ((OBJ_INDEX(targetp) != Player_ai->target_objnum)
              || (Message_shipnum ==
                  Objects[Player_ai->target_objnum].instance))
            {
      ...
      if (!(target_point.flags & PF_OVERFLOW))
        {
          ...
          switch (targetp->type)
            ...
            case OBJ_SHIP:
              modelnum = Ship_info[target_shipp->ship_info_index].model_num;
              bound_rval =
                model_find_2d_bound_min(modelnum, &targetp->orient,
          ...
          if (bound_rval == 0)
            {
              ...
              if (OBJ_INDEX(targetp) == Player_ai->target_objnum)
                {
                  draw_bounding_brackets(x1 - 5, y1 - 5, x2 + 5, y2 + 5, 5,
                                         5);
                  HUD_drew_selection_bracket_on_target = 1;
                }
              else if (Cmdline_targetinfo)
                {
                  //Backslash -- show the distance and a lead indicator
              ...
              if (OBJ_INDEX(targetp) != Player_ai->target_objnum)
                {
                  if (hud_sensors_ok(Player_ship, 0))
                    {
                      float dist;
                      gr_set_color_fast(iff_get_color
                                        (IFF_COLOR_SELECTION, 1));
                      //dist = vm_vec_dist_quick(&Player_obj->pos, &targetp->pos);
                      dist = hud_find_target_distance(targetp, Player_obj);
                      hud_draw_offscreen_indicator(&target_point,
                                                   &targetp->pos, dist);
                    }
                }
            }
      ...
      if (draw_box == TRUE)
        {
          ...
          hud_set_iff_color(targetp, 1);
          draw_bounding_brackets(x1 - 5, y1 - 5, x2 + 5, y2 + 5, 0, 0,
                                 distance, OBJ_INDEX(targetp));
        }
...
void hud_update_target_in_reticle(vertex * projected_v)
{
  ...
  mag_squared =
    (sx - Hud_reticle_center[gr_screen.res][0]) * (sx -
                                                   Hud_reticle_center
                                                   [gr_screen.res][0]) + (sy -
                                                                          Hud_reticle_center
                                                                          [gr_screen.
                                                                           res]
                                                                          [1])
    * (sy - Hud_reticle_center[gr_screen.res][1]);
...
void hud_show_targeting_gauges(float frametime, int in_cockpit)
{
  ...
  if (Player_ai->targeted_subsys != NULL)
    {
      get_subsystem_world_pos(targetp, Player_ai->targeted_subsys,
                              &target_pos);
  ...
  for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
       so = GET_NEXT(so))
    {
      ...
      if (Ship_info[sp->ship_info_index].class_type > -1
          && !(Ship_types[Ship_info[sp->ship_info_index].class_type].hud_bools
               & STI_HUD_SHOW_ATTACK_DIRECTION))
        {
          continue;
        }
      ...
      if (!
          (Ship_info[sp->ship_info_index].flags & (SIF_FIGHTER | SIF_BOMBER)))
        {
...
//
void polish_predicted_target_pos(weapon_info * wip, object * targetp,
                                 vec3d * enemy_pos,
                                 vec3d * predicted_enemy_pos,
                                 float dist_to_enemy, vec3d * last_delta_vec,
...
int hudtarget_lead_indicator_pick_frame(float prange, float srange,
...
void hud_show_lead_indicator(vec3d * target_world_pos)
{
  ...
  if (swp->current_secondary_bank >= 0)
    {
      ...
      if (!(tmp->wi_flags & WIF_HOMING)
          && !(tmp->wi_flags & WIF_LOCKED_HOMING
  ...
  frame_offset =
    hudtarget_lead_indicator_pick_frame(prange, srange, dist_to_target);
                      ...
                      if (indicator_frame >= 0)
                        {
                          ...
                          float reticle2_target_sx = target_vertex.sx -
                            Lead_indicator_half[Hud_reticle_style][gr_screen.
                                                                   res][0] -
                            lead_target_vertex.sx;
                          float reticle2_target_sy =
                            target_vertex.sy -
                            Lead_indicator_half[Hud_reticle_style][gr_screen.
                                                                   res][1] -
                          ...
                          reticle2_target_sx +=
                            HUD_offset_x +
                            Hud_reticle_center[gr_screen.res][0] + 0.5f;
                          reticle2_target_sy +=
                            HUD_offset_y +
                            Hud_reticle_center[gr_screen.res][1] + 0.5f;

                          hud_aabitmap(indicator_frame,
                                       fl2i(reticle2_target_sx) + HUD_nose_x,
              ...
              if (indicator_frame >= 0)
                {
                  ...
                  hud_aabitmap(indicator_frame,
                               fl2i(sx -
                                    Lead_indicator_half[Hud_reticle_style]
                                    [gr_screen.res][0]),
                               fl2i(sy -
                                    Lead_indicator_half[Hud_reticle_style]
                                    [gr_screen.res][1]));
...
void hud_show_lead_indicator_quick(vec3d * target_world_pos, object * targetp)
{
  ...
  frame_offset =
    hudtarget_lead_indicator_pick_frame(prange, -1.0f, dist_to_target);
...
//
vec3d *get_subsystem_world_pos(object * parent_obj, ship_subsys * subsys,
                               vec3d * world_pos)
{
  if (subsys == NULL)
    {
      ...
      if (Player_ai->target_objnum != -1)
        {
          if (Objects[Player_ai->target_objnum].type == OBJ_SHIP)
            {
              hud_restore_subsystem_target(&Ships
...
//
void hud_draw_offscreen_indicator(vertex * target_point, vec3d * tpos,
                                  float distance, int draw_solid)
{
  char buf[32];
  int w = 0, h = 0;
  ...
  if (dist_behind < 0)
    {
      // still in front of player, but not in view
      in_front = 1;
    ...
    {
      triangle_sep =
        dist_behind * Max_offscreen_tri_seperation[gr_screen.res] +
        Max_offscreen_tri_seperation[gr_screen.res];
    }
...
void hud_show_afterburner_gauge()
{
  ...
  if (clip_h > 0)
    {
      hud_aabitmap_ex(Aburn_bar_gauge.first_frame,
                      current_hud->Aburn_coords[0] + nose_offset_x,
                      current_hud->Aburn_coords[1] + nose_offset_y, w, clip_h,
                      0, 0);
    }

  if (clip_h <= current_hud->Aburn_size[0])
    {
      hud_aabitmap_ex(Aburn_bar_gauge.first_frame + 1,
                      current_hud->Aburn_coords[0] + nose_offset_x,
                      current_hud->Aburn_coords[1] + clip_h + nose_offset_y,
                      w, h - clip_h, 0, clip_h);
    }
}
  ...
  if (use_new_gauge)
    {
      ...
      remaining =
        max_w * ((float) Player_ship->weapon_energy /
                 (float) Ship_info[Player_ship->
                                   ship_info_index].max_weapon_reserve);
      if (remaining > 0)
        {
          hud_set_gauge_color(HUD_WEAPONS_ENERGY, HUD_C_BRIGHT);
          for (y = 0; y < 10; y++)
            {
              gr_gradient(currentx, currenty + y, currentx + fl2i(remaining),
                          currenty + y);
            }
        }
      currenty += 12;
      ...
      for (x = 0; x < Player_ship->weapons.num_primary_banks; x++)
        {
          ...
          if (gr_screen.max_w_unscaled == 640)
            ...
            {
              gr_string(currentx, currenty,
                        Weapon_info[Player_ship->weapons.
                                    primary_bank_weapons[x]].name);
            }
          ...
          remaining =
            (max_w -
             4) * ((float) Player_ship->weapons.primary_bank_ammo[x] /
  ...
  else
    {
      ...
      if (percent_left <= 0.3)
        {
          ...
          gr_string(current_hud->Wenergy_text_coords[0] + nose_offset_x,
                    current_hud->Wenergy_text_coords[1] + nose_offset_y, buf);
        }
      ...
      clip_h =
        fl2i((1.0f - percent_left) * current_hud->Wenergy_size[0] + 0.5f);

      bm_get_info(Wenergy_bar_gauge.first_frame + 2, &w, &h);

      if (clip_h > 0)
        {
          hud_aabitmap_ex(Wenergy_bar_gauge.first_frame + 2,
                          current_hud->Wenergy_coords[0] + nose_offset_x,
                          current_hud->Wenergy_coords[1] + nose_offset_y, w,
                          clip_h, 0, 0);
        }

      if (clip_h <= current_hud->Wenergy_size[0])
        {
          hud_aabitmap_ex(Wenergy_bar_gauge.first_frame + 3,
                          current_hud->Wenergy_coords[0] + nose_offset_x,
                          current_hud->Wenergy_coords[1] + clip_h +
                          nose_offset_y, w, h - clip_h, 0, clip_h);
        }
...
void hud_show_cmeasure_gauge()
{
  ...
  // blit the background
  hud_aabitmap(Cmeasure_gauge.first_frame, Cm_coords[gr_screen.res][0],
               Cm_coords[gr_screen.res][1]);

  // blit text
  gr_string(Cm_text_coords[gr_screen.res][0],
            Cm_text_coords[gr_screen.res][1], XSTR("cm.", 327));
  if (!Player_ship)
    {
...
void hud_show_weapons()
{
  ...
  hud_aabitmap(Weapon_gauges[ballistic_hud_index][0].first_frame,
               Weapon_gauge_primary_coords[ballistic_hud_index][gr_screen.
                                                                res][0][0],
               Weapon_gauge_primary_coords[ballistic_hud_index][gr_screen.
                                                                res][0][1]);

  emp_hud_string(Weapon_title_coords[ballistic_hud_index][gr_screen.res][0],
                 Weapon_title_coords[ballistic_hud_index][gr_screen.res][1],
  ...
  for (i = 0; i < np; i++)
    {
      ...
      if (i == 1)
        {
          hud_aabitmap(Weapon_gauges[ballistic_hud_index][1].first_frame,
                       Weapon_gauge_primary_coords[ballistic_hud_index]
                       [gr_screen.res][1][0], y);
      ...
      else if (i != 0)
        {
          if (New_weapon.first_frame != -1)
            hud_aabitmap(New_weapon.first_frame,
                         Weapon_gauge_primary_coords[ballistic_hud_index]
                         [gr_screen.res][1][0], y);
        }
      ...
      strcpy_s(name,
               (Weapon_info[sw->primary_bank_weapons[i]].
                alt_name[0]) ? Weapon_info[sw->primary_bank_weapons[i]].
               alt_name : Weapon_info[sw->primary_bank_weapons[i]].name);
      if (Lcl_gr)
        {
      ...
      if ((sw->current_primary_bank == i)
          || (Player_ship->flags & SF_PRIMARY_LINKED))
        {
          emp_hud_printf(Weapon_plink_coords[gr_screen.res][0][0], name_y,
                         EG_NULL, "%c", Lcl_special_chars + 2);
        }

      if (Weapon_info[sw->primary_bank_weapons[0]].hud_image_index != -1)
        {
          hud_aabitmap(Weapon_info[sw->primary_bank_weapons[i]].
                       hud_image_index,
                       Weapon_pname_coords[gr_screen.res][0][0], name_y);
        }
      else
        {
          emp_hud_printf(Weapon_pname_coords[gr_screen.res][0][0], name_y,
                         EG_WEAPON_P2, "%s", name);
        }
      ...
      if (Weapon_info[sw->primary_bank_weapons[i]].wi_flags2 & WIF2_BALLISTIC)
        {
          ...
          emp_hud_string(Weapon_primary_ammo_x[gr_screen.res] - w, name_y,
                         EG_NULL, ammo_str);
        }
      if (i != 0)
        y += res_diff;
  ...
  for (i = 0; i < ns; i++)
    {
      ...
      if (i != 0)
        hud_aabitmap(Weapon_gauges[ballistic_hud_index][3].first_frame,
                     Weapon_gauge_secondary_coords[ballistic_hud_index]
                     [gr_screen.res][3][0], y);
      ...
      if (!stricmp(wip->name, "cluster bomb"))
        ...
        {
          strcpy_s(weapon_name,
                   (wip->alt_name[0]) ? wip->alt_name : wip->name);
        }
      ...
      if (sw->current_secondary_bank == i)
        {
          emp_hud_printf(Weapon_sunlinked_x[gr_screen.res], name_y, EG_NULL,
                         "%c", Lcl_special_chars + 2);

          if (Player_ship->flags & SF_SECONDARY_DUAL_FIRE)
            {
              emp_hud_printf(Weapon_slinked_x[gr_screen.res], name_y, EG_NULL,
                             "%c", Lcl_special_chars + 2);
            }

          if (wip->hud_image_index != -1)
            {
              hud_aabitmap(wip->hud_image_index,
                           Weapon_secondary_name_x[gr_screen.res], name_y);
            }
          else
            {
              emp_hud_string(Weapon_secondary_name_x[gr_screen.res], name_y,
      ...
      emp_hud_string(Weapon_secondary_ammo_x[gr_screen.res] - w, name_y,
                     EG_NULL, ammo_str);
  ...
  if (ns == 0)
    {
      emp_hud_string(Weapon_pname_coords[gr_screen.res][0][0], name_y,
  ...
  hud_aabitmap(Weapon_gauges[ballistic_hud_index][4].first_frame,
               Weapon_gauge_secondary_coords[ballistic_hud_index][gr_screen.
                                                                  res][0][0],
               y);
...
void hud_target_next_list(int hostile, int next_flag)
{
  ...
  cur_dist =
    hud_find_target_distance(&Objects[Player_ai->target_objnum], Player_obj);
...
void hud_auto_target_icon()
{
  ...
  hud_aabitmap(Toggle_gauge.first_frame + frame_offset,
...
void hud_target_last_transmit()
{
  ...
  if ((targeted_objnum >= 0) && (targeted_objnum < MAX_OBJECTS))
    {
      set_target_objnum(Player_ai,
                        Transmit_target_list[transmit_index].objnum);
      hud_restore_subsystem_target(&Ships
                                   [Objects
                                    [Transmit_target_list[transmit_index].
                                     objnum].instance]);
    }
}
...
void hud_stuff_ship_name(char *ship_name_text, ship * shipp)
{
  ...
  if (((Iff_info[shipp->team].flags & IFFF_WING_NAME_HIDDEN)
       && (shipp->wingnum != -1)) || (shipp->flags2 & SF2_HIDE_SHIP_NAME))
    {
      *ship_name_text = 0;
    }
  else
  ...
  else
    {
      *ship_callsign_text = 0;
      if (shipp->callsign_index >= 0)
        {
          mission_parse_lookup_callsign_index(shipp->callsign_index,
                                              ship_callsign_text);
        }
    }
...
void hud_stuff_ship_class(char *ship_class_text, ship * shipp)
{
  ...
  else
    {
      *ship_class_text = 0;
      if (shipp->alt_type_index >= 0)
        {
          mission_parse_lookup_alt_index(shipp->alt_type_index,
                                         ship_class_text);
        }
    }
  ...
  if (!*ship_class_text)
    {
      strcpy(ship_class_text,
             (Ship_info[shipp->ship_info_index].
              alt_name[0]) ? Ship_info[shipp->
                                       ship_info_index].alt_name :
