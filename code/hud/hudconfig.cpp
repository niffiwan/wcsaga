...
int HUD_observer_default_flags = {
  (1 << HUD_CENTER_RETICLE) |
    ...
    (1 << HUD_TARGET_MINI_ICON) | (1 << HUD_TARGET_MONITOR)
};
...
struct HC_gauge_region
{
  ...
  UI_BUTTON button;
    ...
    HC_gauge_region(char *name, int x1, int y1, int h, int iff, int cp, int b,
                    int nf, int cl):filename(name), x(x1), y(y1), hotspot(h),
    use_iff(iff), can_popup(cp), bitmap(b), nframes(nf), color(cl)
  {
  }
};
...
hc_col HC_colors[3] = {
  {
   0,
   255,
   0},                          // Green - get RGB from Adam so it matches palette?-??.pcx
  {
   67,
   123,
   203},                        // Blue - get RGB from Adam so it matches palette?-??.pcx
  {
   255,
   197,
   0},                          // Amber - get RGB from Adam so it matches palette?-??.pcx
};
...
void hud_config_synch_sliders(int i)
{
  if (i >= 0)
    {
      HC_color_sliders[HCS_RED].
        force_currentItem(HCS_CONV(HUD_config.clr[i].red));
      HC_color_sliders[HCS_GREEN].
        force_currentItem(HCS_CONV(HUD_config.clr[i].green));
      HC_color_sliders[HCS_BLUE].
        force_currentItem(HCS_CONV(HUD_config.clr[i].blue));
      HC_color_sliders[HCS_ALPHA].
        force_currentItem(HCS_CONV(HUD_config.clr[i].alpha));
    }
}
...
void hud_config_init_ui()
{
  ...
  HC_ui_window.create(0, 0, gr_screen.max_w_unscaled,
  ...
  HC_color_sliders[HCS_RED].create(&HC_ui_window,
                                   ...
                                   HC_slider_coords[gr_screen.
                                                    res][HCS_RED][3], 255,
                                   HC_slider_fname[gr_screen.res],
                                   hud_config_red_slider,
                                   hud_config_red_slider,
                                   hud_config_red_slider);

  HC_color_sliders[HCS_GREEN].create(&HC_ui_window,
                                     HC_slider_coords[gr_screen.
                                                      res][HCS_GREEN][0],
                                     HC_slider_coords[gr_screen.
                                                      res][HCS_GREEN][1],
  ...
  HC_color_sliders[HCS_BLUE].create(&HC_ui_window,
                                    HC_slider_coords[gr_screen.
                                                     res][HCS_BLUE][0],
                                    HC_slider_coords[gr_screen.
                                                     res][HCS_BLUE][1],
  ...
  HC_color_sliders[HCS_ALPHA].create(&HC_ui_window,
                                     HC_slider_coords[gr_screen.
                                                      res][HCS_ALPHA][0],
                                     HC_slider_coords[gr_screen.
                                                      res][HCS_ALPHA][1],
    ...
    case HCB_RED_UP:
      if (HCS_CONV(HC_color_sliders[HCS_RED].get_currentItem()) >= 255)
      ...
      else
        {
          gamesnd_play_iface(SND_USER_SELECT);
          HC_color_sliders[HCS_RED].
            force_currentItem(HCS_CONV
                              (HCS_CONV
                               (HC_color_sliders[HCS_RED].get_currentItem()) +
                               1));
          hud_config_red_slider();
        }
      break;
  ...
  if (HC_gauge_selected >= 0)
    {
      ...
      sx =
        fl2i(HC_gauge_description_coords[gr_screen.res][0] +
...
void hud_config_alpha_slider_up()
{
  ...
  if (HC_select_all)
    {
      hud_config_stuff_colors(HCS_CONV
                              (HC_color_sliders[HCS_RED].get_currentItem()),
                              HCS_CONV(HC_color_sliders[HCS_GREEN].
                                       get_currentItem()),
                              HCS_CONV(HC_color_sliders[HCS_BLUE].
                                       get_currentItem()));
    }
  else
    {
      gr_init_alphacolor(&HUD_config.clr[HC_gauge_selected],
                         HCS_CONV(HC_color_sliders[HCS_RED].
                                  get_currentItem()),
                         HCS_CONV(HC_color_sliders[HCS_GREEN].
                                  get_currentItem()),
                         HCS_CONV(HC_color_sliders[HCS_BLUE].
...
void hud_config_color_init()
{
  ...
  HC_num_files =
    cf_get_file_list(MAX_HCF_FILES, HC_filenames, CF_TYPE_PLAYERS, "*.hcf",
                     CF_SORT_NAME);
}
