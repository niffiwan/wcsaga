...
char
  Reticle_frame_names[NUM_HUD_RETICLE_STYLES][GR_NUM_RESOLUTIONS]
...
int
  Reticle_frame_coords[NUM_HUD_RETICLE_STYLES][GR_NUM_RESOLUTIONS]
  [NUM_RETICLE_ANIS][2] = {
  {
   {
    // GR_640
...
void hud_render_throttle_background(int y_end)
{
  ...
  x =
    Reticle_frame_coords[Hud_reticle_style][gr_screen.
                                            res][RETICLE_LEFT_ARC][0];
  y =
    Reticle_frame_coords[Hud_reticle_style][gr_screen.
  ...
  if (y_end > y)
    {
      hud_aabitmap_ex(Reticle_gauges[RETICLE_LEFT_ARC].first_frame + 1,
                      x + HUD_nose_x, y + HUD_nose_y, w, y_end - y + 1, 0, 0);
    }
}
...
void hud_render_throttle_foreground(int y_end)
{
  ...
  if (y_end < (y + h - 1))
    {
      hud_aabitmap_ex(Reticle_gauges[RETICLE_LEFT_ARC].first_frame + 2,
                      x + HUD_nose_x, y_end + HUD_nose_y, w, h - (y_end - y),
                      0, y_end - y);
    }
}
  ...
  x_pos =
    ...
    sqrt(double
         (Outer_circle_radius[gr_screen.res] *
          Outer_circle_radius[gr_screen.res] - y_pos * y_pos));
  ...
  if (object_get_gliding(Player_obj))
  ...
  else if (Players[Player_num].flags & PLAYER_FLAGS_MATCH_TARGET)
    {
      ...
      if (Lcl_gr)
        ...
        {
          gr_printf(sx + offset + HUD_nose_x, sy + h + HUD_nose_y, "%c",
                    Lcl_special_chars + 3);
        }
    }
}
...
void hud_render_throttle_line(int y)
{
  ...
  hud_aabitmap_ex(Reticle_gauges[RETICLE_LEFT_ARC].first_frame + 3,
                  Reticle_frame_coords[Hud_reticle_style][gr_screen.
                                                          res]
                  [RETICLE_LEFT_ARC][0] + HUD_nose_x, y + HUD_nose_y,
                  Hud_throttle_frame_w[gr_screen.res], 1, 0,
                  y -
                  Reticle_frame_coords[Hud_reticle_style][gr_screen.res]
                  [RETICLE_LEFT_ARC][1]);
}
...
void hud_show_throttle()
{
  ...
  if (desired_speed < 0.0f)
    {
      // so ships that go backwards don't force the indicators below where they can go
      desired_speed = 0.0f;
    }

  desired_y_pos =
    Hud_throttle_bottom_y[Hud_reticle_style][gr_screen.res] -
  ...
  if (percent_max > 1)
    {
      percent_max = 1.0f;
      percent_aburn_max =
        (current_speed - max_speed) / (sip->afterburner_max_vel.xyz.z -
                                       max_speed);
  ...
  if (percent_aburn_max > 0)
    {
      y_end -=
        fl2i(percent_aburn_max * Hud_throttle_aburn_h[gr_screen.res] + 0.5f);
    }
  ...
  gr_printf(Max_speed_coords[gr_screen.res][0] + HUD_nose_x,
            Max_speed_coords[gr_screen.res][1] + HUD_nose_y, "%d",
            fl2i(max_displayed_speed + 0.5f));
  gr_printf(Zero_speed_coords[gr_screen.res][0] + HUD_nose_x,
            Zero_speed_coords[gr_screen.res][1] + HUD_nose_y, XSTR("0", 292));
}
...
void hud_show_dumbfire_threat()
{
  ...
  hud_aabitmap(Reticle_gauges[RETICLE_LASER_WARN].first_frame + frame_offset,
               Reticle_frame_coords[Hud_reticle_style][gr_screen.
                                                       res]
               [RETICLE_LASER_WARN][0] + HUD_nose_x,
               Reticle_frame_coords[Hud_reticle_style][gr_screen.
                                                       res]
               [RETICLE_LASER_WARN][1] + HUD_nose_y);
}
...
void hud_show_center_reticle()
{
  ...
  hud_aabitmap(Reticle_gauges[RETICLE_CENTER].first_frame,
               Reticle_frame_coords[Hud_reticle_style][gr_screen.res]
               [RETICLE_CENTER][0] + HUD_nose_x,
               Reticle_frame_coords[Hud_reticle_style][gr_screen.res]
               [RETICLE_CENTER][1] + HUD_nose_y);
}
  ...
  if (hud_gauge_active(HUD_THREAT_GAUGE))
    {
      ...
      hud_aabitmap(Reticle_gauges[RETICLE_TOP_ARC].first_frame + 1,
                   Reticle_frame_coords[Hud_reticle_style][gr_screen.
                                                           res]
                   [RETICLE_TOP_ARC][0] + HUD_nose_x,
                   Reticle_frame_coords[Hud_reticle_style][gr_screen.
                                                           res]
                   [RETICLE_TOP_ARC][1] + HUD_nose_y);
  ...
  else
    {
      ...
      hud_aabitmap(Reticle_gauges[RETICLE_TOP_ARC].first_frame,
                   Reticle_frame_coords[Hud_reticle_style][gr_screen.res]
                   [RETICLE_TOP_ARC][0] + HUD_nose_x,
                   Reticle_frame_coords[Hud_reticle_style][gr_screen.res]
                   [RETICLE_TOP_ARC][1] + HUD_nose_y);
  ...
  if (Hud_reticle_style != HUD_RETICLE_STYLE_FS1)
    {
      hud_aabitmap(Reticle_gauges[RETICLE_RIGHT_ARC].first_frame + 1,
                   Reticle_frame_coords[Hud_reticle_style][gr_screen.
                                                           res]
                   [RETICLE_RIGHT_ARC][0] + HUD_nose_x,
                   Reticle_frame_coords[Hud_reticle_style][gr_screen.
                                                           res]
                   [RETICLE_RIGHT_ARC][1] + HUD_nose_y);
    }
  else
    {
  ...
  if (Hud_reticle_style != HUD_RETICLE_STYLE_FS1)
    {
      // draw left arc (the dark portion of the throttle gauge)
      hud_set_gauge_color(HUD_CENTER_RETICLE);
      hud_aabitmap(Reticle_gauges[RETICLE_LEFT_ARC].first_frame,
                   Reticle_frame_coords[Hud_reticle_style][gr_screen.res]
                   [RETICLE_LEFT_ARC][0] + HUD_nose_x,
                   Reticle_frame_coords[Hud_reticle_style][gr_screen.res]
                   [RETICLE_LEFT_ARC][1] + HUD_nose_y);
    }
