...
void hud_targetbox_init()
{
  if (!Target_view_gauge_loaded)
    {
      Target_view_gauge.first_frame =
        bm_load_animation(Target_view_fname[gr_screen.res],
  ...
  if (!Target_view_integrity_gauge_loaded)
    {
      Target_view_integrity_gauge.first_frame =
        bm_load_animation(Target_integ_fname[gr_screen.res],
  ...
  if (!Target_view_extra_loaded)
    {
      Target_view_extra.first_frame =
        bm_load_animation(Target_extra_fname[gr_screen.res],
...
void hud_render_target_background()
{
  ...
  hud_aabitmap(Target_view_gauge.first_frame,
...
//
void hud_render_target_setup(vec3d * camera_eye, matrix * camera_orient,
                             float zoom)
{
  ...
  if (Keeping_aspect_ratio)
    {
      zoom *=
        (float) Target_window_coords[gr_screen.res][2] / (float) window_width;
    }
...
void hud_blit_target_foreground()
{
  ...
  hud_aabitmap(Target_view_gauge.first_frame + 1,
               Targetbox_coords[gr_screen.res][TBOX_BACKGROUND][0],
               Targetbox_coords[gr_screen.res][TBOX_BACKGROUND][1]);
}
...
void hud_targetbox_show_extra_ship_info(ship * target_shipp,
                                        object * target_objp)
{
  ...
  if (not_training && (hud_gauge_active(HUD_TARGET_MONITOR_EXTRA_DATA))
      && (Targetbox_show_extra_info))
    {
      ...
      bool is_orders_shown_flag =
        (Iff_info[target_shipp->team].flags & IFFF_ORDERS_SHOWN) != 0;
      bool is_orders_hidden_flag =
        (Iff_info[target_shipp->team].flags & IFFF_ORDERS_HIDDEN) != 0;

      if (is_flyable_type && !is_orders_hidden_flag
          && (is_player_team || is_orders_shown_flag))
        {
          extra_data_shown = 1;
          ...
          emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_EXTRA_ORDERS]
                         [0],
                         Targetbox_coords[gr_screen.
                                          res][TBOX_EXTRA_ORDERS][1],
          ...
          if (ship_return_time_to_goal(tmpbuf, target_shipp))
            {
              ...
              emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_EXTRA_TIME]
                             [0],
                             Targetbox_coords[gr_screen.
                                              res][TBOX_EXTRA_TIME][1],
  ...
  if (extra_data_shown)
    {
      ...
      hud_aabitmap(Target_view_extra.first_frame,
  ...
  if (Detail.targetview_model)
    {
      ...
      hud_render_target_setup(&camera_eye, &camera_orient,
                              Viewer_zoom_target);
      model_clear_instance(Asteroid_info[asteroidp->asteroid_type].
                           model_num[pof]);
      ...
      model_render(Asteroid_info[asteroidp->asteroid_type].model_num[pof],
      ...
      if (turret_weapon_has_flags(swp, WIF_BEAM))
      ...
      else
        {
          ...
          if (!turret_weapon_has_subtype(swp, WP_MISSILE)
              && turret_weapon_has_subtype(swp, WP_LASER))
            {
...
void hud_render_target_ship_info(object * target_objp)
{
  ...
  if (Player_ai->targeted_subsys != NULL)
    {
      shield_strength =
        Player_ai->targeted_subsys->current_hits /
      ...
      if (Player_ai->targeted_subsys->system_info->type == SUBSYSTEM_TURRET &&
        ...
        {
          get_turret_subsys_name(&Player_ai->targeted_subsys->weapons,
                                 outstr);
        }
      else
        {
...
void hud_blit_target_integrity(int disabled, int force_obj_num)
{
  ...
  if (disabled)
    {
      hud_aabitmap(Target_view_integrity_gauge.first_frame,
  ...
  clip_h =
    fl2i((1 - Pl_target_integrity) * Integrity_bar_coords[gr_screen.res][3]);
  ...
  emp_hud_string(Integrity_string_coords[gr_screen.res][0],
                 Integrity_string_coords[gr_screen.res][1], EG_TBOX_INTEG,
                 buf);
  ...
  if (clip_h > 0)
    {
      ...
      hud_aabitmap_ex(Target_view_integrity_gauge.first_frame,
                      Integrity_bar_coords[gr_screen.res][0],
                      Integrity_bar_coords[gr_screen.res][1], w, clip_h, 0,
                      0);
    }
  ...
  if (clip_h <= Integrity_bar_coords[gr_screen.res][3])
    {
      ...
      hud_aabitmap_ex(Target_view_integrity_gauge.first_frame + 1,
                      Integrity_bar_coords[gr_screen.res][0],
  ...
  if (subsys != NULL)
    {
      ...
      if (Player->subsys_in_view == -1)
        {
          rval =
            ship_subsystem_in_sight(target_objp, subsys, &View_position,
                                    &subobj_pos, 0);
        }
      else
        {
      ...
      if (target_objp->type == OBJ_SHIP)
        {
          pm =
            model_get(Ship_info[Ships[target_objp->instance].ship_info_index].
                      model_num);
...
void hud_maybe_render_cargo_scan(ship_info * target_sip)
{
  ...
  y1 =
    fl2i(0.5f + Cargo_scan_coords[gr_screen.res][1] +
  ...
  if (Cmdline_dualscanlines)
    {
      ...
      y1 =
        fl2i(Cargo_scan_coords[gr_screen.res][1] +
             Cargo_scan_coords[gr_screen.res][3] -
             ((i2fl(Player->cargo_inspect_time) / scan_time) *
              Cargo_scan_coords[gr_screen.res][3]));
      gr_line(x1, y1, x2, y1);
  ...
  x1 =
    fl2i(0.5f + Cargo_scan_coords[gr_screen.res][0] +
         ((i2fl(Player->cargo_inspect_time) / scan_time) *
  ...
  if (Cmdline_dualscanlines)
    {
      ...
      x1 =
        fl2i(0.5f + Cargo_scan_coords[gr_screen.res][2] +
             Cargo_scan_coords[gr_screen.res][0] -
             ((i2fl(Player->cargo_inspect_time) / scan_time) *
              Cargo_scan_coords[gr_screen.res][2]));
      gr_line(x1, y1 - 3, x1, y2 - 1);
    }
...
void hud_targetbox_get_eye(vec3d * eye_pos, matrix * orient, int ship_num)
{
  ...
  model_find_world_point(eye_pos, &ep->pnt, pm->id, ep->parent, orient,
                         &origin);
}
  ...
  if (Detail.targetview_model)
    {
      ...
      if (Targetbox_wire != 0)
        {
          ...
          if (Highlight_tagged_ships && ship_is_tagged(target_objp))
            model_set_outline_color_fast(iff_get_color
                                         (IFF_COLOR_TAGGED, is_bright));
          else
            {
              model_set_outline_color_fast(iff_get_color_by_team_and_object
                                           (target_shipp->team,
      ...
      if (target_sip->model_num_hud >= 0)
        {
          model_render(target_sip->model_num_hud, &target_objp->orient,
                       &obj_pos,
                       flags | MR_NO_LIGHTING | MR_LOCK_DETAIL | MR_AUTOCENTER
                       | MR_NO_FOGGING);
        }
      else
        {
          model_render(target_sip->model_num, &target_objp->orient, &obj_pos,
                       flags | MR_NO_LIGHTING | MR_LOCK_DETAIL | MR_AUTOCENTER
                       | MR_NO_FOGGING, -1, -1,
...
void hud_render_target_debris(object * target_objp)
{
  ...
  if (Detail.targetview_model)
    {
      ...
      submodel_render(debrisp->model_num, debrisp->submodel_num,
                      &target_objp->orient, &obj_pos,
                      flags | MR_NO_LIGHTING | MR_LOCK_DETAIL |
                      MR_NO_FOGGING);
      hud_render_target_close();
    }
  ...
  if (debrisp->parent_alt_name >= 0)
    mission_parse_lookup_alt_index(debrisp->parent_alt_name,
  ...
  if (Detail.targetview_model)
    {
      ...
      model_render(viewed_model_num, &viewed_obj->orient, &obj_pos,
                   flags | MR_NO_LIGHTING | MR_LOCK_DETAIL | MR_AUTOCENTER |
                   MR_IS_MISSILE | MR_NO_FOGGING, -1, -1,
          ...
          if (hud_gauge_active(HUD_TARGET_MONITOR))
            {
              ...
              emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_CARGO][0] +
                             hx,
                             Targetbox_coords[gr_screen.res][TBOX_CARGO][1] +
                             hy, EG_TBOX_CARGO, outstr);
              hud_set_gauge_color(HUD_TARGET_MONITOR);
            }
        }
    }                           // end if (is_ship)
...
void hud_show_target_data(float frametime)
{
  ...
  current_target_speed =
    vm_vec_dist(&target_objp->pos, &target_objp->last_pos) / frametime;
#endif
  ...
  if (Hud_unit_multiplier > 0.0f)
    {
      // use a different displayed speed scale
  ...
  sprintf(outstr, XSTR("s: %.0f%s", 351), displayed_target_speed,
          (displayed_target_speed >
           1) ? modifiers[Player_ai->current_target_speed_trend] : "");
  hud_num_make_mono(outstr);
          ...
          if (aip->target_objnum != -1)
            {
              ...
              dist =
                vm_vec_dist_quick(&Objects[Player_ai->target_objnum].pos,
              ...
              dot =
                vm_vec_dot(&v2t,
                           &Objects[Player_ai->target_objnum].orient.vec.
                           fvec);
                  ...
                  if (eaip->target_objnum == Player_obj - Objects)
                    {
                      found = 1;
                      dist =
                        vm_vec_dist_quick(&Enemy_attacker->pos,
                                          &Player_obj->pos);
                      vm_vec_normalized_dir(&v2t,
                                            &Objects[eaip->target_objnum].pos,
                                            &Enemy_attacker->pos);

                      dot =
                        vm_vec_dot(&v2t, &Enemy_attacker->orient.vec.fvec);
