...
void hud_init_ets()
{
  ...
  Ets_gauge.first_frame =
    bm_load_animation(Ets_fname[gr_screen.res], &Ets_gauge.num_frames);
...
void update_ets(object * objp, float fl_frametime)
{
  ...
  if (objp->flags & OF_PLAYER_SHIP)
    {
      ship_p->weapon_energy +=
        Energy_levels[ship_p->weapon_recharge_index] * max_new_weapon_energy *
  ...
  max_new_shield_energy =
    fl_frametime * sinfo_p->max_shield_regen_per_second * max_s;
  if (objp->flags & OF_PLAYER_SHIP)
    {
      shield_delta =
        Energy_levels[ship_p->shield_recharge_index] * max_new_shield_energy *
      ...
      if (y < Energy_levels[INTIAL_ENGINE_RECHARGE_INDEX])
        {
          ship_p->current_max_speed =
            0.5f * sinfo_p->max_speed + (y -
                                         Energy_levels[0]) *
            (sinfo_p->max_speed -
             0.5f * sinfo_p->max_speed) /
            (Energy_levels[INTIAL_ENGINE_RECHARGE_INDEX] - Energy_levels[0]);
        }
      else
        {
          ship_p->current_max_speed = sinfo_p->max_speed + (y -
                                                            Energy_levels
                                                            [INTIAL_ENGINE_RECHARGE_INDEX])
            * (sinfo_p->max_overclocked_speed -
               sinfo_p->max_speed) / (Energy_levels[MAX_ENERGY_INDEX] -
                                      Energy_levels
                                      [INTIAL_ENGINE_RECHARGE_INDEX]);
        }
    }
      ...
      if (index < NUM_ENERGY_LEVELS - 1)
        {
          ...
          hud_aabitmap_ex(Ets_gauge.first_frame, x, y_start, w,
                          y_end - y_start, 0,
                          ETS_bar_h[gr_screen.res] - clip_h);
        }
      ...
      if (index > 0)
        {
          ...
          hud_aabitmap_ex(Ets_gauge.first_frame + 1, x, y_start, w,
          ...
          hud_aabitmap_ex(Ets_gauge.first_frame + 2, x, y, w,
...
{
  int *gain_index = NULL, *lose_index1 = NULL, *lose_index2 = NULL, *tmp =
    NULL;
  int count = 0;
  ship *ship_p = &Ships[obj->instance];
...
{
  int *lose_index = NULL, *gain_index1 = NULL, *gain_index2 = NULL, *tmp =
    NULL;
  int count;
  ship *ship_p = &Ships[obj->instance];
...
void transfer_energy_weapon_common(object * objp, float from_field,
                                   float to_field, float *from_delta,
