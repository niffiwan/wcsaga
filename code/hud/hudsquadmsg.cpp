       ...
       so = GET_NEXT(so))
    {
      ...
      if (Ship_info[shipp->ship_info_index].class_type < 0
          || !(Ship_types[Ship_info[shipp->ship_info_index].class_type].
               ai_bools & STI_AI_ACCEPT_PLAYER_ORDERS))
        continue;
      ...
      if (Msg_shortcut_command != -1)
        {
          if (!(shipp->orders_accepted & Msg_shortcut_command))
            ...
            if (!hud_squadmsg_ship_order_valid
      ...
      if (add_to_menu)
        {
          ...
          end_string_at_first_hash_symbol(MsgItems[Num_menu_items].text, (shipp->wingnum != -1));       // truncate the name if it has a # in it
      ...
      if (k == keys_used[i])
        {
          ...
          if (k == KEY_PAGEDOWN)
            {
              // pageup and pagedown scroll the menu -- deal with these seperately!!
...
void hud_squadmsg_display_menu(char *title)
{
  ...
  if (messaging_allowed)
  ...
  else
    {
      ...
      gr_printf(sx, by - Mbox_item_h[gr_screen.res] + 2,
                XSTR("Not allowed to message", 315));
    }
      ...
      else
        {
          ...
          if (robjnum != -1)
          ...
          else
            {
              ...
              mission_log_add_entry(LOG_PLAYER_CALLED_FOR_REARM,
      ...
      if (Wings[i].current_count > 0)
        {
          if (send_message)
            ...
            {
              hud_squadmsg_send_wing_command(i, command, send_message,
      ...
      if (send_message)
        {
          ...
          if (hud_squadmsg_send_ship_command
              (objp->instance, command, send_message, SQUADMSG_HISTORY_UPDATE,
               player_num))
            {
              send_message = 0;
            }
        ...
        {
          hud_squadmsg_send_ship_command(objp->instance, command,
                                         send_message,
                                         SQUADMSG_HISTORY_NO_UPDATE,
                                         player_num);
        }
...
int hud_squadmsg_send_ship_command(int shipnum, int command, int send_message,
                                   int update_history, int player_num)
{
  ai_info *ainfo;
  ...
  if (hud_squadmsg_is_target_order_valid(command, 1, ainfo))
    {
      ...
      switch (command)
        {
          // value of k matches the #defines for ship messages
          ...
          if (Objects[ainfo->target_objnum].type == OBJ_SHIP)
            {
              ...
              if (Ship_info
                  [Ships[Objects[ainfo->target_objnum].instance].
                   ship_info_index].flags & OVERRIDE_PROTECT_SHIP_TYPE)
                {
        ...
        case DISABLE_SUBSYSTEM_ITEM:
          ...
          ai_submode =
            ship_get_subsys_index(&Ships
                                  [Objects[ainfo->target_objnum].instance],
                                  ainfo->targeted_subsys->system_info->
                                  subobj_name);
        ...
        case REARM_REPAIR_ME_ITEM:
          ...
          hud_update_last_order(ordering_shipp->ship_name, player_num,
                                special_index);
          return 0;
      ...
      if (ai_mode != AI_GOAL_NONE)
        {
          ...
          if (update_history == SQUADMSG_HISTORY_ADD_ENTRY)
            {
              hud_add_issued_order(Ships[shipnum].ship_name, command);
              hud_update_last_order(target_shipname, player_num,
                                    special_index);
            }
  ...
  if (send_message && (!(Ships[shipnum].flags2 & SF2_NO_BUILTIN_MESSAGES)))
    {
      message_send_builtin_to_player(message, &Ships[shipnum],
                                     MESSAGE_PRIORITY_NORMAL,
                                     MESSAGE_TIME_ANYTIME, 0, 0, player_num,
                                     message_team_filter);
    }
...
void hud_squadmsg_type_select()
{
  ...
  if (Ai_info[Ships[Player_obj->instance].ai_index].
      ai_flags & (AIF_AWAITING_REPAIR | AIF_BEING_REPAIRED))
    {
...
do_main_menu:
  ...
  if (k != -1)
    {
      // when k != -1, we have a key that associates with menu item
...
void hud_squadmsg_ship_select()
{
  ...
  if (k != -1)
    {
      // if true, we have selected a ship.
      if (Msg_shortcut_command == -1)
        {
      ...
      else
        {
          ...
          hud_squadmsg_send_ship_command(MsgItems[First_menu_item + k].
                                         instance, Msg_shortcut_command, 1,
  ...
  if (k != -1)
    {
      ...
      if (Msg_shortcut_command == -1)
        {
          // do normal menu stuff when no hoykey active
          Msg_instance = MsgItems[First_menu_item + k].instance;        // store the instance id in a global
        ...
        {
          //Assert( Msg_shortcut_command != IGNORE_TARGET_ITEM );
          hud_squadmsg_send_wing_command(MsgItems[First_menu_item + k].
                                         instance, Msg_shortcut_command, 1,
                                         SQUADMSG_HISTORY_ADD_ENTRY);
          hud_squadmsg_toggle();
        }
              ...
              if (!all_accept)
                {
                  ...
                  if (partial_accept)
                    {
                      strcat_s(MsgItems[Num_menu_items].text,
                               XSTR("(*)", 320));
                    }
                  else
                    {
  ...
  if (!Mbox_frames_loaded)
    {
      for (i = 0; i < NUM_MBOX_FRAMES; i++)
        {
          Mbox_gauge[i].first_frame =
            bm_load_animation(Mbox_fnames[gr_screen.res][i],
                              &Mbox_gauge[i].num_frames);
...
int hud_squadmsg_do_frame()
{
  ...
  if ((Game_mode & GM_MULTIPLAYER)
      && !(Net_player->flags & NETINFO_FLAG_AM_MASTER)
      && (Squad_msg_mode == SM_MODE_REPAIR_REARM
          || Squad_msg_mode == SM_MODE_REPAIR_REARM_ABORT))
    {
      ...
      if ((Player_ai->targeted_subsys != NULL)
          && (Player_ai->targeted_subsys->current_hits > 0.0f))
  ...
  if (Mbox_gauge[0].first_frame >= 0)
    {
      ActivateAlignment left(ALIGNMENT_RIGHT);
      hud_aabitmap(Mbox_gauge[0].first_frame,
                   Mbox_top_coords[gr_screen.res][0],
...
int hud_query_order_issued(char *to, char *order_name, char *target_name,
                           int timestamp, char *from, char *special_argument)
{
                  ...
                  if ((!timestamp)
                          ...
                          Missiontime))
                    {
                      ...
                      if ((order & DISABLE_SUBSYSTEM_ITEM)
                          && (special_argument != NULL))
                        {
                          int target_ship = ship_name_lookup(target_name);
                          int subsys_index =
                            ship_get_subsys_index(&Ships[target_ship],
                                                  special_argument, 1);
...
void hudsquadmsg_page_in()
{
  ...
  for (i = 0; i < NUM_MBOX_FRAMES; i++)
    {
      bm_page_in_aabitmap(Mbox_gauge[i].first_frame,
                          Mbox_gauge[i].num_frames);
    }
}
