...
void hud_update_last_order(char *target, int order_source,
                            int special_index);
...
int hud_squadmsg_wing_valid(wing * wingp) 
{
  ...
  int special_ship_index;
    ...
    
    {
           ...
           (Ships[special_ship_index].orders_accepted & Msg_shortcut_command))
...
void hud_squadmsg_call_reinforcement(int reinforcement_num, int player_num) 
{
     ...
     */ 
    mission_log_add_entry(LOG_PLAYER_CALLED_FOR_REINFORCEMENT, rp->name,
                           NULL);
...
int hud_squadmsg_do_frame() 
{
    ...
    {
      GR_AABITMAP(Mbox_gauge[0].first_frame,
