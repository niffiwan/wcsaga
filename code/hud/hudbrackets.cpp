...
void draw_bounding_brackets_subobject() 
{
      ...
      
      {
          ...
          subobj_find_2d_bound(subsys->system_info->radius, &targetp->orient,
          ...
          ship_subsystem_in_sight(targetp, subsys, &View_position,
                                  &subobj_pos, 0);
...
void draw_bounding_brackets(int x1, int y1, int x2, int y2, int w_correction,
                            int h_correction, float distance,
                            int target_objnum) 
{
  ...
   if ((width >= (gr_screen.max_w_unscaled))
         && (height >= (gr_screen.max_h_unscaled)))
  ...
   draw_brackets_square(x1 - w_correction, y1 - h_correction,
    ...
    
    {
        ...
        {
          Ships_attacking_bitmap =
            bm_load(Ships_attack_fname[gr_screen.res]);
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
               hud_stuff_ship_callsign(temp_callsign,
                                         &Ships[t_objp->instance]);
              if (*temp_callsign)
                sprintf(&temp_name[strlen(temp_name)], " (%s)",
                         temp_callsign);
