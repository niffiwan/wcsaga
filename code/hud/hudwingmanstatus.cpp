...
static char
  *Wingman_status_filenames[GR_NUM_RESOLUTIONS][HUD_WINGMAN_STATUS_NUM_FRAMES]
  = {
  //XSTR:OFF
  {
   // GR_640
...
int HUD_wingman_left_coords[GR_NUM_RESOLUTIONS][5][2] = {
  {
   ...
   {
    550,
    144},                       // where to draw the left part of gauge if we have 1 wing
   {
    550,
    144},                       // "" 2 wings
   {
...
int HUD_wingman_middle_coords[GR_NUM_RESOLUTIONS][5][2] = {
  {
   ...
   {
    0,
    0},                         // we never draw this for 1 wing
   {
    0,
    0},                         // we never draw this for 2 wings
   {
    586,
    144},                       // where to draw the _first_ middle gauge for 3 wings
   {
    551,
    144},                       // "" 4 wings
   {
...
int HUD_wingman_right_coords[GR_NUM_RESOLUTIONS][5][2] = {
  {
   ...
   {
    621,
    144},                       // always drawn in the same spot
   {
    621,
    144},
   {
...
int
  HUD_wingman_status_single_coords[GR_NUM_RESOLUTIONS][MAX_SHIPS_PER_WING][2]
  = {
  {
   // GR_640
   {
    589,
    159},                       // where to draw dots if we have only one wing present (special case)
   {
    582,
    167},
   {
...
int
  HUD_wingman_status_coords[GR_NUM_RESOLUTIONS][MAX_SQUADRON_WINGS]
  [MAX_SHIPS_PER_WING][2] = {
  ...
  if (!Wingman_status_gauge_loaded)
    {
      ...
      for (i = 0; i < HUD_WINGMAN_STATUS_NUM_FRAMES; i++)
        {
          Wingman_status_frames[i].first_frame =
            bm_load_animation(Wingman_status_filenames[gr_screen.res][i],
                              &Wingman_status_frames[i].num_frames);
          if (Wingman_status_frames[i].first_frame == -1)
            {
              Warning(LOCATION,
                      NOX
                      ("Error loading Wingman_status_filenames[gr_screen.res][i]'\n"));
              return;
            }
        }
          ...
          if ((wing_index >= 0) && (wing_pos >= 0))
            {
              ...
              if (HUD_wingman_status[wing_index].status[wing_pos] !=
...
void hud_wingman_status_blit_dots(int wing_index, int screen_index,
...
int hud_wingman_status_wing_pos(int shipnum, int wing_status_index,
                                wing * wingp)
{
  int i, wing_pos = -1;
