...
typedef struct hud_anim
{
  ...
  float time_elapsed;           // time that has elapsed (in seconds) since animation started playing
    ...
    hud_anim():first_frame(0),
    num_frames(0), sx(0), sy(0), total_time(0), time_elapsed(0)
  {
    filename[0] = NULL;
  }
} hud_anim;
...
void hud_aabitmap(int bitmap_num, int x, int y, bool resize =
                  true, bool mirror = false);

// hud function for maybe flickering all gauges
void hud_aabitmap_ex(int bitmap_num, int x, int y, int w, int h, int sx,
                     int sy, bool resize = true, bool mirror = false);
...
int hud_anim_render(hud_anim * ha, float frametime, int draw_alpha =
                    0, int loop = 1, int hold_last = 0, int reverse =
                    0, bool resize = true, bool mirror = false);
