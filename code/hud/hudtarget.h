...
int hud_target_closest(int team_mask = -1, int attacked_objnum =
                       -1, int play_fail_sound = TRUE, int filter =
                       0, int turret_attacking_target = 0);
...
int subsystem_in_sight(object * objp, ship_subsys * subsys, vec3d * eye,
                       vec3d * subsystem);
...
void hud_draw_offscreen_indicator(vertex * target_point, vec3d * tpos,
                                  float distance = 0.0f, int draw_solid = 1);
...
extern void hud_target_hotkey_add_remove(int k, object * objp,
                                         int how_to_add);
...
extern void polish_predicted_target_pos(weapon_info * wip, object * targetp,
                                        vec3d * enemy_pos,
