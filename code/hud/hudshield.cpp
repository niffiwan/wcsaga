...
void hud_shield_mini_load()
{
  Shield_mini_gauge.first_frame =
    bm_load_animation(current_hud->Shield_mini_fname,
                      &Shield_mini_gauge.num_frames);
  if (Shield_mini_gauge.first_frame == -1)
    {
      Warning(LOCATION,
              "Could not load in the HUD shield ani: Shield_mini_fname\n");
    }
  else
    {
...
void hud_shield_show(object * objp)
{
  ...
  if (sip->shield_icon_index == 255
      && !(sip->flags2 & SIF2_GENERATE_HUD_ICON) /*&& !digitus_improbus */ )
    {
      return;
    }
  ...
  if (sip->shield_icon_index != 255)
    {
      ...
      if (sgp->first_frame == -1
        ...
        {
          sgp->first_frame =
            bm_load_animation(Hud_shield_filenames[sip->shield_icon_index],
                              &sgp->num_frames);
          if (sgp->first_frame == -1)
            {
  ...
  if (sip->shield_icon_index != 255)
  ...
  else
    {
      ...
      //if(!digitus_improbus)
      {
        model_render(sip->model_num, &object_orient, &vmd_zero_vector,
                     MR_NO_LIGHTING | MR_LOCK_DETAIL | MR_AUTOCENTER |
                     MR_NO_FOGGING, -1, -1, sp->ship_replacement_textures);
      }
      /*else
         {
         if(fod_model == -1)
  ...
  for (i = 0; i < MAX_SHIELD_SECTIONS; i++)
    {
      ...
      hud_color_index =
        fl2i((objp->shield_quadrant[Quadrant_xlate[i]] / max_shield) * range);
              ...
              switch (i)
                ...
                case 0:
                  ...
                  for (j = 0; j < 6; j++)
                    {
                      y_val = sy + 10;
                      gr_gradient(sx + j, sy, sx + j, y_val - j);
                    }
                  mid_val = sy + 5;
                  for (; j < 106; j++)
                    {
                      gr_gradient(sx + j, sy, sx + j, mid_val);
                    }
                  for (; j < 112; j++)
                    {
                      gr_gradient(sx + j, sy, sx + j, sy + (j - 101));
                    }
                  y_val = sy - 1;
                  sy -= 3;
                  for (j = 0; j < 112; j++)
                ...
                case 1:
                  ...
                  for (j = 0; j < 6; j++)
                    {
                      gr_gradient(sx, y_val + j, x_val - j, y_val + j);
                    }
                  mid_val = sx + 5;
                  for (; j < 48; j++)
                    {
                      gr_gradient(sx, y_val + j, mid_val, y_val + j);
                    }
                  for (; j < 54; j++)
                    {
                      gr_gradient(sx, y_val + j, sx + (j - 43), y_val + j);
                    }
                  x_val = sx;
                  sx -= 3;
                  for (j = 0; j < 54; j++)
                ...
                case 2:
                  ...
                  for (j = 0; j < 6; j++)
                    {
                      gr_gradient(x_val,
                                  y_val + j, x_val - (10 - j), y_val + j);
                    }
                  mid_val = x_val - 5;
                  for (; j < 48; j++)
                    {
                      gr_gradient(x_val, y_val + j, mid_val, y_val + j);
                    }
                  for (; j < 54; j++)
                    {
                      gr_gradient(x_val,
                                  y_val + j, x_val - (j - 43), y_val + j);
                    }
                  mid_val = x_val;
                  x_val += 3;
                  for (j = 0; j < 54; j++)
                ...
                case 3:
                  ...
                  for (j = 0; j < 6; j++)
                    gr_gradient(sx + j, y_val, sx + j, y_val - (10 - j));
                  mid_val = y_val - 5;
                  for (; j < 106; j++)
                    gr_gradient(sx + j, y_val, sx + j, mid_val);
                  for (; j < 112; j++)
                    gr_gradient(sx + j, y_val, sx + j, y_val - (j - 101));
                  mid_val = y_val + 1;
                  y_val += 3;
                  for (j = 0; j < 112; j++)
...
void hud_shield_equalize(object * objp, player * pl)
{
  ...
  if (!MULTIPLAYER_CLIENT && (pl->shield_penalty_stamp < 0)
...
void hud_augment_shield_quadrant(object * objp, int direction)
{
  ...
  xfer_amount =
    Ships[objp->instance].ship_max_shield_strength * SHIELD_TRANSFER_PERCENT;
...
void hud_show_mini_ship_integrity(object * objp, int x_force, int y_force)
{
  ...
  gr_string(final_pos[0] + HUD_nose_x, final_pos[1] + HUD_nose_y,
            text_integrity);
}

// Draw the miniature shield icon that is drawn near the reticle
void hud_shield_show_mini(object * objp, int x_force, int y_force,
                          int x_hull_offset, int y_hull_offset)
{
  float max_shield;
  ...
  sx =
    (x_force ==
     -1) ? current_hud->Shield_mini_coords[0] + fl2i(HUD_offset_x) : x_force;
  sy =
    (y_force ==
     -1) ? current_hud->Shield_mini_coords[1] + fl2i(HUD_offset_y) : y_force;
  ...
  for (i = 0; i < MAX_SHIELD_SECTIONS; i++)
    {
      ...
      hud_aabitmap(Shield_mini_gauge.first_frame + frame_offset,
                   sx + HUD_nose_x, sy + HUD_nose_y);
    }
...
void hud_shield_quadrant_hit(object * objp, int quadrant)
{
  ...
  if (quadrant >= 0)
    ...
    {
      shi->shield_hit_timers[HULL_HIT_OFFSET] =
