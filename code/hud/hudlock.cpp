...
char
  Lockspin_fname[NUM_HUD_RETICLE_STYLES][GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN]
  = {
  {
   "lockspin_fs1",
   "2_lockspin_fs1"},
  ...
  if (!Lock_gauge_loaded)
    {
       ...
       */
      hud_anim_init(&Lock_gauge, 0, 0,
                    Lock_fname[Hud_reticle_style][gr_screen.res]);
      hud_anim_load(&Lock_gauge);

      hud_anim_init(&Lock_anim, 0, 0,
                    Lockspin_fname[Hud_reticle_style][gr_screen.res]);
...
void hud_show_lock_indicator(float frametime, vec3d * lock_point_pos)
{
  ...
  if (Player_ai->current_target_is_locked)
    ...
    {
      sx =
        fl2i(lock_point.sx) - (Player->current_target_sx -
                               Players[Player_num].lock_indicator_x);
      sy =
        fl2i(lock_point.sy) - (Player->current_target_sy -
...
DCF_BOOL(nebula_sec_range, Nebula_sec_range)
     int hud_lock_world_pos_in_range(vec3d * target_world_pos,
                                     vec3d * vec_to_target)
{
...
int hud_abort_lock()
{
  ...
  weapon_info *wip =
    &Weapon_info[Player_ship->weapons.
                 secondary_bank_weapons[Player_ship->
                                        weapons.current_secondary_bank]];
  object *target_obj = &Objects[Player_ai->target_objnum];

  if (target_obj->type == OBJ_SHIP)
    {
      if (target_obj->instance >= 0
          && Ships[target_obj->instance].ship_info_index >= 0)
        {
          ship_info *sip =
            &Ship_info[Ships[target_obj->instance].ship_info_index];

          bool is_weapon_huge_or_bomb =
            ((wip->wi_flags & (WIF_HUGE | WIF_BOMB)) != 0);
...
void hud_do_lock_indicator(float frametime)
{
  ...
  if (wip->wi_flags & WIF_HOMING_JAVELIN &&
    ...
    {
      vec3d subobj_pos;
      vm_vec_unrotate(&subobj_pos, &Player->locking_subsys->system_info->pnt,
                      &tobjp->orient);
      ...
      if (!target_subsys_in_sight
          || Player->locking_subsys->system_info->type != SUBSYSTEM_ENGINE)
        {
          Player->locking_subsys =
            ship_get_closest_subsys_in_sight(&Ships[tobjp->instance],
                                             SUBSYSTEM_ENGINE,
                                             &Player_obj->pos);
        }
    }
  ...
  for (; ang <= end_ang; ang += PI_2)
    {
      ...
      xpos =
        center_x + (float) cos(ang) * (radius +
                                       Lock_triangle_height[gr_screen.res] +
                                       2);
      ypos =
        center_y - (float) sin(ang) * (radius +
                                       Lock_triangle_height[gr_screen.res] +
                                       2);
...
void hud_draw_lock_triangles(int center_x, int center_y, float frametime)
{
  if (Lock_anim.first_frame == -1)
  ...
  else
    {
      // render the anim
      Lock_anim.sx =
        center_x - Lockspin_half_w[Hud_reticle_style][gr_screen.res];
      Lock_anim.sy =
        center_y - Lockspin_half_h[Hud_reticle_style][gr_screen.res];
  ...
  if (Player->target_in_lock_cone)
    {
      ...
      if (Missile_track_loop == -1)
        {
          Missile_track_loop =
            snd_play_looping(&Snds[SND_MISSILE_TRACKING], 0.0f, -1, -1);
        }
...
void hud_calculate_lock_start_pos()
{
  ...
  CLAMP(Players[Player_num].lock_indicator_start_x, gr_screen.clip_left,
        gr_screen.clip_right);
  CLAMP(Players[Player_num].lock_indicator_start_y, gr_screen.clip_top,
...
void hud_lock_get_new_lock_pos(object * target_objp, vec3d * lock_world_pos)
{
  ...
  if ((target_shipp) && wip->wi_flags & WIF_HOMING_JAVELIN)
    {
      Player->locking_subsys =
        ship_get_closest_subsys_in_sight(target_shipp, SUBSYSTEM_ENGINE,
                                         &Player_obj->pos);
      if (Player->locking_subsys != NULL)
        {
          get_subsystem_world_pos(target_objp, Player->locking_subsys,
  ...
  if (Player_ai->targeted_subsys && !(wip->wi_flags & WIF_HOMING_JAVELIN))
           ...
           && target_objp->type == OBJ_SHIP)
    {
      if (!Player->locking_subsys ||
        ...
        {
          Player->locking_subsys =
            ship_get_closest_subsys_in_sight(&Ships[target_objp->instance],
                                             SUBSYSTEM_ENGINE,
                                             &Player_obj->pos);
        }
      if (Player->locking_subsys != NULL)
  ...
  if (lock_local_pos.xyz.z > 0.0f)
    {
      ...
      float sx =
        ((gr_screen.clip_center_x * 2.0f) +
         (lock_local_pos.xyz.x * (gr_screen.clip_center_x * 2.0f) * w)) *
        0.5f;
      float sy =
        ((gr_screen.clip_center_y * 2.0f) -
         (lock_local_pos.xyz.y * (gr_screen.clip_center_y * 2.0f) * w)) *
        0.5f;
