...
#define LOCK_GAUGE_BLINK_RATE           5       // blinks/sec
  ...
  Lock_fname[NUM_HUD_RETICLE_STYLES][GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] =
...
void hud_do_lock_indicator(float frametime) 
{
    ...
    
    {
      ...
      int target_subsys_in_sight =
        ship_subsystem_in_sight(tobjp, Player->locking_subsys,
                                &Player_obj->pos, &subobj_pos);
...
void hud_lock_get_new_lock_pos(object * target_objp, vec3d * lock_world_pos) 
{
    ...
    if ((target_shipp)
        && (Ship_info[target_shipp->ship_info_index].
