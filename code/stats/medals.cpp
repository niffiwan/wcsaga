...
int Medal_coords[GR_NUM_RESOLUTIONS][NUM_MEDAL_REGIONS][2] = {
  {
   ...
   {
    486,
    47},                        // imp. order o' vasuda
   {
    129,
    130},                       // dist flying cross
   {
    208,
    132},                       // soc service
   {
    361,
    131},                       // dist intel cross
   {
    439,
    130},                       // order of galatea
   {
    64,
    234},                       // meritorious unit comm.
   {
    153,
    234},                       // medal of valor
   {
    239,
    241},                       // gtva leg of honor
   {
    326,
    240},                       // allied defense citation
   {
    411,
    234},                       // neb campaign victory
   {
    494,
    234},                       // ntf campaign victory
   {
    283,
    91},                        // wings
   {
    372,
    76},                        // bronze kills badge
   {
    403,
    76},                        // silver kills badge
   {
    435,
    76},                        // gold kills badge
   {
   ...
   },
  {
   ...
   {
    363,
    373},                       // Combat Action Medal
   {
    563,
    334},                       // Ace Medal
   {
    463,
    334},                       // Ace of Aces Medal
   {
    363,
    334},                       // Century Award
   {
   ...
   {
    463,
    295},                       // Operation Prowl Growl
   {
    363,
    295},                       // Nephele Liberation
   {
    563,
    256},                       // Cairo Liberation
   {
    463,
    256},                       // Hyperion Service Medal
   {
    363,
    256},                       // Freya Campaign Medal
   }
};
...
DCF(medals, "Grant or revoke medals")
{
  ...
  if (Dc_help)
    {
      dc_printf
        ("Usage: gimmemedals all | clear | promote | demote | [index]\n");
...
void medal_main_init(player * pl, int mode)
{
  ...
  snazzy_menu_init();
  Medals_window.create(0, 0, gr_screen.max_w_unscaled,
  ...
  for (idx = 0; idx < MEDALS_NUM_BUTTONS; idx++)
    {
      // create the object
      Medals_buttons[gr_screen.res][idx].button.create(&Medals_window, "",
                                                       Medals_buttons
                                                       [gr_screen.res][idx].x,
                                                       Medals_buttons
                                                       [gr_screen.res][idx].y,
      ...
      Medals_buttons[gr_screen.res][idx].button.
        set_highlight_action(common_play_highlight_sound);

      // set the ani for the button
      Medals_buttons[gr_screen.res][idx].button.
        set_bmaps(Medals_buttons[gr_screen.res][idx].filename);

      // set the hotspot
      Medals_buttons[gr_screen.res][idx].button.
        link_hotspot(Medals_buttons[gr_screen.res][idx].hotspot);
    }
...
void blit_callsign()
{
  ...
  gr_string(0x8000, Medals_callsign_y[gr_screen.res],
            Medals_player->callsign);
}

void GetEarnedRibbons(std::vector < std::pair < medal_stuff,
                      int > >&earnedMedals)
{
  ...
  for (size_t i = FIRST_RIBBON; i < Medals.size(); ++i)
    {
      if (Player_score->medals[i] > 0)
        {
          earnedMedals.push_back(std::pair < medal_stuff,
                                 int >(Medals[i], Player_score->medals[i]));
        }
    }
}
...
int medal_main_do()
{
  ...
  region =
    snazzy_menu_do((ubyte *) Medals_mask->data, Medals_mask_w, Medals_mask_h,
                   NUM_MEDAL_REGIONS, Medal_regions, &selected);
  switch (region)
    {
    case ESC_PRESSED:
      if (Medals_mode == MM_NORMAL)
    ...
    default:
      {
        ...
        if (isRibbon)
          {
            size_t numRibbons =
              std::count_if(Player_score->medals + FIRST_RIBBON,
            ...
            if (idxRibbon < numRibbons)
              {
                ...
                blit_label(earnedRibbons[idxRibbon].first.name,
                           &Medal_coords[gr_screen.res][idxMedal][0],
                           earnedRibbons[idxRibbon].second);
              }
          }
        else if (Player_score->medals[region - 1] > 0)
          {
            blit_label(Medals[region - 1].name,
                       &Medal_coords[gr_screen.res][region - 1][0],
      ...
      if (Player_score->medals[idx] > 0)
        {
          ...
          gr_bitmap(Medal_coords[gr_screen.res][regionIdx][0],
                    Medal_coords[gr_screen.res][regionIdx][1]);
        }
