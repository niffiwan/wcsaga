...
CMissionNotesDlg::CMissionNotesDlg(CWnd * pParent /*=NULL*/ ):CDialog(CMissionNotesDlg::IDD,
        pParent)
{
...
BEGIN_MESSAGE_MAP(CMissionNotesDlg, CDialog)
  ...
  ON_WM_CLOSE()ON_BN_CLICKED(IDC_TRAINING, OnTraining)
...
void CMissionNotesDlg::OnOK()
{
  ...
  MODIFY(The_mission.support_ships.max_support_ships,
         (m_disallow_support) ? 0 : -1);
  MODIFY(The_mission.support_ships.max_hull_repair_val,
         m_max_hull_repair_val);
  MODIFY(The_mission.support_ships.max_subsys_repair_val,
  ...
  string_copy(The_mission.loading_screen[GR_640], m_loading_640, NAME_LENGTH,
              1);
  string_copy(The_mission.loading_screen[GR_1024], m_loading_1024,
              NAME_LENGTH, 1);
  ...
  if (m_squad_name == CString(NO_SQUAD))
  ...
  else
    {
      ...
      string_copy(The_mission.squad_filename, m_squad_filename,
                  MAX_FILENAME_LEN);
    }
  ...
  MODIFY(The_mission.command_persona,
         (int) ((CComboBox *) GetDlgItem(IDC_COMMAND_PERSONA))->
         GetItemData(m_command_persona));
...
BOOL CMissionNotesDlg::OnInitDialog()
{
  ...
  m_mission_notes_orig = m_mission_notes =
    convert_multiline_string(The_mission.notes);
  m_mission_desc_orig = m_mission_desc =
    convert_multiline_string(The_mission.mission_desc);
  ...
  m_daisy_chained_docking =
    (The_mission.flags & MISSION_FLAG_ALLOW_DOCK_TREES) ? 1 : 0;
  m_disallow_support =
    (The_mission.support_ships.max_support_ships == 0) ? 1 : 0;
  m_no_promotion = (The_mission.flags & MISSION_FLAG_NO_PROMOTION) ? 1 : 0;
  m_no_builtin_msgs =
    (The_mission.flags & MISSION_FLAG_NO_BUILTIN_MSGS) ? 1 : 0;
  m_no_builtin_command_msgs =
    (The_mission.flags & MISSION_FLAG_NO_BUILTIN_COMMAND) ? 1 : 0;
  m_no_traitor = (The_mission.flags & MISSION_FLAG_NO_TRAITOR) ? 1 : 0;
  m_toggle_trails =
    (The_mission.flags & MISSION_FLAG_TOGGLE_SHIP_TRAILS) ? 1 : 0;
  m_support_repairs_hull =
    (The_mission.flags & MISSION_FLAG_SUPPORT_REPAIRS_HULL) ? 1 : 0;
  m_beam_free_all_by_default =
    (The_mission.flags & MISSION_FLAG_BEAM_FREE_ALL_BY_DEFAULT) ? 1 : 0;
  m_player_start_using_ai =
    (The_mission.flags & MISSION_FLAG_PLAYER_START_AI) ? 1 : 0;
  m_no_briefing = (The_mission.flags & MISSION_FLAG_NO_BRIEFING) ? 1 : 0;
  m_no_debriefing =
    (The_mission.flags & MISSION_FLAG_TOGGLE_DEBRIEFING) ? 1 : 0;
  m_autpilot_cinematics =
    (The_mission.flags & MISSION_FLAG_USE_AP_CINEMATICS) ? 1 : 0;
  ...
  if (!
      (m_type &
       (MISSION_TYPE_MULTI_COOP | MISSION_TYPE_MULTI_DOGFIGHT |
...
char *Load_screen_ext =
  "Image Files (*.dds, *.pcx, *.jpg, *.jpeg, *.tga)|*.dds;*.pcx;*.jpg;*.jpeg;*.tga|"
  "DDS Files (*.dds)|*.dds|" "PCX Files (*.pcx)|*.pcx|"
  "JPG Files (*.jpg; *.jpeg)|*.jpg;*.jpeg|" "TGA Files (*.tga)|*.tga|"
  "All Files (*.*)|*.*|" "|";
