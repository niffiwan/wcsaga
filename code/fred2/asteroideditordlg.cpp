...
int Dlg_id[MAX_ACTIVE_DEBRIS_TYPES] =
  { IDC_SHIP_DEBRIS1, IDC_SHIP_DEBRIS2, IDC_SHIP_DEBRIS3 };
...
BEGIN_MESSAGE_MAP(asteroid_editor, CDialog)
  ...
  ON_WM_INITMENU()ON_BN_CLICKED(IDC_ENABLE_ASTEROIDS, OnEnableAsteroids)
ON_WM_CLOSE()ON_BN_CLICKED(IDC_ENABLE_INNER_BOX, OnEnableInnerBox)
...
int asteroid_editor::query_modified()
{
  ...
  for (i = 0; i < 1 /*MAX_ASTEROID_FIELDS */ ; i++)
    {
      if (a_field[i].num_initial_asteroids !=
      ...
      if (a_field[i].has_inner_bound)
        {
          if (a_field[i].inner_max_bound.xyz.x !=
              Asteroid_field.inner_max_bound.xyz.x)
            return 1;

          if (a_field[i].inner_max_bound.xyz.y !=
              Asteroid_field.inner_max_bound.xyz.y)
            return 1;

          if (a_field[i].inner_max_bound.xyz.z !=
              Asteroid_field.inner_max_bound.xyz.z)
            return 1;

          if (a_field[i].inner_min_bound.xyz.x !=
              Asteroid_field.inner_min_bound.xyz.x)
            return 1;

          if (a_field[i].inner_min_bound.xyz.y !=
              Asteroid_field.inner_min_bound.xyz.y)
            return 1;

          if (a_field[i].inner_min_bound.xyz.z !=
  ...
  if (!m_enable_asteroids)
  ...
  else
    {
      ...
      if (a_field[0].has_inner_bound)
        {
          if (a_field[0].inner_max_bound.xyz.x <
              a_field[0].inner_min_bound.xyz.x)
            {
              MessageBox("Asteroid x inner min is greater than inner max");
              return 0;
            }

          if (a_field[0].inner_max_bound.xyz.y <
              a_field[0].inner_min_bound.xyz.y)
            {
              MessageBox("Asteroid y inner min is greater than inner max");
              return 0;
            }

          if (a_field[0].inner_max_bound.xyz.z <
              a_field[0].inner_min_bound.xyz.z)
            {
          ...
          // check x thickness
          if (a_field[0].inner_min_bound.xyz.x - MIN_BOX_THICKNESS <
              a_field[0].min_bound.xyz.x)
            {
              MessageBox
          ...
          if (a_field[0].inner_max_bound.xyz.x + MIN_BOX_THICKNESS >
              a_field[0].max_bound.xyz.x)
            {
              MessageBox
                ("Asteroid x thickness from outer box to inner box must be > 400");
              return 0;
          ...
          // check y thickness
          if (a_field[0].inner_min_bound.xyz.y - MIN_BOX_THICKNESS <
              a_field[0].min_bound.xyz.y)
            {
              MessageBox
          ...
          if (a_field[0].inner_max_bound.xyz.y + MIN_BOX_THICKNESS >
              a_field[0].max_bound.xyz.y)
            {
              MessageBox
                ("Asteroid y thickness from outer box to inner box must be > 400");
              return 0;
          ...
          // check z thickness
          if (a_field[0].inner_min_bound.xyz.z - MIN_BOX_THICKNESS <
              a_field[0].min_bound.xyz.z)
            {
              MessageBox
          ...
          if (a_field[0].inner_max_bound.xyz.z + MIN_BOX_THICKNESS >
              a_field[0].max_bound.xyz.z)
            {
              MessageBox
                ("Asteroid z thickness from outer box to inner box must be > 400");
              return 0;
      ...
      if (a_field[0].field_type == FT_PASSIVE)
        {
          if (a_field[0].debris_genre == DG_SHIP)
            {
              if ((a_field[0].field_debris_type[0] == -1)
...
void asteroid_editor::update_init()
{
  ...
  if (last_field >= 0)
    {
      ...
      a_field[last_field].num_initial_asteroids =
        m_enable_asteroids ? m_density : 0;
          ...
          for (idx = 0; idx < MAX_ACTIVE_DEBRIS_TYPES; idx++)
            {
              ...
              int cur_sel =
                ((CComboBox *) GetDlgItem(Dlg_id[idx]))->GetCurSel();
              if (cur_sel != CB_ERR)
                {
                  cur_choice =
                    ((CComboBox *) GetDlgItem(Dlg_id[idx]))->
                    GetItemData(cur_sel);
                }
              else
                {
      ...
      MODIFY(a_field[last_field].inner_min_bound.xyz.x,
             (float) atof(m_box_min_x));
      MODIFY(a_field[last_field].inner_min_bound.xyz.y,
             (float) atof(m_box_min_y));
      MODIFY(a_field[last_field].inner_min_bound.xyz.z,
             (float) atof(m_box_min_z));
      MODIFY(a_field[last_field].inner_max_bound.xyz.x,
             (float) atof(m_box_max_x));
      MODIFY(a_field[last_field].inner_max_bound.xyz.y,
             (float) atof(m_box_max_y));
      MODIFY(a_field[last_field].inner_max_bound.xyz.z,
             (float) atof(m_box_max_z));
    }

  Assert(cur_field >= 0);
  // get from temp asteroid field into class
  m_enable_asteroids =
    a_field[cur_field].num_initial_asteroids ? TRUE : FALSE;
      ...
      for (j = 0; j < NUM_DEBRIS_SIZES; j++)
        {
          ...
          for (k = 0; k < MAX_ACTIVE_DEBRIS_TYPES; k++)
            {
              box_index =
                ((CComboBox *) GetDlgItem(Dlg_id[k]))->AddString(name);
              ((CComboBox *) GetDlgItem(Dlg_id[k]))->SetItemData(box_index,
                                                                 index);
            }
        }
    }
  ...
  ((CButton *) GetDlgItem(IDC_SUBTYPE1))->SetCheck(a_field[cur_field].
                                                   field_debris_type[0] == 1);
  ((CButton *) GetDlgItem(IDC_SUBTYPE2))->SetCheck(a_field[cur_field].
                                                   field_debris_type[1] == 1);
  ((CButton *) GetDlgItem(IDC_SUBTYPE3))->SetCheck(a_field[cur_field].
                                                   field_debris_type[2] == 1);
...
void asteroid_editor::OnEnableAsteroids()
{
  ...
  GetDlgItem(IDC_SHIP_DEBRIS1)->EnableWindow(m_enable_asteroids
                                             && (Asteroid_field.
                                                 debris_genre == DG_SHIP));
  GetDlgItem(IDC_SHIP_DEBRIS2)->EnableWindow(m_enable_asteroids
                                             && (Asteroid_field.
                                                 debris_genre == DG_SHIP));
  GetDlgItem(IDC_SHIP_DEBRIS3)->EnableWindow(m_enable_asteroids
                                             && (Asteroid_field.
                                                 debris_genre == DG_SHIP));
  GetDlgItem(IDC_SUBTYPE1)->EnableWindow(m_enable_asteroids
                                         && (Asteroid_field.debris_genre ==
                                             DG_ASTEROID));
  GetDlgItem(IDC_SUBTYPE2)->EnableWindow(m_enable_asteroids
                                         && (Asteroid_field.debris_genre ==
                                             DG_ASTEROID));
  GetDlgItem(IDC_SUBTYPE3)->EnableWindow(m_enable_asteroids
                                         && (Asteroid_field.debris_genre ==
                                             DG_ASTEROID));
...
void asteroid_editor::OnClose()
{
  ...
  if (query_modified())
    {
      z =
        MessageBox("Do you want to keep your changes?", "Close",
...
void asteroid_editor::OnEnableInnerBox()
{
  ...
  GetDlgItem(IDC_INNER_MIN_X)->EnableWindow(m_enable_inner_bounds
                                            && m_enable_asteroids);
  GetDlgItem(IDC_INNER_MAX_X)->EnableWindow(m_enable_inner_bounds
                                            && m_enable_asteroids);
  GetDlgItem(IDC_INNER_MIN_Y)->EnableWindow(m_enable_inner_bounds
                                            && m_enable_asteroids);
  GetDlgItem(IDC_INNER_MAX_Y)->EnableWindow(m_enable_inner_bounds
                                            && m_enable_asteroids);
  GetDlgItem(IDC_INNER_MIN_Z)->EnableWindow(m_enable_inner_bounds
                                            && m_enable_asteroids);
  GetDlgItem(IDC_INNER_MAX_Z)->EnableWindow(m_enable_inner_bounds
                                            && m_enable_asteroids);
}
...
void asteroid_editor::OnEnableField()
{
  ...
  if (m_enable_asteroids && (m_field_type == FT_PASSIVE)
...
void asteroid_editor::OnPassiveField()
{
  ...
  ((CButton *) GetDlgItem(IDC_FIELD_ASTEROID))->SetCheck(m_debris_genre ==
                                                         DG_ASTEROID);
  ((CButton *) GetDlgItem(IDC_FIELD_SHIP))->SetCheck(m_debris_genre ==
                                                     DG_SHIP);
