...
ON_WM_MOUSEMOVE()ON_WM_LBUTTONUP()ON_WM_DESTROY()ON_WM_LBUTTONDOWN
      ...
      for (i = 0; i < Num_op_menus; i++)
        {
          ...
          add_op_menu->AppendMenu(MF_POPUP, (UINT) add_op_submenu[i].m_hMenu,
                                  op_menu[i].name);
              ...
              if (tree_nodes[item_index].type & SEXPT_VARIABLE)
                {
                  ...
                  if ((arg_num == 0
                      ...
                      || (arg_num == 5
                          && Operators[op].value == OP_ADD_SUN_BITMAP))
                    {
