...
BEGIN_MESSAGE_MAP(VoiceActingManager, CDialog)
  ...
  ON_WM_CLOSE()ON_EN_SETFOCUS(IDC_ABBREV_BRIEFING, OnSetfocusAbbrevBriefing)
     ...
     BOOL VoiceActingManager::OnInitDialog()
{
  ...
  m_export_everything = m_export_command_briefings = m_export_briefings =
    m_export_debriefings = m_export_messages = FALSE;
...
CString VoiceActingManager::generate_filename(CString section, int number,
                                              int digits)
{
  if (section == "")
    return "none.wav";
  ...
  for (i = 0; i < Cmd_briefs[0].num_stages; i++)
    {
      ...
      if (!m_no_replace || !strlen(filename)
          || !strnicmp(filename, "none.wav", 4)
          || message_filename_is_generic(filename))
        {
          strcpy(filename,
                 LPCTSTR(generate_filename
                         (m_abbrev_command_briefing, i + 1, digits)));
        }
    }
  ...
  for (i = 0; i < Debriefings[0].num_stages; i++)
    {
      ...
      if (!m_no_replace || !strlen(filename)
          ...
          || message_filename_is_generic(filename))
        {
          strcpy(filename,
                 LPCTSTR(generate_filename
                         (m_abbrev_debriefing, i + 1, digits)));
        }
    }
  ...
  for (i = 0; i < Num_messages - Num_builtin_messages; i++)
    {
      ...
      // generate only if we're replacing or if it has a replaceable name
      if (!m_no_replace || !filename || !strlen(filename)
          || !strnicmp(filename, "none.wav", 4)
          || message_filename_is_generic(filename))
        {
          ...
          Messages[i + Num_builtin_messages].wave_info.name =
            strdup(LPCTSTR
