...
BOOL CFREDApp::InitInstance()
{
  ...
  User_interface =
    GetProfileInt("Preferences", "User interface", User_interface);
  Show_stars = GetProfileInt("Preferences", "Show stars", Show_stars);
  Show_grid_positions =
    GetProfileInt("Preferences", "Show grid positions", Show_grid_positions);
  Show_coordinates =
    GetProfileInt("Preferences", "Show coordinates", Show_coordinates);
  Show_compass = GetProfileInt("Preferences", "Show compass", Show_compass);
  Show_ship_models =
    GetProfileInt("Preferences", "Show ship models", Show_ship_models);
  Show_ship_info =
    GetProfileInt("Preferences", "Show ship info", Show_ship_info);
  Show_outlines =
    GetProfileInt("Preferences", "Show outlines", Show_outlines);
  Show_waypoints =
    GetProfileInt("Preferences", "Show waypoints", Show_waypoints);
  Show_sexp_help =
    GetProfileInt("Preferences", "Show sexp help", Show_sexp_help);
  physics_speed =
    GetProfileInt("Preferences", "Physics speed", physics_speed);
  physics_rot = GetProfileInt("Preferences", "Physics rotation", physics_rot);
  Hide_ship_cues =
    GetProfileInt("Preferences", "Hide ship cues", Hide_ship_cues);
  Hide_wing_cues =
    GetProfileInt("Preferences", "Hide wing cues", Hide_wing_cues);
  Autosave_disabled =
    GetProfileInt("Preferences", "Autosave disabled", Autosave_disabled);
  double_fine_gridlines =
    GetProfileInt("Preferences", "Double fine gridlines",
                  double_fine_gridlines);
  Aa_gridlines =
    GetProfileInt("Preferences", "Anti aliased gridlines", Aa_gridlines);
  Show_dock_points =
    GetProfileInt("Preferences", "Show dock points", Show_dock_points);
  Show_paths_fred =
    GetProfileInt("Preferences", "Show paths", Show_paths_fred);
  Lighting_on = GetProfileInt("Preferences", "Lighting On", Lighting_on);

  // Goober5000
  Format_fs2_open =
    GetProfileInt("Preferences", "FS2 open format", Format_fs2_open);
  Format_fs2_retail =
    GetProfileInt("Preferences", "FS2 retail format", Format_fs2_retail);
  Format_fs1_retail =
    GetProfileInt("Preferences", "FS1 retail format", Format_fs1_retail);
  ...
  pDocTemplate = new CSingleDocTemplate(IDR_MAINFRAME, RUNTIME_CLASS(CFREDDoc), RUNTIME_CLASS(CMainFrame),      // main SDI frame window
...
BEGIN_MESSAGE_MAP(CAboutDlg, CDialog)
  ...
  ON_BN_CLICKED(IDC_BUG, OnBug) ON_BN_CLICKED(IDC_FORUMS, OnForums)
  //}}AFX_MSG_MAP
END_MESSAGE_MAP()
  ...
  if (!app_init)
    {
      ...
      Fred_main_wnd->SetWindowPos(&CWnd::wndTop, 0, 0, 0, 0,
...
void CFREDApp::write_ini_file(int degree)
{
  ...
  WriteProfileInt("Preferences", "Double fine gridlines",
                  double_fine_gridlines);
...
void CFREDApp::write_window(char *name, window_data * wndd)
{
  ...
  WriteProfileInt(name, "rcNormalPosition.left",
                  wndd->p.rcNormalPosition.left);
  WriteProfileInt(name, "rcNormalPosition.top", wndd->p.rcNormalPosition.top);
  WriteProfileInt(name, "rcNormalPosition.right",
                  wndd->p.rcNormalPosition.right);
  WriteProfileInt(name, "rcNormalPosition.bottom",
                  wndd->p.rcNormalPosition.bottom);
...
void CFREDApp::read_window(char *name, window_data * wndd)
{
  ...
  wndd->p.rcNormalPosition.left =
    GetProfileInt(name, "rcNormalPosition.left", 0);
  wndd->p.rcNormalPosition.top =
    GetProfileInt(name, "rcNormalPosition.top", 0);
  wndd->p.rcNormalPosition.right =
    GetProfileInt(name, "rcNormalPosition.right", 0);
  wndd->p.rcNormalPosition.bottom =
    GetProfileInt(name, "rcNormalPosition.bottom", 0);
