...
int create_wing()
{
      ...
      if (ptr->flags & OF_MARKED)
        {
          ...
          if (ptr->type == OBJ_WAYPOINT)
          ...
          else if (ptr->type == OBJ_SHIP)
            {
              ...
              if (ship_type < 0
                  || !(Ship_types[ship_type].ai_bools & STI_AI_CAN_FORM_WING))
                {
                  illegal_ships++;
                  unmark_object(OBJ_INDEX(ptr));
                }
      ...
      if (ptr->flags & OF_MARKED)
        {
          if ((ptr->type == OBJ_START)
              && (ptr->instance == Player_start_shipnum))
  ...
  if (illegal_ships)
    Fred_main_wnd->
      MessageBox("Some ship types aren't allowed to be in a wing.\n"
                 "All marked ships of these types were ignored", "Error",
                 MB_ICONEXCLAMATION);
  ...
  for (i = 0; i < Wings[wing].wave_count; i++)
    {
      if (Ships[Wings[wing].ship_index[i]].team != leader_team)
        {
          Fred_main_wnd->MessageBox("Wing contains ships on different teams",
                                    "Warning");
          break;
        }
    }
...
void create_wing(int wing_type, int leader_index, int *wingmen, int max_size,
                 int fill_flag)
{
