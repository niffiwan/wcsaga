  ...
  if (Waypoint_lists[list].count >= MAX_WAYPOINTS_PER_LIST)
    {
      Fred_main_wnd->MessageBox("Unable to create new waypoint.  You have\n"
                                "reached the maximum limit on waypoints per list.",
  ...
  if (type == OBJ_START)
    {
      ...
      if (Player_starts < 2)    // player 1 start
        {
          Fred_main_wnd->
            MessageBox("Must have at least 1 player starting point!", NULL,
                       MB_OK | MB_ICONEXCLAMATION);
...
int reference_handler(char *name, int type, int obj)
{
  ...
  switch (type)
    ...
    default:
      Error(LOCATION, "Type unknown for object \"%s\".  Let Hoffos know now!",
            name);
    }
          ...
          if (swp->secondary_bank_weapons[j] >= 0
              && swp->secondary_bank_weapons[j] < MAX_WEAPON_TYPES)
            {
              arr[swp->secondary_bank_weapons[j]] +=
                (int)
                floor((swp->secondary_bank_ammo[j] *
                       swp->secondary_bank_capacity[j] / 100.0f /
