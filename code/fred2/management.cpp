...
 bool fred_init() 
{
  ...
  species_init();               // Kazan
    ...
    Fred_texture_replacements =
    new texture_replace[MAX_SHIPS * MAX_REPLACEMENT_TEXTURES];
  ...
  strcpy_s(Voice_script_entry_format,
            "Sender: $sender\r\nPersona: $persona\r\nFile: $filename\r\nMessage: $message");
...
}  int create_object_on_grid(int list) 
{
    ...
    fvi_ray_plane(&pos, &The_grid->center, &The_grid->gmatrix.vec.uvec,
                  &view_pos, &dir, 0.0f);
...
 int create_ship(matrix * orient, vec3d * pos, int ship_type) 
{
  ...
  char pwd[128];
    ...
    shipp->team =
    Species_info[Ship_info[shipp->ship_info_index].species].default_iff;
  resolve_parse_flags(&Objects[obj],
                       Iff_info[shipp->team].default_parse_flags,
    ...
    // set orders according to whether the ship is on the player ship's team
  {
      ...
      for (temp_objp = GET_FIRST(&obj_used_list);
           temp_objp != END_OF_LIST(&obj_used_list);
    ...
    min(1000.0f, 200.0f + (temp_max_hull_strength / 4.0f));
...
}  int dup_object(object * objp) 
{
    ...
    
    {
        ...
        create_ship(&objp->orient, &objp->pos, Ships[inst].ship_info_index);
...
 int create_object(vec3d * pos, int list) 
{
    ...
    
    {
        ...
        
        {
            ...
            MessageBox("Unable to create new player start point.\n" 
                       "You have reached the maximum limit.", NULL,
        ...
        
        {
            ...
            MessageBox("You can't have more than one player start in\n" 
  ...
  else                          // creating a ship
    {
        ...
        alloc_sexp("true", SEXP_ATOM, SEXP_ATOM_OPERATOR, -1, -1);
      Ships[n].departure_cue =
        alloc_sexp("false", SEXP_ATOM, SEXP_ATOM_OPERATOR, -1, -1);
...
 int create_player(int num, vec3d * pos, matrix * orient, int type,
                     int init) 
{
...
 int create_waypoint(vec3d * pos, int list) 
{
    ...
    
    {
        ...
        MessageBox("Unable to create new waypoint path.  You\n" 
                   "have reached the maximum limit.", NULL,
                   MB_OK | MB_ICONEXCLAMATION);
  ...
  if (Waypoint_lists[list].count >= MAX_WAYPOINTS_PER_LIST)
    
    {
        ...
        MessageBox("Unable to create new waypoint.  You have\n" 
                   "reached the maximum limit on waypoints per list.", NULL,
                   MB_OK | MB_ICONEXCLAMATION);
    ...
    
    {
        ...
        {
          Waypoint_lists[list].waypoints[i] =
            Waypoint_lists[list].waypoints[i - 1];
    ...
    obj_create(OBJ_WAYPOINT, -1, list * 65536 + index, NULL, pos, 0.0f,
               OF_RENDERS);
...
}  void clear_mission() 
{
  ...
  ai_init();
    ...
    reg_read_string("SOFTWARE\\Microsoft\\Windows\\CurrentVersion",
                    "RegisteredOwner", NULL);
  if (!str)
    
    {
      str =
        reg_read_string("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion",
                        "RegisteredOwner", NULL);
...
 int query_valid_waypoint(int index) 
{
         ...
         || Objects[index].type != OBJ_WAYPOINT)
...
void set_cur_indices(int obj) 
{
  ...
  if (query_valid_object(obj))
    ...
    {
      cur_object_index = cur_ship = cur_wing = cur_waypoint_list =
        cur_waypoint = -1;
...
 int common_object_delete(int obj) 
{
    ...
    
    {
      ...
      if (Player_starts < 2)   // player 1 start
        {
            ...
            MessageBox("Must have at least 1 player starting point!", NULL,
                       MB_OK | MB_ICONEXCLAMATION);
      ...
      invalidate_references(name, REF_TYPE_PLAYER);
        ...
        {
          ai_do_objects_undocked_stuff(&Objects[obj],
                                        dock_get_first_docked_object(&Objects
                                                                     [obj]));
        ...
        
        {
          ...
          if (Wings[wing].special_ship == i)
            ...
            
            {
               ...
               && (Wings[wing].threshold >= Wings[wing].wave_count))
...
 void generate_ship_popup_menu(CMenu * mptr, int first_id, int state,
                                 int filter) 
{
    ...
    
    {
           ...
           || ((ptr->type == OBJ_START) && (filter & SHIP_FILTER_PLAYERS)))
        
        {
            ...
            {
              if (Ship_info[Ships[get_ship_from_obj(ptr)].ship_info_index].
...
 int set_reinforcement(char *name, int state) 
{
    ...
    
    {
      ...
      memset(Reinforcements[Num_reinforcements].no_messages, 0,
              MAX_REINFORCEMENT_MESSAGES * NAME_LENGTH);
      memset(Reinforcements[Num_reinforcements].yes_messages, 0,
              MAX_REINFORCEMENT_MESSAGES * NAME_LENGTH);
...
int reference_handler(char *name, int type, int obj) 
{
    ...
    
    {
        ...
        
        {
          ...
          sprintf(text, "the arrival cue of ship \"%s\"",
                   Ships[n].ship_name);
          break;
         case SRC_SHIP_DEPARTURE:
          sprintf(text, "the departure cue of ship \"%s\"",
                   Ships[n].ship_name);
    ...
    
    {
        ...
        Fred_main_wnd->MessageBox(msg, NULL, MB_YESNO | MB_ICONEXCLAMATION);
...
 int orders_reference_handler(int code, char *msg) 
{
    ...
    Fred_main_wnd->MessageBox(msg, "Warning",
    ...
    
    {
        ...
        if ((Ships[n].wingnum >= 0)
             && (query_initial_orders_conflict(Ships[n].wingnum)))
...
 int sexp_reference_handler(int node, int code, char *msg) 
{
    ...
    
    {
      ...
       Event_editor_dlg->SetWindowPos(&CWnd::wndTop, 0, 0, 0, 0,
                                        SWP_SHOWWINDOW | SWP_NOMOVE |
                                        SWP_NOSIZE);
...
 char *object_name(int obj) 
{
    ...
    
    {
      ...
      sprintf(text, "%s:%d", Waypoint_lists[i / 65536].name,
               (i & 0xffff) + 1);
...
 void object_moved(object * objp) 
{
                    ...
                    65536].waypoints[objp->instance & 0xffff] = objp->pos;
...
 void generate_weaponry_usage_list(int *arr, int wing) 
{
    ...
    
    {
        ...
        
        {
               ...
               && swp->primary_bank_weapons[j] < MAX_WEAPON_TYPES)
        ...
        
        {
               ...
               && swp->secondary_bank_weapons[j] < MAX_WEAPON_TYPES)
            
            {
              arr[swp->secondary_bank_weapons[j]] +=
                int (ceil
                     (swp->secondary_bank_ammo[j] *
                      swp->secondary_bank_capacity[j] / 100 /
                      (Weapon_info[swp->secondary_bank_weapons[j]].
                       cargo_size + 0.5f)));
...
} } }  void generate_weaponry_usage_list(int team, int *arr) 
{
    ...
    
    {
        ...
        {
          generate_weaponry_usage_list(arr,
                                        TVT_wings[(team *
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
              stuff_special_arrival_anchor_name(tmp, i, restrict_to_players,
                                                 0);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                pm =
                model_get(Ship_info[Ships[objp->instance].ship_info_index].
                          model_num);
                ...
                
                {
                    ...
                    box->AddString(Ships[get_ship_from_obj(objp)].ship_name);
...
// Goober5000
void stuff_special_arrival_anchor_name(char *buf, int iff_index,
                                       ...
                                       int retail_format) 
{
    ...
    // stupid retail hack
    if (retail_format && !stricmp(iff_name, "hostile")
        && !restrict_to_players)
...
// Goober5000
void stuff_special_arrival_anchor_name(char *buf, int anchor_num,
                                       int retail_format) 
{
  ...
  int restrict_to_players = (anchor_num & SPECIAL_ARRIVAL_ANCHOR_PLAYER_FLAG);
    ...
    // get name
    stuff_special_arrival_anchor_name(buf, iff_index, restrict_to_players,
                                      retail_format);
