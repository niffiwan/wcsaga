     ...
     void CModifyVariableDlg::OnDeleteVariable() 
{
    ...
    if (stricmp
        (Sexp_variables[get_sexp_var_index()].variable_name, temp_name))
    ...
    rval =
    MessageBox
    ("This will permanantly delete the variable.  Do you want to continue?",
     NULL, MB_OKCANCEL);
  ...
  bool CModifyVariableDlg::IsChangeSafe(char *message) 
{
  ...
  int num_counts =
    m_p_sexp_tree->get_variable_count(Sexp_variables[get_sexp_var_index()].
                                      variable_name);
    ...
    num_counts =
    m_p_sexp_tree->get_loadout_variable_count(get_sexp_var_index());
  if (num_counts > 0)
    
    {
      char buffer[256];
      sprintf(buffer, "%s Used in %d location(s) in loadout.", message,
               num_counts);
...
{
  m_type_player_persistent =
    ((CButton *) GetDlgItem(IDC_TYPE_PLAYER_PERSISTENT))->
    GetCheck()? true : false;
...
{
  m_type_campaign_persistent =
    ((CButton *) GetDlgItem(IDC_TYPE_CAMPAIGN_PERSISTENT))->
    GetCheck()? true : false;
...
{
  m_type_network_variable =
    ((CButton *) GetDlgItem(IDC_TYPE_NETWORK_VARIABLE))->
    GetCheck()? true : false;
...
}  void CModifyVariableDlg::OnSelchangeModifyVariableName() 
{
  ...
  int index = cbox->GetCurSel();
    ...
    if (m_modified_type || m_modified_persistence || m_modified_name
        || m_modified_value)
  ...
  int sexp_variable_index = get_sexp_var_index();
    ...
    m_type_number =
    ((Sexp_variables[sexp_variable_index].type & SEXP_VARIABLE_NUMBER) !=
     0) ? true : false;
  m_type_campaign_persistent =
    ((Sexp_variables[sexp_variable_index].
      type & SEXP_VARIABLE_CAMPAIGN_PERSISTENT) != 0) ? true : false;
  m_type_player_persistent =
    ((Sexp_variables[sexp_variable_index].
      type & SEXP_VARIABLE_PLAYER_PERSISTENT) != 0) ? true : false;
  m_type_network_variable =
    ((Sexp_variables[sexp_variable_index].type & SEXP_VARIABLE_NETWORK) !=
     0) ? true : false;
...
 BOOL CModifyVariableDlg::OnInitDialog() 
{
    ...
    
    {
        ...
        SetWindowText(Sexp_variables
                      [m_translate_combo_to_sexp[last_modified]].text);
    }
   
    // Set old variable name
    m_old_var_name =
    Sexp_variables[m_translate_combo_to_sexp[last_modified]].variable_name;
   
    // Set type
    m_type_number =
    ((Sexp_variables[last_modified].type & SEXP_VARIABLE_NUMBER) !=
     0) ? true : false;
  m_type_campaign_persistent =
    ((Sexp_variables[last_modified].
      type & SEXP_VARIABLE_CAMPAIGN_PERSISTENT) != 0) ? true : false;
  m_type_player_persistent =
    ((Sexp_variables[last_modified].type & SEXP_VARIABLE_PLAYER_PERSISTENT) !=
     0) ? true : false;
  m_type_network_variable =
    ((Sexp_variables[last_modified].type & SEXP_VARIABLE_NETWORK) !=
     0) ? true : false;
...
 void CModifyVariableDlg::set_variable_type() 
{
    ...
    ((CButton *) GetDlgItem(IDC_TYPE_CAMPAIGN_PERSISTENT))->
    SetCheck(m_type_campaign_persistent);
  ((CButton *) GetDlgItem(IDC_TYPE_PLAYER_PERSISTENT))->
    SetCheck(m_type_player_persistent);
  ((CButton *) GetDlgItem(IDC_TYPE_NETWORK_VARIABLE))->
    SetCheck(m_type_network_variable);
}  void CModifyVariableDlg::OnOK() 
{
    ...
    
    {
        ...
        
        {
            ...
            if (m_modified_name || m_modified_persistence || m_modified_value
                || m_modified_type)
...
void CModifyVariableDlg::validate_data(CString & temp_data, int set_focus) 
{
  ...
  int rval = strcspn(temp_data, "@()");
    ...
    
    {
      ...
      sprintf(message_text, "Invalid char '%c' in Default Value",
               temp_data[rval]);
...
void CModifyVariableDlg::validate_var_name(int set_focus) 
{
    ...
    Sexp_variables[m_translate_combo_to_sexp[cur_sel]].variable_name;
    ...
    
    {
      ...
      int rval = strcspn(temp_name, "@()");
        ...
        
        {
          ...
          sprintf(message_text, "Invalid char '%c' in Variable Name",
                   temp_name[rval]);
        ...
        
        {
            ...
            if ((index != -1)
                && (index !=
                    m_translate_combo_to_sexp[m_combo_last_modified_index]))
