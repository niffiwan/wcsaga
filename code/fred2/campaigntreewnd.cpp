...
ON_WM_DESTROY()ON_COMMAND(ID_CPGN_FILE_SAVE, OnCpgnFileSave)
...
ON_WM_CLOSE()ON_COMMAND(ID_INITIAL_SHIPS, OnInitialShips)
ON_COMMAND(ID_INITIAL_WEAPONS, OnInitialWeapons)
  //}}AFX_MSG_MAP
  END_MESSAGE_MAP()
/////////////////////////////////////////////////////////////////////////////
// campaign_tree_wnd message handlers
     BOOL campaign_tree_wnd::OnCreateClient(LPCREATESTRUCT,
                                            CCreateContext * pContext)
{
  ...
  if (!m_splitter.
      CreateView(0, 0, RUNTIME_CLASS(campaign_editor), CSize(0, 0), pContext))
    {
  ...
  if (!m_splitter.
      CreateView(0, 1, RUNTIME_CLASS(campaign_tree_view), CSize(240, 100),
                 pContext))
    {
...
void campaign_tree_wnd::OnCpgnFileSave()
{
  ...
  if (save.save_campaign_file(Campaign.pathname))
    {
      MessageBox("An error occured while saving!", "Error",
...
void campaign_tree_wnd::OnCpgnFileSaveAs()
{
  ...
  CFileDialog dlg(FALSE, "fc2", old_name,
...
void campaign_tree_wnd::OnCpgnFileNew()
{
  ...
  ((CButton *) (Campaign_tree_formp->GetDlgItem(IDC_CUSTOM_TECH_DB)))->
    SetCheck(0);
}
...
int campaign_tree_wnd::save_modified()
{
  ...
  r =
    MessageBox("This campaign has been modified.  Save changes first?",
               "Campaign Modified", MB_YESNOCANCEL | MB_ICONQUESTION);
...
int campaign_tree_wnd::error_checker()
{
  ...
  for (i = 0; i < Total_links; i++)
    {
      ...
      if (fred_check_sexp
          (Links[i].sexp, OPR_BOOL, "formula of branch #%d", i))
        return -1;
      ...
      if (Links[i].sexp == Locked_sexp_false)
        if (error
            ("Mission \"%s\" branch %d is always false",
             Campaign.missions[z].name, mcount[z]))
          return 1;
      ...
      if (Links[i].sexp == Locked_sexp_true)
        {
          if (true_at[z] >= 0)
            if (error
                ("Mission \"%s\" branch %d is true but is not last branch",
                 Campaign.missions[z].name, true_at[z]))
              return 1;
      ...
      for (i = 0; i < Campaign.num_missions; i++)
        {
          ...
          if (a_mission.num_players != Campaign.num_players)
            {
              if (error
                  ("Mission \"%s\" has %d players.  Multiplayer campaign allows %d",
                   Campaign.missions[i].name, a_mission.num_players,
  ...
  for (i = 0; i < Campaign.num_missions; i++)
    if (mcount[i] && true_at[i] < mcount[i])
      if (error
          ("Mission \"%s\" last branch isn't set to true",
  ...
  for (i = z = 0; i < Campaign.num_missions; i++)
    {
      for (j = 0; j < Campaign.num_missions; j++)
        if ((i != j)
            && !stricmp(Campaign.missions[i].name, Campaign.missions[j].name))
          return internal_error("Mission \"%s\" is listed twice in campaign",
                                Campaign.missions[i].name);
