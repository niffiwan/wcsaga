...
ON_WM_CLOSE()ON_BN_CLICKED(IDC_BROWSE_AVI, OnBrowseAvi)
  ...
  for (i = 0; i < Num_mission_events; i++)
    {
      ...
      if (get_operator_const(CTEXT(node)) == OP_WHEN
          || get_operator_const(CTEXT(node)) == OP_EVERY_TIME
      ...
      else
        {
          if (m_event_num >= 0)
          ...
          else
            {
              ...
              string_copy(Mission_events[m_event_num].name, m_message_name,
                          NAME_LENGTH - 1);
          ...
          ((CComboBox *) GetDlgItem(IDC_PRIORITY))->GetLBText(m_priority,
                                                              buf);
          node = alloc_sexp(buf, SEXP_ATOM, SEXP_ATOM_STRING, -1, node);
          ((CComboBox *) GetDlgItem(IDC_SENDER))->GetLBText(m_sender, buf);
          node = alloc_sexp(buf, SEXP_ATOM, SEXP_ATOM_STRING, -1, node);
          node =
            alloc_sexp("send-message", SEXP_ATOM, SEXP_ATOM_OPERATOR, -1,
                       node);
