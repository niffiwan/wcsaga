...
void ship_flags_dlg::DoDataExchange(CDataExchange * pDX)
{
  ...
  DDX_Control(pDX, IDC_TOGGLE_SUBSYSTEM_SCANNING,
  ...
  DDX_Control(pDX, IDC_FRIENDLY_STEALTH_INVISIBLE,
...
{
  int j, first;
  int protect_ship = 0, beam_protect_ship = 0, ignore_count =
    0, reinforcement = 0, cargo_known = 0;
  int destroy_before_mission = 0, no_arrival_music = 0, escort =
    0, invulnerable = 0, targetable_as_bomb = 0;
  int hidden_from_sensors = 0, primitive_sensors = 0, no_subspace_drive =
    0, affected_by_gravity = 0;
  int toggle_subsystem_scanning = 0, scannable = 0, kamikaze = 0, no_dynamic =
    0, red_alert_carry = 0;
  int special_warpin = 0, disable_messages = 0, guardian = 0, vaporize =
    0, stealth = 0, friendly_stealth_invisible = 0;
  int no_death_scream = 0, always_death_scream = 0;
  int nav_carry = 0, nav_needslink = 0, hide_ship_name =
    0, set_class_dynamically = 0;
              ...
              if (first)
                {
                  ...
                  red_alert_carry =
                    (shipp->flags & SF_RED_ALERT_STORE_STATUS) ? 1 : 0;
                  special_warpin = (objp->flags & OF_SPECIAL_WARPIN) ? 1 : 0;
                  protect_ship = (objp->flags & OF_PROTECTED) ? 1 : 0;
                  beam_protect_ship =
                    (objp->flags & OF_BEAM_PROTECTED) ? 1 : 0;
                  invulnerable = (objp->flags & OF_INVULNERABLE) ? 1 : 0;
                  targetable_as_bomb =
                    (objp->flags & OF_TARGETABLE_AS_BOMB) ? 1 : 0;
                  hidden_from_sensors =
                    (shipp->flags & SF_HIDDEN_FROM_SENSORS) ? 1 : 0;
                  primitive_sensors =
                    (shipp->flags2 & SF2_PRIMITIVE_SENSORS) ? 1 : 0;
                  no_subspace_drive =
                    (shipp->flags2 & SF2_NO_SUBSPACE_DRIVE) ? 1 : 0;
                  affected_by_gravity =
                    (shipp->flags2 & SF2_AFFECTED_BY_GRAVITY) ? 1 : 0;
                  toggle_subsystem_scanning =
                    (shipp->flags2 & SF2_TOGGLE_SUBSYSTEM_SCANNING) ? 1 : 0;
                  ignore_count = (shipp->flags & SF_IGNORE_COUNT) ? 1 : 0;
                  no_arrival_music =
                    (shipp->flags & SF_NO_ARRIVAL_MUSIC) ? 1 : 0;
                  cargo_known = (shipp->flags & SF_CARGO_REVEALED) ? 1 : 0;
                  no_dynamic =
                    (Ai_info[shipp->ai_index].
                     ai_flags & AIF_NO_DYNAMIC) ? 1 : 0;
                  disable_messages =
                    (shipp->flags2 & SF2_NO_BUILTIN_MESSAGES) ? 1 : 0;
                  set_class_dynamically =
                    (shipp->flags2 & SF2_SET_CLASS_DYNAMICALLY) ? 1 : 0;
                  no_death_scream =
                    (shipp->flags2 & SF2_NO_DEATH_SCREAM) ? 1 : 0;
                  always_death_scream =
                    (shipp->flags2 & SF2_ALWAYS_DEATH_SCREAM) ? 1 : 0;
                  ...
                  friendly_stealth_invisible =
                    (shipp->flags2 & SF2_FRIENDLY_STEALTH_INVIS) ? 1 : 0;
                  nav_carry = (shipp->flags2 & SF2_NAVPOINT_CARRY) ? 1 : 0;
                  nav_needslink =
                    (shipp->flags2 & SF2_NAVPOINT_NEEDSLINK) ? 1 : 0;
                  hide_ship_name =
                    (shipp->flags2 & SF2_HIDE_SHIP_NAME) ? 1 : 0;
                  hide_log_entries =
                    (shipp->flags2 & SF2_HIDE_LOG_ENTRIES) ? 1 : 0;
                  is_harmless = (shipp->flags2 & SF2_IS_HARMLESS) ? 1 : 0;

                  destroy_before_mission =
                    (shipp->flags & SF_KILL_BEFORE_MISSION) ? 1 : 0;
                  m_destroy_value.init(shipp->final_death_time);

                  kamikaze =
                    (Ai_info[shipp->ai_index].
                     ai_flags & AIF_KAMIKAZE) ? 1 : 0;
                  m_kdamage.init((int) Ai_info[shipp->ai_index].
                                 kamikaze_damage);
              ...
              else
                {
                  ...
                  scannable =
                    tristate_set(shipp->flags & SF_SCANNABLE, scannable);
                  red_alert_carry =
                    tristate_set(shipp->flags & SF_RED_ALERT_STORE_STATUS,
                                 red_alert_carry);
                  special_warpin =
                    tristate_set(objp->flags & OF_SPECIAL_WARPIN,
                                 special_warpin);
                  protect_ship =
                    tristate_set(objp->flags & OF_PROTECTED, protect_ship);
                  beam_protect_ship =
                    tristate_set(objp->flags & OF_BEAM_PROTECTED,
                                 beam_protect_ship);
                  invulnerable =
                    tristate_set(objp->flags & OF_INVULNERABLE, invulnerable);
                  targetable_as_bomb =
                    tristate_set(objp->flags & OF_TARGETABLE_AS_BOMB,
                                 targetable_as_bomb);
                  hidden_from_sensors =
                    tristate_set(shipp->flags & SF_HIDDEN_FROM_SENSORS,
                                 hidden_from_sensors);
                  primitive_sensors =
                    tristate_set(shipp->flags2 & SF2_PRIMITIVE_SENSORS,
                                 primitive_sensors);
                  no_subspace_drive =
                    tristate_set(shipp->flags2 & SF2_NO_SUBSPACE_DRIVE,
                                 no_subspace_drive);
                  affected_by_gravity =
                    tristate_set(shipp->flags2 & SF2_AFFECTED_BY_GRAVITY,
                                 affected_by_gravity);
                  toggle_subsystem_scanning =
                    tristate_set(shipp->
                                 flags2 & SF2_TOGGLE_SUBSYSTEM_SCANNING,
                                 toggle_subsystem_scanning);
                  ignore_count =
                    tristate_set(shipp->flags & SF_IGNORE_COUNT,
                                 ignore_count);
                  no_arrival_music =
                    tristate_set(shipp->flags & SF_NO_ARRIVAL_MUSIC,
                                 no_arrival_music);
                  cargo_known =
                    tristate_set(shipp->flags & SF_CARGO_REVEALED,
                                 cargo_known);
                  no_dynamic =
                    tristate_set(Ai_info[shipp->ai_index].
                                 ai_flags & AIF_NO_DYNAMIC, no_dynamic);
                  disable_messages =
                    tristate_set(shipp->flags2 & SF2_NO_BUILTIN_MESSAGES,
                                 disable_messages);
                  set_class_dynamically =
                    tristate_set(shipp->flags2 & SF2_SET_CLASS_DYNAMICALLY,
                                 set_class_dynamically);
                  no_death_scream =
                    tristate_set(shipp->flags2 & SF2_NO_DEATH_SCREAM,
                                 no_death_scream);
                  always_death_scream =
                    tristate_set(shipp->flags2 & SF2_ALWAYS_DEATH_SCREAM,
                                 always_death_scream);
                  guardian =
                    tristate_set(shipp->ship_guardian_threshold, guardian);
                  vaporize =
                    tristate_set(shipp->flags & SF_VAPORIZE, vaporize);
                  stealth =
                    tristate_set(shipp->flags2 & SF2_STEALTH, stealth);
                  friendly_stealth_invisible =
                    tristate_set(shipp->flags2 & SF2_FRIENDLY_STEALTH_INVIS,
                                 friendly_stealth_invisible);
                  nav_carry =
                    tristate_set(shipp->flags2 & SF2_NAVPOINT_CARRY,
                                 nav_carry);
                  nav_needslink =
                    tristate_set(shipp->flags2 & SF2_NAVPOINT_NEEDSLINK,
                                 nav_needslink);
                  hide_ship_name =
                    tristate_set(shipp->flags2 & SF2_HIDE_SHIP_NAME,
                                 hide_ship_name);
                  hide_log_entries =
                    tristate_set(shipp->flags2 & SF2_HIDE_LOG_ENTRIES,
                                 hide_log_entries);
                  is_harmless =
                    tristate_set(shipp->flags2 & SF2_IS_HARMLESS,
                                 is_harmless);
                  ...
                  destroy_before_mission =
                    tristate_set(shipp->flags & SF_KILL_BEFORE_MISSION,
                                 destroy_before_mission);
                  m_destroy_value.set(shipp->final_death_time);

                  kamikaze =
                    tristate_set(Ai_info[shipp->ai_index].
                                 ai_flags & AIF_KAMIKAZE, kamikaze);
                  m_kdamage.set((int) Ai_info[shipp->ai_index].
                                kamikaze_damage);
