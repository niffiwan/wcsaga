...
 BOOL event_editor::OnInitDialog() 
{
  ...
  m_play_bm.LoadBitmap(IDB_PLAY);
    ...
    
    {
        ...
        {
          m_messages[i].wave_info.name =
            strdup(m_messages[i].wave_info.name);
        }
    }
   ((CEdit *) GetDlgItem(IDC_MESSAGE_NAME))->LimitText(NAME_LENGTH - 1);
  ((CEdit *) GetDlgItem(IDC_MESSAGE_TEXT))->LimitText(MESSAGE_LENGTH - 1);
  ((CComboBox *) GetDlgItem(IDC_AVI_FILENAME))->LimitText(MAX_FILENAME_LEN -
                                                           1);
  ((CComboBox *) GetDlgItem(IDC_WAVE_FILENAME))->LimitText(MAX_FILENAME_LEN -
                                                            1);
...
 void event_editor::load_tree() 
{
    ...
    
    {
        ...
        {
          m_events[i].objective_text =
            strdup(Mission_events[i].objective_text);
        ...
        {
          m_events[i].objective_key_text =
            strdup(Mission_events[i].objective_key_text);
        ...
        m_event_tree.load_sub_tree(Mission_events[i].formula, false,
                                   "do-nothing");
...
 int event_editor::query_modified() 
{
    ...
    
    {
           ...
           (m_events[i].objective_text, Mission_events[i].objective_text))
...
 void event_editor::OnOk() 
{
    ...
    
    {
      ...
      Mission_events[i].formula =
        m_event_tree.save_tree(m_events[i].formula);
...
 void event_editor::reset_event(int num, HTREEITEM after) 
{
    ...
    m_event_tree.insert(m_events[num].name, BITMAP_ROOT, BITMAP_ROOT,
                        TVI_ROOT, after);
...
 int event_editor::save_message(int num) 
{
    ...
    
    {
      ...
       string_copy(m_messages[num].message, m_message_text,
                     MESSAGE_LENGTH - 1);
...
 void event_editor::OnBrowseAvi() 
{
  ...
  CFileDialog dlg(TRUE, "ani", m_avi_filename,
...
 void event_editor::OnBrowseWave() 
{
  ...
   CFileDialog dlg(TRUE, "wav", m_wave_filename,
                     OFN_HIDEREADONLY | OFN_FILEMUSTEXIST | OFN_NOCHANGEDIR,
...
 void event_editor::update_persona() 
{
  ...
   if ((m_wave_filename[0] >= '1') && (m_wave_filename[0] <= '9')
         && (m_wave_filename[1] == '_'))
...
 void event_editor::OnPlay() 
{
    ...
    m_wave_id =
    audiostream_open((char *) (LPCSTR) m_wave_filename, ASF_EVENTMUSIC);
