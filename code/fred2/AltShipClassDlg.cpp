  ...
  for (i = 0; i < (int) alt_class_pool.size(); i++)
    {
      ...
      if (alt_class_pool[i].default_to_this_class
          && (i < (int) alt_class_pool.size() - 1))
        {
          // Int3();
      ...
      if (alt_class_pool[i].variable_index != -1)
        {
          Assert(alt_class_pool[i].variable_index > -1
                 && alt_class_pool[i].variable_index < MAX_SEXP_VARIABLES);
          Assert(Sexp_variables[alt_class_pool[i].variable_index].
                 type & SEXP_VARIABLE_STRING);

          sprintf(buff, "%s[%s]",
                  Sexp_variables[alt_class_pool[i].variable_index].
                  variable_name,
        ...
        {
          if (alt_class_pool[i].ship_class >= 0
              && alt_class_pool[i].ship_class < MAX_SHIP_CLASSES)
            {
              m_alt_class_list.
                AddString(Ship_info[alt_class_pool[i].ship_class].name);
            }
          else
            {
...
void AltShipClassDlg::alt_class_update_entry(alt_class & list_item)
{
  ...
  list_item.default_to_this_class =
    m_default_to_class.GetCheck()? true : false;
}
...
void AltShipClassDlg::OnSelchangeAltClassList()
{
  ...
  if (alt_class_pool[index].variable_index != -1)
    {
      for (i = 0; i < MAX_SEXP_VARIABLES; i++)
        {
          if (string_variable_indices[i] ==
              alt_class_pool[index].variable_index)
            {
  ...
  m_default_to_class.SetCheck(alt_class_pool[index].
                              default_to_this_class ? 1 : 0);
}
...
void AltShipClassDlg::OnDefaultToClass()
{
  ...
  alt_class_pool[index].default_to_this_class =
    m_default_to_class.GetCheck()? true : false;
}
