...
campaign_editor::campaign_editor():CFormView(campaign_editor::IDD)
{
...
ON_EN_CHANGE(IDC_CAMPAIGN_INTRO_CUTSCENE,
             &campaign_editor::
             OnEnChangeCampaignIntroCutscene)
ON_EN_CHANGE(IDC_CAMPAIGN_END_CUTSCENE,
             &campaign_editor::
             OnEnChangeCampaignEndCutscene) ON_BN_CLICKED(IDC_RESET_RANK,
                                                          &campaign_editor::
                                                          OnBnClickedResetRank)
  //}}AFX_MSG_MAP
END_MESSAGE_MAP()
/////////////////////////////////////////////////////////////////////////////
// campaign_editor diagnostics
#ifdef _DEBUG
     void campaign_editor::AssertValid() const const
{
  CFormView::AssertValid();
}

void campaign_editor::Dump(CDumpContext & dc) const const
{
  CFormView::Dump(dc);
}
#endif //_DEBUG
...
void campaign_editor::OnLoad()
{
  ...
  cf_find_file_location(Campaign.missions[Cur_campaign_mission].name,
                        CF_TYPE_MISSIONS, 512, buf, &size, &offset, false);
...
BOOL campaign_editor::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName,
                             ...
                             CCreateContext * pContext)
{
  ...
  r =
    CFormView::Create(lpszClassName, lpszWindowName, dwStyle, rect,
  ...
  for (i = 0; i < Total_links; i++)
    {
      if (Links[i].from == Cur_campaign_mission)
        {
          Links[i].node =
            m_tree.load_sub_tree(Links[i].sexp, true, "do-nothing");
          m_num_links++;
          ...
          if (Links[i].from == Links[i].to)
            ...
            {
              sprintf(text, "Branch to %s",
                      Campaign.missions[Links[i].to].name);
            }
      ...
      if ((last != -1) && (i < Total_links))
        {
          ...
          m_tree.SelectItem(m_tree.
                            GetParentItem(m_tree.handle(Links[i].node)));
...
void campaign_editor::swap_handler(int node1, int node2)
{
  ...
  for (index1 = 0; index1 < Total_links; index1++)
    {
      if ((Links[index1].from == Cur_campaign_mission)
          && (Links[index1].node == node1))
        {
          break;
        }
    }
  ...
  for (index2 = 0; index2 < Total_links; index2++)
    {
      if ((Links[index2].from == Cur_campaign_mission)
          && (Links[index2].node == node2))
        {
          break;
        }
    }
  ...
  if ((Cur_campaign_link >= 0) && Links[Cur_campaign_link].mission_loop)
    {
      ...
      deconvert_multiline_string(buffer, m_loop_brief_sound,
                                 MAX_FILENAME_LEN);
      if (strlen(buffer))
        {
...
void campaign_editor::update_loop_desc_window()
{
  ...
  GetDlgItem(IDC_LOOP_BRIEF_SOUND_BROWSE)->
    EnableWindow(enable_loop_desc_window);

  // set new text
  if ((Cur_campaign_link >= 0) && Links[Cur_campaign_link].mission_loop_txt
      && enable_loop_desc_window)
    {
      m_loop_desc =
        convert_multiline_string(Links[Cur_campaign_link].mission_loop_txt);
    }
  else
    {
      m_loop_desc = _T("");
  ...
  if ((Cur_campaign_link >= 0)
    ...
    {
      m_loop_brief_anim =
        convert_multiline_string(Links[Cur_campaign_link].
                                 mission_loop_brief_anim);
    }
  else
    {
      m_loop_brief_anim = _T("");
...
void campaign_editor::OnToggleLoop()
{
  ...
  if ((Cur_campaign_link >= 0) && Links[Cur_campaign_link].mission_loop)
    {
      ...
      deconvert_multiline_string(buffer, m_loop_brief_anim,
                                 MISSION_DESC_LENGTH);
  ...
  Links[Cur_campaign_link].mission_loop =
    !Links[Cur_campaign_link].mission_loop;
