     ...
     BOOL DumpStats::OnInitDialog()
{
  ...
  ((CEdit *) GetDlgItem(IDC_STATS_TEXT))->SetTabStops(num_tab_stops,
                                                      tab_stops);
...
void DumpStats::OnDumpToFile()
{
  ...
  fp =
    cfopen((char *) LPCTSTR(dump_filename), "wt", CFILE_NORMAL,
           CF_TYPE_MISSIONS);
              ...
              for (i = 0; i < (int) Species_info.size(); i++)
                {
                  if (Asteroid_field.field_debris_type[i] >= 0)
                    {
                      temp +=
                        CString(Species_info
                                [(Asteroid_field.field_debris_type[i] /
                                  NUM_DEBRIS_SIZES) - 1].species_name) + " ";
                    }
                }
...
void DumpStats::get_objectives_and_goals(CString & buffer)
{
  ...
  for (i = 0; i < Num_mission_events; i++)
    {
      ...
      temp.Format("\tObjective: %s,  text: %s,  key_text: %s\r\n",
                  Mission_events[i].name, Mission_events[i].objective_text,
                  Mission_events[i].objective_key_text);
      buffer += temp;
    }
  ...
  for (i = 0; i < Num_goals; i++)
    {
      temp.Format("\tGoal: %s, text: ", Mission_goals[i].name,
                  Mission_goals[i].message);
      buffer += temp;
  ...
  for (i = 0; i < Num_teams; i++)
    {
      ...
      for (j = 0; j < Team_data[i].num_ship_choices; j++)
        {
          temp.Format("\tShip name: %s, count %d",
                      Ship_info[Ships[Team_data[i].ship_list[j]].
      ...
      for (int i = 0; i < MAX_WINGS; i++)
        {
          if (Wings[i].wave_count > 0)
            {
              int wing_leader_shipnum =
                Wings[i].ship_index[Wings[i].special_ship];
              if (Ship_info[Ships[wing_leader_shipnum].ship_info_index].
                  species == species)
                {
                  if (Ship_info[Ships[wing_leader_shipnum].ship_info_index].
                      flags & SIF_FIGHTER)
                    {
                      temp.
                        ...
                        ("\t\tWing: %s, count: %d, waves: %d, type: %s\r\n",
                         ...
                         Wings[i].num_waves,
                         Ship_info[Ships[wing_leader_shipnum].
                                   ship_info_index].name);
                      buffer += temp;
                    }
                }
              ...
              if (Ship_info[shipp->ship_info_index].species == species)
                {
                  ...
                  //if (shipp->cargo1 > 0)
                  if (Ship_info[shipp->ship_info_index].
                      flags & (SIF_NAVBUOY | SIF_ESCAPEPOD | SIF_SENTRYGUN))
                    {
                      temp.Format("\t\tName: %s, Type: %s Cargo: %s\r\n",
                                  shipp->ship_name,
                                  Ship_info[shipp->ship_info_index].name,
              ...
              if (Ship_info[shipp->ship_info_index].species == species)
                {
                  ...
                  //if (shipp->cargo1 > 0)
                  if (Ship_info[shipp->ship_info_index].
                      flags & (SIF_DRYDOCK | SIF_CAPITAL | SIF_SUPERCAP))
                    {
                      temp.Format("\t\tName: %s, Type: %s, Cargo: %s\r\n",
                                  shipp->ship_name,
                                  Ship_info[shipp->ship_info_index].name,
  ...
  for (pri_idx = 0; pri_idx < shipp->weapons.num_primary_banks; pri_idx++)
    {
      if (shipp->weapons.primary_bank_weapons[pri_idx] == -1)
        ...
        {
          weapon_name =
            Weapon_info[shipp->weapons.primary_bank_weapons[pri_idx]].name;
        }
  ...
  for (sec_idx = 0; sec_idx < shipp->weapons.num_secondary_banks; sec_idx++)
    {
      if (shipp->weapons.secondary_bank_weapons[sec_idx] == -1)
        ...
        {
          weapon_name =
            Weapon_info[shipp->weapons.secondary_bank_weapons[sec_idx]].name;
        }
      ...
      if ((ss->system_info->type == SUBSYSTEM_TURRET))
        {
          ...
          // PRIMARY
          for (pri_idx = 0; pri_idx < ss->weapons.num_primary_banks;
               pri_idx++)
            {
              if (ss->weapons.primary_bank_weapons[pri_idx] == -1)
                ...
                {
                  weapon_name =
                    Weapon_info[ss->weapons.primary_bank_weapons[pri_idx]].
                    name;
                }
              temp.Format("\t\t\t\tPrimary[%d]: %s\r\n", pri_idx + 1,
                          weapon_name);
          ...
          // SECONDARY
          for (sec_idx = 0; sec_idx < ss->weapons.num_secondary_banks;
               sec_idx++)
            {
              if (ss->weapons.secondary_bank_weapons[sec_idx] == -1)
                ...
                {
                  weapon_name =
                    Weapon_info[ss->weapons.secondary_bank_weapons[sec_idx]].
                    name;
                }
              temp.Format("\t\t\t\tSecondary[%d]: %s\r\n", sec_idx + 1,
                          weapon_name);
              ...
              if (Ship_info[shipp->ship_info_index].species == species)
                {
                  ...
                  //if (shipp->cargo1 > 0)
                  if (Ship_info[shipp->ship_info_index].
                      flags & (SIF_FREIGHTER | SIF_TRANSPORT))
                    {
                      temp.Format("\t\tName: %s\r\n", shipp->ship_name);
                      buffer += temp;
              ...
              if (Ship_info[shipp->ship_info_index].species == species)
                {
                  ...
                  //if (shipp->cargo1 > 0)
                  if (Ship_info[shipp->ship_info_index].
                      flags & (SIF_ESCAPEPOD | SIF_SENTRYGUN))
                    {
                      temp.Format("\t\tName: %s\r\n", shipp->ship_name);
                      buffer += temp;
                      dump_loadout(shipp, loadout);
              ...
              if (Ship_info[shipp->ship_info_index].species == species)
                {
                  ...
                  //if (shipp->cargo1 > 0)
                  if (Ship_info[shipp->ship_info_index].
                      flags & (SIF_CAPITAL | SIF_SUPERCAP))
                    {
                      temp.Format("\t\tName: %s\r\n", shipp->ship_name);
                      buffer += temp;
                      dump_loadout(shipp, loadout);
