...
void initial_status_mark_dock_leader_helper(object * objp,
  ...
  BOOL initial_status::OnInitDialog() 
{
    ...
    {
      Assert((Objects[cur_object_index].type == OBJ_SHIP)
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
              dockpoint_array[i].dockee_point =
                dock_find_dockpoint_used_by_object(docked_objp, objp);
        ...
        
        {
            ...
            
            {
                ...
                
                {
                        ...
                        || (objp->type == OBJ_START));
               if (Ships[get_ship_from_obj(objp)].
                     flags2 & SF2_FORCE_SHIELDS_ON)
                ...
                
                {
                     ...
                     flags2 & SF2_LOCK_ALL_TURRETS_INITIALLY)
    ...
    
    {
        ...
        
        {
            ...
            lstDockerPoints->
            AddString(model_get_dock_name
                      (Ship_info[Ships[m_ship].ship_info_index].model_num,
                       dockpoint));
          lstDockerPoints->SetItemData(z, dockpoint);
      }  for (ptr = GET_FIRST(&Ships[m_ship].subsys_list);
                   ptr != END_OF_LIST(&Ships[m_ship].subsys_list);
                   ptr = GET_NEXT(ptr))
        
        {
          ((CListBox *) GetDlgItem(IDC_SUBSYS))->AddString(ptr->system_info->
                                                            subobj_name);
...
 void initial_status::OnOK() 
{
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                {
                  Ships[get_ship_from_obj(objp)].flags2 |=
                    SF2_FORCE_SHIELDS_ON;
                ...
                
                {
                    ...
                    SF2_LOCK_ALL_TURRETS_INITIALLY;
    ...
    
    {
      ...
      MODIFY(Objects[cur_object_index].shield_quadrant[0],
              (float) m_shields);
...
 void initial_status::OnHasShields() 
{
    ...
    
    {
        ...
        
        {
          ...
          ((CButton *) GetDlgItem(IDC_FORCE_SHIELDS))->
            SetCheck(m_force_shields);
...
}  void initial_status::change_subsys() 
{
    ...
    // Goober5000
    ship_has_scannable_subsystems =
    (Ship_info[Ships[m_ship].ship_info_index].flags & SIF_HUGE_SHIP);
    ...
    
    {
        ...
        
        {
           ...
           && valid_cap_subsys_cargo_list(ptr->system_info->subobj_name))
...
}  void initial_status::change_docker_point(bool store_selection) 
{
    ...
    
    {
        ...
        list_dockees(model_get_dock_index_type
                     (Ship_info[Ships[m_ship].ship_info_index].model_num,
                      cur_docker_point));
        ...
        
        {
            ...
            // select the dockee
            cboDockees->SelectString(-1,
                                     Ships[dockpoint_array[cur_docker_point].
                                           dockee_shipnum].ship_name);
...
 void initial_status::list_dockees(int dock_types) 
{
    ...
    
    {
        ...
        
        {
            ...
            if (!ship_docking_valid(m_ship, ship)
                && !ship_docking_valid(ship, m_ship))
            continue;
           
            // dock types must match
            if (!
                (model_get_dock_types
                 (Ship_info[Ships[ship].ship_info_index].
                  model_num) & dock_types))
...
 void initial_status::list_dockee_points(int shipnum) 
{
  ...
  int dock_type =
    model_get_dock_index_type(Ship_info[shipp->ship_info_index].model_num,
                              cur_docker_point);
   
    // populate with the right kind of dockee points
    for (int i = 0;
         i <
         model_get_num_dock_points(Ship_info[other_shipp->ship_info_index].
                                   model_num); i++)
    
    {
        ...
        object * docked_objp =
        dock_find_object_at_dockpoint(&Objects[other_shipp->objnum], i);
      if ((docked_objp != NULL) && (docked_objp != &Objects[shipp->objnum]))
        continue;
       
        // make sure its type matches
        if (!
            (model_get_dock_index_type
             (Ship_info[other_shipp->ship_info_index].model_num,
              i) & dock_type))
        ...
        cboDockeePoints->
        AddString(model_get_dock_name
                  (Ship_info[other_shipp->ship_info_index].model_num, i));
...
 void initial_status::update_docking_info() 
{
    ...
    
    {
        ...
        
        {
            ...
            if (objp !=
                dock_find_object_at_dockpoint(dockee_objp,
                                              dockpoint_array[i].
                                              dockee_point))
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                &Objects[Ships[dockpoint_array[i].dockee_shipnum].objnum];
...
}  void initial_status::dock(object * objp, int dockpoint,
                                object * other_objp, int other_dockpoint) 
{
    ...
    ai_dock_with_object(other_objp, other_dockpoint, objp, dockpoint,
                        AIDO_DOCK_NOW);
   
    // unmark the handled flag in preparation for the next step
    dock_evaluate_all_docked_objects(objp, &dfi,
                                     initial_status_unmark_dock_handled_flag);
    ...
    // set the dock leader
    dock_evaluate_all_docked_objects(objp, &dfi,
                                     initial_status_mark_dock_leader_helper);
...
 void initial_status::undock(object * objp1, object * objp2) 
{
         ...
         (Ships[ship_num].ship_info_index,
          Ships[other_ship_num].ship_info_index) > 0)
    ...
    
    {
           ...
           && Ships[other_ship_num].wingnum < 0)
                                            ...
                                            dock_function_info * infop) 
{
    ...
    
    {
        ...
        
        {
            ...
            if (ship_class_compare
                (shipp->ship_info_index, leader_shipp->ship_info_index) < 0)
...
void reset_arrival_to_false(int shipnum, bool reset_wing) 
{
    ...
    
    {
               ...
               "Setting arrival cue of ship %s\nto false for initial docking purposes.",
               shipp->ship_name);
...
 void initial_status::OnPrimariesLocked() 
{
    ...
    ((CButton *) GetDlgItem(IDC_PRIMARIES_LOCKED))->
    SetCheck(m_primaries_locked);
...
 void initial_status::OnSecondariesLocked() 
{
    ...
    ((CButton *) GetDlgItem(IDC_SECONDARIES_LOCKED))->
    SetCheck(m_secondaries_locked);
...
 void initial_status::OnAfterburnersLocked() 
{
    ...
    ((CButton *) GetDlgItem(IDC_AFTERBURNER_LOCKED))->
    SetCheck(m_afterburner_locked);
