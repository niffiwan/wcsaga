...
 void debriefing_editor_dlg::update_data(int update) 
{
    ...
    
    {
      ...
      deconvert_multiline_string(ptr->new_recommendation_text, m_rec_text,
                                  MAX_RECOMMENDATION_LEN);
      lcl_fred_replace_stuff(ptr->new_recommendation_text,
                              MAX_RECOMMENDATION_LEN);
...
 void debriefing_editor_dlg::copy_stage(int from, int to,
                                          int clear_formula) 
{
  ...
   strcpy(Debriefing->stages[to].new_text,
            Debriefing->stages[from].new_text);
  strcpy_s(Debriefing->stages[to].voice, Debriefing->stages[from].voice);
  strcpy(Debriefing->stages[to].new_recommendation_text,
          Debriefing->stages[from].new_recommendation_text);
