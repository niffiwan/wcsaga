  ...
  BOOL CFREDDoc::OnSaveDocument(LPCTSTR pathname) 
{
  ...
  while (len--)
    ...
    {
      Fred_main_wnd->MessageBox("An error occured while saving!", NULL,
...
 int CFREDDoc::check_undo() 
{
  ...
    cf_create_default_path_string(name, sizeof(name) - 1,
                                     CF_TYPE_MISSIONS);
...
int CFREDDoc::load_mission(char *pathname, int flags) 
{
    ...
    
    {
        ...
        
        {
            ...
            if ((Team_data[i].weaponry_pool_variable[j] == -1)
                && (Team_data[i].weaponry_amount_variable[j] == -1))
        ...
        
        {
            ...
            
            {
                ...
                Team_data[i].weaponry_pool[Team_data[i].num_weapon_choices] =
                j;
              Team_data[i].weaponry_count[Team_data[i].num_weapon_choices] =
                used_pool[j];
              Team_data[i].weaponry_amount_variable[Team_data[i].
                                                     num_weapon_choices] = -1;
              Team_data[i].weaponry_pool_variable[Team_data[i].
                                                   num_weapon_choices++] = -1;
    ...
    
    {
        ...
        
        {
            ...
            {
              mission_parse_lookup_alt_index(Ships[objp->instance].
                                              alt_type_index,
                                              Fred_alt_names[objp->instance]);
            ...
            {
              mission_parse_lookup_callsign_index(Ships[objp->instance].
                                                   callsign_index,
                                                   Fred_callsigns[objp->
                                                                  instance]);
...
#ifdef _DEBUG
void CFREDDoc::AssertValid() const const 
{
  CDocument::AssertValid();
}  void CFREDDoc::Dump(CDumpContext & dc) const const 
{
...
#else   /*  */
    CFolderDialog dlgFolder(_T("Select a location to save in"),
                            fs2_mission_path, NULL);
...
 void CFREDDoc::OnFileImportWeapons() 
{
    ...
    
    {
      ...
       MessageBox(NULL, error_str, "Unable to import weapon loadouts!",
                    MB_ICONERROR | MB_OK);
    ...
    read_file_text(ships_tbl, CF_TYPE_ANY, ships_tbl_text,
                   ships_tbl_text_raw);
    ...
    if (!Fred_view_wnd->global_error_check())
      ...
      MessageBox("Weapon loadouts successfully imported with no errors.",
                 "Woohoo!");
