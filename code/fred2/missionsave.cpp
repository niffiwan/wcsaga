  ...
  if (total_variables > 0)
    {
      ...
      for (i = 0; i < num_variables; i++)
        {
          ...
          fout("\n\t\t%d\t\t\"%s\"\t\t\"%s\"\t\t\"%s\"", i,
               Sexp_variables[i].variable_name, Sexp_variables[i].text, type);
      ...
      for (i = MAX_SEXP_VARIABLES - num_block_vars; i < MAX_SEXP_VARIABLES;
           i++)
        {
          type = block;
          fout("\n\t\t%d\t\t\"%s\"\t\t\"%s\"\t\t\"%s\"", i,
               Block_variables[i].variable_name, Block_variables[i].text,
               type);
        }
          ...
          if (Team_data[i].weaponry_amount_variable[j] != -1)
            {
              ...
              fout("@%s\n",
                   Sexp_variables[Team_data[i].weaponry_amount_variable[j]].
                   variable_name);
            }
          else
            {
              ...
              for (k = 0; k < (int) Ships[i].s_alt_classes.size(); k++)
                {
                  ...
                  if (Ships[i].s_alt_classes[k].variable_index != -1)
                    ...
                    {
                      fout_version("\n;;FSO 3.6.10;; $Alt Ship Class: \"%s\"",
                                   Ship_info[Ships[i].s_alt_classes[k].
                                             ship_class].name);
                    }
              ...
              if (optional_string_fred("$Special Explosion:", "$Name:"))
                {
                  ...
                  if (Ships[i].use_shockwave
                      && (Ships[i].special_exp_shockwave_speed > 0))
                    {
          ...
          if (Ships[i].use_special_explosion)
            {
              ...
              if (has_special_explosion_block_index
                  (&Ships[i], &special_exp_index))
                {
                  fout("\n+Special Exp index:");
  ...
  while (ptr != END_OF_LIST(&shipp->subsys_list) && ptr)
    {
      // Crashing here!
      if ((ptr->current_hits)
          || (ptr->system_info && ptr->system_info->type == SUBSYSTEM_TURRET)
