...
vec3d view_pos, eye_pos, Viewer_pos, Last_eye_pos =
{
0.0f};
...
matrix view_orient = IDENTITY_MATRIX, eye_orient, Last_eye_orient =
  IDENTITY_MATRIX;
...
void process_controls(vec3d * pos, matrix * orient, float frametime, int key,
                       int mode = 0);
...
void fred_enable_htl() 
{
    ...
    gr_set_proj_matrix((4.0f / 9.0f) * 3.14159f * FRED_DEFAULT_HTL_FOV,
                       ...
                       (float) gr_screen.clip_height, 1.0f,
...
//
  vec3d * get_subsystem_world_pos2(object * parent_obj, ship_subsys * subsys,
                                    vec3d * world_pos) 
{
...
int get_subsys_bounding_rect(object * ship_obj, ship_subsys * subsys, int *x1,
                             int *x2, int *y1, int *y2) 
{
    ...
    
    {
        ...
        subobj_find_2d_bound(subsys->system_info->radius, &ship_obj->orient,
                             &subobj_pos, x1, y1, x2, y2);
...
}  void fredhtl_render_subsystem_bounding_box(subsys_to_render * s2r) 
{
    ...
    model_get(Ship_info[Ships[s2r->ship_obj->instance].ship_info_index].
              model_num);
    ...
    g3_start_instance_matrix(&s2r->ship_obj->pos, &s2r->ship_obj->orient,
                             true);
    ...
    if ((s2r->cur_subsys->system_info->turret_gun_sobj >= 0)
        && (s2r->cur_subsys->system_info->turret_gun_sobj !=
            s2r->cur_subsys->system_info->subobj_num))
    
    {
      bsp_info * bsp_turret =
        &pm->submodel[s2r->cur_subsys->system_info->turret_gun_sobj];
      ...
       g3_start_instance_matrix(&bsp_turret->offset, &vmd_identity_matrix,
                                  true);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                // get subsys name
                strcpy_s(buf,
                         Render_subsys.cur_subsys->system_info->subobj_name);
               if (Cmdline_nohtl)
                
                {
                  
                    // get bounding box
                    if (get_subsys_bounding_rect
                        (objp, Render_subsys.cur_subsys, &x1, &x2, &y1, &y2))
...
 void render_one_model_nohtl(object * objp) 
{
    ...
    
    {
        ...
        iff_get_color_by_team_and_object(Ships[objp->instance].team, -1, true,
                                         objp);
       Fred_outline =
        (iff_color->red << 16) | (iff_color->green << 8) | (iff_color->blue);
    ...
    // build flags
    if ((Show_ship_models || Show_outlines)
        && ((objp->type == OBJ_SHIP) || (objp->type == OBJ_START)))
    
    {
        ...
        //      if (!viewpoint || OBJ_INDEX(objp) != cur_object_index)
      {
          ...
          {
            model_set_outline_color(Fred_outline >> 16,
                                     (Fred_outline >> 8) & 0xff,
                                     Fred_outline & 0xff);
            model_render(Ship_info[Ships[z].ship_info_index].model_num,
                          &objp->orient, &objp->pos, j | MR_SHOW_OUTLINE, -1,
                          -1, Ships[z].ship_replacement_textures);
    ...
    
    {
        ...
        
        {
            ...
            iff_get_color_by_team_and_object(Ships[objp->instance].team, -1,
                                             true, objp);
    ...
    
    {
        ...
        
        {
            ...
            if ((o2->instance == objp->instance - 1)
                 || (o2->instance == objp->instance + 1))
...
 void render_one_model_htl(object * objp) 
{
    ...
    
    {
        ...
        {
          gr_set_color(__min(r * 2, 255), __min(g * 2, 255),
                        __min(b * 2, 255));
          ...
          
          {
              ...
              
              {
                  ...
                  
                  {
                             ...
                             Ship_info[ship_type].short_name);
              ...
              {
                sprintf(pos, "(%.0f,%.0f,%.0f)", objp->pos.xyz.x,
                         objp->pos.xyz.y, objp->pos.xyz.z);
...
} }  void draw_orient_sphere(object * obj, int r, int g, int b) 
{
    ...
    
    {
        ...
        (vm_vec_dotprod(&eye_orient.vec.fvec, &obj->orient.vec.fvec) < 0.0f);
...
 void render_model_x(vec3d * pos, grid * gridp, int col_scheme) 
{
  ...
  vec3d tpos;
       ...
       gv->xyz.z * pos->xyz.z < -gridp->planeD)
...
 void render_active_rect(void) 
{
    ...
    
    {
      ...
      gr_line(marking_box.x1, marking_box.y1, marking_box.x1,
               marking_box.y2);
      gr_line(marking_box.x1, marking_box.y2, marking_box.x2,
               marking_box.y2);
      gr_line(marking_box.x2, marking_box.y2, marking_box.x2,
               marking_box.y1);
      gr_line(marking_box.x2, marking_box.y1, marking_box.x1,
               marking_box.y1);
...
 void process_controls(vec3d * pos, matrix * orient, float frametime,
                         int key, int mode) 
{
    ...
    
    {
      ...
      physics_read_flying_controls(orient, &view_physics, &view_controls,
                                    flFrametime);
...
color Fred_grid_bright, Fred_grid_dark, Fred_grid_bright_aa,
  Fred_grid_dark_aa;
...
 void render_frame() 
{
    ...
    
    {
        ...
        {
          if ((float) True_rh / (float) True_rw >
               (float) BRIEF_GRID_H / (float) BRIEF_GRID_W)
            
            {
              True_rh =
                (int) ((float) BRIEF_GRID_H * (float) True_rw /
                       (float) BRIEF_GRID_W);
          ...
          else                  // Fred is wider than briefing window
            {
                ...
                (int) ((float) BRIEF_GRID_W * (float) True_rh /
                       (float) BRIEF_GRID_H);
  ...
  // 1 means use zbuffering
  ...
   if (query_valid_object(Cursor_over))       // display a tool-tip like infobox
    {
      ...
      if ((Objects[Cursor_over].type == OBJ_SHIP)
           || (Objects[Cursor_over].type == OBJ_START))
        ...
        
        {
                   ...
                   Waypoint_lists[inst / 65536].name, (inst & 0xffff) + 1,
        ...
        sprintf(buf, "( %.1f , %.1f , %.1f ) ", pos.xyz.x, pos.xyz.y,
                 pos.xyz.z);
  ...
   sprintf(buf, "(%.1f,%.1f,%.1f)", eye_pos.xyz.x, eye_pos.xyz.y,
             eye_pos.xyz.z);
...
 void game_do_frame() 
{
    ...
    
    {
      ...
      process_controls(&Objects[view_obj].pos, &Objects[view_obj].orient,
                       f2fl(Frametime), key);
        ...
        
        {
          ...
          control_pos = leader->pos;
            ...
            
            {
                   ...
                   && (cur_object_index != OBJ_INDEX(objp)))
                
                {
                    ...
                    
                    {
                        ...
                        vm_copy_transpose_matrix(&rot_trans,
                                                 &view_physics.last_rotmat);
                        ...
                        vm_matrix_x_matrix(&tmp, &objp->orient,
                                           &view_physics.last_rotmat);
    ...
    if (vm_vec_cmp(&control_pos, &Last_control_pos)
        || vm_matrix_cmp(&control_orient, &Last_control_orient))
    ...
    if (vm_vec_cmp(&eye_pos, &Last_eye_pos)
        || vm_matrix_cmp(&eye_orient, &Last_eye_orient))
...
}  void draw_asteroid_field() 
{
      ...
      {
        p[0].xyz.x = p[2].xyz.x = p[4].xyz.x = p[6].xyz.x =
          Asteroid_field.min_bound.xyz.x;
        p[1].xyz.x = p[3].xyz.x = p[5].xyz.x = p[7].xyz.x =
          Asteroid_field.max_bound.xyz.x;
        p[0].xyz.y = p[1].xyz.y = p[4].xyz.y = p[5].xyz.y =
          Asteroid_field.min_bound.xyz.y;
        p[2].xyz.y = p[3].xyz.y = p[6].xyz.y = p[7].xyz.y =
          Asteroid_field.max_bound.xyz.y;
        p[0].xyz.z = p[1].xyz.z = p[2].xyz.z = p[3].xyz.z =
          Asteroid_field.min_bound.xyz.z;
        p[4].xyz.z = p[5].xyz.z = p[6].xyz.z = p[7].xyz.z =
          ...
          
          {
            ...
             ip[0].xyz.x = ip[2].xyz.x = ip[4].xyz.x = ip[6].xyz.x =
              Asteroid_field.inner_min_bound.xyz.x;
            ip[1].xyz.x = ip[3].xyz.x = ip[5].xyz.x = ip[7].xyz.x =
              Asteroid_field.inner_max_bound.xyz.x;
            ip[0].xyz.y = ip[1].xyz.y = ip[4].xyz.y = ip[5].xyz.y =
              Asteroid_field.inner_min_bound.xyz.y;
            ip[2].xyz.y = ip[3].xyz.y = ip[6].xyz.y = ip[7].xyz.y =
              Asteroid_field.inner_max_bound.xyz.y;
            ip[0].xyz.z = ip[1].xyz.z = ip[2].xyz.z = ip[3].xyz.z =
              Asteroid_field.inner_min_bound.xyz.z;
            ip[4].xyz.z = ip[5].xyz.z = ip[6].xyz.z = ip[7].xyz.z =
              Asteroid_field.inner_max_bound.xyz.z;
...
//  If not, return false.
int object_check_collision(object * objp, vec3d * p0, vec3d * p1,
                           vec3d * hitpos) 
{
    ...
    return fvi_ray_sphere(hitpos, p0, p1, &objp->pos,
                           (objp->radius >
                            0.1f) ? objp->radius : LOLLIPOP_SIZE);
...
int select_object(int cx, int cy) 
{
    ...
    
    {
        ...
        
        {
            ...
            hitpos.xyz.x * hitpos.xyz.x + hitpos.xyz.y * hitpos.xyz.y +
            hitpos.xyz.z * hitpos.xyz.z;
