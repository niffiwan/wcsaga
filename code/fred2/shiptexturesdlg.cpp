...
BEGIN_MESSAGE_MAP(CShipTexturesDlg, CDialog)
  ...
  ON_WM_CLOSE()ON_CBN_SELCHANGE(IDC_OLD_TEXTURE_LIST,
          ...
          if (strlen(new_texture_name[i]))
            {
              ...
              if (temp_bmp < 0)
                {
                  temp_bmp =
                    bm_load_animation(new_texture_name[i], &temp_frames,
                                      &temp_fps, NULL, 1);
                }
      ...
      if (not_found)
        {
          ...
          z =
            MessageBox(message,
                       ((not_found >
                         1) ? "Some textures were not found." :
      ...
      while (k < (MAX_SHIPS * MAX_REPLACEMENT_TEXTURES))
        {
          ...
          if (stricmp
               ...
               Ships[self_ship].ship_name))
            {
              ...
              if (k != write_index)
                {
                  texture_set(&Fred_texture_replacements[write_index],
                              &Fred_texture_replacements[k]);
                }
              write_index++;
            }
...
BOOL CShipTexturesDlg::OnInitDialog()
{
  ...
  // get our model
  polymodel *pm =
    model_get(Ship_info[Ships[self_ship].ship_info_index].model_num);
      ...
      for (j = 0; j < TM_NUM_TYPES; j++)
        {
          ...
          bm_get_filename(pm->maps[i].textures[j].GetOriginalTexture(),
                          texture_file);
...
BOOL CShipTexturesDlg::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName,
...
texture_replace *CShipTexturesDlg::texture_set(texture_replace * dest,
                                               const texture_replace * src)
{
