...
 void waypoint_path_dlg::OnInitMenu(CMenu * pMenu) 
{
    ...
    
    {
        ...
        {
          m->AppendMenu(MF_ENABLED | MF_STRING, ID_WAYPOINT_MENU + i,
    ...
    {
      m->AppendMenu(MF_ENABLED | MF_STRING, ID_JUMP_NODE_MENU + i,
                     jnp->get_name_ptr());
    ...
    m->CheckMenuItem(ID_WAYPOINT_MENU + cur_waypoint_list,
...
 void waypoint_path_dlg::initialize_data(int full_update) 
{
    ...
    Assert(cur_waypoint_list ==
            (Objects[cur_object_index].instance / 65536));
...
 int waypoint_path_dlg::update_data(int redraw) 
{
    ...
    
    {
        ...
        
        {
               ...
               && !stricmp(Waypoint_lists[i].name, m_name)
               && (i != cur_waypoint_list))
      ...
      string_copy(Waypoint_lists[cur_waypoint_list].name, m_name,
                   NAME_LENGTH, 1);
...
 BOOL waypoint_path_dlg::OnCommand(WPARAM wParam, LPARAM lParam) 
{
  ...
  if ((id >= ID_WAYPOINT_MENU)
       && (id < ID_WAYPOINT_MENU + MAX_WAYPOINT_LISTS))
  ...
   if ((id >= ID_JUMP_NODE_MENU)
         && (id < ID_JUMP_NODE_MENU + Jump_nodes.get_num_elements()))
