...
void WeaponEditorDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	if (pDX->m_bSaveAndValidate) {
		...
		if (save_number((char *) (LPCSTR) str, &m_ammo5)) {
			m_ammo_max5 = (m_missile5 <= 0) ? 0 : get_max_ammo_count_for_bank(m_ship_class, 4, m_missile5 + First_secondary_index - 1);
			if (m_ammo5 < 0)
				m_ammo5 = 0;
		...
		if (save_number((char *) (LPCSTR) str, &m_ammo6)) {
			m_ammo_max6 = (m_missile6 <= 0) ? 0 : get_max_ammo_count_for_bank(m_ship_class, 5, m_missile6 + First_secondary_index - 1);
			if (m_ammo6 < 0)
				m_ammo6 = 0;
