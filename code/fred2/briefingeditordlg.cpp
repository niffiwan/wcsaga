...
}  void briefing_editor_dlg::OnClose() 
{
    ...
    MessageBox("You have duplicate icons names.  You should resolve these.",
                "Warning");
...
}  void briefing_editor_dlg::update_data(int update) 
{
  ...
  strcpy_s(The_mission.substitute_briefing_music_name,
            m_substitute_briefing_music);
    ...
    
    {
        ...
        
        {
            ...
            for (i = 0; i < ptr->num_icons - 1; i++)
              ...
              
              {
                  ...
                  
                  {
                      ...
                      if (((ptr->lines[l].start_icon == i)
                            && (ptr->lines[l].end_icon == j))
                           || ((ptr->lines[l].start_icon == j)
                               && (ptr->lines[l].end_icon == i)))
                        break;
                     if ((l == ptr->num_lines)
                           && (l < MAX_BRIEF_STAGE_LINES))
            ...
            if (icon_marked[ptr->lines[i].start_icon]
                 && icon_marked[ptr->lines[i].end_icon])
        ...
        
        {
               ...
               || ptr->lines[i].start_icon >= ptr->num_icons
               || ptr->lines[i].end_icon >= ptr->num_icons)
        ...
        
        {
                ...
                
                {
                    ...
                    
                    {
                        ...
                        
                        {
                                   ...
                                   "change to that ID locally.  Icon ID has been reset back to %d",
          ...
           if ((ptr->icons[m_last_icon].type != m_icon_image)
                 && !m_change_local)
          ...
          if ((ptr->icons[m_last_icon].team != m_icon_team)
               && !m_change_local)
          ...
          if ((ptr->icons[m_last_icon].ship_class != m_ship_type)
               && !m_change_local)
    ...
    
    {
      ...
      m_stage_title.Format("Stage %d of %d", m_cur_stage + 1,
                            Briefing->num_stages);
  ...
   if ((m_cur_stage >= 0) && (m_cur_icon >= 0)
         && (m_cur_icon < ptr->num_icons))
  ...
   if ((valid > 1) && !invalid && (m_cur_stage >= 0)
         && (i <= MAX_BRIEF_STAGE_LINES))
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                save_briefing->stages[m_last_stage].icons[i].pos =
                Objects[icon_obj[i]].pos;
        ...
        
        {
            ...
            
            {
                ...
                icon_obj[i] =
                obj_create(OBJ_POINT, -1, i, NULL, &ptr->icons[i].pos, 0.0f,
                           OF_RENDERS);
...
 void briefing_editor_dlg::draw_icon(object * objp) 
{
  ...
   brief_render_icon(m_cur_stage, objp->instance, 1.0f / 30.0f,
                       objp->flags & OF_MARKED,
                       (float) True_rw / BRIEF_GRID_W,
                       (float) True_rh / BRIEF_GRID_H);
...
 void briefing_editor_dlg::copy_stage(int from, int to) 
{
  ...
   memmove(Briefing->stages[to].icons, Briefing->stages[from].icons,
             sizeof(brief_icon) * MAX_STAGE_ICONS);
  memmove(Briefing->stages[to].lines, Briefing->stages[from].lines,
...
 void briefing_editor_dlg::update_positions() 
{
    ...
    
    {
        ...
        {
          Briefing->stages[m_cur_stage].icons[i].pos =
            Objects[icon_obj[i]].pos;
...
 void briefing_editor_dlg::OnMakeIcon() 
{
    ...
    
    {
        ...
        
        {
            ...
            
            {
              ...
               if (Ship_info[Ships[ship].ship_info_index].
                     flags & SIF_FREIGHTER)
                
                {
                  
                    // direct docked with any marked cargo?
                    for (dock_instance * dock_ptr = ptr->dock_list;
                    ...
                    
                    {
                        ...
                        
                        {
                               ...
                               [Ships[dock_ptr->docked_objp->instance].
                                ship_info_index].flags & SIF_CARGO)
    ...
    
    {
      ...
      switch (Ship_info[Ships[ship].ship_info_index].
               flags & SIF_ALL_SHIP_TYPES)
        
        {
              ...
              count ? ICON_FREIGHTER_WING_NO_CARGO : ICON_FREIGHTER_NO_CARGO;
    ...
    obj_create(OBJ_POINT, -1, m_cur_icon, NULL, &pos, 0.0f, OF_RENDERS);
...
}  void briefing_editor_dlg::delete_icon(int num) 
{
    ...
    {
      Briefing->stages[m_cur_stage].icons[i - 1] =
        Briefing->stages[m_cur_stage].icons[i];
...
}  int briefing_editor_dlg::check_mouse_hit(int x, int y) 
{
    ...
    
    {
      ...
      if ((x > ptr->x) && (x < ptr->x + ptr->w) && (y > ptr->y)
           && (y < ptr->y + ptr->h))
...
 void briefing_editor_dlg::propagate_icon(int num) 
{
    ...
    
    {
      ...
      Briefing->stages[s].icons[i] =
        Briefing->stages[m_cur_stage].icons[num];
