...
ON_CBN_SELCHANGE(ID_NEW_SHIP_TYPE, OnNewShipTypeChange)
  ...
  ON_WM_CREATE()ON_COMMAND(ID_EDITORS_AI_CLASSES, OnEditorsAiClasses)
...
ON_WM_LBUTTONUP()ON_WM_DESTROY()ON_COMMAND(ID_VIEW_STATUS_BAR,
                                           OnViewStatusBar)
...
ON_UPDATE_COMMAND_UI(ID_INDICATOR_LEFT,
                     OnUpdateLeft) ON_UPDATE_COMMAND_UI(ID_INDICATOR_RIGHT,
                                                        OnUpdateRight)
ON_COMMAND(ID_MIKE_GRIDCONTROL,
           OnMikeGridcontrol) ON_COMMAND(IDR_MENU_POPUP_TOGGLE1,
                                         OnMenuPopupToggle1)
ON_UPDATE_COMMAND_UI(IDR_MENU_POPUP_TOGGLE1,
                     OnUpdateMenuPopupToggle1)
ON_WM_RBUTTONDOWN()ON_COMMAND(ID_HELP_INPUT_INTERFACE,
                              OnHelpInputInterface)
ON_WM_CLOSE()ON_WM_INITMENU()ON_COMMAND(ID_HELP_FINDER,
                                        OnFredHelp) ON_COMMAND(ID_HELP,
                                                               OnFredHelp)
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
  ...
  if (!m_new_ship_type_combo_box.
      Create(CBS_DROPDOWNLIST | WS_VISIBLE | WS_VSCROLL | CBS_HASSTRINGS |
             LBS_OWNERDRAWFIXED, rect, &m_wndToolBar, ID_NEW_SHIP_TYPE))
    {
  ...
  if ((z >= 0)
      && (lpDrawItemStruct->itemAction & (ODA_DRAWENTIRE | ODA_SELECT)))
    {
      ...
      if (!fDisabled)
        {
          if (z >= Num_ship_classes)
          ...
          else
            {
              ...
              newTextColor =
                RGB(sinfo->fred_color.rgb.r, sinfo->fred_color.rgb.g,
                    sinfo->fred_color.rgb.b);
            }
        }
      ...
      pDC->ExtTextOut(lpDrawItemStruct->rcItem.left,
                      lpDrawItemStruct->rcItem.top + max(0,
                                                         (cyItem -
                                                          m_cyText) / 2),
                      ETO_OPAQUE, &(lpDrawItemStruct->rcItem), strText,
      ...
      if (i < Num_iffs)
        {
          ...
          pMenu->ModifyMenu(ID_SHOW_IFF[i], MF_BYCOMMAND | MF_STRING,
                            ID_SHOW_IFF[i], text);
        }
      else
        {
  ...
  if (r < 32)
    {
      ...
      switch (r)
        ...
        case 0:
          txt =
            XSTR("The operating system is out of memory or resources.", 1107);
          break;
        ...
        case SE_ERR_ACCESSDENIED:
          txt =
            XSTR("The operating system denied access to the specified file. ",
                 1109);
          break;
        ...
        case SE_ERR_DLLNOTFOUND:
          txt =
            XSTR("The specified dynamic-link library was not found.", 1114);
          break;
        case SE_ERR_OOM:
          txt =
            XSTR("There was not enough memory to complete the operation.",
                 1115);
          break;
        case SE_ERR_SHARE:
