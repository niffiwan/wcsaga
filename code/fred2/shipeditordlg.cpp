...
BEGIN_MESSAGE_MAP(CShipEditorDlg, CDialog)
  ...
  ON_WM_CLOSE()ON_NOTIFY(NM_RCLICK, IDC_ARRIVAL_TREE, OnRclickArrivalTree)
...
ON_NOTIFY(TVN_BEGINLABELEDIT, IDC_DEPARTURE_TREE,
          OnBeginlabeleditDepartureTree) ON_NOTIFY(TVN_ENDLABELEDIT,
                                                   IDC_ARRIVAL_TREE,
                                                   OnEndlabeleditArrivalTree)
ON_NOTIFY(TVN_ENDLABELEDIT, IDC_DEPARTURE_TREE,
          OnEndlabeleditDepartureTree) ON_BN_CLICKED(IDC_GOALS,
                                                     OnGoals)
ON_CBN_SELCHANGE(IDC_SHIP_CLASS,
                 OnSelchangeShipClass) ON_BN_CLICKED(IDC_INITIAL_STATUS,
                                                     OnInitialStatus)
ON_BN_CLICKED(IDC_WEAPONS, OnWeapons) ON_BN_CLICKED(IDC_SHIP_RESET,
                                                    OnShipReset)
ON_BN_CLICKED(IDC_DELETE_SHIP, OnDeleteShip) ON_BN_CLICKED(IDC_SHIP_TBL,
                                                           OnShipTbl)
ON_BN_CLICKED(IDC_NEXT, OnNext) ON_NOTIFY(TVN_SELCHANGED, IDC_ARRIVAL_TREE,
                                          OnSelchangedArrivalTree)
ON_NOTIFY(TVN_SELCHANGED, IDC_DEPARTURE_TREE,
          OnSelchangedDepartureTree) ON_BN_CLICKED(IDC_HIDE_CUES,
                                                   OnHideCues)
ON_BN_CLICKED(IDC_PREV, OnPrev) ON_CBN_SELCHANGE(IDC_ARRIVAL_LOCATION,
                                                 OnSelchangeArrivalLocation)
ON_BN_CLICKED(IDC_PLAYER_SHIP,
              OnPlayerShip) ON_BN_CLICKED(IDC_NO_ARRIVAL_WARP,
                                          OnNoArrivalWarp)
ON_BN_CLICKED(IDC_NO_DEPARTURE_WARP,
              OnNoDepartureWarp) ON_CBN_SELCHANGE(IDC_DEPARTURE_LOCATION,
                                                  OnSelchangeDepartureLocation)
ON_CBN_SELCHANGE(IDC_HOTKEY, OnSelchangeHotkey) ON_BN_CLICKED(IDC_FLAGS,
                                                              OnFlags)
ON_BN_CLICKED(IDC_IGNORE_ORDERS,
              OnIgnoreOrders) ON_BN_CLICKED(IDC_SPECIAL_EXP,
                                            OnSpecialExp)
ON_BN_CLICKED(IDC_TEXTURES, OnTextures) ON_BN_CLICKED(IDC_SPECIAL_HITPOINTS,
                                                      OnSpecialHitpoints)
ON_BN_CLICKED(IDC_ALT_SHIP_CLASS,
              OnAltShipClass) ON_BN_CLICKED(IDC_RESTRICT_ARRIVAL,
                                            OnRestrictArrival)
ON_BN_CLICKED(IDC_RESTRICT_DEPARTURE,
              OnRestrictDeparture)
ON_WM_INITMENU()ON_BN_CLICKED(IDC_SET_AS_PLAYER_SHIP,
                              OnSetAsPlayerShip)
ON_BN_CLICKED(IDC_NO_ARRIVAL_LOG,
              OnBnClickedNoArrivalLog) ON_BN_CLICKED(IDC_NO_DEPARTURE_LOG,
                                                     OnBnClickedNoDepartureLog)
  //}}AFX_MSG_MAP
END_MESSAGE_MAP()
/////////////////////////////////////////////////////////////////////////////
...
void CShipEditorDlg::initialize_data(int full_update)
{
  ...
  int a_cue, d_cue, cue_init = 0, cargo = 0, base_ship, base_player, pship =
    -1;
  int no_arrival_warp = 0, no_departure_warp =
    0, escort_count, ship_orders, current_orders;
  ...
  management_add_ships_to_combo(box,
                                SHIPS_2_COMBO_SPECIAL |
                                SHIPS_2_COMBO_ALL_SHIPS);
  ...
  while (objp != END_OF_LIST(&obj_used_list))
    {
      if ((objp->type == OBJ_SHIP)
          && (Ships[objp->instance].flags & SF_ESCORT))
        {
  ...
  m_arrival_tree.select_sexp_node = m_departure_tree.select_sexp_node =
    select_sexp_node;
  select_sexp_node = -1;
  ...
  if (ship_count)
              ...
              if (objp->flags & OF_MARKED)
                {
                  ...
                  if (base_player >= 0)
                  ...
                  else
                    {
                      ...
                      pship =
                        tristate_set(Objects[Ships[i].objnum].type ==
                                     OBJ_START, pship);
                    }
                  ...
                  if ((objp->type == OBJ_SHIP)
                      || ((objp->type == OBJ_START) && !mission_type))
                    {
                          ...
                          if (!cue_init)
                            {
                              ...
                              m_departure_location =
                                Ships[i].departure_location;
                              m_departure_delay.init(Ships[i].
                                                     departure_delay);
                            ...
                            {
                              cue_init++;
                              if (Ships[i].arrival_location !=
                                  m_arrival_location)
                                {
                                  m_arrival_location = -1;
                                }

                              if (Ships[i].departure_location !=
                                  m_departure_location)
                                {
                                  m_departure_location = -1;
                                }
                      ...
                      if (base_ship >= 0)
                        {
                          ...
                          m_assist_score.
                            init((int) (Ships[i].assist_score_pct * 100));
                          ...
                          if (wing < 0)
                            ...
                            {
                              GetDlgItem(IDC_WING)->SetWindowText(Wings[wing].
                                                                  name);
                          ...
                          no_arrival_warp =
                            (Ships[i].flags & SF_NO_ARRIVAL_WARP) ? 1 : 0;
                          no_departure_warp =
                            (Ships[i].flags & SF_NO_DEPARTURE_WARP) ? 1 : 0;
                          no_arrival_log =
                            (Ships[i].flags2 & SF2_NO_ARRIVAL_LOG) ? 1 : 0;
                          no_departure_log =
                            (Ships[i].flags2 & SF2_NO_DEPARTURE_LOG) ? 1 : 0;
                      ...
                      else
                        {
                          ...
                          m_assist_score.
                            set((int) (Ships[i].assist_score_pct * 100));
                          ...
                          no_arrival_warp =
                            tristate_set(Ships[i].flags & SF_NO_ARRIVAL_WARP,
                                         no_arrival_warp);
                          no_departure_warp =
                            tristate_set(Ships[i].
                                         flags & SF_NO_DEPARTURE_WARP,
                                         no_departure_warp);

                          no_arrival_log =
                            tristate_set(Ships[i].flags2 & SF2_NO_ARRIVAL_LOG,
                                         no_arrival_log);
                          no_departure_log =
                            tristate_set(Ships[i].
                                         flags2 & SF2_NO_DEPARTURE_LOG,
                                         no_departure_log);
                        }
                    }
                }
  ...
  else                          // no ships selected, 0 or more player ships selected
    {
      ...
      else if (query_valid_object()
               && (Objects[cur_object_index].type == OBJ_START))
        {
  ...
  if (m_arrival_target >= 0)
    {
      if (m_arrival_target & SPECIAL_ARRIVAL_ANCHOR_FLAG)
      ...
      else
        {
          // find it in the box
          m_arrival_target =
            box->FindStringExact(-1, Ships[m_arrival_target].ship_name);
        }
    }
  ...
  if (m_departure_target >= 0)
    {
      m_departure_target =
        box->FindStringExact(-1, Ships[m_departure_target].ship_name);
    }
  ...
  if (mission_type || !pship_count
      || (pship_count + total_count > MAX_PLAYERS)
      || (pvalid_count < total_count))
  ...
  // enable the "set player" button only if single player, single edit, and ship is in player wing
  {
    ...
    if (mission_type && total_count && !multi_edit
        && wing_is_player_wing(Ships[marked_ship].wingnum))
  ...
  else if (single_ship >= 0)    // editing a single ship
    {
          ...
          if (Wings[i].wave_count && !stricmp(Wings[i].name, m_ship_name))
            {
              ...
              bypass_errors = 1;
              z =
                MessageBox("This ship name is already being used by a wing\n"
                           "Press OK to restore old name", "Error",
                           MB_ICONEXCLAMATION | MB_OKCANCEL);
          ...
          if (!stricmp(m_ship_name, Iff_info[i].iff_name))
            {
              ...
              bypass_errors = 1;
              z =
                MessageBox("This ship name is already being used by a team.\n"
                           "Press OK to restore old name", "Error",
                           MB_ICONEXCLAMATION | MB_OKCANCEL);
      ...
      for (i = 0; i < MAX_WAYPOINT_LISTS; i++)
        if (Waypoint_lists[i].count
            && !stricmp(Waypoint_lists[i].name, m_ship_name))
          {
            if (bypass_errors)
              return 0;
      ...
      if (jumpnode_get_by_name(m_ship_name) != NULL)
        {
          ...
          z =
            MessageBox("This ship name is already being used by a jump node\n"
                       "Press OK to restore old name", "Error",
                       MB_ICONEXCLAMATION | MB_OKCANCEL);
  ...
  if (Ships[ship].wingnum < 0)
    {
      ...
      if (m_arrival_target >= 0)
        {
          z =
            ((CComboBox *) GetDlgItem(IDC_ARRIVAL_TARGET))->
            GetItemData(m_arrival_target);
          ...
          if (((m_arrival_location != ARRIVE_FROM_DOCK_BAY)
               && (m_arrival_location != ARRIVE_AT_LOCATION)) && (z >= 0)
              && !(z & SPECIAL_ARRIVAL_ANCHOR_FLAG))
            {
              d = int (min(500, 2.0f * Objects[Ships[ship].objnum].radius));
              if ((Ships[ship].arrival_distance < d)
      ...
      if (m_departure_target >= 0)
        {
          z =
            ((CComboBox *) GetDlgItem(IDC_DEPARTURE_TARGET))->
            GetItemData(m_departure_target);
...
void CShipEditorDlg::OnGoals()
{
  ...
  dlg_goals.DoModal();
  if (!multi_edit
      && !query_initial_orders_empty(Ai_info[Ships[single_ship].ai_index].
                                     goals))
    if ((Ships[single_ship].wingnum >= 0)
        && (query_initial_orders_conflict(Ships[single_ship].wingnum)))
      MessageBox("This ship's wing also has initial orders",
                 "Possible conflict");
}
      ...
      if (((objp->type == OBJ_SHIP)
          ...
          && (objp->flags & OF_MARKED))
        {
          ...
          for (i = 0; i < sip->num_primary_banks; i++)
            Ships[ship].weapons.primary_bank_weapons[i] =
              sip->primary_bank_weapons[i];

          for (i = 0; i < sip->num_secondary_banks; i++)
            {
              Ships[ship].weapons.secondary_bank_weapons[i] =
                sip->secondary_bank_weapons[i];
              Ships[ship].weapons.secondary_bank_capacity[i] =
              ...
              if (ptr->system_info->type == SUBSYSTEM_TURRET)
                {
                  wp = &ptr->weapons;
                  sp =
                    &Ship_info[Ships[ship].ship_info_index].subsystems[index];
                  ...
                  for (i = 0; i < MAX_SHIP_SECONDARY_BANKS; i++)
                    {
                      if (sp->secondary_banks[i] != -1)
                        {
                          wp->secondary_bank_weapons[j] =
                            sp->secondary_banks[i];
                          wp->secondary_bank_capacity[j++] =
                            sp->secondary_bank_capacity[i];
                        }
                    }
...
void CShipEditorDlg::OnTextures()
{
  ...
  if (multi_edit)
    {
      MessageBox("Sorry, you can only edit textures for one ship at a time.",
