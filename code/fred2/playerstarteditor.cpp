...
BEGIN_MESSAGE_MAP(player_start_editor, CDialog)
  ...
  ON_WM_INITMENU()ON_LBN_SELCHANGE(IDC_SHIP_LIST, OnSelchangeShipList)
...
ON_WM_CLOSE()ON_CBN_SELCHANGE(IDC_WEAPON_VARIABLES_COMBO,
  ...
  if (selection != -1)
    {
      ...
      if (sexp_index > -1)
        {
          Assert(selection ==
                 sexp_variable_typed_count(sexp_index, SEXP_VARIABLE_STRING));
...
void player_start_editor::OnSelchangeWeaponVariablesCombo()
{
  ...
  m_weapon_quantity_variable.GetLBText(m_weapon_quantity_variable.GetCurSel(),
                                       variable_name);
  int sexp_index = get_index_sexp_variable_name(variable_name);
          ...
          if (dynamic_weapon_pool[i][idx] != -1)
            {
              ...
              if (dynamic_weapon_variable_pool[i][idx] == -1)
                ...
                {
                  Assert(Sexp_variables[dynamic_weapon_variable_pool[i][idx]].
                         type & SEXP_VARIABLE_NUMBER);
  ...
  if (variable_index > -1)
    {
      ...
      sexp_fred_modify_variable(variable_value, variable_name, variable_index,
                                SEXP_VARIABLE_NUMBER);
    }
}
