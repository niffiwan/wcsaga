...
player_start_editor::player_start_editor(CWnd * pParent):CDialog(player_start_editor::IDD,
        pParent) 
{
...
ON_WM_CLOSE() 
ON_CBN_SELCHANGE(IDC_WEAPON_VARIABLES_COMBO,
                 OnSelchangeWeaponVariablesCombo) 
  //}}AFX_MSG_MAP
END_MESSAGE_MAP()  
/////////////////////////////////////////////////////////////////////////////
  ...
   BOOL player_start_editor::OnInitDialog() 
{
    ...
    memset(static_ship_pool, -1,
           sizeof(int) * MAX_TVT_TEAMS * MAX_SHIP_CLASSES);
  memset(dynamic_ship_pool, -1,
          sizeof(int) * MAX_TVT_TEAMS * MAX_SEXP_VARIABLES);
  memset(static_ship_variable_pool, -1,
          sizeof(int) * MAX_TVT_TEAMS * MAX_SHIP_CLASSES);
  memset(dynamic_ship_variable_pool, -1,
          sizeof(int) * MAX_TVT_TEAMS * MAX_SEXP_VARIABLES);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                static_ship_pool[i][Team_data[i].ship_list[idx]] =
                Team_data[i].ship_count[idx];
                ...
                static_ship_variable_pool[i][Team_data[i].ship_list[idx]] =
            ...
            
            {
                ...
                dynamic_ship_pool[i][Team_data[i].ship_list_variables[idx]] =
                Team_data[i].ship_count[idx];
              
                // This pool is set to hold whether a ship at a Ship_info index has been set by a variable (and the
                // variables index in Sexp_variables) if it has).
                dynamic_ship_variable_pool[i][Team_data[i].
                                              ship_list_variables[idx]] =
                Team_data[i].ship_count_variables[idx];
    ...
    memset(static_weapon_pool, -1,
           sizeof(int) * MAX_TVT_TEAMS * MAX_WEAPON_TYPES);
  memset(dynamic_weapon_pool, -1,
          sizeof(int) * MAX_TVT_TEAMS * MAX_SEXP_VARIABLES);
  memset(static_weapon_variable_pool, -1,
          sizeof(int) * MAX_TVT_TEAMS * MAX_WEAPON_TYPES);
  memset(dynamic_weapon_variable_pool, -1,
          sizeof(int) * MAX_TVT_TEAMS * MAX_SEXP_VARIABLES);
    ...
    
    {
        ...
        
        {
            ...
            {
              static_weapon_pool[i][Team_data[i].weaponry_pool[idx]] =
                Team_data[i].weaponry_count[idx];
              static_weapon_variable_pool[i][Team_data[i].
                                              weaponry_pool[idx]] =
            ...
            {
              dynamic_weapon_pool[i][Team_data[i].
                                      weaponry_pool_variable[idx]] =
                Team_data[i].weaponry_count[idx];
              dynamic_weapon_variable_pool[i][Team_data[i].
                                               weaponry_pool_variable[idx]] =
                Team_data[i].weaponry_amount_variable[idx];
    ...
    
    {
        ...
        
        {
            ...
            {
              generate_ship_usage_list(ship_usage[i],
                                        TVT_wings[(i *
                                                   MAX_TVT_WINGS_PER_TEAM) +
                                                  j]);
              generate_weaponry_usage_list(weapon_usage[i],
                                            TVT_wings[(i *
                                                       MAX_TVT_WINGS_PER_TEAM)
                                                      + j]);
...
void player_start_editor::reset_controls() 
{
  ...
  if (autobalance && (previous_team != -1)
    ...
    
    {
        ...
        {
          static_ship_pool[selected_team][i] =
            static_ship_pool[previous_team][i];
          static_ship_variable_pool[selected_team][i] =
            static_ship_variable_pool[previous_team][i];
        ...
        {
          static_weapon_pool[selected_team][i] =
            static_weapon_pool[previous_team][i];
          static_weapon_variable_pool[selected_team][i] =
            static_weapon_variable_pool[previous_team][i];
        ...
        {
          dynamic_ship_pool[selected_team][i] =
            dynamic_ship_pool[previous_team][i];
          dynamic_ship_variable_pool[selected_team][i] =
            dynamic_ship_variable_pool[previous_team][i];
          dynamic_weapon_pool[selected_team][i] =
            dynamic_weapon_pool[previous_team][i];
          dynamic_weapon_variable_pool[selected_team][i] =
            dynamic_weapon_variable_pool[previous_team][i];
    ...
    
    {
        ...
        {
          sprintf(buff, "%s [%s]", Sexp_variables[i].variable_name,
        ...
        {
          m_ship_quantity_variable.AddString(Sexp_variables[i].
                                              variable_name);
          m_weapon_quantity_variable.AddString(Sexp_variables[i].
                                                variable_name);
    ...
    
    {
        ...
        
        {
            ...
            if ((static_ship_pool[selected_team][i] > 0)
                || (static_ship_variable_pool[selected_team][i] != -1))
    ...
    
    {
      ...
      else if (static_weapon_pool[selected_team][i] > 0
               || (static_weapon_variable_pool[selected_team][i] != -1))
...
 void player_start_editor::OnInitMenu(CMenu * pMenu) 
{
    ...
    {
      m->CheckMenuItem(ID_AUTOBALANCE,
                        autobalance ? MF_CHECKED : MF_UNCHECKED);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                {
                  Assert(Sexp_variables
                          [static_ship_variable_pool[selected_team]
                           [si_index]].type & SEXP_VARIABLE_NUMBER);
                   m_ship_pool =
                    atoi(Sexp_variables
                         [static_ship_variable_pool[selected_team][si_index]].
                         text);
                  int selected_variable =
                    sexp_variable_typed_count(static_ship_variable_pool
                                              [selected_team][si_index],
            ...
            sprintf(ship_usage_buff, "%d",
                    ship_usage[selected_team][si_index]);
...
void player_start_editor::OnSelchangeShipVariablesList() 
{
    ...
    
    {
        ...
        get_nth_variable_index(selection + 1, SEXP_VARIABLE_STRING);
       if (sexp_index > -1)
        
        {
          Assert(selection ==
                  sexp_variable_typed_count(sexp_index,
            ...
            {
              if (dynamic_ship_variable_pool[selected_team][sexp_index] ==
                   -1)
                ...
                
                {
                          ...
                          [dynamic_ship_variable_pool[selected_team]
                           [sexp_index]].type & SEXP_VARIABLE_NUMBER);
                  m_ship_pool =
                    atoi(Sexp_variables
                         [dynamic_ship_variable_pool[selected_team]
                          [sexp_index]].text);
                  int selected_variable =
                    sexp_variable_typed_count(dynamic_ship_variable_pool
                                              [selected_team][sexp_index],
                                              SEXP_VARIABLE_NUMBER);
...
 void player_start_editor::OnSelchangeShipVariablesCombo() 
{
  ...
   m_ship_quantity_variable.GetLBText(m_ship_quantity_variable.GetCurSel(),
                                        variable_name);
  int sexp_index = get_index_sexp_variable_name(variable_name);
   Assert((sexp_index > -1)
            || (!strcmp("Don't Use Variables", variable_name)));
  ...
  int ship_variable_index = GetSelectedShipVariableListIndex();
    ...
    {
      dynamic_ship_variable_pool[selected_team][ship_variable_index] =
        sexp_index;
    ...
    
    {
        ...
        {
          dynamic_ship_pool[selected_team][ship_variable_index] =
            new_quantity;
        ...
        
        {
            ...
            
            {
                ...
                
                {
                    ...
                    static_weapon_pool[selected_team][wi_index];
                ...
                
                {
                          ...
                          [static_weapon_variable_pool[selected_team]
                           [wi_index]].type & SEXP_VARIABLE_NUMBER);
                   m_weapon_pool =
                    atoi(Sexp_variables
                         [static_weapon_variable_pool[selected_team]
                          [wi_index]].text);
                  int selected_variable =
                    sexp_variable_typed_count(static_weapon_variable_pool
                                              [selected_team][wi_index],
                                              SEXP_VARIABLE_NUMBER);
                  m_weapon_quantity_variable.SetCurSel(selected_variable +
                                                        1);
            ...
            // set the number used in wings
            sprintf(weapon_usage_buff, "%d",
                    weapon_usage[selected_team][wi_index]);
...
 void player_start_editor::OnSelchangeWeaponVariablesList() 
{
    ...
    
    {
        ...
        
        {
            ...
            {
              if (dynamic_weapon_variable_pool[selected_team][sexp_index] ==
                   -1)
                
                {
                    ...
                    dynamic_weapon_pool[selected_team][sexp_index];
                ...
                
                {
                          ...
                          [dynamic_weapon_variable_pool[selected_team]
                           [sexp_index]].type & SEXP_VARIABLE_NUMBER);
                    ...
                    sexp_variable_typed_count(dynamic_weapon_variable_pool
                                              [selected_team][sexp_index],
                                              SEXP_VARIABLE_NUMBER);
...
 void player_start_editor::OnSelchangeWeaponVariablesCombo() 
{
  ...
   m_weapon_quantity_variable.GetLBText(m_weapon_quantity_variable.
                                          GetCurSel(), variable_name);
  ...
  int weapon_variable_index = GetSelectedWeaponVariableListIndex();
    ...
    {
      dynamic_weapon_variable_pool[selected_team][weapon_variable_index] =
        sexp_index;
    ...
    
    {
        ...
        {
          dynamic_weapon_pool[selected_team][weapon_variable_index] =
            new_quantity;
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                {
                  Team_data[i].ship_count[num_choices] =
                    dynamic_ship_pool[i][idx];
                ...
                {
                  Assert(Sexp_variables[dynamic_ship_variable_pool[i][idx]].
                          type & SEXP_VARIABLE_NUMBER);
                   Team_data[i].ship_count_variables[num_choices] =
                    dynamic_ship_variable_pool[i][idx];
                  Team_data[i].ship_count[num_choices] =
                    atoi(Sexp_variables[dynamic_ship_variable_pool[i][idx]].
                         text);
        ...
        
        {
            ...
            if (static_ship_pool[i][idx] > 0
                || static_ship_variable_pool[i][idx] > -1)
            
            {
                ...
                {
                  Assert(Sexp_variables[static_ship_variable_pool[i][idx]].
                          type & SEXP_VARIABLE_NUMBER);
                   Team_data[i].ship_count_variables[num_choices] =
                    static_ship_variable_pool[i][idx];
                  Team_data[i].ship_count[num_choices] =
                    atoi(Sexp_variables[static_ship_variable_pool[i][idx]].
                         text);
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                weapon_info_lookup(Sexp_variables[idx].text);
                ...
                {
                  Team_data[i].weaponry_count[num_choices] =
                    dynamic_weapon_pool[i][idx];
                ...
                
                {
                          ...
                          [dynamic_weapon_variable_pool[i][idx]].
                          type & SEXP_VARIABLE_NUMBER);
                   Team_data[i].weaponry_amount_variable[num_choices] =
                    dynamic_weapon_variable_pool[i][idx];
                  Team_data[i].weaponry_count[num_choices] =
                    atoi(Sexp_variables[dynamic_weapon_variable_pool[i][idx]].
                         text);
        ...
        
        {
            ...
            if (static_weapon_pool[i][idx] > 0
                || static_weapon_variable_pool[i][idx] > -1)
            
            {
                ...
                {
                  Assert(Sexp_variables[static_weapon_variable_pool[i][idx]].
                          type & SEXP_VARIABLE_NUMBER);
                   Team_data[i].weaponry_amount_variable[num_choices] =
                    static_weapon_variable_pool[i][idx];
                  Team_data[i].weaponry_count[num_choices] =
                    atoi(Sexp_variables[static_weapon_variable_pool[i][idx]].
                         text);
...
void player_start_editor::UpdateQuantityVariable(CComboBox * variable_list,
                                                 int pool_value) 
{
    ...
    
    {
      ...
      sexp_fred_modify_variable(variable_value, variable_name,
                                 variable_index, SEXP_VARIABLE_NUMBER);
