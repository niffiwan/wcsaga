...
BEGIN_MESSAGE_MAP(wing_editor, CDialog)
  ...
  ON_WM_INITMENU()ON_NOTIFY(UDN_DELTAPOS, IDC_SPIN_WAVES, OnDeltaposSpinWaves)
...
ON_NOTIFY(TVN_ENDLABELEDIT, IDC_DEPARTURE_TREE,
          OnEndlabeleditDepartureTree) ON_BN_CLICKED(IDC_DELETE_WING,
                                                     OnDeleteWing)
ON_BN_CLICKED(IDC_DISBAND_WING,
              OnDisbandWing) ON_WM_CLOSE()ON_BN_CLICKED(IDC_GOALS2,
                                                        OnGoals2)
ON_BN_CLICKED(IDC_REINFORCEMENT, OnReinforcement) ON_BN_CLICKED(IDC_NEXT,
                                                                OnNext)
ON_NOTIFY(TVN_SELCHANGED, IDC_ARRIVAL_TREE,
          OnSelchangedArrivalTree) ON_NOTIFY(TVN_SELCHANGED,
                                             IDC_DEPARTURE_TREE,
                                             OnSelchangedDepartureTree)
ON_BN_CLICKED(IDC_HIDE_CUES, OnHideCues) ON_BN_CLICKED(IDC_PREV,
                                                       OnPrev)
ON_CBN_SELCHANGE(IDC_ARRIVAL_LOCATION,
                 OnSelchangeArrivalLocation)
ON_CBN_SELCHANGE(IDC_DEPARTURE_LOCATION,
                 OnSelchangeDepartureLocation) ON_CBN_SELCHANGE(IDC_HOTKEY,
                                                                OnSelchangeHotkey)
ON_BN_CLICKED(IDC_WING_SQUAD_LOGO_BUTTON,
              OnSquadLogo) ON_BN_CLICKED(IDC_RESTRICT_ARRIVAL,
                                         OnRestrictArrival)
  ...
  if (cur_wing < 0)
  ...
  else
    {
      ...
      if (m_arrival_location == ARRIVE_FROM_DOCK_BAY)
        {
          management_add_ships_to_combo(arrival_box,
                                        SHIPS_2_COMBO_DOCKING_BAY_ONLY);
        }
      else
        {
          management_add_ships_to_combo(arrival_box,
      ...
      if (m_arrival_target >= 0)
        {
          if (m_arrival_target & SPECIAL_ARRIVAL_ANCHOR_FLAG)
          ...
          else
            {
              // find it in the box
              m_arrival_target =
                arrival_box->FindStringExact(-1,
                                             Ships[m_arrival_target].
                                             ship_name);
            }
        }
      ...
      if (m_departure_location == DEPART_AT_DOCK_BAY)
        {
          management_add_ships_to_combo(departure_box,
                                        SHIPS_2_COMBO_DOCKING_BAY_ONLY);
        }
      else
        {
      ...
      if (m_departure_target >= 0)
        m_departure_target =
          departure_box->FindStringExact(-1,
                                         Ships[m_departure_target].ship_name);
      ...
      for (i = 0; i < Wings[cur_wing].wave_count; i++)
        {
          ...
          sflag =
            Ship_info[Ships[Wings[cur_wing].ship_index[i]].ship_info_index].
            flags;
  ...
  if (cur_wing >= 0)
    {
      for (i = 0; i < MAX_WINGS; i++)
        if (Wings[i].wave_count && !stricmp(Wings[i].name, m_wing_name)
            && (i != cur_wing))
          {
            if (bypass_errors)
              return 1;
          ...
          if (!stricmp(m_wing_name, Iff_info[i].iff_name))
            {
              ...
              bypass_errors = 1;
              z =
                MessageBox("This wing name is already being used by a team.\n"
                           "Press OK to restore old name", "Error",
                           MB_ICONEXCLAMATION | MB_OKCANCEL);
      ...
      for (i = 0; i < MAX_WAYPOINT_LISTS; i++)
        {
          if (Waypoint_lists[i].count
              && !stricmp(Waypoint_lists[i].name, m_wing_name))
            {
              if (bypass_errors)
                return 1;
      ...
      if (jumpnode_get_by_name(m_wing_name) != NULL)
        {
          ...
          bypass_errors = 1;
          z =
            MessageBox("This wing name is already being used by a jump node\n"
                       "Press OK to restore old name", "Error",
                       MB_ICONEXCLAMATION | MB_OKCANCEL);
...
void wing_editor::update_data_safe()
{
  ...
  if (m_arrival_delay_min > m_arrival_delay_max)
    {
      if (!bypass_errors)
        {
          sprintf(buf,
                  "Arrival delay minimum greater than maximum.  Value lowered to %d",
                  m_arrival_delay_max);
          MessageBox(buf);
        }
  ...
  if (m_arrival_target >= 0)
    {
      ...
      if (((m_arrival_location != ARRIVE_AT_LOCATION)
           && (m_arrival_location != ARRIVE_FROM_DOCK_BAY)) && (i >= 0)
          && !(i & SPECIAL_ARRIVAL_ANCHOR_FLAG))
        {
          d = int (min(500, 2.0f * Objects[Ships[i].objnum].radius));
          if ((Wings[cur_wing].arrival_distance < d)
  ...
  if (stricmp(m_wing_squad_filename, Wings[cur_wing].wing_squad_filename))
    {
      string_copy(Wings[cur_wing].wing_squad_filename, m_wing_squad_filename,
                  MAX_FILENAME_LEN);
      set_modified();
    }
...
void wing_editor::OnSelchangeHotkey()
{
  ...
  if ((set_num >= MAX_STARTING_WINGS) || (set_num != i))
    {
      sprintf(buf, "Assigning nonstandard hotkey to wing %s (default is F%d)",
              Wings[cur_wing].name, 5 + i);
