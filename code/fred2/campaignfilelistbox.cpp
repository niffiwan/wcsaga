     ...
     void campaign_filelist_box::initialize()
{
  ...
  num_files =
    cf_get_file_list_preallocated(2000, mission_filenames_arr,
                                  mission_filenames, CF_TYPE_MISSIONS,
                                  "*.fs2");
  ...
  while (i < num_files)
    {
      ...
      if ((Campaign.type == CAMPAIGN_TYPE_SINGLE)
          && (a_mission.
              game_type & (MISSION_TYPE_SINGLE | MISSION_TYPE_TRAINING)))
        AddString(mission_filenames[i]);
      else if ((Campaign.type == CAMPAIGN_TYPE_MULTI_COOP)
               && (a_mission.game_type & MISSION_TYPE_MULTI_COOP))
        AddString(mission_filenames[i]);
      else if ((Campaign.type == CAMPAIGN_TYPE_MULTI_TEAMS)
