...
bg_bitmap_dlg::bg_bitmap_dlg(CWnd * pParent):CDialog(bg_bitmap_dlg::IDD,
        pParent)
{
...
BEGIN_MESSAGE_MAP(bg_bitmap_dlg, CDialog)
  ...
  ON_WM_CLOSE()ON_CBN_SELCHANGE(IDC_NEBCOLOR, OnSelchangeNebcolor)
ON_CBN_SELCHANGE(IDC_NEBPATTERN, OnSelchangeNebpattern)
ON_BN_CLICKED(IDC_FULLNEB, OnFullNeb)
ON_WM_HSCROLL()ON_LBN_SELCHANGE(IDC_SUN1_LIST, OnSunChange)
     ...
     void bg_bitmap_dlg::create()
{
  ...
  for (i = 0; i < MAX_NEB2_BITMAPS; i++)
    {
      if (strlen(Neb2_bitmap_filenames[i]) > 0)
        {
          ((CComboBox *) GetDlgItem(IDC_NEB2_TEXTURE))->
            AddString(Neb2_bitmap_filenames[i]);
        }
    }
  ...
  if (strlen(Neb2_texture_name) > 0)
    {
      m_neb2_texture =
        ((CComboBox *) GetDlgItem(IDC_NEB2_TEXTURE))->SelectString(-1,
                                                                   Neb2_texture_name);
  ...
  for (i = 0; i < Num_storm_types; i++)
    {
      ((CComboBox *) GetDlgItem(IDC_NEB2_LIGHTNING))->
        AddString(CString(Storm_types[i].name));
    }
  ((CComboBox *) GetDlgItem(IDC_NEB2_LIGHTNING))->SelectString(-1,
...
void bg_bitmap_dlg::OnClose()
{
  ...
  // get selected storm
  ((CComboBox *) GetDlgItem(IDC_NEB2_LIGHTNING))->
    GetLBText(((CComboBox *) GetDlgItem(IDC_NEB2_LIGHTNING))->GetCurSel(),
              Mission_parse_storm_name);
...
void bg_bitmap_dlg::OnHScroll(UINT nSBCode, UINT nPos,
                              CScrollBar * pScrollBar)
{
  ...
  gr_set_ambient_light(m_amb_red.GetPos(), m_amb_green.GetPos(),
                       m_amb_blue.GetPos());
}
  ...
  if (s_index >= 0)
    {
      ...
      drop_index =
        ((CComboBox *) GetDlgItem(IDC_SUN1))->FindString(-1, sle->filename);
...
void bg_bitmap_dlg::OnAddSun()
{
  ...
  int add_index =
    ((CListBox *) GetDlgItem(IDC_SUN1_LIST))->AddString(sle.filename);
  ...
  if (new_index != CB_ERR)
    {
      ...
      ((CListBox *) GetDlgItem(IDC_SUN1_LIST))->DeleteString(s_index);
      ((CListBox *) GetDlgItem(IDC_SUN1_LIST))->InsertString(s_index,
                                                             (const char *)
                                                             s_name);
  ...
  if (b_index >= 0)
    {
      ...
      // select the proper item from the dropdown
      drop_index =
        ((CComboBox *) GetDlgItem(IDC_SBITMAP))->FindString(-1,
                                                            sle->filename);
      if (drop_index != CB_ERR)
...
void bg_bitmap_dlg::OnAddBitmap()
{
  ...
  // add to the listbox and select it
  int add_index =
    ((CListBox *) GetDlgItem(IDC_SBITMAP_LIST))->AddString(sle.filename);
  ...
  if (new_index != CB_ERR)
    {
      ...
      // change the name of the string in the listbox
      ((CListBox *) GetDlgItem(IDC_SBITMAP_LIST))->DeleteString(b_index);
      ((CListBox *) GetDlgItem(IDC_SBITMAP_LIST))->InsertString(b_index,
                                                                (const char *)
                                                                b_name);
...
void bg_bitmap_dlg::get_data_spinner(NM_UPDOWN * pUD, int id, int *var,
                                     int min, int max)
{
  if (pUD->iDelta > 0)
    {
...
{
  CFileDialog cfd(TRUE, ".fs2", NULL, 0,
  ...
  if (MessageBox
      ("This action will erase any nebulae and suns already placed.  Continue?",
       "Fred2", MB_ICONWARNING | MB_YESNO) == IDNO)
    return;
  ...
  if ((rval = setjmp(parse_abort)) != 0)
    {
      ...
      MessageBox((LPCTSTR) error_str,
                 (LPCTSTR) "Unable to import mission background!",
                 MB_ICONERROR | MB_OK);
      return;
    }
...
void bg_bitmap_dlg::OnSkyboxBrowse()
{
  ...
  CFileDialog dlg(TRUE, NULL, filename, OFN_FILEMUSTEXIST | OFN_NOCHANGEDIR,
                  Model_file_ext);
