...
int pcx_read_header(char *real_filename, CFILE * img_cfp, int *w, int *h,
                    int *bpp, ubyte * pal)
{
  PCXHeader header;
  CFILE *PCXfile;
...
int pcx_read_bitmap(char *real_filename, ubyte * org_data, ubyte * pal,
                    int byte_size, int aabitmap, int nondark, int cf_type)
{
  PCXHeader header;
  CFILE *PCXfile;
  int row, col, count, xsize, ysize;
                  ...
                  if ((byte_size == 2) && aabitmap)
                  ...
                  else
                    {
                      ...
                      if (byte_size == 2)
                        {
                          // stuff the color
                          bm_set_components((ubyte *) & bit_16, &r, &g, &b,
                                            &al);
...
int pcx_write_bitmap(char *real_filename, int w, int h, ubyte ** row_ptrs,
                     ubyte * palette)
{
  int retval;
  int i;
...
char pcx_error_messages[] = {
  "No error.\0"
    ...
    "Couldn't find palette information.\0" "Error writing data.\0"
};
