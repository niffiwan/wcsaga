...
void shield_add_strength(object * objp, float delta)
{
  ...
  if (!(Ai_info[Ships[objp->instance].ai_index].ai_profile_flags & AIPF_SMART_SHIELD_MANAGEMENT) || delta <= 0.0f)      //SUSHI: We don't want smart shield management for negative delta
    {
...
float scale_quad(float generator_fraction, float quad_strength)
{
  ...
  float effective_strength =
    quad_strength * ((float) log(generator_fraction) * (float) factor);
...
int shield_is_up(object * objp, int quadrant_num)
{
  if ((quadrant_num >= 0) && (quadrant_num < MAX_SHIELD_SECTIONS))
  ...
  else
    {
      ...
      if (strength >
          MAX(2.0f * MAX_SHIELD_SECTIONS,
              0.1f * shield_get_max_strength(objp)))
        return 1;
    }
