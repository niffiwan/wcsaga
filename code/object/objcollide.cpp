...
void obj_add_pair(object * A, object * B, int check_time, int add_to_end) 
{
  ...
  if (!(B->flags & OF_COLLIDES))
    ...
    
    {
        ...
        // weapon is B
        if (!
            (Weapon_info[Weapons[B->instance].weapon_info_index].
             wi_flags & WIF_TURNS))
    ...
    
    {
      ...
      int prev_free_mark = (pair_free_list.next - old_pairs_ptr);
        ...
        (obj_pair *) vm_realloc_q(Obj_pairs,
                                  sizeof(obj_pair) * (Num_pairs_allocated +
                                                      PAIRS_BUMP));
...
int collide_subdivide(vec3d * p0, vec3d * p1, float prad, vec3d * q0,
                      vec3d * q1, float qrad) 
{
                         ...
                         float radius_scale) 
{
    ...
    
    {
        ...
        
        {
            ...
            find_nearest_point_on_line(&nearest_point, &A->pos, &A_future.pos,
                                       &B->pos);
...
int vector_object_collision(vec3d * start_pos, vec3d * end_pos, object * objp,
                            float radius_scale) 
{
  float dist, r;
  vec3d nearest_point;
   r =
    find_nearest_point_on_line(&nearest_point, start_pos, end_pos,
                               &objp->pos);
...
int weapon_will_never_hit(object * obj_weapon, object * other,
                          obj_pair * current_pair) 
{
    ...
    
    {
        ...
        
        {
          ...
          vm_vec_sub(&velocity_rel_weapon, &obj_weapon->phys_info.vel,
                      &other->phys_info.vel);
          vdot =
            -vm_vec_dot(&velocity_rel_weapon, &obj_weapon->orient.vec.fvec);
    ...
    
    {
        ...
        
        {
          ...
          delta_x_dot_vl = vm_vec_dotprod(&delta_x, &laser_vel);
            ...
            max_vel_weapon * max_vel_weapon - max_vel_other * max_vel_other;
          b =
            2.0f * (delta_x_dot_vl - max_vel_other * max_vel_other * delta_t);
          c =
            vm_vec_mag_squared(&delta_x) -
        ...
        
        {
            ...
            dist =
            vm_vec_dist(&other->pos,
                        &obj_weapon->pos) - (other->radius + 10.0f);
...
int pp_collide(vec3d * curpos, vec3d * goalpos, object * goalobjp,
               float radius) 
{
