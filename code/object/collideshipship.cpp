...
void calculate_ship_ship_collision_physics(collision_info_struct *
                                           ship_ship_hit_info);
...
bool model_precise_collide(mc_info * mc_info, polymodel * pm_light,
                           object * heavy, object * light, float radius1,
                           float radius2)
{
                              ...
                              vec3d * hitpos)
{
  ...
  if ((vm_vec_mag_squared(&heavy_obj->phys_info.max_rotvel) * .04) <
  ...
  else
    {
      ...
      vm_vec_sub(&ship_ship_hit_info->light_rel_vel,
                 &light_obj->phys_info.vel, &heavy_obj->phys_info.vel);
    }
  ...
  polymodel *pm_light =
    model_get(Ship_info[light_shipp->ship_info_index].model_num);
  float rx =
    fl_abs(pm_light->submodel[pm_light->detail[0]].max.xyz.x -
           pm_light->submodel[pm_light->detail[0]].min.xyz.x);
  float ry =
    fl_abs(pm_light->submodel[pm_light->detail[0]].max.xyz.y -
           pm_light->submodel[pm_light->detail[0]].min.xyz.y);
  float rz =
    fl_abs(pm_light->submodel[pm_light->detail[0]].max.xyz.z -
           pm_light->submodel[pm_light->detail[0]].min.xyz.z);
          ...
          for (i = 0; i < num_rotating_submodels; i++)
            {
              ...
              if (model_precise_collide
                  (&mc, pm_light, heavy_obj, light_obj, radius1, radius2))
                {
                  if (mc.hit_dist < ship_ship_hit_info->hit_time)
                    {
                      ...
                      if (ship_ship_hit_info->edge_hit == 0)
                        {
                          model_find_obj_dir(&ship_ship_hit_info->
                                             collision_normal, &mc.hit_normal,
                                             heavy_obj, mc.hit_submodel);
                        }
                      ...
                      vm_vec_scale_add(&int_light_pos, mc.p0, &diff,
                                       mc.hit_dist);
                      model_find_world_point(&ship_ship_hit_info->
                                             light_collision_cm_pos,
                                             &int_light_pos, mc.model_num,
                                             mc.hit_submodel,
                                             &heavy_obj->orient, &zero);
                    }
                }
      ...
      if ((collide_obj != NULL)
              ...
              flags & (SIF_FIGHTER | SIF_BOMBER)))
        {
          ...
          nprintf(("AI",
                   ...
                   Ships[collide_obj->instance].ship_name,
                   Mode_text[aip->mode], submode_string));
        }
#endif
      ...
      if (heavy_shipp->team == light_shipp->team)
        {
          ...
          if ((Ship_info[heavy_shipp->ship_info_index].flags &
               SIF_SMALL_SHIP)
              && (Ship_info[light_shipp->ship_info_index].
          ...
          else
            {
              ...
              vm_vec_scale_add2(&heavy_obj->phys_info.vel,
                                &ship_ship_hit_info->light_rel_vel,
                                SEP_VEL * light_obj->phys_info.mass /
                                (mass_sum * rel_vel));
              vm_vec_rotate(&heavy_obj->phys_info.prev_ramp_vel,
                            &heavy_obj->phys_info.vel, &heavy_obj->orient);
              vm_vec_scale_add2(&light_obj->phys_info.vel,
                                &ship_ship_hit_info->light_rel_vel,
                                -SEP_VEL * heavy_obj->phys_info.mass /
                                (mass_sum * rel_vel));
              vm_vec_rotate(&light_obj->phys_info.prev_ramp_vel,
...
int check_special_cruiser_asteroid_collision(object * heavy, object * light,
                                             float *cruiser_mass,
                                             int *cruiser_light)
{
#ifndef FS2_DEMO
  ...
  if (heavy->type == OBJ_ASTEROID)
    {
      ...
      if (Ship_info[Ships[light->instance].ship_info_index].
                                           ...
                                           ship_ship_hit_info)
{
  ...
  special_cruiser_asteroid_collision =
    check_special_cruiser_asteroid_collision(heavy, light, &cruiser_mass,
                                             &cruiser_light);
  ...
  if (ship_ship_hit_info->submodel_rot_hit == 1)
    {
      ...
      polymodel *pm =
        model_get(Ship_info[Ships[heavy->instance].ship_info_index].
                  model_num);
      ...
      if (!pm->submodel[ship_ship_hit_info->submodel_num].sii->axis_set)
        {
          model_init_submodel_axis_pt(pm->
                                      submodel[ship_ship_hit_info->
                                               submodel_num].sii, pm->id,
      ...
      vec3d temp;
      vm_vec_unrotate(&temp,
                      &pm->submodel[ship_ship_hit_info->submodel_num].sii->
                      pt_on_axis, &heavy->orient);
  ...
  parallel_mag =
    float (-COLLISION_FRICTION_FACTOR) * collision_speed_parallel /
    vm_vec_mag(&v_rel_m);
  ...
  if (heavy == Player_obj)
    ...
    {
      vm_vec_crossprod(&rotational_impulse_heavy,
                       &ship_ship_hit_info->r_heavy, &impulse);
      ...
      vm_vec_crossprod(&delta_vel_from_delta_rotvel_heavy,
                       &delta_rotvel_heavy, &ship_ship_hit_info->r_heavy);
      heavy_denom =
        vm_vec_dotprod(&delta_vel_from_delta_rotvel_heavy,
                       &ship_ship_hit_info->collision_normal);
      if (heavy_denom < 0)
        {
          // sanity check
  ...
  if (light == Player_obj)
    ...
    {
      vm_vec_crossprod(&rotational_impulse_light,
                       &ship_ship_hit_info->r_light, &impulse);
      ...
      vm_vec_crossprod(&delta_vel_from_delta_rotvel_light,
                       &delta_rotvel_light, &ship_ship_hit_info->r_light);
      light_denom =
        vm_vec_dotprod(&delta_vel_from_delta_rotvel_light,
                       &ship_ship_hit_info->collision_normal);
      if (light_denom < 0)
        {
          // sanity check
  ...
  impulse_mag =
    -(1.0f + coeff_restitution) * v_rel_normal_m / (heavy_denom +
                                                    light_denom);
...
void mcp_1(object * player_objp, object * planet_objp)
{
  ...
  ship_apply_global_damage(player_objp, planet_objp, NULL,
                           ...
                           (float) pow((planet_radius * PLANET_DAMAGE_RANGE) /
                                       dist, 3.0f));
...
void collide_ship_ship_do_sound(vec3d * world_hit_pos, object * A, object * B,
                                int player_involved)
{
  vec3d rel_vel;
  float rel_speed;
      ...
      if (player_involved)
        {
          if (!snd_is_playing(Player_collide_sound))
            {
              Player_collide_sound =
                snd_play_3d(&Snds[SND_SHIP_SHIP_LIGHT], world_hit_pos,
                            &View_position);
            }
        }
      else
      ...
      if (player_involved)
        {
          if (!snd_is_playing(Player_collide_sound))
            {
              Player_collide_shield_sound =
                snd_play_3d(&Snds[SND_SHIP_SHIP_SHIELD], world_hit_pos,
                            &View_position);
            }
        }
      else
  ...
  if (ship1->team != ship2->team)
    {
      if (aip1->ai_flags & AIF_KAMIKAZE)
        {
          if (Ship_info[ship2->ship_info_index].
              flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP))
            {
      ...
      if (aip2->ai_flags & AIF_KAMIKAZE)
        {
          if (Ship_info[ship1->ship_info_index].
              flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP))
            {
                                               ...
                                               float impulse, vec3d * normal)
{
  ...
  if (big_class > -1
      && Ship_types[big_class].ship_bools & STI_SHIP_WARP_PUSHES)
    {
      if (small_class > -1
          && Ship_types[small_class].ship_bools & STI_SHIP_WARP_PUSHABLE)
        {
          ...
          if (big_speed > 3 * big_obj->phys_info.max_vel.xyz.z)
            {
              ...
              vm_vec_scale_add(&perp, &temp, &big_obj->orient.vec.fvec,
                               -vm_vec_dotprod(&temp,
                                               &big_obj->orient.vec.fvec));
              ...
              vm_vec_scale_add2(&small_obj->phys_info.vel, &perp,
                                added_perp_vel_mag);
              vm_vec_rotate(&small_obj->phys_info.prev_ramp_vel,
...
int collide_ship_ship(obj_pair * pair)
{
  ...
  bool check_cap_vs_cap =
    ((The_mission.ai_profile->flags2 & AIPF2_CAP_VS_CAP_COLLISIONS) != 0);
  ...
  if (dist < A->radius + B->radius)
    {
      ...
      if (hit)
        {
          Script_system.SetHookObjects(4, "Ship", A, "ShipB", B, "Self", A,
                                       "Object", B);
          bool a_override =
            Script_system.IsConditionOverride(CHA_COLLIDESHIP, A);

          //Yes this should be reversed.
          Script_system.SetHookObjects(4, "Ship", B, "ShipB", A, "Self", B,
                                       "Object", A);
          bool b_override =
          ...
          if (!a_override && !b_override)
            {
              ...
              collide_ship_ship_do_sound(&world_hit_pos, A, B,
                                         player_involved);
              ...
              if (player_involved && (ship_ship_hit_info.impulse > 0))
                {
                  ...
                  scaler =
                    -ship_ship_hit_info.impulse / Player_obj->phys_info.mass *
                    300;
                  ...
                  if (Ships[A->instance].team == Ships[B->instance].team)
                    {
                      ...
                      if (vm_vec_dot(&collision_vec, &A->orient.vec.fvec) >
                        ...
                        {
                          vm_vec_cross(&right_angle_vec, &A->orient.vec.uvec,
                                       &collision_vec);
                        }

                      vm_vec_scale_add2(&A->phys_info.vel, &right_angle_vec,
                                        +2.0f);
                      vm_vec_scale_add2(&B->phys_info.vel, &right_angle_vec,
                                        -2.0f);
              ...
              if ((LightOne->flags & OF_PLAYER_SHIP)
                ...
                {
                  damage *=
                    (float) (Game_skill_level * Game_skill_level +
                             1) / (NUM_SKILL_LEVELS + 1);
                }
              ...
              int quadrant_num =
                get_ship_quadrant_from_global(&world_hit_pos,
              ...
              if ((ship_ship_hit_info.heavy->flags & OF_NO_SHIELDS)
                  || !ship_is_shield_up(ship_ship_hit_info.heavy,
                                        quadrant_num))
                {
                  quadrant_num = -1;
                }
              ...
              ship_apply_local_damage(ship_ship_hit_info.heavy,
                                      ...
                                      &world_hit_pos,
                                      100.0f * damage /
                                      HeavyOne->phys_info.mass, quadrant_num,
                                      CREATE_SPARKS,
                                      ship_ship_hit_info.submodel_num,
                                      &ship_ship_hit_info.collision_normal);
              ...
              ship_apply_local_damage(ship_ship_hit_info.light,
                                      ship_ship_hit_info.heavy,
                                      &world_hit_pos, dam2, MISS_SHIELDS,
                                      NO_SPARKS, -1,
                                      &ship_ship_hit_info.collision_normal);
              hud_shield_quadrant_hit(ship_ship_hit_info.light, quadrant_num);

              maybe_push_little_ship_from_fast_big_ship(ship_ship_hit_info.
                                                        heavy,
                                                        ship_ship_hit_info.
                                                        light,
                                                        ship_ship_hit_info.
          ...
          if (!(b_override && !a_override))
            {
              Script_system.SetHookObjects(4, "Ship", A, "ShipB", B, "Self",
                                           A, "Object", B);
          ...
          if ((b_override && !a_override) || (!b_override && !a_override))
            {
              //Yes this should be reversed.
              Script_system.SetHookObjects(4, "Ship", B, "ShipB", A, "Self",
                                           B, "Object", A);
  ...
  else
    {
      ...
      if (ship_is_beginning_warpout_speedup(A))
        {
          shipA_max_speed =
            MAX(ship_get_max_speed(&Ships[A->instance]),
      ...
      if (ship_is_beginning_warpout_speedup(B))
        {
          shipB_max_speed =
            MAX(ship_get_max_speed(&Ships[B->instance]),
      ...
      time =
        1000.0f * (dist - A->radius - B->radius) / (shipA_max_speed +
                                                    shipB_max_speed);
                                    ...
                                    ship_ship_hit_info)
{
  ...
  float core_rad =
    model_get_core_radius(Ship_info[Ships[light->instance].ship_info_index].
                          model_num);
  ...
  float q =
    vm_vec_dist(heavy_collide_cm_pos,
                light_collide_cm_pos) / (heavy->radius + core_rad);
  if (q > 1.0f)
    {
  ...
  *r_heavy = ship_ship_hit_info->hit_pos;
  ...
  nprintf(("Physics",
           ...
           heavy->phys_info.vel.x, heavy->phys_info.vel.y,
           heavy->phys_info.vel.z));
  ...
  nprintf(("Physics",
           ...
           light->phys_info.vel.x, light->phys_info.vel.y,
           light->phys_info.vel.z));
#endif
