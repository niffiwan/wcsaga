    ...
    
    {
        ...
        &Weapon_info[Weapons[weapon_obj->instance].weapon_info_index];
                      ...
                      wip->decal_burn_time);
                                   ...
                                   float time_limit = 0.0f, int *next_hit =
                                   NULL) 
{
  ...
  int shield_collision =
    ...
    
    {
        ...
        
        {
            ...
            // do the hit effect
            if (shield_collision)
            add_shield_point(OBJ_INDEX(ship_objp), mc_shield.shield_hit_tri,
                              &mc_shield.hit_point);
    ...
    
    {
        ...
        
        {
          ...
           vm_vec_normalized_dir(&vec_to_ship, &ship_objp->pos,
                                   &weapon_objp->pos);
...
int collide_ship_weapon(obj_pair * pair) 
{
    ...
    // If it does not hit and is within error tolerance, cull the pair.
         ...
         && (Weapon_info[Weapons[weapon->instance].weapon_info_index].
             subtype == WP_LASER))
  ...
  float time_to_max_error, time_to_exit_sphere;
    ...
    (ship->radius +
     vm_vec_dist(&ship->pos,
                 &weapon->pos)) / (weapon->phys_info.max_vel.xyz.z -
                                   ship_speed_at_exit_sphere);
  vm_vec_scale_add(&error_vel, &ship->phys_info.vel,
