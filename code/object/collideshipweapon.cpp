...
void update_danger_weapon(object * ship_obj, object * weapon_obj)
{
  ...
  if (aip->danger_weapon_objnum == -1)
           ...
           Objects[aip->danger_weapon_objnum].signature)
    {
      ...
      danger_old_time =
        ai_endangered_time(ship_obj, &Objects[aip->danger_weapon_objnum]);
...
void ship_weapon_do_hit_stuff(object * ship_obj, object * weapon_obj,
                              vec3d * world_hitpos, vec3d * hitpos,
  ...
  if ((MULTIPLAYER_MASTER) && !(shipp->flags & SF_DYING))
    {
      ...
      if ((np_index >= 0) && (np_index != MY_NET_PLAYER_NUM)
        ...
        {
          send_player_pain_packet(&Net_players[np_index],
                                  wp->weapon_info_index,
                                  wip->damage * weapon_get_damage_scale(wip,
                                                                        weapon_obj,
                                                                        ship_obj),
  ...
  if ((quadrant_num == -1) && Cmdline_decals)
    {
      ...
      if ((dec.radius > 0) && (wip->decal_texture.bitmap_id > -1))
        decal_create(ship_obj, &dec, submodel_num,
                     wip->decal_texture.bitmap_id,
                     wip->decal_backface_texture.bitmap_id,
                     wip->decal_glow_texture_id, wip->decal_burn_texture_id,
                     wip->decal_burn_time);
    }
...
int ship_weapon_check_collision(object * ship_objp, object * weapon_objp,
                                float time_limit = 0.0f, int *next_hit = NULL)
{
  ...
  vm_vec_scale_add(&weapon_end_pos, &weapon_objp->pos,
                   &weapon_objp->phys_info.vel, time_limit);
  ...
  if (!(ship_objp->flags & OF_NO_SHIELDS))
    {
      ...
      if (hull_collision && !shield_collision && (pm->shield.ntris > 0)
          && !(wip->wi_flags2 & WIF2_PIERCE_SHIELDS))
        {
          ...
          vm_vec_scale_add(mc_ray_shield.p0, mc_ray_shield.p1,
                           &weapon_objp->phys_info.vel, -pm->rad * 10.0f);

          int redundant_shield_collision = model_collide(&mc_ray_shield);

          float dist_to_shield =
            vm_vec_dist_squared(&mc_ray_shield.hit_point_world,
                                &mc_hull.hit_point_world);
          float dist_to_start =
            vm_vec_dist_squared(&wp->start_pos, &mc_hull.hit_point_world);

          if (redundant_shield_collision && (dist_to_shield < dist_to_start))
            {
              int redundant_quadrant_num =
                get_quadrant(&mc_ray_shield.hit_point);
              ...
              if (ship_is_shield_up(ship_objp, redundant_quadrant_num))
                {
                  ...
                  float hit_dist =
                    vm_vec_dist(mc_shield.p0, &mc_ray_shield.hit_point_world);
      ...
      if (!shield_collision && hull_collision
  ...
  if (next_hit)
    {
      ...
      *next_hit =
        (int) (1000.0f *
               (mc.hit_dist * (flFrametime + time_limit) - flFrametime));
  ...
  if (valid_hit_occurred)
    {
      Script_system.SetHookObjects(4, "Ship", ship_objp, "Weapon",
                                   weapon_objp, "Self", ship_objp, "Object",
                                   weapon_objp);
      bool ship_override =
        Script_system.IsConditionOverride(CHA_COLLIDEWEAPON, ship_objp);

      Script_system.SetHookObjects(2, "Self", weapon_objp, "Object",
                                   ship_objp);
      ...
      if (!ship_override && !weapon_override)
        {
          ship_weapon_do_hit_stuff(ship_objp, weapon_objp,
                                   &mc.hit_point_world, &mc.hit_point,
                                   quadrant_num, mc.hit_submodel,
                                   mc.hit_normal);
        }

      Script_system.SetHookObjects(2, "Self", ship_objp, "Object",
                                   weapon_objp);
...
int collide_ship_weapon(obj_pair * pair)
{
  ...
  if ((Ship_info[Ships[ship->instance].ship_info_index].
...
float estimate_ship_speed_upper_limit(object * ship, float time)
{
  ...
  delta_v =
    Ship_info[Ships[ship->instance].ship_info_index].max_vel.xyz.z -
                                      ...
                                      obj_pair * pair)
{
  ...
  time_to_exit_sphere =
    (ship->radius +
     vm_vec_dist(&ship->pos,
                 &weapon->pos)) / (weapon->phys_info.max_vel.xyz.z -
  ...
  time_to_exit_sphere =
    (ship->radius +
     vm_vec_dist(&ship->pos,
                 &weapon->pos)) / (weapon->phys_info.max_vel.xyz.z -
                                   ship_speed_at_exit_sphere);
  vm_vec_scale_add(&error_vel, &ship->phys_info.vel, &weapon->orient.vec.fvec,
                   -vm_vec_dotprod(&ship->phys_info.vel,
                                   &weapon->orient.vec.fvec));
  error_vel_mag = vm_vec_mag_quick(&error_vel);
  error_vel_mag +=
    0.5f * (ship->phys_info.max_vel.xyz.z -
            error_vel_mag) * (time_to_exit_sphere /
