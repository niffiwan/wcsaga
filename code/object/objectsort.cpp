...
bool obj_in_view_cone(Frustum * frustum, object * objp)
{
  ...
  return !behindUserPlane
    && frustum->sphereInFrustum(&objp->pos, objp->radius);
}
                    ...
                    bool * draw_viewer_last)
{
  ...
  Frustum frustum(Proj_fov, gr_screen.clip_aspect, Min_draw_distance,
                  Max_draw_distance, &Eye_position, &Eye_matrix);
  ...
  if (!Cmdline_nohtl)
    {
      ...
      gr_set_proj_matrix(Proj_fov, gr_screen.clip_aspect, closest_obj,
                         farthest_obj);
  ...
  for (i = 0; i < Num_sorted_objects; i++)
    {
      ...
      if (os->obj == Viewer_obj
          ...
          && (Ship_info[Ships[os->obj->instance].ship_info_index].
              flags2 & SIF2_SHOW_SHIP_MODEL))
        {
