...
int collide_debris_weapon(obj_pair * pair)
{
  ...
  hit =
    fvi_segment_sphere(&hitpos, &weapon->last_pos, &weapon->pos,
  ...
  if (hit)
    {
      ...
      Script_system.SetHookObjects(4, "Weapon", weapon, "Debris", pdebris,
                                   "Self", weapon, "Object", pdebris);
      bool weapon_override =
        Script_system.IsConditionOverride(CHA_COLLIDEDEBRIS, weapon);
      ...
      if (!weapon_override && !debris_override)
        {
          ...
          debris_hit(pdebris, weapon, &hitpos,
  ...
  if (hit)
    {
      ...
      Script_system.SetHookObjects(4, "Weapon", weapon, "Asteroid", pasteroid,
                                   "Self", weapon, "Object", pasteroid);

      bool weapon_override =
        Script_system.IsConditionOverride(CHA_COLLIDEASTEROID, weapon);
      ...
      if (!weapon_override && !asteroid_override)
        {
          ...
          asteroid_hit(pasteroid, weapon, &hitpos,
                       Weapon_info[Weapons[weapon->instance].
                                   weapon_info_index].damage);
        }
